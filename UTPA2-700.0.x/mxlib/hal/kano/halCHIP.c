...
static MS_U32 _CHIP_LISR0(MS_U32 u32VectorNum, MS_U32 u32Data)
{
    ...
    while(32 != (u32Bit = MAsm_CPU_GetTrailOne(u32Reg)))
    {
        ...
        {
            eVector = (IRQFIQNum)((u32Bit-16) + E_IRQH_START);
        }
    ...
    while(32 != (u32Bit = MAsm_CPU_GetTrailOne(u32Reg)))
    {
        ...
        {
            eVector = (IRQFIQNum)((u32Bit-16) + E_IRQEXPH_START);
        }
    ...
    u32Reg = 0;
    u32Reg = IRQHYP_REG(REG_IRQHYP_PENDING_H);
    u32Reg <<= 16;
    u32Reg |= IRQHYP_REG(REG_IRQHYP_PENDING_L);

    while(32 != (u32Bit = MAsm_CPU_GetTrailOne(u32Reg)))
    {
        if(u32Bit < 16)
        {
            eVector = (IRQFIQNum)(u32Bit + E_IRQHYPL_START);
        }
        else
        {
            eVector = (IRQFIQNum)((u32Bit-16) + E_IRQHYPH_START);
        }
...
static MS_U32 _CHIP_LISR1(MS_U32 u32VectorNum, MS_U32 u32Data)
{
    ...
    while(32 != (u32Bit = MAsm_CPU_GetTrailOne(u32Reg)))
    {
        ...
        {
            eVector = (IRQFIQNum)((u32Bit-16) + E_FIQH_START);
        }
        eIntNum = (InterruptNum) HWIdx2IntEnum[eVector];
        CHIP_DisableIRQ(eIntNum);
        if(_HISR_Info[eVector].bUsed)
    ...
    while(32 != (u32Bit = MAsm_CPU_GetTrailOne(u32Reg)))
    {
        ...
        {
            eVector = (IRQFIQNum)((u32Bit-16) + E_FIQEXPH_START);
        }
        eIntNum = (InterruptNum) HWIdx2IntEnum[eVector];
        CHIP_DisableIRQ(eIntNum);
        if(_HISR_Info[eVector].bUsed)
    ...
    u32Reg = 0;
    u32Reg = IRQHYP_REG(REG_FIQHYP_PENDING_H);
    u32Reg <<=16;
    u32Reg |= IRQHYP_REG(REG_FIQHYP_PENDING_L);

    while(32 != (u32Bit = MAsm_CPU_GetTrailOne(u32Reg)))
    {
        if(u32Bit<16)
        {
            eVector = (IRQFIQNum)(u32Bit + E_FIQHYPL_START);
        }
        else
        {
            eVector = (IRQFIQNum)((u32Bit-16) + E_FIQHYPH_START);
        }
        eIntNum = (InterruptNum) HWIdx2IntEnum[eVector];
        CHIP_DisableIRQ(eIntNum);
        if(_HISR_Info[eVector].bUsed)
...
static void _CHIP_HISR0(MS_U32 u32VectorNum, MS_U32 u32Count, MS_U32 u32Data)
{
    ...
    for(i = (InterruptNum) E_IRQHYPL_START; i <= (InterruptNum) E_IRQHYPL_END; i++)
    {
        if(_HISR_Info[i].bPending)
        {
            _HISR_Info[i].bPending = FALSE;
    ...
    for(i = (InterruptNum) E_IRQHYPH_START; i <= (InterruptNum) E_IRQHYPH_END; i++)
    {
        if(_HISR_Info[i].bPending)
        {
            _HISR_Info[i].bPending = FALSE;
...
static void _CHIP_HISR1(MS_U32 u32VectorNum, MS_U32 u32Count, MS_U32 u32Data)
{
    ...
    for(i = (InterruptNum) E_FIQHYPL_START; i <= (InterruptNum) E_FIQHYPL_END; i++)
    {
        if(_HISR_Info[i].bPending)
        {
            _HISR_Info[i].bPending = FALSE;
    ...
    for(i = (InterruptNum) E_FIQHYPH_START; i<= (InterruptNum) E_FIQHYPH_END; i++)
    {
        if(_HISR_Info[i].bPending)
        {
            _HISR_Info[i].bPending = FALSE;
...
MS_BOOL CHIP_EnableIRQ(InterruptNum eIntNum)
{
    ...
    if(_HISR_Info[u8VectorIndex].bUsed)
    {
        if (u8VectorIndex == E_IRQ_FIQ_ALL)
        {
            ...
            IRQHYP_REG(REG_IRQHYP_MASK_L) &= ~0xFFFF;
            IRQHYP_REG(REG_IRQHYP_MASK_H) &= ~0xFFFF;
            IRQHYP_REG(REG_FIQHYP_MASK_L) &= ~0xFFFF;
            IRQHYP_REG(REG_FIQHYP_MASK_H) &= ~0xFFFF;
        }
        ...
        else if((u8VectorIndex >= (MS_U8) E_IRQHYPL_START) && (u8VectorIndex <= (MS_U8) E_IRQHYPL_END))
        {
            IRQHYP_REG(REG_IRQHYP_MASK_L) &= ~(0x01 << (u8VectorIndex - E_IRQHYPL_START));
        }
        else if((u8VectorIndex >= (MS_U8) E_IRQHYPH_START) && (u8VectorIndex <= (MS_U8) E_IRQHYPH_END))
        {
            IRQHYP_REG(REG_IRQHYP_MASK_H) &= ~(0x01 << (u8VectorIndex - E_IRQHYPH_START));
        }
        ...
        else if((u8VectorIndex >= (MS_U8) E_FIQHYPL_START) && (u8VectorIndex <= (MS_U8) E_FIQHYPL_END))
        {
            IRQHYP_REG(REG_FIQHYP_MASK_L) &= ~(0x01 << (u8VectorIndex - E_FIQHYPL_START));
        }
        else if((u8VectorIndex >= (MS_U8) E_FIQHYPH_START) && (u8VectorIndex <= (MS_U8) E_FIQHYPH_END))
        {
            IRQHYP_REG(REG_FIQHYP_MASK_H) &= ~(0x01 << (u8VectorIndex - E_FIQHYPH_START));
        }
    }
    else
    {
...
MS_BOOL CHIP_DisableIRQ(InterruptNum eIntNum)
{
    ...
    {
        IRQHYP_REG(REG_FIQHYP_MASK_L) |= (0x01 << (u8VectorIndex - E_FIQHYPL_START));
        IRQHYP_REG(REG_FIQHYP_CLEAR_L) = (0x01 << (u8VectorIndex - E_FIQHYPL_START));
    }
    else if((u8VectorIndex >= (MS_U8) E_FIQHYPH_START) && (u8VectorIndex <= (MS_U8) E_FIQHYPH_END))
    {
        IRQHYP_REG(REG_FIQHYP_MASK_H) |= (0x01 << (u8VectorIndex - E_FIQHYPH_START));
        IRQHYP_REG(REG_FIQHYP_CLEAR_H) = (0x01 << (u8VectorIndex - E_FIQHYPH_START));
    }
    return TRUE;
}
...
//
// CHIP_DetectIRQSource will be called by hal_IRQ_handler() of eCospro
//
InterruptNum CHIP_DetectIRQSource(void)
{
    MS_U32              u32Reg;
...
static void *interrupt_thread(void *arg)
{
    ...
    for(;;)
    {
        ...
        if (err == -1)
        {
            // errno use from standard c library
...
MS_BOOL CHIP_AttachISR(InterruptNum eIntNum, InterruptCb pIntCb)
{
    ...
    if(0 != pthread_create(&_HISR_Info[u8VectorIndex].ithr, &attr, interrupt_thread, idp))
    {
        printf("Create interrupt thread fail!!!\n");
        close(fd);
        free(idp);
        return FALSE;
    }
...
//
// _CHIP_DetectIRQSource will be called by _CHIP_HISR0
//
InterruptNum _CHIP_DetectIRQSource(void)
{
    MS_U32              u32Reg;
...
void __CHIP_HISR1(MS_U32 u32VectorNum, MS_U32 u32Data)
{
    MS_U32          u32Reg;
    MS_U32          u32Bit;
    IRQFIQNum       eVector;
    ...
    while(32 != (u32Bit = MAsm_CPU_GetTrailOne(u32Reg)))
    {
        ...
        if(_HISR_Info[eVector].bUsed)
        {
            _HISR_Info[eVector].bPending = FALSE;
            _HISR_Info[eVector].pIntCb((InterruptNum)eIntNum);
        }
        u32Reg &= ~(0x01 << u32Bit);
    }
...
// @return None
//-------------------------------------------------------------------------------------------------
void _CHIP_HISR0(MS_U32 u32VectorNum, MS_U32 u32Data)
{
    MS_U32          u32Reg;
    MS_U32          u32Bit;
    IRQFIQNum       eVector;
    ...
    OS_ENTER_CRITICAL();
    if (_CHIP_DetectIRQSource()==E_INTERRUPT_IRQ)
    {
        _bInHISR = TRUE;    //in interrupt context
    ...
    else if (_CHIP_DetectIRQSource()==E_INTERRUPT_FIQ)
    {
        __CHIP_HISR1(u32VectorNum,u32Data);
    }
    OS_EXIT_CRITICAL();
}

void _CHIP_HISR1(MS_U32 u32VectorNum, MS_U32 u32Data)
{
    //in interrupt context
#if OS_CRITICAL_METHOD == 3                      /* Allocate storage for CPU status register           */
    OS_CPU_SR  cpu_sr = 0;
...
#endif // #if defined (MSOS_TYPE_UCOS)
