...
//#define MBXINT_DEBUG
#ifdef MBXINT_DEBUG
    #define MBXINT_ERROR(fmt, args...)           ULOGE(TAG_MBX, "[MBX INT Driver USER ERR][%06d]     " fmt, __LINE__, ## args)
    #define MBXINT_WARN(fmt, args...)            ULOGW(TAG_MBX, "[MBX INT Driver WARN][%06d]    " fmt, __LINE__, ## args)
    #define MBXINT_PRINT(fmt, args...)           ULOGD(TAG_MBX, "[MBX INT Driver][%06d]     " fmt, __LINE__, ## args)
    ...
    switch(srcCPUID)
    {
        ...
        case E_MBX_CPU_MIPS:
        {
            if(dstCPUID==E_MBX_CPU_PM)
            {
                ...
                CPU_INT_REG(REG_INT_H1FIRE) &= ~(INT_H1_PM);
                MBXINT_PRINT("FIQ [48-63] Mask = 0x%x\n", FIQ_REG(REG_FIQ_H1_48_63));
                MBXINT_PRINT("FIQ [48-63] Status = 0x%x\n", FIQ_REG(REG_FIQS_H1_48_63));
            }
            else
            {
                //ARM Core 0-to-R2
