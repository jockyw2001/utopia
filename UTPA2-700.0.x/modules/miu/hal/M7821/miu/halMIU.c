...
//  Local Structures
//-------------------------------------------------------------------------------------------------
//[MIU][HAL][006] Check data structure of MIU client table [START]
...
//[MIU][HAL][007] Check kernel protect table [START]
...
//[MIU][HAL][007] Check kernel protect table [END]
//-------------------------------------------------------------------------------------------------
//  Global Variables
//-------------------------------------------------------------------------------------------------
...
MS_U32 IDs[MIU_MAX_DEVICE][MIU_MAX_PROTECT_ID] = {{0}, {0}, {0}}; //IDs for protection
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
    ...
    if(E_MIU_2 == (MIU_ID)MiuID)
    {
        ...
        HAL_MIU_WriteByte(u32RegAddr, MIU_PROTECT_DDR_1024MB);
            ...
            case E_MIU_DDR_4096MB:
                HAL_MIU_WriteByte(u32RegAddr, MIU_PROTECT_DDR_4096MB);
                break;
            case E_MIU_DDR_8192MB:
                HAL_MIU_WriteByte(u32RegAddr, MIU_PROTECT_DDR_8192MB);
                break;
            default:
                return false;
        }
    ...
    switch (DramSize)
    {
        ...
        case MIU_PROTECT_DDR_1024MB:
            *pDramSize = E_MIU_DDR_1024MB;
            break;
        case MIU_PROTECT_DDR_2048MB:
            *pDramSize = E_MIU_DDR_2048MB;
            break;
        case MIU_PROTECT_DDR_4096MB:
            *pDramSize = E_MIU_DDR_4096MB;
            break;
        case MIU_PROTECT_DDR_8192MB:
                         ...
                         )
{
    ...
    _phy_to_miu_offset(u8MiuSel, phy64EndOffset, phy64End - 1) // minus 1 to avoid end address boundary issue
    _phy_to_miu_offset(u8MiuSel, phy64StartOffset, phy64Start)

    phy64Start = phy64StartOffset;
    phy64End = phy64EndOffset + 1; // plus 1 back to get correct end offset
...
void HAL_MIU_MaskReq(MS_U8 u8Miu, eMIUClientID eClientID)
{
    ...
    case 2:
    case 3:
        u32Reg = REG_MIU_RQX_MASK (u32Grp);
        u32RegBase = (u8Miu == E_MIU_0 ? MIU_REG_BASE : (u8Miu == E_MIU_1 ? MIU1_REG_BASE : MIU2_REG_BASE));
        break;
    case 4:
    case 5:
        u32Reg = REG_MIU_ARB_RQX_MASK (u32Grp);
        u32RegBase = (u8Miu == E_MIU_0 ? MIU_ARB_REG_BASE : (u8Miu == E_MIU_1 ? MIU1_ARB_REG_BASE : MIU2_ARB_REG_BASE));
        break;
    case 6:
    case 7:
        u32Reg = REG_MIU_ARBB_RQX_MASK (u32Grp);
        u32RegBase = (u8Miu == E_MIU_0 ? MIU_ARBB_REG_BASE : (u8Miu == E_MIU_1 ? MIU1_ARBB_REG_BASE : MIU2_ARBB_REG_BASE));
        break;
    default:
        return;
    }
...
MS_BOOL HAL_MIU_SelMIU(eMIU_SelType eType, eMIUClientID eClientID)
{
    ...
    //
    // 1. Get ID Position in Client Table
    //
    switch (eType)
    {
    ...
    //
    // 2. Set the base address of MIU Select
    //
    u32IdGroup = MIU_GET_CLIENT_GROUP(sVal);
    switch (u32IdGroup)
    {
    case 0:
    case 1:
    ...
    case 4:
    case 5:
        u32Reg0 = MIU_REG_BASE + REG_MIU_SELX(u32IdGroup);
        u32Reg1 = MIU1_REG_BASE + REG_MIU_SELX(u32IdGroup);
        u32Reg2 = MIU2_REG_BASE + REG_MIU_SELX(u32IdGroup);
        break;
    case 6:
        u32Reg0 = MIU_ARBB_REG_BASE + REG_MIU_SELX(0);
        u32Reg1 = MIU1_ARBB_REG_BASE + REG_MIU_SELX(0);
        u32Reg2 = MIU2_ARBB_REG_BASE + REG_MIU_SELX(0);
        break;
    default:
        break;
    }
