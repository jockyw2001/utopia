...
//-------------------------------------------------------------------------------------------------
#define TAG_MIU "MIU"
#define MIU_HAL_ERR(x, args...)     {ULOGE(TAG_MIU, x, ##args);}
...
//  Local Variables
//-------------------------------------------------------------------------------------------------
static MS_VIRT _gMIU_MapBase = 0x1F200000;      //default set to MIPS platfrom
static MS_VIRT _gPM_MapBase = 0x1F000000;       //default set to MIPS platfrom
...
void HAL_MIU_SetIOMapBase(MS_VIRT virtBase)
{
    ...
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "MIU _gMIU_MapBase= %lx\n", _gMIU_MapBase));
}

void HAL_MIU_SetPMIOMapBase(MS_VIRT virtBase)
{
    _gPM_MapBase = virtBase;
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "MIU _gPM_MapBase= %lx\n", _gPM_MapBase));
}
...
MS_S16 HAL_MIU_GetClientInfo(MS_U8 u8MiuDev, eMIUClientID eClientID)
{
    ...
    {
        ULOGE(TAG_MIU, "Wrong MIU device:%u\n", u8MiuDev);
        return (-1);
    }
...
MS_BOOL HAL_MIU_Dram_Size(MS_U8 MiuID, MS_U8 DramSize)
{
    ...
    if(E_MIU_3 == (MIU_ID)MiuID)
    {
        u32RegAddr = MIU3_PROTECT_DDR_SIZE;
        switch (DramSize)
        {
            case E_MIU_DDR_32MB:
    ...
    else if(E_MIU_0 == (MIU_ID)MiuID)
    {
     ...
     {
         ULOGE(TAG_MIU, "%s not support MIU%u!\n", __FUNCTION__, MiuID );
          return FALSE;
     }
...
MS_BOOL HAL_MIU_Dram_ReadSize(MS_U8 MiuID, MIU_DDR_SIZE *pDramSize)
{
    ...
    DramSize = HAL_MIU_ReadByte(u32RegAddr);
    DramSize &= 0xF0;
    switch (DramSize)
    {
        case MIU_PROTECT_DDR_32MB:
            *pDramSize = E_MIU_DDR_32MB;
            break;
        case MIU_PROTECT_DDR_64MB:
            *pDramSize = E_MIU_DDR_64MB;
            break;
        case MIU_PROTECT_DDR_128MB:
            *pDramSize = E_MIU_DDR_128MB;
            break;
        case MIU_PROTECT_DDR_256MB:
            *pDramSize = E_MIU_DDR_256MB;
            break;
        case MIU_PROTECT_DDR_512MB:
            *pDramSize = E_MIU_DDR_512MB;
            break;
        case MIU_PROTECT_DDR_1024MB:
...
MS_BOOL HAL_MIU_GetProtectInfo(MS_U8 u8MiuDev, MIU_PortectInfo *pInfo)
{
    ...
    if (REG_MIU_PROTECT_HIT_FALG & ret)
    {
        ...
        ULOGE(TAG_MIU, "MIU%u Block:%u Group:%u ClientID:%u Hitted_Address:0x%x<->0x%x\n", u8MiuDev,
        pInfo->u8Block, pInfo->u8Group, pInfo->u8ClientID, (unsigned int)u32Address, (unsigned int)(u32Address + MIU_PROTECT_ADDRESS_UNIT - 1));
...
MS_BOOL HAL_MIU_SetSsc(MS_U8 u8MiuDev, MS_U16 u16Fmodulation, MS_U16 u16FDeviation, MS_BOOL bEnable)
{
    ...
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "MMIO base:%lx uRegBase:%lx\n", _gMIU_MapBase, uRegBase));
    ...
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "---> u16Fmodulation = %d u16FDeviation = %d \n",(int)u16Fmodulation,(int)u16FDeviation));
    ...
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "---> Loop_First:%u Loop_Second:%u\n", u16Loop_DIV_First, u16Loop_DIV_Second));
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "---> Input_first:%u Input_second:%u\n", u16Input_DIV_First, u16Input_DIV_Second));
    uDDR_MHz = (MPPL * DDR_FACTOR * u16Loop_DIV_First * u16Loop_DIV_Second)/ (uDDFSET*u16Input_DIV_First*u16Input_DIV_Second);
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "---> uDDFSET = 0x%lx\n",uDDFSET));
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "---> DDR_MHZ = 0x%lx (%d MHz)\n",uDDR_MHz,(int)uDDR_MHz));
    ...
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "---> DDFSPAN = 0x%x (%d)\n",u16DDFSpan,(int)u16DDFSpan));
    if (u16DDFSpan > 0x3FFF)
    {
        u16DDFSpan = 0x3FFF;
        HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "??? DDFSPAN overflow > 0x3FFF, Fource set to 0x03FF\n"));
    }
    ...
    HAL_MIU_SSC_DBG(ULOGD(TAG_MIU, "---> DDFSTEP = 0x%lx (%lu)\n",uDDFStep,uDDFStep));
    //Write to Register
    uDDFStep &= (0x03FF);
...
void HAL_MIU_MaskReq(MS_U8 u8Miu, eMIUClientID eClientID)
{
    ...
    if (sVal < 0)
        ULOGE(TAG_MIU, "%s not support client ID:%u!\n", __FUNCTION__, eClientID);
    else
    {
        u32Group = MIU_GET_CLIENT_GROUP(sVal);

        //For Group 4~7
        if (u32Group >=4)
        {
            u32Reg = REG_MIU_ARB_RQX_MASK (u32Group);

            if(u8Miu == E_MIU_0)
            {
                u32Reg += MIU_ARB_REG_BASE;
            }
            else if(u8Miu == E_MIU_1)
            {
                u32Reg += MIU1_ARB_REG_BASE;
            }
            else if(u8Miu == E_MIU_2)
            {
                u32Reg += MIU2_ARB_REG_BASE;
            }
            else
            {
               ULOGE(TAG_MIU, "%s not support MIU%u!\n", __FUNCTION__, u8Miu );
        ...
        else
        {
            ...
            u32Reg = REG_MIU_RQX_MASK (u32Group);
...
MS_BOOL HAL_MIU_SelMIU(eMIU_SelType eType, eMIUClientID eClientID)
{
    ...
    {
        ULOGE(TAG_MIU, "%s not support MIU selection:%u!\n", __FUNCTION__, eType );
        return FALSE;
    }
...
MS_BOOL HAL_MIU_SetGroupPriority(MS_U8 u8MiuDev, MIU_GroupPriority sPriority)
{
    ...
    ULOGD(TAG_MIU, "Change miu%u group priority:%x\n", u8MiuDev, u8Val);
...
void HAL_MIU_PrintMIUProtectArea(MS_U8 u8Blockx,MS_U8 miu_dev)
{
    ...
    if( miu_dev == E_MIU_0 )
    {
        ...
        switch (u8Blockx)
        {
            ...
            default:
                ULOGE(TAG_MIU, "error miu protect block number\n");
                return ;
        }
    }
    ...
    ULOGD(TAG_MIU, "miu%d protect%d startaddr is 0x%x\n", miu_dev, u8Blockx, (unsigned int)val_32);

    //protect end address
    val_16 = HAL_MIU_Read2Byte(u32RegAddrStar+0x2);
    val_32 = ((( val_16 + 1 ) | (( val1_16  ) << 4 ) ) << MIU_PAGE_SHIFT ) - 1;

    ULOGD(TAG_MIU, "miu%d protect%d endaddr   is 0x%x\n", miu_dev, u8Blockx, (unsigned int)val_32);
}
...
void HAL_MIU_PrintMIUProtectInfo(void)
{
    ...
    if ( (val_8 & 0xf) != 0 )
    {
        ULOGD(TAG_MIU, "miu0 protect is enabled\n");

        //protect_ID_enable information
        val_16 =HAL_MIU_Read2Byte(u32MiuProtectIdEn);
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu0 protect0_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu0 protect1_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu0 protect2_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu0 protect3_ID_enable is 0x%x\n",val_16);
    ...
    {
        ULOGE(TAG_MIU, "miu0 protect is not enabled\n");
    }
    ...
    if ( (val_8 & 0xf) != 0x0 )
    {
        ULOGD(TAG_MIU, "miu1 protect is enabled\n");
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu1 protect0_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu1 protect1_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu1 protect2_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu1 protect3_ID_enable is 0x%x\n",val_16);
    ...
    {
        ULOGE(TAG_MIU, "miu1 protect is not enabled\n");
    ...
    if ( (val_8 & 0xf) != 0x0 )
    {
        ULOGD(TAG_MIU, "miu2 protect is enabled\n");
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu2 protect0_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu2 protect1_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu2 protect2_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu2 protect3_ID_enable is 0x%x\n",val_16);
    ...
    {
        ULOGE(TAG_MIU, "miu2 protect is not enabled\n");
    ...
    if ( (val_8 & 0xf) != 0x0 )
    {
        ULOGD(TAG_MIU, "miu3 protect is enabled\n");
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu3 protect0_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu3 protect1_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu3 protect2_ID_enable is 0x%x\n",val_16);
        ...
        if ( val_16 != 0 )
        {
            ULOGD(TAG_MIU, "miu3 protect3_ID_enable is 0x%x\n",val_16);
    ...
    {
        ULOGE(TAG_MIU, "miu3 protect is not enabled\n");
