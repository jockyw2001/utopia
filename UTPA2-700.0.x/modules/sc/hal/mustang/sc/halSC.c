...
void HAL_SC_RegMap(MS_U32 u32RegBase)
{
    _regSCBase[0] = u32RegBase;
}

void HAL_SC_HW_RegMap(MS_U32 u32RegBase)
{
...
void HAL_SC_Init(MS_U8 u8SCID)
{
    ...
    HW_WRITE(REG_TOP_ALLPAD_IN, HW_READ(REG_TOP_ALLPAD_IN) & ~TOP_ALLPAD_IN_EN); // disable all pad in
    ...
    if (u8SCID == 0)
    {
        ...
        HW_WRITE(REG_DSPEJTAG_MODE,HW_READ(REG_DSPEJTAG_MODE)&(~(BIT(8)|BIT(9))));
        HW_WRITE(REG_TS1CONFIG,HW_READ(REG_TS1CONFIG)&(~(BIT(11)|BIT(12)|BIT(13))));
    }
    if (u8SCID ==1)
    {
        SC_ERR("[SC] not support SM1\n");
...
void HAL_SC_PowerCtrl(MS_U8 u8SCID, MS_BOOL bEnable)
{
    if (u8SCID == 0)
    {
        if (bEnable)
        {
            ...
            //HW_WRITE(REG_TOP_CKG_UART_SRC,  HW_READ(REG_TOP_CKG_UART_SRC) & ~TOP_CKG_UART1_SRC_CLK); //how
