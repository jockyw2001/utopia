...
MsHDMITX_RX_STATUS MHal_HDMITx_GetRXStatus(void)
{
    ...
    hpd_s = MHal_HDMITx_Read(0x001100, 0x33); //temp solution for kernal mode: GPIO 23
    #else
				...
				else
				{
					...
					else
					{
						if (gbInfoFrmPktList[ucInfoPktType].EnableUserDef == TRUE)
						{
							...
							//MDrv_Write2Byte(REG_HDMITX_10_L, 0x05 | (ucChkSum << 8) | (gbInfoFrmPktList[ucInfoPktType].FrmCntNum<< 3));
						}
						else
						{
...
void MHal_HDMITx_SetVideoInfoByCustomer(MsHDMITX_VIDEO_TIMING idx, stHDMITX_TIMING_INFO_BY_CUSTOMER stTimingInfo)
{
    return;
}
