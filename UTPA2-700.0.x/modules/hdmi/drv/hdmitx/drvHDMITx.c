...
void MDrv_HDCPTx_StartAuth(void* pInstance, MS_BOOL bFlag)
{
    ...
    MDrv_HDMITx_SetHDCPOnOff(pInstance, bFlag);
			...
			if ( *(pu8BlkData + 7) & 0x20 ) // HDMI_Video_Present
			{
				...
				if (u8BlkLen > (u8IdxOffset + 9))
				{
					if ( (*(pu8BlkData + u8IdxOffset + 8) & 0x80) || ( ((*(pu8BlkData + u8IdxOffset + 9) & 0xE0) >> 5) > 0 ))//3D present and VIC LEN
					{
                        ...
                        #if (DBG_HDMIVSDB_INFO == 1)
                            printf("%s::3dmulti_present=%x, 3DLen=%d, VicLen=%d !\n",
						...
						if ( u8_HdmiVICLen > 0 )
						{
							if ( u8BlkLen > (u8IdxOffset + 9 + u8_HdmiVICLen))
							{
								...
								#if (DBG_HDMIVSDB_INFO == 1)
									printf("%s::HDMI VIC list\r\n",__FUNCTION__);
									for ( i = 0; i < u8_HdmiVICLen; i++ )
										printf("0x%02X ", psHDMITXResPri->stHDMITxInfo.RxEdidInfo.HdmiVICList[i]);
									printf("\n");
								#endif
						...
						if (u8_3dMultiPresent == 0x01) // 3D_Structure_ALL_15..0
						{
							...
							if (u8BlkLen > (u8IdxOffset + 11))
							{
								...
								for ( i = 0; i < ((u8SupTimingNum > 16) ? 16 : u8SupTimingNum); i++ )
								{
                                    ...
                                    #if (DBG_HDMIVSDB_INFO == 1)
                                        printf("SupVidTiming[%02d].Video3DInfo = 0x%02X\r\n", i, u16_3dStructAll);
                                    #endif
							    }
								u8_Hdmi3DLen -= 2;
							}
                ...
                case CEATag_UseExtTag:
                {
                    ...
                    switch (ucExtTagCode)
                    {
                        ...
                        case CEAExtTag_ColorimetryDB:
                            ...
                            {
                                printf("Invalid Length of Colorimetry Data Block\r\n");
                                return FALSE;
                            }
                        ...
                        case CEAExtTag_HDRStaticMetaDB:
                            ...
                            for ( i = 0; i < (u8BlkLen - 1); i++ )
                            {
                                ...
                                u8CurIdx++;
                                printf("0x%02X, ", psHDMITXResPri->stHDMITxInfo.RxEdidInfo.HDRStaticDataBlk[i]);
                                if ((i+1)%16 == 0)
                                    printf("\r\n");
                            }
                            printf("\r\n");
                        ...
                        default:
                            printf("drv_HDMITx_ParseCEAExtEdid()::Extend Tag Code Unsupport!!\n");
                            u8CurIdx += (u8BlkLen - 1);
                        break;
                    }
                    psHDMITXResPri->stHDMITxInfo.RxEdidInfo.CEADataBlkLen[u8BlkTagCode] = u8BlkLen;
...
#define DBG_HDMITx_CheckEDID 1
...
#if (DBG_HDMITx_CheckEDID == 1)
        printf("%s::Extension Block Number = %d\r\n", __FUNCTION__, u8ExtBlkNum);

        if (u8ExtBlkNum > 4)
            printf("%s::Number of Extension Block > 4!!\n", __FUNCTION__);
#endif
    ...
    for ( i = 1; i <= ((u8ExtBlkNum >= 4) ? 4 : u8ExtBlkNum); i++ )
    {
        ...
        while (MHal_HDMITx_EdidReadBlock(i, blkData) == FALSE)
        {
			...
			#if (DBG_HDMITx_CheckEDID == 1)
                printf("[HDMITx] Check EDID fail--1!!\n");
			#endif
    ...
    for ( i = 1; i <= u8ValidBlkNum; i++)
    {
        while (MHal_HDMITx_EdidReadBlock(i, blkData) == FALSE)
        {
			...
			#if (DBG_HDMITx_CheckEDID == 1)
                printf("[HDMITx] Check EDID fail--2!!\n");
			#endif
        ...
        {
		#if (DBG_HDMITx_CheckEDID == 1)
            printf("%s::Access EDID fail @ Block 0x%02X!!\n", __FUNCTION__, i);
		#endif
...
void MDrv_HDMITx_InitVariable(void* pInstance)
{
    ...
    #else
    psHDMITXResPri->stHDMITxInfo.output_video_timing = E_HDMITX_RES_1280x720p_60Hz;
    #endif
...
/// @return None
//------------------------------------------------------------------------------
MS_U32 MDrv_HDMITx_GetTMDSStatus(void* pInstance)
{
    MS_U32 u32TMDSStatus = 0;
    u32TMDSStatus = MHal_HDMITx_GetTMDSStatus();
...
void MDrv_HDMITx_SetHDMITxMode(void* pInstance, MsHDMITX_OUTPUT_MODE mode)
{
    ...
    if (psHDMITXResPri->stHDMITxInfo.output_mode & BIT1) // HDMI
    {
        ...
        if ((psHDMITXResPri->stHDMITxInfo.output_colordepth_val) || (psHDMITXResPri->stHDMITxInfo.hdmitx_avmute_flag)) // RX support DC or SET_AVMUTE
        {
...
MS_BOOL MDrv_HDMITx_ColorandRange_Transform(void* pInstance, MsHDMITX_VIDEO_COLOR_FORMAT incolor, MsHDMITX_VIDEO_COLOR_FORMAT outcolor, MsHDMITX_YCC_QUANT_RANGE inange, MsHDMITX_YCC_QUANT_RANGE outrange)
{
    ...
    if(!MDrv_HDMITx_EdidGetHDMISupportFlag(pInstance) && (!psHDMITXResPri->stHDMITxInfo.hdmitx_force_output_color))
    {
        ...
        {
            printf("DVI mode needs RGB only\r\n");
            return FALSE;
        }
    }
...
void MDrv_HDMITx_SetAVMUTE(void* pInstance, MS_BOOL bflag)
{
    ...
    else
    {
        ...
        if (psHDMITXResPri->stHDMITxInfo.output_colordepth_val) // RX support DC
        {
            MDrv_HDMITx_SendPacket(E_HDMITX_GC_PACKET, E_HDMITX_CYCLIC_PACKET);
        }
        else
...
//------------------------------------------------------------------------------
/// @brief This routine sets video output mode by customers' bring in info (color/repetition/regen)
/// @return None
//------------------------------------------------------------------------------
void MDrv_HDMITx_SetVideoOutputModeByCustomer(void* pInstance, MsHDMITX_VIDEO_TIMING enVidTiming, stHDMITX_TIMING_INFO_BY_CUSTOMER stTimingInfo)
{
    _HDMITX_GET_VARIABLE();
    MHal_HDMITx_SetVideoInfoByCustomer(enVidTiming, stTimingInfo);
}
...
{
    printf("start hdmi, hdcp init\r\n");
    if (MDrv_HDMITx_Init(pInstance) == TRUE)
    {
        printf("hdmitx init done\r\n");

        if (MDrv_HDCPTx_Init(pInstance) == TRUE)
        {
            printf("hdcptx init done\r\n");
            return TRUE;
        }
        else
        {
