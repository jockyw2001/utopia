...
MS_U16 _HAL_CA_Random(void)
{
    ...
    REG32(REG_RNG_TRNG) |= REG_RNG_TRNG_ACK;//  Every one can read this value so it can not be trusted.

    do{
        n = REG32(REG_RNG_TRNG);
        if( n & REG_RNG_TRNG_VALID_MASK) //check valid
        {
            n &= REG_RNG_TRNG_OUT_MASK; // Request a trusted value to RNG
            break;
        }
    }while(1);

    REG32(REG_RNG_TRNG) |= REG_RNG_TRNG_ACK;// Request a new value to RNG to prevent an other process to read the generated value.
...
MS_U32 HAL_CA_OTP_ReadCAVendor(void)
{
    ...
    for(u32Idx = 0; u32Idx < CA_SCK_NUM; u32Idx++)
    {
        ...
        u32Len   = u32CAV_Key_VendorID_Hi - u32CAV_Key_VendorID_Lo + 1;
