...
#include "../tsp/regTSP.h"
#include "../tsp/halTSP.h"
#include "../../include/drvDSCMB.h"
#include "../dscmb/halDSCMB.h"

#include "../../include/drvNSK2Type.h"
#include "halNSK2OtpField.h"
#include "../../include/drvCA.h"
//#include "../ca2/halCA.h"		//postpone it until halCA is ready
...
{
    {
        ...
        .capability[3] = (NSK2HDI_M2MCHANNELGROUP_NSK_CAPABILITY_FLAG | NSK2HDI_M2MCHANNELGROUP_NON_NSK_CAPABILITY_FLAG),
    },
...
//enable NSK2 clock
//RSA address 1 bit[1] bit[1]: PM_NSKClkEnable
...
MS_U32 HAL_NSK2_GetOTPFieldValue(MS_U8 u8Tag, MS_U8 *pLength, MS_U32 *pRetValue)
{
	...
	switch (u8Tag){
	    ...
	    case ContentProtectionEnable: //			0x3E30
			// TODO: check it

			HAL_NSK2_OTP_Get(0xf8c*4,  16,  16, &u32tmp1);
			HAL_NSK2_OTP_Get(0xf8c*4, 7, 4, &u32tmp2);

			*pRetValue = ( u32tmp1 << 2 ) | u32tmp2;
			*pLength = 1;
			break;

	    case U_OTP_SBoot_0:		//Keres: U_OTP_Sboot. 	0x3C74
			HAL_NSK2_OTP_Get(0xf1d*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_NSK2: //			0x3E94
			HAL_NSK2_OTP_Get(0xfa5*4, 11, 8, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_ACPUUseNSK2: //			0x3E7C
			HAL_NSK2_OTP_Get(0xf9f*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_DBUSUseNSK2://			0x3E7C
			HAL_NSK2_OTP_Get(0xf9f*4, 7, 4, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_ForceTenSec://			0x3E7C
			HAL_NSK2_OTP_Get(0xf9f*4, 11, 8, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_SCCheck://			0x3E7C
			HAL_NSK2_OTP_Get(0xf9f*4, 15, 12, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_SWRN://			0x3E7C
			HAL_NSK2_OTP_Get(0xf9f*4, 23, 20, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_TestRCFreq://			0x3E7C
			HAL_NSK2_OTP_Get(0xf9f*4, 19, 16, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_VenderConstSel://				0x38FC
			HAL_NSK2_OTP_Get(0xe3f*4, 2, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_RANDOM_0://			0x3C28
			HAL_NSK2_OTP_Get(0xf0a*4, 13, 12, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_NOISE_0://			0x3C28
			HAL_NSK2_OTP_Get(0xf0a*4, 19, 18, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_NDS_Parity_ChkEn_0://			0x3C28
			HAL_NSK2_OTP_Get(0xf0a*4, 25, 24, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_OTPWritePWDProtect_0://			0x3E28
			HAL_NSK2_OTP_Get(0xf8a*4, 5, 2, pRetValue);
			*pLength = 1;
			break;

        case U_OTP_concurrency_configuration:    //return as same as 4A
            //HAL_NSK2_OTP_Get(0xf8c*4, 15, 14, pRetValue);
			HAL_NSK2_OTP_Get(0xf0a*4, 13, 12, pRetValue);	//			0x3C28
			*pLength = 1;
            break;


        case U_OTP_nds_fc_disable:  //			0x39AC
            HAL_NSK2_OTP_Get(0xe6b*4, 0, 0, pRetValue);
			*pLength = 1;
            break;

		/*HERE 50,51 missing. Need to check */
		/*Added manually*/
		case 0x50: //			0x3C28
            HAL_NSK2_OTP_Get(0xf0a*4, 25, 24, pRetValue);
			*pLength = 1;
	    ...
	    case U_OTP_ena_EMMFilter: //			0x39A8
			HAL_NSK2_OTP_Get(0xe6a*4, 11, 8, pRetValue);
			*pLength = 1;
			break;
	    case U_OTP_allow_NSKCW2CryptoDMA://			0x3E2C
			HAL_NSK2_OTP_Get(0xf8b*4, 27, 24, pRetValue);
			*pLength = 1;
			break;


	    case U_OTP_ena_ACPU2DMA://			0x3C80
			HAL_NSK2_OTP_Get(0xf20*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_DMA_DESBasedCipher://			0x3C80
			HAL_NSK2_OTP_Get(0xf20*4, 7, 4, pRetValue);
			*pLength = 1;
			break;

		#if 0
	    case U_OTP_ena_DMA_GetKeyFromKL:						//3 MISSING FROM FUSEMAP. ASK YUCHIA
			//HAL_NSK2_OTP_Get(0xf6b*4, 15,12 , pRetValue);
			*pRetValue = 0;
			*pLength = 1;
			break;
		#endif

	    case U_OTP_allow_ACPU2KT://			0x3E2C
			HAL_NSK2_OTP_Get(0xf8b*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_ACPUWrNSKKey2KT://			0x3E2C
			HAL_NSK2_OTP_Get(0xf8b*4, 19, 16, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_NSK2KT://			0x3E2C
			HAL_NSK2_OTP_Get(0xf8b*4, 7, 4, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ESAAlgo_invalidate://			0x3E30
			HAL_NSK2_OTP_Get(0xf8c*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_NDS_ContentProtect_sel://		0x3E30
			HAL_NSK2_OTP_Get(0xf8c*4, 16, 16, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_CSA2Var: //			0x3E50
			HAL_NSK2_OTP_Get(0xf94*4, 23, 20, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_CSA2Var_nonNSK: //			0x3E50
			HAL_NSK2_OTP_Get(0xf94*4, 27, 24, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_CSA3NonNskVariant: //			0x3E50
			HAL_NSK2_OTP_Get(0xf94*4, 19, 16, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_DVBCSA2ComfCipher://			0x3E50
			HAL_NSK2_OTP_Get(0xf94*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_DVBCSA3Var://			0x3E50
			HAL_NSK2_OTP_Get(0xf94*4, 15, 12, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_allow_Multi2BasedCipher://			0x3E4C
			HAL_NSK2_OTP_Get(0xf93*4, 19, 16, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_Multi2SysKey://fe8~fef


        	#if 1
            HAL_NSK2_OTP_Get(0xf96*4, 31, 0, pRetValue);
            *pLength = 1;
            *pRetValue >>= 24;

        	#else
            HAL_NSK2_OTP_Get(0xfe8*4, 31, 0, &pRetValue[0]);
			HAL_NSK2_OTP_Get(0xfe9*4, 31, 0, &pRetValue[1]);
			HAL_NSK2_OTP_Get(0xfea*4, 31, 0, &pRetValue[2]);
			HAL_NSK2_OTP_Get(0xfeb*4, 31, 0, &pRetValue[3]);

			HAL_NSK2_OTP_Get(0xfec*4, 31, 0, &pRetValue[4]);
			HAL_NSK2_OTP_Get(0xfed*4, 31, 0, &pRetValue[5]);
			HAL_NSK2_OTP_Get(0xfee*4, 31, 0, &pRetValue[6]);
			HAL_NSK2_OTP_Get(0xfef*4, 31, 0, &pRetValue[7]);
			*pLength = 8;
	        #endif

			break;

	    case U_OTP_SWMulti2SysKey://			0x3E54
			// TODO: check it
			HAL_NSK2_OTP_Get(0xf95*4, 23, 20, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ModifyXRC_Override: //			0x3E54
			HAL_NSK2_OTP_Get(0xf95*4, 30, 30, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_Permutation_Override://			0x3E54
			HAL_NSK2_OTP_Get(0xf95*4, 29, 24, pRetValue);
			*pLength = 1;
			break;
		#if 0
	    case U_OTP_ena_TS2TSO: 						//3 MISSING FROM FUSEMAP. ASK YUCHIA
			//HAL_NSK2_OTP_Get(0xf79*4, 13, 10, pRetValue);
			*pRetValue = 0;
			*pLength = 1;
			break;
		#endif
	    case U_OTP_ena_TS2TSO_0: //			0x39A8
			HAL_NSK2_OTP_Get(0xe6a*4, 15, 12, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_TS2TSO_1://			0x39A8
			HAL_NSK2_OTP_Get(0xe6a*4, 19, 16, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ContentProEn_all1: //			0x3E30
			HAL_NSK2_OTP_Get(0xf8c*4, 7, 4, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_LSACPCM: //			0x3E4C
			// TODO: check it
			HAL_NSK2_OTP_Get(0xf93*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_EJTAG_MODE://			0x3C78
			HAL_NSK2_OTP_Get(0xf1e*4, 20, 15, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_DIG_JTAGMode://		0x3C78
			HAL_NSK2_OTP_Get(0xf1e*4, 22, 21, pRetValue);
			*pLength = 1;

			break;
	    case U_OTP_SBJTAGMode://			0x3C74
			HAL_NSK2_OTP_Get(0xf1d*4, 11, 8, pRetValue);
			*pLength = 1;
			break;
		#if 0
	    case U_OTP_SBI2CMode: 				//3 MISSING FROM FUSEMAP. ASK YUCHIA
			//HAL_NSK2_OTP_Get(0xf73*4, 13, 8, pRetValue);
			*pRetValue = 0;
			*pLength = 1;
			break;
		#endif
	    case U_OTP_I2CMode: //			0x3C78
			HAL_NSK2_OTP_Get(0xf1e*4, 14, 9, pRetValue);
			*pLength = 1;

			break;
	    case U_OTP_SCANMode://			0x3C78
			HAL_NSK2_OTP_Get(0xf1e*4, 8, 0, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_USBSlaveMode://			0x3C78
			HAL_NSK2_OTP_Get(0xf1e*4, 31, 28, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_Disable_TestInOut: //			0x3C74
			HAL_NSK2_OTP_Get(0xf1d*4, 15, 14, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_ena_NDS_JTAG_PWD:	//			0x3E80		CHECK!!!
			HAL_NSK2_OTP_Get(0xfa0*4, 19, 18, pRetValue);
			*pLength = 1;
			break;

	    case U_OTP_SBoot_1:		//			as same as  0x41
			// TODO: check it
			//HAL_NSK2_OTP_Get(0xf1b*4, 13, 8, pRetValue);
			HAL_NSK2_OTP_Get(0xf1d*4, 3, 0, pRetValue);
			*pLength = 1;
			break;

		default:
            HALNSK2_DBG(NSK2_DBGLV_ERR,"no such tag\n");
			*pRetValue = 0;
			*pLength = 0;
            return FALSE;
    ...
    HALNSK2_DBG(NSK2_DBGLV_INFO,"Get otp tag ok\n");
    return TRUE;
}
...
static ASST_t KanoAsst[] =
{
    ...
    {0x21, 16,  0x3BC0, 127,   0}, //Bulk data 1
    {0x22, 16,  0x3BD0, 127,   0}, //Bulk data 2
    {0x23, 16,  0x3BE0, 127,   0}, //Bulk data 3
    {0x24, 16,  0x3BF0, 127,   0}, //Bulk data 4


    {0x40,  1,  0x3D24,  11,   8}, //U_OTP_ena_NSK2
    ...
    {0x44,  1,  0x3B24,  15,  12}, //U_OTP_ena_SCCheck, U_OTP_allow_SCCheck
    {0x45,  1,  0x3B24,  23,  20}, //U_OTP_ena_SWRN
    {0x46,  1,  0x3B24,  19,  16}, //U_OTP_ena_TestRCFreq
    {0x47,  1,  0x3B18,   2,   0}, //U_OTP_venderConstSel

    {0x48,  1,  0x3B1F,   7,   0}, //U_OTP_NDSKeyValid
    //{0x49,  1,  0x3E8C,  15,   8}, //U_OTP_NDSSecMode
    {0x4A,  1,  0x3C50,   1,   0}, //U_OTP_NDS_ESCK_obfuscation

    {0x4B,  1,  0x3CC0,  13,  12}, //U_OTP_RANDOM, U_OTP_allow_RANDOM double chekc with yuchia.
    {0x4C,  1,  0x3B2C,  11,  10}, //U_OTP_NDS_Parity_ChkEn, U_OTP_allow_NDS_Parity_chk
    {0x4D,  1,  0x3B24,  31,  28}, //U_OTP_OTPWritePWDProtect
    {0x4E,  1,  0x3C28,   9,   8}, //U_OTP_forbid_DbusWriteOTP
    {0x4F,  1,  0x3C28,  11,  10}, //U_OTP_forbid_DbusReadOTP
    {0x50,  1,  0x3C28,   5,   4}, //U_OTP_forbid_SEPWriteOTP
    {0x51,  1,  0x3C28,   7,   6}, //U_OTP_forbid_SEPReadOTP
    {0x52,  1,  0x3C28,   1,   0}, //U_OTP_forbid_MCUWriteOTP
    {0x53,  1,  0x3C28,   3,   2}, //U_OTP_forbid_MCUReadOTP
    {0x54,  1,  0x3C28,  23,  22}, //U_OTP_allow_NDS_Rd55AA
    {0x55,  1,  0x3E80,  23,  20}, //U_OTP_allow_illegal_chk
    {0x56,  1,  0x3E80,  27,  24}, //U_OTP_allow_parity_fail_issue_rst
    {0x57,  1,  0x3C4C,  24,  24}, //U_OTP_SKIP_0
    {0x58,  1,  0x3C4C,  25,  25}, //U_OTP_SKIP_1
    {0x59,  1,  0x3C28,  21,  20}, //U_OTP_allow_OTP_BgCRC
    {0x5A,  1,  0x3E2C,  27,  24}, //U_OTP_allow_NSKCW2CryptoDMA, U_OTP_ena_NSKCW2CryptoDMA
    {0x5B,  1,  0x3C80,   3,   0}, //U_OTP_ena_ACPU2DMA
    {0x5C,  1,  0x3C80,   7,   4}, //U_OTP_ena_DMA_DESBasedCipher
    {0x5D,  1,  0x3C80,  10,  10}, //U_OTP_NDSM2MSWIV, U_OTP_forbid_NDSM2MSWIV
    {0x5E,  1,  0x3C80,   9,   8}, //U_OTP_ena_AESMAC_MaxRate
    {0x5F,  1,  0x3E2C,   3,   0}, //U_OTP_allow_ACPU2KT, U_OTP_ena_ACPU2KT
    {0x60,  1,  0x3E2C,  19,  16}, //U_OTP_allow_ACPUWrNSKKey2KT, U_OTP_ena_ACPUWrNSKKey2KT
    {0x61,  1,  0x3E2C,   7,   4}, //U_OTP_allow_NSK2KT
    {0x62,  1,  0x3E30,   3,   0}, //U_OTP_ESAAlgo_invalidate, U_OTP_ena_ESAAlgo_invalidate
    {0x63,  1,  0x3E30,  16,  16}, //U_OTP_NDS_ContentProtect_sel
    {0x64,  1,  0x3E50,  23,  20}, //U_OTP_allow_CSA2Var, U_OTP_ena_DVBCSA2Var
    {0x65,  1,  0x3E50,  27,  24}, //U_OTP_allow_CSA2Var_nonNSK, U_OTP_ena_DVBCSA2Var_nonNSK
    {0x66,  1,  0x3E50,  19,  16}, //U_OTP_allow_CSA3NonNskVariant, U_OTP_ena_DVBCSA3NonNskVariant
    {0x67,  1,  0x3E50,   3,   0}, //U_OTP_allow_DVBCSA2ComfCipher, U_OTP_ena_DVBCSA2ConfCipher
    {0x68,  1,  0x3E50,  15,  12}, //U_OTP_allow_DVBCSA3Var, U_OTP_ena_DVBCSA3Var
    {0x69,  1,  0x3E4C,  19,  16}, //U_OTP_allow_Multi2BasedCipher, U_OTP_ena_Multi2BasedCipher
    {0x6A,  1,  0x3E54,  23,  20}, //U_OTP_SWMulti2SysKey, U_OTP_ena_SWMulti2SysKey
    {0x6B,  1,  0x3E54,  30,  30}, //ModifyXrc_Override, U_OTP_ModifyXRC_Override
    {0x6C,  1,  0x3E54,  29,  24}, //Permutation_Override, U_OTP_Permutation_Override
    {0x6D,  1,  0x39A8,  15,  12}, //U_OTP_ena_TS2TSO_0
    {0x6E,  1,  0x39A8,  19,  16}, //U_OTP_ena_TS2TSO_1
    {0x6F,  1,  0x3E30,   7,   4}, //U_OTP_ContentProEn_all1
    {0x70,  1,  0x3E4C,   3,   0}, //U_OTP_ena_LSACPCM
    {0x71,  1,  0x3E50,  31,  28}, //U_OTP_dis_NonSecRangeEncrypt
    {0x72,  1,  0x3C78,  21,  16}, //U_OTP_EJTAG_MODE
    {0x73,  1,  0x3C78,  23,  22}, //U_OTP_DIG_JTAGMode
    {0x74,  1,  0x3C74,  11,   8}, //U_OTP_SBJTAGMode
    {0x75,  1,  0x3C78,  14,   9}, //U_OTP_I2CMode
    {0x76,  2,  0x3C78,   8,   0}, //U_OTP_SCANMode
    {0x77,  1,  0x3C78,  31,  28}, //U_OTP_ena_USBSlaveMode, U_OTP_forbid_USBSlaveMode ????
    {0x78,  1,  0x3C74,  15,  14}, //U_OTP_Dis_TestInOut, U_OTP_forbid_TestInOut
    {0x79,  1,  0x3E80,  19,  18}, //U_OTP_ena_NDS_JTAG_PWD, U_OTP_allow_NSK2_EJTAG_Mode  ???
    {0x7A,  1,  0x3C6C,  13,   8}, //U_OTP_BootMode
    {0x7B,  1,  0x3C74,   3,   0}, //U_OTP_SBoot
    {0x7C,  1,  0x39A0,   3,   0}, //U_OTP_ena_PVR_secure_protect_0
    {0x7D,  1,  0x399C,   7,   4}, //U_OTP_ena_PVRS2NS
    {0x7E,  1,  0x39AC,   0,   0}, //U_OTP_nds_fc_disable
    {0x7F,  4,  0x3990,   31,   0}, //U_OTP_MSID
};
...
{
    MS_U32 NumOfTag = sizeof(KanoAsst)/sizeof(ASST_t);
    ...
    {
        ret_size += (KanoAsst[i].u8Length + 2);
    }
    ...
    *desc_size = ret_size;
    ...
    for( i=0; i<NumOfTag; i++)
    {
        *pDesc++ = KanoAsst[i].u8Tag;
        *pDesc++ = KanoAsst[i].u8Length;

        if(0x20 == KanoAsst[i].u8Tag) //pubOTP...special case....
        {
        #if 1
            *pDesc++ = 0;
            //4 Chip ID
            ...
            {
                MS_U32 u32Tmp;
                u32Tmp = MDrv_CA_OTP_Read(0x3EA0 + j);
            ...
            pDesc += 8;
        #else
        extern void endian_change(MS_U8* ptr, MS_U32 bytes);
            MS_U8 PubOTP[16];
            HAL_NSK2_GetPubOTP(PubOTP);
            endian_change(PubOTP, 16);
            memcpy(pDesc,PubOTP,16);
            pDesc += 16;
        #endif
        }
        else
        {
            RunLens = KanoAsst[i].u8Length;
            RunOffset = KanoAsst[i].Offset;
            //printf("Offset = %lx, length = %x\n",RunOffset, RunLens);
            if(KanoAsst[i].u8Length <= 4)
            {
                HAL_NSK2_OTP_Get(KanoAsst[i].Offset, KanoAsst[i].MSB, KanoAsst[i].LSB, &OTPValue);
                if(1 == RunLens)
...
MS_BOOL HAL_NSK2_RSA_SetSecureRange(MS_U32 u32SecSet, MS_U32 u32SecStart, MS_U32 u32SecEnd)
{
    if(u32SecSet > REG_RSA_SECRANGE_SET  || u32SecEnd <= u32SecStart)
        return FALSE;

    *(volatile MS_U32*) (_gBasicAddr + REG_RSA_SECRANGE_START(u32SecSet)) = REG_RSA_SECRANGE_ENABLE | ( u32SecStart & REG_RSA_SECRANGE_MASK);
    *(volatile MS_U32*) (_gBasicAddr + REG_RSA_SECRANGE_END(u32SecSet))   = REG_RSA_SECRANGE_ENABLE | ( u32SecEnd   & REG_RSA_SECRANGE_MASK);
...
/* D:\Chip\Kano\acpu.bin (2015/2/2 ¤W¤È 10:59:28)
   StartOffset: 00000000, EndOffset: 00003FFF, Length: 00004000 */

unsigned char rawData1[16384] = {
	...
	0xDB, 0xB6, 0x6D, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x11, 0x00, 0x02, 0x01,
	0x40, 0x01, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	,
	...
	,
	...
	,
	...
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xCD, 0x07, 0x2C, 0xD8,
	0xBE, 0x6F, 0x9F, 0x62, 0xAC, 0x4C, 0x09, 0xC2, 0x82, 0x06, 0xE7, 0xE3,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static MS_U32 OTPConfig[0x4000];

void HAL_NSK2_WriteAllOTP(MS_U32 index)
{
    memset(OTPConfig,0x0,0x4000);

    if(index == 1)
    {
        memcpy(OTPConfig, rawData1, 0x4000);
    }

    MS_U32 addr;

    printf("start to write OTP\n");
    for( addr=0 ; addr<0x4000; addr+=4)
    {
        if(OTPConfig[addr/4] != 0)
        {
            printf("addr = %x, value = %x\n", addr, OTPConfig[addr/4]);
            OTP_REG(addr) = OTPConfig[addr/4];
        }
    }

    printf("finish HAL_NSK2_WriteAllOTP\n");
}
...
MS_U32 HAL_NSK21_ModifyGenIn(MS_U32 MaskVal,MS_U32 XorVal)
{
    ...
    NI_REG(REG_NI_NSK21_GENIN) = (WriteGenIn & BMASK(9:0));
...
MS_U32 HAL_NSK21_WriteJTagKey(MS_U32 OverrideOid, MS_U32 Select)
{
    ...
    NI_REG(REG_NI_COMMAND) = (NI_COMMAND_START | NI_WriteJTAGKey | functionality);
