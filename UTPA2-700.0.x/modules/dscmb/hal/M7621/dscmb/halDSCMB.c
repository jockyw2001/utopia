...
MS_BOOL HAL_DSCMB_Init(void)
{
    ...
    REG16_W(REG_DSCMB2_CIPHERENG_CTRL, REG16_R(REG_DSCMB2_CIPHERENG_CTRL) | ( REG_BLK_AF_ENG0 | REG_BLK_AF_ENG1 | REG_BLK_AF_ENG2 ));

#if (DSCMB_LAST_ENG_MULTI2_ONLY == 1)
    //for Engine #2, only supports multi2, need to set ESA mode for scramble status, not CSA mode
    REG16_W(REG_DSCMB2_KT_CTRL_EXT, REG16_R(REG_DSCMB2_KT_CTRL_EXT)|REG_DSCMB2_ESA_UPDATE_TSC_ESA);
#endif
...
MS_BOOL HAL_DSCMB_GetCap(MS_U32 u32EngId, HAL_DSCMB_Query_Type eQueryType, void* pInData, void* pOutData)
{
   ...
   case E_HAL_DSCMB_CAP_SHARE_KEY_REGION_NUM:
    ...
    case E_HAL_DSCMB_CAP_SUPPORT_ALGORITHM:
        switch (*((HAL_DSCMB_Type*)pInData)) // engine 2 support MULTI2 only
        {
...
MS_BOOL HAL_DSCMB_OptConfig(MS_U32 u32EngId, HAL_DSCMB_Option eOpt, void* pInput, void* pOutput, MS_BOOL bSet)
{
    ...
    if(bSet)
    {
        switch(eOpt)
        {
            case E_HAL_DSCMB_CFG_HDCP2_TX_CONTENT_CATEGORY_SUPPORT:
            {
...
MS_BOOL HAL_DSCMB_FltIVSet(MS_U32 u32EngId, MS_U32 u32PidFltId, MS_U32 u32Slot, HAL_DSCMB_Type DscmbType, DSCMB_Key_Type eKeyType, MS_U8* pu8IV)
{
    ...
    if((u32EngId == (ENG_NUM - 1)) && (DscmbType != E_HAL_DSCMB_TYPE_MULTI2))
        return FALSE;
    if(pu8IV==NULL)
        return FALSE;
