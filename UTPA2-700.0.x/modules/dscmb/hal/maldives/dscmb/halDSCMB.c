...
void inline _pid_key_table_w(MS_U32 eng, MS_U32 addr, MS_U32 key_idx)
{
    MS_U32 engbit = (eng == 0) ? REG_DSCMB2_ARRAY_ENG0_VLD :
                    (eng == 1) ? REG_DSCMB2_ARRAY_ENG1_VLD : 0;
...
MS_BOOL _HAL_DSCMB_Mode(MS_U32 u32EngId, HAL_DSCMB_Type type)
{
    ...
    else if (E_HAL_DSCMB_TYPE_CSA == type || E_HAL_DSCMB_TYPE_CSA_CONF == type)
    {
        if(u32EngId == 0)
        {
            ...
            if(E_HAL_DSCMB_TYPE_CSA_CONF == type)
            {
                REG16_W(REG_DSCMB_CTRL, REG16_R(REG_DSCMB_CTRL) | REG_DSCMB_CTRL_CONFORMANCE_MECH);
            }
            else
            {
                REG16_W(REG_DSCMB_CTRL, REG16_R(REG_DSCMB_CTRL) & ~REG_DSCMB_CTRL_CONFORMANCE_MECH);
        ...
        else if(u32EngId == 1)
        {
            ...
            if(E_HAL_DSCMB_TYPE_CSA_CONF == type)
            {
                REG16_W(REG_CSA1_CTRL, REG16_R(REG_CSA1_CTRL) | REG_DSCMB1_CTRL_CONFORMANCE_MECH);
            }
            else
            {
                REG16_W(REG_CSA1_CTRL, REG16_R(REG_CSA1_CTRL) & ~REG_DSCMB1_CTRL_CONFORMANCE_MECH);
...
MS_BOOL HAL_DSCMB_FltTypeSet(MS_U32 u32EngId, MS_U32 u32PidFltId, MS_U32 u32Slot, HAL_DSCMB_Type DscmbType, MS_BOOL bDscmb)
{
    ...
    case E_HAL_DSCMB_TYPE_CSA_CONF: // CSA conformance mechanism
...
MS_BOOL HAL_DSCMB_FltKeyReset(MS_U32 u32EngId, MS_U32 u32PidFltId, MS_U32 u32Slot, HAL_DSCMB_Type DscmbType, DSCMB_Key_Type eKeyType)
{
    ...
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 0, 0, FALSE, 0, 0);
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 1, 0, FALSE, 0, 0);
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 2, 0, FALSE, 0, 0);
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 3, 0, FALSE, 0, 0);
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 4, 0, FALSE, 0, 0);
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 5, 0, FALSE, 0, 0);
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 6, 0, FALSE, 0, 0);
    _HAL_DSCMB_SlotKeyWrite(u32EngId, u32Slot, eKeyType, 7, 0, FALSE, 0, 0);
