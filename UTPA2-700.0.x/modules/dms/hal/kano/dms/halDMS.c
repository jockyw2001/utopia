...
{
    MS_U16 u16MemSyncMask = BIT_MEMSYNC_DELAY_ONE_FRAME|BIT_MEMSYNC_MOTION_RATIO_MASK_EN|BIT_MEMSYNC_PIXEL_MASK_EN;
    if(MApi_XC_IsCurrentFrameBufferLessMode() ||
        MApi_XC_IsCurrentRequest_FrameBufferLessMode())
    {
        u16MemSyncMask &= ~BIT_MEMSYNC_PIXEL_MASK_EN;
    }

    if(bEnable == TRUE)
    {
        u16MemSyncMask |= BIT_MEMSYNC_RESET;
        MApi_XC_GenerateBlackVideo(bEnable, MAIN_WINDOW);
        MApi_XC_W2BYTEMSK(REG_MEMSYNC_BLACK_MODE, u16MemSyncMask, BIT_MEMSYNC_DOTLINE|BIT_MEMSYNC_DELAY_ONE_FRAME|BIT_MEMSYNC_RESET|BIT_MEMSYNC_MOTION_RATIO_MASK_EN|BIT_MEMSYNC_PIXEL_MASK_EN);
        MApi_XC_Mux_OnOffPeriodicHandler(INPUT_SOURCE_DTV, DISABLE);
    }
    else
    {
        MApi_XC_W2BYTEMSK(REG_MEMSYNC_BLACK_MODE, u16MemSyncMask, BIT_MEMSYNC_DOTLINE|BIT_MEMSYNC_DELAY_ONE_FRAME|BIT_MEMSYNC_RESET|BIT_MEMSYNC_MOTION_RATIO_MASK_EN|BIT_MEMSYNC_PIXEL_MASK_EN);
        ...
        MApi_XC_GenerateBlackVideo(bEnable, MAIN_WINDOW);
        MApi_XC_Mux_OnOffPeriodicHandler(INPUT_SOURCE_DTV, ENABLE);
    }
    return TRUE;
}
