...
MS_U32 Hal_VE_ConvertARGB2YUV(MS_U32 u32aRGB)
{
    ...
    s16Y = (s16R*257/1000) + (s16G*504/1000) + (s16B*98/1000) + 16;
    s16U = -(s16R*148/1000) - (s16G*291/1000) + (s16B*439/1000) + 128;
    s16V = (s16R*439/1000) - (s16G*368/1000) - (s16B*71/1000)+ 128;
...
{
    MDrv_WriteByteMask(L_BK_VE_SRC(0x07), (bEnable)?BIT(2):0, BIT(2));	//VE use GOP DE signal
    MDrv_WriteByteMask(L_BK_VE_ENC_EX(0x7C), (bEnable)?(BIT(7)|(0x6<<4)):0, BIT(7)|BIT(6)|BIT(5)|BIT(4));	//SW select delay 6 for fix pipe delay
    return;
}
