...
void Hal_VE_set_inputsource(PMS_VE_InputSrc_Info pInputSrcInfo)
{
    ...
    switch(pInputSrcInfo->eInputSrcType)
    {
        case MS_VE_SRC_DTV:
            ...
            u8Data_Mux = VE_IPMUX_DI; //VE_IPMUX_SC_IP1;
            #else
            u8Data_Mux = VE_IPMUX_MVOP;
            #endif
            u8Clk_Mux = CKG_VE_IN_CLK_DI; //CKG_VE_IN_CLK_MPEG0;
...
void Hal_VE_set_mux(MS_U8 u8clk, MS_U8 u8data)
{
    ...
    MDrv_Write2ByteMask(REG_SC_BK20_03_L, 0x0002, 0x0002); // set source from main
    MDrv_Write2ByteMask(REG_SC_BK57_2F_L, 0x0000, 0x8000); // set source from DI
    MDrv_Write2ByteMask(REG_SC_BK57_7F_L, 0x6000, 0x6000); // set 422 to 444
}
...
void Hal_VE_Set_PreHSD(MS_VE_INPUT_SRC_TYPE eInputSrcType,MS_U16 *u16In_Hsize,MS_U16 *u16In_Vsize)
{
    ...
    if(eInputSrcType == MS_VE_SRC_DTV_FROM_MVOP &&(*u16In_Hsize> 1920)) // for MOVP 4k2k source will be 2P mode
    {
        bEnable = TRUE;
    }
    else if(eInputSrcType == MS_VE_SRC_SCALER)
    {
        bEnable = TRUE;
    }
    ...
    {
        MApi_XC_W2BYTEMSK(_SC_PK_L_(0x0D,0x22), 0, BIT(15)|BIT(8)|BIT(11));
    }
}
