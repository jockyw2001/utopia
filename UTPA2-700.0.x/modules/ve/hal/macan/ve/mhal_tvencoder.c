...
void Hal_VE_SelMIU(MS_U8 u8MIU_Seltype)
{
    ...
    {
        MDrv_MIU_SelMIU(MIU_CLIENT_VE_RW,MIU_SELTYPE_MIU3);
    }
    else if (u8MIU_Seltype == E_CHIP_MIU_2)
    {
        MDrv_MIU_SelMIU(MIU_CLIENT_VE_RW,MIU_SELTYPE_MIU2);
    }
    else if (u8MIU_Seltype == E_CHIP_MIU_1)
    {
        MDrv_MIU_SelMIU(MIU_CLIENT_VE_RW,MIU_SELTYPE_MIU1);
    }
    else
    {
        MDrv_MIU_SelMIU(MIU_CLIENT_VE_RW,MIU_SELTYPE_MIU0);
    }
}
...
void Hal_VE_set_inputsource(PMS_VE_InputSrc_Info pInputSrcInfo)
{
    ...
    switch(pInputSrcInfo->eInputSrcType)
    {
        case MS_VE_SRC_DTV:
            ...
            #endif
            u8Clk_Mux = CKG_VE_IN_EDCLK; //CKG_VE_IN_CLK_MPEG0;
...
void Hal_VE_Set_PreHSD(MS_VE_INPUT_SRC_TYPE eInputSrcType,MS_U16 *u16In_Hsize,MS_U16 *u16In_Vsize)
{
    ...
    {
        *u16In_Hsize /= 2;
#if 0 //Macan 2Pto1P hw bug, not supported.
...
void Hal_VE_Set_Mirror_Mode(MS_VE_MirrorStatus eMirrorMode)
{
    if(eMirrorMode == MS_VE_HMirrorMode || eMirrorMode == MS_VE_HVBothMirrorMode)
    {
        MDrv_Write2ByteMask(L_BK_VE_SRC(0x1A), BIT(14) , BIT(14)); //H mirror
    }

    if(eMirrorMode == MS_VE_VMirrorMode || eMirrorMode == MS_VE_HVBothMirrorMode)
    {
        MDrv_Write2ByteMask(L_BK_VE_SRC(0x1A), BIT(15) , BIT(15)); //V mirror
    }

    if(eMirrorMode == MS_VE_NoneMirror)
    {
        MDrv_Write2ByteMask(L_BK_VE_SRC(0x1A), 0 , (BIT(15)| BIT(14))); //Non mirror
    }
}
