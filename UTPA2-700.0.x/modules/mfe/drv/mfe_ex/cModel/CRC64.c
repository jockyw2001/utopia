...
#pragma optimize( "", off )
#endif
static void CRC64_BIT(BitArray* D, BitArray* X_next, MS_S32 interface_mode);
...
void CRC_Generator(CRC_INFO* pInfo)
{
	...
	if (pInfo->nLastBytes!=0) {
		MS_S32 nByteToFill = pInfo->nBufSize<8-pInfo->nLastBytes ? pInfo->nBufSize : 8-pInfo->nLastBytes;
