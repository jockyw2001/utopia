...
#define TAG_IRQ "IRQ"
#define IRQ_HAL_ERR(x, args...)        {ULOGE(TAG_IRQ, x, ##args);}

#if defined (__mips__)
#define mtspr(spr, value)  ULOGI(TAG_IRQ, "[NIY] mtspr in line:%s %d\n",__FILE__, __LINE__);
#define mfspr(spr)		   ULOGI(TAG_IRQ, "[NIY] mfspr in line:%s %d\n",__FILE__, __LINE__);
...
{
    type = type;
    IRQ_HAL_ERR("[%s][%d] has not implemented yet\n", __FUNCTION__, __LINE__);
#if 0
    switch (type)
    {
