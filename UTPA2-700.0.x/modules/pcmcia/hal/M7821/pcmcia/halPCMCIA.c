...
static void _MSPI_Set_Bank_base( void )
{
    u8MSPICmd[0] = MSPI_CMD_CFG_W;
    u8MSPICmd[1] = 0x02;
    u8MSPICmd[2] = 0x10; // bank = 0x10xxxx
    MDrv_MSPI_SlaveEnable(TRUE);   
    MDrv_MSPI_Write(u8MSPICmd, 3);
...
void HAL_PCMCIA_Write_Byte(MS_VIRT ptrAddr, MS_U8 u8Val)
{
    ...
    MDrv_MSPI_SlaveEnable(TRUE);   
    MDrv_MSPI_Write(u8MSPICmd, 4);
    MDrv_MSPI_SlaveEnable(FALSE);    
}

void HAL_PCMCIA_Write_ByteMask(MS_VIRT ptrAddr, MS_U8 u8Val, MS_U8 u8Mask)
{
    MS_U8 u8reg = HAL_PCMCIA_Read_Byte(ptrAddr) & ~u8Mask;
    u8Val &= u8Mask;
    u8reg |= u8Val;
...
void   HAL_PCMCIA_ClkCtrl(MS_BOOL bEnable)
{
    if(bEnable)
    {
        ...
        _MSPI_Set_Bank_base();
        HAL_PCMCIA_Write_ByteMask(REG_CHIPTOP_ALLPAD_IN, ALL_PAD_IN_DISABLE, ALL_PAD_IN_MASK);
        HAL_PCMCIA_Write_ByteMask(REG_CHIPTOP_PCM_EN, PCM_ENABLE, PCM_ENABLE_MASK);
        HAL_PCMCIA_Write_Byte(REG_CHIPTOP_PCM_PE, PE_ENABLE);
        
        HAL_PCMCIA_Write_ByteMask(REG_HST0_IRQ_MASK_15_0, IRQ_UNMASK, REG_PCM_IRQ_BIT_MASK);
...
void   HAL_PCMCIA_MaskInt(MS_U32 bits, MS_BOOL bMask)
{
    ...
    HAL_PCMCIA_Write_ByteMask(REG_HST0_IRQ_MASK_15_0, u8Tmp2, REG_PCM_IRQ_BIT_MASK);
}
