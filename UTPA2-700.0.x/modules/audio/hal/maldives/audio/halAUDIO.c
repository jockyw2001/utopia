...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock[] =
{
    ...
    {0x160348, 0xff, 0x70},
    {0x160349, 0xff, 0x86},
    {0x160346, 0xff, 0x48},  //protect for old MBBOT setting.

    {0x112CB1, 0x03, 0x00},  // DSP CLK select DFS from AUPLL_KF0 = 368.64MHz
    {0x112CB3, 0x03, 0x00},  // AU_R2 CLK select AUR2_PLL
    ...
    {0x112B55, 0xFF, 0x01},
    {0x112B52, 0xFF, 0x0F},// DC offset ENABLE, Dither ENABLE
    {0x112B53, 0xFF, 0xF0},
    ...
    {0x163D03, 0x60, 0x20},  // auR2 Use MIU0
    {0x163D07, 0x60, 0x20},  // auR2_1 Use MIU0
    {0x163D88, 0xFF, 0x00},  // unmask DEC-R2 miu_arb high priority
    {0x163DA8, 0xFF, 0x00},  // unmask DEC-R2 miu_arb high priority
    {0x112F68, 0xFF, 0x00},  // unmask dma miu_arb high priority
    {0x112F88, 0xFF, 0x00},  // unmask chche miu_arb high priority
    ...
    {0x112AA2, 0x40, 0x40},  //IRQ2n select SPDIF_TX
    {0x112AA2, 0x10, 0x00},  //IRQ1L select SPDIF_TX2
    {0x112BE1, 0x01, 0x01},
    {0x112BE2, 0x02, 0x02},
    {0x112BE2, 0x04, 0x04},
    {0x112BE2, 0x01, 0x00},
...
const AUDIO_REG_TYPE  AudioInitTbl_0[]=
{
    ...
    {0xFFFFFF, 0x01, 0x32},     // delay 50m sec

    {0x112CEE, 0xFF, 0x01},
    {0x112CEF, 0xFF, 0x13},

    {0xFFFFFF, 0x01, 0x0A},     // delay 10m sec
...
// item 24 for second boot to reduce pop noise
const AUDIO_REG_TYPE  AudioInitTbl_2[]=
{
    //----------------Audio Reset---------------------------//Item 24
    {0x112C00, 0xFF, 0x00},
    {0x112C01, 0xFF, 0x7F},
    {0x112C00, 0xFF, 0x0C},// CLKGEN_RESET
    {0x112C01, 0xFF, 0x7F},
    {0x112C00, 0xFF, 0x00},
    {0x112C01, 0xFF, 0x7F},
    {0x112C00, 0xFF, 0x00},
...
const AUDIO_REG_TYPE  AudioInitTbl_3[]=
{
    ...
    {0x112C6C, 0x80, 0x00},
    {0x101A02, 0xFF, 0xA3},      // HDMI:Select CTS, for MIK 7256 480P mode
    {0x101A03, 0xFF, 0x00},
...
const AUDIO_REG_TYPE  AudioInitTbl_4[]=
{
    ...
    {0x112CDA, 0xFF, 0xE8},
    {0x112CDB, 0xFF, 0x03},
    {0x112CF2, 0xFF, 0x21},      // 28 nm ADC not invert
    {0x112CF3, 0xFF, 0x00},
    {0x112CEE, 0x20, 0x00},
...
const AUDIO_REG_TYPE  AudioInitTbl_5[]=
{
    ...
    {0x112C9B, 0x01, 0x01},      // enable CH8HW force mute   @@VVV
    {0x112F68, 0xFF, 0x00},      //unmask high priority in local dma arbitor
    {0x112F88, 0xFF, 0x00},      //unmask high priority in local cache arbitor
...
MS_BOOL HAL_AUDIO_ResetDefaultVars(AUDIO_SHARED_VARS2 * pAUDIOShared)
{
    ...
    pAUDIOShared->g_u8AdvSndEffType = AU_DVB_STANDARD_INVALID;
...
void  HAL_AUDIO_WriteInitTable(void)
{
    ...
    // check 0x112CEE bit 8 to make sure audio has initialized or not
...
void HAL_AUDIO_SetInternalPath(AUDIO_INTERNAL_PATH_TYPE u8Path,  AUDIO_OUTPUT_TYPE u8Output)
{
    ...
    HAL_MAD2_Write_DSP_sram(DSP2PmAddr_SPK_OutChannel, g_AudioVars2->AudioPathInfo.SpeakerOut, DSP_MEM_TYPE_PM);
    HAL_MAD2_Write_DSP_sram(DSP2PmAddr_HP_OutChannel, g_AudioVars2->AudioPathInfo.HpOut, DSP_MEM_TYPE_PM);
                ...
                case E_AUDIO_INFO_HDMI_IN:
                {
                    if (bIsNonPCMInDec2)
                    {
                        if ((HAL_MAD_GetAC3PInfo(Audio_AC3P_infoType_hdmiAC3inSE)) &&
                            (u8Dsp2CodeType == AU_DVB2_STANDARD_AC3P_AD))
                        {
...
void HAL_AUDIO_SPDIF_SetMode(MS_U8 u8Spdif_mode, MS_U8 u8Input_src)
{
    ...
    {
        if ((u8MainDspCodeType == AU_DVB_STANDARD_DTS) ||
            (u8MainDspCodeType == AU_DVB_STANDARD_DTSLBR))
        {
...
void HAL_AUDIO_SetDspBaseAddr(MS_U8 u8Index, MS_U32 u32Bin_Base_Address, MS_U32 u32Mad_Base_Buffer_Adr)
{
    ...
    printf ("In %s, line(%d), u8Index(0x%x), u32Bin_Base_Address(0x%lx), u32Mad_Base_Buffer_Adr(0x%lx)\r\n", __FUNCTION__, __LINE__, u8Index, u32Bin_Base_Address, u32Mad_Base_Buffer_Adr);
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_SetAdvSndEffType()
/// @brief \b Function \b Description:  This function is used to set the AdvSndEff type.
/// @param <IN>        \b NONE    :
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b MS_U8: DSP code type.
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_SetAdvSndEffType(MS_U8 u8Type)
{
    HALAUDIO_CHECK_SHM_INIT;

    g_AudioVars2->g_u8AdvSndEffType = u8Type;
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_GetAdvSndEffType()
/// @brief \b Function \b Description:  This function is used to get the AdvSndEff type..
/// @param <IN>        \b NONE    :
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b MS_U8: DSP code type.
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
MS_U8 HAL_AUDIO_GetAdvSndEffType(void)
{
    HALAUDIO_CHECK_SHM_INIT;

    return g_AudioVars2->g_u8AdvSndEffType;
}
...
{
    MS_U8 *bufptr = (MS_U8 *)MS_PA2KSEG1((MS_U32)buffer);
...
MS_BOOL HAL_AUDIO_AlgReloadCode(MS_U8 u8Type)
{
    ...
    bRet = HAL_AUDSP_DspLoadCode(u8Type);
    if ( bRet == FALSE )
    {
...
{
    MS_BOOL ret_status = FALSE;
        void   * pAudio_alg_info = NULL;

        if ( id == AUDIO_DSP_ID_DEC )
        {
            if ( code_type == HAL_AUDIO_GetDspCodeType() )
            {
                HALAUDIO_ERROR ("same audio format, don't need to reload !!\r\n");
                return TRUE;
            }
        }
        else if ( id == AUDIO_DSP_ID_SND )
        {
            if ( code_type == HAL_AUDIO_GetDsp2CodeType() )
            {
                HALAUDIO_ERROR ("same audio format, don't need to reload !!\r\n");
                return TRUE;
            }
        ...
        {
            HALAUDIO_ERROR ("Wrong DSP id !!\r\n");
        }

        pAudio_alg_info = HAL_AUDSP_Get_Alg_Info((MS_U8) id, (MS_U8) code_type);

    if ( pAudio_alg_info )
    {
        if ( (ret_status = HAL_AUDIO_AlgReloadCode2(id, code_type, pAudio_alg_info)) == TRUE )
        {
            if ( id == AUDIO_DSP_ID_DEC )
            {
                HAL_AUDIO_SetDspCodeType((MS_U8) code_type, ret_status);
            }
            else if ( id == AUDIO_DSP_ID_SND )
            {
                HAL_AUDIO_SetDsp2CodeType((MS_U8) code_type, ret_status);
            }
            else
            {
                HALAUDIO_ERROR ("Wrong DSP id !!\r\n");
...
MS_BOOL HAL_AUDIO_SetDecodeSystem(AUDIO_DEC_ID DecId, AudioDecStatus_t *p_AudioDecStatus)
{
    ...
    if ((p_AudioDecStatus->eAudFormat & AU_STANDARD_SIF_TYPE) == AU_STANDARD_SIF_TYPE)
    {
