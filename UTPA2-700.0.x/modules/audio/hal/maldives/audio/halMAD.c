...
MMA_AudioType _GetAudioCaps(AUDIO_DEC_ID eDecId)
{
    ...
    else if ( eDecId == AU_DEC_ID3 )
    {
        eAudioCaps = Audio_DEC_PCM        | Audio_DEC_MPEG     |
                     ...
                     Audio_DEC_AMR_WB     | Audio_DEC_VORBIS   |
                     Audio_DEC_DTS        | Audio_DEC_FLAC     |
                     Audio_DEC_Dolby_TrueHD_Bypass_R2   |Audio_DEC_DTS_HD_ADO_R2    |
                     Audio_DEC_DRA;
    }
...
MS_BOOL HAL_MAD_MM2_inputAesFinished( AUDIO_DEC_ID dec_id, MS_U32 es_size, MS_BOOL ptsExist, MS_U64 pts )
{
    ...
    if ( es_size > audio_aes_info[0].aes_freeSpace )
        {
        printf("==> Error, input AES size(0x%lx) is larger than current Free space(0x%lx)\r\n", es_size, audio_aes_info[0].aes_freeSpace );
        return FALSE;
        }
    ...
    switch(infoType)
    {
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_Set_DFS_PowerSave:
        {
            ...
            MS_U8 u8MainDspCodeType = g_AudioVars2->g_u8DspCodeType;
            if(param1)
            {
...
MS_BOOL HAL_MAD_SetWmaInfo( Audio_WMA_infoType infoType, MS_U32 param1, MS_U32 param2 )
{
    switch(infoType)
    {
        ...
        default:
            printf("%s: infoType[%x,%lx,%lx] not Supported\n",__FUNCTION__, infoType, param1, param2);
            break;
    }
...
MS_U32 HAL_MAD_GetAACInfo( Audio_AAC_infoType infoType )
{
    ...
    MS_U8 u8MainDspCodeType = 0;

    HALMAD_CHECK_SHM_INIT;

    u8MainDspCodeType = g_AudioVars2->g_u8DspCodeType;
    ...
    switch ( paramType )
    {
        ...
        case Audio_ParamType_FreeDecoder:
                    ...
                    (g_AudioVars2->AudioDecStatus[DecId].eSourceType == E_AUDIO_INFO_GAME_IN))
            {
                    ...
                    if ( DecId == AU_DEC_ID1 )
                {
                        g_AudioVars2->g_u8DspCodeType = AU_DVB_NONE;
                    }
    ...
    switch ( infoType )
    {
        ...
        case Audio_infoType_Decoder_Type:
        {
            ...
            switch (adec_id)
            {
                case ADEC1:
                {
                    eDecoder = g_AudioVars2->g_u8DspCodeType;
                    break;
                }

                case ADEC2:
                {
                    eDecoder = g_AudioVars2->g_u8Dsp2CodeType;
                    break;
                }
...
MS_S32 HAL_MAD_GetCodecCapability(AUDIO_CODEC_CAPABILITY_t * pCodecCapability)
{
    ...
    switch (pCodecCapability->CodecType)
    {
        ...
        default:
        {
            HALMAD_ERROR("%s: Error! Unsupportted CodecType %ld! \n", __FUNCTION__, pCodecCapability->CodecType);
            s32Ret = -A_EINVAL;
            break;
        }
