...
void HAL_MAD2_SetMemInfo(void)
{
    ...
    HALMAD2_PRINT("HAL_MAD2_SetMemInfo : SE_DSP_DDR_ADDR = 0x%08lX, DEC_R2_ADDR = 0x%08lX\n", SE_DSP_DDR_ADDR, DEC_R2_ADDR);
  ...
  ********************************************************/
    ...
    HALMAD2_PRINT(" ======> Audio DEC_R2 Base : %lx\n", DEC_R2_ADDR);
...
void HAL_MAD2_SetDecCmd(MS_U16 u16DecCmd)
{
    ...
    if (u16DecCmd & 0x0100)
    {
        if ((HAL_AUDIO_GetDsp2CodeType() & 0xF0) == AU_STANDARD_SIF_TYPE) // whether current decoder at MAD2 is sif?
        {
            // SIF command
