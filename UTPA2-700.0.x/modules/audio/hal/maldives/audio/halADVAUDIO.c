...
DBXTV_DM TotSurDM[1];
DBXTV_PM TotSurPM[1];
static AUDIO_OUTPUT_TYPE ADVSND_PATH = AUDIO_I2S_OUTPUT;
...
MS_BOOL HAL_ADVSOUND_ProcessEnable(AUDIO_DSP_CODE_TYPE type)
{
    ...
    MS_BOOL DSPReload_status = FALSE;
    MS_U8 AdvSndEffType = AU_DVB_STANDARD_INVALID;
    MS_U8 AdvSndEffDSPCodeType = AU_DVB_STANDARD_INVALID;

    switch (type)
    {
        case AU_DVB2_ADVSND_SRS:
        case AU_DVB2_ADVSND_TSHD:
        case AU_DVB2_ADVSND_BONGIOVI_DPS:
            if( type != HAL_AUDIO_GetAdvSndEffDspCodeType() )
            {
                DSPReload_status = HAL_AUDIO_Alg2ReloadCode(type);
                HAL_AUDIO_WriteMaskReg(REG_SOUND_ADV_CFG2, 0xFFFF, 0x0000);        //Reset the advsnd mode setting
            }
            else
            {
                DSPReload_status = TRUE;
            }

            if( DSPReload_status == TRUE)
            {
                AdvSndEffDSPCodeType = type;
                AdvSndEffType = type;

                HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG1, 0x7F, AdvSndEffType);
                HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG1,0x80,0x80);
            }
            status = TRUE;
            break;
        ...
        case AU_DVB2_ADVSND_PURESND:
            ...
            if( DSPReload_status == TRUE)
            {
                ...
                HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG1, 0x7F, AdvSndEffType);
                HAL_SOUND_ADVSND_VOL_Enable(type, TRUE);
            }
            status = TRUE;
            break;
			
        case AU_DVB2_ADVSND_MAX:
            AdvSndEffDSPCodeType = HAL_AUDIO_GetAdvSndEffDspCodeType();

            if((AdvSndEffDSPCodeType == AU_DVB2_ADVSND_DBX) || (AdvSndEffDSPCodeType == AU_DVB2_ADVSND_PURESND))
            {
                HAL_SOUND_ADVSND_VOL_Enable(AdvSndEffDSPCodeType, FALSE);
            }
            HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG1,0x80,0x00);
            DSPReload_status = TRUE;
            AdvSndEffType = type;
        ...
        default:
            if((HAL_AUDIO_GetAdvSndEffDspCodeType() == AU_DVB2_ADVSND_DBX) || (HAL_AUDIO_GetAdvSndEffDspCodeType() == AU_DVB2_ADVSND_PURESND))
            {
                HAL_SOUND_ADVSND_VOL_Enable(HAL_AUDIO_GetAdvSndEffDspCodeType(), FALSE);
            }
            printf("\n\033[0;34m-----------Error : Unknown Advance Sound Effect Type=%d !!!------------\033[0m \n", type);
            break;
    ...
    if(status == TRUE)
    {
        if(DSPReload_status == FALSE)
        {
            HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG1, 0xFF, 0x00);
            printf("\n\033[0;34m-----------Error : Reload ADV DSP Code Fail !!! type=%d------------\033[0m \n", type);
        }
        HAL_AUDIO_SetAdvSndEffDspCodeType(AdvSndEffDSPCodeType, DSPReload_status);
        HAL_AUDIO_SetAdvSndEffType(AdvSndEffType);
    }
...
MS_BOOL  HAL_SOUND_ADVSND_VOL_Enable(AUDIO_DSP_CODE_TYPE type, MS_BOOL u8en)
{
    MS_U8 tmp_volume;

    if (u8en)
    {
        if(ADVSND_PATH == AUDIO_I2S_OUTPUT)
        {
            tmp_volume = HAL_AUDIO_ReadByte(REG_SOUND_I2S_VOLUME);
            HAL_AUDIO_WriteMaskByte(REG_SOUND_I2S_VOLUME, 0x80, 0x80);          //mute
            MsOS_DelayTask(20);
            if(type == AU_DVB2_ADVSND_PURESND)
            {
                HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG4,0x01,0x01);
            }
        ...
        else if(ADVSND_PATH == AUDIO_AUOUT0_OUTPUT)
        {
        ...
        else if(ADVSND_PATH == AUDIO_AUOUT1_OUTPUT || ADVSND_PATH == AUDIO_HP_OUTPUT)
        {
        ...
        else if(ADVSND_PATH == AUDIO_AUOUT2_OUTPUT)
        {
        ...
        else if(ADVSND_PATH == AUDIO_AUOUT3_OUTPUT)
        {
    ...
    else
    {
        if(ADVSND_PATH == AUDIO_I2S_OUTPUT)
        {
            ...
            if(type == AU_DVB2_ADVSND_PURESND)
            {
                HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG4,0x01,0x00);
            }
            else if(type == AU_DVB2_ADVSND_DBX)
            {
