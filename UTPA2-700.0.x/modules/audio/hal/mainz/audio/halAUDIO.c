...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock[] =
{
  ...
  //  {0x0C01, 0xFF, 0x10},
    ...
    {0x112C70, 0xFF, 0xD4},// Cathy need check
    {0x112C71, 0xFF, 0x12},
    {0x112C72, 0xFF, 0x40},
    {0x112C73, 0xFF, 0x04},
    {0x112C74, 0xFF, 0x04},
    ...
    {0x160344, 0xFF, 0x00},
    {0x160345, 0xFF, 0x01},
    {0x160346, 0xFF, 0x12},  // DSP clk = 0x12 * 24 = 432MHz
    {0x160347, 0xFF, 0x00},
    {0x160348, 0xFF, 0x70},
    {0x160349, 0xFF, 0x84},
    {0x160342, 0xFF, 0x00},
    ...
    {0x112CB1, 0x03, 0x00}, // DSP CLK select DFS from AUR2PLL
    {0x163C61, 0x10, 0x10}, //DSP DFS select
    {0x163C61, 0x03, 0x01}, //DSP DFS source = AUR2PLL
    {0x163C61, 0x08, 0x08}, //DSP DFS clock enable
    {0x163C60, 0x40, 0x40}, //DSP DFS block enable
    ...
    {0x112CA4, 0xFF, 0xFF},
    {0x112CA5, 0xFF, 0x1F},
    {0x112CB3, 0x60, 0x60}, // Enable CLK R2
    ...
    //-----------------AudioBand----------------------------//Item 21
    {0x112B58, 0x30, 0xFF}, // DWA fix value
    {0x112B59, 0xFF, 0x01},
    ...
    {0x112B51, 0xFF, 0x00},
    {0x112B10, 0x80, 0x80}, // DC offset value for earphone 9LEVEL SDM
    ...
    {0x163D88, 0xFF, 0x00},  // unmask DEC-R2 miu_arb high priority
    {0x112F68, 0xFF, 0x00},  // unmask dma miu_arb high priority
    {0x112F88, 0xFF, 0x00},  // unmask cache miu_arb high priority

    {0x112A80, 0x80, 0x00},  // uDSP boot from MCU

    #ifdef CONFIG_MBOOT
    {0xFFFFFF, 0x01, 0x80},  // For Mboot power on sequence
    #else
...
const AUDIO_REG_TYPE  AudioInitTbl_1[]=
{
    ...
    //-----------------Synthesizer--------------------------//Item 29
    {0x112BCF, 0x01, 0x01},  // DVB1 N.F. by R2/DSP
    {0x112BCE, 0x01, 0x01},
    {0x112BA8, 0xFF, 0x94},  // N.f[30:16]
    {0x112BA9, 0xFF, 0x11},  // N.f[15:0]
    {0x112BAA, 0xFF, 0x00},
    {0x112BAB, 0xFF, 0x00},
    {0x112C60, 0x80, 0x80},
    {0x112BCE, 0x01, 0x00},
    ...
    {0x112BF2, 0x20, 0x20},  // ParserA selects AFIFO2
    {0x112BF3, 0x08, 0x08},  // ParserC selects AFIFO0

    //-----------------Input: HDMI (only Vivaldi side)------//Item 1
    {0x112C6C, 0x80, 0x00},
    {0x111A02, 0xFF, 0xA3},// HDMI:Select CTS, for MIK 7256 480P mode
    {0x111A03, 0xFF, 0x00},
    {0x112C44, 0xFF, 0x10},// HDMI Matrix mapping
    {0x112C45, 0xFF, 0x10},
    {0x112C46, 0xFF, 0x10},// HDMI Matrix mapping
    {0x112C47, 0xFF, 0x10},
    {0x112CA5, 0x40, 0x40},
    {0x112C03, 0x40, 0x40},
    {0x112C48, 0x08, 0x08},
...
const AUDIO_REG_TYPE  AudioInitTbl_2[]=
{
    //-----------------Input: ADC---------------------------//Item 4
    ...
    {0x112CDA, 0xFF, 0xE8},
    {0x112CDB, 0xFF, 0x03},
    {0x112CF2, 0xFF, 0x21},  // ADC set not invert
    {0x112CF3, 0xFF, 0x00},
    {0x112CEE, 0x20, 0x00},
...
const AUDIO_REG_TYPE  AudioInitTbl_3[]=
{
    //-----------------Decode Path--------------------------//Item 28
    {0x112C60, 0xFF, 0x82},// Decoder 1 source from DVB1
    {0x112C61, 0xFF, 0x82},// Decoder 3 source from DVB3
    {0x112C62, 0xFF, 0x82},// Decoder 2 source from DVB2
    {0x112C63, 0xFF, 0x87},// Decoder 4 source from SIF

    //-----------------Input: Channel Source----------------//Item 6
    {0x112C64, 0xFF, 0x82},// CH1: Source0 , CH5: ADC0
    {0x112C65, 0xFF, 0x82},// CH5: default input source = decoder (avoid data output directly before F/W executing SourceSwitch)
    {0x112C66, 0xFF, 0x82},// CH2: Source0 , CH6: Source1
    {0x112C67, 0xFF, 0x81},
    {0x112C68, 0xFF, 0x82},// CH3: Source0 , CH7: Source3
    {0x112C69, 0xFF, 0x83},
    {0x112C6A, 0xFF, 0x82},// CH4: Source0 , CH8: Source7
    {0x112C6B, 0xFF, 0x87},
...
MS_BOOL HAL_AUDIO_ResetDefaultVars(AUDIO_SHARED_VARS2 * pAUDIOShared)
{
    ...
    pAUDIOShared->g_debug_level = AUDIO_DEBUG_LEVEL_WARN;
#elif defined(CONFIG_MBOOT)
    pAUDIOShared->g_debug_level = AUDIO_DEBUG_LEVEL_ERROR;
#else
    pAUDIOShared->g_debug_level = AUDIO_DEBUG_LEVEL_INFO;
#endif
...
AUDIO_SHARED_VARS2 * HAL_AUDIO_AllocateVars(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Start %s InitShmFlag=%d\r\n", __FUNCTION__, g_bInitShmFlag);
    ...
    if (MsOS_SHM_GetId((MS_U8 *)AUDIO_SHM_NAME, sizeof(AUDIO_SHARED_VARS2), &u32ShmId, (MS_VIRT *)&virtAddr, &u32BufSize, MSOS_SHM_QUERY) == TRUE)
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s QUERY ClientCounter=%u\r\n", __FUNCTION__, pAUDIOShared->g_u32ClientCounter);

        if (pAUDIOShared->g_u32ClientCounter == 0)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Resume from Suspend mode, reset Audio SHM data ...\n");
        }
    }
    else
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "create Audio SHM data ...\n");
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Audio SHM data is created\n");
            pAUDIOShared = (AUDIO_SHARED_VARS2 *) virtAddr;

            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "reset Audio SHM data ...\n");
            HAL_AUDIO_ResetDefaultVars(pAUDIOShared);
        }
        else
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "AUDIO SHM allocation failed!\n");
            return NULL;
        }
    }
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "End %s InitShmFlag=%d, ClientCounter=%u \r\n", __FUNCTION__, g_bInitShmFlag, pAUDIOShared->g_u32ClientCounter);
    ...
    if (g_bInitShmFlag == FALSE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "create Audio SHM data ...\n");
        g_bInitShmFlag = TRUE;
...
MS_BOOL HAL_AUDIO_DeAllocateVars(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Start HAL_AUDIO_DeAllocateVars g_bInitShmFlag=%d\r\n", g_bInitShmFlag);
    ...
    if (g_bInitShmFlag == TRUE)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : No AUDIO SHM is created!\n", __FUNCTION__);
            return FALSE;
        }
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "HAL_AUDIO_DeAllocateVars ClientCounter=%u\r\n", pAUDIOShared->g_u32ClientCounter);

        if (pAUDIOShared->g_u32ClientCounter == 0)
        {
            HAL_AUDIO_ResetDefaultVars(pAUDIOShared);

            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "free Audio SHM data ...\n");
#if defined(MSOS_TYPE_LINUX)
            if (MsOS_SHM_FreeId((MS_U8 *)AUDIO_SHM_NAME, u32ShmId) == TRUE)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Audio SHM data is freed\n");
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : AUDIO SHM de-allocation failed!\n", __FUNCTION__);
                return FALSE;
            }
#endif
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "End HAL_AUDIO_DeAllocateVars g_bInitShmFlag=%d\r\n", g_bInitShmFlag);
#else
    pAUDIOShared = &g_audioShared;

    if (g_bInitShmFlag == TRUE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "free Audio SHM data ...\n");
        g_bInitShmFlag = FALSE;
...
MS_BOOL HAL_AUDIO_InitialVars(void)
{
    if (HAL_AUDIO_GET_INIT_FLAG() == FALSE)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Create Audio TEE INFO SHM data fail...\n");
            return FALSE;
        }
    }
#else
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "MAD Register MBX MSG error\n");
               return FALSE;
            }
            else
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "MAD MBX register msg ok %d\n", audio_mbx_class);

            _s32MadEventId = MsOS_CreateEventGroup("MAD_Event");
            if (_s32MadEventId < 0)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "MAD CreateEventGroup error....\n");
                return FALSE;
            }
            ...
            if (_s32MadTaskId < 0)
            {
                ...
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "MAD CreateTask error....\n");
                return FALSE;
            }
        }
    }
...
MS_BOOL HAL_AUDIO_DeInitialVars(void)
{
    ...
    if (HAL_AUDIO_GET_INIT_FLAG() == TRUE)
    {
...
#ifndef MSOS_TYPE_NUTTX
        {
            ...
            if (ret1 == FALSE)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : AUDIO TEE Info SHM de-allocation failed!\n", __FUNCTION__);
                ret = FALSE;
            }
        }
#endif
...
MS_BOOL HAL_AUDIO_Mutex_Init(void)
{
    ...
    if (_s32AUDIOMutex == -1)
    {
        ...
        if (_s32AUDIOMutex == -1)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Create Mutex failed!\n");
            ret = FALSE;
        }
    }
    ...
    if (_s32AUDIOMutexReboot == -1)
    {
        ...
        if (_s32AUDIOMutexReboot == -1)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Create Mutex for Reboot failed!\n");
            ret = FALSE;
        }
    }
    ...
    if (_s32MutexLoadCode == -1)
    {
        ...
        if (_s32MutexLoadCode == -1)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Create Mutex for Load DSP failed!\n");
            ret = FALSE;
        }
    }
    ...
    if (_s32AUDIOMutexIDMA == -1)
    {
        ...
        if (_s32AUDIOMutexIDMA == -1)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Create Mutex for IMDA failed!\n");
            ret = FALSE;
        }
    }
...
MS_BOOL HAL_AUDIO_Mutex_DeInit(void)
{
    ...
    if (_s32AUDIOMutex != -1)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Delete Mutex failed!\n");
            ret = FALSE;
        }
        _s32AUDIOMutex = -1;
    }
    ...
    if (_s32AUDIOMutexReboot != -1)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Delete Mutex for Reboot failed!\n");
            ret = FALSE;
        }
        _s32AUDIOMutexReboot = -1;
    }
    ...
    if (_s32MutexLoadCode != -1)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Delete Mutex for Load DSP failed!\n");
            ret = FALSE;
        }
        _s32MutexLoadCode = -1;
    }
    ...
    if (_s32AUDIOMutexIDMA != -1)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Error! Delete Mutex for IMDA failed!\n");
            ret = FALSE;
        }
        _s32AUDIOMutexIDMA = -1;
    }
...
MS_U16 HAL_AUDIO_ReadMailBox(MS_BOOL bDspType, MS_U8 u8ParamNum)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Read Mailbox fail! \r\n");
    return 0;
...
void  HAL_AUDIO_WriteInitTable(void)
{
    ...
    // check 0x112CE0 bit12 to make sure audio has initialized or not
    if ((HAL_AUDIO_ReadByte(REG_AUDIO_CODEC_CFG0+1) & 0x10) == 0)
        has_init = 1;
            ...
            if((AudioInitTbl_0[i].u32Addr == 0xFFFFFF))
            {
                if(AudioInitTbl_0[i].u8Mask == 1)
                {
                    if(AudioInitTbl_0[i].u8Value!=0)
                        AUDIO_DELAY1MS((MS_U32)(AudioInitTbl_0[i].u8Value));
                }
            }
            else
            {
        ...
        if((AudioInitTbl_1[i].u32Addr == 0xFFFFFF))
        {
            if(AudioInitTbl_1[i].u8Mask == 1)
            {
                if(AudioInitTbl_1[i].u8Value!=0)
                    AUDIO_DELAY1MS((MS_U32)(AudioInitTbl_1[i].u8Value));
            }
        }
        else
        {
    ...
    i = 0;
    if (has_init != 1) {
        while( !((AudioInitTbl_2[i].u32Addr == 0xFFFFFF) && (AudioInitTbl_2[i].u8Mask == 0)) )
        {
            if((AudioInitTbl_2[i].u32Addr == 0xFFFFFF))
            {
                if(AudioInitTbl_2[i].u8Mask == 1)
                {
                    if(AudioInitTbl_2[i].u8Value!=0)
                        AUDIO_DELAY1MS((MS_U32)(AudioInitTbl_2[i].u8Value));
                }
            }
            else
            {
                HAL_AUDIO_AbsWriteMaskByte(AudioInitTbl_2[i].u32Addr, AudioInitTbl_2[i].u8Mask, AudioInitTbl_2[i].u8Value);
            }
    ...
    i = 0;
    while( !((AudioInitTbl_3[i].u32Addr == 0xFFFFFF) && (AudioInitTbl_3[i].u8Mask == 0)) )
    {
        if((AudioInitTbl_3[i].u32Addr == 0xFFFFFF))
        {
            if(AudioInitTbl_3[i].u8Mask == 1)
            {
                if(AudioInitTbl_3[i].u8Value!=0)
                    AUDIO_DELAY1MS((MS_U32)(AudioInitTbl_3[i].u8Value));
            }
        }
        else
        {
            HAL_AUDIO_AbsWriteMaskByte(AudioInitTbl_3[i].u32Addr, AudioInitTbl_3[i].u8Mask, AudioInitTbl_3[i].u8Value);
        }
...
{
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_AUDIO_SetPowerOn(%x)\r\n", bPower_on);
    if(bPower_on)
    {
    ...
    {
        #if 0 // Apply power save related setting provided by Akuan.Liu.
        // ausdm power down, -70dB
        HAL_AUDIO_WriteMaskByte(0x2CEC, 0xFF, 0x00); //EN_AB=0
        HAL_AUDIO_WriteMaskByte(0x2CED, 0xFF, 0x08);
        AUDIO_DELAY1MS(50);
        HAL_AUDIO_WriteMaskByte(0x2CEE, 0xFF, 0x82); //EN_MSP=0,EN_LTX/RTX_EAR=1,EN_MUTE_EAR=1,SEL_DEPOP_EAR=10
        HAL_AUDIO_WriteMaskByte(0x2CEF, 0xFF, 0x03);
        AUDIO_DELAY1MS(100);
        HAL_AUDIO_WriteMaskByte(0x2CE8, 0xFF, 0x00); //PD_VREF=1, EN_VREF_DIS=1, SFTDCH[1:0]=00
        HAL_AUDIO_WriteMaskByte(0x2CE9, 0xFF, 0xC0);
        AUDIO_DELAY1MS(800);
        HAL_AUDIO_WriteMaskByte(0x2CE6, 0xFF, 0x01); //PD_VI=1
        HAL_AUDIO_WriteMaskByte(0x2CE7, 0xFF, 0x00);
        HAL_AUDIO_WriteMaskByte(0x2CE0, 0xFF, 0x30); //PD_LT_EAR/PD_RT_EAR=1,PD_IBIAS_EAR=1
        HAL_AUDIO_WriteMaskByte(0x2CE1, 0xFF, 0x10);
        HAL_AUDIO_WriteMaskByte(0x2CEA, 0xFF, 0x0C); //VMI2GND=1, PD_OPLP=1
        HAL_AUDIO_WriteMaskByte(0x2CEB, 0xFF, 0x00);
        #endif
        ...
        HAL_AUDIO_WriteMaskByte(0x2B42, 0xFF, 0xFF);      // CH1~7 H/W by-pass
...
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_DecDspISR(void) //Cathy need modify  //@@@no need add pvr
{
...
MS_BOOL HAL_AUDIO_GetPlayFileFlag(MS_BOOL bDspType)
{
    ...
    return g_AudioVars2->g_bSePlayFileFlag;
}
...
void HAL_AUDIO_SetInputPath(AUDIO_INPUT_TYPE input , AUDIO_PATH_TYPE u8Path)
{
    ...
    {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "DSP is not support more than CH8\n");
            return;
    }
    ...
    switch(u8input_src)
    {
        case AUDIO_DSP1_INPUT: //DSP can't output to Decode1 for Monet
...
void HAL_AUDIO_SetInternalPath(AUDIO_INTERNAL_PATH_TYPE u8Path,  AUDIO_OUTPUT_TYPE u8Output)
{
      ...
      switch(u8Output)
    {
        case AUDIO_AUOUT0_OUTPUT:
            //path =0x4 ; // patch for SE not okay version
...
void HAL_AUDIO_FwTriggerDSP(MS_U16 u16Cmd)
{
    ...
    HAL_AUDIO_DecWriteByte(REG_M2S_MAILBOX_PIO_ID, u16Cmd);

    HAL_AUDIO_WriteMaskByte(REG_SE_IDMA_CTRL0, 0x20, 0x00);
    AUDIO_DELAY1MS(1);
    HAL_AUDIO_WriteMaskByte(REG_SE_IDMA_CTRL0, 0x20, 0x20); // Raising edge trigger
    AUDIO_DELAY1MS(2);
    HAL_AUDIO_WriteMaskByte(REG_SE_IDMA_CTRL0, 0x20, 0x00);
}
...
{
    return(HAL_AUDIO_ReadByte(REG_SE_DECODE_CMD));
}
...
void HAL_AUDIO_USBPCM_Enable(MS_BOOL bEnable)
{
    ...
    UNUSED(bEnable);
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : Error! unsupported funcion !!!\n", __FUNCTION__);
}
...
static void HAL_AUDIO_DigitalOut_Set_Encode(AUDIO_ENCODE_TYPE eType, MS_BOOL bEnable)
{
    switch(eType)
    {
        ...
        case MP3_ENCODE:
        default:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() - Choosen Encoder not exist\n",__FUNCTION__);
            break;
    }
}
...
static void HAL_AUDIO_DigitalOut_NPCM_SrcSwitch(DIGITAL_OUTPUT_TYPE ePath, AUDIO_DSP_ID eDspID)
{
    switch (ePath)
    {
        case DIGITAL_SPDIF_OUTPUT:
            {
                switch(eDspID)
                {
                    ...
                    case AUDIO_DSP_ID_DEC:
                        ...
                        break;
                    default:
                        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() - DEC ID %d not exist\n",__FUNCTION__,eDspID);
                        break;
                }
            }
            break;
        ...
        default:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() - Digital Path %d not exist\n",__FUNCTION__,ePath);
            break;
    }
...
static void HAL_AUDIO_DigitalOut_SetDataPath(DIGITAL_OUTPUT_TYPE ePath, DigitalOutSetting_t *stDigitalOutSetting)
{
    ...
    switch (stDigitalOutSetting->eDigitalOutfMode)
    {
        ...
        default:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() - Digital Mode %d not exist\n",__FUNCTION__,stDigitalOutSetting->eDigitalOutfMode);
            break;
    }
...
static MS_BOOL HAL_AUDIO_DigitalOut_DetermineDataPath(DIGITAL_OUTPUT_TYPE ePath, DigitalOutSetting_t *stDigitalOutSegtting)
{
    ...
    u8Dsp1CodeType = g_AudioVars2->g_u8DspCodeType;
    u8Dsp2CodeType = g_AudioVars2->g_u8Dsp2CodeType;

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%s() - eDigitalOutfMode  = %x, eNonPcmPath = %x , Source = %x, R2NonPcmSetting = %x\n\r", \
    __FUNCTION__, \
    ...
    switch (stDigitalOutSegtting->eDigitalOutfMode)
    {
        ...
        case DIGITAL_OUT_DOLBYDIGITAL:
        {
            switch (stDigitalOutSegtting->eSourceType)
            {
                case E_AUDIO_INFO_HDMI_IN:
                {
                    if (bIsNonPCMInDec2)
                    {
                        if ((HAL_MAD_GetAC3PInfo(Audio_AC3P_infoType_hdmiAC3inSE)) &&
                            (u8Dsp2CodeType == AU_DVB_STANDARD_AC3 || u8Dsp2CodeType == AU_DVB_STANDARD_AC3P))
                        {
                            stDigitalOutSegtting->u8NonPcmPath = AUDIO_DSP_ID_DEC; /* DSP2 */
                        }
                    }
                    else if ((u8Dsp1CodeType == AU_DVB_STANDARD_AC3) ||
                        (u8Dsp1CodeType == AU_DVB_STANDARD_AC3P))
                    {
                ...
                {
                    u8SourceDspCodeType = u8Dsp1CodeType;
                    ...
                    switch(u8SourceDspCodeType)
                    {
                        ...
                        case AU_DVB_STANDARD_MS10_DDT:
                        {
                            ...
                            {
                                if (HAL_MAD2_Read_DSP_sram(DSP1PmAddr_smpRate, DSP_MEM_TYPE_PM) == 44100)
                                {
                                    stDigitalOutSegtting->eDigitalOutfMode = DIGITAL_OUT_PCM;
                                }
                                else
        ...
        case DIGITAL_OUT_DTS:
        {
            switch (stDigitalOutSegtting->eSourceType)
            {
                case E_AUDIO_INFO_HDMI_IN:
                {
                    if (bIsNonPCMInDec2)
                    {
                        if (u8Dsp2CodeType == AU_DVB2_STANDARD_DTS)  // Need check
                        {
                            stDigitalOutSegtting->u8NonPcmPath = AUDIO_DSP_ID_SND; /* DSP2 */
                        }
                    }
                    else if (u8Dsp1CodeType == AU_DVB_STANDARD_DTS)
                    {
                        stDigitalOutSegtting->u8NonPcmPath = AUDIO_DSP_ID_DEC; /* DSP1 */
                    }
                    break;
...
void HAL_AUDIO_SetDigitalOut(DIGITAL_OUTPUT_TYPE ePath, AUDIO_DIGITAL_OUTPUT_TYPE eDigitalMode)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%s() - eDigitalOutfMode  = %x, eNonPcmPath = %x\n\r", \
                           __FUNCTION__, \
...
MS_BOOL HAL_AUDIO_DigitalOut_SetChannelStatus(DIGITAL_OUTPUT_TYPE ePath, Digital_Out_Channel_Status_t *stChannelStatus)
{
    ...
    switch (ePath)
    {
        ...
        default:
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() - Invalid SPDIF Path\n",__FUNCTION__);
            break;
        }
    }
...
MS_BOOL HAL_AUDIO_DigitalOut_GetChannelStatus(DIGITAL_OUTPUT_TYPE ePath, Digital_Out_Channel_Status_t *stChannelStatus)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : stChannelStatus shouldn't be NULL !!!\n", __FUNCTION__);
    }
    else
    {
...
void HAL_AUDIO_SPDIF_SetMode(MS_U8 u8Spdif_mode, MS_U8 u8Input_src)
{
    ...
    MS_U8 u8SpdifMode = (u8Spdif_mode & SPDIF_OUT_NONPCM) ? SPDIF_OUT_NONPCM : SPDIF_OUT_PCM;
    ...
    u8MainDspCodeType =  g_AudioVars2->g_u8Dsp2CodeType;
    ...
    switch(eAudioSource)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            ...
            if (HAL_MAD_GetDSP2DecCmd() == AU_DVB_DECCMD_STOP)
            {
            ...
            switch(u8MainDspCodeType)
            {
                ...
                case AU_DVB_STANDARD_MS10_DDT:
                {
                    /* Monet doesn't support AAC's DDEnc/DDCO at all, so force to PCM mode */
                    u8SpdifMode  = SPDIF_OUT_PCM;
                    break;
                }
        ...
        default:
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid eAudioSource(%d)!\r\n", __FUNCTION__, eAudioSource);
            break;
        }
    }

    if (((_HAL_AUDIO_SPDIF_GetMode() & SPDIF_OUT_NONPCM) == u8SpdifMode) &&
    ...
    if (u8SpdifMode == SPDIF_OUT_NONPCM)
    {
        ...
        HAL_AUDIO_WriteMaskByte(REG_AUDIO_CH1_CFG, 0x0F, 0x02); /* set CH1 input mux to decode 3 for encoder */
        HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL, 0x0C, 0x08); /* Select DSP2 for SPDIF */
    ...
    else
    {
        ...
        HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL, 0x0C, 0x00); /* Select DSP1 for SPDIF*/
        HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL, 0x2, 0x0); /* PCM Mode */
    }
...
//=======================================================
    ...
    if (bIsNonPCM)  //====== HDMI nonpcm mode =======
    {
     ...
     // Audio Input path config
        ...
        MsOS_DelayTask(10);
        HAL_MAD_SetDSP2DecCmd(AU_DVB_DECCMD_PLAY);      // @@@PlaY


   //========= Set for S/PDIF out ===========
        if ((_HAL_AUDIO_SPDIF_GetMode()&SPDIF_OUT_NONPCM) == SPDIF_OUT_NONPCM)
        {
            HAL_AUDIO_SPDIF_SetMode(SPDIF_OUT_NONPCM, E_AUDIO_INFO_HDMI_IN);
        }
        else
        {
...
void HAL_AUDIO_SetDspBaseAddr(MS_U8 u8Index, MS_PHY phyBin_Base_Address, MS_PHY phyMad_Base_Buffer_Adr)
{
    ...
    {
        case(DSP_SE):
            g_AudioVars2->g_DSPBinBaseAddress[DSP_SE] = 0;                                 //SE ADEC ADDR setting
            g_AudioVars2->g_DSPMadBaseBufferAdr[DSP_SE] = phyMad_Base_Buffer_Adr;
            g_DSPMadMIUBank[DSP_SE] = g_AudioVars2->g_DSPMadMIUBank[u8Index];
            g_DSPBinBaseAddress[DSP_SE] = 0;
            g_DSPMadBaseBufferAdr[DSP_SE] = g_AudioVars2->g_DSPMadBaseBufferAdr[DSP_SE];

            g_AudioVars2->g_DSPBinBaseAddress[DSP_DEC] = 0;                                    //DSP_DEC= Comm ADDR
            g_AudioVars2->g_DSPMadBaseBufferAdr[DSP_DEC] = g_DSPMadBaseBufferAdr[DSP_SE] + DSP_DDR_SIZE;
            g_DSPMadMIUBank[DSP_DEC] = g_AudioVars2->g_DSPMadMIUBank[u8Index];
            g_DSPBinBaseAddress[DSP_DEC] = 0;
            g_DSPMadBaseBufferAdr[DSP_DEC] = g_AudioVars2->g_DSPMadBaseBufferAdr[DSP_DEC];

            g_AudioVars2->g_DSPBinBaseAddress[DSP_ADV] = 0;
            g_AudioVars2->g_DSPMadBaseBufferAdr[DSP_ADV] = phyMad_Base_Buffer_Adr;
            g_DSPMadMIUBank[DSP_ADV] = g_AudioVars2->g_DSPMadMIUBank[u8Index];
            g_DSPBinBaseAddress[DSP_ADV] =0;
            g_DSPMadBaseBufferAdr[DSP_ADV] =  g_AudioVars2->g_DSPMadBaseBufferAdr[DSP_ADV] ;
...
#endif
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%d DSPBinBaseAddress = 0x%08X\n",   u8Index, (unsigned int) g_AudioVars2->g_DSPBinBaseAddress[u8Index]);
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%d DSPMadBaseBufferAdr = 0x%08X\n", u8Index, (unsigned int) g_AudioVars2->g_DSPMadBaseBufferAdr[u8Index]);
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%d DSPMadMIUBank = %d\n",          u8Index, g_AudioVars2->g_DSPMadMIUBank[u8Index]);
}
...
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_SetDspCodeType(MS_U8 u8Type, MS_BOOL bFlag)
{
    HALAUDIO_CHECK_SHM_INIT;

    if(bFlag == FALSE)
    {
   	AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "  [Audio] DSP1 Load code fail !!! SetDspCodeType Fail !!! \r\n");
   	return;
    }

    g_AudioVars2->g_u8DspCodeType = u8Type;
}
...
AUDIO_DSP_CODE_TYPE HAL_AUDIO_GetDspCodeType(void)
{
    ...
    return (AUDIO_DSP_CODE_TYPE)g_AudioVars2->g_u8DspCodeType;
}
...
void HAL_AUDIO_SetAdvSndEffDspCodeType(MS_U8 u8Type, MS_BOOL bFlag)
{
    ...
    if(bFlag == FALSE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "  [Audio] ADV_DSP Load code fail !!! SetAdvSndEffDspCodeType Fail !!! \r\n");
        return;
    }
...
{
    MS_U32 audio_pcm_dmaRdr_bufSz = HW_DMA_RDR1_BUF_SIZE;  //must be multiple of HW_DMA_RDR1_BUF_UNIT*2 = 0x2000
    ...
    dma_writePtr = (MS_U8 *) MS_PA2KSEG1(audio_pcm_dmaRdr_base);
    dma_bufStartAddress =  (MS_U8 *) MS_PA2KSEG1(audio_pcm_dmaRdr_base);
    dma_bufEndAddress =  (MS_U8 *) MS_PA2KSEG1(audio_pcm_dmaRdr_base + audio_pcm_dmaRdr_bufSz);
...
MS_BOOL HAL_AUDIO_DmaReader_WritePCM(void* buffer, MS_U32 bytes)
{
    ...
    if ( level_cnt <= 6 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "***** Audio DMAReader Buffer empty !! ***** \r\n");
    }
...
void HAL_AUDIO_SetEntertainmentMode(AUDIO_ENTERTAINMENT_MODE eMode)
{
    switch((int)eMode)
    {
        ...
        case AUDIO_ETMODE_VOIP_ON:
            ...
            HAL_MAD_SetCommInfo(Audio_Comm_infoType_SoundMode, AU_DVB_MODE_STEREO, 0);  // Switch SE-DSP sound mode to stereo
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "HAL_AUDIO_SetEntertainmentMode: AUDIO_ETMODE_VOIP_ON\r\n");
            break;
        ...
        case AUDIO_ETMODE_VOIP_OFF:
            ...
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "HAL_AUDIO_SetEntertainmentMode: AUDIO_ETMODE_VOIP_OFF\r\n");
            break;
...
{
    return  (HAL_AUDIO_SetSystem2(AU_DEC_ID1, enDecSystem));
}
...
{
    return HAL_AUDIO_Alg2ReloadCode(u8Type);
}
...
MS_U16 HAL_AUDIO_GetUniDecodeDoneTag(void)
{
    ...
    for (i = 0; i < 100; i++)
    {
        temp1 = HAL_AUDIO_ReadReg(REG_DEC2_UNI_DECODE_TAG);
        temp2 = HAL_AUDIO_ReadReg(REG_DEC2_UNI_DECODE_TAG);
...
MS_U16 HAL_AUDIO_GetUniDecodeFrameCnt(AUDIO_DEC_ID eDSPId)
{
    ...
    if ( eDSPId < AU_DEC_ID1 || eDSPId >= AU_DEC_MAX )
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: invalid DSP id (%d) !!\r\n", __FUNCTION__, eDSPId);
        return 0;
    }
...
void HAL_AUDIO_RebootDecDSP(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "*** Load code and reset DEC DSP \n");
    HAL_MAD_SetMemInfo();
    HAL_AUDIO_ResetDSP();
...
MS_BOOL HAL_AUDIO_SeSystemLoadCode(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "MDrv_AUDIO_SeSystemLoadCode() \r\n");
    ...
    if (time_out>=100) {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "DSP2 Re-Active\n");
    }
    else {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "audio DSP_SE LoadCode success..\n");
    }
...
MS_BOOL HAL_AUDIO_SetDataCaptureSource(AUDIO_DEVICE_TYPE eID, AUDIO_CAPTURE_SOURCE_TYPE eSource)
{
    ...
    switch(eSource)
    {
        ...
        case E_CAPTURE_ADC1_AUMIC_INPUT:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "===The audio capture setting is failed by the following reason:=== \n");
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "The input setting of ADC1 is occupied by main or sub channel\n");
                ret = FALSE;
                break;
            }
        ...
        case E_CAPTURE_ADC2_AUMIC_INPUT:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "===The audio capture setting is faiedl by the following reason:=== \n");
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "The input setting of ADC2 is occupied by main or sub channel \n");
                ret = FALSE;
                break;
            }
...
MS_BOOL _AudioReloadCodePreProcess(AUDIO_DSP_ID eDspId, AUDIO_DSP_CODE_TYPE code_type)
{
    ...
    if ( eDspId == AUDIO_DSP_ID_SND )
    {
        ...
        if ( time_out >= 2000 )
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "  SND DSP Reload timeOut1: %d\r\n", time_out);
            return FALSE;
        }
...
MS_BOOL _AudioReloadCodePostProcess(AUDIO_DSP_ID eDspId,  AUDIO_DSP_CODE_TYPE code_type)
{
    ...
    if ( eDspId == AUDIO_DSP_ID_SND )
    {
        ...
        if ( time_out >= 3000 )
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "  SND DSP Reload timeOut2\r\n");
            return FALSE;
        }
...
MS_BOOL HAL_AUDIO_SetSystem2(AUDIO_DEC_ID DecId, En_DVB_decSystemType Param)
{
    ...
    {
        case MSAPI_AUD_DVB_MPEG:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_MPEG);
            break;

        case MSAPI_AUD_DVB_MS10_DDC:
        case MSAPI_AUD_DVB_AC3P:
        case MSAPI_AUD_DVB_AC3:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_AC3);
            break;

        case MSAPI_AUD_DVB_MP3:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_MP3);
            break;

        case MSAPI_AUD_DVB_MS10_DDT:
        case MSAPI_AUD_DVB_AAC:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_AAC);
            break;

        case MSAPI_AUD_DVB_XPCM:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_XPCM);
            break;

        case MSAPI_AUD_DVB_RA8LBR:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_RA8LBR);
            break;

        case MSAPI_AUD_DVB_WMA_PRO:
        case MSAPI_AUD_DVB_WMA:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_WMA);
            break;

        case MSAPI_AUD_DVB_DTS:
        case MSAPI_AUD_DVB_DTSLBR:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_DTS);
            HAL_MAD_SetCommInfo(Audio_Comm_infoType_setBypassSPDIF_PAPB_chk, TRUE, 0);
            break;

        case MSAPI_AUD_DVB_FLAC:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_FLAC);
            break;

        case MSAPI_AUD_DVB_VORBIS:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_VORBIS);
            break;

        case MSAPI_AUD_DVB_AMR_NB:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Not support AMR-NB now !!\r\n", __FUNCTION__);
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_AMR_NB);
            break;

        case MSAPI_AUD_DVB_AMR_WB:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Not support AMR-NB now !!\r\n", __FUNCTION__);
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_AMR_WB);
            break;


        case MSAPI_AUD_DVB_DRA:
            bRet = HAL_MAD2_ReLoadCode((MS_U8)AU_DVB_STANDARD_DRA);
            break;

        default:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: UN-KNOW DECODER TYPE (0x%x) !!\r\n", __FUNCTION__, Param);
            break;
    }

    HAL_AUDIO_WriteMaskReg(REG_FD230_SELECT, 0x8000, 0x0000); //select Paser2 to SE-DSP
    HAL_MAD_PcmLevelControl();
...
AUDIO_DEC_ID AU_GetDecID(AUDIO_DEC_ID *DecPriority, AudioDecStatus_t * p_AudioDecStatus)
{
    ...
    for(Index=0; Index<AU_DEC_MAX; Index++)
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "DecID = %d %s() %d \n",DecID,__FUNCTION__,__LINE__);
...
MS_BOOL HAL_AUDIO_SetDecodeSystem(AUDIO_DEC_ID DecId, AudioDecStatus_t *p_AudioDecStatus)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s() - DSP code is the same\n",__FUNCTION__);
        return bRet;
    }
...
MS_PHY HAL_AUDIO_GetDDRInfo(AUDIO_DEC_ID DecId, EN_AUDIO_DDRINFO DDRInfo)
{
    ...
    if (DecId == AU_DEC_ID1)
    {
        switch(DDRInfo)
        {
            ...
            case  E_AUD_MEMORY_SIZE:
                ...
                break;
            default:
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: UN-KNOW DDRInfo (0x%x) !!\r\n", __FUNCTION__, DDRInfo);
                break;
        }
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: UN-KNOW DecId (0x%x) !!\r\n", __FUNCTION__, DecId);
    }
    return DDR_Value;
}
...
void HAL_AUDIO_DumpDspInfo(void)
{
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_Avsync) != 0)
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "[%08u]", (unsigned int)sysTime);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_ACCUM_ES_WORDCNT, DSP_MEM_TYPE_DM)&0xFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "ES=%03X(%04X),", (unsigned int)ES_Lvl, (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_PCMISR_EMPTY_CNT, DSP_MEM_TYPE_DM)&0xFFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ELAPSED_PCM_CNT , DSP_MEM_TYPE_DM)&0xFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "PCM=%04X(%04X,%04X)|", (unsigned int)PCM_Lvl, (unsigned int)tmp_H, (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_PLAYSMPFLAG, DSP_MEM_TYPE_DM)&0x1;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "play=%X",   (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_CURR_SYNTHRATE, DSP_MEM_TYPE_DM)&0xFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "<%04X>,", (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_STATE, DSP_MEM_TYPE_DM)&0x0F;
        tmp_M = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_MMTS_SYNC_STC, DSP_MEM_TYPE_DM)&0x0F;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_FREERUN_TYPE, DSP_MEM_TYPE_DM)&0x0F;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "State=%X,%X(%X)|", (unsigned int)tmp_L,(unsigned int)tmp_M, (unsigned int)tmp_H);
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "frmCnt:%04X,",     (unsigned int)tmp_L);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%04X,",    (unsigned int)tmp_H);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_SKIP_CNT, DSP_MEM_TYPE_DM)&0x0000FF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_REPT_CNT, DSP_MEM_TYPE_DM)&0x0000FF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%02X,",    (unsigned int)tmp_L);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%02X,",    (unsigned int)tmp_H);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_WAIT_1STPTS_CNT, DSP_MEM_TYPE_DM)&0x00FF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_WAIT_STC_CNT, DSP_MEM_TYPE_DM)&0x00FF;
        tmp_M = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_SMP_FLUSH_CNT, DSP_MEM_TYPE_DM)&0x00FF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%02X,%02X,%02X|",  (unsigned int)tmp_L, (unsigned int)tmp_H, (unsigned int)tmp_M);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_PTS_TAG_WR_IDX, DSP_MEM_TYPE_DM)&0xF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_PTS_TAG_RD_IDX, DSP_MEM_TYPE_DM)&0xF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "tag:%01X,%01X|",  (unsigned int)tmp_L, (unsigned int)tmp_H);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_AVSYNC_OFFSET, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "AvDly=%05X,", (unsigned int)tmp_L);


        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_STC_L, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_STC_H, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        result = ((MS_U32)tmp_H)<<24;
        result +=(MS_U32)tmp_L;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "STC=%08d,", (unsigned int)result/90);
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_PTS_L, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_PTS_H, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        result = ((MS_U32)tmp_H)<<24;
        result += tmp_L;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "PTS=%08d,", (unsigned int)result/90);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_CURR_PTS_L, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_CURR_PTS_H, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        result = ((MS_U32)tmp_H)<<24;
        result += tmp_L;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "cPTS=%08d,", (unsigned int)result/90);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_TD_L, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_TD_H, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        result = tmp_H;
        result <<= 24;
        result += tmp_L;
        result <<= 31;
        result >>= 31;
        result/=90;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "|Td=%08d\n", (unsigned int)result);
    }
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_Dec1) != 0)
    {
        ...
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_PCMISR_EMPTY_CNT, DSP_MEM_TYPE_DM)&0xFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ELAPSED_PCM_CNT , DSP_MEM_TYPE_DM)&0xFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "PCM=%04X(%04X,%02X)|", (unsigned int)PCM_Lvl, (unsigned int)tmp_H, (unsigned int)tmp_L);
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ID, DSP_MEM_TYPE_DM)&0x00FF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "ID=%02X,", (unsigned int)tmp_L);
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_DEC_TYPE, DSP_MEM_TYPE_DM)&0x00FF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "Type=%02X,", (unsigned int)tmp_L);
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_PLAY_STATE, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "pState=%06X,", (unsigned int)tmp_L);
        tmp_L = (HAL_AUDIO_ReadReg(REG_DEC_DECODE_CMD)) & 0x003F;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_RECEIVE_STOP_CNT, DSP_MEM_TYPE_DM)&0x00FF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "|Cmd=%02X,Stop=%02X|",  (unsigned int)tmp_L, (unsigned int)tmp_H);

        tmp_L = HAL_AUDIO_ReadReg(REG_AUDIO_DECODER1_CFG);
        tmp_H = HAL_AUDIO_ReadReg(REG_AUDIO_DECODER2_CFG);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "Input_Mux:%02X,", (unsigned int)tmp_L&0x7);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%02X,", (unsigned int)tmp_H&0x7);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%02X|", (unsigned int)(tmp_L>>16)&0x7);
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_OK_CNT, DSP_MEM_TYPE_DM)&0x0000FFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ERR_CNT, DSP_MEM_TYPE_DM)&0x0000FFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "frmCnt:%04X,",     (unsigned int)tmp_L);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%04X,",    (unsigned int)tmp_H);
        ...
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_DEC_CALLING_CNT, DSP_MEM_TYPE_DM)&0x0000FFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_SYNC_MISS_CNT, DSP_MEM_TYPE_DM)&0x0000FFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "Call:%04X,",    (unsigned int)tmp_L);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "sMiss:%04X|",  (unsigned int)tmp_H);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_IoInfo+SYS_IOINFO_DEC1_OUT_L, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "pcm=%06X,",  (unsigned int)tmp_L);
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_IoInfo+SYS_IOINFO_DEC1_OUT_R, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "%06X|",  (unsigned int)tmp_L);
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_PLAYSMPFLAG, DSP_MEM_TYPE_DM)&0x1;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "play=%X\n",   (unsigned int)tmp_L);
    }
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_IoInfo) != 0)
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "CH5 mux=0x%02X,",  (unsigned int)tmp_L);

        tmp_L = (HAL_AUDIO_ReadByte(REG_AUDIO_CH6_CFG)) & 0x000F;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "Ch6 mux=0x%02X,",  (unsigned int)tmp_L);

        tmp_L = (HAL_AUDIO_ReadByte(REG_AUDIO_CH7_CFG)) & 0x000F;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "Ch7 mux=0x%02X",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_IoInfo+SYS_IOINFO_DEC1_OUT_L, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "|DEC1=0x%06X,",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_IoInfo+SYS_IOINFO_DEC2_OUT_L, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "DEC2=0x%06X",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_IO_Info2+IO_INFO2_RAW+0, DSP_MEM_TYPE_DM);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "|CH5=0x%06X,",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_IO_Info2+IO_INFO2_MUL_CH6+0, DSP_MEM_TYPE_DM);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "CH6=0x%06X,",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_IO_Info2+IO_INFO2_SCART+0, DSP_MEM_TYPE_DM);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "CH7=0x%06X",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_IO_Info1+IO_INFO1_DAC1_OUT+0, DSP_MEM_TYPE_DM);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "|DAC1=0x%06X,",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_IO_Info1+IO_INFO1_IIS1_OUT+0, DSP_MEM_TYPE_DM);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "I2S=0x%06X,",  (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_IO_Info1+IO_INFO1_SPDIF_OUT+0, DSP_MEM_TYPE_DM);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "SPDIF=0x%06X\n",  (unsigned int)tmp_L);
    }

    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_AutoTest) != 0)
    {
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_OK_CNT, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ERR_CNT, DSP_MEM_TYPE_DM)&0x00FFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "[AutoTest][AUDIO][FrameCount][%d]\n",     (unsigned int)tmp_L);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "[AutoTest][AUDIO][ErrorCount][%d]\n",     (unsigned int)tmp_H);

        tmp_L = HAL_MAD_GetCommInfo(Audio_Comm_infoType_ADEC1_sampleRate);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "[AutoTest][AUDIO][SampleRate][%d]\n",     (unsigned int)tmp_L);
        tmp_L = HAL_MAD_GetCommInfo(Audio_Comm_infoType_ADEC1_acmod);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "[AutoTest][AUDIO][ACMod][%d]\n",     (unsigned int)tmp_L);
    }

    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_mmFileAVsync) != 0)
    {
        sysTime = MsOS_GetSystemTime();
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "[%08u](%03u):", (unsigned int)sysTime, (unsigned int)sysTime - (unsigned int)preSysTime);
        preSysTime = sysTime;

        mmFile_APTS = HAL_MAD_GetCommInfo(Audio_Comm_infoType_1ms_PTS);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "APTS:%07u(%03u)|", (unsigned int)mmFile_APTS, (unsigned int)mmFile_APTS - (unsigned int)prv_mmFile_APTS);
        prv_mmFile_APTS = mmFile_APTS;

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_NEED_DATA_INT, DSP_MEM_TYPE_DM)&0xF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ES_REQ_SIZE ,  DSP_MEM_TYPE_DM)&0xFFFF;
        tmp_M = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ES_TAG_FROM_MCU ,  DSP_MEM_TYPE_DM)&0xFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "MM:%01X,%04X,%02X,%02X|", (unsigned int)tmp_L, (unsigned int)tmp_H, HAL_AUDIO_ReadByte(REG_MB_DEC2_MM_INT_TAG), (unsigned int)tmp_M);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_ACCUM_ES_WORDCNT, DSP_MEM_TYPE_DM)&0xFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "ES=%04X(%04X),", (unsigned int)ES_Lvl, (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_PCMISR_EMPTY_CNT, DSP_MEM_TYPE_DM)&0xFF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_ELAPSED_PCM_CNT , DSP_MEM_TYPE_DM)&0xFFFFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "PCM=%04X(%06X,%02X)|", (unsigned int)PCM_Lvl, (unsigned int)tmp_H, (unsigned int)tmp_L);
        ...
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_STATE, DSP_MEM_TYPE_DM)&0x0F;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_AvSync+SYS_AVSYNC_FREERUN_TYPE, DSP_MEM_TYPE_DM)&0x0F;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "State=%X(%X)|", (unsigned int)tmp_L, (unsigned int)tmp_H);
        ...
        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_PLAY_STATE, DSP_MEM_TYPE_DM)&0x00FFFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "pState=%04X,", (unsigned int)tmp_L);

        tmp_L = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_DEC_CALLING_CNT, DSP_MEM_TYPE_DM)&0x0000FF;
        tmp_H = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sys_DecStatus+SYS_DECSTATUS_SYNC_MISS_CNT, DSP_MEM_TYPE_DM)&0x0000FFF;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "Call:%02X,",    (unsigned int)tmp_L);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "Miss:%03X\n",  (unsigned int)tmp_H);
    }
...
AUDIO_TEE_INFO_SHARE_MEM *HAL_AUDIO_AllocateTeeInfoShm(void)
{
    ...
    if (MsOS_SHM_GetId((MS_U8 *)AUDIO_TEE_INFO_SHM_NAME, sizeof(AUDIO_TEE_INFO_SHARE_MEM), &u32ShmId, (MS_VIRT*)&virtAddr, &u32BufSize, MSOS_SHM_QUERY) == TRUE)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "reset Audio TEE Info SHM data ...\n");
    ...
    else
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Audio TEE Info SHM is created...\n");
            pAudioTeeInfoShm = (AUDIO_TEE_INFO_SHARE_MEM *) virtAddr;
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "SHM allocation for Audio TEE Info failed!\n");
            return NULL;
        }
    }
...
MS_BOOL HAL_AUDIO_DeAllocateTeeInfoShm(void)
{
    ...
    if (g_bAudioTeeInfoShmInitFlag == TRUE)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : No AUDIO TEE Info SHM is created!\n", __FUNCTION__);
            return FALSE;
        }
        ...
        if (pAUDIOShared->g_u32ClientCounter == 0)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO,  "free Audio TEE Info SHM data ...\n");
#if defined(MSOS_TYPE_LINUX)
            if (MsOS_SHM_FreeId((MS_U8 *)AUDIO_TEE_INFO_SHM_NAME, u32ShmId) == TRUE)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Audio TEE Info SHM data is freed\n");
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : AUDIO TEE Info SHM de-allocation failed!\n", __FUNCTION__);
...
MS_BOOL HAL_AUDIO_RegisterMBX(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Error> MBX init failed !!\n");
        return FALSE;
    }
    else
    {
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "MAD MApi_MBX_QueryDynamicClass fail\n");
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_AUDIO_RegisterMBX fail");
        return FALSE;
    }
    else
    {
        TEE_MBX_MSG_CLASS_SET(ClassNum);
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_AUDIO_RegisterMBX ok");
        return TRUE;
    }
}
...
TEE_TO_REE_MBX_ACK_MSG_TYPE HAL_AUDIO_SendMBXMsg(REE_TO_TEE_MBX_MSG_TYPE msg_type)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Recv TEE Ack Msg OK cmd:%x\n", u8Index);

    if (TEE_TO_REE_MBX_ACK_MSG_ACTION_FAIL == u8Index)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "RecvMsg TEE_TO_REE_MBX_MSG_ACK_FAIL..\n");
        return TEE_TO_REE_MBX_ACK_MSG_ACTION_FAIL;
    }
    if (TEE_TO_REE_MBX_ACK_MSG_ACTION_SUCCESS == u8Index)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "RecvMsg TEE_TO_REE_MBX_MSG_ACK_SUCCESS..\n");
    }
...
static void _MAD_Proc(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "_MAD_Proc...........\n");
    ...
    while (1)
    {
        ...
        if (E_MBX_SUCCESS != result)
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "MAD Reply Ack fail classid: %x cmd:%x \n", msg.u8MsgClass, TeeToReeMbxAckMsg.u8Index);
        else
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "MAD Reply Ack ok classid:%x cmd:%x\n", msg.u8MsgClass, TeeToReeMbxAckMsg.u8Index);
    }
}
...
static void HAL_AUDIO_HandleDynamicMBX(MBX_Msg *pMsg, MS_BOOL *pbAddToQueue)
{
    if(pMsg==NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "pMsg is a null pointer\n");
        return;
    }
    if (_s32MadEventId < 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "MAD EventGroup Id is not valid...\n");
        return;
    }
...
void HAL_AUDIO_update_DspUsage(AUDIO_SOURCE_INFO_TYPE eSourceType)
{
    ...
    if ( (g_AudioVars2->eMainSourceType == E_AUDIO_INFO_DTV_IN && eSourceType != E_AUDIO_INFO_DTV_IN) )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Release DEC DSP from DTV !!\r\n", __FUNCTION__);
    }
    else if ( (g_AudioVars2->eMainSourceType == E_AUDIO_INFO_ATV_IN && eSourceType != E_AUDIO_INFO_ATV_IN) )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Release SND DSP from ATV !!\r\n", __FUNCTION__);
    }
    else if ( (g_AudioVars2->eMainSourceType == E_AUDIO_INFO_HDMI_IN && eSourceType != E_AUDIO_INFO_HDMI_IN) )
    {
        ...
        g_AudioVars2->DspUsage[AU_DEC_ID3].eAudioType = Audio_DEC_NULL;
        g_AudioVars2->DspUsage[AU_DEC_ID3].eUsageType = AUDIO_USAGE_TYPE_NONE;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Release SND DSP from HDMI !!\r\n", __FUNCTION__);
    }
    ...
    if ( g_AudioVars2->eMainSourceType == E_AUDIO_INFO_DTV_IN )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Alloc DEC DSP for DTV !!\r\n", __FUNCTION__);
    }
    else if ( g_AudioVars2->eMainSourceType == E_AUDIO_INFO_ATV_IN )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Alloc DEC DSP for ATV !!\r\n", __FUNCTION__);
    }
    else if ( g_AudioVars2->eMainSourceType == E_AUDIO_INFO_HDMI_IN )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Alloc DEC DSP for HDMI !!\r\n", __FUNCTION__);
    }

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: update PrevSource %d NewSource %d !!\r\n", __FUNCTION__, g_AudioVars2->ePrevSourceType, g_AudioVars2->eMainSourceType);
...
MS_U32 HAL_AUDIO_GetCaps(AUDIO_DEC_ID DecId, MS_U32 *pCaps)
{
    ...
    if (pCaps == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! pCaps should not be NULL!\n", __FUNCTION__);
        return FALSE;
    }

    if ((DecId <= AU_DEC_INVALID) || (DecId >= AU_DEC_MAX) || (DecId == AU_DEC_ID2))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Invalid DecId (%d)!\n", __FUNCTION__, DecId);
        return FALSE;
    }
    else if ((g_AudioVars2->AudioCaps[AU_DEC_ID1].u8MaxDecoderNum == 1) && (DecId > AU_DEC_ID1))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! DecId (%d) is greater than max decoder number (%d)!\n", __FUNCTION__, DecId, g_AudioVars2->AudioCaps[AU_DEC_ID1].u8MaxDecoderNum);
        return FALSE;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Invalid version number (%u) from function caller!\n", __FUNCTION__, (unsigned int)pCapsTmp->u32AudioCapsVersion);
        return FALSE;
    }

    if (pCapsTmp->u32AudioCapsStructSize == 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Invalid structure size (%u) from function caller!\n", __FUNCTION__, (unsigned int)pCapsTmp->u32AudioCapsStructSize);
        return FALSE;
    }

    if (pCapsTmp->u32AudioCapsVersion != g_AudioVars2->AudioCaps[DecId].u32AudioCapsVersion)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! function caller's version (%u) is not matched to driver's version (%u) at DecId (%d)\n", __FUNCTION__, (unsigned int)pCapsTmp->u32AudioCapsVersion, (unsigned int)g_AudioVars2->AudioCaps[DecId].u32AudioCapsVersion, (unsigned int)DecId);
...
MS_U32 HAL_AUDIO_PCM_HwDma_Reader2_Write(const void *pBuf, MS_U32 u32Size)
{
    ...
    {
        do {
            u32SizeToCopy = (pPcmInfo->pBuffer + u32BufferSize) - pPcmInfo->pWritePtr;
...
MS_S32 HAL_AUDIO_PCM_R2Dma_Reader1_Restart(void)
{
    ...
    /* clear PCM buffer */
    memset((void *)pPcmInfo->pBuffer, 0x00, R2_DMA_RDR1_BUF_SIZE);
...
MS_S32 HAL_AUDIO_PCM_R2Dma_Reader2_Restart(void)
{
    ...
    /* clear PCM buffer */
    memset((void *)pPcmInfo->pBuffer, 0x00, R2_DMA_RDR2_BUF_SIZE);
