...
void HAL_SIF_TriggerSifPLL(void)
{
    ...
    else    //enable SIF
    {
        ...
        HAL_SIF_AbsWriteMaskByte(0x112840, 0x10, 0x00);
        HAL_SIF_AbsWriteMaskByte(0x112834, 0x10, 0x10);

        HAL_SIF_AbsWriteMaskByte(0x112802, 0xFF, 0x40);
        HAL_SIF_AbsWriteMaskByte(0x112803, 0xFF, 0x04);
        ...
        HAL_SIF_AbsWriteMaskByte(0x112864, 0xFF, 0x00);
        ...
        HAL_SIF_AbsWriteMaskByte(0x000013, 0xFF, 0x01);

        HAL_SIF_AbsWriteMaskByte(0x112817, 0xFF, 0x00);
        HAL_SIF_AbsWriteMaskByte(0x112816, 0xFF, 0x05);

        HAL_SIF_AbsWriteMaskByte(0x11287f, 0xFF, 0x00);
        HAL_SIF_AbsWriteMaskByte(0x11287e, 0xFF, 0x00);
        ...
        HAL_SIF_AbsWriteMaskByte(0x112dc2, 0x40, 0x40);
        HAL_SIF_AbsWriteMaskByte(0x112d8e, 0xFF, 0x00);
        HAL_SIF_AbsWriteMaskByte(0x112d0a, 0x80, 0x00);
        HAL_SIF_AbsWriteMaskByte(0x102d5c, 0x80, 0x00);
...
MS_U8 HAL_SIF_GetDspType(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s() : invalid g_AudioVars2!\n", __FUNCTION__);
        return DSP_SE;
    }
...
MS_U8 HAL_SIF_GetDspCodeType(void)
{
    ...
    if (g_AudioVars2->g_u8SifDspType == DSP_DEC)
    {
        return g_AudioVars2->g_u8DspCodeType;
    }
    else
    {
        return g_AudioVars2->g_u8Dsp2CodeType;
    }
}
...
MS_BOOL HAL_SIF_SetSoundMode(AUDIO_SIF_SOUND_MODE u8Mode_Type)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_SIF_SetSoundMode(mode=%d)\r\n", u8Mode_Type);
...
MS_U8 HAL_SIF_SendCmd(AU_SIF_CMD u8DecCmd, MS_U8 comm_arg1, MS_U8 comm_arg2)
{
    ...
    switch( u8DecCmd )
    {
        ...
        case AU_SIF_CMD_GET_FM_A2_MODE_STATUS:
            if ((HAL_SIF_GetDspCodeType()) != AU_SIF_PALSUM)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "DSP type not palsum\n");
                break;
            }
        ...
        case AU_SIF_CMD_GET_BTSC_MODE_STATUS:
            if ((HAL_SIF_GetDspCodeType()) != AU_SIF_BTSC) {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "DSP type not btsc\n");
                break;
            }
            u8Return = HAL_SIF_ReadReg(AU_STATUS_MODE1);
            break;
        ...
        case AU_SIF_CMD_GET_STANDARD_STATUS:
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "DSP type not SIF \n");
                return FALSE;
            }
            break;
...
MS_U32 HAL_SIF_AccessThreshold(AUDIO_SIF_SYSTEM_TYPE rw_standard_type, AUDIO_SIF_THRESHOLD_TYPE  u8Threshold_type, MS_U16 u16Value)
{
    ...
    if (u16Value > 0x7fff)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Threshold value is too large\n");
        return FALSE;
    }
...
MS_BOOL HAL_SIF_ReLoadCode(MS_U8 u8Type)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "MDrv_SIF_ReLoadCode(u8Type= SIF(%x))\r\n",u8Type);
