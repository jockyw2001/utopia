...
MS_BOOL HAL_MAD_MM2_checkAesInfo( AUDIO_DEC_ID  dec_id, AES_INFO *aes_info )
{
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        if ( tmp2 < (BYTES_IN_MIU_LINE*2) )                 //至少 keep 2 條 line 的空間
            tmp2 = 0;
        else
...
//            result = HAL_MAD2_Read_DSP_sram(REG_PTS_AccumEsCnt, DSP_MEM_TYPE_DM);
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_ADEC1_Get_PCM_WPTR:
            result = HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + (HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sysDec1_pcm_dram_wrPtr_report, DSP_MEM_TYPE_DM) << BYTES_IN_MIU_LINE_LOG2);
            break;
