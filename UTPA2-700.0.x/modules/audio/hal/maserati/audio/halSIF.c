...
void HAL_SIF_TriggerSifPLL(void)
{
    ...
    else    //enable SIF
    {
        ...
        HAL_SIF_AbsWriteMaskByte(0x112878, 0xFF, 0x00);  // enable LDOs
        HAL_SIF_AbsWriteMaskByte(0x112879, 0xFF, 0x00);

        HAL_SIF_AbsWriteMaskByte(0x112840, 0x10, 0x00);  // ref enable
        HAL_SIF_AbsWriteMaskByte(0x112834, 0x10, 0x10);  // reg_en_vco_dig
...
MS_U8 HAL_SIF_GetDspType(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() : invalid g_AudioVars2!\n", __FUNCTION__);
        return DSP_SE;
    }
...
MS_BOOL HAL_SIF_SetSoundMode(AUDIO_SIF_SOUND_MODE u8Mode_Type)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "HAL_SIF_SetSoundMode(mode=%x)\r\n", u8Mode_Type);
...
MS_U8 HAL_SIF_SendCmd(AU_SIF_CMD u8DecCmd, MS_U8 comm_arg1, MS_U8 comm_arg2)
{
    ...
    switch( u8DecCmd )
    {
        ...
        case AU_SIF_CMD_GET_BTSC_MODE_STATUS:
            if ((HAL_SIF_GetDspCodeType()) != AU_SIF_BTSC) {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "DSP type not btsc\n");
                break;
            }
            u8Return = HAL_SIF_ReadReg(AU_STATUS_MODE1);
            break;
        ...
        case AU_SIF_CMD_GET_STANDARD_STATUS:
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "DSP type not SIF \n");
                return FALSE;
            }
            break;
...
MS_U32 HAL_SIF_AccessThreshold(AUDIO_SIF_SYSTEM_TYPE rw_standard_type, AUDIO_SIF_THRESHOLD_TYPE  u8Threshold_type, MS_U16 u16Value)
{
    ...
    if (u16Value > 0x7fff)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Threshold value is too large\n");
        return FALSE;
    }
