...
void HAL_MAD_SetDecCmd(AU_DVB_DECCMD u8DecCmd)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_MAD_setDecCmd:(%x)\r\n",u8DecCmd);
...
{
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_MAD_setFreeRun:(%x)\r\n",u8FreeRun);
    if( u8FreeRun >= 2 ){
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Invalid mode\r\n");
        return;
    }
...
{
    Audio_id audio_id = HAL_AUDIO_Convert_DecId_to_ADECId(dec_id);
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, audio_id, 0, 0);           //Set ES write pointer  to start point.

        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "ID1: HAL_AUDIO_MM2_init end !!\r\n");
        return TRUE;
    }
    else if( dec_id == AU_DEC_ID3 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "ID3: HAL_AUDIO_MM2_init end !!\r\n");
        return TRUE;
    }
    else
        return FALSE;
...
MS_BOOL HAL_MAD_MM2_checkAesInfo( AUDIO_DEC_ID  dec_id, AES_INFO *aes_info )
{
    ...
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        /* es level */
        audio_aes_info[0].aes_level = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, audio_id);
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "[ID1:HAL_MAD_MM2_checkAesInfo] Ptr[w:%x,r:%x] LvL:%x, FreeSpace:%x, tmpFree:%x \n",
            (unsigned int)audio_aes_info[0].aes_write_addr,
            (unsigned int)audio_aes_info[0].aes_read_addr,
            (unsigned int)audio_aes_info[0].aes_level,
            (unsigned int)audio_aes_info[0].aes_BufSize - (unsigned int)audio_aes_info[0].aes_level,
            (unsigned int)audio_aes_info[0].aes_freeSpace);
    ...
    else if( dec_id == AU_DEC_ID3 )
    {
        ...
        /* es level */
        audio_aes_info[1].aes_level = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, audio_id);
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "[ID3:HAL_MAD_MM2_checkAesInfo] Ptr[w:%x,r:%x] LvL:%x, FreeSpace:%x, tmpFree:%x \n",
            (unsigned int)audio_aes_info[1].aes_write_addr,
            (unsigned int)audio_aes_info[1].aes_read_addr,
            (unsigned int)audio_aes_info[1].aes_level,
            (unsigned int)audio_aes_info[1].aes_BufSize - (unsigned int)audio_aes_info[1].aes_level,
            (unsigned int)audio_aes_info[1].aes_freeSpace);
...
MS_BOOL HAL_MAD_MM2_inputAesFinished( AUDIO_DEC_ID dec_id, MS_U32 es_size, MS_BOOL ptsExist, MS_U64 pts )
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "ID1:==> Error, input AES size(0x%08u) is larger than current Free space(0x%08u)\r\n", (unsigned int)es_size, (unsigned int)audio_aes_info[0].aes_freeSpace );
        return FALSE;
    }

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "[ID1:HAL_MAD_MM2_inputAesFinished] wPtr:%08x => ", (unsigned int)audio_aes_info[0].aes_write_addr);
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%08x, size=%08x, isPtsExist:%d, PTS=%09lx\n",
#else
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%08x, size=%08x, isPtsExist:%d, PTS=%09llx\n",
#endif
        (unsigned int)audio_aes_info[0].aes_write_addr,
    ...
    if (ptsExist)
    {
        if (prv_PTS != 0)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, ",PTS_diff=%08lld,accum_ES = %08x,ES_diff= %08d, ES PTS diff=%08d\n",
                (long long int)(pts - prv_PTS)/90,
                (unsigned int)accum_ES,
                (unsigned int)accum_ES - (unsigned int)prv_accum_ES,
                (unsigned int)(accum_ES - prv_accum_ES)*32/1536);
        }
        else
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, ", PTS_diff=%08x, accum_ES = %08x\n", 0, 0);
        }
    ...
    if (ptsExist)
        {
            if ((audio_aes_info[0].aes_lastPTS  == pts)&&(audio_aes_info[0].aes_ptsCnt != 0))
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, " ======== Error PTS is same !! =============\n");
            else
                {
                        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_INPUT_PTS, audio_id, (pts >> 1), 0);
    ...
    /* Set ES write pointer to DSP */
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, audio_id, es_writeLineOffset, 0);
    ...
    else if (dec_id == AU_DEC_ID3)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "ID3:==> Error, input AES size(0x%08u) is larger than current Free space(0x%08u)\r\n", (unsigned int)es_size, (unsigned int)audio_aes_info[1].aes_freeSpace );
            return FALSE;
        }

        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "[ID3:HAL_MAD_MM2_inputAesFinished] wPtr:%08x => ", (unsigned int)audio_aes_info[1].aes_write_addr);
        ...
        if (ptsExist)
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, ",PTS_diff=%08lld,accum_ES = %08x,ES_diff= %08d, ES PTS diff=%08d\n",
                (long long int)(pts - prv_PTS1)/90,
                (unsigned int)accum_ES1,
                (unsigned int)accum_ES1 - (unsigned int)prv_accum_ES1,
                (unsigned int)(accum_ES1 - prv_accum_ES1)*32/1536);
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, ", PTS_diff=%08x, accum_ES = %08x\n", 0, 0);
...
//                HAL_AUDIO_WriteMaskReg(M2S_MBOX_DBG_CMD2, 0xFFFF, param1);
        ...
        ********************************************************************************/
        case Audio_Comm_infoType_ADEC1_Set_play_smpRate:
        {
            ...
            if (play_cmd != 0x02)
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "[%s : Audio_Comm_infoType_ADEC1_Set_play_smpRate]: audio play cmd is not Ts file Play !!\n", __FUNCTION__);

            if (param1)
            {
                if ((param2 > 50000) ||(param2<6000))
#if defined (__aarch64__)
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "[%s : Audio_Comm_infoType_ADEC1_Set_play_smpRate]: play rate [%ld] is invalid !!\n", __FUNCTION__, param2);
#else
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "[%s : Audio_Comm_infoType_ADEC1_Set_play_smpRate]: play rate [%d] is invalid !!\n", __FUNCTION__, (int)param2);
#endif
        ...
        case Audio_Comm_infoType_ADEC1_UNI_PCMOutCnt:
            uniPlay_ackCnt[0]++;
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "\r\nTrigger decInt[%d] %d\r\n", 0, (unsigned int)uniPlay_ackCnt[0]);
       ...
       case Audio_Comm_infoType_ADEC1_avsync_delay:
            {
                ...
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AC3_AVOFFSET, ADEC1, delay, 0);
                ...
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AAC_AVOFFSET, ADEC1, delay, 0);
                ...
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MPG_AVOFFSET, ADEC1, delay, 0);
            }
            break;
...
//            result = HAL_MAD_Read_DSP_sram(REG_PTS_AccumEsCnt, DSP_MEM_TYPE_DM);
        ...
        case Audio_Comm_infoType_Get_Max_DelayTime:
            result = AUDIO_DELAY_UPPER_BOUND;
            break;

        case Audio_Comm_infoType_Get_Min_DelayTime:
            result = AUDIO_DELAY_LOWER_BOUND;
            break;
...
MS_BOOL HAL_MAD_SetMpegInfo( Audio_MPEG_infoType infoType, MS_VIRT param1, MS_VIRT param2 )
{
    switch(infoType)
    {
        ...
        case Audio_MPEG_DTV_AVSYNC_OFFSET:
            if ((param1<DELAY_LOWER_LIMIT)||(param1>DELAY_UPPER_LIMIT))              //delay offset range from 30ms ~ 1050ms
            {
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MPG_AVOFFSET, ADEC1, g_mpeg_delay, 0);
            break;
...
MS_BOOL HAL_MAD_SetWmaInfo( Audio_WMA_infoType infoType, MS_VIRT param1, MS_VIRT param2 )
{
    switch(infoType)
    {
        ...
        case Audio_WMA_infoType_FFSpeed:                                    //HAL_MAD_WMA_SetFastFowardSpeed
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Currently, it is not used\n");
            break;
...
MS_BOOL HAL_MAD_SetAC3PInfo( Audio_AC3P_infoType infoType, MS_VIRT param1, MS_VIRT param2 )
{
    ...
    switch(infoType)
    {
        ...
        case Audio_AC3P_infoType_hdmiTxBypass_enable:
            ...
            {
                #if defined (CONFIG_UTOPIA_ANDROID_L)
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: param1[%x,%08x,%08x] out of range\n",__FUNCTION__, infoType, param1, param2);
                #else
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: param1[%x,%x,%x] out of range\n",__FUNCTION__, infoType, (unsigned int)param1, (unsigned int)param2);
                #endif
            }
        ...
        case Audio_AC3P_DTV_AVSYNC_OFFSET:
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AC3_AVOFFSET, ADEC1, g_ac3_delay, 0);
            break;
...
MS_U32 HAL_MAD_GetDTSInfo( Audio_DTS_infoType infoType )
{
    ...
    switch(infoType)
    {
        case Audio_DTS_infoType_DecStatus:
            if (((HAL_AUDIO_AbsReadByte(REG_R2_DECODE1_TYPE) & adec_type_regmask) == adec_type_dts) && (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, u8MainDecodeId)&0x0001))
                result = 1;
            else
                result = 0;
            break;
        ...
        case Audio_DTS_infoType_SampleRate: // Original FS from Header
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, u8MainDecodeId);
            break;

        case Audio_DTS_infoType_BitRate:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_BIT_RATE, u8MainDecodeId);
            break;

        case Audio_DTS_infoType_CD_MODE:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DTS_CD_MODE, u8MainDecodeId);
            break;
        ...
        case Audio_DTS_infoType_Amode:
        {
            MS_U32 ac_mode = 0xFF;
            MS_U32 channel_mapping = 0x0;
            ac_mode = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MODE, u8MainDecodeId);
            channel_mapping = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MAPPING, u8MainDecodeId);
...
MS_BOOL HAL_MAD_SetAACInfo( Audio_AAC_infoType infoType, MS_VIRT param1, MS_VIRT param2 )
{
    switch(infoType)
    {
        ...
        case Audio_AAC_DTV_AVSYNC_OFFSET:
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AAC_AVOFFSET, ADEC1, g_aac_delay, 0);
            break;
...
MS_U32 HAL_MAD_GetAACInfo( Audio_AAC_infoType infoType )
{
    ...
    AUDIO_DSP_CODE_TYPE MainDspCodeType = AU_DVB_NONE;
    HALMAD_CHECK_SHM_INIT;
...
MS_BOOL HAL_MAD_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    switch ( paramType )
    {
        ...
        case Audio_ParamType_reqDataSize:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: adec_id %d not Supported\n", __FUNCTION__, adec_id );
                return FALSE;
            }
        ...
        case Audio_ParamType_UNI_PCMOutCnt:
        {
            if(adec_id < MAX_ADEC)
            {
                ...
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "\r\nTrigger decInt[%d] %d\r\n", adec_id, (unsigned int)uniPlay_ackCnt[adec_id]);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_UNI_PCM_SET_OUTPUT_CNT, adec_id, Param, 0);
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: adec_id %d over array size !!\r\n", __FUNCTION__, adec_id);
                return FALSE;
            }
            break;
        }
        ...
        {
            // This case only used for OMX case
            ...
            AUDIO_OUTPORT_SOURCE_TYPE eGroup = E_CONNECT_NULL;

            OS_OBTAIN_MUTEX(_s32MutexLoadCode, MSOS_WAIT_FOREVER);

            memset(&AudioDecStatus,  0, sizeof(AudioDecStatus_t));
            ...
            if ( (eAudioCaps & eAudioType) == eAudioType )
            {
                ...
                if (DecId == AU_DEC_ID3)
                {
                    eGroup = E_CONNECT_MAIN;
                }
                else
                {
                    eGroup = E_CONNECT_SUB;
                }

                AudioDecStatus.eSourceType = E_AUDIO_INFO_MM_IN;
                AudioDecStatus.eAudFormat = eAudioFormat;
                AudioDecStatus.eGroup = eGroup;
                AudioDecStatus.eMMType = AUDIO_MM_OMX;

                bRet = HAL_AUDIO_SetDecodeSystem(DecId, &AudioDecStatus);
                if ( bRet == TRUE )
                {
                    memcpy(&g_AudioVars2->AudioDecStatus[DecId], &AudioDecStatus, sizeof(AudioDecStatus_t));
                    g_AudioVars2->AudioDecStatus[DecId].bConnect = TRUE;
                }
            }
            else
        ...
        case Audio_ParamType_Multi_Channel:
        {
            if ((Param == TRUE) || (Param == FALSE))
            {
                ...
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: DecId(%d) %s Multi-Channel\n", __FUNCTION__, adec_id, Param?"Enable":"disable");
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Invalid value(%lu) to enable/disable Multi-Channel\n", __FUNCTION__, Param);
            }
...
MS_BOOL HAL_MAD_GetAudioInfo2( AUDIO_DEC_ID DecId, Audio_InfoType infoType, void * pInfo )
{
    ...
    switch ( infoType )
    {
...
#if OMX_AUDIO_DEBUG
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "\r\n===DecId:%d  Audio_infoType_UNI_PCM1_Info  addr :0x%X     size:0x%X      DecodeFrame=0x%X==========\r\n", (unsigned int)DecId,(unsigned int)pPCMInfo->U32WrtAddr, (unsigned int)pPCMInfo->U32WrtBytes,(unsigned int)g_AudioVars2->u16DecodeFrmCnt[DecId]);
#endif
            }
        ...
        case Audio_infoType_UNI_ES_Level:
        {
            ...
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, adec_id);
#if OMX_AUDIO_DEBUG
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "\r\n=====ID:%X  Audio_infoType_UNI_ES_Level =0x%X==========\r\n", (unsigned int)adec_id, (unsigned int)*pRet);
#endif
            break;
        }
        ...
        case Audio_infoType_omx_no_mixing:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            ...
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OMX_NO_MIXING, adec_id);
            break;
        }

        case Audio_infoType_omx_ES_bypass:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            ...
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OMX_ES_BYPASS, adec_id);
            break;
        }

        case Audio_infoType_acmod:
        {
            ...
            if (pInfo == NULL)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Audio_infoType_acmod pInfo is NULL\n", __FUNCTION__);
                return FALSE;
            }

            if (adec_id >= MAX_ADEC) // SHM only has 2 mem
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Audio_infoType_acmod more than MAX_ADEC ID %d\n", __FUNCTION__, adec_id);
                return FALSE;
            }

            if (adec_id == ADEC1)
            {
                eInfoType = Audio_Comm_infoType_ADEC1_acmod;
            }
            else if (adec_id == ADEC2)
            {
                eInfoType = Audio_Comm_infoType_ADEC2_acmod;
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Audio_infoType_acmod DEC_ID %d not Supported\n", __FUNCTION__, adec_id);
                return FALSE;
            }

            *pRet = HAL_MAD_GetCommInfo(eInfoType);
        ...
        case Audio_infoType_Decoder_Type:
        {
            ...
            switch (adec_id)
            {
                ...
                default:
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: invalid ID (%d, %d)\n", __FUNCTION__, DecId, adec_id);
                    result = FALSE;
                    break;
                }
            }
