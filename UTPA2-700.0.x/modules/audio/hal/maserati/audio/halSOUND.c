...
{
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_SOUND_SetMute = %x\n", (MS_U8)bEnable );
...
void HAL_SOUND_SetBalance(MS_U8 u8Balance)
{
    ...
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "apiAud>> u8Balance = %02bx\n", u8Balance);
    if(u8Balance==50)
    {
        value_l = 0x00;
        value_r = 0x00;
...
void HAL_SOUND_SetBass(MS_U8 u8Level)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Bass:%d",(unsigned int)u8Level);
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "==>%x\r\n",(unsigned int)u8Value);
...
void HAL_SOUND_SetTreble(MS_U8 u8Level)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Treble:%d",(unsigned int)u8Level);
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "==>%x\r\n",(unsigned int)u8Value);
...
/// @brief \b Function \b Description: This routine is used to set the DRC Entry u8Mode.
/// @param <IN>        \b u8Mode    :   DRC EntrySelect u8Mode
///                                     0 -- DRC before VOL function
///                                     1 -- DRC behind VOL function
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_SOUND_SetDRCEntry(MS_U8 u8Mode)
{
    ...
    {
        case 0:
            HAL_AUDIO_WriteMaskByte(REG_SOUND_DRC_ENTRY,0x01,0x00);
            break;

        case 1:
            HAL_AUDIO_WriteMaskByte(REG_SOUND_DRC_ENTRY,0x01,0x01);
            break;
...
/// @brief \b Function \b Description: This routine is used to set Main audio delay.
/// @param <IN>        \b u16Delay  :   0x20~0xC8(32~200), in ms unit
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_SOUND_SetCH1AudioDelay(MS_U16 u16Delay)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Warning! Invalid value (%u), main audio delay should be less than %u ms. \r\n", u16Delay, AUDIO_DELAY_UPPER_BOUND);
        u16Delay = AUDIO_DELAY_UPPER_BOUND;
    }

    if (u16Delay < AUDIO_DELAY_LOWER_BOUND)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Warning! Invalid value (%u), main audio delay should be larger than %u ms. \r\n", u16Delay, AUDIO_DELAY_LOWER_BOUND);
        u16Delay = AUDIO_DELAY_LOWER_BOUND;
    }
...
/// @brief \b Function \b Description: This routine is used to get the value of Main audio delay.
/// @param <RET>       \b u16Delay  :   0x14~0xC8(20~200), in ms unit
////////////////////////////////////////////////////////////////////////////////
...
/// @brief \b Function \b Description: This routine is used to set SPDIF audio delay.
/// @param <IN>        \b u16Delay  :   0x14~0xFA(20~250), in ms unit
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_SOUND_SetSpdifAudioDelay(MS_U16 u16Delay)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Warning! Invalid value (%u), spdif audio delay should be less than %u ms. \r\n", u16Delay, AUDIO_DELAY_UPPER_BOUND_SPDIF);
        u16Delay = AUDIO_DELAY_UPPER_BOUND_SPDIF;
    }

    if (u16Delay < AUDIO_DELAY_LOWER_BOUND_SPDIF)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Warning! Invalid value (%u), spdif audio delay should be larger than %u ms. \r\n", u16Delay, AUDIO_DELAY_LOWER_BOUND_SPDIF);
        u16Delay = AUDIO_DELAY_LOWER_BOUND_SPDIF;
    }
...
void HAL_SOUND_SetPEQCoef(AUDIO_PEQ_COEF *peq_coef)
{
    ...
    if(peq_coef->sfs == 0)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "!!!only support 48K coefficient ! ! !\n");
        return;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "!!! undefined coefficient type ! ! !\n");
        return;
    }

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL a0 = %x", (MS_U8)(peq_coef->a0>>16));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x", (MS_U8)(peq_coef->a0>>8));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x\r\n", (MS_U8)peq_coef->a0);

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL a1 = %x", (MS_U8)(peq_coef->a1>>16));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x", (MS_U8)(peq_coef->a1>>8));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x\r\n", (MS_U8)peq_coef->a1);

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL a2 = %x", (MS_U8)(peq_coef->a2>>16));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x", (MS_U8)(peq_coef->a2>>8));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x\r\n", (MS_U8)peq_coef->a2);

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL b1 = %x", (MS_U8)(peq_coef->b1>>16));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x", (MS_U8)(peq_coef->b1>>8));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x\r\n", (MS_U8)peq_coef->b1);

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL b2 = %x", (MS_U8)(peq_coef->b2>>16));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x", (MS_U8)(peq_coef->b2>>8));
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " %x\r\n", (MS_U8)peq_coef->b2);
}
...
MS_BOOL HAL_SND_SetParam(Sound_SET_PARAM_Type Type, MS_U16 param1, MS_U16 param2)
{
    ...
    switch(Type)
    {
        ...
        case Sound_SET_PARAM_AbsoluteEQ:                // absolute GEQ, param1: band 0~4, param2: 0x30(+12dB)~0x00(0dB)~0xD0(-12dB)
            if(param1 > 4)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Invalid value, GEQ Band should be 0~4\n");
            }
            else
            {
        ...
        case Sound_SET_PARAM_PEQ_48K_A0:
            if((param1>>8) > MAX_PEQ_BAND-1)
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "PEQ paramater Band should be 0~4\n");
            else
        ...
        case Sound_SET_PARAM_AVC_S_Offset:
            {
                ...
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "AVC offset is out of range 0~12 dB \n");
                }
                else
                {
        ...
        case Sound_SET_PARAM_Ease_Type_B:
        {
            ...
            switch (Type)
            {
                ...
                default:
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: invalid audio ease command %d\n", __FUNCTION__, Type);
                    status = FALSE;
                    break;
                }
            }
            ...
            switch (param1)
            {
                ...
                default:
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: invalid audio ease type %d\n", __FUNCTION__, param1);
                    status = FALSE;
                    break;
                }
            }
        ...
        case Sound_SET_PARAM_Ease_Duration_B:
        {
            ...
            if (status == TRUE)
            {
                if (param1 > 60000)
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: target duration (%d) of audio ease is too large!\n", __FUNCTION__, param1);
                    u32MiniSecond = 60000;
                }
                else
                {
        ...
        case Sound_SET_PARAM_Ease_Gain_B:
        {
            ...
            if (status == TRUE)
            {
                ...
                if (u32Gain > 0x7FFFFF)
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: target gain (0x%X) of audio ease is too large!\n", __FUNCTION__, (unsigned int)u32Gain);
                    u32Gain = 0x7FFFFF;
                }
        ...
        case Sound_SET_PARAM_Ease_Target_Channel_B:
        {
            ...
            switch (param1)
            {
                ...
                default:
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: invalid audio ease path %d\n", __FUNCTION__, param1);
                    status = FALSE;
                    break;
                }
            }
        ...
        {
            HAL_SOUND_SetDRCEntry((MS_U8)param1);
            status = TRUE;
	        break;
        }
    ...
    switch(Type)
    {
        ...
        case Sound_GET_PARAM_Ease_Type_B:
        {
            ...
            switch (u32EaseType)
            {
                ...
                default:
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: invalid audio ease type %u\n", __FUNCTION__, (unsigned int)u32EaseType);
                    break;
                }
            }
