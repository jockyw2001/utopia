...
void HAL_MAD2_SetMemInfo(void)
{
...
#if ASND_R2_SUPPORT
    MS_PHY SND_R2_ADDR = DEC_R2_ADDR + ADEC__R2_DDR_SIZE; // Get SND-R2 Start address
#endif
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "HAL_MAD2_SetMemInfo : SE_DSP_DDR_ADDR = 0x%08X, DEC_R2_ADDR = 0x%08X\n", (unsigned int) SE_DSP_DDR_ADDR, (unsigned int) DEC_R2_ADDR);
  ...
  ********************************************************/
    HAL_AUDIO_WriteMaskByte(REG_AUDIO_SYNTH_EXPANDER, 0x02, 0x00);        //BANK 1630 control by MCU
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " ======> Audio DEC_R2 Base : %x\n", (unsigned int) DEC_R2_ADDR);
  ...
  ********************************************************/
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, " ======> Audio SND_R2 Base : %08x\n", (unsigned int) SND_R2_ADDR);
...
void HAL_MAD2_SetFreeRun(MS_U8 u8FreeRun)
{
    ...
    if( u8FreeRun >= 2 )
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "Invalid mode\r\n");
    }
...
void HAL_MAD2_RebootDsp(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "*** g_Dsp2CodeType: 0x%x \n", g_Dsp2CodeType);

    // Reset MAD module
    HAL_MAD2_RSTMAD_DisEn_MIUREQ();
    AUDIO_DELAY1MS(2);

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "*** Load code and reset SE DSP \n");
