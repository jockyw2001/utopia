...
MS_BOOL g_DDPBypassenable_1 = FALSE;
MS_BOOL g_DDPBypassenable_2 = FALSE;
...
void HAL_MAD_Monitor_DDPlus_SPDIF_Rate(void)
{
    ...
    static MS_U8 u8PreTx1SamplingRate = 0;
    static MS_U8 u8PreTx2SamplingRate = 0;
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            ...
            switch(MainDspCodeType)
            {
                ...
                case AU_DVB_STANDARD_DTSLBR:
                {
                    ...
                    u32spdif_info_flag = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TRANSCODE_FLAG, u8MainDecodeId);
    ...
    if ((u8PreTx1SamplingRate != stDigitalChannelStatusTx1.u8SamplingRate) ||
        (u8PreTx2SamplingRate != stDigitalChannelStatusTx2.u8SamplingRate))
    {
    ...
    u8PreTx1SamplingRate = stDigitalChannelStatusTx1.u8SamplingRate;
    u8PreTx2SamplingRate = stDigitalChannelStatusTx2.u8SamplingRate;
}
...
//                HAL_AUDIO_WriteMaskReg(M2S_MBOX_DBG_CMD2, 0xFFFF, param1);
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_Multi_Channel:
        {
            ...
            {
                HALMAD_ERROR("%s: Error! Invalid value(0x%x) to enable/disable Multi-Channel\n", __FUNCTION__, (unsigned int)param1);
            }
        ...
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_PassThrough_DelaySmp, ADEC1, param1, 0);
            break;
        }

        case Audio_Comm_infoType_ES_PassThrough_PcmSmpRate:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_PassThrough_PcmSamRate, ADEC1, param1, 0);
            break;
        }

        case Audio_Comm_infoType_ES_PassThrough_HdmiSmpRate:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_PassThrough_HdmiSmpRate, ADEC1, param1, 0);
            break;
        }
    ...
    switch ( paramType )
    {
        ...
        case Audio_ParamType_WriteDM_Value:
        {
            switch(DecId)
            {
                case AU_DEC_ID1:
                {
                    HAL_MAD_Write_DSP_sram(g_WriteDMTargetAddr, Param, DSP_MEM_TYPE_DM);
                }
                break;
                case AU_DEC_ID3:
                {
        ...
        case Audio_ParamType_WritePM_Value:
        {
            switch(DecId)
            {
                case AU_DEC_ID1:
                {
                    HAL_MAD_Write_DSP_sram(g_WritePMTargetAddr, Param, DSP_MEM_TYPE_PM);
                }
                break;
                case AU_DEC_ID3:
                {
        ...
        {
            bRet =  HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DRC_MODE, adec_id, Param, 0);
        }
            break;
    ...
    switch ( infoType )
    {
        ...
        case Audio_infoType_ReadDM_Value:
        {
            ...
            switch(DecId)
            {
                case AU_DEC_ID1:
                {
                    *pRet = (MS_U16)HAL_MAD_Read_DSP_sram(g_ReadDMTargetAddr, DSP_MEM_TYPE_DM);
                }
                break;
                case AU_DEC_ID3:
                {
        ...
        case Audio_infoType_ReadPM_Value:
        {
            ...
            switch(DecId)
            {
                case AU_DEC_ID1:
                {
                    *pRet = (MS_U16)HAL_MAD_Read_DSP_sram(g_ReadPMTargetAddr, DSP_MEM_TYPE_PM);
                }
                break;
                case AU_DEC_ID3:
                {
        ...
        case Audio_infoType_Get_DDPEnc_Status:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            ...
            *pRet = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_DDP_ENCODE_EN, adec_id, 0);

            break;
        }

        case Audio_infoType_Is_DDP_8ch_Stream:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;

            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_DDP_71_STREAM, adec_id);
...
MS_S32 HAL_MAD_GetCodecCapability(AUDIO_CODEC_CAPABILITY_t * pCodecCapability)
{
    MS_S32 s32Ret  = 0;
    MS_U32 u32UserStrucVer = 0;
    MS_U32 u32MaxSupportCH = 0;

    if (pCodecCapability == NULL)
    {
        HALMAD_ERROR("%s: Error! pCodecCapability should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }

    u32UserStrucVer = pCodecCapability->u32StructVersion;

    if ((u32UserStrucVer & AUDIO_MAJOR_VERSION_MASK) != (AUDIO_CODEC_CAPABILITY_VERSION & AUDIO_MAJOR_VERSION_MASK))
    {
        HALMAD_ERROR("%s(): structure version is mismatch, user is 0x%08lX, driver is 0x%08X!\n", __FUNCTION__, (long unsigned int)u32UserStrucVer, (unsigned int)AUDIO_CODEC_CAPABILITY_VERSION);
        return -A_EPERM;
    }


    switch (pCodecCapability->CodecType)
    {
        case AUDIO_CODEC_MPEG:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_MPEG, ADEC1);
            break;
        }

        case AUDIO_CODEC_AC3:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_AC3, ADEC1);
            break;
        }
        ...
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_AAC, ADEC1);
            break;
        }

        case AUDIO_CODEC_DTS:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_DTS, ADEC1);
            break;
        }

        case AUDIO_CODEC_DTS_LBR:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_DTS_LBR, ADEC1);
            break;
        }

        case AUDIO_CODEC_DTS_HD:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_DTS_HD, ADEC1);
            break;
        }

        case AUDIO_CODEC_WMA:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_WMA, ADEC1);
            break;
        }

        case AUDIO_CODEC_DRA:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_DRA, ADEC1);
            break;
        }

        case AUDIO_CODEC_COOK:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_COOK, ADEC1);
            break;
        }

        case AUDIO_CODEC_XPCM:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_XPCM, ADEC1);
            break;
        }

        case AUDIO_CODEC_VORBIS:
        {
            u32MaxSupportCH = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CH_VORBIS, ADEC1);
            break;
        }

        default:
        {
            HALMAD_ERROR("%s: Error! Unsupportted CodecType %d! \n", __FUNCTION__, pCodecCapability->CodecType);
            s32Ret = -A_EINVAL;
            break;
        }
    ...
    pCodecCapability->SupportChannel = u32MaxSupportCH;
...
MS_S32 HAL_MAD_CommonCmd(AUDIO_COMMON_CMD_t * cmd)
{
    ...
    else if (!(strcmp(name, "GetCodecCapability")))
    {
        s32Ret = HAL_MAD_GetCodecCapability((AUDIO_CODEC_CAPABILITY_t *)cmd->pData);
    }
