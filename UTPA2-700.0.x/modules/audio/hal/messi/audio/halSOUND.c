...
void HAL_SOUND_Init2(void)
{
    ...
    HAL_SOUND_Init();
// Load decoder
    HAL_AUDSP_DspLoadCode(AU_DVB2_STANDARD_MP3);
// Load system
...
void HAL_SOUND_SetBass(MS_U8 u8Level)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Bass:%d",u8Level);
...
void HAL_SOUND_SetTreble(MS_U8 u8Level)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Treble:%d",u8Level);
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "==>%x\r\n",u8Value);
...
{
    HAL_AUDIO_WriteMaskByte(REG_SOUND_AD_VOLUME_HI, 0x07, (u8Vol1&0xE0)>>5);
    HAL_AUDIO_WriteMaskByte(REG_SOUND_AD_VOLUME, 0xFF, (((u8Vol1&0x1F)<<3) | (u8Vol2&0x07)) );
}
...
{
    if(bEnable)  // Mute
        HAL_AUDIO_WriteMaskReg(REG_SOUND_AD_VOLUME, 0x0400, 0x0400);
    else        // UnMute
        HAL_AUDIO_WriteMaskReg(REG_SOUND_AD_VOLUME, 0x0400, 0x0000);
}
        ...
        case Sound_SET_PARAM_AVC_S_Offset:
            {
                ...
                //HAL_MAD2_Write_DSP_sram(DSP2DmAddr_AvcSOffsetAddr, S_offset, DSP_MEM_TYPE_DM); //Cathy need to do
                status = TRUE;
                break;
            }
...
MS_U16 HAL_SND_GetParam( Sound_GET_PARAM_Type Type, MS_U16 param1)
{
    ...
    switch(Type)
    {
        ...
        case Sound_GET_PARAM_getSignal_Energy:
          //  tmp = HAL_MAD2_Read_DSP_sram(0x3F2B, DSP_MEM_TYPE_DM);
            break;
