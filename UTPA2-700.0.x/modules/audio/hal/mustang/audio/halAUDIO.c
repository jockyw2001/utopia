...
MS_BOOL HAL_AUDIO_ResetDefaultVars(AUDIO_SHARED_VARS2 * pAUDIOShared)
{
    ...
    pAUDIOShared->g_u8DecR2Adec1DecType = AU_DVB_STANDARD_INVALID;
    pAUDIOShared->g_u8DecR2Adec2DecType = AU_DVB_STANDARD_INVALID;
...
static MS_BOOL HAL_AUDIO_DigitalOut_DetermineDataPath(DIGITAL_OUTPUT_TYPE ePath, DigitalOutSetting_t *stDigitalOutSegtting)
{
    ...
    MS_U8 u8ADEC1CodeType = AU_DVB_STANDARD_INVALID;
    MS_U8 u8ADEC2CodeType = AU_DVB_STANDARD_INVALID;
...
void HAL_AUDIO_SPDIF_SetMode(MS_U8 u8Spdif_mode, MS_U8 u8Input_src)
{
    ...
    //HALAUDIO_ERROR("%s: u8Spdif_mode = %d, eAudioSource = %d, u8ADEC1CodeType = %d, u8ADEC2CodeType = %d, u8Input_src = %d\r\n", __FUNCTION__, u8Spdif_mode, eAudioSource, u8ADEC1CodeType, u8ADEC2CodeType, u8Input_src);
    ...
    switch(eAudioSource)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            ...
            switch(u8MainDspCodeType)
            {
                ...
                {
                    //Digital_Out_DD_Support = 1;
                    //Digital_Out_DDP_Support = 1;
                    /* 0: default, 1: support, 2: not support */
                    if (Digital_Out_DD_Support == 2)
                    {
                        u8SpdifMode  = SPDIF_OUT_PCM;
...
AUDIO_DSP_CODE_TYPE HAL_AUDIO_GetDspCodeType(void)
{
    ...
    return HAL_AUDIO_GetDecR2_ADEC1_DecType();
}
...
void HAL_AUDIO_RebootDecDSP(void)
{
    ...
    dec2cmd_status = HAL_MAD_GetDecCmd(DVB_Audio_Decoder2);
...
MS_BOOL HAL_AUDIO_SetSystem2(AUDIO_DEC_ID DecId, En_DVB_decSystemType Param)
{
    ...
    switch ( Param )
    {
        ...
        case MSAPI_AUD_DVB_MS10_DDT:
        {
            AUDIO_DSP_CODE_TYPE decType;
            HAL_MAD2_ReLoadCode(AU_DVB2_ENCODE_MS10_DDE);
            if (g_AudioVars2->DolbyAACFlag == 1)
            {
                HAL_AUR2_WriteByte(regDecoderType, adec_type_aac);
                decType = AU_DVB_STANDARD_MS10_DDT;
            }
            else
            {
                HAL_AUR2_WriteByte(regDecoderType, adec_type_gaac);
                decType = AU_DVB_STANDARD_AAC;
            }

            if (DecId == AU_DEC_ID3)
            {
                HAL_AUDIO_SetDecR2_ADEC2_DecType(decType, TRUE);
            }
            else
            {
                HAL_AUDIO_SetDecR2_ADEC1_DecType(decType, TRUE);
            }
...
MS_S32 HAL_AUDIO_PCM_Init(void)
{
    ...
    if (u32RequestClientInfoSize > PCM_SWMIXER_CLIENT_INFO_SIZE)
    {
        HALAUDIO_ERROR("%s: Error! request %u bytes ((%u + %u) * %u), but there is only %u\n",
            __FUNCTION__,
            (unsigned int)u32RequestClientInfoSize,
    ...
    pPcmBufferInfo = (AUDIO_PCM_BUFFER_INFO_t *)pPcmInfo + AUDIO_PCM_MAXIMUM;
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Restart(void)
{
    ...
    /* clear PCM buffer */
    memset((void *)MS_PA2KSEG1((int)pPCM->pPcmBufferInfo->pPhyBuffer), 0x00, HW_DMA_RDR1_BUF_SIZE);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Open(void *pData)
{
    ...
    pPCM->pPcmBufferInfo->pPhyBuffer = (MS_S8 *)u32TargetBufferAddrPa;
...
MS_U32 HAL_AUDIO_PCM_HwDma_Reader1_Write(const void *pBuf, MS_U32 u32Size)
{
    ...
    pBufTmp = (MS_S8 *)pBuf;
    pBuffer = (MS_S8 *)MS_PA2KSEG1((int)pPCM->pPcmBufferInfo->pPhyBuffer);
    if (pBuffer == NULL)
    {
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader2_Restart(void)
{
    ...
    /* clear PCM buffer */
    memset((void *)MS_PA2KSEG1((int)pPCM->pPcmBufferInfo->pPhyBuffer), 0x00, HW_DMA_RDR2_BUF_SIZE);
...
MS_S32 HAL_AUDIO_PCM_SwMixer_Client_Restart(AUDIO_PCM_t *pPCM)
{
    ...
    /* clear PCM buffer */
    memset((void *)MS_PA2KSEG1((MS_PHY)pPCM->pPcmBufferInfo->pPhyBuffer), 0x00, (PCM_SWMIXER_CLIENT_BUFFER_SIZE / AUDIO_PCM_SWMIXER_CLIENT_MAXIMUM));
...
MS_U32 HAL_AUDIO_PCM_SwMixer_Client_Write(AUDIO_PCM_t *pPCM, const void *pBuf, MS_U32 u32Size)
{
    ...
    pBufTmp = (MS_S8 *)pBuf;
    pBuffer = (MS_S8 *)MS_PA2KSEG1((MS_PHY)pPCM->pPcmBufferInfo->pPhyBuffer);
    if (pBuffer == NULL)
    {
        HALAUDIO_ERROR("%s: Error! pBuffer should not be NULL!\n", __FUNCTION__);
        return 0;
...
MS_S32 HAL_AUDIO_PCM_Capture1_Restart(void)
{
    ...
    /* clear PCM buffer */
    memset((void *)MS_PA2KSEG1((int)pPCM->pPcmBufferInfo->pPhyBuffer), 0x00, PCM_CAPTURE1_BUF_SIZE);
...
MS_S32 HAL_AUDIO_PCM_Capture2_Restart(void)
{
    ...
    /* clear PCM buffer */
    memset((void *)MS_PA2KSEG1((int)pPCM->pPcmBufferInfo->pPhyBuffer), 0x00, PCM_CAPTURE2_BUF_SIZE);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Writer1_Restart(void)
{
    ...
    /* clear PCM buffer */
    memset((void *)MS_PA2KSEG1((int)pPCM->pPcmBufferInfo->pPhyBuffer), 0x00, HW_DMA_WTR1_BUF_SIZE);
