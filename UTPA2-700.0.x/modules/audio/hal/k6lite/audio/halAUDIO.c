...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock_0[] =
{
    ...
    {0x160346, 0xff, 0x12}, //[7:0]   loop_div_second, R2 clock = 0x12 * 24 = 432MHz
...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock_1[] =
{
    ...
    //-----------------AudioBand----------------------------//Item 21
    {0x112b58, 0xff, 0x00}, //[15:0]  SDM fix value
    {0x112b59, 0xff, 0x00},
    {0x112b55, 0x08, 0x00}, //[11]    SDM fix enable
    {0x112b46, 0xff, 0x00}, //[15:0]  Enable Group C
    {0x112b47, 0xff, 0xff},
    {0x112b56, 0xff, 0x30}, //[15:0]  Enable ADC1/2
    {0x112b57, 0xff, 0x00},
    {0x112c6e, 0xff, 0x88}, //[7:0]   Enable Grp A/B
    {0x112b32, 0xff, 0x10}, //[4]     SDM fix MSB enable
    ...
    {0x112b52, 0xff, 0x0f}, //[3:0]   SDM DC offset enable
    {0x112b53, 0xff, 0xf0}, //[15:12] SDM Dither enable
...
const AUDIO_REG_TYPE  AudioInitTbl_0[]=
{
    ...
    {0x112c02, 0xff, 0x0f}, // [3:0] I2S/SIF/SPDIF clk EN, [4]=1 I2S left-justified , [4]=0 I2S format
    ...
    {0x112cae, 0x30, 0x00}, // [5:4] spdif rx synthesizer source SEL 432MHz
    {0x112caf, 0xff, 0x00}, // [15:13]i2s fs synthesizer SEL source clock 216MHz
...
//---------------REG_AUDIO_DECIMATION_CFG--------------------------//
    {0x112C58, 0x04, 0x04},       // AUDIO_HDMI_OUTPUT decimation mode always on
   ...
   switch(u8Mode)
   {
     ...
     case AUDIO_I2S_RXMODE:
        ...
        {   //0x2C8C[2], 2CAE[13], 2CCE[15:13][1:0]
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_I2S_TRX_TIMING_GEN, 0x10, 0x00);
        ...
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_I2S_TRX_TIMING_GEN, 0x10, 0x10);
    ...
    case AUDIO_I2S_RX_JUSTIFIED_MODE:
        if(u8Val==I2S_FORMAT_STANDARD)      //I2S_FORMAT_STANDARD
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x08,0x00);
        }
        else      //I2S_FORMAT_LEFT_JUSTIFIED
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x08,0x08);
        }

        break;
    //Div = 7680 / (Width*4 / (48K / Sample Rate) )  Ex: 16 bit Div = 7680/(16*4/3) = 360 = 0x168
    case AUDIO_I2S_RX_CLOCK:

        u8Width_Reg = HAL_AUDIO_ReadByte(REG_AUDIO_ASIF_TST + 1);

        if((u8Width_Reg & 0x18) == 0x00)
        {
            if(u8Val == I2S_SAMPLE_RATE_16K)
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x01);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST,0xFF,0x68);
            }
            else if(u8Val == I2S_SAMPLE_RATE_32K)
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST,0xFF,0xB4);
            }
            else if(u8Val == I2S_SAMPLE_RATE_48K)
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST,0xFF,0x78);
            }
        }
        else if((u8Width_Reg & 0x18) == 0x08)
        {
            if(u8Val == I2S_SAMPLE_RATE_16K)
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST,0xFF,0xF0);
            }
            else if(u8Val == I2S_SAMPLE_RATE_32K)
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
            ...
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST,0xFF,0x4F);
            }

        }
        else if((u8Width_Reg & 0x18) == 0x10)
        {
            if(u8Val == I2S_SAMPLE_RATE_16K)
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
            ...
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST,0xFF,0x5A);
            }
            else if(u8Val == I2S_SAMPLE_RATE_48K)
            {
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x01,0x00);
                HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST,0xFF,0x3B);
            }
    ...
    case AUDIO_I2S_RX_WORD_WIDTH:
        if(u8Val == I2S_WORD_WIDTH_16BIT)
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x18,0x00);
        }
        else if(u8Val == I2S_WORD_WIDTH_24BIT)
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x18,0x08);

        }
        else if(u8Val == I2S_WORD_WIDTH_32BIT)
        {
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_ASIF_TST + 1,0x18,0x10);
        }
    ...
    switch (eSource)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            ...
            else
            {
                ...
                {
                    if ((MainDspCodeType != AU_DVB_STANDARD_AC3) &&
                    (MainDspCodeType != AU_DVB_STANDARD_AC3P) &&
                    (MainDspCodeType != AU_DVB_STANDARD_DTS) &&
                    (MainDspCodeType != AU_DVB_STANDARD_DTSLBR) &&
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_DMIOReset()
/// @brief \b Function \b Description:  This function is used to do hw DMIO reset
/// @param <IN>        \b NONE    :
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE  :
/// @param <GLOBAL>    \b NONE    :
...
{
    HAL_AUDIO_WriteMaskByte(REG_MAD_DMA_IDLE_RESET, 0x01, 0x01);
    MsOS_DelayTask(1);
    HAL_AUDIO_WriteMaskByte(REG_MAD_DMA_IDLE_RESET, 0x01, 0x00);

    HAL_AUDIO_WriteMaskByte(REG_SE_DMA_IDLE_RESET, 0x01, 0x01);
    MsOS_DelayTask(1);
    HAL_AUDIO_WriteMaskByte(REG_SE_DMA_IDLE_RESET, 0x01, 0x00);
}
