...
void HAL_SOUND_SetDRCEntry(AUDIO_SOUNDEFFECT_TYPE Type, MS_U8 u8Enable)
{
    switch (Type)
    {
        case AUDIO_SOUNDEFFECT_SPEAKER:
            if(u8Enable)
            {
                HAL_AUDIO_WriteMaskReg(REG_SOUND_SNDEFF_CTRL2, 1<<(M2S_MBOX_SPK_VOL_BSND_CTRL_BIT), 1<<(M2S_MBOX_SPK_VOL_BSND_CTRL_BIT));
            }
            else
            {
                HAL_AUDIO_WriteMaskReg(REG_SOUND_SNDEFF_CTRL2, 1<<(M2S_MBOX_SPK_VOL_BSND_CTRL_BIT), 0x0000);
            }
            break;
        ...
        case AUDIO_SOUNDEFFECT_HEADPHONE:
            if(u8Enable)
            {
                HAL_AUDIO_WriteMaskReg(REG_SOUND_SNDEFF_CTRL2, 1<<(M2S_MBOX_HP_VOL_BSND_CTRL_BIT), 1<<(M2S_MBOX_HP_VOL_BSND_CTRL_BIT));
            }
            else
            {
                HAL_AUDIO_WriteMaskReg(REG_SOUND_SNDEFF_CTRL2, 1<<(M2S_MBOX_HP_VOL_BSND_CTRL_BIT), 0x0000);
            }
            break;
    ...
    case Sound_ENABL_Type_DRC:
       ...
       case Sound_ENABL_Type_KTVEcho:
        ...
        case Sound_ENABL_Type_HeadPhoneSoundEffect:
            if(enable)
            {
                HAL_AUDIO_WriteMaskReg(REG_SOUND_SNDEFF_CTRL2, 1<<(M2S_MBOX_HEADPHONEPSND_CTRL_BIT), 1<<(M2S_MBOX_HEADPHONEPSND_CTRL_BIT));
            }
            else
            {
                HAL_AUDIO_WriteMaskReg(REG_SOUND_SNDEFF_CTRL2, 1<<(M2S_MBOX_HEADPHONEPSND_CTRL_BIT), 0x0000);
            }
            status = TRUE;
            break;
