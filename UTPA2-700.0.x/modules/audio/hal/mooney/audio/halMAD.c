...
MS_BOOL g_DDPBypassenable = TRUE;
...
MMA_AudioType _GetAudioCaps(AUDIO_DEC_ID eDecId)
{
    ...
    if ( eDecId == AU_DEC_ID1 )
    {
        eAudioCaps = Audio_DEC_PCM        | Audio_DEC_MPEG     |
                     ...
                     Audio_DEC_DTSLBR     | Audio_DEC_Dolby_TrueHD_Bypass_R2|
                     Audio_DEC_DTS_HD_ADO_R2    | Audio_DEC_DRA;
    }
...
MS_BOOL HAL_MAD_MM2_initAesInfo( AUDIO_DEC_ID dec_id )
{
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, (Audio_id)dec_id, 0, 0);           //Set ES write pointer  to start point.
...
MS_BOOL HAL_MAD_MM2_checkAesInfo( AUDIO_DEC_ID  dec_id, AES_INFO *aes_info )
{
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        /* es level */
        audio_aes_info[0].aes_level = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, ADEC1);
        ...
        DBG_MAD(printf("[HAL_MAD_MM2_checkAesInfo] Ptr[w:%lx,r:%lx] LvL:%lx, FreeSpace:%lx, tmpFree:%lx \n",
...
MS_BOOL HAL_MAD_MM2_inputAesFinished( AUDIO_DEC_ID dec_id, MS_U32 es_size, MS_BOOL ptsExist, MS_U64 pts )
{
    ...
    Audio_id adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(dec_id);
    /* es level */
    ...
    if ( es_size > audio_aes_info[0].aes_freeSpace )
        {
        printf("==> Error, input AES size(0x%x) is larger than current Free space(0x%x)\r\n", es_size, audio_aes_info[0].aes_freeSpace );
        return FALSE;
        }

    DBG_MAD(printf("[HAL_MAD_MM2_inputAesFinished] wPtr:%08lx => ", audio_aes_info[0].aes_write_addr));
    ...
    DBG_MAD(printf("%08lx, size=%08lx,isPtsExist:%d,PTS=%09lld",
        audio_aes_info[0].aes_write_addr,
        es_size,
        ptsExist,
        pts/90));
...
MS_BOOL HAL_MAD_SetWmaInfo( Audio_WMA_infoType infoType, MS_U32 param1, MS_U32 param2 )
{
    switch(infoType)
    {
        ...
        default:
            printf("%s: infoType[%x,%x,%x] not Supported\n",__FUNCTION__, infoType, param1, param2);
            break;
    }
...
MS_BOOL HAL_MAD_SetAC3PInfo( Audio_AC3P_infoType infoType, MS_U32 param1, MS_U32 param2 )
{
    ...
    switch(infoType)
    {
        ...
        case Audio_AC3P_infoType_hdmiTxBypass_enable:
            ...
            if(g_DDPBypassenable != param1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DDP_HDMI_BYPASS,  ADEC1, param1, 0);
                HAL_AUDIO_DecWriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL+1, 0x40, (param1 << 6));
                g_DDPBypassenable = param1;
            }
            break;
        ...
        case Audio_AC3P_infoType_Mixer_Balance:
            {
                MS_U32 temp = 0;
                switch((DOLBY_MIXER_BALANCE_MODE)param1)
                {
                    case BALANCE_MAIN_0:
                        temp = 0;
                        break;
                ...
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_MIXER_BALANCE, ADEC1, temp, 0);
            }
            break;
...
MS_BOOL HAL_MAD_LoadCode(AUDIO_DSP_CODE_TYPE dspCodeType)
{
    ...
    if ((IS_AUDIO_DSP_CODE_TYPE_DEC1_SYSTEM(dspCodeType)) || (IS_AUDIO_DSP_CODE_TYPE_SIF(dspCodeType))) //mad load code
    {
        AUD_DBGMSG(printf("HAL_MAD_LoadCode(dspCodeType=dvb (%x))\r\n", dspCodeType));

        //HAL_AUDIO_WriteMaskByte(REG_AUDIO_DECODER1_CFG,0x07, 0x00);  // decoder1 Input --> DVB

        ret_status = HAL_AUDSP_DspLoadCode(dspCodeType);
        HAL_AUDIO_SetDspCodeType(dspCodeType, ret_status);
        HAL_AUDIO_SetIsDtvFlag(TRUE);
        return TRUE;
    }
    else
    {
        AUD_DBGMSG(printf("HAL_MAD_SIF_LoadCode: dspCodeType(%x) is invalid\r\n",dspCodeType));
        return FALSE;
    }
}
...
MS_BOOL HAL_MAD_XPCM_setParam2( AUDIO_DEC_ID DecId, Audio_XPCM_Param * pParam )
{
    ...
    MS_U8 adec_id = (MS_U8)HAL_AUDIO_Convert_DecId_to_ADECId(DecId);
...
MS_BOOL HAL_MAD_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    switch ( paramType )
    {
        ...
        case Audio_ParamType_omx_spdif_pcmLevel:
        {
            ...
            {
                HALMAD_PRINT ("%s: DecId %d Fail to setup PCM Level !!\r\n", __FUNCTION__, DecId);
            }
        }
        break;
        ...
        case Audio_ParamType_trueHD_bypass_enable:
            if(Param)
                g_AudioVars2->g_dolby_truehd_enable = TRUE;
            else
                g_AudioVars2->g_dolby_truehd_enable = FALSE;
            break;

        case Audio_ParamType_CommonCmd:
            if ( HAL_MAD_CommonCmd((AUDIO_COMMON_CMD_t *)Param) < 0 )
            {
                bRet = FALSE;
            }
            break;
...
MS_BOOL HAL_MAD_GetAudioInfo2( AUDIO_DEC_ID DecId, Audio_InfoType infoType, void * pInfo )
{
    ...
    switch ( infoType )
    {
        case Audio_infoType_UNI_PCM3_Base:
        {
            ...
            *pRet = (MS_U32)(HAL_AUDIO_GetDspMadBaseAddr(DSP_DEC) + OFFSET_MM_PCM_PRE_BUFFER_DRAM_ADDR + ASND_DSP_DDR_SIZE);
            break;
        }
        ...
        case Audio_infoType_UNI_PCM3_Size:
        {
            ...
            *pRet = MM_PCM_PRE_BUFFER_DRAM_SIZE;
            break;
        }
        ...
        case Audio_infoType_BufferAddr:
        {
            ...
            if ( DecId == AU_DEC_ID3 )
            {
                *pRet = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, ADEC1, 0) + OFFSET_ES3_DRAM_ADDR;
            }
            else
            {
        ...
        case Audio_infoType_get_trueHD_bypass_enable:
        {
            MS_U8 * pRet = (MS_U8 *) pInfo;
              if (g_AudioVars2->g_dolby_truehd_enable)
              {
                  *pRet = 1;
              }
              else
...
MS_S32 HAL_MAD_CommonCmd_TrasferData(void *pSrc, MS_U32 u32SrcSize, void *pDst, MS_U32 u32DstSzie)
{
    ...
    pTmpDst = (MS_U32 *)malloc(u32DstSzie);

    if (pTmpDst == NULL)
    {
        HALMAD_ERROR("%s: Error! no memory for pTmpDst!\n", __FUNCTION__);
        return -A_ENOMEM;
    }

    u32TransferSize = (u32SrcSize < u32DstSzie) ? u32SrcSize : u32DstSzie;

    memset((MS_U32 *)pTmpDst, 0x00, u32TransferSize);
    memcpy((MS_U32 *)pTmpDst, (MS_U32 *)pSrc, u32TransferSize);
    memcpy(((MS_U32 *)pDst)+2, ((MS_U32 *)pTmpDst)+2, u32DstSzie-8);
...
MS_S32 HAL_MAD_CommonCmd(AUDIO_COMMON_CMD_t * cmd)
{
    ...
    if (!(strcmp(name, "GetCodecCapability")))
    {
