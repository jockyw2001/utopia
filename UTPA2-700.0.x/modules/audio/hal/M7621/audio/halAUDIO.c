...
void HAL_AUDIO_SPDIF_SetMode(MS_U8 u8Spdif_mode, MS_U8 u8Input_src)
{
    ...
    /*
     * A patch here!
     * Change multi channel input source for HDMI / OMX decode in AU_DEC_ID3
     */
    if (((g_AudioVars2->AudioDecStatus[AU_DEC_ID3].eSourceType == E_AUDIO_INFO_MM_IN) ||
        (g_AudioVars2->AudioDecStatus[AU_DEC_ID3].eSourceType == E_AUDIO_INFO_HDMI_IN)) &&
        (g_AudioVars2->AudioDecStatus[AU_DEC_ID3].eGroup == E_CONNECT_MAIN))
    {
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Open(void *pData)
{
    ...
    pPcmInfo->u8Name[AUDIO_COMMON_PCM_IO_NAME_LENGTH-1] = '\0';
    strncpy((char *)pUserPcmInfo->u8Name, (char *)pPCM->u8Name, AUDIO_COMMON_PCM_IO_NAME_LENGTH);
    pUserPcmInfo->u8Name[AUDIO_COMMON_PCM_IO_NAME_LENGTH-1] = '\0';
