...
void HAL_SOUND_SetAvcMode(MS_U8 u8AvcMode )
{
    switch(u8AvcMode)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            HAL_AUDIO_WriteMaskByte(REG_SOUND_AVC_MODE ,0x03 ,0x03);
            break;
        default:
            break;
    }
...
/// @brief \b Function \b Description: This routine is used to bEnable/disable HPF featue.
...
{
    HAL_AUDIO_WriteMaskByte(M2S_MBOX_SNDEFF_EN, 0x04, bEnable<<2);
}
...
void HAL_SOUND_EnableKTVEcho(MS_BOOL bEnable)
{
    if(bEnable)
    {
        HAL_AUDIO_WriteMaskByte(0x2D47, 0x70, 0x10);   // 0x112D_46[14:12] = 0x1
    }
    else
    {
        HAL_AUDIO_WriteMaskByte(0x2D47, 0x70, 0x00);   // 0x112D_46[14:12] = 0x0
    }
}
...
void HAL_SOUND_SetAdAbsoluteVolume(MS_U8 u8Vol1, MS_U8 u8Vol2)
{
    ...
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AD_VOLUME, AU_DEC_ID1, Vol, 0);
...
{
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AD_VOLUME_MUTE, AU_DEC_ID1, bEnable, 0);
}
...
void HAL_SOUND_SetCH1AudioDelay(MS_U16 u16Delay)
{
    ...
    {
        if (u16Delay < KTV_DELAY_LOWER_BOUND)
        {
            printf("Warning! Invalid value (%u), KTV audio delay should be great than %u ms. \r\n", u16Delay, KTV_DELAY_LOWER_BOUND);
            u16Delay = KTV_DELAY_LOWER_BOUND;
        }
    }
    else
...
{
    return( HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_spdifDelay));
}
...
{
    return( HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_hpDelay));
}
////////////////////////////////////////////////////////////////////////////////
...
MS_BOOL HAL_SND_SetParam(Sound_SET_PARAM_Type Type, MS_U16 param1, MS_U16 param2)
{
    ...
    switch(Type)
    {
        ...
        case Sound_SET_PARAM_AVC_S_Offset:
        {
            ...
            HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_AvcSOffsetAddr, S_offset);
            status = TRUE;
        }
        break;
        ...
        case Sound_SET_PARAM_KTVEcho_XA:
        {
            HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_KTV_XAGain, param1);
            break;
        }

        case Sound_SET_PARAM_KTVEcho_XB:
        {
            HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_KTV_XBGain, param1);
            break;
        }

        case Sound_SET_PARAM_KTVEcho_ReverbStep:
        {
            if (param1 > 0x100)
            {
                param1 = 0x100; // surround delay buffer len max
            }

            HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_mSurrReverbLen, param1);
            HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_mSurrUpdateFlag, 0x1);
            break;
        }

        case Sound_SET_PARAM_Avc_R_Begin:
        {
            //default: 50 (-50dB), range: 0~60 (0dB ~ -60dB), End Must larger than Begin
            if(param1 <= 60)
            {
                HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_AVCRmodeP1, (param1 & 0xFFFE)); //param1 needs to be even
                HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_avcUpdateFlag, 1);
                status = TRUE;
            }
            break;
        }

        case Sound_SET_PARAM_Avc_R_End:
        {
            //default: 20 (-20dB), range: 0~60 (0dB ~ -60dB), End Must larger than Begin
            if(param1 <= 60)
            {
                HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_AVCRmodeP2, (param1 & 0xFFFE)); //param1 needs to be even
                HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_avcUpdateFlag, 1);
                status = TRUE;
            }
            break;
        ...
        case Sound_SET_PARAM_Avc_R_Slope:
        {
            //1: 0.75, 2: 0.5, 3: 0.25, default: 2
            if((param1 >= 1) && (param1 <= 3))
            {
                HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_AVCRmodeType, param1);
                HAL_MAD2_Write_DSP_Xbox(DSP2XboxAddr_avcUpdateFlag, 1);
                status = TRUE;
            }
            break;
...
MS_U16 HAL_SND_GetParam( Sound_GET_PARAM_Type Type, MS_U16 param1)
{
    ...
    switch(Type)
    {
        ...
        case Sound_GET_PARAM_KTVEcho_XA:
        {
            tmp = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_KTV_XAGain);
            break;
        }

        case Sound_GET_PARAM_KTVEcho_XB:
        {
            tmp = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_KTV_XBGain);
            break;
        }

        case Sound_GET_PARAM_KTVEcho_ReverbStep:
        {
            tmp = HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_mSurrReverbLen);
            break;
        }

        case Sound_GET_PARAM_Avc_R_Begin:
        {
            tmp = (MS_U16)HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_AVCRmodeP1);
            break;
        }

        case Sound_GET_PARAM_Avc_R_End:
        {
            tmp = (MS_U16)HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_AVCRmodeP2);
            break;
        }

        case Sound_GET_PARAM_Avc_R_Slope:
        {
            tmp = (MS_U16)HAL_MAD2_Read_DSP_Xbox(DSP2XboxAddr_AVCRmodeType);
            break;
        }
