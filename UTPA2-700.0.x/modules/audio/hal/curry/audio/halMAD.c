...
static MS_U32 g_ReadTargetXbox  = DSP2XboxAddr_IO_Info1;
static MS_U32 g_WriteTargetXobx = DSP2XboxAddr_IO_Info1;

static MS_BOOL g_bR2Adec1AD = FALSE;
static MS_BOOL g_bR2Adec2AD = FALSE;

MS_BOOL g_dolbyBulletin11_support = TRUE;
    ...
    else
    {
        ...
        switch(u8DecCmd)
        {
            case AU_DVB_DECCMD_STOP:
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FILE_REQ_SIZE, AU_DEC_ID1, 0, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, 0, 0);           //Set ES write pointer  to start point.
            ...
            case AU_DVB_DECCMD_STARTBROWSE:
                HAL_AUDIO_AbsWriteMaskByte(REG_R2_DECODE1_CMD, 0x20, AU_DEC_R2_CMD_STARTBROWSE);
                break;
        ...
        {
            u32_r2_cmd = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PLAY_STATE, AU_DEC_ID1)&0x00FFFFFF;
            AUDIO_DELAY1MS(1);
        ...
        if (u16_timeout >= 100 )
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, " ===  HAL_MAD_SetDecCmd %d fail===\n",u8DecCmd);
        }
    }
}
...
void HAL_MAD_SetDSP2DecCmd(AU_DVB_DECCMD u8DecCmd)
{
    ...
    else
    {
        ...
        {
            case AU_DVB_DECCMD_STOP:
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FILE_REQ_SIZE, AU_DEC_ID3, 0, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID3, 0, 0);           //Set ES write pointer  to start point.
            ...
            case AU_DVB_DECCMD_STARTBROWSE:
                HAL_AUDIO_AbsWriteMaskByte(REG_R2_DECODE2_CMD, 0x20, AU_DEC_R2_CMD_STARTBROWSE);
                break;
        ...
        while(u16_timeout++ < 100)
        {
            u32_r2_cmd = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PLAY_STATE, AU_DEC_ID3)&0x00FFFFFF;
            AUDIO_DELAY1MS(1);
            if (u32_r2_cmd == u32_driver_cmd)
                break;
        }
        ...
        if (u16_timeout >= 100 )
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, " ===  HAL_MAD_SetDSP2DecCmd %d fail===\n",u8DecCmd);
        }
    }
}
...
{
    MS_U8   Rpt_DTVES_tmp;
    Rpt_DTVES_tmp = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, AU_DEC_ID1);
...
MS_U32 HAL_MAD_ReadTimeStamp(void)
{
    ...
    pts = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CURR_PTS, AU_DEC_ID1);
    timestamp = pts/45;
...
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
static MS_U32 prv_PTS = 0, prv_PTS1 = 0, prv_PTS2 = 0;
static MS_U32 accum_ES = 0, prv_accum_ES = 0, accum_ES1 = 0, prv_accum_ES1 = 0, accum_ES2 = 0, prv_accum_ES2 = 0;
MS_BOOL HAL_MAD_MM2_initAesInfo( AUDIO_DEC_ID dec_id )
{
    ...
    else if( dec_id == AU_DEC_ID2 )
    {
        audio_aes_info[1].aes_base_addr =  OFFSET_ES2_DRAM_ADDR;
        audio_aes_info[1].aes_BufSize = ES2_DRAM_SIZE;
        ...
        DBG_MAD(printf("ID2: HAL_AUDIO_MM2_init end !!\r\n"));
        return TRUE;
    }
    else if( dec_id == AU_DEC_ID3 )
    {
        audio_aes_info[2].aes_base_addr =  OFFSET_ES3_DRAM_ADDR;
        audio_aes_info[2].aes_BufSize = ES3_DRAM_SIZE;
        audio_aes_info[2].aes_end_addr = audio_aes_info[2].aes_base_addr + audio_aes_info[2].aes_BufSize;
        audio_aes_info[2].aes_write_addr = audio_aes_info[2].aes_base_addr;
        audio_aes_info[2].aes_freeSpace = audio_aes_info[2].aes_BufSize -  (BYTES_IN_MIU_LINE*2);
        audio_aes_info[2].aes_level = 0;
        audio_aes_info[2].aes_lastPTS = 0;
        audio_aes_info[2].aes_ptsCnt = 0;
...
MS_BOOL HAL_MAD_MM2_checkAesInfo( AUDIO_DEC_ID  dec_id, AES_INFO *aes_info )
{
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        audio_aes_info[0].aes_read_addr = OFFSET_ES1_DRAM_ADDR + HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_ES_RD_PTR, AU_DEC_ID1 );
    ...
    else if( dec_id == AU_DEC_ID2 )
    {
        ...
        audio_aes_info[1].aes_read_addr = OFFSET_ES2_DRAM_ADDR + HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_ES_RD_PTR, AU_DEC_ID2 );

        memcpy( (void *)aes_info,  (void *)&audio_aes_info[1], sizeof(AES_INFO));
        DBG_MAD(printf("[ID2:HAL_MAD_MM2_checkAesInfo] Ptr[w:%x,r:%x] LvL:%x, FreeSpace:%x, tmpFree:%x \n",
    ...
    else if( dec_id == AU_DEC_ID3 )
    {
        ...
        /* es level */
        audio_aes_info[2].aes_level = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, dec_id);

        /* calculate free space for next data input */
        tmp1 = audio_aes_info[2].aes_end_addr - audio_aes_info[2].aes_write_addr;
        tmp2 = audio_aes_info[2].aes_BufSize - audio_aes_info[2].aes_level;

        if ( tmp2 <  audio_aes_info[2].aes_BufSize/2)
            tmp2 = 0;
        else
            tmp2 -= (audio_aes_info[2].aes_BufSize/2);

        if (tmp2 > (audio_aes_info[2].aes_BufSize/4))
            tmp2 = audio_aes_info[2].aes_BufSize/4;

        audio_aes_info[2].aes_freeSpace = (tmp1 > tmp2)?tmp2:tmp1;
        audio_aes_info[2].aes_read_addr = OFFSET_ES3_DRAM_ADDR + HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_ES_RD_PTR, AU_DEC_ID3 );

        memcpy( (void *)aes_info,  (void *)&audio_aes_info[2], sizeof(AES_INFO));
        DBG_MAD(printf("[ID3:HAL_MAD_MM2_checkAesInfo] Ptr[w:%x,r:%x] LvL:%x, FreeSpace:%x, tmpFree:%x \n",
            audio_aes_info[2].aes_write_addr,
            audio_aes_info[2].aes_read_addr,
            audio_aes_info[2].aes_level,
            audio_aes_info[2].aes_BufSize - audio_aes_info[2].aes_level,
            audio_aes_info[2].aes_freeSpace));
...
MS_BOOL HAL_MAD_MM2_inputAesFinished( AUDIO_DEC_ID dec_id, MS_U32 es_size, MS_BOOL ptsExist, MS_U64 pts )
{
    ...
    else if (dec_id == AU_DEC_ID2)
    {
        ...
        {
            printf("ID2:==> Error, input AES size(0x%x) is larger than current Free space(0x%x)\r\n", es_size, audio_aes_info[1].aes_freeSpace );
            return FALSE;
        }

        DBG_MAD(printf("[ID2:HAL_MAD_MM2_inputAesFinished] wPtr:%08x => ", audio_aes_info[1].aes_write_addr));
    ...
    else if (dec_id == AU_DEC_ID3)
    {
        ...
        if ( es_size > audio_aes_info[2].aes_freeSpace )
        {
            printf("ID3:==> Error, input AES size(0x%x) is larger than current Free space(0x%x)\r\n", es_size, audio_aes_info[2].aes_freeSpace );
            return FALSE;
        }

        DBG_MAD(printf("[ID3:HAL_MAD_MM2_inputAesFinished] wPtr:%08x => ", audio_aes_info[2].aes_write_addr));

        /* update write pointer */
        audio_aes_info[2].aes_write_addr += es_size;

        if ( audio_aes_info[2].aes_write_addr >= audio_aes_info[2].aes_end_addr )
            audio_aes_info[2].aes_write_addr -= audio_aes_info[2].aes_BufSize;
        ...
        if (ptsExist)
        {
            if (prv_PTS2 != 0)
            {
                ...
                (pts - prv_PTS2)/90,
                accum_ES2,
                accum_ES2 - prv_accum_ES2,
                (accum_ES2 - prv_accum_ES2)*32/1536));
            }
            else
            {
                DBG_MAD(printf(", PTS_diff=%08x, accum_ES = %08x\n", 0, 0));
        ...
        es_writeLineOffset = audio_aes_info[2].aes_write_addr - audio_aes_info[2].aes_base_addr;

        if (ptsExist)
        {
            if ((audio_aes_info[2].aes_lastPTS  == pts)&&(audio_aes_info[2].aes_ptsCnt != 0))
                printf(" ======== Error PTS is same !! =============\n");
            else
            {
                    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_INPUT_PTS, dec_id, (pts >> 1), 0);
                    audio_aes_info[2].aes_ptsCnt++;
                    audio_aes_info[2].aes_lastPTS = pts;
                    AUDIO_DELAY1US(500);
            }
        }
...
MS_U32 HAL_MAD_MPEG_GetHeaderInfo(void)
{
    ...
    data = (MS_U16)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MPEG_HEADER, AU_DEC_ID1);
...
{
    return (MS_U16)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID1);
}
...
{
   return (MS_U16) HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_BIT_RATE, AU_DEC_ID1);
}
...
{
    return (MPEG_SOUND_MODE)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MPEG_STEREOMODE, AU_DEC_ID1);
}
...
void HAL_MAD_Monitor_DDPlus_SPDIF_Rate(void)
{
    ...
    MS_U8 u8Dsp1CodeType = MSAPI_AUD_DVB_INVALID;
    MS_U8 u8Dsp2CodeType = MSAPI_AUD_DVB_INVALID;
    Digital_Out_Channel_Status_t stDigitalChannelStatusTx1, stDigitalChannelStatusTx2;
    MS_BOOL retTx1, retTx2;
    MS_U8 u8MainADEC = AU_DEC_ID1;
    AUDIO_DEC_ID DecId = AU_DEC_ID1;
    ...
    u8Dsp1CodeType = g_AudioVars2->g_DspCodeType;
    u8Dsp2CodeType = g_AudioVars2->g_Dsp2CodeType;
    ...
    switch(eAudioSource)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            ...
            if(g_AudioVars2->g_R2_NonPcm_Sel == AU_DEC_ID1)
            {
                u8MainDspCodeType = u8Dsp1CodeType;
                u8MainADEC = AU_DEC_ID1;
                DecId = AU_DEC_ID1;
            }
            else
            {
                u8MainDspCodeType = u8Dsp2CodeType;
                u8MainADEC = AU_DEC_ID3;
                DecId = AU_DEC_ID3;
            }
            ...
            switch(u8MainDspCodeType)
            {
                ...
                case AU_DVB_STANDARD_MS10_DDC:
                {
                    ...
                    switch(HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, u8MainADEC))
                    {
                        case 44100:
                        {
                ...
                case AU_DVB_STANDARD_DTSLBR:
                {
                    ...
                    u32spdif_info_flag = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TRANSCODE_FLAG, u8MainADEC);
                    if(u32spdif_info_flag == 0) //u32spdif_info_flag == 0 means NPCM output processing is at DSP or SND-R2 for M2
                    {
                        /* 48KHz */
                    ...
                    {
                        if (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, u8MainADEC) == 44100)
                        {
                            /* 44.1KHz */
...
void HAL_MAD_WMA_SetASFParm(WMA_ASF_PARMTYPE parm_type, MS_U32 value)
{
    switch ( parm_type )
    {
        case WMA_PARAMTYPE_VERSION:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_VERSION, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_CHANNELS:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_CHANNELS, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_SAMPLERATE:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_SAMPLERATE, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_BYTERATE:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_BYTERATE, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_BLOCKALIGN:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_BLOCKALIGN, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_ENCOPT:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_ENCOPT, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_PARSINGBYAPP:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_PARSINGBYAPP, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_BITS_PER_SAMPLE:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_BITS_PER_SAMPLE, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_CHANNELMASK:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_CHANNELMASK, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_DRC_PARAM_EXIST:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_PARAM_EXIST, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_DRC_RMS_AMP_REF:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_RMS_AMP_REF, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_DRC_RMS_AMP_TARGET:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_RMS_AMP_TARGET, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_DRC_PEAK_AMP_REF:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_PEAK_AMP_REF, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_DRC_PEAK_AMP_TARGET:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_PEAK_AMP_TARGET, AU_DEC_ID1, value, 0);
            break;
        case WMA_PARAMTYPE_MAX_PACKET_SIZE:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_MAX_PACKET_SIZE, AU_DEC_ID1, value, 0);
            break;
        default:
            break;
    }
                           ...
                           MS_U16* FrameSize)
{
    ...
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_NUMCODECS, AU_DEC_ID1, mNumCodecs, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_SAMPLES, AU_DEC_ID1, mSamples, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_SAMPLERATES, AU_DEC_ID1, mSampleRate, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, Channels[0], 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, Regions[0], 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, cplStart[0], 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, cplQbits[0], 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, FrameSize[0], 0 );

    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, Channels[1], 1 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, Regions[1], 1 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, cplStart[1], 1 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, cplQbits[1], 1 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, FrameSize[1], 1 );

    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, Channels[2], 2 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, Regions[2], 2 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, cplStart[2], 2 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, cplQbits[2], 2 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, FrameSize[2], 2 );

    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, Channels[3], 3 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, Regions[3], 3 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, cplStart[3], 3 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, cplQbits[3], 3 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, FrameSize[3], 3 );

    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, Channels[4], 4 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, Regions[4], 4 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, cplStart[4], 4 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, cplQbits[4], 4 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, FrameSize[4], 4 );
    ...
    switch(infoType)
    {
        ...
        ********************************************************************************/
        case Audio_Comm_infoType_Set_UNI_NEED_DECODE_FRMCNT:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_UNI_SETUP_DECODE_FRMCNT, AU_DEC_ID1, param1, 0);
            break;

        case Audio_Comm_infoType_Set_UNI_ES_Wptr:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, param1, 0);
            break;
        ...
        case Audio_Comm_infoType_syncSTC_in_mmTs:
            if ( param1 )
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_TS_SYNC_STC, AU_DEC_ID1, 1, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_TS_SYNC_STC, AU_DEC_ID1, 0, 0);
            break;

        case Audio_Comm_infoType_PUSI_disable_Info:
            if(param1)
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_PUSI_DISABLE, AU_DEC_ID1, 1, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_PUSI_DISABLE, AU_DEC_ID1, 0, 0);
            break;

        case Audio_Comm_infoType_Parser_Spec:
            if(param1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_SPEC, AU_DEC_ID1, 1, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_PUSI_DISABLE, AU_DEC_ID1, 1, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_SPEC, AU_DEC_ID1, 0, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_PARSER_PUSI_DISABLE, AU_DEC_ID1, 0, 0);
            }
            break;
        ...
        case Audio_Comm_infoType_Set_DFS_PowerSave:
        {
            ...
            MS_U8 u8MainDspCodeType = g_AudioVars2->g_DspCodeType;
            if(param1)
            {
                    ...
                    ((HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL)&0x02) == 0x02))
                {
                    if(param2<4)
                    {
                        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: When NonPCM case, audio fs can't set under level 4\n", __FUNCTION__ );
                        mistake_case = TRUE;
                        param2 = 4;
                    }
                 }
                else if(HAL_AUR2_ReadByte(REG_R2_DECODE1_CMD)!=0x0)  //not_stop case
                {
                    switch(u8MainDspCodeType)
                    {
                        ...
                        case AU_DVB_STANDARD_DTSLBR:
                            if(param2<3)
                            {
                                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: When playing AAC/DTS stream, audio fs can't set under level 3\n", __FUNCTION__ );
                                mistake_case = TRUE;
                                param2 = 3;
                            }
                            break;
                        ...
                        default:
                            if(param2<2)
                            {
                                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: When playing any stream except AAC/DTS, audio fs can't set under level 2\n", __FUNCTION__ );
                                mistake_case = TRUE;
                                param2 = 2;
                             }
                            break;
	 ...
	 case Audio_Comm_infoType_DAC_Clock_OnOff:  //DAC off for power saving
	 {
            HAL_AUDIO_DAC_Clock_OnOff(param1);
	 }
	 break;
	 ...
	 /********************************************************************************
        ...
        ********************************************************************************/
        case Audio_Comm_infoType_ADEC1_Set_play_smpRate:
        {
            ...
            if (param1)
            {
                ...
                synth_H <<= 16;
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_EXT_SMPRATE_CTRL, AU_DEC_ID1, 1, synth_H);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_EXT_SMPRATE_CTRL, AU_DEC_ID1, 0, 0x11940000);
            }
        }
        break;
        ...
        case Audio_Comm_infoType_ADEC1_soundmod:
            {
                ...
                //NOT_DEFINE : to do
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_SOUND_MODE, AU_DEC_ID1, value, 0);
            }
            break;
        ...
        case Audio_Comm_infoType_MMFileSize:                    //HAL_MAD_SetMadFileSize
            {
                ...
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FILE_REQ_SIZE, AU_DEC_ID1, file_size_value, 0);
            }
            break;
        ...
        case Audio_Comm_infoType_DEC1_MMTag:                    //HAL_MAD_SetMMIntTag
            {
               MS_U32 es_wrPtr = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, 0);
               MS_U32 es_bufSz = ES1_DRAM_SIZE;
               MS_U32 es_reqsz = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID1);
                es_wrPtr += es_reqsz;

                if ( es_wrPtr >= es_bufSz )
                    es_wrPtr -= es_bufSz;

                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, es_wrPtr, 0);
            }
            break;
        ...
        case Audio_Comm_infoType_MM_FFx2:                       //HAL_MAD_SetMMFFx2Mode
            if ( param1 )
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FF2X, AU_DEC_ID1, 1, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FF2X, AU_DEC_ID1, 0, 0);
            break;

        case Audio_Comm_infoType_ADEC1_mute:
            if ( param1 )
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DECODE_MUTE, AU_DEC_ID1, 1, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DECODE_MUTE, AU_DEC_ID1, 0, 0);
            break;
        ...
        case Audio_Comm_infoType_ADEC1_UNI_PCMOutCnt:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_UNI_PCM_SET_OUTPUT_CNT, AU_DEC_ID1, param1, 0);
            break;
        ...
        case Audio_Comm_infoType_ADEC1_33bit_PTS:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_REPORT_MM_TS_PTS_WITH_MSB, AU_DEC_ID1, param1, 0);
            break;
        ...
        case Audio_Comm_infoType_DEC2_MMTag:                    //HAL_MAD2_SetMMIntTag
            {
                ...
                es_wrPtr = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID3, 0);
                ...
                if ( es_wrPtr >= es_bufSz )
                   es_wrPtr -= es_bufSz;
                 HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID3, es_wrPtr, 0);
            }
            break;

        case Audio_Comm_infoType_ADEC2_syncSTC_in_mmTs:
            if ( param1 )
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_TS_SYNC_STC, AU_DEC_ID3, 1, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_TS_SYNC_STC, AU_DEC_ID3, 0, 0);
            break;
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_Vorbis_header_size:
            if ( param1 )
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_VORBIS_HEADER_SIZE, AU_DEC_ID1, param2, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_VORBIS_HEADER_SIZE, AU_DEC_ID1, param2, 0);
        ...
        case Audio_Comm_infoType_Set_Parser:
            ...
            switch(param1)
            {
                ...
                case 3 :  // SubWindow ADEC2 :  AFIFO_B->ES3  ;
                    HAL_AUDIO_WriteMaskReg(REG_AUDIO_PARSER_CTRL, 0x0030, 0x0000); // AFIFO_A-> ES1
                    HAL_AUDIO_WriteMaskReg(REG_AUDIO_PARSER_CTRL, 0x0180, 0x0080); // AFIFO_C-> ES2(Dummy)
        ...
        case Audio_Comm_infoType_STC_Select:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECTION, AU_DEC_ID1, param1, 0);
            break;
        ...
        case Audio_Comm_infoType_Multi_Channel:
        {
            ...
            {
                printf("!!!!!!!!!set Audio_Comm_infoType_Multi_Channel = [0x%x] \n", (unsigned int)param1);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MULTI_CHANNEL, AU_DEC_ID1, param1, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MULTI_CHANNEL, AU_DEC_ID3, param1, 0);
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Invalid value(0x%x) to enable/disable Multi-Channel\n", __FUNCTION__, (unsigned int)param1);
            }

            break;
        }

        case Audio_Comm_infoType_ES_PassThrough_DelaySmp:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_PassThrough_DelaySmp, AU_DEC_ID1, param1, 0);
            break;
        }

        case Audio_Comm_infoType_ES_PassThrough_PcmSmpRate:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_PassThrough_PcmSamRate, AU_DEC_ID1, param1, 0);
            break;
        }

        case Audio_Comm_infoType_ES_PassThrough_HdmiSmpRate:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_PassThrough_HdmiSmpRate, AU_DEC_ID1, param1, 0);
            break;
        }

        case Audio_Comm_infoType_HDMITx_Status:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AUR2_HDMITx_TypeChanged, AU_DEC_ID1, param1, param2);
            break;
        }
    ...
    switch(infoType)
    {
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_Get_UNI_ES_MEMCNT:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, AU_DEC_ID1);
            break;
        ...
        case Audio_Comm_infoType_Get_UNI_PCM_Base:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_ADDR, AU_DEC_ID1);
            break;

        case Audio_Comm_infoType_Get_UNI_PCM_Size:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_SIZE, AU_DEC_ID1);
            break;
...
//            result = HAL_MAD_Read_DSP_sram(REG_PTS_AccumEsCnt, DSP_MEM_TYPE_DM);
        ...
        case Audio_Comm_infoType_Is_Audio_Sync:
            {
                ...
                tmp = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_STATE, AU_DEC_ID1)&0x0F;
                if (tmp != 2)
                {
                    result = 0;
                    break;
        ...
        case Audio_Comm_infoType_Parser_Info:
                result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PARSER_INFO, AU_DEC_ID1);
            break;
        /********************************************************************************
        *       hdmiTx
        ********************************************************************************/
        case Audio_Comm_infoType_hdmiTx_outType:
        {
            ...
            if(HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL+1)&0x20)
            {
                ...
                {
                    r2_hdmiTx_HBR = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMIbuf_HBR, AU_DEC_ID1);
                }
                else
                {
                    r2_hdmiTx_HBR = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMIbuf_HBR, AU_DEC_ID3);
                }
        ...
        case Audio_Comm_infoType_hdmiTx_outFreq:
        {
            if(HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL+1)&0x20)
            {
                ...
                if(HAL_AUR2_ReadByte(REG_R2_HDMI_NONPCM_PATH) == ADEC1)
                {
                    r2_hdmiTx_smpRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMITx_SMP_RATE, AU_DEC_ID1);
                }
                else
                {
                    r2_hdmiTx_smpRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMITx_SMP_RATE, AU_DEC_ID3);
                }
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_DecStatus:                     //no change
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, AU_DEC_ID1);
            break;
        ...
        case Audio_Comm_infoType_SampleRate:                    //HAL_MAD_GetMadSamplingRate
        {
            ...
            r2_smpRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID1);
            result = AUDIO_FS_NONE;
        ...
        case Audio_Comm_infoType_ADEC1_bitRate:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_BIT_RATE, AU_DEC_ID1);
            break;
        ...
        case Audio_Comm_infoType_ChannelMode:                   //HAL_MAD_GetMadChannelMode
        {
            ...
            ac_mode = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MODE, AU_DEC_ID1);
            channel_mapping = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MAPPING, AU_DEC_ID1);
        ...
        case Audio_Comm_infoType_ADEC1_okFrmCnt:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OK_FRMCNT, AU_DEC_ID1);
            break;

        case Audio_Comm_infoType_ADEC1_errFrmCnt:
        case Audio_Comm_infoType_DecodeErrorCnt:                //HAL_MAD_Get_DecodeErrorCnt
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ERR_FRMCNT, AU_DEC_ID1);
            break;
        ...
        case Audio_Comm_infoType_33Bit_PTS:                     //HAL_MAD_GetMadPts
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CURR_PTS, AU_DEC_ID1);
            result <<= 1;
            break;

        case Audio_Comm_infoType_ADEC1_getlatest_PTS:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PTS, AU_DEC_ID1);
            result <<= 1;
            break;

        case Audio_Comm_infoType_ADEC1_33bit_STCPTS_DIFF:
        case Audio_Comm_infoType_33Bit_STCPTS_DIFF:             //HAL_MAD_GetMadPtsStcDelta
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TD, AU_DEC_ID1);
        ...
        case Audio_Comm_infoType_ADEC1_Get_PTS_latency:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PTS_LATENCY, AU_DEC_ID1);
        ...
        case Audio_Comm_infoType_ADEC1_Get_PCM_bufferEmpty:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_EMPTY_CNT, AU_DEC_ID1);
            break;
        ...
        case Audio_Comm_infoType_DEC1_BufferAddr:               //HAL_MAD_GetBufferAddr
            result = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, 0) + OFFSET_ES1_DRAM_ADDR;
            break;

        case Audio_Comm_infoType_ADEC1_esBuf_reqAddr:
            result = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, 0) + OFFSET_ES1_DRAM_ADDR + HAL_AUDIO_GetDspMadBaseAddr(DSP_SE);
            break;

        case Audio_Comm_infoType_ADEC1_esBuf_reqSize:
        case Audio_Comm_infoType_DEC1_BufferSize:               //HAL_MAD_GetBufferSize
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID1);
            break;
        ...
        case Audio_Comm_infoType_DEC1_PCMBufferSize:            //HAL_MAD_GetPCMBufferSize
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_DMX_LEVEL, AU_DEC_ID1);
            break;

        case Audio_Comm_infoType_ADEC1_UNI_pcm_wptr:
            result = HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + ASND_DSP_DDR_SIZE + OFFSET_PCM1_DRAM_ADDR + HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_WR_PTR, AU_DEC_ID1);
            break;

        case Audio_Comm_infoType_ADEC1_UNI_pcm_decSize:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UNI_DECODE_DONE_PCM_SIZE, AU_DEC_ID1);
            break;
        ...
        case Audio_Comm_infoType_syncSTC_in_mmTs:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_MMTsSyncFlag, AU_DEC_ID1);
            break;

        case Audio_Comm_infoType_ADEC1_getDecChannelInfo:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MAPPING, AU_DEC_ID1);
            break;

        case Audio_Comm_infoType_ADEC1_audio_unsupported:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UNSUPPORT_TYPE, AU_DEC_ID1);
            break;
        ...
        *******************************************************************************/
        ...
        case Audio_Comm_infoType_ADEC2_currAudDecStatus:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, AU_DEC_ID3);
            break;

        case Audio_Comm_infoType_ADEC2_sampleRate:
            {
                MS_U32 r2_smpRate = 0xFFFF;
                r2_smpRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID3);
                result = AUDIO_FS_NONE;
        ...
        case Audio_Comm_infoType_ADEC2_bitRate:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_BIT_RATE, AU_DEC_ID3);
            break;
        ...
        case Audio_Comm_infoType_ADEC2_acmod:
        {
            MS_U32 ac_mode = 0xFF;
            MS_U32 channel_mapping = 0x0;
            ac_mode = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MODE, AU_DEC_ID3);
            channel_mapping = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MAPPING, AU_DEC_ID3);
        ...
        case Audio_Comm_infoType_ADEC2_1ms_timeStamp:
            {
                MS_U32  pts;
                pts = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CURR_PTS, AU_DEC_ID3);
                result = pts/45;
            }
            break;

        case Audio_Comm_infoType_ADEC2_33bit_PTS:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CURR_PTS, AU_DEC_ID3);
            result <<= 1;
            break;

        case Audio_Comm_infoType_ADEC2_33bit_STCPTS_DIFF:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TD, AU_DEC_ID3);
            //sign ext
            result <<= 32;
            result >>= 31;
            break;
        ...
        case Audio_Comm_infoType_DEC2_BufferAddr:               //HAL_MAD2_GetBufferAddr
            result = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID3, 0) + OFFSET_ES2_DRAM_ADDR;
            break;

        case Audio_Comm_infoType_ADEC2_esBuf_reqAddr:
            result = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID3, 0) + OFFSET_ES2_DRAM_ADDR + HAL_AUDIO_GetDspMadBaseAddr(DSP_SE);
            break;

        case Audio_Comm_infoType_ADEC2_esBuf_reqSize:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID3);
            break;
        ...
        case Audio_Comm_infoType_ADEC2_esBuf_currLevel:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, AU_DEC_ID3);
            break;

        case Audio_Comm_infoType_ADEC2_pcmBuf_currLevel:
        case Audio_Comm_infoType_DEC2_MMResidualPCM:            //HAL_MAD2_GetMMResidualPCM
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_DMX_LEVEL, AU_DEC_ID3);
            break;

        case Audio_Comm_infoType_ADEC2_syncSTC_in_mmTs:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_MMTsSyncFlag, AU_DEC_ID3);
            break;
        ...
        case Audio_Comm_infoType_ADEC2_getDecChannelInfo:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MAPPING, AU_DEC_ID3);
            break;
        ...
        ********************************************************************************/
        ...
        case Audio_Comm_infoType_Get_Parser_Architecture:
            result = E_AUD_4_PARSER_type1; // 2 AFIFO & 4 audio parser arch
            break;
        ...
        case Audio_comm_infoType_Get_Main_Input_Type:
            result = g_AudioVars2->AudioSrcType;
            break;

        case Audio_comm_infoType_Get_Sub_Input_Type:
            result = g_AudioVars2->AudioSubSrcType;
            break;

        case Audio_Comm_infoType_AUDIO_GetDsp2CodeType:
        {
            result = HAL_AUDIO_GetDsp2CodeType();
            break;
        }
    ...
    switch(infoType)
    {
        case Audio_MPEG_infoType_SoundMode:                 //HAL_MAD_MPEG_SetSoundMode
            {
                ...
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MPEG_SOUNDMODE, AU_DEC_ID1, (MS_U32)value, 0);
            }
            break;
        ...
        case Audio_MPEG_DTV_AVSYNC_OFFSET:
            if ((param1<DELAY_LOWER_LIMIT)||(param1>DELAY_UPPER_LIMIT))              //delay offset range from 30ms ~ 300ms
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MPG_AVOFFSET, AU_DEC_ID1, (MS_U32)MPG_AV_DEFAULT_DELAY, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MPG_AVOFFSET, AU_DEC_ID1, (MS_U32)param1, 0);
            break;
...
MS_U32 HAL_MAD_GetMpegInfo( Audio_MPEG_infoType infoType )
{
    ...
    switch(infoType)
    {
        case Audio_MPEG_infoType_DecStatus:     //no change
            if (((HAL_AUDIO_AbsReadByte(REG_R2_DECODE1_TYPE) & adec_type_regmask) == adec_type_mpeg) && (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, AU_DEC_ID1) & 0x0001))
                result = 1;
            else
                result = 0;
            break;
        ...
        case Audio_MPEG_infoType_Layer:         //HAL_MAD_MPEG_GetLayer
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MPEG_LAYER, AU_DEC_ID1);
            break;
...
MS_U32 HAL_MAD_GetWmaInfo( Audio_WMA_infoType infoType )
{
    ...
    switch(infoType)
    {
        ...
        case Audio_WMA_infoType_SampleRate:                 //HAL_MAD_WMA_GetSampleRate
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID1);
            break;
...
{
    HALMAD_CHECK_SHM_INIT;
    MS_U8 ADEC1_CodeType = g_AudioVars2->g_DspCodeType ;
    MS_U8 ADEC2_CodeType = g_AudioVars2->g_Dsp2CodeType ;
    ...
    switch(infoType)
    {
        case Audio_AC3P_infoType_DownmixMode:                //HAL_MAD_SetDolbyDownmixMode
            ...
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DMX_MODE, AU_DEC_ID1, 1, 0);
            }
            else if ((DOLBY_DOWNMIX_MODE)param1 == DOLBY_DOWNMIX_MODE_LTRT)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DMX_MODE, AU_DEC_ID1, 0, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DMX_MODE, AU_DEC_ID1, 2, 0);
            }
            break;
        ...
        case Audio_AC3P_infoType_DrcMode:                    //HAL_MAD_SetDolbyDrcMode
            ...
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_RF_MODE, AU_DEC_ID1, 1, 0);
                g_AudioVars2->g_dolbyDrcMode = RF_MODE;
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_RF_MODE, AU_DEC_ID1, 0, 0);
                g_AudioVars2->g_dolbyDrcMode = LINE_MODE;
            }
        ...
        case Audio_AC3P_infoType_hdmiTxBypass_enable:
            ...
            // MS12_hashkey on
            if(g_AudioVars2->g_ddenc_owner)
            {
                if( g_AudioVars2->g_R2_NonPcm_Sel == AU_DEC_ID1 )
                {
                    if(ADEC1_CodeType == AU_DVB_STANDARD_AC3 || ADEC1_CodeType == AU_DVB_STANDARD_AC3P || ADEC1_CodeType == AU_DVB_STANDARD_AAC)
                    {
                        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN,  AU_DEC_ID1, param1, 0);
                        //printf("ADEC1 DoblbyCodeType = %x\n", ADEC1_CodeType);
                    }
                    else
                    {
                        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN,  AU_DEC_ID1, 0, 0);
                        //printf("ADEC1 CodeType = %x\n", ADEC1_CodeType);
                    }
                }
                else if( g_AudioVars2->g_R2_NonPcm_Sel == AU_DEC_ID3 )
                {
                    if(ADEC2_CodeType == AU_DVB_STANDARD_AC3 || ADEC2_CodeType == AU_DVB_STANDARD_AC3P || ADEC2_CodeType == AU_DVB_STANDARD_AAC)
                    {
                        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN,  AU_DEC_ID3, param1, 0);
                        //printf("ADEC2 DoblbyCodeType = %x\n", ADEC2_CodeType);
                    }
                    else
                    {
                        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN,  AU_DEC_ID3, 0, 0);
                        //printf("ADEC2 CodeType = %x\n", ADEC2_CodeType);
                    }
                }
                else
                {
                ...
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DDP_ENCODE_EN,  param2, param1, 0);
            }
            break;
        ...
        case Audio_AC3P_DTV_AVSYNC_OFFSET:
            if ((param1<DELAY_LOWER_LIMIT)||(param1>DELAY_UPPER_LIMIT))
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AC3_AVOFFSET, AU_DEC_ID1, AC3_AV_DEFAULT_DELAY, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AC3_AVOFFSET, AU_DEC_ID1, param1, 0);
            break;
        ...
        case Audio_AC3P_infoType_LoudnessMatching:
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_RFSIGNAL_TYPE, AU_DEC_ID1, g_AudioVars2->g_dolbyLoudness_DTVsystem, 0);
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_TB11_ENABLE, AU_DEC_ID1, g_AudioVars2->g_dolbyLoudness_Enable, 0);
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_RFSIGNAL_TYPE, AU_DEC_ID3, g_AudioVars2->g_dolbyLoudness_DTVsystem, 0);
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_TB11_ENABLE, AU_DEC_ID3, g_AudioVars2->g_dolbyLoudness_Enable, 0);
            break;

        case Audio_AC3P_infoType_Drc_HighCutScale:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_HIGH_CUT, AU_DEC_ID1, param1, 0);
            break;

        case Audio_AC3P_infoType_Drc_LowBootScale:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_LOW_BOOST, AU_DEC_ID1, param1, 0);
            break;

        case Audio_AC3P_infoType_AD_Mixer_Balance:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_MIXER_BALANCE, AU_DEC_ID1, param1, 0);
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_MIXER_BALANCE, AU_DEC_ID3, param1, 0);
            break;
    ...
    switch(infoType)
    {
        case Audio_AC3P_infoType_DecStatus:
            if ((((HAL_AUDIO_AbsReadByte(REG_R2_DECODE1_TYPE) & adec_type_regmask) == adec_type_ac3) ||
                ...
                (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, AU_DEC_ID1)&0x0001))
                result = 1;
            else
                result = 0;
            break;
        ...
        case Audio_AC3P_infoType_SampleRate:
            {
                ...
                SMP_RATE = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID1);        //DD+
                switch(SMP_RATE)
                {
        ...
        case Audio_AC3P_infoType_BitRate:  //unit : kbps
            {
                result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_BIT_RATE, AU_DEC_ID1);        //DD+
            }
            break;
        ...
        case Audio_AC3P_infoType_DrcMode:
            if (HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_DOLBY_RF_MODE, AU_DEC_ID1, 0) == 1)
                result = RF_MODE;
            else
                result = LINE_MODE;
            break;
        ...
        case Audio_AC3P_infoType_DownmixMode:
            if (HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_DOLBY_DMX_MODE, AU_DEC_ID1, 0) == 1)
        ...
        case Audio_AC3P_infoType_AC3Type:
            if ((((HAL_AUDIO_AbsReadByte(REG_R2_DECODE1_TYPE) & adec_type_regmask) == adec_type_ac3) ||
                ...
                (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, AU_DEC_ID1) & 0x0001))
            {
                result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_DEC_TYPE, AU_DEC_ID1);
                result += 1;
            }
            else
            {
        ...
        case Audio_AC3P_infoType_FrameSize:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_FRAME_SIZE, AU_DEC_ID1);
            break;

        case Audio_AC3P_infoType_BSMode:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_BS_MODE, AU_DEC_ID1);
            break;

        case Audio_AC3P_infoType_DDP_71_Stream:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_DDP_71_STREAM, AU_DEC_ID1);
        break;
...
MS_BOOL HAL_MAD_SetDTSCommonCtrl( Audio_DTS_infoType infoType, Audio_DTS_CtrlType ctrl_type )
{
    switch(infoType)
    {
        case Audio_DTS_infoType_DMIX_LFE_EN:
            ...
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_DMX_LFE_ENABLE, AU_DEC_ID1, 0, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_DMX_LFE_ENABLE, AU_DEC_ID1, 1, 0);
            }
            break;
        ...
        case Audio_DTS_infoType_DownmixMode:
            ...
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_DMX_MODE, AU_DEC_ID1, 0, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_DMX_MODE, AU_DEC_ID1, 1, 0);
            }
            break;
        ...
        case Audio_DTS_infoType_DTSHD_HDMITxBypass_Enable:
            ...
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_HD_HDMI_BYPASS,  AU_DEC_ID1, Audio_DTS_Ctrl_DTSHDBypass_ON, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_HD_HDMI_BYPASS,  AU_DEC_ID3, Audio_DTS_Ctrl_DTSHDBypass_ON, 0);
                HAL_AUDIO_DecWriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL+1, 0x40, 0x40);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_HD_HDMI_BYPASS,  AU_DEC_ID1, Audio_DTS_Ctrl_DTSHDBypass_OFF, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_HD_HDMI_BYPASS,  AU_DEC_ID3, Audio_DTS_Ctrl_DTSHDBypass_OFF, 0);
                HAL_AUDIO_DecWriteMaskByte(REG_M2D_MAILBOX_SPDIF_CTRL+1, 0x40, 0x00);
            }
            break;
...
MS_U32 HAL_MAD_GetDTSInfo( Audio_DTS_infoType infoType )
{
    ...
    switch(infoType)
    {
        case Audio_DTS_infoType_DecStatus:
            if (((HAL_AUDIO_AbsReadByte(REG_R2_DECODE1_TYPE) & adec_type_regmask) == adec_type_dts) && (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, AU_DEC_ID1)&0x0001))
                result = 1;
            else
                result = 0;
            break;
        ...
        case Audio_DTS_infoType_CD_MODE:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DTS_CD_MODE, AU_DEC_ID1);
            break;
...
MS_BOOL HAL_MAD_SetAACInfo( Audio_AAC_infoType infoType, MS_VIRT param1, MS_VIRT param2 )
{
    switch(infoType)
    {
        ...
        case Audio_AAC_infoType_DrcMode:                    //HAL_MAD_SetDolbyDrcMode
            ...
            g_AudioVars2->g_dolbyDrcMode = (DOLBY_DRC_MODE)param1;
            break;

        case Audio_AAC_DTV_AVSYNC_OFFSET:
            if ((param1<DELAY_LOWER_LIMIT)||(param1>DELAY_UPPER_LIMIT))                                                      //delay offset range from 30ms ~ 300ms
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AAC_AVOFFSET, AU_DEC_ID1, AAC_AV_DEFAULT_DELAY, 0);
            else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AAC_AVOFFSET, AU_DEC_ID1, param1, 0);
            break;

        case Audio_AAC_mm_single_frame_mode:
            if (param1 > 0)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_MUL_FRAME_HEADER, AU_DEC_ID1, 1, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_MUL_FRAME_HEADER, AU_DEC_ID1, 0, 0);
            }
            break;
...
MS_U32 HAL_MAD_GetAACInfo( Audio_AAC_infoType infoType )
{
    ...
    switch(infoType)
    {
        ...
        case Audio_AAC_type:
            result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_AAC_TYPE, AU_DEC_ID1);
            break;
...
MS_BOOL HAL_MAD_SetRA8LBRInfo( Audio_RA8LBR_infoType infoType, MS_U32 param1, MS_U32 param2 )
{
    switch ( infoType )
    {
        case Audio_RA8LBR_NumCodes:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_NUMCODECS, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_Samples:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_SAMPLES, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_SampleRate:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_SAMPLERATES, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_Channels1:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_Regions1:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_cplStart1:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_cplQbits1:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_FrameSize1:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, param1, 0 );
            break;
        case Audio_RA8LBR_Channels2:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, param1, 1 );
            break;
        case Audio_RA8LBR_Regions2:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, param1, 1 );
            break;
        case Audio_RA8LBR_cplStart2:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, param1, 1 );
            break;
        case Audio_RA8LBR_cplQbits2:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, param1, 1 );
            break;
        case Audio_RA8LBR_FrameSize2:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, param1, 1 );
            break;
        case Audio_RA8LBR_Channels3:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, param1, 2 );
            break;
        case Audio_RA8LBR_Regions3:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, param1, 2 );
            break;
        case Audio_RA8LBR_cplStart3:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, param1, 2 );
            break;
        case Audio_RA8LBR_cplQbits3:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, param1, 2 );
            break;
        case Audio_RA8LBR_FrameSize3:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, param1, 2 );
            break;
        case Audio_RA8LBR_Channels4:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, param1, 3 );
            break;
        case Audio_RA8LBR_Regions4:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, param1, 3 );
            break;
        case Audio_RA8LBR_cplStart4:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, param1, 3 );
            break;
        case Audio_RA8LBR_cplQbits4:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, param1, 3 );
            break;
        case Audio_RA8LBR_FrameSize4:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, param1, 3 );
            break;
        case Audio_RA8LBR_Channels5:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, AU_DEC_ID1, param1, 4 );
            break;
        case Audio_RA8LBR_Regions5:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, AU_DEC_ID1, param1, 4 );
            break;
        case Audio_RA8LBR_cplStart5:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, AU_DEC_ID1, param1, 4 );
            break;
        case Audio_RA8LBR_cplQbits5:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, AU_DEC_ID1, param1, 4 );
            break;
        case Audio_RA8LBR_FrameSize5:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, AU_DEC_ID1, param1, 4 );
            break;
        default:
            printf("%s: infoType[%x,%08x,%08x] not Supported\n",__FUNCTION__, infoType, param1, param2);
            break;
...
////////////////////////////////////////////////////////////////////////////////
MS_U8 HAL_MAD_XPCM_setParam(XPCM_TYPE audioType, MS_U8  channels, MS_U32 sampleRate,
                                                    MS_U8  bitsPerSample, MS_U16 blockSize, MS_U16 samplePerBlock)
{
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_TYPE, AU_DEC_ID1, audioType, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_CHANNELS, AU_DEC_ID1, channels, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_SAMPLERATES, AU_DEC_ID1, sampleRate, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_BITS_PER_SAMPLE, AU_DEC_ID1, bitsPerSample, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_BLOCK_SIZE, AU_DEC_ID1, blockSize, 0 );
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_SAMPLE_PER_BLOCK, AU_DEC_ID1, samplePerBlock, 0 );
...
MS_BOOL HAL_MAD_XPCM_setParam2( AUDIO_DEC_ID DecId, Audio_XPCM_Param * pParam )
{
    ...
    if(pParamXPCM->sampleRate == 30464) //patch for 96K sample rate
    {
        s32smprate = 96000;
    }
    ...
    if ( DecId < AU_DEC_MAX)
    {
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_TYPE, DecId, (MS_U32) pParamXPCM->audioType, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_CHANNELS, DecId, (MS_U32) pParamXPCM->channels, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_SAMPLERATES, DecId, s32smprate, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_BITS_PER_SAMPLE, DecId, (MS_U32) pParamXPCM->bitsPerSample, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_BLOCK_SIZE, DecId, (MS_U32) pParamXPCM->blockSize, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_XPCM_SAMPLE_PER_BLOCK, DecId, (MS_U32) pParamXPCM->samplePerBlock, 0 );
        return TRUE;
    }
    else
    {
...
MS_BOOL HAL_MAD_COOK_setParam2( AUDIO_DEC_ID DecId, Audio_COOK_Param * pParam )
{
    ...
    if ( DecId < AU_DEC_MAX )
    {
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_NUMCODECS, DecId, pParamCOOK->mNumCodecs, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_SAMPLES, DecId, pParamCOOK->mSamples, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_SAMPLERATES, DecId, pParamCOOK->mSampleRate, 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, DecId, pParamCOOK->Channels[0], 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, DecId, pParamCOOK->Regions[0], 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, DecId, pParamCOOK->cplStart[0], 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, DecId, pParamCOOK->cplQbits[0], 0 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, DecId, pParamCOOK->FrameSize[0], 0 );

        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, DecId, pParamCOOK->Channels[1], 1 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, DecId, pParamCOOK->Regions[1], 1 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, DecId, pParamCOOK->cplStart[1], 1 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, DecId, pParamCOOK->cplQbits[1], 1 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, DecId, pParamCOOK->FrameSize[1], 1 );

        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, DecId, pParamCOOK->Channels[2], 2 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, DecId, pParamCOOK->Regions[2], 2 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, DecId, pParamCOOK->cplStart[2], 2 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, DecId, pParamCOOK->cplQbits[2], 2 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, DecId, pParamCOOK->FrameSize[2], 2 );

        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, DecId, pParamCOOK->Channels[3], 3 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, DecId, pParamCOOK->Regions[3], 3 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, DecId, pParamCOOK->cplStart[3], 3 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, DecId, pParamCOOK->cplQbits[3], 3 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, DecId, pParamCOOK->FrameSize[3], 3 );

        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CHANNELS, DecId, pParamCOOK->Channels[4], 4 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_REGIONS, DecId, pParamCOOK->Regions[4], 4 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLSTART, DecId, pParamCOOK->cplStart[4], 4 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_CPLQBITS, DecId, pParamCOOK->cplQbits[4], 4 );
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_COOK_FRAMESIZE, DecId, pParamCOOK->FrameSize[4], 4 );
...
MS_BOOL HAL_MAD_WMA_SetASFParm2( AUDIO_DEC_ID DecId, Audio_ASF_Param * pParam )
{
    ...
    if ( DecId < AU_DEC_MAX )
    {
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_VERSION, DecId, pParam->u32Version, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_CHANNELS, DecId, pParam->u32Channels, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_SAMPLERATE, DecId, pParam->u32SampleRate, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_BYTERATE, DecId, pParam->u32ByteRate, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_BLOCKALIGN, DecId, pParam->u32BlockAlign, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_ENCOPT, DecId, pParam->u32Encopt, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_PARSINGBYAPP, DecId, pParam->u32ParsingByApp, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_BITS_PER_SAMPLE, DecId, pParam->u32BitsPerSample, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_CHANNELMASK, DecId, pParam->u32ChannelMask, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_PARAM_EXIST, DecId, pParam->u32DrcParamExist, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_RMS_AMP_REF, DecId, pParam->u32DrcRmsAmpRef, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_RMS_AMP_TARGET, DecId, pParam->u32DrcRmsAmpTarget, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_PEAK_AMP_REF, DecId, pParam->u32DrcPeakAmpRef, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_DRC_PEAK_AMP_TARGET, DecId, pParam->u32DrcPeakAmpTarget, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ASF_MAX_PACKET_SIZE, DecId, pParam->u32MaxPacketSize, 0);
        return TRUE;
    }
    else
    {
...
MS_BOOL HAL_MAD_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    switch ( paramType )
    {
        ...
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_TS_SYNC_STC, DecId, Param, 0);
            break;
        }
        ...
        case Audio_ParamType_acMode:
        {
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_SOUND_MODE, AU_DEC_ID1, Param, 0);
            break;
        }
        ...
        case Audio_ParamType_reqDataSize:
        {
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FILE_REQ_SIZE, DecId, Param, 0);
            break;
        }
        ...
        case Audio_ParamType_MM_FFx2:
        {
            if ( Param )
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FF2X, DecId, 1, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_MM_FF2X, DecId, 0, 0);
            }
            break;
        }
        ...
        case Audio_ParamType_mute:
        {
            if ( Param )
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DECODE_MUTE, DecId, 1, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DECODE_MUTE, DecId, 0, 0);
            }
            break;
        }
        ...
        case Audio_ParamType_esBuf_informDataRdy:
            ...
            {
                MS_U32 es_wrPtr, es_bufSz, es_reqsz;
                es_wrPtr = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID2, 0);
                es_bufSz = ES2_DRAM_SIZE;
                es_reqsz = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID2);;
                es_wrPtr += es_reqsz;

                if ( es_wrPtr >= es_bufSz )
                   es_wrPtr -= es_bufSz;
                 HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID2, es_wrPtr, 0);
            }
            else if (DecId == AU_DEC_ID3)
            {
                ...
                es_reqsz = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID3);;
                es_wrPtr += es_reqsz;
        ...
        case Audio_ParamType_UNI_NEED_DECODE_FRMCNT:
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_UNI_SETUP_DECODE_FRMCNT, DecId, Param, 0);
            break;

        case Audio_ParamType_UNI_ES_Wptr:  // OMX Wptr = offset to MAD start address
            if(DecId == AU_DEC_ID3)  // ES For 2nd Main Decoder
            {
                Param = Param - (MS_U32)(ADEC__R2_DDR_SIZE + ASND__R2_DDR_SIZE + OFFSET_ES3_DRAM_ADDR);
            }
            else if(DecId == AU_DEC_ID2)  // ES For 1st AD Decoder
            {
                Param = Param - (MS_U32)(ADEC__R2_DDR_SIZE + ASND__R2_DDR_SIZE + OFFSET_ES2_DRAM_ADDR);
            }
            else  // ES For 1st Main Decoder
            {
...
#endif
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, DecId , Param, 0);
            break;

        case Audio_ParamType_UNI_PCMOutCnt:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_UNI_PCM_SET_OUTPUT_CNT, DecId , Param, 0);
            break;
        }
        ...
        case Audio_ParamType_AllocDecoder:
        {
            ...
            if ( (eAudioCaps & eAudioType) == eAudioType )
            {
                ...
                if ( bRet == TRUE )
                {
                    ...
                    //patch for curry sub dtv because of hw unsupport dual dtv.------chasen.cai
                    if (DecId == AU_DEC_ID3)
                    {
                        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES2_IsUsedBy_Dual, AU_DEC_ID3, 0, 0);
                    }
                }
            }
            else
        ...
        case Audio_ParamType_FreeDecoder:
                    ...
                    (g_AudioVars2->AudioDecStatus[DecId].eSourceType == E_AUDIO_INFO_GAME_IN))
            {
                    ...
                    {
                        g_AudioVars2->g_Dsp2CodeType = AU_DVB_NONE;
                    }
                }
                else
                {
        ...
        case Audio_ParamType_omx_spdif_mode:
            ...
            if ( DecId <= AU_DEC_ID3 )
            {
                ...
                u32SpdifParam = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_OMX_SPDIF_CTRL, DecId, 0);
                ...
                if ( (bRet = HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_OMX_SPDIF_CTRL, DecId, u32SpdifParam, 0)) != TRUE )
                {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Param Type %d DecId %d Un-Support Now !!\r\n", __FUNCTION__, Audio_ParamType_omx_spdif_mode, DecId);
                bRet = FALSE;
            }
            break;
        ...
        case Audio_ParamType_omx_spdif_pcmLevel:
            ...
            if ( DecId  <= AU_DEC_ID3 )
            {
                if ( (bRet = HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_OMX_SPDIF_PCM_LEVEL, DecId, Param, 0)) != TRUE )
                {
        ...
        case Audio_ParamType_tvos2_PCM_rdPtr:
            ...
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TVOS2_PCM_RDPTR, DecId, Param, 0);
            break;

        case Audio_ParamType_tvos2_DTV_SyncType:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TVOS2_DTV_SYNCTYPE, DecId, Param, 0);
            break;

        case Audio_R2_SHM_PARAM_DOLBY_DRC_MODE:
        {
            bRet =  HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DRC_MODE, DecId, Param, 0);
        }
        break;
        ...
        case Audio_R2_SHM_PARAM_DOLBY_DMX_MODE:
        {
            ...
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DMX_MODE, DecId, 1, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DMX_MODE, DecId, 0, 0);
            }
        }
        break;
        ...
        {
            bRet =  HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_HIGH_CUT, DecId, Param, 0);
        }
        break;

        case Audio_R2_SHM_PARAM_DOLBY_LOW_BOOST:
        {
            bRet =  HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_LOW_BOOST, DecId, Param, 0);
        }
        break;

        case Audio_R2_SHM_PARAM_AD_Volume:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_AD_VOLUME, DecId, Param, 0);
        }
        break;
        ...
        case Audio_ParamType_Abs_ReadXbox_Target:
        {
            g_ReadTargetXbox = Param;
        }
        break;

        case Audio_ParamType_Abs_WriteXbox_Target:
        {
            g_WriteTargetXobx = Param;
        }
        break;

        case Audio_ParamType_Abs_WriteXbox_Value:
        {
            HAL_MAD2_Write_DSP_Xbox(g_WriteTargetXobx, Param);
        }
        break;

        case Audio_ParamType_omx_spdif_pause:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_OMX_SPDIF_PAUSE, DecId, Param, 0);
        }
        break;

        case Audio_ParamType_Audio_AAC_mm_single_frame_mode:
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_MUL_FRAME_HEADER, DecId, Param, 0);
            break;

        case Audio_ParamType_Low_Latency_Limiter_ES_enable:
        {
            if(Param)
            {
                HAL_AUDIO_AbsWriteMaskReg(REG_R2_WORD_CTRL, 0x8000, 0x8000);
            }
            else
            {
                HAL_AUDIO_AbsWriteMaskReg(REG_R2_WORD_CTRL, 0x8000, 0x0000);
            }
        }
        break;

        case Audio_ParamType_Low_Latency_Limiter_ES_Threshold:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_LOW_LATENCY_LIMITER_ES_THRESHOLD, DecId, Param, 0);
        }
        break;

        case Audio_ParamType_Low_Latency_Limiter_PCM_Threshold:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_LOW_LATENCY_LIMITER_PCM_THRESHOLD, DecId, Param, 0);
        }
        break;

        case Audio_ParamType_Low_Latency_Limiter_PCM_Skip_Target:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_LOW_LATENCY_LIMITER_PCM_SKIP_TARGET, DecId, Param, 0);
        }
        break;

        case Audio_ParamType_ES2_IsUsedBy_Dual:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES2_IsUsedBy_Dual, DecId, Param, 0);
        }
        break;

        case Audio_ParamType_ddp_disable_atmos:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DDP_DISABLE_ATMOS, DecId, Param, 0);
        }
        break;

        case Audio_ParamType_Decoder_RepeatPlay:
        {
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES_REPEAT_MODE, DecId, Param, 0);
        }
        break;
...
MS_BOOL HAL_MAD_GetAudioInfo2( AUDIO_DEC_ID DecId, Audio_InfoType infoType, void * pInfo )
{
    ...
    switch ( infoType )
    {
        ...
        case Audio_infoType_DecStatus:                     //no change
        {
            ...
            if ( HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, DecId) != 0 )
                {
                    *pResult = 1;
                }
                else
        ...
        case Audio_infoType_SampleRate:                    //HAL_MAD_GetMadSamplingRate
        {
            ...
            fs_type = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, DecId);
        ...
        case Audio_infoType_okFrmCnt:
        {
            ...
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OK_FRMCNT, DecId);
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ERR_FRMCNT, DecId);
        ...
        case Audio_infoType_1ms_timeStamp:
        {
            ...
            MS_U32  pts;
            pts = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CURR_PTS, DecId);
            *pRet = pts/45;
        ...
        case Audio_infoType_33bit_PTS:
        {
            ...
            if(DecId >= AU_DEC_MAX)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: DEC_ID[%d]  not Supported\n", __FUNCTION__, DecId);
                return FALSE;
            }

            *pRet = ((MS_U64)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_CURR_PTS, DecId))<<1;
            break;
        }
        ...
        case Audio_infoType_33bit_latestPTS:
        {
            ...
            if(DecId >= AU_DEC_MAX)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: DEC_ID  not Supported\n", __FUNCTION__ );
                return FALSE;
            }

            *pRet = ((MS_U64)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PTS, DecId))<<1;
            break;
        }
        ...
        case Audio_infoType_BufferAddr:
        {
            ...
            if ( DecId == AU_DEC_ID3 )
            {
                *pRet = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID3, 0) + OFFSET_ES3_DRAM_ADDR;
            }
            else if ( DecId == AU_DEC_ID2)
            {
                *pRet = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID2, 0) + OFFSET_ES2_DRAM_ADDR;
            }
            else
            {
                *pRet = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, 0) + OFFSET_ES1_DRAM_ADDR;
            }
            break;
        }
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, DecId);
        ...
        case Audio_infoType_esBuf_req:
        {
            ...
            if ( bFlag )
            {
                ...
                if ( DecId == AU_DEC_ID3 )
                {
                    pDataReq->U32WrtBytes = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID3);
                    pDataReq->U32WrtAddr  = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID3, 0) + OFFSET_ES3_DRAM_ADDR + HAL_AUDIO_GetDspMadBaseAddr(DSP_SE);
                }
                else
                {
                    pDataReq->U32WrtBytes = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID1);
                    pDataReq->U32WrtAddr  = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, AU_DEC_ID1, 0) + OFFSET_ES1_DRAM_ADDR + HAL_AUDIO_GetDspMadBaseAddr(DSP_SE);
                }
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, DecId);
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_DMX_LEVEL, DecId);
        ...
        case Audio_infoType_UNI_PCM1_Info:
        {
           ...
           if ( HAL_AUDIO_GetUniDecodeFrameCnt(DecId) != g_AudioVars2->u16DecodeFrmCnt[DecId] )
           {
                ...
                if ( DecId == AU_DEC_ID3 )
                {
                    pPCMInfo->U32WrtAddr = HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + ASND_DSP_DDR_SIZE + OFFSET_PCM2_DRAM_ADDR+HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UNI_DECODE_DONE_PCM_ADDR, DecId);
                }
                else
                {
                    pPCMInfo->U32WrtAddr = HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + ASND_DSP_DDR_SIZE + OFFSET_PCM1_DRAM_ADDR+ HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UNI_DECODE_DONE_PCM_ADDR, DecId);
                }
                pPCMInfo->U32WrtBytes = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_UNI_DECODE_DONE_PCM_SIZE, DecId);
#if OMX_AUDIO_DEBUG
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            MS_U32 R2_ES_wrptr, R2_ES_rdptr;
            MS_S32 ES_level;  //for high rate ES data input
            R2_ES_wrptr = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_ES_WR_PTR, DecId, 0);
            R2_ES_rdptr = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_RD_PTR, DecId);
            ES_level = (MS_S32)(R2_ES_wrptr - R2_ES_rdptr);

            if(ES_level < 0)
            {
                if(DecId == AU_DEC_ID1)
                    ES_level += (ES1_DRAM_SIZE);
                else if(DecId == AU_DEC_ID2)
                    ES_level += (ES2_DRAM_SIZE);
                else
                    ES_level += (ES3_DRAM_SIZE);
            }
            *pRet  = (MS_U32) ES_level;
            break;
#if OMX_AUDIO_DEBUG
        ...
        case Audio_infoType_UNI_ES_Base:
        {
            ...
            {
                *pRet = (MS_U32) (HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + OFFSET_ES2_DRAM_ADDR);
            }
            else
            {
        ...
        case Audio_infoType_hashkey:
        {
            ...
            MS_U32 audio_ipauth = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_HASH_KEY, AU_DEC_ID1, 0);
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ACCUM_ES_BYTECNT, DecId);
            break;
        }
        ...
        case Audio_infoType_Pcm_Output_Channel:
        {
            ...
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_OUTPUT_CHANNEL, DecId);
            break;
        }
        ...
        case Audio_infoType_Decoder_Type:
        {
            ...
            switch (DecId)
            {
                ...
                default:
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: invalid ID (%d)\n", __FUNCTION__, DecId);
                    result = FALSE;
                    break;
                }
            }
            ...
            if (result == TRUE)
            {
                switch (eDecoder)
                {
                    ...
                    case AU_DVB_STANDARD_DTSLBR:
                    {
                        u32DecoderType = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DTS_ES_TYPE, DecId) & 0x00FFFFFF;
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_MM_TS_SYNC_STC, DecId, 0);
            break;
        }
        /* TVOS2.0 DTV */
        case Audio_infoType_tvos2_PCM1_Info:
        {
            ...
            pPCMInfo->U32WriteAddr  = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TVOS2_PCM_WRPT, DecId) +  HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + ASND_DSP_DDR_SIZE + OFFSET_PCM1_DRAM_ADDR;
                           u64cpts  = ((MS_U64)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TVOS2_CPTS, DecId))<<1;
                           u64cpts  =  STC_TICK_TO_NS(u64cpts);
            pPCMInfo->S64CPTS       = (MS_S64)u64cpts;
            pPCMInfo->U32FrameSize  = (MS_U32)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TVOS2_PCM_FRAMESIZE, DecId);
            pPCMInfo->U32Channels   = (MS_U32)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TVOS2_PCM_CH, DecId);
            pPCMInfo->U32Bps        = 2;
            pPCMInfo->U32SampleRate = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, DecId);
            break;
        }
        ...
        case Audio_infoType_Abs_ReadXbox:
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;

            *pRet = HAL_MAD2_Read_DSP_Xbox(g_ReadTargetXbox);
        }
        break;
        ...
        {
            MS_U32 * pRet = (MS_U32 *) pInfo;
            *pRet = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PARSER_INFO, DecId);
            break;
        }
        ...
        case Audio_infoType_Is_Audio_Sync:
        {
            if ( DecId == AU_DEC_ID3 )
            {
                ...
                tmp = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_STATE, AU_DEC_ID3)&0x0F;
                if (tmp != 2)
                {
                    result = FALSE;
                    break;
