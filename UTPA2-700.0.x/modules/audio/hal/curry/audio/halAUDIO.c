...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock[] =
{
    ...
    {0x112c70, 0xff, 0xd0}, //[1:0]   AUPLL_KP0, [7:2]AUPLL_FBDIV
    ...
    {0x163e04, 0x04, 0x04}, //[2]     REG_ENABLE_CLK_PasDMA_RIU_BRG
    {0x163e04, 0x03, 0x01}, //[1:0]   REG_PasDMA_RIU_W_SEL R2
    ...
    {0x112b50, 0xff, 0x00}, //[15:0]  DC offset value
    {0x112b51, 0xff, 0x00},
    {0x112b60, 0xff, 0xc0},
    {0x112b61, 0xff, 0x38},
    {0x112b62, 0xff, 0x38},
    ...
    {0x112b55, 0xff, 0x01},
    {0x112b52, 0xff, 0x00}, //[15:0]  DC offset ENABLE=0, Dither ENABLE=0
    {0x112b53, 0xff, 0x00},
    ...
    //-----------------De-POP_1-----------------------------//Item 25
    {0x112cdc, 0xff, 0xf0},    //PD_DAC
    {0x112cdd, 0xff, 0x3f},
    {0x112cde, 0xff, 0x00},    //EN_CK_DAC
    {0x112cdf, 0xff, 0x00},
    {0x112ce6, 0xff, 0x01},    //PD_VI
    {0x112ce7, 0xff, 0x00},
    {0x112ce8, 0xff, 0x00},    //PD_VREF
    {0x112ce9, 0xff, 0x80},
    {0x112cec, 0xff, 0x10},    //PD_LDO_DAC,EN_QS_LDO_DAC
    {0x112ced, 0xff, 0x00},
    {0x112cee, 0xff, 0x00},    //EN_MSP
    {0x112cef, 0xff, 0x00},
    {0xffffff, 0x01, 0x01},    //delay    1ms

    {0x112cde, 0xff, 0x00},    //EN_CK_DAC
    {0x112cdf, 0xff, 0x08},
    {0x112cec, 0xff, 0x08},    //PD_LDO_DAC,EN_QS_LDO_DAC
    {0x112ced, 0xff, 0x00},
    {0x112cee, 0xff, 0x00},    //EN_MSP
    {0x112cef, 0xff, 0x10},
    {0xffffff, 0x01, 0x01},    //delay    1ms

    {0x112cdc, 0xff, 0x00},    //PD_DAC
    {0x112cdd, 0xff, 0x00},
    {0x112ce6, 0xff, 0x00},    //PD_VI
    {0x112ce7, 0xff, 0x00},
    {0x112ce8, 0xff, 0x00},    //PD_VREF
    {0x112ce9, 0xff, 0x00},
...
{
    //-----------------De-POP_2-----------------------------//Item 26
    {0x112cec, 0xff, 0x00},    //PD_LDO_DAC,EN_QS_LDO_DAC
    {0x112ced, 0xff, 0x00},
    {0x112cee, 0xff, 0x00},    //EN_MSP
    {0x112cef, 0xff, 0x00},
    ...
    {0x112c45, 0xff, 0x32}, // [1:0]/[5:4]/[9:8]/[13:12] ch1/ch2/ch3/ch4 matrix
    ...
    {0x112ca4, 0x08, 0x08}, // [3:2] CLK 256fs rx/tx EN
    {0x112ca4, 0x04, 0x04},
    ...
    {0x112c8a, 0x40, 0x00},
    {0x112c8a, 0x20, 0x00},
    {0x112c8a, 0x90, 0x90}, // [4] EN_SPDIF_OUT_CS (referred from SPDIF_OUT_CS0~4)
    ...
    // RD mark for not to set priority
    ...
    //-----------------Interrupt--------------------------//Item 35
    {0x112aa2, 0x60, 0x00}, //[6]       IRQ2n select HDMI TX
    {0x112aa2, 0x80, 0x80}, //[7]       IRQE1 select SPDIF_TX
...
//-----------------------------------------------------
    ...
    {0x112D5B, 0xFF, 0x0C},
    {0x112D5A, 0xFF, 0x00},
    {0x16038B, 0x08, 0x08},      // enable WFI mode

    {0x112DC0, 0xFF, 0xB5},     //DSP clock = 364.5Mhz
    {0x112DC1, 0xFF, 0x00},     //DSP Time Constant = (364.5/2 - 1) = 181 = 0x00B5
...
const AUDIO_REG_TYPE2 ResetDMIOTbl[]=
{
    ...
    {0x112c58, 0x0006, 0x0006}, //SPDIF HDMI Decemational mode on

    //end DMIO a0xx reset
    {0xffffff, 0x0000, 0x0000}, // end of table
};

const AUDIO_REG_TYPE  AudioAUSDMTbl_On[] =
{
    {0x112CDC, 0xFF, 0x00},
    {0x112CDD, 0xFF, 0x00},
    {0x112CDE, 0xFF, 0x00},
...
const AUDIO_REG_TYPE  AudioAUSDMTbl_Off[] =
{
    {0x112CDC, 0xFF, 0x00},
    {0x112CDD, 0xFF, 0x3F},
    {0x112CDE, 0xFF, 0x00},
...
MS_BOOL HAL_AUDIO_ResetDefaultVars(AUDIO_SHARED_VARS2 * pAUDIOShared)
{
    ...
    pAUDIOShared->g_dolbyDrcMode = RF_MODE;
    ...
    pAUDIOShared->g_hbr_bypass_enable = TRUE;

    pAUDIOShared->g_R2_NonPcm_Sel = AU_DEC_ID1;
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_DAC_Clock_OnOff()
/// @brief \b Function \b Description:  This function is used to set DAC power on and off sequence.
/// @param <IN>        \b NONE    :
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
//Refine power on sequence for earphone & DAC pop noise issue
void HAL_AUDIO_DAC_Clock_OnOff(MS_BOOL b_enable)
{
    MS_U16 i = 0;

    if (b_enable)
    {
       while( !((AudioAUSDMTbl_On[i].u32Addr == 0xFFFFFF) && (AudioAUSDMTbl_On[i].u8Mask == 0)) )
       {
           if((AudioAUSDMTbl_On[i].u32Addr == 0xFFFFFF))
          {
            if(AudioAUSDMTbl_On[i].u8Mask == 1)
            {
                if(AudioAUSDMTbl_On[i].u8Value!=0)
                    MsOS_DelayTask((MS_U32)(AudioAUSDMTbl_On[i].u8Value));
            }
        }
        else
        {
            HAL_AUDIO_AbsWriteMaskByte(AudioAUSDMTbl_On[i].u32Addr, AudioAUSDMTbl_On[i].u8Mask, AudioAUSDMTbl_On[i].u8Value);
        }
    ...
    {
        while( !((AudioAUSDMTbl_Off[i].u32Addr == 0xFFFFFF) && (AudioAUSDMTbl_Off[i].u8Mask == 0)) )
        {
           if((AudioAUSDMTbl_Off[i].u32Addr == 0xFFFFFF))
          {
            if(AudioAUSDMTbl_Off[i].u8Mask == 1)
            {
                if(AudioAUSDMTbl_Off[i].u8Value!=0)
                    MsOS_DelayTask((MS_U32)(AudioAUSDMTbl_Off[i].u8Value));
            }
        }
        else
        {
            HAL_AUDIO_AbsWriteMaskByte(AudioAUSDMTbl_Off[i].u32Addr, AudioAUSDMTbl_Off[i].u8Mask, AudioAUSDMTbl_Off[i].u8Value);
        }
...
MS_BOOL HAL_AUDIO_GetPlayFileFlag(MS_BOOL bDspType)
{
        ...
        if(bDspType ==DSP_DEC)
            r2_dec_id = AU_DEC_ID1;
        else if(bDspType ==DSP_SE)
            r2_dec_id = AU_DEC_ID3;
        else
            r2_dec_id = AU_DEC_ID1; //default case
...
void HAL_AUDIO_SetInputPath(AUDIO_INPUT_TYPE input , AUDIO_PATH_TYPE u8Path)
{
    ...
    if( u8Path == AUDIO_PATH_MAIN )
    {
       g_AudioVars2->AudioSrcType = input1;
    }
    /* save sub channel audio input */
    else if( u8Path == AUDIO_PATH_6 )
    {
       g_AudioVars2->AudioSubSrcType = input1;
    }
...
void HAL_AUDIO_SPDIF_SetMode(MS_U8 u8Spdif_mode, MS_U8 u8Input_src)
{
    ...
    switch(eAudioSource)
    {
            ...
            switch(u8MainDspCodeType)
            {
                ...
                case AU_DVB_STANDARD_MS10_DDT:
                {
                    ...
                    else
                    {
                        ...
                        {
                            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_AAC_BYPASS, AU_DEC_ID1, 0, 0);
                        }
                        else if (u8SpdifMode == SPDIF_OUT_BYPASS) // AAC Bypass
                        {
                            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_AAC_BYPASS, AU_DEC_ID1, 1, 0);
                        }
                    }
                    break;
                }
                ...
                case AU_DVB_STANDARD_DolbyTrueHD_Bypass:
                    break;
        ...
        case E_AUDIO_INFO_HDMI_IN:
        {
            ...
            {
                u8MainDspCodeType = u8Dsp1CodeType;
                u8NonPcmPath = ADEC1;   //DEC1
            }

            if (u8Dsp1CodeType == AU_DVB_STANDARD_DTS)
            {
                u32bDTSCD = HAL_MAD_GetDTSInfo(Audio_DTS_infoType_CD_MODE);
            }
    ...
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_SPDIF_TYPE, AU_DEC_ID1, u8R2SpdifMode, 0);
    HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_SPDIF_TYPE, AU_DEC_ID3, u8R2SpdifMode, 0);
    ...
    switch (u8SpdifMode)
    {
        ...
        case SPDIF_OUT_TRANSCODE:
        {
            ...
            HAL_AUR2_WriteMaskByte(REG_R2_HDMI_NONPCM_PATH, 0x01, u8NonPcmPath);
            AUDIO_DELAY1MS(5);
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF_OUT_CFG, 0x07, 0x02); /* Grp C in */
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF_OUT_CFG+1, 0x01, 0x00);
            HAL_AUDIO_WriteMaskByte(REG_AUDIO_SPDIF2_OUT_CFG, 0x07, 0x02); /* Grp C in */
            ...
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN, AU_DEC_ID1, TRUE, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN, AU_DEC_ID3, TRUE, 0);
            }
            else
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN, AU_DEC_ID1, FALSE, 0);
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TTS_EN, AU_DEC_ID3, FALSE, 0);
            }
...
MS_BOOL HAL_AUDIO_SetSystem(En_DVB_decSystemType enDecSystem)
{
    ...
    switch (enDecSystem)
    {
            ...
            case MSAPI_AUD_ATVENC_FMTX:
                HAL_MAD2_ReLoadCode(AU_ATVENC_FMTx);
                break;

            case MSAPI_AUD_ATVENC_BTSC:
                HAL_MAD2_ReLoadCode(AU_ATVENC_BTSC);
                break;
...
MS_U16 HAL_AUDIO_GetUniDecodeDoneTag(void)
{
    ...
    for (i = 0; i < 100; i++)
    {
        temp1 = HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_UNI_DECODE_DONE_CNT, AU_DEC_ID1 );
        temp2 = HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_UNI_DECODE_DONE_CNT, AU_DEC_ID1 );
...
MS_U16 HAL_AUDIO_GetUniDecodeFrameCnt(AUDIO_DEC_ID eDSPId)
{
    ...
    for ( i = 0; i < 100; i++ )
    {
        temp1 = HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_UNI_DECODE_DONE_CNT, eDSPId);
        temp2 = HAL_DEC_R2_Get_SHM_INFO( R2_SHM_INFO_UNI_DECODE_DONE_CNT, eDSPId);
...
MS_BOOL HAL_AUDIO_SeSystemLoadCode(void)
{
    ...
    //Wait Dsp/R2 init finished Ack
        ...
        while(time_out++<100)
        {
            if((HAL_AUR2_ReadByte(REG_DECR2_ACK1) == 0xE3)&&(HAL_AUR2_ReadByte(REG_SNDR2_ACK1) == 0xE3))
                break;
            AUDIO_DELAY1MS(2);
        }
    #else
...
void HAL_AUDIO_HDMI_TX_SetMode(HDMI_TX_OUTPUT_TYPE outType, AUDIO_SOURCE_INFO_TYPE eSource)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: g_AudioVars2 should not be NULL !!\r\n", __FUNCTION__);
        return;
    }
    ...
    switch (eSource)
    {
        case E_AUDIO_INFO_DTV_IN:
        {
            ...
            if((u8MainDspCodeType == AU_DVB_STANDARD_AAC)||(u8MainDspCodeType == AU_DVB_STANDARD_MS10_DDT))
            {
                if((HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID1)&0xFFFF) < 32000)    // check if less than 32KHz
                    outType_tmp = HDMI_OUT_PCM;
            }
            else if ((u8MainDspCodeType != AU_DVB_STANDARD_AC3)  &&
                (u8MainDspCodeType != AU_DVB_STANDARD_AC3P) &&
                (u8MainDspCodeType != AU_DVB_STANDARD_DTS) &&
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            ...
            if((u8MainDspCodeType == AU_DVB_STANDARD_AAC)||(u8MainDspCodeType == AU_DVB_STANDARD_MS10_DDT))
            {
                ...
                {
                    if((HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID3)&0xFFFF) < 32000)    // check if less than 32KHz
                    {
                        outType_tmp = HDMI_OUT_PCM;
                        u8NonPcmPath = ADEC1; /* R2 */
                    }
            ...
            else
            {
                ...
                {
                    if ((u8MainDspCodeType != AU_DVB_STANDARD_AC3) &&
                    (u8MainDspCodeType != AU_DVB_STANDARD_AC3P) &&
                    (u8MainDspCodeType != AU_DVB_STANDARD_DTS) &&
                    (u8MainDspCodeType != AU_DVB_STANDARD_DTSLBR) &&
                    (u8MainDspCodeType != AU_DVB_STANDARD_DTSHD_ADO) &&
                    (u8MainDspCodeType != AU_DVB_STANDARD_DolbyTrueHD_Bypass)&&
                    (u8MainDspCodeType != AU_DEC_ES_BYPASS))
                    {
                        outType_tmp = HDMI_OUT_PCM;
                        u8NonPcmPath = ADEC1; /* R2 */
                    }
    ...
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: outType_tmp %d, u8NonPcmPath %d\r\n", __FUNCTION__, outType_tmp, u8NonPcmPath);
    ...
    switch (outType_tmp)
    {
        ...
        case HDMI_OUT_BYPASS:
        {
            ...
            HAL_AUR2_WriteMaskByte(REG_R2_HDMI_NONPCM_PATH, 0x01, u8NonPcmPath);
            AUDIO_DELAY1MS(5);
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, AU_DEC_ID1, 2, 0); /* Non-PCM */
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, AU_DEC_ID3, 2, 0); /* Non-PCM */
            ...
            HAL_AUDIO_WriteMaskByte(0x2C48, 0x80, 0x80); /* enable synthesizer */
            HAL_AUDIO_WriteMaskByte(0x2CB1, 0x04, 0x04); /* sel HDMI TX clock from synthesizer */
...
#else
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DOLBY_DDP_HDMI_BYPASS,  AU_DEC_ID1, TRUE, 0);
        ...
        {
            /* PCM */
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, AU_DEC_ID1, 1, 0); /* PCM Mode */
            HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_HDMI_TYPE, AU_DEC_ID3, 1, 0); /* PCM Mode */
            HAL_AUDIO_WriteMaskByte((REG_M2D_MAILBOX_SPDIF_CTRL+1), 0x20, 0x00); /* PCM Mode */
            HAL_AUDIO_WriteMaskByte(0x2CB1, 0x04, 0x00);
            HAL_AUDIO_WriteMaskByte(0x2C48, 0x80, 0x00);
    ...
    // For STB power saving
    HAL_AUDIO_DSPPowerSaving(outType_tmp);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_AUDIO_DSPPowerSaving()
/// @brief \b Function \b Description:  For DSP change clock from 284Mhz to 364.5Mhz
/// @param <IN>        \b HDMI_TX_OUTPUT_TYPE outType_tmp
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_DSPPowerSaving(HDMI_TX_OUTPUT_TYPE outType_tmp)
{
    MS_BOOL u8DspPowerSaveMode = FALSE;

    // for DSP Power saving
    if ((HAL_MAD_GetDecCmd(DVB_Audio_Decoder1) == AU_DVB_DECCMD_STOP) &&
        (HAL_MAD_GetDecCmd(DVB_Audio_Decoder2) == AU_DVB_DECCMD_STOP) )
    {// Decode stop
        u8DspPowerSaveMode = TRUE;
    }
    else
    {// Decode
        if(outType_tmp == HDMI_OUT_PCM)
            u8DspPowerSaveMode = TRUE;
        else
            u8DspPowerSaveMode = FALSE;
    }

    if(u8DspPowerSaveMode == TRUE)
    {//DSP = 283.5Mhz
        // Mute
        // HW Reg
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x1f, 0x14);
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x80, 0x00);
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x80, 0x80);
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x80, 0x00);
        // 283.5Mhz
        HAL_AUDIO_WriteMaskByte(0x2DC0, 0xFF, 0x8C);    //(283.5/2 - 1) = 140.75 = 0x8C
        HAL_AUDIO_WriteMaskByte(0x2DC1, 0xFF, 0x00);
        // Un-Mute
    }
    else
    {//DSP = 364.5Mhz
        // Mute
        // HW Reg
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x1f, 0x1a);
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x40, 0x40);
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x80, 0x00);
        HAL_AUDIO_AbsWriteMaskByte(REG_DSP_DFS_REG, 0x80, 0x80);
        // 364.5Mhz
        HAL_AUDIO_WriteMaskByte(0x2DC0, 0xFF, 0xB5);    //(364.5/2 - 1) = 181.25 = 0xB5
        HAL_AUDIO_WriteMaskByte(0x2DC1, 0xFF, 0x00);
        // Un-Mute
    }
}
...
MS_BOOL HAL_AUDIO_SetDataCaptureSource(AUDIO_DEVICE_TYPE eID, AUDIO_CAPTURE_SOURCE_TYPE eSource)
{
    ...
    switch(eSource)
    {
        ...
        case E_CAPTURE_ADC1_AUMIC_INPUT:
        {
            ...
            if((LONIBBLE(g_AudioVars2->AudioSrcType) == 2 || LONIBBLE(g_AudioVars2->AudioSubSrcType) == 2))
            {
        ...
        case E_CAPTURE_ADC2_AUMIC_INPUT:
        {
            ...
            if((LONIBBLE(g_AudioVars2->AudioSrcType) == 9 || LONIBBLE(g_AudioVars2->AudioSubSrcType) == 9))
            {
...
MS_BOOL HAL_AUDIO_SetDecodeSystem(AUDIO_DEC_ID DecId, AudioDecStatus_t *p_AudioDecStatus)
{
    ...
    switch(p_AudioDecStatus->eStcSource)
    {
        case E_TSP_0:
            if (DecId == AU_DEC_ID1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECTION, AU_DEC_ID1, 0, 0);
            }
            else if(DecId == AU_DEC_ID3)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECTION, AU_DEC_ID3, 0, 0);
            }
            break;
        ...
        case E_TSP_1:
            if (DecId == AU_DEC_ID1)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECTION, AU_DEC_ID1, 1, 0);
            }
            else if(DecId == AU_DEC_ID3)
            {
                HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_STC_SELECTION, AU_DEC_ID3, 1, 0);
            }
            break;
        case E_TSP_2:
        case E_TSP_3:
    ...
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "\033[1;31m [%s: %d][DecId = %d, eAudFormat = %d, g_AudioVars2->AudioDecStatus[DecId].eAudFormat = %d] \033[0m \n", __FILE__, __LINE__,
    //    DecId,
    ...
    if ((DecId == AU_DEC_ID3)
        && (g_AudioVars2->AudioDecStatus[DecId].eSourceType == E_AUDIO_INFO_DTV_IN)
        && (p_AudioDecStatus->eGroup == E_CONNECT_SUB))
    {
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_ES2_IsUsedBy_Dual, AU_DEC_ID3, 1, 0);
    }
...
MS_BOOL HAL_AUDIO_SetSystem2(AUDIO_DEC_ID DecId, En_DVB_decSystemType Param)
{
    ...
    switch ( Param )
    {
        ...
        case MSAPI_AUD_DVB_DolbyTrueHDBypass:
            HAL_AUR2_WriteByte(regDecoderType, adec_type_dolby_truhd_bypass);  /*  select r2 decoder to dolby TrueHD bypss */
            if(DecId == AU_DEC_ID3)
            {
            ...
            {
                HAL_AUDIO_SetDspCodeType(AU_DVB_STANDARD_DolbyTrueHD_Bypass, TRUE);
            }
            break;

        case MSAPI_AUD_DVB_ES_Bypass:
            HAL_AUR2_WriteByte(regDecoderType, adec_type_dummy);  //  select r2 decoder to HDMI bypss
            if(DecId == AU_DEC_ID3)
            {
                HAL_AUDIO_SetDsp2CodeType(AU_DEC_ES_BYPASS, TRUE); //set decoder type for HDMI bypass
            }
            else
            {
...
void HAL_AUDIO_DumpDspInfo(void)
{
    ...
    ES_Lvl = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_LEVEL, AU_DEC_ID1);
    PCM_Lvl = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCM_LEVEL, AU_DEC_ID1)/5;
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_Avsync) != 0)
    {
        ...
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ACCUM_ES_BYTECNT, AU_DEC_ID1)&0xFFFF;
        printf("ES=%03X(%04X),", ES_Lvl, tmp_L);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCMISR_EMPTY_CNT, AU_DEC_ID1)&0xFFFF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ELAPSED_PCM_CNT, AU_DEC_ID1)&0xFFFF;
        printf("PCM=%04X(%04X,%04X)|", PCM_Lvl, tmp_H, tmp_L);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PLAYSMPFLAG, AU_DEC_ID1)&0x1;
        printf("play=%X",   tmp_L);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SYNTH_RATE, AU_DEC_ID1)&0xFFFF;
        printf("<%04X>,", tmp_L);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_STATE, AU_DEC_ID1)&0x0F;
        tmp_M = HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_MM_TS_SYNC_STC, 0, 0)&0x0F;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_FREERUN_TYPE, AU_DEC_ID1)&0x0F;
        printf("State=%X,%X(%X)|", tmp_L,tmp_M, tmp_H);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OK_FRMCNT, AU_DEC_ID1)&0x0000FFFF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ERR_FRMCNT, AU_DEC_ID1)&0x0000FFFF;
        printf("frmCnt:%04X,",     tmp_L);
        printf("%04X,",    tmp_H);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SKIP_FRMCNT, AU_DEC_ID1)&0x0000FF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_REPT_FRMCNT, AU_DEC_ID1)&0x0000FF;
        printf("%02X,",    tmp_L);
        printf("%02X,",    tmp_H);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_WAIT_1STPTS_CNT, AU_DEC_ID1)&0x00FF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_WAIT_STC_CNT, AU_DEC_ID1)&0x00FF;
        tmp_M = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_FLUSH_CNT, AU_DEC_ID1)&0x00FF;
        printf("%02X,%02X,%02X|",  tmp_L, tmp_H, tmp_M);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PTS_TAG_WR_IDX, AU_DEC_ID1)&0xF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PTS_TAG_RD_IDX, AU_DEC_ID1)&0xF;
        printf("tag:%01X,%01X|",  tmp_L, tmp_H);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_AVSYNC_OFFSET, AU_DEC_ID1)&0x00FFFFFF;
        printf("AvDly=%05X,", tmp_L);

        result = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_STC, AU_DEC_ID1);
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_Dec1) != 0)
    {
        ...
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCMISR_EMPTY_CNT, AU_DEC_ID1)&0xFF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ELAPSED_PCM_CNT, AU_DEC_ID1)&0xFFFF;
        ...
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_TYPE, AU_DEC_ID1)&0x00FF;
        printf("Type=%02X,", tmp_L);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PLAY_STATE, AU_DEC_ID1)&0x00FFFFFF;
        printf("pState=%06X,", tmp_L);

        tmp_L = (MS_U32)HAL_AUR2_ReadByte(REG_R2_DECODE1_CMD);
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_RECEIVE_STOP_CNT, AU_DEC_ID1)&0x00FF;
        ...
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CALLING_CNT, AU_DEC_ID1)&0x0000FFFF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SYNC_MISS_CNT, AU_DEC_ID1)&0x0000FFFF;
        printf("Call:%04X,",    tmp_L);
        printf("sMiss:%04X|",  tmp_H);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_OUT_L, AU_DEC_ID1)&0x00FFFFFF;
        printf("pcm=%06X,",  tmp_L);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_OUT_R, AU_DEC_ID1)&0x00FFFFFF;
        printf("%06X|",  tmp_L);
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_IoInfo) != 0)
    {
        ...
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_OUT_L, AU_DEC_ID3)&0x00FFFFFF;
    ...
    {
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OK_FRMCNT, AU_DEC_ID1)&0x00FFFFFF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ERR_FRMCNT, AU_DEC_ID1)&0x00FFFFFF;
        printf("[AutoTest][AUDIO][FrameCount][%d]\n",     tmp_L);
        printf("[AutoTest][AUDIO][ErrorCount][%d]\n",     tmp_H);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SMP_RATE, AU_DEC_ID1);
        printf("[AutoTest][AUDIO][SampleRate][%d]\n",     tmp_L);
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CHANNEL_MODE, AU_DEC_ID1);
    ...
    if ((g_AudioDumpInfoCtrl & Audio_DumpInfo_mmFileAVsync) != 0)
    {
        ...
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_MM_FILE_REQ_SIZE, AU_DEC_ID1)&0xFFFF;
        printf("MM:%04X|", tmp_H);
        ...
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PCMISR_EMPTY_CNT, AU_DEC_ID1)&0xFF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ELAPSED_PCM_CNT, AU_DEC_ID1)&0xFFFFFF;
        ...
        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_PLAY_STATE, AU_DEC_ID1)&0x00FFFF;
        printf("pState=%04X,", tmp_L);

        tmp_L = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_CALLING_CNT, AU_DEC_ID1)&0x0000FF;
        tmp_H = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SYNC_MISS_CNT, AU_DEC_ID1)&0x00000FFF;
...
void  HAL_AUDIO_Set_DecimationMode(AUDIO_OUTPUT_TYPE Output_Type, Audio_id ID)
{
    ...
    switch(Output_Type)
    {
        case AUDIO_HDMI_OUTPUT:  //high rate nonPCM application need decimation
            if( (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMITx_SMP_RATE, DecID) > 48000) &&
        ...
        case AUDIO_SPDIF_OUTPUT:
            if( (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_SPDIFTx_SMP_RATE, DecID) > 48000) &&
                (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL)&0x02))
            {
