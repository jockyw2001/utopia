...
void HAL_DEC_R2_init_SHM_param( void )
{
    ...
    memset(p_shm , 0, sizeof(DEC_R2_SHARE_MEM));
    ...
    for (dec_id = 0; dec_id < MAX_ADEC; dec_id++ )
    {
        ...
        if (dec_id == 0)
            p_shm_esparam = &p_shm->adec_esParam_shm[0];
        else
            p_shm_esparam = &p_shm->adec_esParam_shm[2];
        ...
        //    (p_shm_param->bootMagicID != SHM_PARAM_BOOT_MAGIC_ID))
        {
            ...
            /* low latency limiter */
            p_shm_param->Low_Latency_Limiter_ES_Threshold = 0x100;
            p_shm_param->Low_Latency_Limiter_PCM_Threshold = 0x100;
            p_shm_param->Low_Latency_Limiter_PCM_Skip_Target = 0x32;

            /* ES Repeat mode */
            p_shm_param->es_repeatMode = 0;
...
MS_U32 HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TYPE decInfoType, MS_U8 dec_id )
{
    ...
    volatile MS_U32 *ptr;
    DEC_R2_SHARE_MEM  *p_shm = (DEC_R2_SHARE_MEM *)(MS_PA2KSEG1(R2_1_DDR_BASE+DEC_R2_SHM_DDR_OFFSET));
    AUR2_ADEC_INFO_SHM      *p_shm_info;
    AUR2_ADEC_ES_INFO_SHM   *p_shm_esinfo = &p_shm->adec_esInfo_shm[dec_id];

    if(dec_id <= AU_DEC_INVALID || dec_id >= AU_DEC_MAX)
    {
        AUR2_ERRMSG(printf("[%s] Error! Dec ID%d is invalid !! \n", __FUNCTION__, dec_id));
        return ret_value;
    }

    if (dec_id == AU_DEC_ID1 ||dec_id == AU_DEC_ID2)
        p_shm_info = &p_shm->adec_info_shm[0];        //dec id 1(1st Main) & dec id 2(1st AD) use dec0
    else
        p_shm_info = &p_shm->adec_info_shm[1];        //dec id 3(2nd Main) use dec1
...
MS_BOOL HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TYPE decParamType, MS_U8 dec_id, MS_U32 param, MS_U32 param2 )
{
    ...
    {
        p_shm_param = &p_shm->adec_param_shm[0];        //dec id1 use dec0 (1st Main)
    }
    else if (dec_id == AU_DEC_ID2)
    {
        p_shm_param = NULL;                                              //dec id 2 (1st AD) is unable to set dec param
    }
    else if (dec_id == AU_DEC_ID3)
    {
        p_shm_param = &p_shm->adec_param_shm[1];        //dec id 3 use dec1 (2nd Main)
    }
    else
    {
        p_shm_param = NULL;
    ...
    switch( decParamType )
    {
        case R2_SHM_PARAM_ES_WR_PTR:
            ...
            *ptr = param;               //(param + offset_R2_to_DSP + OFFSET_ES1_DRAM_ADDR;
            ...
            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_updateEsWritePtr, ADEC_id, 0, 0);
            ret_status = TRUE;
            break;
        ...
        case R2_SHM_PARAM_EXT_SMPRATE_CTRL:
            if( p_shm_param != NULL)
            {
                ...
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ptr = (volatile MS_U32 *) &p_shm_param->extSynthsizer_value;
                *ptr = param2;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_MPG_AVOFFSET:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dtv_mpg_avDelay;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_AC3_AVOFFSET:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dtv_ac3_avDelay;
                *ptr = param;   MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_AAC_AVOFFSET:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dtv_aac_avDelay;
                *ptr = param;   MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_MM_FILE_REQ_SIZE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->mmFileReqSize;
                *ptr = param;   MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_MM_INPUT_PTS:
            ...
            *ptr = param;
            MsOS_Dcache_Flush((MS_VIRT)ptr,4);
            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_PTS_input, ADEC_id, param>>16, param&0xFFFF);
            ret_status = TRUE;
            break;

        case R2_SHM_PARAM_UNI_SETUP_DECODE_FRMCNT:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->mmUni_needDecodeFrmCnt;
                *ptr = param; MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_UNI_PCM_SET_OUTPUT_CNT:
            HAL_DEC_R2_SetCommInfo(AUR2_CommParam_ADEC_UNI_setOutput, ADEC_id, param>>16, param&0xFFFF);
            ret_status = TRUE;
            break;
        ...
        case R2_SHM_PARAM_DOLBY_DMX_MODE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.dmxMode;
                *ptr = param;   MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_DOLBY_HIGH_CUT:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.high_cut;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_DOLBY_LOW_BOOST:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.low_boost;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_DOLBY_GAIN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.gain;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_DOLBY_MUL_FRAME_HEADER:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.mul_frame_header;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_DOLBY_DDP_HDMI_BYPASS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.DDP_HDMI_bypass;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_DOLBY_AAC_BYPASS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.AAC_HDMI_bypass;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_DOLBY_TB11_ENABLE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.TB11_enable;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_DOLBY_DDP_DISABLE_ATMOS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.ddp_disable_atmos;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_STC_SELECTION:          //[10]
            if( p_shm_param != NULL)
            {
        ...
        case R2_SHM_PARAM_MPEG_SOUNDMODE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->mpeg_param.MPEG_soundMode;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_MPEG_GAIN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->mpeg_param.gain;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_OMX_SPDIF_CTRL:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->OMX_SPDIF_Ctrl;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_OMX_SPDIF_PCM_LEVEL:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->OMX_SPDIF_PCM_Level;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_MAIN_VOLUME:
            if( p_shm_param != NULL)
            {
        ...
        case R2_SHM_PARAM_COOK_NUMCODECS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mNumCodecs;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_COOK_SAMPLES:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mSamples;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_COOK_SAMPLERATES:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mSampleRate;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_COOK_CHANNELS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.Channels[param2];
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_COOK_REGIONS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.Regions[param2];
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_COOK_CPLSTART:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.cplStart[param2];
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_COOK_CPLQBITS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.cplQbits[param2];
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_COOK_FRAMESIZE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->cook_param.FrameSize[param2];
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_DTS_DMX_LFE_ENABLE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dts_param.dmxLfeEnable;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_DTS_DMX_MODE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dts_param.dmxMode;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_DTS_HD_HDMI_BYPASS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->dts_param.DTSHD_HDMI_bypass;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_VERSION:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.asf_version;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_CHANNELS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.nChannels;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_SAMPLERATE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.smpRate;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_BYTERATE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.byteRate;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_BLOCKALIGN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.blockAlign;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_ENCOPT:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.encOpt;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_PARSINGBYAPP:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.parsingByApp;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_BITS_PER_SAMPLE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.bitsPerSmp;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_CHANNELMASK:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.channelMask;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_DRC_PARAM_EXIST:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcParamExist;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_DRC_RMS_AMP_REF:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcRmsAmpRef;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_DRC_RMS_AMP_TARGET:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcRmsAmpTarget;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_DRC_PEAK_AMP_REF:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcPeakAmpRef;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_DRC_PEAK_AMP_TARGET:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcPeakAmpTarget;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ASF_MAX_PACKET_SIZE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->asf_param.maxPacketSize;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_XPCM_TYPE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.xPcmType;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_XPCM_CHANNELS:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.nChannels;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_XPCM_SAMPLERATES:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.smpRate;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_XPCM_BITS_PER_SAMPLE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.bitsPerSmp;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_XPCM_BLOCK_SIZE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.blockSize;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_XPCM_SAMPLE_PER_BLOCK:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.samplePerBlock;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_SPDIF_TYPE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->spdif_type;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_RFSIGNAL_TYPE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm->RfSignalType;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_SPEAKER_EN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->speaker_enable;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_HDMI_TYPE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->hdmi_type;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_VORBIS_HEADER_SIZE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->vorbis_header_size;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ES_Limiter_EN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->ES_Limiter_EN;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_ES_Limiter_Threshold:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->ES_Limiter_Threshold;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_PCM_Limiter_EN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->PCM_Limiter_EN;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_PCM_Limiter_Threshold:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->PCM_Limiter_Threshold;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_TVOS2_PCM_RDPTR:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->TVOS2_PCM_rdPtr;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_TVOS2_DTV_SYNCTYPE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->DTV_SyncType;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_MULTI_CHANNEL:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->force_mmuni_mch_pcm;
                *ptr = param;    MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_OMX_SPDIF_PAUSE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->OMX_SPDIF_PAUSE;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_DDP_ENCODE_EN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->DDP_encode_enable;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;
        ...
        case R2_SHM_PARAM_TTS_EN:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->TTS_On_flag;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_ES_PassThrough_DelaySmp:
            ptr = (volatile MS_U32 *) &p_shm->esPassThrough_delaySmp;
            *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
            ret_status = TRUE;
            break;

        case R2_SHM_PARAM_ES_PassThrough_PcmSamRate:
            ptr = (volatile MS_U32 *) &p_shm->esPassThrough_pcmSmpRate;
            *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
            ret_status = TRUE;
            break;

        case R2_SHM_PARAM_ES_PassThrough_HdmiSmpRate:
            ptr = (volatile MS_U32 *) &p_shm->esPassThrough_hdmiSmpRate;
            *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
            ret_status = TRUE;
            break;

        case R2_SHM_PARAM_ES2_IsUsedBy_Dual:
            ptr = (volatile MS_U32 *) &p_shm->es2_isUsed_by_Dual;
            *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
            ret_status = TRUE;
            break;

        case R2_SHM_PARAM_LOW_LATENCY_LIMITER_ES_THRESHOLD:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->Low_Latency_Limiter_ES_Threshold;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_LOW_LATENCY_LIMITER_PCM_THRESHOLD:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->Low_Latency_Limiter_PCM_Threshold;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_LOW_LATENCY_LIMITER_PCM_SKIP_TARGET:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->Low_Latency_Limiter_PCM_Skip_Target;
                *ptr = param;  MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_ES_REPEAT_MODE:
            if( p_shm_param != NULL)
            {
                ptr = (volatile MS_U32 *) &p_shm_param->es_repeatMode;
                *ptr = param;   MsOS_Dcache_Flush((MS_VIRT)ptr,4);
                ret_status = TRUE;
            }
            break;

        case R2_SHM_PARAM_AUR2_HDMITx_TypeChanged:
            HAL_DEC_R2_SetCommInfo(AUR2_HDMITx_TypeChanged, ADEC_id, param, param2);
            ret_status = TRUE;
            break;
    ...
    if( p_shm_param == NULL && ret_status == FALSE)
    {
        AUR2_ERRMSG(printf("[%s] Dec ID%d is unable to set dec parameter!! \n", __FUNCTION__, dec_id));
    }
...
MS_U32 HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_TYPE decParamType, MS_U8 dec_id, MS_U32 param )
{
    ...
    if (dec_id == AU_DEC_ID1 ||dec_id == AU_DEC_ID2)
        p_shm_param = &p_shm->adec_param_shm[0];        //dec id 1(1st Main) & dec id 2(1st AD) use dec0
    else
        p_shm_param = &p_shm->adec_param_shm[1];        //dec id 3(2nd Main) use dec1

    switch( decParamType )
    {
        case R2_SHM_PARAM_ES_WR_PTR:        ptr = (volatile MS_U32 *) &p_shm_esparam->ES_wrPtr;          MsOS_Dcache_Flush((MS_VIRT)ptr, 4);  ret_value = *ptr;       break;
