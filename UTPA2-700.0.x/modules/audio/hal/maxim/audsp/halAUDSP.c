   ...
   HALAUDSP_CHECK_SHM_INIT;
             ...
             (IS_AUDIO_DSP_CODE_TYPE_SE_ENC(dspCodeType)))
    {
        ...
        if (ii < (sizeof(dsp_info)/sizeof(AUDIO_ALG_INFO)))
            ...
            {
            DBG_AUDSP_LOAD("    [dsp_info]:decoder type not found.\r\n");
                return FALSE;
            }
    }
    else
    {
        DBG_AUDSP_LOAD("    [HAL_AUDSP_DspLoadCode]:======  Loading the wrong DSP code type!======\r\n");
            return FALSE;
    }
    ...
    DBG_AUDSP_LOAD("    PM addr: 0x%x\r\n", pau_info->pm_addr);
#if defined (__aarch64__)
    DBG_AUDSP_LOAD("    PM buf addr: 0x%lx\r\n", (MS_VIRT)pau_info->pm_buf);
#else
    DBG_AUDSP_LOAD("    PM buf addr: 0x%x\r\n", (MS_VIRT)pau_info->pm_buf);
#endif
    DBG_AUDSP_LOAD("    PM size: 0x%x\r\n",pau_info->pm_len);
    ...
    DBG_AUDSP_LOAD("    CM addr: 0x%x\r\n", pau_info->cm_addr);
#if defined (__aarch64__)
    DBG_AUDSP_LOAD("    CM buf addr: 0x%lx\r\n", (MS_VIRT)pau_info->cm_buf);
#else
    DBG_AUDSP_LOAD("    CM buf addr: 0x%x\r\n", (MS_VIRT)pau_info->cm_buf);
#endif
    DBG_AUDSP_LOAD("    CM size: 0x%x\r\n", pau_info->cm_len);
    ...
    {
        DBG_AUDSP_LOAD("    Cache PM addr: 0x%x\r\n", pau_info->cache_addr);
#if defined (__aarch64__)
        DBG_AUDSP_LOAD("    Cache PM buf addr: 0x%lx\r\n", (MS_VIRT)pau_info->cache_buf);
#else
        DBG_AUDSP_LOAD("    Cache PM buf addr: 0x%x\r\n", (MS_VIRT)pau_info->cache_buf);
#endif
        DBG_AUDSP_LOAD("    Cache PM size: 0x%x\r\n", pau_info->cache_len);

        MIU_addr = pau_info->cache_addr * 3 + HAL_AUDIO_GetDspMadBaseAddr(DSP_select);
        DBG_AUDSP_LOAD("    MIU of Cache: 0x%08X\r\n", MIU_addr);
...
//    HALAUDSP_ERROR ("@@@@@@@@@@    ====== %s: %s start ======\r\n", __FUNCTION__, pau_info->AlgName);
    ...
    if ( pau_info->cache_len != 0 )
    {
        ...
        DBG_AUDSP_LOAD("    MIU of Cache: 0x%X\r\n", MIU_addr);
