...
void HAL_SND_R2_EnableR2( MS_BOOL bEnable)
{
    if (bEnable)
    {
        ...
        HAL_AUR2_WriteMaskByte((REG_R2_0_CTRL_BASE + 0x40), 0xFF, 0x10);
...
void HAL_DEC_R2_EnableR2(MS_BOOL bEnable )
{
    if (bEnable)
    {
        ...
        HAL_AUR2_WriteMaskByte((REG_R2_1_CTRL_BASE + 0x40), 0xFF, 0x10);
...
void HAL_SND_R2_init_SHM_param(void)
{
    ...
    AUR2_AENC_PARAM *p_shm_encparam = NULL;
    AUR2_PCMR_PARAM *p_shm_pcmr_param = NULL;
    ...
    p_shm_encparam = &p_shm->commEncParam;
    p_shm_pcmr_param = &p_shm->ms12_pcmr_param_shm;

#if (defined(MSOS_TYPE_LINUX_KERNEL) && defined(CONFIG_ARM64))
    memset_io(p_shm_encparam, 0, sizeof(AUR2_AENC_PARAM));
    memset_io(p_shm_pcmr_param, 0, sizeof(AUR2_PCMR_PARAM));
#else
    memset(p_shm_encparam, 0, sizeof(AUR2_AENC_PARAM));
    memset(p_shm_pcmr_param, 0, sizeof(AUR2_PCMR_PARAM));
#endif

    MsOS_Dcache_Flush((MS_VIRT)p_shm, sizeof(SND_R2_SHARE_MEM));
    printf("[======= initialize SND-R2 SHM =======]\n");
...
MS_BOOL HAL_SND_R2_Set_SHM_PARAM(R2_SHM_PARAM_TYPE decParamType, MS_U8 dec_id, MS_U32 param, MS_U32 param2)
{
    ...
    p_shm = (SND_R2_SHARE_MEM *)(MS_PA2KSEG1(R2_0_DDR_BASE+SND_R2_SHM_DDR_OFFSET));
    if (p_shm == NULL)
    {
    ...
    switch (decParamType)
    {
        ...
        case R2_SHM_PARAM_PCMR_SOUND_MODE:
        {
            ptr = (volatile MS_U32 *) &p_shm_pcmr_param->sound_mode;
            *ptr &= 0xFFFFFFFC;
            *ptr |= (param&0x03);
            MsOS_Dcache_Flush((MS_VIRT)ptr,4);
            break;
