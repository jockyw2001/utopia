...
MS_BOOL HAL_MAD_MM2_inputAesFinished( AUDIO_DEC_ID dec_id, MS_U32 es_size, MS_BOOL ptsExist, MS_U64 pts )
{
    ...
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        {
            printf("ID1:==> Error, input AES size(0x%08u) is larger than current Free space(0x%08u)\r\n", (unsigned int)es_size, (unsigned int)audio_aes_info[0].aes_freeSpace );
            return FALSE;
        }
    ...
    else if (dec_id == AU_DEC_ID3)
    {
        ...
        {
            printf("ID3:==> Error, input AES size(0x%08u) is larger than current Free space(0x%08u)\r\n", (unsigned int)es_size, (unsigned int)audio_aes_info[1].aes_freeSpace );
            return FALSE;
        }
...
void HAL_MAD_Monitor_DDPlus_SPDIF_Rate(void)
{
    ...
    AUDIO_DSP_CODE_TYPE u8MainDspCodeType = MSAPI_AUD_DVB_INVALID;
    AUDIO_DSP_CODE_TYPE u8Dsp1CodeType = MSAPI_AUD_DVB_INVALID;
    AUDIO_DSP_CODE_TYPE u8Dsp2CodeType = MSAPI_AUD_DVB_INVALID;
        ...
        case Audio_Comm_infoType_SoundMode:
            {
                ...
                HAL_SND_R2_Set_SHM_PARAM(R2_SHM_PARAM_PCMR_SOUND_MODE, ADEC1, value, 0);
...
MS_BOOL HAL_MAD_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    switch ( paramType )
    {
        ...
        case Audio_ParamType_acMode:
        {
            ...
            HAL_SND_R2_Set_SHM_PARAM(R2_SHM_PARAM_PCMR_SOUND_MODE, ADEC1, Param, 0);
            break;
        }
        ...
        case Audio_ParamType_SetEDIDOutputInterface:
        {
            ...
            bRet = HAL_AUDIO_SetEDIDOutputInterface((DIGITAL_OUTPUT_TYPE)Param);
...
MS_BOOL HAL_MAD_GetAudioInfo2( AUDIO_DEC_ID DecId, Audio_InfoType infoType, void * pInfo )
{
    ...
    switch ( infoType )
    {
        ...
        case Audio_infoType_acmod:
        {
            ...
            if (pInfo == NULL)
            {
                HALMAD_ERROR ("%s: Audio_infoType_acmod pInfo is NULL\n", __FUNCTION__);
                return FALSE;
            }

            if (adec_id >= MAX_ADEC) // SHM only has 2 mem
            {
                HALMAD_ERROR ("%s: Audio_infoType_acmod more than MAX_ADEC ID %d\n", __FUNCTION__, adec_id);
                return FALSE;
            }
            ...
            {
                HALMAD_ERROR ("%s: Audio_infoType_acmod DEC_ID %d not Supported\n", __FUNCTION__, adec_id);
                return FALSE;
            }
