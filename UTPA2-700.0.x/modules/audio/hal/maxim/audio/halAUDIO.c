...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock_0[] =
{
	...
	{0x163c61, 0x10, 0x10}, //[12]    CLK DSP DFS SEL[2] = 1
	...
	{0x160346, 0xff, 0x23}, //[7:0]   35x24=840MHz, step:24MHz, range 300MHz ~ 600MHz

	//----------------AUR2_1PLL------------------------
	{0x160362, 0xff, 0x01}, //
	{0x160363, 0xff, 0x00}, //[8]     Set AUR2_1PLL power down = 0
	{0x160366, 0xff, 0x21}, //[7:0]   33x24=792MHz, step:24MHz, range 300MHz ~ 600MHz
	...
	{0x163c63, 0x08, 0x08}, //[11]    REG_ENABLE_CLK_R2_WB MPLL 432MHz
	{0x100200, 0x01, 0x01}, //[0]     reg_enable_clk_r2     R2_0
	{0x100202, 0x01, 0x01}, //[0]     reg_enable_clk_r2_dfs R2_0
	{0x100202, 0x1c, 0x00}, //[4:2]   reg_clk_r2_dfs_sel    R2_0 select R2_0PLL
	{0x100300, 0x01, 0x01}, //[0]     reg_enable_clk_r2     R2_1
	{0x100302, 0x01, 0x01}, //[0]     reg_enable_clk_r2_dfs R2_1
	{0x100302, 0x1c, 0x1c}, //[4:2]   reg_clk_r2_dfs_sel    R2_1 select R2_1PLL
...
const AUDIO_REG_TYPE  AudioPreInitTbl_Clock_1[] =
{
	...
	{0x112cee, 0xff, 0x00},
	{0x112cef, 0xff, 0x00},
	{0x112cec, 0xff, 0x00}, //[7]PD_LDO_ADC,[5]QC_LDO_ADC,[4]PD_LDO_DAC,[3]QC_LDO_DAC
	{0x112ced, 0xff, 0x00},
	{0x112cde, 0xff, 0x00},
	{0x112cdf, 0xff, 0x78},
	{0x112cea, 0xff, 0x08}, //[2]EN_VMID2GND_EAR
	{0x112ceb, 0xff, 0x00},
	{0x112cee, 0xff, 0x81}, //[7]EN_MUTE_EAR,[1:0]SEL_DEPOP_EAR
	{0x112cef, 0xff, 0x10},
	{0x112cec, 0xff, 0x28}, //[7]PD_LDO_ADC,[5]QC_LDO_ADC,[4]PD_LDO_DAC,[3]QC_LDO_DAC
	{0x112ced, 0xff, 0x00},
	{0xffffff, 0x01, 0x01}, // delay 1m sec
	{0x112cdc, 0xff, 0x00},
	{0x112cdd, 0xff, 0x00},
	...
	{0x112cea, 0xff, 0x00},
	{0x112ceb, 0xff, 0x00},
        {0xffffff, 0x01, 0xFF}, // delay 255m sec
        {0xffffff, 0x01, 0xF5}, // delay 245m sec
...
const AUDIO_REG_TYPE  AudioInitTbl_0[]=
{
	...
	{0x112CEF, 0xFF, 0x10},
	{0xffffff, 0x01, 0x01}, // delay 5m sec
	{0x112cee, 0xff, 0x01},
	{0x112cef, 0xff, 0x13}, //[9]EN_LTX_EAR,[8]EN_RTX_EAR
	{0xffffff, 0x01, 0x32}, // delay 300ms -> 50ms
	{0x112cee, 0xff, 0x03},
	{0x112cef, 0xff, 0x13},
	...
	//-----------------Output: DAC--------------------------//Item 12
	{0xFFFFFF, 0x01, 0x64},  // delay 100ms
	{0x112CEE, 0xFF, 0x03},
	{0x112CEF, 0xFF, 0x13}, //[12]EN_MSP
	...
	{0x112aa2, 0x40, 0x40}, //[6:5]=2'b10 IRQ2n select SPDIF_TX
	{0x112aa2, 0x10, 0x00}, //[4]=0       IRQ1L select SPDIF_TX2
	{0x112ad8, 0x20, 0x20}, //[5]=1       IRQ1L select SPDIF_TX2
	{0x112d8c, 0x01, 0x00}, //[0]=0       SPDIF+ARC+HDMITx(PCM)
				//[0]=1       SPDIF+HDMITx
	{0x112c58, 0x04, 0x04}, //[2]=1       HDMI TX  Full Decimation mode.
	{0x112c58, 0x02, 0x02}, //[1]=1       HDMI ARC Full Decimation mode.
	...
	{0x112f61, 0xff, 0x80},
	{0x112f88, 0xff, 0xfc}, //        cache arb, enable DSP i-cache high priority
	{0x112f89, 0xff, 0xff},
        ...
        if((ResetDMIOTbl[i].u32Addr == 0xFFFFFF))
        {
                if(ResetDMIOTbl[i].u16Mask == 1)
                {
                    if(ResetDMIOTbl[i].u16Value!=0)
                        AUDIO_DELAY1MS((MS_U32)(ResetDMIOTbl[i].u16Value));
                }
        }
        else
        {
...
void HAL_AUDIO_SwResetMAD(void)
{
    ...
    // For SW Reset patch reset irq table.
    //SPDIF + HDMI ARC + HDMI TX(PCM)
    HAL_AUDIO_WriteMaskByte(REG_SE_MAD_OFFSET_BASE_L,      0x80, 0x00); //{0x112aa2, 0x80, 0x00}, //[7]=0       IRQ1E select HDMI TX
    HAL_AUDIO_WriteMaskByte(REG_ES_CTRL_CFG,               0x10, 0x00); //{0x112ad8, 0x10, 0x20}, //[4]=0       IRQ1E select HDMI Tx
    HAL_AUDIO_WriteMaskByte(REG_SE_MAD_OFFSET_BASE_L,      0x40, 0x40); //{0x112aa2, 0x40, 0x40}, //[6:5]=2'b10 IRQ2n select SPDIF_TX
    HAL_AUDIO_WriteMaskByte(REG_SE_MAD_OFFSET_BASE_L,      0x10, 0x00); //{0x112aa2, 0x10, 0x00}, //[4]=0       IRQ1L select SPDIF_TX2
    HAL_AUDIO_WriteMaskByte(REG_ES_CTRL_CFG,               0x20, 0x20); //{0x112ad8, 0x20, 0x20}, //[5]=1       IRQ1L select SPDIF_TX2
    HAL_AUDIO_WriteMaskByte(0x2c58, 0x04, 0x04); //[2]=1        HDMI TX  Full Decimation mode.
    HAL_AUDIO_WriteMaskByte(0x2c58, 0x02, 0x02); //[1]=1        HDMI ARC Full Decimation mode.
}
...
{
    AUDIO_DSP_CODE_TYPE u8SourceDspCodeType = AU_DVB_STANDARD_INVALID;
    AUDIO_DSP_CODE_TYPE u8Dsp1CodeType = AU_DVB_STANDARD_INVALID;
    AUDIO_DSP_CODE_TYPE u8Dsp2CodeType = AU_DVB_STANDARD_INVALID;
...
AUDIO_DEC_ID _HAL_AUDIO_Get_SourceDecoderIndex(AUDIO_SOURCE_INFO_TYPE eAudioSource)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() u32CurrDecIndex %u - Invalid u32DecIndex\n",__FUNCTION__, u32CurrDecIndex);
        return AU_DEC_INVALID;
    }

    if (g_AudioVars2->AudioDecStatus[u32CurrDecIndex].bConnect == FALSE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s() Decoder%u Not Connect\n",__FUNCTION__, u32CurrDecIndex);
        return AU_DEC_INVALID;
    }
...
AUDIO_DSP_CODE_TYPE HAL_AUDIO_GetDspCodeType(void)
{
    ...
    return (AUDIO_DSP_CODE_TYPE)g_AudioVars2->g_DspCodeType;
}
...
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_AUDIO_SetDsp2CodeType(AUDIO_DSP_CODE_TYPE u8Type, MS_BOOL bFlag)
{
    HALAUDIO_CHECK_SHM_INIT;
    ...
    g_AudioVars2->g_Dsp2CodeType = u8Type;
}
...
AUDIO_DSP_CODE_TYPE HAL_AUDIO_GetDsp2CodeType(void)
{
    ...
    return (AUDIO_DSP_CODE_TYPE)g_AudioVars2->g_Dsp2CodeType;
}
...
MS_BOOL HAL_AUDIO_Alg2ReloadCode(AUDIO_DSP_CODE_TYPE dspCodeType)
{
    ...
    {
        //No Decoder in DSP
        DBG_AUDIO_ERROR("Warning! un-support DSP version decoder.\n");
        return FALSE;
    }
...
MS_BOOL HAL_AUDIO_SetDecodeSystem(AUDIO_DEC_ID DecId, AudioDecStatus_t *p_AudioDecStatus)
{
    ...
    {
		HALAUDIO_ERROR ("%s: MainDecoderId:%d \n", __FUNCTION__, MainDecoderId);
        ...
        if (MainDecoderId != -1)
        {
			...
			HALAUDIO_ERROR ("%s: 2MainDecoderId:%d \n", __FUNCTION__, MainDecoderId);
    ...
    switch(Output_Type)
    {
        ...
        case AUDIO_HDMI_ARC_OUTPUT:
            if( (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_HDMITx_SMP_RATE, ID) > 48000) && (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL)&0x02))
                ...
                if( (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_ARC_HDMI_CTRL) & 0x1) == 1)
                {
                    HAL_AUDIO_WriteMaskByte(REG_AUDIO_DOWN_SAMPLE, 0x80, 0x80); /* enable synthesizer */
                ...
                if( (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_ARC_HDMI_CTRL) & 0x1) == 1)
                {
                    HAL_AUDIO_WriteMaskByte(REG_AUDIO_DOWN_SAMPLE, 0x80, 0x00); /* disable synthesizer */
                    HAL_AUDIO_WriteMaskByte(REG_AUDIO_CLK_CFG6+1,  0x04, 0x00); /* disable sel HDMI TX clock from synthesizer */
                }
                }
            break;
...
MS_S32 HAL_AUDIO_SetEDIDOutputInterface(DIGITAL_OUTPUT_TYPE outpath)
{
    if (outpath == DIGITAL_HDMI_ARC_OUTPUT)
    {
        ...
        HAL_AUDIO_WriteMaskByte(REG_SE_MAD_OFFSET_BASE_L,      0x10, 0x00); //{0x112aa2, 0x10, 0x00}, //[4]=0       IRQ1L select SPDIF_TX2
        HAL_AUDIO_WriteMaskByte(REG_ES_CTRL_CFG,               0x20, 0x20); //{0x112ad8, 0x20, 0x20}, //[5]=1       IRQ1L select SPDIF_TX2
        HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_ARC_HDMI_CTRL, 0x01, 0x00); //{0x112d8c, 0x01, 0x00}  //[0]=0       SPDIF+ARC+HDMITx(PCM)
    }
    else if (outpath == DIGITAL_HDMI_OUTPUT)
    {
        //SPDIF + HDMI TX
        HAL_AUDIO_WriteMaskByte(REG_SE_MAD_OFFSET_BASE_L,      0x80, 0x00); //{0x112aa2, 0x80, 0x00}, //[7]=0       IRQ1E select SPDIF_TX2
        HAL_AUDIO_WriteMaskByte(REG_ES_CTRL_CFG,               0x10, 0x10); //{0x112ad8, 0x10, 0x10}, //[4]=1       IRQ1E select SPDIF_TX2
        HAL_AUDIO_WriteMaskByte(REG_SE_MAD_OFFSET_BASE_L,      0x40, 0x40); //{0x112aa2, 0x40, 0x40}, //[6:5]=2'b10 IRQ2n select SPDIF_TX
        HAL_AUDIO_WriteMaskByte(REG_SE_MAD_OFFSET_BASE_L,      0x10, 0x00); //{0x112aa2, 0x10, 0x00}, //[4]=0       IRQ1L select HDMI TX
        HAL_AUDIO_WriteMaskByte(REG_ES_CTRL_CFG,               0x20, 0x00); //{0x112ad8, 0x20, 0x00}, //[5]=0       IRQ1L select HDMI TX
        HAL_AUDIO_WriteMaskByte(REG_M2D_MAILBOX_ARC_HDMI_CTRL, 0x01, 0x01); //{0x112d8c, 0x01, 0x01}  //[0]=0       SPDIF+HDMITx
    }
    else
    {
        // Not Support now
...
MS_U32 HAL_AUDIO_MP3_ENCODER1_Read(void *pBuf, MS_U32 u32Size)
{
    ...
    //HALAUDIO_ERROR("%s: Read %u bytes from %s\n", __FUNCTION__, u32Size, pPCM->u8Name);
    ...
    if (pEncoderBuf == NULL)
    {
        HALAUDIO_ERROR("%s: Error! pEncoderBuf should not be NULL!\n", __FUNCTION__);
        return 0;
    }
    ...
    {
        HALAUDIO_ERROR("%s: Warning! MP3 buffer full!\n", __FUNCTION__);
    ...
    {
        HALAUDIO_ERROR("%s: Warning! MP3 buffer empty!\n", __FUNCTION__);
    }
