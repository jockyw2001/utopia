...
/// @brief \b Function \b Name: HAL_SOUND_EnableSineTone()  @@VVV
/// @brief \b Function \b Description: This routine is used to bEnable SineTone play.
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_SOUND_EnableSineTone(void)
{
    HAL_AUDIO_WriteMaskByte(REG_SOUND_SINE_GEN, 0x01, 0x01);
    AUDIO_DELAY1MS(40);
    HAL_AUDIO_WriteMaskByte(REG_SOUND_SINE_GEN, 0x01, 0x00);
}
...
/// @brief \b Function \b Description: This routine is used to get the u8Value of audio SPDIF delay.
/// @param <RET>        \b delay    :     0x14~0xC8(20~200), in ms unit
////////////////////////////////////////////////////////////////////////////////
...
MS_BOOL HAL_SND_ProcessEnable(Sound_ENABLE_Type Type, MS_BOOL enable)
{
    ...
    case Sound_ENABL_Type_DRC:
       ...
       case Sound_ENABL_Type_KTVEcho:
        ...
        case Sound_ENABL_Type_SineTone:
            HAL_SOUND_EnableSineTone();
            status = TRUE;
            break;
...
MS_BOOL HAL_SND_SetParam(Sound_SET_PARAM_Type Type, MS_U16 param1, MS_U16 param2)
{
    ...
    switch(Type)
    {
        ...
        case Sound_SET_PARAM_Sinetone_StartFreq:
        {
            if (param1 < 100)
            {
                param1 = 100;
            ...
            HAL_MAD2_Write_DSP_sram(DSP2DmAddr_sinetone_StartFreq_Addr, param1*SINE_GEN_PARAM_FREQ_48k, DSP_MEM_TYPE_PM);
            status = true;

            break;
        }

        case Sound_SET_PARAM_Sinetone_EndFreq:
        {
            if(param1 < 100)
            {
                param1 = 100;
            ...
            HAL_MAD2_Write_DSP_sram(DSP2DmAddr_sinetone_EndFreq_Addr, param1*SINE_GEN_PARAM_FREQ_48k, DSP_MEM_TYPE_PM);
            status = true;

            break;
        }

        case Sound_SET_PARAM_Sinetone_Duration:
        {
            MS_U16 u16StartFreq;
            MS_U16 u16EndFreq;
            MS_U16 u16Step;
            ...
            u16StartFreq = (MS_U16)HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sinetone_StartFreq_Addr, DSP_MEM_TYPE_PM);
            u16EndFreq = (MS_U16)HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sinetone_EndFreq_Addr, DSP_MEM_TYPE_PM);
            u16Step = (u16StartFreq - u16EndFreq)*SINE_GEN_PARAM_STEPS/(param1*48);
            HAL_MAD2_Write_DSP_sram(DSP2DmAddr_sinetone_SweepStep_Addr, u16Step , DSP_MEM_TYPE_PM);
            HAL_MAD2_Write_DSP_sram(DSP2DmAddr_sinetone_Duration_Addr, param1 , DSP_MEM_TYPE_PM);
            status = true;

            break;
        }

        case Sound_SET_PARAM_Sinetone_Mode:
        {
            if (param1 > E_AUDIO_SINETONE_MODE_800TO300)
            {
                param1 = E_AUDIO_SINETONE_MODE_800TO300;
            }

            HAL_AUDIO_WriteMaskByte(REG_SOUND_SINE_GEN, M2S_MBOX_MENUSOUND_MODE_SEL_MASK, (MS_U8)param1<<4);
            status = true;

            break;
        }

        case Sound_SET_PARAM_Sinetone_Ctrl:
        {
            MS_U8 u8Enable = (MS_U8)param2;
            status = TRUE;
            switch (param1)
            {
                case E_AUDIO_SINETONE_CTRL_RIGHTMUTE:
                {
                    HAL_AUDIO_WriteMaskByte(REG_SOUND_SINE_GEN, 0x08, u8Enable << (M2S_MBOX_MENUSOUND_MUTE_RIGHT_BIT));
                    break;
                }

                case E_AUDIO_SINETONE_CTRL_LEFTMUTE:
                {
                    HAL_AUDIO_WriteMaskByte(REG_SOUND_SINE_GEN, 0x04, u8Enable << (M2S_MBOX_MENUSOUND_MUTE_LEFT_BIT));
                    break;
                }

                case E_AUDIO_SINETONE_CTRL_PAUSE:
                {
                    HAL_AUDIO_WriteMaskByte(REG_SOUND_SINE_GEN, 0x02, u8Enable << (M2S_MBOX_MENUSOUND_PAUSE_BIT));
                    break;
                }

                case E_AUDIO_SINETONE_CTRL_PLAY:
                {
                    HAL_AUDIO_WriteMaskByte(REG_SOUND_SINE_GEN, 0x01, u8Enable << (M2S_MBOX_MENUSOUND_PLAY_BIT));
                    break;
                }

                default:
                {
                    HALSOUND_ERROR("%s: invalid audio menu sound ctrl %d\n", __FUNCTION__, param1);
                    status = FALSE;
                    break;
                }
            }
...
MS_U16 HAL_SND_GetParam( Sound_GET_PARAM_Type Type, MS_U16 param1)
{
    ...
    switch(Type)
    {
        ...
        case Sound_GET_PARAM_Sinetone_StartFreq:
        {
            MS_U32 u32Value;
            u32Value = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sinetone_StartFreq_Addr, DSP_MEM_TYPE_PM);
            tmp = (MS_U16)(u32Value >> 8);
            break;
        }

        case Sound_GET_PARAM_Sinetone_EndFreq:
        {
            MS_U32 u32Value;
            u32Value = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sinetone_EndFreq_Addr, DSP_MEM_TYPE_PM);
            tmp = (MS_U16)(u32Value >> 8);
            break;
        }

        case Sound_GET_PARAM_Sinetone_Duration:
        {
            MS_U32 u32Value;
            u32Value = HAL_MAD2_Read_DSP_sram(DSP2DmAddr_sinetone_Duration_Addr, DSP_MEM_TYPE_PM);
            tmp = (MS_U16)(u32Value >> 8);
            break;
        }
