...
// VPU Reg
//------------------------------------------------------------------------------
// on kano, pVPUHalContext->_stCtx[0] for VPU_EVDR2 and pVPUHalContext->_stCtx[1] for VPU_EVDR2_LITE
#define VPU_EVDR2       0
#define VPU_EVDR2_LITE  1

#define REG_VPU_BASE              (0x0300)
#define REG_MBX_BASE              (0x0400)
#define REG_VPU_LITE_BASE        (0x12700)
#define REG_MBX_LITE_BASE        (0x12800)

#define VPU_BASE(vpu)        ((vpu==VPU_EVDR2)?(REG_VPU_BASE):(REG_VPU_LITE_BASE))
#define MBX_BASE(vpu)        ((vpu==VPU_EVDR2)?(REG_MBX_BASE):(REG_MBX_LITE_BASE))

#ifdef HAL_FEATURE_MAU  // abandoned, no mau on kano
#define REG_MAU1_BASE             (0x0400)
#define REG_MAU1_LV2_0_BASE      (0x63500)
#define REG_MAU1_LV2_1_BASE      (0x63600)
#define REG_MAU1_LITE_BASE        (0x0400)
#define REG_MAU1_LITE_LV2_0_BASE (0x63500)
#define REG_MAU1_LITE_LV2_1_BASE (0x63600)

#define MAU1_BASE(vpu)       ((vpu==VPU_EVDR2)?(REG_MAU1_BASE):(REG_MAU1_LITE_BASE))
#define MAU1_LV2_0_BASE(vpu) ((vpu==VPU_EVDR2)?(REG_MAU1_LV2_0_BASE):(REG_MAU1_LITE_LV2_0_BASE))
#define MAU1_LV2_1_BASE(vpu) ((vpu==VPU_EVDR2)?(REG_MAU1_LV2_1_BASE):(REG_MAU1_LITE_LV2_1_BASE))
#endif



//write back stage PC
#define VPU_REG_EXPC_L(vpu)          (VPU_BASE(vpu)+(0x000a<<1))
#define VPU_REG_EXPC_H(vpu)          (VPU_BASE(vpu)+(0x000b<<1))

#define VPU_REG_ICU_STATUS(vpu)      (VPU_BASE(vpu)+(0x001f<<1))
    #define VPU_REG_ICU_IDLE            BIT(1)
    #define VPU_REG_ISB_IDLE            BIT(8)

#define VPU_REG_DCU_DBG_SEL(vpu)     (VPU_BASE(vpu)+(0x0028<<1))
    #define VPU_REG_DCU_DBG_SEL_0	    BIT(2)                 
    #define VPU_REG_DCU_DBG_SEL_1	    BIT(3)                 
#define VPU_REG_DCU_STATUS(vpu)      (VPU_BASE(vpu)+(0x0029<<1))
    #define VPU_REG_BIU_EMPTY           BIT(11)

#define VPU_REG_CPU_SETTING(vpu)     (VPU_BASE(vpu)+(0x0040<<1))
...
#define VPU_REG_ICU_SDR_BASE_L(vpu)  (VPU_BASE(vpu)+(0x0041<<1))  //byte address
#define VPU_REG_ICU_SDR_BASE_H(vpu)  (VPU_BASE(vpu)+(0x0042<<1))
#define VPU_REG_DCU_SDR_BASE_L(vpu)  (VPU_BASE(vpu)+(0x0043<<1))  //byte address
#define VPU_REG_DCU_SDR_BASE_H(vpu)  (VPU_BASE(vpu)+(0x0044<<1))

#define VPU_REG_SPI_BASE(vpu)        (VPU_BASE(vpu)+(0x0048<<1))  //REG ACCESS BASE32


#define VPU_REG_IQMEM_BASE_L(vpu)    (VPU_BASE(vpu)+(0x0049<<1))
#define VPU_REG_IQMEM_BASE_H(vpu)    (VPU_BASE(vpu)+(0x004a<<1))
#define VPU_REG_IQMEM_MASK_L(vpu)    (VPU_BASE(vpu)+(0x004b<<1))
#define VPU_REG_IQMEM_MASK_H(vpu)    (VPU_BASE(vpu)+(0x004c<<1))

#define VPU_REG_DQMEM_BASE_L(vpu)    (VPU_BASE(vpu)+(0x004d<<1))
#define VPU_REG_DQMEM_BASE_H(vpu)    (VPU_BASE(vpu)+(0x004e<<1))
#define VPU_REG_DQMEM_MASK_L(vpu)    (VPU_BASE(vpu)+(0x004f<<1))
#define VPU_REG_DQMEM_MASK_H(vpu)    (VPU_BASE(vpu)+(0x0050<<1))

#define VPU_REG_PQMEM_BASE_L(vpu)    (VPU_BASE(vpu)+(0x0051<<1))
#define VPU_REG_PQMEM_BASE_H(vpu)    (VPU_BASE(vpu)+(0x0052<<1))
#define VPU_REG_PQMEM_MASK_L(vpu)    (VPU_BASE(vpu)+(0x0053<<1))
#define VPU_REG_PQMEM_MASK_H(vpu)    (VPU_BASE(vpu)+(0x0054<<1))
#define VPU_REG_IQMEM_SETTING(vpu)   (VPU_BASE(vpu)+(0x0064<<1))
#define VPU_REG_VQMEM_BASE_L(vpu)    (VPU_BASE(vpu)+(0x0067<<1))
#define VPU_REG_VQMEM_BASE_H(vpu)    (VPU_BASE(vpu)+(0x0068<<1))
#define VPU_REG_VQMEM_MASK_L(vpu)    (VPU_BASE(vpu)+(0x0069<<1))
#define VPU_REG_VQMEM_MASK_H(vpu)    (VPU_BASE(vpu)+(0x006a<<1))
#define VPU_REG_CPU_CONFIG(vpu)      (VPU_BASE(vpu)+(0x006b<<1))
    #define VPU_REG_CPU_STALL_EN        BIT(15)

#define VPU_REG_QMEM_OWNER(vpu)      (VPU_BASE(vpu)+(0x006c<<1))

#define VPU_REG_IO0_BASE(vpu)        (VPU_BASE(vpu)+(0x0045<<1))   //RIU
#define VPU_REG_IO1_BASE(vpu)        (VPU_BASE(vpu)+(0x0055<<1))   //R2 internal UART
#define VPU_REG_IO2_BASE(vpu)        (VPU_BASE(vpu)+(0x0056<<1))   //R2 read SPI
#define VPU_REG_IO3_BASE(vpu)        (VPU_BASE(vpu)+(0x0057<<1))   //IP use
#define VPU_REG_CONTROL_SET(vpu)     (VPU_BASE(vpu)+(0x0058<<1))

#define VPU_REG_R2_MI_SEL_BASE(vpu)  (VPU_BASE(vpu)+(0x0072<<1))
...
#define VPU_REG_VERSION(vpu)         (MBX_BASE(vpu)+(0x0055<<1))

#define VPU_REG_HI_MBOX0_L(vpu)      (MBX_BASE(vpu)+(0x005b<<1))
#define VPU_REG_HI_MBOX0_H(vpu)      (MBX_BASE(vpu)+(0x005c<<1))
#define VPU_REG_HI_MBOX1_L(vpu)      (MBX_BASE(vpu)+(0x005d<<1))
#define VPU_REG_HI_MBOX1_H(vpu)      (MBX_BASE(vpu)+(0x005e<<1))

#define VPU_REG_HI_MBOX_SET(vpu)     (MBX_BASE(vpu)+(0x005f<<1))
    #define VPU_REG_HI_MBOX0_SET        BIT(0)
    #define VPU_REG_HI_MBOX1_SET        BIT(1)

#define VPU_REG_RISC_MBOX_CLR(vpu)   (MBX_BASE(vpu)+(0x0067<<1))
...
#define VPU_REG_RISC_MBOX_RDY(vpu)   (MBX_BASE(vpu)+( 0x0068<<1))
    #define VPU_REG_RISC_MBOX0_RDY      BIT(0)
    #define VPU_REG_RISC_MBOX1_RDY      BIT(1)
    #define VPU_REG_RISC_ISR_VALID      BIT(2)

#define VPU_REG_HI_MBOX_RDY(vpu)     (MBX_BASE(vpu)+(0x0069<<1))
    #define VPU_REG_HI_MBOX0_RDY        BIT(0)
    #define VPU_REG_HI_MBOX1_RDY        BIT(1)

#define VPU_REG_RISC_MBOX0_L(vpu)    (MBX_BASE(vpu)+(0x006b<<1))
#define VPU_REG_RISC_MBOX0_H(vpu)    (MBX_BASE(vpu)+(0x006c<<1))
#define VPU_REG_RISC_MBOX1_L(vpu)    (MBX_BASE(vpu)+(0x006d<<1))
#define VPU_REG_RISC_MBOX1_H(vpu)    (MBX_BASE(vpu)+(0x006e<<1))



#ifdef HAL_FEATURE_MAU  // abandoned, no mau on kano
#define MAU1_CPU_RST(vpu)            (MAU1_BASE(vpu)+(0x0002<<1))
    #define MAU1_REG_SW_RESET           BIT(0)

#define MAU1_ARB0_DBG0(vpu)          (MAU1_BASE(vpu)+(0x0008<<1))
#define MAU1_ARB1_DBG0(vpu)          (MAU1_BASE(vpu)+(0x000a<<1))
    #define MAU1_FSM_CS_MASK            BMASK(13:9)
    #define MAU1_FSM_CS_IDLE            BITS(13:9,1)

#define MAU1_MIU_SEL(vpu)            (MAU1_BASE(vpu)+(0x0001<<1))

#define MAU1_LV2_0_MIU_SEL(vpu)      (MAU1_LV2_0_BASE(vpu)+(0x0001<<1))

#define MAU1_LV2_1_MIU_SEL(vpu)      (MAU1_LV2_1_BASE(vpu)+(0x0001<<1))
#endif
