...
MS_BOOL HAL_MVD_RstHW(MS_U32 u32Id)
{
    ...
    HAL_MVD_SetReqMask(u32Id, ENABLE);
    ...
    HAL_MVD_SetReqMask(u32Id, DISABLE);
...
static void MVD_GetTaskInfo(VPU_EX_TaskInfo* pstTaskInfo, MS_U32 u32Id, MS_U8 u8Idx, HAL_VPU_StreamId eVpuId)
{
    pstTaskInfo->u32Id = u32Id & 0xFFFF;
...
void HAL_MVD_Eng_MiuClientSel(MS_U8 u8MiuSel)  //SW select
{
    if (u8MiuSel == E_CHIP_MIU_0)
    {
        HAL_MVD_RegWriteBit(MIU0_SEL2_L, 0, BIT6);
        HAL_MVD_RegWriteBit(MIU0_SEL4_H, 0, BIT1);
    }
    else if (u8MiuSel == E_CHIP_MIU_1)
    {
        HAL_MVD_RegWriteBit(MIU0_SEL2_L, 1, BIT6);
        HAL_MVD_RegWriteBit(MIU0_SEL4_H, 1, BIT1);
    }
}
...
MS_BOOL HAL_MVD_CreateTask(MS_U8 u8Idx, HAL_VPU_StreamId eVpuId)
{
    ...
    HAL_VPU_EX_SetFWReload(u32Id, !(pstCfg->bNotReload));

    MVD_DEBUGINFO(MVD_PRINT("\n\n Create Task(%x)!!!\n", u8Idx));
    VPU_EX_TaskInfo stTaskInfo;
    MVD_GetTaskInfo(&stTaskInfo, u32Id, u8Idx, eVpuId);
    ...
    _phy_to_miu_offset(u8MiuSel, u32StartOffset, pMemCfg->u32FBAddr);
    //VPRINTF("[MVD FB] addr=0x%x, size=0x%x, miu=%d \n", (MS_U32)(pMemCfg->u32FBAddr), pMemCfg->u32FBSize, u8MiuSel);
    HAL_MVD_Eng_MiuClientSel(u8MiuSel);
...
MS_BOOL HAL_MVD_ClearCmdFinished(MS_U8 u8Idx, MVD_HANDSHAKE_CMD eCmd)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("after CLR u32CmdState.value = 0x%x\n",
...
static MS_BOOL MVD_GetFWBuffAdd(MS_VIRT u32Start, MS_U32 u32AvailLen, MVD_FWBuff* pBuff)
{
    ...
    {
        //shared memory should not overlap with FW memory pool.
        MVD_DEBUGERROR(MVD_PRINT("%s err: 0x%x out of memory boundary!\n", __FUNCTION__,
...
MS_BOOL HAL_MVD_SetCodecInfo(MS_U8 u8Idx, MVD_CodecType eCodecType, MVD_SrcMode eSrcMode, MS_U8 bDisablePESParsing, MS_U8 u8ChipECORev)
{
    MVD_CmdArg stCmdArg;
...
MS_BOOL HAL_MVD_DeinitHW(MS_U32 u32Id, VPU_EX_SourceType SourceType, VPU_EX_DecoderType eDecType)
{
    //MVD HW reset
    if (!HAL_MVD_RstHW(u32Id))
    {
...
E_MVD_Result HAL_MVD_IsDispFinish(MS_U8 u8Idx)
{
    ...
    if ((E_MVD_SLQ_TBL_MODE == curSrcMode) && (TRUE != FLAG_LAST_FRM_SHOW))
    {
        ...
        if(pstCtrlCfg->u8McuMode == TRUE)
        {
            ...
            {
                if(HAL_VPU_EX_GetVBBUVacancy(u32Id, u32VBBUAddr) > 0)
                {
...
MS_BOOL HAL_MVD_DecodeIFrame(MS_U8 u8Idx, MS_PHY u32FrameBufAddr, MS_PHY u32StreamBufAddr, MS_PHY u32StreamBufEndAddr, MS_U8 u8ChipECORev)
{
    ...
    HAL_MVD_SetCodecInfo(u8Idx, E_MVD_CODEC_MPEG2, E_MVD_SLQ_MODE, DISABLE_PARSER, u8ChipECORev);
            ...
            ||E_MVD_CODEC_VC1_ADV == curCodecType)
        {
            ...
            if((pstSlqTblInfo->pSlqStatus->bSlqCtrlBit) || (E_MVD_CODEC_DIVX311 == curCodecType))
            {
                ...
                if(pstSlqTblInfo->pSlqStatus->bSlqPicStart||(!pstSlqTblInfo->pSlqStatus->bSlqCtrlBit))
                {
                    ...
                    if(pstCtrlCfg->u8McuMode == TRUE)
                    {
                        if(HAL_VPU_EX_GetVBBUVacancy(u32Id, u32VBBUAddr) > 1)
                        {
                            HAL_VPU_EX_PacketInfo stVpuPkt;
...
E_MVD_Result HAL_MVD_EnableDispOneField(MS_U8 u8Idx, MS_BOOL bEn)
{
    ...
    {
        stCmdArg.Arg0 = 1;
        //stCmdArg.Arg1 = 1; //0:default 1:top 2:bottom
    }
...
void HAL_MVD_DbgDump(MS_U8 u8Idx)
{
    ...
    MVD_PRINT("state: fw=0x%x, lastCmd=0x%x, pc=0x%x\n",
            HAL_MVD_GetDecodeStatus(u8Idx), HAL_MVD_GetLastCmd(u8Idx), HAL_VPU_EX_GetProgCnt(u32Id));
...
E_MVD_DISP_QUEUE HAL_MVD_IsDispQueueEmpty(MS_U8 u8Idx)
{
    ...
    //MVD_PRINT("dispQ_rd: %u,dispQ_wr: %u\n",pTemp->dispQ_rd,pTemp->dispQ_wr);
    if(pTemp)
    {
        if (pTemp->dispQ_rd == pTemp->dispQ_wr)
