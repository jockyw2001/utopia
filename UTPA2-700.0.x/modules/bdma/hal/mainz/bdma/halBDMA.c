...
#define BDMA_HAL_NOTSUPPORT()           {ULOGE(TAG_BDMA, "%s not support!\n", __FUNCTION__);}
...
MS_U8 HAL_BDMA_GetDevId(BDMA_Dev eDev)
{
    switch (eDev)
    {
        ...
        case E_BDMA_DEV_MIU2:
            return E_BDMA_HALDEV_MIU2;
        case E_BDMA_DEV_MIU3:
            return E_BDMA_HALDEV_MIU3;
...
{
    MS_U8 u8RegVal; 
    MS_PHY phy64Reg0 = _HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_SRC_SEL, u8Ch);
    MS_PHY phy64Reg1 = _HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_DWUM_CNT, u8Ch);
	
    if( (u8DevCfg & 0x0f) == E_BDMA_HALDEV_MIU0)
    {
        u8DevCfg = u8DevCfg & 0xf0;
        u8DevCfg = u8DevCfg | E_BDMA_HALDEV_MIU0;
        HAL_BDMA_WriteByte(phy64Reg0, u8DevCfg);

        u8RegVal = HAL_BDMA_ReadByte(phy64Reg1);
        u8RegVal &= ~(0x30);
        u8RegVal |=  E_BDMA_MIU0_ALIAS << 4;
        HAL_BDMA_WriteByte(phy64Reg1, u8RegVal);
    }
    else if((u8DevCfg & 0x0f) == E_BDMA_HALDEV_MIU1)
    {
        u8DevCfg = u8DevCfg & 0xf0;
        u8DevCfg = u8DevCfg | E_BDMA_HALDEV_MIU0;
        HAL_BDMA_WriteByte(phy64Reg0, u8DevCfg);
        ...
        u8RegVal = HAL_BDMA_ReadByte(phy64Reg1);
        u8RegVal &= ~(0x30);
        u8RegVal |=  E_BDMA_MIU1_ALIAS << 4;
        HAL_BDMA_WriteByte(phy64Reg1, u8RegVal);

    }
    else if((u8DevCfg & 0x0f) == E_BDMA_HALDEV_MIU2)
    {
        u8DevCfg = u8DevCfg & 0xf0;
        u8DevCfg = u8DevCfg | E_BDMA_HALDEV_MIU0;
        HAL_BDMA_WriteByte(phy64Reg0, u8DevCfg);
        ...
        u8RegVal = HAL_BDMA_ReadByte(phy64Reg1);
        u8RegVal &= ~(0x30);
        u8RegVal |=  E_BDMA_MIU2_ALIAS << 4;
        HAL_BDMA_WriteByte(phy64Reg1, u8RegVal);
    }
    else if((u8DevCfg & 0x0f) == E_BDMA_HALDEV_MIU3)
    {
        u8DevCfg = u8DevCfg & 0xf0;
        u8DevCfg = u8DevCfg | E_BDMA_HALDEV_MIU0;
        HAL_BDMA_WriteByte(phy64Reg0, u8DevCfg);
...
{
    MS_U8 u8RegVal; 
    MS_PHY phy64Reg0 = _HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_DST_SEL, u8Ch);
    MS_PHY phy64Reg1 = _HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_DWUM_CNT, u8Ch);
	
    if( (u8DevCfg & 0x0f) == E_BDMA_HALDEV_MIU0)
    {
        u8DevCfg = u8DevCfg & 0xf0;
        u8DevCfg = u8DevCfg | E_BDMA_HALDEV_MIU1;
        HAL_BDMA_WriteByte(phy64Reg0, u8DevCfg);

        u8RegVal = HAL_BDMA_ReadByte(phy64Reg1);
        u8RegVal &= ~(0xc0);
        u8RegVal |= E_BDMA_MIU0_ALIAS << 6;
        HAL_BDMA_WriteByte(phy64Reg1, u8RegVal);
    }
    else if((u8DevCfg & 0x0f) == E_BDMA_HALDEV_MIU1)
    {
...
void HAL_BDMA_Dump_AllReg(void)
{
    ...
    for (; phy64Reg < phy64EndAddr; phy64Reg+=2)
    {
        if (0 == (phy64Reg%0x10))
            ...
            ULOGD(TAG_BDMA, "\n%lx:   ", (unsigned long long)phy64Reg);
#else
            ULOGD(TAG_BDMA, "\n%llx:   ",(unsigned long long)phy64Reg);
#endif            
        ULOGD(TAG_BDMA, "%4x ", HAL_BDMA_Read2Byte(phy64Reg));
...
void HAL_BDMA_SetSPIOffsetForMCU()
{
     ...
     HAL_BDMA_Write2Byte(0x2e52UL,0x4002);   //PM 51 rst
    ...
    HAL_BDMA_WriteByte(0x2e53UL,0x50);
             ...
             ULOGD(TAG_BDMA, "Wait for PM51 standby...........\n");
	while(HAL_BDMA_Read2Byte(0x0e24UL)!=0x02);

              ULOGD(TAG_BDMA, "PM51 run ok...........\n");
