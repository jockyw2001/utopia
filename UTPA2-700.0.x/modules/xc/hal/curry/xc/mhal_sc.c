...
void Hal_SC_set_shiftline( void *pInstance, MS_U8 u8Val, SCALER_WIN eWindow )
{
    ...
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    //[patch]
    //In Kano, cannot use FBL mode except 4K in/out case, so u8Val only be 0
    //(now PQ send MS_DEINT_3DDI_HISTORY and get u8Val = 2 in 4K in/out case, need fix in PQ)
    u8Val = 0x0;
...
static MS_BOOL _Kano_DB_Patch(void *pInstance, MS_BOOL bUseMenuLoad, SCALER_WIN eWindow)
{
    //Do not need any patch now
    return TRUE;
    #if 0
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[E_XC_POOL_ID_INTERNAL_VARIABLE],(void**)(&pXCResourcePrivate));
    //Because of MVOP GEN + HSK in I mode patch will cause blinking(HW finding root cause)
    //We use SW patch to refine trggier point temporary
    if(0)//MDrv_XC_GetDynamicScalingStatus(pInstance))
    {
        MS_U32 u32RegMVOPHSK = 0;
        if((pXCResourcePrivate->stdrvXC_MVideo.enOldInputSrc_ForScalerSetSource[eWindow] == INPUT_SOURCE_DTV) ||
            (pXCResourcePrivate->stdrvXC_MVideo.enOldInputSrc_ForScalerSetSource[eWindow] == INPUT_SOURCE_STORAGE))
        {
            u32RegMVOPHSK = REG_MVOP_HSK;
        }
        else if((pXCResourcePrivate->stdrvXC_MVideo.enOldInputSrc_ForScalerSetSource[eWindow] == INPUT_SOURCE_DTV2) ||
            (pXCResourcePrivate->stdrvXC_MVideo.enOldInputSrc_ForScalerSetSource[eWindow] == INPUT_SOURCE_STORAGE2))
        {
            u32RegMVOPHSK = REG_SUBMVOP_HSK;
        }

        if(u32RegMVOPHSK != 0)
        {
            if((MDrv_ReadByte(u32RegMVOPHSK) & 0x6) == 0x0)
            {
                //MVOP GEN TIMING
                if(bUseMenuLoad)
                {
                    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_19_L, 0x0 ,0x300); //patch for MVOP GEN TIMING + HSK
                    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_21_L, 0x19, 0xFF);   //patch for MVOP GEN TIMING + HSK
                }
                else
                {
                    //MVOP GEN TIMING
                    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_19_L, 0x0, 0x300); //patch for MVOP GEN TIMING + HSK
                    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_21_L, 0x19, 0xFF);   //patch for MVOP GEN TIMING + HSK
                }
            }
        }
    }
...
static void _Hal_SC_fill_main_sw_db_burst(void *pInstance, P_SC_SWDB_INFO pDBreg)
{
    ...
    if(E_XC_DEVICE1 == psXCInstPri->u32DeviceID)
    {
        ...
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0B_L, BIT(14), BIT(15)|BIT(14));
        }
        else
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0B_L, 0, BIT(15)|BIT(14));
        }

        if(IS_H_POST_SCALING(pDBreg->u32H_PostScalingRatio))
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0B_L, BIT(6), BIT(7)|BIT(6));
        }
        else
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0B_L, 0, BIT(7)|BIT(6));
        }
    }
    ...
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
    {
        _Kano_DB_Patch(pInstance, TRUE, MAIN_WINDOW);
    }
}
...
static void _Hal_SC_fill_sub_sw_db_burst(void *pInstance, P_SC_SWDB_INFO pDBreg)
{
    ...
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
    {
        _Kano_DB_Patch(pInstance, TRUE, SUB_WINDOW);
    }
}
...
void Hal_SC_sw_db( void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow )
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        ...
        if(E_XC_DEVICE1 == psXCInstPri->u32DeviceID)
        {
            ...
            if(IS_V_POST_SCALING(pDBreg->u32V_PostScalingRatio))
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_0B_L, BIT(14), BIT(15)|BIT(14));
            }
            else
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_0B_L, 0, BIT(15)|BIT(14));
            }

            if(IS_H_POST_SCALING(pDBreg->u32H_PostScalingRatio))
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_0B_L, BIT(6), BIT(7)|BIT(6));
            }
            else
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_0B_L, 0, BIT(7)|BIT(6));
            }
        }
    ...
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
    {
        _Kano_DB_Patch(pInstance, FALSE, eWindow);
    }
}
...
//3.XC
            ...
            //Patch for Kano P mode force VDEC FRC and I mode force XC FRC
            if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
            {
                _Kano_DB_Patch(pInstance, FALSE, eWindow);
            }
...
void HAL_SC_SetDisplay_Start_Mode(void *pInstance, MS_BOOL bEnable)
{
    ...
    if(psXCInstPri->u32DeviceID == 0)
    {
        ...
        {
            //If DE mode, to avoid display window v start not 0 cause capture window unstable.
            //We can not set advance 1 display line mode
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_1F_L, 0, BIT(5));
        }
    }
}
...
void Hal_SC_Set_OSD2VE(void *pInstance, EN_VOP_SEL_OSD_XC2VE_MUX  eVOPSelOSD_MUX)
{
    ...
    switch(eVOPSelOSD_MUX)
    {
        ...
        default:
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_28_L, 0x0000, 0x0001);
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0F_46_L, 0x0050, 0x00F0);
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0F_57_L, 0x0800, 0x0800);
            break;
    }
}
    ...
    *pbForceH = FALSE;
    ...
    else if(E_XC_DEVICE1 == psXCInstPri->u32DeviceID) // SC1
    {
            ...
            || IsSrcTypeStorage(gSrcInfo[eWindow].enInputSourceType))
        {
            ...
            {
                MDrv_WriteByteMask(REG_CKG_SC1_EDCLK, CKG_SC1_EDCLK_172MHZ, CKG_SC1_EDCLK_MASK);    //default setting
            }
        }
...
MS_BOOL Hal_SC_Init(void *pInstance)
{
    ...
    if(psXCInstPri->u32DeviceID == 0)
    {
        ...
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0F_46_L, 0x0050, 0x00F0); // cap_stage
        ...
        // Disable HDR clock for power saving
        MDrv_WriteByteMask(0x100B16, 0x0, 0x1); // Disable HDR clock
        MDrv_WriteByteMask(0x100A4A, 0x0, 0x11);// Disable HDR SRAM clock

        if(MDrv_SYS_GetChipRev() > 0x00)
        {
            //Init ECO bit
            //OPM auto black does not effect OPS
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_30_L, 0x0004, 0x0004);
            //HDR y gain/offset eco bit
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK42_0F_L, 0x0001, 0x0001);
...
MS_BOOL Hal_SC_CheckMuteStatusByRegister(void *pInstance, SCALER_WIN eWindow)
{
    ...
    //In Kano we force return Mute if Main OR Sub window black screen
    UNUSED(eWindow);
