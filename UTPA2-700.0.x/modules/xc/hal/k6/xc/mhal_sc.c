    ...
    if(E_XC_DEVICE1 == psXCInstPri->u32DeviceID)
    {
        ...
        for (i=0; i<HAL_SC1_HVSP_SRAM_PHASE; i++)
        {
            ...
            if((u16SC1VSPFilter == E_XC_UNSET)
                &&(gSrcInfo[MAIN_WINDOW].stCapWin.width == 0)
                &&(gSrcInfo[MAIN_WINDOW].stCapWin.height == 0))
            {
            ...
            {
                //SC0 SD input
                if((gSrcInfo[MAIN_WINDOW].stCapWin.width < 1200)
                    &&(gSrcInfo[MAIN_WINDOW].stCapWin.height < 700))
                {
                    if(u16SC1VSPFilter == E_XC_UNSET)
                    {
                        pu8filter = au8Sc1HvspFilterFc100[0];
                ...
                else
                {
                    ...
                    //SC0 HD output
                    else if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 700)
                        && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 1200))
                    {
