...
static MS_U16 u16RegBK43Table[BANK_REG_MAX]= {0};
static MS_U16 u16RegBK49Table[BANK_REG_MAX]= {0};
...
({\
    ...
    else if((u32Reg&0xff00) == 0x4300)\
    {\
        u16RegBK43Table[((u32Reg&0xff)>>1)] = u16Val;\
    }\
    else if((u32Reg&0xff00) == 0x4900)\
    {\
        u16RegBK49Table[((u32Reg&0xff)>>1)] = u16Val;\
    }\
    if ((eWindow==DIP_WINDOW)&&(g_bCMDQ_Enable[eWindow] == TRUE)){\
...
({\
    ...
    {\
        u16RegBK43Table[((u32Reg&0xff)>>1)] = u32Val&0x00FF;\
        u16RegBK43Table[(((u32Reg+2)&0xff)>>1)] = (u32Val&0xFF00)>>16;\
    }\
    else if((u32Reg&0xff00) == 0x4900)\
    {\
        u16RegBK49Table[((u32Reg&0xff)>>1)] = u32Val&0x00FF;\
        u16RegBK49Table[(((u32Reg+2)&0xff)>>1)] = (u32Val&0xFF00)>>16;\
    }\
    if ((eWindow==DIP_WINDOW)&&(g_bCMDQ_Enable[eWindow] == TRUE))\
...
static MS_U16 DIP_R2BYTE(MS_U32 u32Id,MS_U32 u32Reg,SCALER_DIP_WIN eWindow)
{
    ...
    if((eWindow==DIP_WINDOW)&&(g_bCMDQ_Enable[eWindow] == TRUE))
    {
        ...
        {
            u16val=u16RegBK43Table[u32idx];
        }
        else if((u32Reg&0xff00)==0x4900)
        {
            u16val=u16RegBK49Table[u32idx];
        }
        else
        {
...
static MS_U32 DIP_R4BYTE(MS_U32 u32Id, MS_U32 u32Reg,SCALER_DIP_WIN eWindow)
{
    ...
    if((eWindow==DIP_WINDOW)&&(g_bCMDQ_Enable[eWindow] == TRUE))
    {
        ...
        {
            u32val=(u16RegBK43Table[u32idx])|(u16RegBK43Table[u32idx+1]<<16);
        }
        else if((u32Reg&0xff00)==0x4900)
        {
            u32val=(u16RegBK49Table[u32idx])|(u16RegBK49Table[u32idx+1]<<16);
        }
        else
        {
...
static MS_U16 DIP_R2BYTEMSK(MS_U32 u32Id, MS_U32 u32Reg, MS_U16 u16Mask,SCALER_DIP_WIN eWindow)
{
    ...
    if((eWindow==DIP_WINDOW)&&(g_bCMDQ_Enable[eWindow] == TRUE))
    {
        ...
        {
            u16val=u16RegBK43Table[u32idx]&u16Mask;
        }
        else if((u32Reg&0xff00)==0x4900)
        {
            u16val=u16RegBK49Table[u32idx]&u16Mask;
        }
        else
        {
...
static MS_U16 HAL_XC_DIP_GetWidth(void *pInstance, SCALER_DIP_SOURCE_TYPE eSource, SCALER_DIP_WIN eWindow)
{
    ...
    else if(eSource == SCALER_DIP_SOURCE_TYPE_DRAM)
    {
        ...
        {
            u16MaxWith = DIP_R2BYTEMSK(0, REG_SC_BK3C_27_L,BMASK(13:0),eWindow);
        }
    }
    return u16MaxWith;
}
...
void HAL_XC_DIP_Init(void *pInstance, SCALER_DIP_WIN eWindow)
{
    ...
    MDrv_DIP_WriteByteMask(REG_CKG_DIP_FCLK, CKG_FCLK_345MHZ, CKG_FCLK_MASK);
    ...
    if( eWindow == DIP_WINDOW )
    {
        ...
        MDrv_DIP_WriteRegBit(REG_CKG_IDCLK3, CKG_IDCLK3_GATED, CKG_IDCLK3_GATED);                   // Disable clock
        ...
        DIP_W2BYTEMSK(0, REG_SC_BK6C_40_L, BIT(7), BIT(7));    //Disable clock
    ...
    else if( eWindow == DWIN0_WINDOW )
    {
        ...
        MDrv_DIP_WriteRegBit(REG_CKG_PDW0, CKG_PDW0_GATED, CKG_PDW0_GATED);                       // Disable clock
        ...
        DIP_W2BYTEMSK(0, REG_SC_BK6C_60_L, BIT(7), BIT(7));    //Disable clock
#ifdef CONFIG_MSTAR_SRAMPD
...
#endif
        MDrv_DIP_WriteRegBit(REG_CKG_IDCLK3, DISABLE, CKG_IDCLK3_GATED);
...
void HAL_XC_DIP_HSPScaling(void *pInstance,MS_BOOL bEnable,MS_U16 ScalingSrc,MS_U16 ScalingDst, SCALER_DIP_WIN eWindow)
{
    ...
    if(eWindow == DIP_WINDOW)
    {
        DIP_W2BYTE(0, REG_SC_BK49_22_L, ScalingSrc);
        DIP_W2BYTE(0, REG_SC_BK49_24_L, ScalingDst);
        DIP_W4BYTE(0, REG_SC_BK49_07_L, u32Ratio);
        DIP_W2BYTEMSK(0, REG_SC_BK49_08_L,bEnable?BIT(8):0,BIT(8));
    }
    else if(eWindow == DWIN0_WINDOW)
    {
        DIP_W2BYTE(0, REG_SC_BK4E_22_L, ScalingSrc);
        DIP_W2BYTE(0, REG_SC_BK4E_24_L, ScalingDst);
        DIP_W4BYTE(0, REG_SC_BK4E_07_L, u32Ratio);
        DIP_W2BYTEMSK(0, REG_SC_BK4E_08_L,bEnable?BIT(8):0,BIT(8));
    }
}
...
void HAL_XC_DIP_VSPScaling(void *pInstance,MS_BOOL bEnable,MS_U16 ScalingSrc,MS_U16 ScalingDst, SCALER_DIP_WIN eWindow)
{
    ...
    if(eWindow == DIP_WINDOW)
    {
        DIP_W2BYTE(0, REG_SC_BK49_23_L, ScalingSrc);
        DIP_W2BYTE(0, REG_SC_BK49_25_L, ScalingDst);
        DIP_W4BYTE(0, REG_SC_BK49_09_L, u32Ratio);
        DIP_W2BYTEMSK(0, REG_SC_BK49_0A_L,bEnable?BIT(8):0,BIT(8));
    }
    else if(eWindow == DWIN0_WINDOW)
    {
        DIP_W2BYTE(0, REG_SC_BK4E_23_L, ScalingSrc);
        DIP_W2BYTE(0, REG_SC_BK4E_25_L, ScalingDst);
        DIP_W4BYTE(0, REG_SC_BK4E_09_L, u32Ratio);
        DIP_W2BYTEMSK(0, REG_SC_BK4E_0A_L,bEnable?BIT(8):0,BIT(8));
    }
}
...
void HAL_XC_DIP_AdjustScaling(void *pInstance,MS_BOOL bAVG,SCALER_DIP_WIN eWindow)
{
    ...
    if(eHVSPWindow != MAX_DIP_WINDOW)
    {
        ...
        {
           HAL_XC_DIP_HSPScaling(pInstance,TRUE,HCusScalingSrc,g_DIPSrcInfo[eWindow].u16HCusScalingDst,eWindow);
           HAL_XC_DIP_VSPScaling(pInstance,TRUE,g_DIPSrcInfo[eWindow].u16VCusScalingSrc,g_DIPSrcInfo[eWindow].u16VCusScalingDst,eWindow);
        }
    }
}
...
void HAL_XC_DIP_LOAD_MFDEC_TABLE(void *pInstance,MS_U8 mfdec_id, SCALER_DIP_WIN eWindow)
{
    ...
    MDrv_DIP_Write2ByteMask(REG_MFDEC_CW_LEN2(mfdec_id), 0x0007 , MFDEC_CW_LEN2_MASK);
    MDrv_DIP_Write2ByteMask(REG_MFDEC_CW_LEN3(mfdec_id), 0x007F , MFDEC_CW_LEN3_MASK);
    MDrv_DIP_Write2ByteMask(REG_MFDEC_CW_LEN4(mfdec_id), 0x0DFE , MFDEC_CW_LEN4_MASK);
    MDrv_DIP_Write2ByteMask(REG_MFDEC_CW_LEN5(mfdec_id), 0x09FE , MFDEC_CW_LEN5_MASK);

    for (u8index=0;u8index< MFEDC_TABLE_INDEX_COUNT; u8index++)
    {
        MDrv_DIP_Write2ByteMask(REG_MFDEC_CW_BASE(mfdec_id), MFDec_MUF_Table[u8index][0] , BMASK(15:0));
        MDrv_DIP_Write2ByteMask(REG_MFDEC_CW_MAX(mfdec_id), MFDec_MUF_Table[u8index][1] , BMASK(15:0));
        MDrv_DIP_Write2ByteMask(REG_MFDEC_SYMB_BASE(mfdec_id), MFDec_MUF_Table[u8index][2] , BMASK(15:0));
        MDrv_DIP_Write2ByteMask(REG_MFDEC_HUF_TAB(mfdec_id), MFDec_MUF_Table[u8index][3] , BMASK(15:0));
    }
}
...
void HAL_XC_DIP_SetDIPRProperty_MFDEC(void *pInstance, ST_DIP_MFDEC_INFO stDIPR_MFDecInfo, SCALER_DIP_WIN eWindow)
{
    ...
    if(stDIPR_MFDecInfo.bMFDec_Enable == TRUE )
    {
        ...
        MDrv_DIP_Write2ByteMask(REG_MFDEC_ENA(u8mfdec_id), u16regtmp , REG_MFDEC_ENA_MASK);
        ...
        MDrv_DIP_Write2ByteMask(REG_MFDEC_FB_MIU(u8mfdec_id), u16regtmp , (MFDEC_LUMA_FB_MIU|MFDEC_CHROMA_FB_MIU|MFDEC_BITLEN_FB_MIU));

        // Set MFDEC X/Y start
        u16str_x = (stDIPR_MFDecInfo.u16StartX)/MFDEC_X_START_ALIGN;
        MDrv_DIP_Write2ByteMask(REG_MFDEC_X_START(u8mfdec_id), u16str_x , MFDEC_X_START);
        MDrv_DIP_Write2ByteMask(REG_MFDEC_Y_START(u8mfdec_id), stDIPR_MFDecInfo.u16StartY , MFDEC_Y_START);

        // Set MFDEC X/Y Size
        u16hsize = (stDIPR_MFDecInfo.u16HSize)/MFDEC_HSIZE_ALIGN;
        MDrv_DIP_Write2ByteMask(REG_MFDEC_H_SIZE(u8mfdec_id), u16hsize , MFDEC_H_SIZE);
        MDrv_DIP_Write2ByteMask(REG_MFDEC_V_SIZE(u8mfdec_id), stDIPR_MFDecInfo.u16VSize , MFDEC_V_SIZE);
        ...
        MDrv_DIP_Write2ByteMask(REG_MFDEC_FB_PITCH(u8mfdec_id), u16regtmp , MFDEC_FB_PITCH);

        // Set MFDEC bitlen pitch
        MDrv_DIP_Write2ByteMask(REG_MFDEC_BITLEN_FB_PITCH(u8mfdec_id), stDIPR_MFDecInfo.u16Bitlen_Pitch , MFDEC_BITLEN_FB_PITCH_MASK);

        // Set MFDEC bitlen FB address
        phybitlen_base = stDIPR_MFDecInfo.phyBitlen_Base / MFDEC_ADDR_ALIGN;
        MDrv_DIP_Write2ByteMask(REG_MFDEC_BITLEN_FB_BASE_L(u8mfdec_id), phybitlen_base , MFDEC_BITLEN_FB_BASE_L);
        MDrv_DIP_Write2ByteMask(REG_MFDEC_BITLEN_FB_BASE_H(u8mfdec_id), phybitlen_base>>16 , MFDEC_BITLEN_FB_BASE_H);
        ...
        {
            MDrv_DIP_WriteRegBit(REG_MFDEC_LINEBUFFER, ENABLE_MFDEC_LB, MFDEC_MAIN_LB);
        }
        else if(u8mfdec_id == MFDEC_ID1)
        {
            MDrv_DIP_WriteRegBit(REG_MFDEC_LINEBUFFER, ENABLE_MFDEC_LB, MFDEC_SUB_LB);
        }
        ...
        if(eWindow == DIP_WINDOW)
        {
            // swap MFDEC
            if(u8mfdec_id == MFDEC_ID0)
                DIP_W2BYTEMSK(0, REG_SC_BK34_5D_L,0,BIT(0));
            else if(u8mfdec_id == MFDEC_ID1)
                DIP_W2BYTEMSK(0, REG_SC_BK34_5D_L,BIT(0),BIT(0));

            // Enable DIP with MFDEC
            DIP_W2BYTEMSK(0, REG_SC_BK36_75_L,BIT(6),BIT(6));
        }
        else if(eWindow == DWIN0_WINDOW)
        {
            // swap MFDEC
            if(u8mfdec_id == MFDEC_ID0)
                DIP_W2BYTEMSK(0, REG_SC_BK3B_5D_L,BIT(0),BIT(0));
            else if(u8mfdec_id == MFDEC_ID1)
                DIP_W2BYTEMSK(0, REG_SC_BK3B_5D_L,0,BIT(0));

            // Enable DIP with MFDEC
            DIP_W2BYTEMSK(0, REG_SC_BK3C_75_L,BIT(6),BIT(6));
        }
    }
    else
    {
        // Disable MFDEC
        MDrv_DIP_Write2ByteMask(REG_MFDEC_ENA(u8mfdec_id), 0 , (MFDEC_RIU1_MODE|MFDEC_RIU_MODE|MFDEC_ENABLE));

        // Disable DIP with MFDEC
        if(eWindow == DIP_WINDOW)
        {
            DIP_W2BYTEMSK(0, REG_SC_BK36_75_L,0,BIT(6));
        }
        else if(eWindow == DWIN0_WINDOW)
        {
            DIP_W2BYTEMSK(0, REG_SC_BK3C_75_L,0,BIT(6));
        }
    }
}
...
SCALER_DIP_WIN HAL_XC_DIP_GetHVSP(void* pInstance,SCALER_DIP_WIN eWindow)
{
        ...
        if(eWindow == DIP_WINDOW)
        {
             u16Reg = DIP_R2BYTEMSK(0, REG_SC_BK49_70_L, BMASK(2:1),eWindow);
        }
        else if(eWindow == DWIN0_WINDOW)
        {
             u16Reg = DIP_R2BYTEMSK(0, REG_SC_BK4E_70_L, BMASK(2:1),eWindow);
        }
...
MS_BOOL HAL_XC_DIP_SetHVSP(void* pInstance,MS_BOOL bSelect, SCALER_DIP_WIN eWindow)
{
    ...
    {
        if(eWindow == DIP_WINDOW)
            DIP_W2BYTEMSK(0, REG_SC_BK49_70_L,BMASK(2:1),BMASK(2:1));
        else if(eWindow == DWIN0_WINDOW)
            DIP_W2BYTEMSK(0, REG_SC_BK4E_70_L,BMASK(2:1),BMASK(2:1));
    }
    else
    {
         if(eWindow == DIP_WINDOW)
            DIP_W2BYTEMSK(0, REG_SC_BK49_70_L,BIT(2),BMASK(2:1));
        else if(eWindow == DWIN0_WINDOW)
            DIP_W2BYTEMSK(0, REG_SC_BK4E_70_L,0x0,BMASK(2:1));
    }
    return TRUE;
}
...
MS_BOOL HAL_XC_DIP_Set420TileBlock(void* pInstance,EN_XC_DIP_TILE_BLOCK eTileBlock,SCALER_DIP_WIN eWindow)
{
    ...
    if(eWindow == DIP_WINDOW)
        ...
        DIP_W2BYTEMSK(0, REG_SC_BK3C_0C_L,u16SAV_BW,BIT(6));
