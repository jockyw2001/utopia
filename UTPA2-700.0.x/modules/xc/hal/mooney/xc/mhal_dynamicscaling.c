...
void Hal_SC_ResetSWDSCommand(void *pInstance,SCALER_WIN eWindow, MS_U8 u8DSIndex)
{
    ...
    MS_U32 u32SWRstCmd_Pointer = (BYTE_PER_WORD*(pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext-1))/DS_CMD_LEN_64BITS;
    ...
    if((u32SWRstCmd_Pointer == 0 ) || (u32SWRstCmd_Pointer == 1))
    {
        printf("%s :u16SWRstCmd_Pointer <= 0,error!!!\n", __FUNCTION__);
        MS_ASSERT(u32SWRstCmd_Pointer > 0);
    }
    else
    {
        for(i = 0; i < u16TotalCmdCnt; i++)
        {
            ...
            if(i == 0)
            {
                ...
                PSTXC_DS_CMDCNT.u16CMDCNT_IPM = u32SWRstCmd_Pointer/2;
            }
            else if(i == u32SWRstCmd_Pointer)
            {
            ...
            {
                *pu64Data = 0x00000009FFFF0000; //Dummy cmd BKFF_7F=0x0000
            }
        }
    }
#else
    //MS_PHY u32IndexBase = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr + (MS_U32)u8DSIndex * pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth * MS_MLOAD_MEM_BASE_UNIT;
    MS_PHY u32IndexBase = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr + (MS_U32)u8DSIndex * pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * MS_MLOAD_MEM_BASE_UNIT;

    MS_U32 *pu32Data = NULL;
    int i = 0;

    //XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] u32DSBaseAddr:%lx  u32IndexBase:%ld  u16DSIndexDepth:%d  \n",__func__,__LINE__,pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr,u32IndexBase,pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext);

    //for(i = 0; i < (pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth * MS_MLOAD_MEM_BASE_UNIT)/4; i++) //need check
    for(i = 0; i < (pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * MS_MLOAD_MEM_BASE_UNIT)/4; i++) //need check
    {
        if(i % 8 < 4)
        {
            if(MAIN_WINDOW == eWindow)
...
void Hal_SC_WriteSWDSCommand(void *pInstance,SCALER_WIN eWindow, MS_U32 u32CmdRegAddr, MS_U16 u16CmdRegValue,ds_reg_ip_op_sel IPOP_Sel,ds_reg_source_sel Source_Select,XC_DS_CMDCNT *pstXC_DS_CmdCnt)
{
    ...
    MS_PHY u32Addr = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr + pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex_Ring[eWindow] * pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * MS_MLOAD_MEM_BASE_UNIT;
    MS_PHY u32Addr_OPM = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_OPM + pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex_Ring[eWindow] * pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * MS_MLOAD_MEM_BASE_UNIT;
    MS_PHY u32Addr_IPS = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_IPS + pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex_Ring[eWindow] * pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * MS_MLOAD_MEM_BASE_UNIT;
    MS_PHY u32Addr_OPS = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_OPS + pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex_Ring[eWindow] * pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * MS_MLOAD_MEM_BASE_UNIT;
#else
    MS_PHY u32Addr = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr + pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex_Ring[eWindow] * pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth * MS_MLOAD_MEM_BASE_UNIT;
    MS_PHY u32Addr = pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr + pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex_Ring[eWindow] * pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * MS_MLOAD_MEM_BASE_UNIT;
#endif

    //XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] u32DynamicScalingBaseAddr:%x  CurIdx:%d  IdxDepth:%d  BYTE_PER_WORD:%d  \n",__func__,__LINE__,(int)pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr,(int)pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex[eWindow],(int)pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext,(int)BYTE_PER_WORD);
    ...
    MS_U8 u32MaxCmdCnt = pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * 2;
    ...
    if(Source_Select == DS_XC)
    {
        ...
        if(IPOP_Sel == DS_IP)
        {
            if(eWindow == MAIN_WINDOW)
            {
                ...
                if(pstXC_DS_CmdCnt->u16CMDCNT_IPM < u32MaxCmdCnt)
                {
                    *pu64Data = u64CmdTemp;
                ...
                {
                    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] IPM Command count overflow !!  u16CMDCNT_IPM:%d  Max command count: %d\n",__func__,__LINE__,pstXC_DS_CmdCnt->u16CMDCNT_IPM,pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext);
                }
            }
            else//subwindow
            {
                ...
                if(pstXC_DS_CmdCnt->u16CMDCNT_IPS < u32MaxCmdCnt)
                {
                    *pu64Data = u64CmdTemp;
                ...
                {
                    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] IPS Command count overflow !!  u16CMDCNT_IPS:%d  Max command count: %d\n",__func__,__LINE__,pstXC_DS_CmdCnt->u16CMDCNT_IPS,pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext);
                }
            }
        }
        else//DS_OP
        {
            if(eWindow == MAIN_WINDOW)
            {
                ...
                if(pstXC_DS_CmdCnt->u16CMDCNT_OPM < u32MaxCmdCnt)
                {
                    *pu64Data = u64CmdTemp;
                ...
                {
                    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] OPM Command count overflow !!  u16CMDCNT_OPM:%d  Max command count: %d\n",__func__,__LINE__,pstXC_DS_CmdCnt->u16CMDCNT_OPM,pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext);
                }
            }
            else
            {
                ...
                if(pstXC_DS_CmdCnt->u16CMDCNT_OPS < u32MaxCmdCnt)
                {
                    *pu64Data = u64CmdTemp;
                ...
                {
                    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] OPS Command count overflow !!  u16CMDCNT_OPS:%d  Max command count: %d\n",__func__,__LINE__,pstXC_DS_CmdCnt->u16CMDCNT_OPS,pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext);
                }
            }
        }
    }
...
//=============================================//
    ...
    if(IPOP_Sel == DS_OP)
    {
        ...
        MS_U16 u16OPCount = pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext;
    ...
    else
    {
        ...
        MS_U16 u16IPCount = pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext;
...
void Hal_SC_Add_NullCommand(void *pInstance,SCALER_WIN eWindow,ds_reg_ip_op_sel IPOP_Sel,XC_DS_CMDCNT *pstXC_DS_CmdCnt)
{
    ...
    MS_U32 u32MaxCmdCnt = pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * 2; // It has two 64bit commands per MIU bus width
    ...
    if(eWindow == MAIN_WINDOW)
    {
        if(IPOP_Sel == DS_IP)
        {
            ...
            {
                MS_PHY u32IndexBase= pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr + (MS_U32)pXCResourcePrivate->sthal_SC.u8DynamicScalingNextIndex[eWindow] * u32MaxCmdCnt + pstXC_DS_CmdCnt->u16CMDCNT_IPM * DS_CMD_LEN_64BITS;
        ...
        else
        {
            ...
            {
                MS_PHY u32IndexBase= pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_OPM + (MS_U32)pXCResourcePrivate->sthal_SC.u8DynamicScalingNextIndex[eWindow] * u32MaxCmdCnt + pstXC_DS_CmdCnt->u16CMDCNT_OPM * DS_CMD_LEN_64BITS;
                pu64Data  = (MS_U64*)MsOS_PA2KSEG1(u32IndexBase);
                *pu64Data = 0x00000009FFFF0000;
            }
        }
    ...
    else
    {
        if(IPOP_Sel == DS_IP)
        {
            ...
            {
                MS_PHY u32IndexBase= pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_IPS + (MS_U32)pXCResourcePrivate->sthal_SC.u8DynamicScalingNextIndex[eWindow] * u32MaxCmdCnt + pstXC_DS_CmdCnt->u16CMDCNT_IPS * DS_CMD_LEN_64BITS;
                pu64Data  = (MS_U64*)MsOS_PA2KSEG1(u32IndexBase);
                *pu64Data = 0x00000009FFFF0000;
            }
        }
        else
        {
            ...
            {
                MS_PHY u32IndexBase= pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_OPS + (MS_U32)pXCResourcePrivate->sthal_SC.u8DynamicScalingNextIndex[eWindow] * u32MaxCmdCnt + pstXC_DS_CmdCnt->u16CMDCNT_OPS * DS_CMD_LEN_64BITS;
                pu64Data  = (MS_U64*)MsOS_PA2KSEG1(u32IndexBase);
                *pu64Data = 0x00000009FFFF0000;
            }
        }
...
//===============================================================//
    ...
    pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext = u8IdxDepth / (MS_MLOAD_MEM_BASE_UNIT/MS_MLOAD_BUS_WIDTH);

    //pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = BYTE_PER_WORD * u8IdxDepth * DS_MAX_INDEX;
    pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = 0x30000; //For pureSN case, DS DRAM size is 0xCOO
    ...
    pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext = u8IdxDepth;
    pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = BYTE_PER_WORD * u8IdxDepth * DS_MAX_INDEX;
#endif

    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] u32DynamicScalingBaseAddr:%x  u8DynamicScalingIndexDepth:%d u32DynamicScalingBufSize:%d   \n",__func__,__LINE__,(int)pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr,(int)pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext,pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize);
    ...
    if(u16OnOff & 0x0E00)
    {
        if (FALSE == pXCResourcePrivate->sthal_SC.bTrig_pStored)
        {
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_21_L, 0x23, 0xFF);  // ds_trig_dly
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_22_L, 0x1E, 0x0FFF);
            //Hal_XC_MLoad_set_trig_p(pInstance, 0x14, 0x18);
            Hal_XC_MLoad_set_trig_p(pInstance, 0x25, 0x27);

            //add
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_5D_L, 0x1D2A, 0xFFFF);
    ...
    else
    {
        ...
        if (pXCResourcePrivate->sthal_SC.bTrig_pStored)
        {
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_22_L, 0x05, 0x0FFF);
        }
    }
...
MS_BOOL MHal_XC_Is_DSForceIndexEnabled(void *pInstance, SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
...
void Hal_XC_Set_DS_BaseAddress(void *pInstance,MS_PHY u32Base)
{
    ...
    u32Base = u32Base/MS_MLOAD_MEM_BASE_UNIT;
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK1F_0F_L, ((pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_OPM / MS_MLOAD_MEM_BASE_UNIT) >> 16),0xFFFF);
...
void Hal_XC_Set_DS_IndexDepth(void *pInstance)
{
    ...
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK1F_13_L, pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext , 0x00FF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK1F_48_L, ((pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext)&(0xFF00))>>8, 0x00FF);
}
