...
#define IS_OUTPUT_4K2K()   ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 3000) && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 2000))
...
void Hal_SC_set_ficlk(void *pInstance, MS_BOOL bPreDown, SCALER_WIN eWindow)
{
    ...
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
    {
        ...
        if( eWindow == MAIN_WINDOW )
        {
            ...
            {
                W2BYTEMSK(REG_CKG_FICLK_F2, CKG_FICLK_F2_FCLK, CKG_FICLK_F2_MASK); // clk_fclk
            }
        }
        else
        {
...
void Hal_SC_sw_db_burst(void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow )
{
    ...
    {
#ifdef UFO_XC_SUPPORT_DUAL_MIU
        if(!IsEnableDualMode(MAIN_WINDOW) && bEnable)
        {
            ...
            MDrv_MIU_MaskReq(1, MIU_CLIENT_SC_OPMAIN_RW);
            MsOS_DelayTask(1);
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK11_29_L, 0x00, BIT(15));
            MsOS_DelayTask(1);
            MDrv_MIU_UnMaskReq(0, MIU_CLIENT_SC_IPMAIN_RW);
            MDrv_MIU_UnMaskReq(1, MIU_CLIENT_SC_OPMAIN_RW);
            MsOS_DelayTask(1);
            bEnable = FALSE;
        }
#endif
        ...
        if (IsEnableDualMode(MAIN_WINDOW))
        {
            ...
            MDrv_MIU_MaskReq(1, MIU_CLIENT_SC_OPMAIN_RW);
            MsOS_DelayTask(1);
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK11_29_L, BIT(15), BIT(15));
            MsOS_DelayTask(1);
            MDrv_MIU_UnMaskReq(0, MIU_CLIENT_SC_IPMAIN_RW);
            MDrv_MIU_UnMaskReq(1, MIU_CLIENT_SC_OPMAIN_RW);
            MsOS_DelayTask(1);
...
//3.XC
            ...
            if(IsHMirrorMode(MAIN_WINDOW)||IsVMirrorMode(MAIN_WINDOW))
            {
                ...
                H_mirror_offset = (gSrcInfo[MAIN_WINDOW].Status2.u16XCVirtualBoxWidth * gSrcInfo[MAIN_WINDOW].u8BitPerPixel / 8 * MDrv_XC_TransFrameNumToFactor(pInstance,gSrcInfo[MAIN_WINDOW].Status2.eFrameStoreNumber, gSrcInfo[MAIN_WINDOW].bLinearMode, _FIELD_PACKING_MODE_SUPPORTED)) / BYTE_PER_WORD;
...
void Hal_SC_set_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_10_L, BIT(0) | BIT(1), BIT(0) | BIT(1));
    }
#endif
    ...
    if (u8MIUSel==0)
    {
        ...
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK52_05_L, 0x0010, 0x0010);     // reg_miu_select_f2 to MIU0
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK51_05_L, 0x0000, 0x0010);     // reg_miu_select_f2 to MIU0
        }
...
void Hal_SC_get_miusel(void *pInstance, SC_MIUSEL_t* stMIUSel)
{
    ...
    if(SC_R2BYTEMSK(0, REG_SC_BK32_05_L, BIT(4)))
    {
        stMIUSel-> u8SubFBSel = 1;
    }
    else
    {
        if(SC_R2BYTEMSK(0, REG_SC_BK31_05_L, BIT(4)))
        {
...
#else
    if(SC_R2BYTEMSK(0, REG_SC_BK7F_10_L, BIT(0)))
    {
        stMIUSel-> u8MainFBSel = 1;
    }
    else
...
void Hal_SC_set_dual_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    MDrv_WriteByteMask(REG_MIU1_BASE + 0xFB, 0xF7, 0xF7); // IP select, group5
#endif
    // For monet BK52_05 can not work and need not to be set.
    // When MIU select controlled by XC internal, XC dual miu auto adjust BK12_05[4]
    // When MIU select cntrolled by MIU, xc dual miu auto adjust BK7F_10[1..0]
#if 1
    if (u8MIUSel==0)
    {
        ///SCM
...
void Hal_SC_set_disp_window(void *pInstance, SCALER_WIN eWindow, MS_WINDOW_TYPE *pdspwin)
{
    ...
    if (MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        ...
        if(eWindow == MAIN_WINDOW)
        {
            ...
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, pdspwin->y,0xFFF);                  // Display V start
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, pdspwin->y+pdspwin->height-1,0xFFF);  // Display V end
        }
        else
        {
            //Display window
        ...
        if(bEn == TRUE)
        {
            ...
            if ((MDrv_XC_IsSupport2StepScaling() == TRUE) && MDrv_XC_GetMWEStatus(pInstance))
            {
                if (SC_R2BYTEMSK(0, REG_SC_BK4F_10_L, BIT(0)) == BIT(0)) // enable 2-step scaling.
                {
                    ...
                    u32TmpScaling |= 0x1000000L;
                    //H post-scaling
                    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_07_L, (MS_U16)(u32TmpScaling>>00), 0xFFFF);
                    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_08_L, (MS_U16)(u32TmpScaling>>16), 0xFFFF);
                    //V post-scaling
                    u32TmpScaling = pDBreg->u32V_PostScalingRatio & (~0x1000000L);
                    u32TmpScaling = u32TmpScaling/2;
                    u32TmpScaling |= 0x1000000L;
                    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_09_L, (MS_U16)(u32TmpScaling>>00), 0xFFFF);
                    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0A_L, (MS_U16)(u32TmpScaling>>16), 0xFFFF);
                }
            }
#endif
...
void Hal_XC_H3D_LR_Toggle_Enable(void *pInstance, MS_BOOL bEn, MS_BOOL b2DTo3D, MS_BOOL bSkipDefaultLRFlag)
{
    ...
    if(b2DTo3D)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_24_L, BIT(11), BIT(0)|BIT(1)|BIT(11));        //Output 3D L/R selection
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_30_L, (bEn<<8), (BIT(8))); //enable Enable free run 3D LR flag
    }
...
E_APIXC_ReturnValue Hal_XC_IsForcePrescaling(void *pInstance, XC_InternalStatus *pSrcInfo, MS_BOOL *pbForceV, MS_BOOL *pbForceH, SCALER_WIN eWindow)
{
    ...
    *pbForceH = FALSE;
    ...
    if(MDrv_XC_Is_SupportSWDS(pInstance)
        ...
        && (pSrcInfo->Status2.bPreVCusScaling || pSrcInfo->Status2.bPreHCusScaling))
    {
        if(pSrcInfo->Status2.u16PreHCusScalingDst > gSrcInfo[eWindow].Status2.u16XCVirtualBoxWidth)
        {
            *pbForceH = TRUE;
            pSrcInfo->Status2.u16PreHCusScalingDst = gSrcInfo[eWindow].Status2.u16XCVirtualBoxWidth;
        }

        if(pSrcInfo->Status2.u16PreVCusScalingDst > gSrcInfo[eWindow].Status2.u16XCVirtualBoxHeight)
        {
            *pbForceV = TRUE;
            pSrcInfo->Status2.u16PreVCusScalingDst = gSrcInfo[eWindow].Status2.u16XCVirtualBoxHeight;
        }
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"Check force pre-scaling for limitation, even if customer pre-scaling has been set.\n");
    }
...
void MHal_XC_Calc_IGainPGain(void *pInstance, MS_U8 *u8GainI, MS_U8 *u8GainP, MS_U32 u32XTAL_Clock, MS_U8 u8LGain, MS_U8 u8Vco, MS_U16 u16HTotal, MS_U16 u16VTotal, MS_U8 u8FRC_Out)
{
    ...
    *u8GainI = MHal_SC_cal_usedgain_to_reggain(u32Factor);
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "new i_gain=%d\n", u32Factor);
...
MS_BOOL MHal_XC_Get_CMA_UsingCondition(void *pInstance, MS_BOOL bIsGetCMABuff[], MS_U32 au32CMAMemSCMSize[],MS_U32 au32CMAMemFRCMSize[], MS_U32* pu32DualMiuMemSize, MS_U32 u32DataSize, SCALER_WIN eWindow)
{
        ...
        (pXCResourcePrivate->sthal_Optee.op_tee_xc[MAIN_WINDOW].isEnable == TRUE))
    {
        ...
        {
            //DS ON , 4K2K case
            if(gSrcInfo[eWindow].eFBLevel == E_XC_FB_LEVEL_FB)
            {
                // 4K2K input source,prescaling case,run in FB mode
                u32CMAMemSCMSize = XC_CMA_48MB;
            }
            else
            {
                u32CMAMemSCMSize = XC_CMA_18MB;
            }
        }
    }
    else if (IsSrcTypeStorage(gSrcInfo[eWindow].enInputSourceType))
    {
        ...
        {
            if(gSrcInfo[eWindow].u16InputVFreq <= 151)
            {
                u32CMAMemSCMSize = XC_CMA_48MB;
            }
            else
    ...
    {
        if (  ((gSrcInfo[eWindow].enInputSourceType == INPUT_SOURCE_HDMI)||(gSrcInfo[eWindow].enInputSourceType == INPUT_SOURCE_HDMI2)||(gSrcInfo[eWindow].enInputSourceType == INPUT_SOURCE_HDMI3))
    ...
    {
        if (gSrcInfo[eWindow].Status2.stXCConfigCMA[CMA_XC_COBUFF_MEM].u32HeapID == XC_INVALID_HEAP_ID || (IsSrcTypeStorage(gSrcInfo[eWindow].enInputSourceType) && (gSrcInfo[eWindow].bInterlace)) || IsSrcTypeDTV(gSrcInfo[eWindow].enInputSourceType))
        {
            bIsGetCMABuff[CMA_XC_SELF_MEM] = TRUE;
...
E_APIXC_ReturnValue MHal_XC_HDR_GetCaps(void *pInstance, XC_HDR_SUPPORTED_CAPS *pstHDRCaps)
{
    ...
    switch (enHDRType)
    {
        ...
        {
            if (MDrv_AUTH_IPCheck(IPAUTH_DOLBY_HDR_PIN) == TRUE)
            {
                pstHDRCaps->bSupported = TRUE;
            }
            else
...
E_XC_FB_LEVEL MHal_XC_Get_FB_Level(void* pInstance, MS_WINDOW_TYPE* pstCropWin, MS_WINDOW_TYPE* pstDispWin, MS_BOOL bInterlace, MS_U16* pu16HSize, MS_U16* pu16VSize)
{
    ...
    if(E_XC_DEVICE0 == psXCInstPri->u32DeviceID)
    {
        ...
        {
            // Need force pre-scaling if disp.V size < 88% of crop.V size.
            if((pstDispWin->height * 25) < (pstCropWin->height * 22))
            {
                bEnablePreV = TRUE;
            }

            if((pstDispWin->width * 25) < (pstCropWin->width * 22))
            {
                bEnablePreH = TRUE;
            }
        }
...
void Hal_SC_add_dram_to_shm(void *pInstance,SCALER_WIN eWindow,MS_U32 u32address,MS_U32 u32length)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
...
MS_BOOL MHal_XC_CheckHwSupport2StepScaling(void)
{
    return TRUE;
}
#endif
