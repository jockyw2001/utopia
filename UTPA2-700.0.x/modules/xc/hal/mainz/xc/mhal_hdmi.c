...
//
//**************************************************************************
void _Hal_tmds_ResetHDMI20VersionFlag(void)
{
...
//
//**************************************************************************
void _Hal_tmds_ClearSCDCStatusFlag(MS_U8 ucStatusMask)
{
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_GetSCDCStatusFlag(MS_U8 ucStatusMask)
{
    MS_BOOL bStatusFlag = FALSE;
    ...
    if(bStatusFlag)
    {
        _Hal_tmds_ClearSCDCStatusFlag(ucStatusMask);
    }
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_GetHDMI20VersionFlag(void)
{
    MS_BOOL bHDMI20Flag = ((R2BYTE(REG_COMBO_PHY1_P0_30_L) &BIT(3)) ?TRUE: FALSE);
...
//
//**************************************************************************
void _Hal_tmds_HDMI20PHYSetting(MS_BOOL bHDMI20Flag, MS_BOOL bAvaageModeFlag)
{
    ...
    W2BYTEMSK(REG_COMBO_GP_TOP_10_L, bHDMI20Flag? 0: (BIT(3)| BIT(2)| BIT(0)), BMASK(3:0)); // [3]: reg_pix_sd_clk_div2_en; [2]: reg_pix_clk_div2_en; [1]: reg_xtal_clk_div2_en; [0]: reg_tmds_clk_div2_en
    W2BYTEMSK(REG_COMBO_PHY1_P0_7C_L, bHDMI20Flag? (BIT(7)| BIT(6)| BIT(3)): (BIT(10)| BIT(7)| BIT(3)), BMASK(10:0));

    if(!bHDMI20Flag)
    {
        W2BYTEMSK(REG_COMBO_GP_TOP_10_L, bAvaageModeFlag? BIT(3): 0, BIT(3));
    }
}
...
//
//**************************************************************************
void _Hal_tmds_YUV420PHYSetting(MS_BOOL bHDMI20Flag, MS_BOOL bYUV420Flag)
{
    MS_BOOL bSettingFlag = FALSE;

    if((!bHDMI20Flag) || bYUV420Flag)
    {
        bSettingFlag = TRUE;
    }

    W2BYTEMSK(REG_COMBO_PHY0_P0_6B_L, bSettingFlag? BMASK(2:1): BIT(1), BMASK(2:1)); // [2]: enable clko_pix 2x; [1]: reg_atop_en_clko_pix=1

    if(bYUV420Flag || bHDMI20Flag)
    {
        bSettingFlag = TRUE;
    }

    W2BYTEMSK(REG_COMBO_GP_TOP_10_L, bSettingFlag? 0: BIT(3), BIT(3)); // [4]: reg_pix_420t_clk_div2_en_p0;  [3]: reg_pix_sd_clk_div2_en; [2]: reg_pix_clk_div2_en; [1]: reg_xtal_clk_div2_en; [0]: reg_tmds_clk_div2_en
}
...
//
//**************************************************************************
void _Hal_tmds_EQBandWidthSetting(MS_BOOL bHDMI20Flag)
{
...
//
//**************************************************************************
void _Hal_tmds_HDMI20AutoEQSetting(MS_BOOL bHDMI20Flag)
{
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_ReceiveAVIInfoFrame(void)
{
...
//
//**************************************************************************
void _Hal_tmds_SetErrorCountStatus(MS_BOOL bHDMI20Flag)
{
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_GetErrorCountStatus20(void)
{
    MS_BOOL bErrorFlag = FALSE;
...
//
//**************************************************************************
void _Hal_tmds_AutoEQFunctionEnable(MS_BOOL bAutoEQEnable)
{
...
//
//**************************************************************************
MS_U16 _Hal_tmds_GetHDCP22IntStatus(void)
{
...
//                  TRUE or FALSE
//**************************************************************************
MS_BOOL _Hal_tmds_CheckTMDSDataInFlag(void)
{
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_CheckScrambleStatus(void)
{
...
//
//**************************************************************************
void _Hal_tmds_DisableScrambleFlag(MS_BOOL bDisableFlag)
{
...
//
//**************************************************************************
void _Hal_tmds_ClearErrorCheck(void)
{
    W2BYTEMSK(REG_DVI_DTOP_DUAL_P0_63_L, BIT(0), BIT(0));
    W2BYTEMSK(REG_DVI_DTOP_DUAL_P0_63_L, 0, BIT(0));
}
...
{
    MS_BOOL bAutoEQTrigger = FALSE;
    MS_BOOL bAvaageModeFlag = FALSE;
    MS_BOOL bDEStableFlag = _Hal_tmds_GetDEStableFlag(1);
    MS_BOOL bYUV420PortSelect0 = _Hal_tmds_CheckYUV420PortSelect0();
    MS_U16 usClockCount = 0;

    if(!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bDisplayFlag)
    {
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bDisplayFlag = TRUE;
    ...
    if(_Hal_tmds_GetHDMI20VersionFlag()) // Get HDMI 2.0 flag
    {
        if(!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag)
        {
            _Hal_tmds_HDMI20PHYSetting(TRUE, FALSE);
            _Hal_tmds_HDMI20AutoEQSetting(TRUE);
        ...
        {
            _Hal_tmds_EQBandWidthSetting(TRUE);
            ...
            _Hal_tmds_DisableScrambleFlag(FALSE);
            _Hal_tmds_ClearErrorCheck();
        }

        if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucCheckScrambleCounter <= HDMI_CHECK_SCRAMBLE_INTERVAL)
        {
            if(_Hal_tmds_CheckTMDSDataInFlag()) // Check TMDS data in
            {
                if(_Hal_tmds_CheckScrambleStatus())
                {
                ...
                else
                {
                    if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucCheckScrambleCounter == HDMI_CHECK_SCRAMBLE_INTERVAL)
                    {
                        _Hal_tmds_DisableScrambleFlag(TRUE);
    ...
    else // HDMI 1.4 setting
    {
        if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag)
        {
            _Hal_tmds_HDMI20AutoEQSetting(FALSE);
        ...
        if(!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bTimingStableFlag)
        {
            if((usClockCount > 166) && (!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bYUV420Flag))
            {
                bAvaageModeFlag = TRUE;
            }

            _Hal_tmds_HDMI20PHYSetting(FALSE, bAvaageModeFlag);
            _Hal_tmds_EQBandWidthSetting(FALSE);
            _Hal_tmds_DisableScrambleFlag(FALSE);
            ...
            {
                _Hal_tmds_AutoEQFunctionEnable(FALSE);
    ...
    if(bAutoEQTrigger)
    {
        _Hal_tmds_AutoEQFunctionEnable(TRUE);
    ...
    if(bDEStableFlag)
    {
        if(_Hal_tmds_ReceiveAVIInfoFrame())
        {
            if(bYUV420PortSelect0)
            {
                if(!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bYUV420Flag)
                {
                    _Hal_tmds_YUV420PHYSetting(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag, TRUE);
            ...
            else
            {
                if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bYUV420Flag)
                {
                    _Hal_tmds_YUV420PHYSetting(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag, FALSE);
        ...
        if(_Hal_tmds_GetHDMIModeFlag()) // HDMI mode
        {
        ...
        if(!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bIsRepeater) // not Repeater mode
        {
            ...
            else if(_Hal_tmds_GetHDCP22IntStatus() & BIT(1)) // AKE_Init interrupt status: HDCP2.2
            {
        ...
        {
            if(_Hal_tmds_GetSCDCStatusFlag(TMDS_SCDC_ACCESS_ADDERSS_A8))
            {
        ...
        else if(_Hal_tmds_GetSCDCStatusFlag(TMDS_SCDC_ACCESS_ADDERSS_A8))
        {
            ...
            switch(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucAutoEQState)
            {
                ...
                case HDMI_AUTO_EQ_CHECK_STABLE:
                    ...
                    else // DE unstable, trigger auto EQ again and add auto EQ trigger count
                    {
                        if(!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag)
                        {
                            _Hal_tmds_AutoEQ14ModeChange(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucAutoEQ14Mode);
                ...
                case HDMI_AUTO_EQ_14_CLEAR_STATUS:
                    ...
                    _Hal_tmds_SetErrorCountStatus(FALSE);
                ...
                case HDMI_AUTO_EQ_20_CLEAR_STATUS:
                    _Hal_tmds_SetErrorCountStatus(TRUE);
                ...
                case HDMI_AUTO_EQ_20_CHECK_STATUS:
                    ...
                    else if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCheckErrorTimes < TMDS_20_CHECK_ERROR_TIMES)
                    {
                        ...
                        if(_Hal_tmds_GetErrorCountStatus20()) // Get error
                        {
                            bAutoEQTrigger = TRUE;
...
//
//**************************************************************************
void _Hal_tmds_PostEQSetting(void)
{
#if(TMDS_POST_EQ_FUNCTION_ENABLE)
    W2BYTEMSK(REG_COMBO_PHY0_P0_2A_L, 0x4F, BMASK(15:0));
    //W2BYTEMSK(REG_COMBO_PHY1_P0_50_L, 0xC8, BMASK(15:0));
    W2BYTEMSK(REG_COMBO_PHY1_P0_51_L, 0xC8, BMASK(15:0));
    W2BYTEMSK(REG_COMBO_PHY1_P0_53_L, BIT(15)| BIT(14)| (1 << 7)| (1 << 4)| 3, BIT(15)| BIT(14)| BIT(11)| BMASK(9:7)| BMASK(6:4)| BMASK(3:0));
    W2BYTEMSK(REG_COMBO_PHY1_P0_54_L, (10 << 10), BMASK(14:10));
#else
    W2BYTEMSK(REG_COMBO_PHY1_P0_53_L, 0, BIT(15));
...
void Hal_HDMI_init(void *pInstance)// void )
{
   ...
   // PM_W2BYTE(REG_PM_SLEEP_4A_L, 0xF000, 0xF000); // power off DVI off-line detection clock
    ...
    W2BYTEMSK(REG_COMBO_PHY0_P0_74_L, (HDMI_EQ_14_SETTING_VALUE << 8)| HDMI_EQ_14_SETTING_VALUE, 0x1F1F);
    W2BYTEMSK(REG_COMBO_PHY0_P0_73_L, (HDMI_EQ_14_SETTING_VALUE << 8)| HDMI_EQ_14_SETTING_VALUE, 0x1F1F);
    ...
    _Hal_tmds_AutoEQInitialSetting();
    _Hal_tmds_HDMI20AutoEQSetting(FALSE);
    _Hal_tmds_ClearSCDCStatusFlag(TMDS_SCDC_ACCESS_ADDERSS_A8);
    for(port_index = INPUT_PORT_DVI0; port_index <= INPUT_PORT_DVI3; port_index++)
    {
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(port_index)].bNoInputFlag = TRUE;
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(port_index)].bHDMIModeFlag = FALSE;
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(port_index)].bDisplayFlag = FALSE;
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(port_index)].bTimingStableFlag = FALSE;
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(port_index)].bAutoEQRetrigger = FALSE;
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(port_index)].ucSourceVersion = HDMI_SOURCE_VERSION_NOT_SURE;
        stHDMIPollingInfo[HDMI_GET_PORT_SELECT(port_index)].usClockCount = 0;
    }
    ...
    _Hal_tmds_HDMI20PHYSetting(FALSE, FALSE); // Always set 2P mode
...
void Hal_HDMI_StablePolling(void)
{
    ...
    for(enPortIndex = INPUT_PORT_DVI0; enPortIndex <= INPUT_PORT_DVI_END; enPortIndex++)
    {
        ...
        else if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enPortIndex)].bDisplayFlag) // Off line port
        {
            stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enPortIndex)].bDisplayFlag = FALSE;
        }
    }
...
{
    return stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucSourceVersion;
}
...
MS_BOOL Hal_HDCP_WriteX74(E_MUX_INPUTPORT enInputPortType, MS_U8 ubOffset, MS_U8 ubData)
{
    ...
    HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_17_L, ubOffset, BMASK(9:0)); // address
    HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_19_L, BIT(5), BIT(5)); // trigger latch address

    HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_18_L, ubData, BMASK(7:0)); // data
    HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_19_L, BIT(4), BIT(4)); // trigger latch data
...
MS_U8 Hal_HDCP_ReadX74(E_MUX_INPUTPORT enInputPortType, MS_U8 ubOffset)
{
    ...
    HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_17_L, BIT(10), BIT(10)); // [10]: HDCP enable for DDC
    HDCP_W2BYTEMSK(REG_HDCP_DUAL_P0_19_L, 0, BIT(15)|BIT(14)); // [15]: CPU write enable, [14]: 0: 74 RAM, 1 :HDCP RAM
    ...
    while(R2BYTE(REG_HDCP_DUAL_P0_19_L) & BIT(6)); // wait read ready

    usRData = HDCP_R2BYTEMSK(REG_HDCP_DUAL_P0_18_L, BMASK(15:0)); // read data
...
void Hal_HDCP_WriteKSVList(E_MUX_INPUTPORT enInputPortType, MS_U8* pucData, MS_U32 ulDataLen)
{
	...
	W2BYTEMSK(REG_COMBO_GP_TOP_40_L, BIT(2)| BIT(0), BMASK(2:0)); //enable XIU write
    W2BYTEMSK(REG_HDCPKEY_04_L, 0x00, 0x7FF); //set address
	...
	W2BYTEMSK(REG_HDCP_DUAL_P0_63_L, ulDataLen, 0x7FF); // write length

	W2BYTEMSK(REG_HDCP_DUAL_P0_64_L, BIT(15), BIT(15)); // address pulse
    W2BYTEMSK(REG_HDCP_DUAL_P0_65_L, BIT(15) | BIT(14), BIT(15) | BIT(14));
...
void Hal_HDCP_ClearStatus(E_MUX_INPUTPORT enInputPortType, MS_U16 usInt)
{
    ...
    HDCP_W2BYTE(REG_HDCP_DUAL_P0_01_L, usInt << 8);
}
