    ...
    else
    {
        ...
        switch (PortId)
        {
            case HAL_DVI_IP_A: // DVI0 (port A)
				...
				W2BYTEMSK(REG_PM_SLEEP_4D_L, BIT(0), BMASK(3:0)); // MHL3 ATOP Port-0 enable
				W2BYTEMSK(REG_PM_SLEEP_4D_L, 0, BMASK(5:4)); // reg_hdcp_scdc_sel => DDCDA
				W2BYTEMSK(REG_PM_SLEEP_39_L, 0, BMASK(15:14)); // 5V detect mux select port A
				W2BYTEMSK(REG_PM_MISC_62_L, 0, BMASK(1:0)); // switch EJTAG PAD MUX = HDMI
            ...
            case HAL_DVI_IP_B: // DVI1 (port B)
				...
				W2BYTEMSK(REG_PM_SLEEP_4D_L, BIT(1), BMASK(3:0)); // MHL3 ATOP Port-1 enable
				W2BYTEMSK(REG_PM_SLEEP_4D_L, BIT(4), BMASK(5:4)); // reg_hdcp_scdc_sel => DDCDB
				W2BYTEMSK(REG_PM_SLEEP_39_L, BIT(14), BMASK(15:14)); // 5V detect mux select port B
				W2BYTEMSK(REG_PM_MISC_62_L, 0, BMASK(1:0)); // switch EJTAG PAD MUX = HDMI
            ...
            case HAL_DVI_IP_C: // DVI2 (port C)
				...
				W2BYTEMSK(REG_PM_SLEEP_4D_L, BIT(2), BMASK(3:0)); // MHL3 ATOP Port-2 enable
				W2BYTEMSK(REG_PM_SLEEP_4D_L, BIT(5), BMASK(5:4)); // reg_hdcp_scdc_sel => DDCDC
				W2BYTEMSK(REG_PM_SLEEP_39_L, BIT(15), BMASK(15:14)); // 5V detect mux select port C
				W2BYTEMSK(REG_PM_MISC_62_L, 0, BMASK(1:0)); // switch EJTAG PAD MUX = HDMI
            ...
            case HAL_DVI_IP_D: // DVI3 (port D)
				...
				W2BYTEMSK(REG_PM_SLEEP_4D_L, BIT(3), BMASK(3:0)); // MHL3 ATOP Port-3 enable
				W2BYTEMSK(REG_PM_SLEEP_4D_L, BMASK(5:4), BMASK(5:4)); // reg_hdcp_scdc_sel => DDCDD
				W2BYTEMSK(REG_PM_SLEEP_39_L, BMASK(15:14), BMASK(15:14)); // 5V detect mux select port D
				W2BYTEMSK(REG_PM_MISC_62_L, 0, BMASK(1:0)); // switch EJTAG PAD MUX = HDMI
            ...
            default: // turn off all DVI path
				...
				W2BYTEMSK(REG_PM_SLEEP_4D_L, 0, BMASK(3:0)); // MHL3 ATOP Port-0/1/2/3 disable
...
void Hal_SC_EnableCLK_for_DIP(void *pInstance, MS_BOOL bEnable)
{
    // Enable DIP window clock
     ...
     {
         // Disable IDClk3
         MDrv_WriteRegBit(REG_CKG_PRE_IDCLK, ENABLE, CKG_PRE_IDCLK_INVERT);                   // Not Invert
         MDrv_WriteRegBit(REG_CKG_PRE_IDCLK, ENABLE, CKG_PRE_IDCLK_GATED);                    // Enable clock
     }
