...
void Hal_SC_set_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_11_L, BIT(0)| BIT(1)| BIT(7))==0)
    {
        ...
        if (u8MIUSel == 0)
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_10_L, 0, BIT(0) | BIT(1)| BIT(7)|BIT(10));
        }
        else
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_10_L, BIT(0) | BIT(1)| BIT(7), BIT(0) | BIT(1)| BIT(7));
        }
...
void Hal_SC_get_miusel(void *pInstance, SC_MIUSEL_t* stMIUSel)
{
    ...
    if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_11_L, BIT(0)| BIT(1)| BIT(7))==0)
    {
        if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_10_L,  BIT(0) | BIT(1)| BIT(7)) == BIT(10))
        {
            stMIUSel-> u8MainFBSel = 0;
            stMIUSel-> u8SubFBSel = 0;
        }

        if(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_10_L, BIT(0) | BIT(1)| BIT(7)) == (BIT(0) | BIT(1)| BIT(7)|BIT(10)))
        {
...
E_APIXC_ReturnValue MHal_XC_HDR_GetCaps(EN_XC_HDR_TYPE enHDRType, MS_BOOL *pbSupported)
{
    switch (enHDRType)
    {
...
E_XC_FB_LEVEL MHal_XC_Get_FB_Level(MS_WINDOW_TYPE* pstCropWin, MS_WINDOW_TYPE* pstDispWin, MS_BOOL bInterlace, MS_U16* pu16HSize, MS_U16* pu16VSize)
{
    ...
    // Vertical
    //--------------------------------
    if((pstCropWin->height > 2000) && (pstCropWin->width > 1900))
    {
        ret = E_XC_FB_LEVEL_FBL;
        // Need force pre-scaling if disp.V size < 70% of crop.V size.
        if(((pstDispWin->height * 100) < (pstCropWin->height * 30)) || ((pstDispWin->width * 100) < (pstCropWin->width * 40)))
        {
