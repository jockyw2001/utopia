...
#define new_chakra
#define DIP_CAP_IP_DISCRETE_PATCH
...
#ifdef DIP_CAP_IP_DISCRETE_PATCH
            if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
            {
                SC_W2BYTEMSK(0, REG_SC_BK34_50_L, BIT(15),BIT(15));
                //reg_4k_h_size
                SC_W2BYTEMSK(0, REG_SC_BK34_6D_L, (u16MaxWith+1),BMASK(11:0));
                SC_W2BYTEMSK(0, REG_SC_BK34_6D_L, BIT(15),BIT(15));
                SC_W2BYTEMSK(0, REG_SC_BK34_6C_L, BIT(13),BIT(13));
            }
            else
            {
                SC_W2BYTEMSK(0, REG_SC_BK34_50_L, BIT(15),BIT(15));
                //reg_4k_h_size
                SC_W2BYTEMSK(0, REG_SC_BK34_6D_L, 0,BMASK(11:0));
                SC_W2BYTEMSK(0, REG_SC_BK34_6D_L, 0,BIT(15));
                SC_W2BYTEMSK(0, REG_SC_BK34_6C_L, 0,BIT(13));
            }
#endif
        }
        else
...
void HAL_XC_DIP_Enable(void *pInstance, MS_BOOL bEnable)
{
    if(bEnable)
    {
        ...
        MDrv_WriteByteMask(REG_TZPC_NONPM_DIP, 0, BMASK(1:0));
#endif
    }
    else
    {
#ifdef MSOS_TYPE_OPTEE
        MDrv_WriteByteMask(REG_TZPC_NONPM_DIP, BMASK(1:0), BMASK(1:0));
#endif
