    ...
    else
    {
        for(i = 0; i < u16TotalCmdCnt; i++)
        {
            ...
            if(i == 0)
            {
                ...
                PSTXC_DS_CMDCNT.u16CMDCNT_IPM = u16SWRstCmd_Pointer/(BYTE_PER_WORD/16);
            }
    ...
    if(Source_Select == DS_XC)
    {
        ...
        if(IPOP_Sel == DS_IP)
        {
            if(eWindow == MAIN_WINDOW)
            {
                ...
                //XC_PRINTF("[%s,%5d] [DS_IP]*pu64Data:%llx u32Addr:%lx \n",__func__,__LINE__,*pu64Data,u32Addr);
                }
                else
                {
...
//===============================================================//
    ...
    pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = 0xC00; //For pureSN and TVOS case, DS DRAM size is 0xC00
    ...
    if(u16OnOff & 0x0E00)
    {
        if (FALSE == pXCResourcePrivate->sthal_SC.bTrig_pStored)
        {
            ...
            // set the trigger point from delayed line,  DS: 0x09, Train:0x14, Disp:0x18
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_21_L, 0x09, 0xFF);  // ds_trig_dly
...
MS_BOOL MHAL_SC_Get_DynamicScaling_Status(void *pInstance)
{
    ...
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    return (pXCResourcePrivate->sthal_SC.bDynamicScalingEnable || pXCResourcePrivate->stdrvXC_Dynamicscaling._bDSForceIndexEnable[MAIN_WINDOW]) ;
}
