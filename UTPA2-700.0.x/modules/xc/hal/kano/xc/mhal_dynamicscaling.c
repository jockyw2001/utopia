...
//===============================================================//
    ...
    if(u16OnOff & 0x0E00)
    {
        ...
        SC_W2BYTE(0, REG_SC_BK00_03_L, 0x0000);
    }
    else
    {
        SC_W2BYTE(0, REG_SC_BK20_19_L, 0x30F8);
        SC_W2BYTE(0, REG_SC_BK00_03_L, 0x1000);
    }

    if((u16OnOff & 0x0E00)
        && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height > 2000)
        && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 3200))
    {
        //4K2K output timing case, need refine trigger point (all trigger points +18)
        //(Kano can not use K6 patch, Need HW to check)
        SC_W2BYTEMSK(0, REG_SC_BK20_1A_L, 0x25, 0x0FFF);
        SC_W2BYTEMSK(0, REG_SC_BK20_1B_L, 0x27, 0x0FFF);
        SC_W2BYTE(0, REG_SC_BK10_5D_L, 0x1D2A);
        SC_W2BYTEMSK(0, REG_SC_BK20_21_L, 0x23, 0xFF);
        SC_W2BYTEMSK(0, REG_SC_BK20_22_L, 0x1E, 0xFF);
    }
    else
    {
        SC_W2BYTEMSK(0, REG_SC_BK20_1A_L, 0x13, 0x0FFF);
        SC_W2BYTEMSK(0, REG_SC_BK20_1B_L, 0x15,  0x0FFF);
        SC_W2BYTE(0, REG_SC_BK10_5D_L, 0x0B18);
        SC_W2BYTEMSK(0, REG_SC_BK20_21_L, 0x11, 0xFF);
        SC_W2BYTEMSK(0, REG_SC_BK20_22_L, 0x0C, 0xFF);
    }
