...
#include "Maserati_2D_4K2K.c"
#include "Maserati_2D_FHD.c"
#include "Maserati_2D_720.c"
#include "Maserati_2D_576.c"
#include "Maserati_2D_480.c"
#include "Maserati_ACT_4K1K.c"
#include "Maserati_ACT_4K0_5K.c"

#include "Maserati_FRC_PAS_4K2K_60.c"
#include "Maserati_FRC_PAS_4K2K_120.c"
#include "Maserati_FRC_ACT_4K2K_120.c"
#include "Maserati_FRC_ACT_4K1K_120.c"
#include "Maserati_FRC_ACT_4K0_5K_LLRR_240.c"
#include "Maserati_FRC_ACT_4K1K_LLRR_240.c"
...
void MHal_FRC_SetYTrig(void *pInstance, MS_U16 u16Trig1, MS_U16 u16Trig2)
{
    ...
    {
        //HW RD suggest that FO does not adjust the trigger value.
    }
    else
    {
...
// FRC MIU Select
//======================================================================
static SC_MIUMASK_t MHal_FRC_get_miu2mask(void)
{
    SC_MIUMASK_t mask;
...
static void MHal_FRC_set_miu2mask(SC_MIUMASK_t mask)
{
...
void MHal_FRC_Enable_MiuMask(void *pInstance)
{
    ...
    pXCResourcePrivate->sthal_FRC.FRC_Miu0MaskOld = MHal_FRC_get_miu2mask(); // FRC not support MIU0, use MIU2 to be FRC_Miu0MaskOld
    ...
    MHal_FRC_set_miu2mask(pXCResourcePrivate->sthal_FRC.FRC_Miu0Mask); // FRC not support MIU0, use MIU2 to be FRC_Miu0Mask
...
void MHal_FRC_IPM_SetBaseAddr(void *pInstance, MS_PHY addr, MS_U32 u32FrameYcout, MS_U16 u16FB_YcountLinePitch, MS_U8 u8MirrorMode )
{
    ...
    if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bIsAddrUpdated)
    {
        _phy_to_miu_offset(u8MIUSel, addr, pXCResourcePrivate->stdrvXC_MVideo_Context.g_stXC_FRC_Address.u64FRC_IPM_L_MemAddr);
    }
    else
    {
    ...
    // FRC only support MIU1 and MIU2
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, BIT(2), BIT(2));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, BIT(2));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, BIT(2));
        W2BYTEMSK(REG_FRC_BK3F_18_L, BIT(2), BIT(2));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
    }

    addr = addr / FRC_BYTE_PER_WORD; // unit: 1/256bit
    W4BYTE(REG_FRC_BK134_08_L, addr);

    if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForFrc)
    {
        u32MemSize = XC_FRC_IPM_L + XC_FRC_IPM_L/10;
    }
    else
    {
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, BIT(0), BIT(0));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, BIT(0));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, BIT(0));
        W2BYTEMSK(REG_FRC_BK3F_18_L, BIT(0), BIT(0));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForFrc)
    {
        u32MemSize = XC_FRC_MEDS_L + XC_FRC_MEDS_L/10;
    }
    else
    {
...
void MHal_FRC_IPM_R_SetBaseAddr(void *pInstance, MS_PHY addr)
{
    ...
    if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bIsAddrUpdated)
    {
        _phy_to_miu_offset(u8MIUSel, addr, pXCResourcePrivate->stdrvXC_MVideo_Context.g_stXC_FRC_Address.u64FRC_IPM_R_MemAddr);
    }
    else
    {
        // remove the MIU offset
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, BIT(3), BIT(3));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, BIT(3));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, BIT(3));
        W2BYTEMSK(REG_FRC_BK3F_18_L, BIT(3), BIT(3));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForFrc)
    {
        u32MemSize = XC_FRC_IPM_R + XC_FRC_IPM_R/10;
    }
    else
    {
        u32MemSize = XC_FRC_IPM_R;
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, BIT(1), BIT(1));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, BIT(1));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, BIT(1));
        W2BYTEMSK(REG_FRC_BK3F_18_L, BIT(1), BIT(1));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForFrc)
    {
        u32MemSize = XC_FRC_MEDS_R + XC_FRC_MEDS_R/10;
    }
    else
    {
        u32MemSize = XC_FRC_MEDS_R;
...
void MHal_FRC_OPM_SetBaseAddr(void *pInstance, MS_PHY addr, MS_U32 u32FrameYcout)
{
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, (BIT(4)|BIT(6)|BIT(8)), (BIT(4)|BIT(6)|BIT(8)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, (BIT(4)|BIT(6)|BIT(8)));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, (BIT(4)|BIT(6)|BIT(8)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, (BIT(4)|BIT(6)|BIT(8)), (BIT(4)|BIT(6)|BIT(8)));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
...
void MHal_FRC_OPM_R_SetBaseAddr(void *pInstance, MS_PHY addr)
{
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, (BIT(5)|BIT(7)|BIT(9)), (BIT(5)|BIT(7)|BIT(9)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, (BIT(5)|BIT(7)|BIT(9)));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, (BIT(5)|BIT(7)|BIT(9)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, (BIT(5)|BIT(7)|BIT(9)), (BIT(5)|BIT(7)|BIT(9)));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
...
void MHal_FRC_ME1_SetBaseAddr(void *pInstance, MS_PHY addr, MS_U32 u32Size)
{
    ...
    if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bIsAddrUpdated)
    {
        _phy_to_miu_offset(u8MIUSel, addr, pXCResourcePrivate->stdrvXC_MVideo_Context.g_stXC_FRC_Address.u64FRC_PQ_MemAddr);
    }
    else
    {
        // remove the MIU offset
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, (BIT(10)|BIT(11)), (BIT(10)|BIT(11)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, (BIT(10)|BIT(11)));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, (BIT(10)|BIT(11)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, (BIT(10)|BIT(11)), (BIT(10)|BIT(11)));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
...
void MHal_FRC_ME2_SetBaseAddr(void *pInstance, MS_PHY addr, MS_U32 u32Size)
{
    ...
    if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bIsAddrUpdated)
    {
        MS_U64 u64ME2Addr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_stXC_FRC_Address.u64FRC_PQ_MemAddr + XC_FRC_ME1_X1_READ + XC_FRC_ME1_X1 + XC_FRC_ME1_S1;
        _phy_to_miu_offset(u8MIUSel, addr, u64ME2Addr);
    }
    else
    {
        // remove the MIU offset
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, (BIT(10)|BIT(11)|BIT(14)), (BIT(10)|BIT(11)|BIT(14)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, (BIT(10)|BIT(11)|BIT(14)));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, (BIT(10)|BIT(11)|BIT(14)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, (BIT(10)|BIT(11)|BIT(14)), (BIT(10)|BIT(11)|BIT(14)));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
...
void MHal_FRC_Halo_SetBaseAddr(void *pInstance, MS_PHY addr, MS_U32 u32Size)
{
    ...
    if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bIsAddrUpdated)
    {
        MS_U64 u64HaloAddr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_stXC_FRC_Address.u64FRC_PQ_MemAddr + XC_FRC_ME1_X1_READ + XC_FRC_ME1_X1 + XC_FRC_ME1_S1 + XC_FRC_ME2_X2 + XC_FRC_ME2_Y2 + XC_FRC_ME2_F2 + XC_FRC_ME2_LOGO + XC_FRC_ME2_S2;
        _phy_to_miu_offset(u8MIUSel, addr, u64HaloAddr);
    }
    else
    {
        // remove the MIU offset
    ...
    if (u8MIUSel == 1)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, (BIT(12)|BIT(13)|BIT(14)), (BIT(12)|BIT(13)|BIT(14)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, 0x00, (BIT(12)|BIT(13)|BIT(14)));
    }
    else if (u8MIUSel == 2)
    {
        W2BYTEMSK(REG_FRC_BK3F_10_L, 0x00, (BIT(12)|BIT(13)|BIT(14)));
        W2BYTEMSK(REG_FRC_BK3F_18_L, (BIT(12)|BIT(13)|BIT(14)), (BIT(12)|BIT(13)|BIT(14)));
    }
    else
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"FRC only support MIU1 and MIU2, wrong u8MIUSel: %d !\n", u8MIUSel);
...
void MHal_FRC_Init(void *pInstance, PMST_PANEL_INFO_t pPanelInfo, PFRC_INFO_t pFRCInfo)
{
    ...
    {
        // Maserati mode
        MHal_FRC_Set_FO_Path(pInstance, E_FRC_FO_PATH_MODE_120HZ);
        MHal_FRC_Set_FO_VOP_H_Size(pInstance, FRC_FO_H_4K_SZIE);
        MHal_FRC_Set_FO_SPTP_Setting(pInstance, FRC_FO_H_4K_SZIE, FALSE);
        // FO TGEN setting
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_7F_L, 0x00, BIT(3)); // disable FO TGEN double buffer first
        //---------------------------------------------------------------
        // For maserati 120Hz case, FO_TGEN always keep as panel size
        MHal_FRC_FO_TGEN_SetVTotal(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VTotal);
        MHal_FRC_FO_TGEN_SetHTotal(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HTotal);
        MHal_FRC_FO_TGEN_SetFdeStartEndY(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1));
        MHal_FRC_FO_TGEN_SetFdeStartEndX(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1));
        MHal_FRC_FO_TGEN_SetMdeStartEndY(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1));
        MHal_FRC_FO_TGEN_SetMdeStartEndX(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1));
        MS_U16 u16AlignHStart = (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart +3) &~3; // Maserati 4P mode, Hstart need to 4 align
        MHal_FRC_FO_TGEN_SetMdeStartEndX_L(pInstance, (u16AlignHStart/4), (u16AlignHStart/4 + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width/4 - 1));
        MHal_FRC_FO_TGEN_SetMdeStartEndX_R(pInstance, (u16AlignHStart/4), (u16AlignHStart/4 + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width/4 - 1));

        MS_U16 u16HSyncStart = 0;
        MS_U16 u16VSyncStart = (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VTotal -
        ...
        MHal_FRC_FO_TGEN_SetHSyncStartEndX(pInstance, u16HSyncStart, (u16HSyncStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.u8PanelHSyncWidth));
        MHal_FRC_FO_TGEN_SetVSyncStartEndY(pInstance, u16VSyncStart, (u16VSyncStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.u8PanelVSyncWidth));

        MHal_FRC_FO_TGEN_SetVTrigY(pInstance, pPanelInfo->u16VTrigY);
        MHal_FRC_FO_TGEN_SetVTrigX(pInstance, pPanelInfo->u16VTrigX);
        ///--------------------------------------------------------------
        MHal_FRC_Set_SC2_PLL_Vsync_Ref(pInstance, E_FRC_SC2_PLL_VS_MODE_FRC);

        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCC_0B_L, 0x00, BIT(13)); // disable FO ip no_signal flag enable, to make no signal case OSD can shown

        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_18_L, 0x0000, 0xFF00); // FO_TGEN test pattern color as black

        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_17_L, BIT(1), BIT(1)); // Force FRC mute by FO_TGEN test pattern, to avoid STR temp green garbage
    }
    else
    {
        // Manhattan mode
        MHal_FRC_Set_FO_Path(pInstance, E_FRC_FO_PATH_MODE_60HZ);
        MHal_FRC_Set_FO_VOP_H_Size(pInstance, FRC_FO_H_4K_SZIE);
        MHal_FRC_Set_FO_2Line_Setting(pInstance, FRC_FO_H_4K_SZIE, FRC_FO_V_2K_SZIE);
        MHal_FRC_Set_FO_OSDDS_Setting(pInstance, ENABLE, ENABLE, DISABLE, ENABLE, DISABLE, ENABLE);
        MHal_FRC_Set_SC2_PLL_Vsync_Ref(pInstance, E_FRC_SC2_PLL_VS_MODE_OP);
    }
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForFrc)
    {
        MDrv_WriteByteMask( REG_FRC_BK13A_32, 0x0B, 0x0F); // ipm meds frame no.
        MDrv_WriteByteMask( REG_FRC_BK13A_B2, 0x0B, 0x0F); // ipm meds frame no.

        //for DDR4 performance
        MDrv_WriteByteMask( REG_FRC_BK134_27, 0x0, 0x80);
        MDrv_WriteByteMask( REG_FRC_BK13A_5A, 0x03, 0x03);
        MDrv_WriteByteMask( REG_FRC_BK13B_5A, 0x03, 0x03);
    }
    else
    {
    ...
    if (gSrcInfo[MAIN_WINDOW].bUseYUVSpace)
    {
        if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForFrc)
        {
            MDrv_WriteByteMask( REG_FRC_BK134_32, 0x0B, 0x0F); // ipm frame no.
            MDrv_WriteByteMask( REG_FRC_BK134_B2, 0x0B, 0x0F); // ipm frame no.

            MDrv_WriteByteMask( REG_FRC_BK22C_82, 0x8B, 0xFF);  // frame cnt = 11 + rw_diff = 8
        }
        else
        {
    ...
    else
    {
        if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForFrc)
        {
            MDrv_WriteByteMask( REG_FRC_BK134_32, 0x08, 0x0F); // ipm frame no.
            MDrv_WriteByteMask( REG_FRC_BK134_B2, 0x08, 0x0F); // ipm frame no.

            MDrv_WriteByteMask( REG_FRC_BK22C_82, 0x58, 0xFF);  // frame cnt = 8 + rw_diff = 5
        }
        else
        {
    ...
    // --- for FRC 120Hz PQ setting
    if (IsVBY1_16LANE(u8LPLL_Type))
    {
        // R2Y setting, before FRC moule (only YUV data to FRC module can use MEMC)
        MDrv_Write2Byte( REG_FRC_BK339_22, 0x01C2);
        MDrv_Write2Byte( REG_FRC_BK339_24, 0x1E67);
        MDrv_Write2Byte( REG_FRC_BK339_26, 0x1FD7);
        MDrv_Write2Byte( REG_FRC_BK339_28, 0x00BB);
        MDrv_Write2Byte( REG_FRC_BK339_2A, 0x0275);
        MDrv_Write2Byte( REG_FRC_BK339_2C, 0x003F);
        MDrv_Write2Byte( REG_FRC_BK339_2E, 0x1F99);
        MDrv_Write2Byte( REG_FRC_BK339_30, 0x1EA6);
        MDrv_Write2Byte( REG_FRC_BK339_32, 0x01C2);

        // Y2R setting, after FRC moule (need to convert data after FRC module from YUV to RGB)
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC0_57_L, BIT(6), BIT(6));
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_26_L, 0x0731);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_27_L, 0x04AC);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_28_L, 0x0000);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_29_L, 0x1DDD);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_2A_L, 0x04AC);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_2B_L, 0x1F25);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_2C_L, 0x0000);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_2D_L, 0x04AC);
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKC1_2E_L, 0x0879);

        //set default on
        if (IsVBY1_16LANE(u8LPLL_Type))
        {
            MDrv_Write2ByteMask( REG_FRC_BK339_20, 0xE010, 0xE010); // enable R2Y
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC1_2F_L, 0x0035, 0x0035); // enable Y2R
        }
    }
...
void MHal_FRC_Set_Pipe_Delay_Value(void *pInstance, MS_U16 u16Hcnt, MS_U16 u16Vcnt)
{
    ...
    MS_U16 u16VTotal = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK68_37_L);
    ...
    {
        MS_U16 u16DiffVcnt = u16Vcnt - (u16FRCTrig2 - u16VsyncEnd)+1;
        //To fix pipe delay, adjust YTrig instead.
        if( (u16FRCTrig2+u16DiffVcnt) < u16VTotal)
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK68_65_L, u16FRCTrig2+u16DiffVcnt-1, 0x3FFF);
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK68_66_L, u16FRCTrig2+u16DiffVcnt, 0x1FFF);
        }
        else
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK68_65_L, u16VTotal-2, 0x3FFF);
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK68_66_L, u16VTotal-1, 0x1FFF);
        }
    }
...
void MHal_FRC_Set_STGEN_ODCLK(void *pInstance, E_FRC_STGEN_ODCLK eMode, MS_BOOL bMenuload)
{
    ...
    else if (eMode == E_FRC_STGEN_ODCLK_N) //Maserati Raptor output 480p/576p is 1-lane output, ODCLK is set div N
    {
        eFSCIDCLK = E_FSC_IDCLK_N;
    }
    ...
    if (ENABLE == bMenuload)
    {
        ...
        //For ODCLK div N setting.
        if(eMode == E_FRC_STGEN_ODCLK_N)
        {
            MDrv_XC_MLoad_WriteCommand_NonXC(pInstance, 0x1420, 0x04, BIT(5), BIT(5));
        }
        else
        {
            MDrv_XC_MLoad_WriteCommand_NonXC(pInstance, 0x1420, 0x04, 0, BIT(5));
        }
    }
    else
    {
        ...
        if(eMode == E_FRC_STGEN_ODCLK_N)
        {
            MDrv_WriteByteMask( REG_FSC_BK20_04, 0x20, 0x20);
        }
        else
        {
            MDrv_WriteByteMask( REG_FSC_BK20_04, 0, 0x20);
        }
    }
}
...
void MHal_FRC_Set_3D_QMap(void *pInstance, MS_U8 u8FRC3DPanelType, MS_U16 u16sc_in_3dformat, MS_U16 u16sc_out_3dformat, MS_U8 u83D_FI_out)
{
    ...
    // if IsVBY1_16LANE (Maserati mode), always load FRC 3D table
        ...
        (psXCInstPri->u32DeviceID != 1))
    {
        ...
        if (!IsVBY1_16LANE(u8LPLL_Type) //manhattan mode 4k1k FA out or 4k0.5k FA_LLRR out
            && (((pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_FRAME_ALTERNATIVE)
                     || (pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_FRAME_ALTERNATIVE_HW))
                  &&(((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width == 3840)
                          && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height == 1080))
                     ||((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width == 3840)
                          && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height == 540)))))
        {
            if(pXCResourcePrivate->stdrvXC_3D._bFALLRRToFA == TRUE)///4k0.5k@240 FA_LLRR out
            {
                MFC_3D_4K0_5K_ACT_3D_4K0_5K_ACT_NO_FRC();
                XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 4k1k load 3D tabel MFC_3D_4K0_5K_ACT_3D_4K0_5K_ACT_NO_FRC \n");
            }
            else  ///4k1k@120 FA out
            {
                MFC_3D_4K1K_ACT_3D_4K1K_ACT_NO_FRC();
                XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## mark ## 4k1k load 3D tabel MFC_3D_4K1K_ACT_3D_4K1K_ACT_NO_FRC \n");
            }
        }
        else
#endif
                             ...
                             pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height)) /// 4k0.5k out
        {
            ...
            else
            {
                ...
                if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width == SCALER_LINE_BUFFER_MAX) //For 4096*2160, need to refine hsize
                {
                    MDrv_WriteByteMask( REG_FSC_BK23_34, 0x00, 0xff); // hsp_hsize_in0
                    MDrv_WriteByteMask( REG_FSC_BK23_35, 0x08, 0xff); // hsp_hsize_in1
                    MDrv_WriteByteMask( REG_FSC_BK23_36, 0x00, 0xff); // hsp_hsize_out0
                    MDrv_WriteByteMask( REG_FSC_BK23_37, 0x08, 0xff); // hsp_hsize_out1
                }
            }
...
#if !defined (MSOS_TYPE_NOS)
        else if(IS_OUTPUT_FP1080P(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, \
                      pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height)) //fp1080p out
        {
            if (gSrcInfo[MAIN_WINDOW].bUseYUVSpace)
            {
                MFC_3D_2D_4K2K_2D_2205_YUV(); // load FRC 3D-table as FP1080P in
                XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## MFC_3D_2D_4K2K_2D_2205_YUV\n");
            }
            else
            {
                MFC_3D_2D_4K2K_2D_2205_RGB_BYPASS();   // for by pass MEMC test
                XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## MFC_3D_2D_4K2K_2D_2205_RGB_BYPASS\n");
            }
            MHal_FRC_Set_FO_2Line_Setting(pInstance, FRC_FO_H_2K_SZIE, FRC_FO_V_1K_SZIE);
        }
#endif
        else if (IS_OUTPUT_720(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, \
                             pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height))// 720 out
        {
            if (gSrcInfo[MAIN_WINDOW].bUseYUVSpace)
            {
                MFC_3D_2D_720_2D_720_YUV(); // load FRC 3D-table as 720 in
            }
            else
            {
                MFC_3D_2D_720_2D_720_RGB_BYPASS();   // for by pass MEMC test
            }
            MHal_FRC_Set_FO_2Line_Setting(pInstance, FRC_FO_H_1280_SZIE, FRC_FO_V_720_SZIE);
        }
        else if (IS_OUTPUT_576(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, \
                             pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height))// 576 out
        {
            if (gSrcInfo[MAIN_WINDOW].bUseYUVSpace)
            {
                MFC_3D_2D_576_2D_576_YUV(); // load FRC 3D-table as 576 in
            }
            else
            {
                MFC_3D_2D_576_2D_576_RGB_BYPASS();   // for by pass MEMC test
            }
            MHal_FRC_Set_FO_2Line_Setting(pInstance, FRC_FO_H_720_SZIE, FRC_FO_V_576_SZIE);
        }
        else if (IS_OUTPUT_480(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, \
                             pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height))// 480 out
        {
            if (gSrcInfo[MAIN_WINDOW].bUseYUVSpace)
            {
                MFC_3D_2D_480_2D_480_YUV(); // load FRC 3D-table as 480 in
            }
            else
            {
                MFC_3D_2D_480_2D_480_RGB_BYPASS();   // for by pass MEMC test
            }
            MHal_FRC_Set_FO_2Line_Setting(pInstance, FRC_FO_H_720_SZIE, FRC_FO_V_480_SZIE);
        }
        else  // FHD out
        {
            if (gSrcInfo[MAIN_WINDOW].bUseYUVSpace)
        ...
        if(pXCResourcePrivate->stdrvXC_3D._bFRC3DEnabled )//FRC 3D enabled, need load 3D table
        {
            if(IS_OUTPUT_4K2K(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, \
                             pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height)// 4K2K out
                && IsVBY1_16LANE(u8LPLL_Type)///120Hz
                ...
                && (pXCResourcePrivate->stdrvXC_3D._eFrcOutput3DMode == E_XC_3D_OUTPUT_FRAME_ALTERNATIVE))//FA out
            {
                ...
                MHal_FRC_Set_FO_2Line_Setting(pInstance, FRC_FO_H_4K_SZIE, FRC_FO_V_1K_SZIE);
                ...
                && (pXCResourcePrivate->stdrvXC_3D._eFrcOutput3DMode == E_XC_3D_OUTPUT_FRAME_ALTERNATIVE))//FA out
            {
                ...
                MHal_FRC_Set_FO_2Line_Setting(pInstance, FRC_FO_H_4K_SZIE, FRC_FO_V_0_5K_SZIE);
                XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 0406 4k0.5k 240Hz  load 3D tabel TB to FA\n");
            }
...
MS_BOOL MHal_FRC_IsFHDToFSC(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if ( pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFSCEnabled &&
        ...
        (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width < SCALER_LINE_BUFFER_MAX) &&
        (psXCInstPri->u32DeviceID != 1) && //sc1 no fsc
        #if 0 //(defined(CONFIG_PURE_SN))
        // For pure-SN, MHEG5 test items, always use FHD to FSC
        ((IsSrcTypeDTV(gSrcInfo[MAIN_WINDOW].enInputSourceType) && ((gSrcInfo[eWindow].stCapWin.width <= 1920) && (gSrcInfo[eWindow].stCapWin.height <= 1080))) || // DTV
          ((((gSrcInfo[eWindow].stCapWin.width <= 1920) && (gSrcInfo[eWindow].stCapWin.height <= 1080)) && ((gSrcInfo[eWindow].stDispWin.width > 1920) && (gSrcInfo[eWindow].stDispWin.height > 1080))) &&  // no DS case, input FHD
          (!((MDrv_XC_Is_DSForceIndexEnabled(pInstance, MAIN_WINDOW))||(MDrv_XC_GetDynamicScalingStatus(pInstance))))))
        #endif
         ((((gSrcInfo[eWindow].stCapWin.width <= 1920) && (gSrcInfo[eWindow].stCapWin.height <= 1080)) &&  // no DS case, input FHD
         (!((MDrv_XC_Is_DSForceIndexEnabled(pInstance, MAIN_WINDOW))||(MDrv_XC_GetDynamicScalingStatus(pInstance))))  )|| // with non DS case
         (IS_DS_FHD(eWindow) &&((MDrv_XC_Is_DSForceIndexEnabled(pInstance, MAIN_WINDOW))||(MDrv_XC_GetDynamicScalingStatus(pInstance))) && IS_SUPPORT_DS_SR(gSrcInfo[eWindow].u16InputVFreq))) && //with DS case
...
MS_BOOL MHal_FRC_IsSupportFRC_byEfuse(void *pInstance)
{
    ...
    // For bring up stage, need to wait for FRC_R2 ready
    // so disable FRC part first
...
MS_BOOL MHal_FRC_Check_Condition(void *pInstance)
{
    ...
    //For Maserati Raptor case, if output size is under FHD, turn off FRC
    if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE)
    {
        if(((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultHTotal < 3000) //Vby1 but non 4K2K out
            && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVTotal < 2000)))
        {
            bSupport = FALSE;
        }
        if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width > 3840) //FRC hw limitation, can't output larger than 3840 point.
        {
            bSupport = FALSE;
        }
        //If input frame rate > outout frame rate, can not support FRC.
        if(gSrcInfo[MAIN_WINDOW].u16InputVFreq > (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq+20))
        {
            bSupport = FALSE;
        }
    }
...
void MHal_FRC_Set_FO_Path(void *pInstance, E_FRC_FO_PATH_MODE eMode)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_02_L, eMode << 8, BIT(8));
    W2BYTEMSK(REG_FRC_BK119_47_L, eMode, 0x0001);
}

void MHal_FRC_Set_FO_VOP_H_Size(void *pInstance, MS_U16 u16HSize)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC1_1B_L, u16HSize, 0x1FFF);
}

void MHal_FRC_Set_FO_SPTP_Setting(void *pInstance, MS_U16 u16HSzie, MS_BOOL bUserMode)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    MS_U16 u16Start = 0;
    MS_U16 u16End = u16Start + u16HSzie/16 -1; // u16HSzie/8/2

    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_40_L, (u16HSzie-1), 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_42_L, bUserMode, BIT(0));

    // f0 start/end
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_44_L, u16Start, 0x3FF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_45_L, u16End, 0x3FF);

    u16Start = u16End +1;
    u16End = u16Start + u16HSzie/16 -1; // u16HSzie/8/2
    // f1 start/end
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_46_L, u16Start, 0x3FF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_47_L, u16End, 0x3FF);

}

void MHal_FRC_Set_FO_2Line_Setting(void *pInstance, MS_U16 u16HSzie, MS_U16 u16VSzie)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);

    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_23_L, u16HSzie, 0x3FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_24_L, u16VSzie, 0x1FFF);
}

void MHal_FRC_Set_FO_OSDDS_Setting(void *pInstance, MS_BOOL bEnBypass, MS_BOOL bEnBypassUser, MS_BOOL bEnOPM, MS_BOOL bEnOPMUser, MS_BOOL bEnMFC, MS_BOOL bEnMFCUser)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);

    MS_U16 u16Setting = 0x00;
    ...
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCA_20_L, u16Setting, (BIT(0)|BIT(3)));
    ...
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCA_30_L, u16Setting, (BIT(0)|BIT(3)));
    ...
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCA_40_L, u16Setting, (BIT(0)|BIT(3)));
}

void MHal_FRC_FO_TGEN_SetVTotal(void *pInstance, MS_U16 u16VTotal)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_01_L, (u16VTotal-1), 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetHTotal(void *pInstance, MS_U16 u16HTotal)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_02_L, (u16HTotal-1), 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetVTrigY(void *pInstance, MS_U16 u16VTrigY)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_03_L, u16VTrigY, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetVTrigX(void *pInstance, MS_U16 u16VTrigX)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_04_L, u16VTrigX, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetVSyncStartEndY(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);

    MS_U16 u16VFDE_Start = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKCB_0A_L);
    MS_U16 u16VFDE_End = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BKCB_0B_L);
    MS_U16 u16VsyncSize = u16End - u16Start;

    // Erorr handling: if Vsync start between FDE_start and FDE_end
    // Make the Vsync start after FDE_end
    if ((u16VFDE_Start <= u16Start) && (u16Start <= u16VFDE_End))
    {
        u16Start = u16VFDE_End + VSYNCSTART_OFFSET;
        u16End = u16Start + u16VsyncSize;
    }

    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_05_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_06_L, u16End, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetHSyncStartEndX(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_08_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_09_L, u16End, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetFdeStartEndY(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_0A_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_0B_L, u16End, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetFdeStartEndX(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_0C_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_0D_L, u16End, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetMdeStartEndY(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_0E_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_0F_L, u16End, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetMdeStartEndX(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_10_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_11_L, u16End, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetMdeStartEndX_L(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_38_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_39_L, u16End, 0x1FFF);
}

void MHal_FRC_FO_TGEN_SetMdeStartEndX_R(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_3A_L, u16Start, 0x1FFF);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_3B_L, u16End, 0x1FFF);
}

void MHal_FRC_Set_SC2_PLL_Vsync_Ref(void *pInstance, E_FRC_SC2_PLL_VS_MODE eMode)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC9_78_L, eMode, (BIT(0)|BIT(1)));
}
...
MS_BOOL Hal_XC_FRC_R2_Init(void *pInstance, MS_U8 u8Panel3DType, MS_U8 u8PanelMaxFreq, MS_U8 u8LocalDimingPanelType, MS_U8 u8ChipRevision)
{
    ...
    {
        // Maserati mode
        u8PanelMaxFreq = u8PanelMaxFreq*2;
    }
...
MS_BOOL Hal_XC_FRC_R2_Set_Timing(void *pInstance, MS_U8 u8InVFreq, MS_U8 u8OutVFreq)
{
    ...
    {
        // Maserati mode
        u8InVFreq = u8OutVFreq;
        u8OutVFreq = u8OutVFreq*2;
    }
