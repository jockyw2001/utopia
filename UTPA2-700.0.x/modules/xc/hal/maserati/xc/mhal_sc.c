...
static void _Hal_SC_fill_main_sw_db_burst(void *pInstance, P_SC_SWDB_INFO pDBreg)
{
    ...
    if (psXCInstPri->u32DeviceID == 1)
    {
        ...
        else
        {
            ...
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, (MS_U16)(pDBreg->u16V_DisStart), VOP_DISPLAY_VSTART_MASK);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, (MS_U16)(pDBreg->u16V_DisEnd), VOP_DISPLAY_VEND_MASK);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_06_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart, 0x1FFF);                                       // DE V start
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height-1, 0x1FFF);  // DE V end
        }
...
//TODO: Before disable Dual miu, need disable XC miu request.
        ...
        if (psXCInstPri->u32DeviceID == 1)
        {
                ...
                )
            {
                ...
                else
                {
                    ...
                    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_06_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart-1, 0x1FFF);                                       // DE V start
                    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height-2, 0x1FFF);  // DE V end
                }
...
extern stDS_PQ_REG DS_UFSC_REG[DS_PQ_MAX_NUM];
extern MS_U16 DS_UFSC_REG_CNT;
extern MS_BOOL bUFSC_DSEn;
#endif
void Hal_SC_SWDS_AddCmd(void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        if(pXCResourcePrivate->sthal_SC.bDynamicScalingEnable)
        {
            ...
            if (IsEnableDualMode(eWindow))
            {
                // SCMI dual miu IPM baseAddress
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_08_L, (MS_U16)(pDBreg->u32DualDNRBase0>>00), DS_IP, DS_XC, &PSTXC_DS_CMDCNT);
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_09_L, (MS_U16)(pDBreg->u32DualDNRBase0>>16), DS_IP, DS_XC, &PSTXC_DS_CMDCNT);
                // SCMI dual miu IPM Offset
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_0E_L, (MS_U16)(pDBreg->u16DualDNROffset), DS_IP, DS_XC, &PSTXC_DS_CMDCNT);
                // SCMI dual miu IPM fetch
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_0F_L, (MS_U16)(pDBreg->u16DualDNRFetch), DS_IP, DS_XC, &PSTXC_DS_CMDCNT);
                // SCMI dual miu IPM fetch total
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK11_28_L, (MS_U16)(pDBreg->u16DualDNRFetch + pDBreg->u16DNRFetch), DS_IP, DS_XC, &PSTXC_DS_CMDCNT);
            }
#endif
#endif
           ...
           //STGEN setting
            ...
            if (IsEnableDualMode(eWindow))
            {
                // SCMI dual miu OPM baseAddress
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_10_L, (MS_U16)(pDBreg->u32DualOPMBase0>>00), DS_OP, DS_XC, &PSTXC_DS_CMDCNT);
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_11_L, (MS_U16)(pDBreg->u32DualOPMBase0>>16), DS_OP, DS_XC, &PSTXC_DS_CMDCNT);

                // SCMI dual miu OPM offset
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_16_L, (MS_U16)(pDBreg->u16DualOPMOffset), DS_OP, DS_XC, &PSTXC_DS_CMDCNT);
                // SCMI dual miu fetch number
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK52_17_L, (MS_U16)(pDBreg->u16DualOPMFetch), DS_OP, DS_XC, &PSTXC_DS_CMDCNT);

                // SCMI dual miu OPM fetch total and dual miu enable.
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK11_29_L, (MS_U16)(BIT(15) | (pDBreg->u16DualOPMFetch + pDBreg->u16OPMFetch)), DS_OP,DS_XC, &PSTXC_DS_CMDCNT);

            }
            else
            {
                Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK11_29_L, 0, DS_OP, DS_XC, &PSTXC_DS_CMDCNT);
            }
#endif
#endif
            ...
            if(bUFSC_DSEn==TRUE)
            {
                for(cnt_x = 0 ;cnt_x < DS_UFSC_REG_CNT; cnt_x++)
                {
                    //printf("\033[1;32m[%s:%d]u16Addr=0x%x,u16Value=0x%x,u16Mask=0x%x\033[m\n",__FUNCTION__,__LINE__,
                    //DS_REG[cnt_x].u16Addr | 0x130000,
                    //DS_REG[cnt_x].u16Value,
                    ...
                    //u16TempRegVal = (u16TempRegVal & ~DS_REG[cnt_x].u16Mask) | DS_REG[cnt_x].u16Value;
                    //printf("\033[1;31m[%s:%d][0x%x]Org=0x%x,Mod=0x%x\033[m\n",__FUNCTION__,__LINE__,DS_REG[cnt_x].u16Addr,SC_R2BYTE(psXCInstPri->u32DeviceID, DS_REG[cnt_x].u16Addr),u16TempRegVal);
                    Hal_SC_WriteSWDSCommand_NonXC(pInstance,eWindow,DS_UFSC_REG[cnt_x].u16Bank,DS_UFSC_REG[cnt_x].u16Addr,DS_UFSC_REG[cnt_x].u16Value,DS_UFSC_REG[cnt_x].u16Mask,DS_OP,&PSTXC_DS_CMDCNT);
                }
                DS_UFSC_REG_CNT = 0;
            }
#endif
...
void Hal_SC_set_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    if (psXCInstPri->u32DeviceID == 0)
    {
        ...
        //ML and DS need controled by BK1F
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_11_L, BIT(9), BIT(9));
    }
    else
    {
...
void Hal_SC_set_dual_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    MDrv_WriteByteMask(REG_MIU1_BASE + 0xFB, 0xF7, 0xF7); // IP select, group5
#endif
    // TODO: dual miu support auto switch miu select for manhattan/maserati.
...
void Hal_SC_set_main_black_screen_burst(void *pInstance, MS_BOOL bEn)
{
    ...
    {
        //For maints issue 1196708
        //If set main window next or previous the sub window will temp black in PIP Mode(120Hz)
        //Get PIP mode staus ON/OFF
        MS_U16 u16ReturnValue = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK68_40_L, BIT(4));
        if(u16ReturnValue)
        {
            //To avoid sub window have temp black
            //For mantis issue 1219876
            //Disenable FRC mute when pip/pop is enable
            MS_U16 u16IsEnableFRCMute = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_17_L, BIT(1));
            if(u16IsEnableFRCMute)
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_19_L, bEn ? BIT(1) : 0, BIT(1));
                MS_U16 u16DelayTimeMs = MHal_XC_GetDelayTime(pInstance,E_DELAY_TIME_FRC);
                MsOS_DelayTask(u16DelayTimeMs);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BKCB_17_L, 0, BIT(1));
            }
            else
            {
        ...
        {
            //PIP/3D mode & FRC_INSIDE mode
            //Step 1 : Set main window muting(REG_SC_BK10_19_L bit 4 set 1)
            //Step 2 : Change to non-pip mode and play video
            //Step 3 : Because REG_SC_BK10_19_L bit 4 set 1, main window will always muting
            //To solve the problem

            //Disable FRC muting step
            //Step 1 : W2BYTE REG_SC_BK10_19_L disable black
            //Step 2 : delay Time
            //Step 3 : MLload REG_SC_BKCB_17_L disable FRC muting
            if(!bEn &&
                ((pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_TOP_BOTTOM_HW)
                ||(pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_TOP_BOTTOM)
                ||(pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_SIDE_BY_SIDE_HALF_HW)
                ||(pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_SIDE_BY_SIDE_HALF)
                ||(pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_FRAME_PACKING)
                ||(pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_LINE_ALTERNATIVE)))///only 3D unmute
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_19_L, bEn ? (BIT(5)|BIT(1)) : 0 ,(BIT(5)|BIT(1)));
            }
            else //PIP mode and chang 2D to 3D mode
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_19_L, bEn ? BIT(1) : 0 ,BIT(1));
            }

            // Wait for OP send signal,
            // otherwise garbage will be showed,we need to add delay time.
            // only unmute need delay
            if(!bEn)
            {
                MS_U16 u16DelayTimeMs = MHal_XC_GetDelayTime(pInstance,E_DELAY_TIME_FRC);
                MsOS_DelayTask(u16DelayTimeMs);
           }
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BKCB_17_L, bEn ? BIT(1) : 0, BIT(1));
        }
    }
    else
#endif
...
void Hal_SC_set_main_black_screen(void *pInstance, MS_BOOL bEn)
{
    ...
    if (IsVBY1_16LANE(u8LPLL_Type))
    {
        ...
        else
        {
            ...
            //Step 3 : Because REG_SC_BK10_19_L bit 4 set 1, main window will always muting
            //To solve the problem
            //Disable XC main window avoid main window always muting
            if(!bEn &&
                ((pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_TOP_BOTTOM_HW)
                ||(pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_TOP_BOTTOM)
                ||(pXCResourcePrivate->stdrvXC_3D._eOutput3DMode == E_XC_3D_OUTPUT_SIDE_BY_SIDE_HALF_HW)
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_17_L, bEn ? BIT(1) : 0, BIT(1));
        }
    }
    else
#endif
...
void Hal_SC_set_sub_blue_screen_burst(void *pInstance, MS_BOOL bEn, MS_BOOL bBlue)
{
    ...
    _MLOAD_ENTRY(pInstance);
#if (FRC_INSIDE && (HW_DESIGN_4K2K_VER == 6))
    MS_U16 u16ReturnValue = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_17_L, BIT(1));
    if(u16ReturnValue)
    {
        //Disable FRC muting step
        //Step 1 : W2BYTE REG_SC_BK10_19_L disable black
...
void Hal_SC_set_nosignal_color(void *pInstance, MS_U8 u8Color,SCALER_WIN eWindow)
{
    ...
    if (MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
...
#endif
        {
            ...
            if (IsVBY1_16LANE(u8LPLL_Type))
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BKCB_18_L, ((MS_U16)u8Color)<<8, HBMASK);
            }
            #endif
        }
        else
    ...
    else
    {
...
#endif
        {
            ...
            if (IsVBY1_16LANE(u8LPLL_Type))
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_18_L, ((MS_U16)u8Color)<<8, HBMASK);
            }
            #endif
        }
        else
...
void HAL_SC_Set_FPLL_Limit(void *pInstance, MS_U32 *u32PllSet, MS_U32 u32LowBound, MS_U32 u32UpBound, MS_BOOL _bInFPLLDbgMode, MS_U32 _U32LimitD5D6D7)
{
    ...
    else
    {
        ...
        if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE)
        {
            u32LpllLimitHigh = 0x1600;
            u32LpllLimitLow = 0x1600;
        }
#endif
...
void MHal_CLKGEN_FRC_Init(void *pInstance, MS_U8 u8LPLL_Mode)
{
    ...
    {
        // Maserati mode
        W2BYTEMSK(REG_FRC_BK119_03_L, 0x0000, 0x0D00);
        W2BYTEMSK(REG_FRC_BK121_03_L, 0x0000, 0xD000);

        W2BYTEMSK(L_CLKGEN2(0x44), 0x0000, 0x0C00); // 354MHz
...
MS_BOOL MHal_XC_Init_Patch(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    MDrv_WriteRegBit(REG_CLKGEN2_BASE + 0x87, TRUE, BIT(1));//FIXME:patch for PIP
...
MS_BOOL Hal_SC_Init(void *pInstance)
{
    ...
    MHal_FRC_Set_Composer_User_Mode(pInstance, E_FRC_COMPOSER_SOURCE_MODE_FRC, FALSE); //Default init FRC.
...
MS_BOOL MHal_XC_DolbySWBonded(void *pInstance)
{
    MS_U16 u16OldVal = 0;
    MS_U16 u16Val = 0;
    MS_U32 u32Count = 0;
    ...
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_28_L, 0x60<<2, 0x1FFF);
...
MS_BOOL Hal_SC_CheckMuteStatusByRegister(void *pInstance, SCALER_WIN eWindow)
{
    ...
    else if(MAIN_WINDOW == eWindow)
    {
        ...
        if (IsVBY1_16LANE(u8LPLL_Type))
        {
            bRet = (SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_19_L, BIT(1)) ? TRUE: FALSE)||(SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKCB_17_L, BIT(1)) ? TRUE: FALSE);
        }
        else
        {
...
MS_BOOL Hal_SC_secure_lock(void *pInstance, SCALER_WIN eWindow, MS_U32 u32SecureDMA, MS_U32 u32OperationMode)
{
    ...
    pXCResourcePrivate->sthal_Optee.op_tee_xc[eWindow].isEnable=u32OperationMode; //Update OPtee status for tee side
    #endif
    return TRUE;
}


MS_BOOL Hal_SC_secure_Check_RBase(void *pInstance)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    ...
    MS_U32 u32HW_OPM_BaseAddr;
    u32HW_OPM_BaseAddr = (SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_10_L) & 0xFFFFFFFF) * BYTE_PER_WORD;

    MS_U32 u32HW_IPM_BaseAddr,u32HW_IMP_MaxLimitAddress,u32HW_OPM_MaxLimitAddress;
    MS_U32 u32Frame_H = (SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_16_L) & 0x00000FFF);
    MS_U32 u32Frame_L = (SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK20_15_L) & 0xFFFFFFFF);
    MS_U32 u32FBnum = (SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_19_L) & 0x00000003);
    u32HW_IPM_BaseAddr = (SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_08_L) & 0xFFFFFFFF) * BYTE_PER_WORD;
    u32HW_IMP_MaxLimitAddress = (SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK0D_42_L) & 0xFFFFFFFF) * BYTE_PER_WORD;
    u32HW_OPM_MaxLimitAddress = u32HW_OPM_BaseAddr + u32Frame_H * u32Frame_L * 3 * (u32FBnum+1);
    MS_U32 u32IPMSize = u32HW_IMP_MaxLimitAddress - u32HW_IPM_BaseAddr;
    MS_U32 u32OPMSize = u32Frame_H * u32Frame_L * 3 * (u32FBnum+1);

    //Check OPM Addr is in legal range
    if ((u32HW_IPM_BaseAddr <= u32HW_OPM_BaseAddr && u32HW_OPM_BaseAddr <= u32HW_IMP_MaxLimitAddress) && u32HW_OPM_MaxLimitAddress <= u32HW_IMP_MaxLimitAddress && u32OPMSize<=u32IPMSize)
    {
        return TRUE;
    }
    else
...
E_APIXC_ReturnValue MHal_XC_Vtrack_SetPayloadData(void *pInstance, MS_U16 u16Timecode, MS_U8 u8OperatorID)
{
    ...
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_08_L, (MS_U16)u8OperatorID, 0x00ff);
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_0A_L, (u16Timecode << 8) , 0xff00);
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_0B_L, (u16Timecode >> 8) , 0x00ff);
    }
...
E_APIXC_ReturnValue MHal_XC_Vtrack_Enable(void *pInstance, MS_U8 u8FrameRate, MS_BOOL bEnable)
{
    ...
    if (bEnable == TRUE)
    {
        //Vtrack_h_pol_inv: [8] => 1
        //Vtrack_v_pol_inv: [9] => 1
        u16Setting |= (BIT(8)|BIT(9));

        //EnableIn: [5:4][0] => 1
        //DebugEn: [1]        => 0
        u16Setting |= (BIT(5)|BIT(4)|BIT(0));
    }
    ...
    if(MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        ...
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK2F_01_L , u16Setting, (BIT(9)|BIT(8)|BIT(5)|BIT(4)|BIT(1)|BIT(0)));
    ...
    {
        //FrameRateIn     => 8'h1E
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_02_L , u8FrameRate, 0xFF);

        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK2F_01_L , u16Setting, (BIT(9)|BIT(8)|BIT(5)|BIT(4)|BIT(1)|BIT(0)));
    }
...
MS_U16 MHal_XC_GetDelayTime(void* pInstance,XC_DELAY_TIME_STATUS enStatus)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    MS_U16 u16DelayTimeMS = 0;
    switch(enStatus)
    {
        case E_DELAY_TIME_FRC:
            if(pXCResourcePrivate->stdrvXC_MVideo._SCShared.StatusInfo[psXCInstPri->u32DeviceID].u16InputVFreq > 0)
            {
                u16DelayTimeMS = 5 * ((10000 + pXCResourcePrivate->stdrvXC_MVideo._SCShared.StatusInfo[psXCInstPri->u32DeviceID].u16InputVFreq/2))/pXCResourcePrivate->stdrvXC_MVideo._SCShared.StatusInfo[psXCInstPri->u32DeviceID].u16InputVFreq;
            }
            else
            {
                u16DelayTimeMS = 0;
