...
void Hal_ADC_init(void *pInstance,MS_U16 u16XTAL_CLK,MS_BOOL IsShareGrd,  MS_U16 eScartIDPortSelection)
{
    ...
    // 1.ADCX be open when load ADC init table
    // 2.ADCX mux != main/sub input mux
    // if some project does not use ADCX and ADCX Mux hw initial value is 0 == main input mux,
    // then main will be something wrong
    // so we change ADCX mux to unoccupied mux ( =4, this value from designer )
    W2BYTEMSK(REG_ADC_ATOP_03_L, 0x4000, 0xF000);
    ...
    if(!Hal_ADC_SetCalibrationRefSource(pInstance, ADC_REFERENCE_VOLATAGE_BANDGAP)) MHAL_ADC_DBG(printf("Set ADC Cal Ref to BG fail!!\n"););
}
...
#ifdef STELLAR
    if ((pXCResourcePrivate->stdrvXC_MVideo_Context.u32CusMISC_A & E_XC_S_DISPLAY_INITMISC_A_ENABLE_CVBSOX_ADCX))
    {
...
MS_BOOL Hal_ADC_SetCalibrationRefSource(void *pInstance,MS_U16 type)
{
    ...
    switch(type)
    {
        ...
        case ADC_REFERENCE_VOLATAGE_BANDGAP:
            {
                ///MS_U8 efuseBandgap = 0;
                _bUseBandgap = R2BYTEMSK(REG_ADC_ATOP_5B_L, BIT(13))? TRUE : FALSE;
            #if 0
                if(Hal_ADC_GetBandgapTrimData(pInstance,&efuseBandgap))
                {
                    _bUseBandgap = TRUE;
