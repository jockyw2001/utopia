...
#include "Manhattan_pnl_lpll_tbl.h"
#include "Manhattan_pnl_lpll_ext_tbl.h"
...
MS_U8 MHal_MOD_PowerOn(void *pInstance, MS_BOOL bEn, MS_U8 u8LPLL_Type,MS_U8 DualModeType, MS_U16 u16OutputCFG0_7, MS_U16 u16OutputCFG8_15, MS_U16 u16OutputCFG16_21)
{
    ...
    else
    {
        ...
        if((u8LPLL_Type>=E_PNL_LPLL_VBY1_10BIT_4LANE)&&(u8LPLL_Type<=E_PNL_LPLL_VBY1_8BIT_8LANE))
        {
                               ...
                               MS_U16 u16OutputOrder12_13)
{
    ...
    else
    {
        ...
        {
            MOD_W2BYTE(REG_MOD_BK00_08_L, 0x76DC);
            MOD_W2BYTE(REG_MOD_BK00_09_L, 0xBA98);
            MOD_W2BYTE(REG_MOD_BK00_0A_L, 0x3210);
            MOD_W2BYTE(REG_MOD_BK00_0B_L, 0x0054);
        }
    }
...
void Hal_PNL_Set12BitGammaPerChannel(void *pInstance, MS_U8 u8Channel, MS_U8 * u8Tab, DRVPNL_GAMMA_MAPPEING_MODE GammaMapMode)
{
    MS_U16 i , j ;
    MS_U16 u16Addr             = 0;
    MS_U16 u16GammaValueR[4]  = {0, 0, 0, 0};
    MS_U16 u16GammaDeltaR[3]  = {0, 0, 0};
    MS_U16 u16MaxGammaValue1  = 0;
    ...
    bUsingBurstWrite = bUsingBurstWrite;

    if (u8Channel >= 3 )
        return;

    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_71_L, 0x00 , 0x00FF);  //Initial  write address, GAMMA_ADR[7:0]
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_72_L, 0x00, 0xFFFF);   // Initial GAMMA_DATA[26:0]
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_73_L, 0x00, 0x07FF);   // Initial GAMMA_DATA[26:0]

    //Select write channel, GAMMA_CH_SEL[1:0] .
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_70_L, u8Channel<<5 , BIT(5)| BIT(6));

    if(bUsingBurstWrite)
    {
        //Enable burst write mode, GAMMA_BW_EN .
        SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_70_L, BIT(7) , BIT(7));
    }

    if (( GammaMapMode ==E_DRVPNL_GAMMA_10BIT_MAPPING))//For 1024 Entry Mode
    {
        //  write ref of one entry of gamma table, WR_R[11:0]
        for (i = 0; u16Addr < u16NumOfLevel/4; i += 6)
        {
            // gamma x   of R
            u16GammaValueR[0]  = u8Tab[i] & 0x0F;
            u16GammaValueR[0] |= (MS_U16)u8Tab[i+1] << 4;

            // gamma x+1 of R
            u16GammaValueR[1]  = (u8Tab[i] & 0xF0) >>  4;
            u16GammaValueR[1] |= (MS_U16)u8Tab[i+2] << 4;

            // gamma x+2 of R
            u16GammaValueR[2]  = u8Tab[i+3] & 0x0F;
            u16GammaValueR[2] |= (MS_U16)u8Tab[i+4] << 4;

            // gamma x+3 of R
            u16GammaValueR[3]  = (u8Tab[i+3] & 0xF0) >> 4;
            u16GammaValueR[3] |=  (MS_U16)u8Tab[i+5] << 4;

            for (j = 0; j < 3; j ++)
            {
                u16GammaDeltaR[j] = abs(u16GammaValueR[j+1] - u16GammaValueR[j]);
            }

            u16MaxGammaValue1 = MAX(MAX(u16GammaValueR[0],u16GammaValueR[1]),MAX(u16GammaValueR[2],u16GammaValueR[3]));

            if(u16MaxGammaValue < u16MaxGammaValue1)
            {
                u16MaxGammaValue = u16MaxGammaValue1;
            }

            if(!bUsingBurstWrite)  //Normal write mode
            {
                SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_71_L, u16Addr, 0x00FF);  // Set write address, GAMMA_ADR[7:0] .
            }

            u16GammaDeltaR[0]=(((u16GammaValueR[0]<<15)&0x8000)|((u16GammaDeltaR[0]<<10)&0x7C00)|
                               ((u16GammaDeltaR[1]<<5)&0x03E0)|((u16GammaDeltaR[2]<<0)&0x001F));
            //write ref of one entry of gamma table, GAMMA_DATA[26:0]
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_73_L, u16GammaValueR[0]>>1, 0x07FF);
            SC_W2BYTE(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_72_L, u16GammaDeltaR[0]);

            if(!bUsingBurstWrite)  //Normal write mode
            {
                //After finish normal write mode,toggle write pulse, GAMMA_WR_EN.
                SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_70_L,BIT(3),BIT(3));
                while ( SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_70_L,BIT(3)));
            }
            u16Addr ++ ;   // Continue to write next entry of gamma table .
        }
    }
    else  //For 256 Entry Mode
    {
        //  write ref of one entry of gamma table, WR_R[11:0]
        for(i = 0; u16Addr < u16NumOfLevel; i += 3)
        {
            // gamma x
            u16GammaValueR[0] = u8Tab[i] & 0x0F;
            u16GammaValueR[0] |= u8Tab[i+1] << 4;

            // gamma x+1
            u16GammaValueR[1] = (u8Tab[i] & 0xF0) >> 4;
            u16GammaValueR[1] |= u8Tab[i+2] << 4;

            // gamma x+2
            u16GammaValueR[2] = u8Tab[i+3] & 0x0F;
            u16GammaValueR[2] |= u8Tab[i+4] << 4;

            u16MaxGammaValue1 = MAX(u16GammaValueR[0],u16GammaValueR[1]);
            ...
            u16GammaDeltaR[0] = ( u16GammaValueR[1] - u16GammaValueR[0] ) / 4;
            u16GammaDeltaR[0]  =     ((u16GammaValueR[0]<<15)&0x8000)  | ((u16GammaDeltaR[0]<<10)&0x7C00)
                                    |((u16GammaDeltaR[0]<<5)&0x03E0)   | ((u16GammaDeltaR[0]&0x001F));
            //write ref of one entry of gamma table, GAMMA_DATA[26:0]
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_73_L, u16GammaValueR[0]>>1, 0x07FF);
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_72_L, u16GammaDeltaR[0] ,   0xFFFF);
            ...
            u16GammaDeltaR[1] = ( u16GammaValueR[2] - u16GammaValueR[1] ) / 4;
            u16GammaDeltaR[1]  =     ((u16GammaValueR[1]<<15)&0x8000)  | ((u16GammaDeltaR[1]<<10)&0x7C00)
                                    |((u16GammaDeltaR[1]<<5)&0x03E0)   | ((u16GammaDeltaR[1]&0x001F));
            //write ref of one entry of gamma table, GAMMA_DATA[26:0]
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_73_L, u16GammaValueR[1]>>1,   0x07FF);
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_72_L, u16GammaDeltaR[1],   0xFFFF);
    ...
    if(bUsingBurstWrite)
    {
        //After finish burst write data of one channel, disable burst write mode .
        ...
        case E_PNL_LPLL_VIDEO:
        {
            switch (eLPLL_Type)
            {
                case E_PNL_TYPE_TTL:
                    ...
                    else if ((ldHz >= 250000000) && (ldHz < 500000000))
                    {
                ...
                case E_PNL_TYPE_HS_LVDS:
                    ...
                    switch (eLPLL_Mode)
                    {
                        case E_PNL_MODE_SINGLE:
                            ...
                            else if((ldHz >= 500000000) && (ldHz < 1150000000))
                            {
                        ...
                        case E_PNL_MODE_DUAL:
                            ...
                            else if((ldHz >= 250000000) && (ldHz < 570000000))
                            {
                                u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_HS_LVDS_2CH_25to57MHz;
                            }
                            else if((ldHz >= 570000000) && (ldHz < 1150000000UL))
                            {
                                u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_HS_LVDS_2CH_57to115MHz;
                            }
                            else
                            {
                                u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_HS_LVDS_2CH_115to150MHz;
                            }
                        break;
                    }
                break;
///Not Support
                ...
                case E_PNL_LPLL_EPI28_6P:
                    if((ldHz >= 800000000) && (ldHz < 1720000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI28_8BIT_6PAIR_80to172MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI28_8BIT_6PAIR_172to300MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_EPI28_8P:
                    if((ldHz >= 800000000) && (ldHz < 1140000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI28_8BIT_8PAIR_80to114MHz;
                    }
                    else if((ldHz >= 1140000000UL) && (ldHz < 2280000000UL))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI28_8BIT_8PAIR_114to228MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI28_8BIT_8PAIR_228to300MHz;
                    }
                break;
#endif
                case E_PNL_LPLL_EPI28_12P:
                    u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_28_8BIT_12PAIR_4K_V15_330to330MHz;
                break;
/*
...
*/
                ...
                case E_PNL_LPLL_VBY1_10BIT_8LANE:
                    if(ldHz < 1500000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_8CH_10BIT_4K_150to150MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_8CH_10BIT_4K_150to340MHz;
                    }
                ...
                case E_PNL_LPLL_VBY1_10BIT_2LANE:
                    if(ldHz < 380000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_2CH_10BIT_38to38MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_2CH_10BIT_38to75MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_8BIT_8LANE:
                    ...
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_8CH_8BIT_4K_150to150MHz;
                    }
                    else if((ldHz >= 1500000000UL) && (ldHz < 2200000000UL))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_8CH_8BIT_4K_150to200MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_8CH_8BIT_4K_200to340MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_8BIT_4LANE:
                    ...
                    else if((ldHz >= 750000000) && (ldHz < 1100000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_4CH_8BIT_75to110MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_4CH_8BIT_110to150MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_8BIT_2LANE:
                    if(ldHz < 380000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_2CH_8BIT_38to55MHz;
                    }
                    else if((ldHz >= 380000000) && (ldHz < 550000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_2CH_8BIT_38to55MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_VBY1_2CH_8BIT_55to75MHz;
                    }
                break;
        ...
        case E_PNL_LPLL_OSD:
        {
            switch (eLPLL_Type)
            {
                case E_PNL_TYPE_HS_LVDS:
                {
                    ...
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_HS_LVDS_1CH_25to57MHz;
                    }
                    else if((ldHz >= 570000000) && (ldHz < 1150000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_HS_LVDS_1CH_57to115MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_HS_LVDS_1CH_115to150MHz;
                    }
                }
                break;
                ...
                case E_PNL_LPLL_VBY1_10BIT_4LANE:
                    if(ldHz < 1500000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_4CH_10BIT_150to150MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_4CH_10BIT_150to340MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_10BIT_2LANE:
                    if(ldHz < 750000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_2CH_10BIT_75to75MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_2CH_10BIT_75to150MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_10BIT_1LANE:
                    if(ldHz < 380000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_1CH_10BIT_38to38MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_1CH_10BIT_38to75MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_8BIT_4LANE:
                    if(ldHz < 1500000000UL)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_4CH_8BIT_150to150MHz;
                    }
                    else if((ldHz >= 1500000000UL) && (ldHz < 2200000000UL))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_4CH_8BIT_150to220MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_4CH_8BIT_220to340MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_8BIT_2LANE:
                    if(ldHz < 750000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_2CH_8BIT_75to75MHz;
                    }
                    else if((ldHz >= 750000000) && (ldHz < 1100000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_2CH_8BIT_75to110MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_2CH_8BIT_110to150MHz;
                    }
                break;
                ...
                case E_PNL_LPLL_VBY1_8BIT_1LANE:
                    if(ldHz < 380000000)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_1CH_8BIT_38to38MHz;
                    }
                    else if((ldHz >= 380000000) && (ldHz < 550000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_1CH_8BIT_38to55MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EXT_VBY1_1CH_8BIT_55to75MHz;
                    }
                break;
...
void MHal_PNL_HWLVDSReservedtoLRFlag(void *pInstance, PNL_DrvHW_LVDSResInfo lvdsresinfo)
{
    ...
    else
    {
        ...
        if((pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type >= E_PNL_LPLL_VBY1_10BIT_4LANE)
            &&(pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type <= E_PNL_LPLL_VBY1_8BIT_8LANE))
        {
            MOD_W2BYTEMSK(REG_MOD_BK00_30_L, 0x00, BIT(14));
        }
    }
}
...
MS_BOOL MHal_PNL_MOD_Control_Out_Swing(void *pInstance, MS_U16 u16Swing_Level)
{
       ...
       (pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type <= E_PNL_LPLL_VBY1_8BIT_8LANE))
    {
        ...
        // ch12+ch13
        MOD_A_W2BYTEMSK(REG_MOD_A_BK00_23_L, (u16ValidSwing << 4 | u16ValidSwing), 0x00ff);
    }
    else
    {
        u16ValidSwing &=0xFF;
        ...
        switch(eOutputMode)
        {
            ...
            default:
                if(eLPLL_Type == E_PNL_LPLL_VBY1_10BIT_8LANE)
                {
                    ...
                    MOD_A_W2BYTE(REG_MOD_A_BK00_00_L, 0x5555); //[15:0]reg_output_conf[15:0]
                }
                ...
                else if( eLPLL_Type == E_PNL_LPLL_VBY1_10BIT_2LANE)
                {
                    ...
                    MOD_A_W2BYTE(REG_MOD_A_BK00_00_L, 0x0500);
                }
                break;
        }
    }
...
void MHal_PNL_Init_XC_Clk(void *pInstance, PNL_InitData *pstPanelInitData)
{
    ...
    if((pstPanelInitData->eLPLL_Type >= E_PNL_LPLL_VBY1_10BIT_4LANE)&&
       (pstPanelInitData->eLPLL_Type <= E_PNL_LPLL_VBY1_8BIT_8LANE))
    {
...
void MHal_PNL_Init_MOD(void *pInstance, PNL_InitData *pstPanelInitData)
{
        ...
        (pstPanelInitData->eLPLL_Type_Ext<= E_PNL_LPLL_VBY1_8BIT_8LANE)))
    {
        ...
        MOD_W2BYTEMSK(REG_MOD_BK00_62_L, 0xAC40, 0xFDCF); //[15]proc_st[13:12]byte_mode 4 byte mode[6]4ch_vby1[9]swap
        ...
        {
            printf("\n[%s][%d]Vx1 1 division\n", __FUNCTION__, __LINE__);
            MOD_W2BYTEMSK(REG_MOD_BK00_20_L, 0x0000, 0x0007);
        ...
        MOD_A_W2BYTEMSK(REG_MOD_A_BK00_30_L, 0x3FFF, 0x3FFF);   //reg_gcr_pe_en_ch
        MOD_A_W2BYTEMSK(REG_MOD_A_BK00_32_L, 0x0000, 0x3FFF);   //reg_gcr_en_rint_ch
        MOD_A_W2BYTEMSK(REG_MOD_A_BK00_3C_L, 0x3FFF, 0x3FFF);   //reg_gcr_test_ch

        /// reg_gcr_pe_adj ch0~ch13
        MOD_A_W2BYTE(REG_MOD_A_BK00_18_L,0x2222);
        MOD_A_W2BYTE(REG_MOD_A_BK00_19_L,0x2222);
        MOD_A_W2BYTE(REG_MOD_A_BK00_1A_L,0x2222);
        MOD_A_W2BYTE(REG_MOD_A_BK00_1B_L,0x2222);

        MOD_W2BYTEMSK(REG_MOD_BK00_67_L, BIT(0)|BIT(3), BIT(0)|BIT(3));  // [0] reg_vby1_8v4o_mode, [3]reg_vby1_ext_ptr_en
        MOD_W2BYTEMSK(REG_MOD_BK00_67_L, BIT(15), BIT(15));
...
MS_U8 msModCurrentCalibration(void *pInstance)
{
    ...
    MOD_W2BYTEMSK(REG_MOD_A_BK00_70_L, pPNLResourcePrivate->sthalPNL._u8MOD_CALI_TARGET, BIT(2)|BIT(1)|BIT(0));    // Select calibration target voltage, 00: 250mV, 01:350mV, 10: 300mV, 11: 200mV

    // (5) Enable Calibration mode
    MOD_W2BYTEMSK(REG_MOD_A_BK00_70_L, BIT(7), BIT(7));         // Enable calibration function
...
//        MOD_W2BYTE(REG_MOD_BK00_25_L, 0x003f);
    ...
    if( eLPLL_Type == E_PNL_LPLL_VBY1_10BIT_4LANE)
    {
        ...
        W2BYTE(REG_CLKGEN0_53_L,0x00CC); //[13:8] clk_bt656 -> clk_lpll_buf
        W2BYTE(REG_CLKGEN0_63_L,0x0410); //[11:8] clk_tx_mod_osd, [4:0] osd2mod
        W2BYTE(REG_RVD_09_L, 0x1800); //[12:8]ckg_vby1_fifo_osd [3:0]clk_vby1_fifo

        MOD_W2BYTE(REG_MOD_BK00_42_L, 0x1000); //[12]sw_rst, [3]enable osd lvds channel
        MOD_A_W2BYTE(REG_MOD_A_BK00_38_L, 0xc01f);
...
void MHal_PNL_Bringup(void *pInstance)
{
    ...
    else if(pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type == E_PNL_LPLL_VBY1_10BIT_8LANE)
    {
        ...
        //=   ICON RCON PE         =//
        //==========================//
        //W2BYTE(0x111e10, 0xffff);
        //W2BYTE(0x111e12, 0xffff);
        //W2BYTE(0x111e14, 0xffff);
        //W2BYTE(0x111e16, 0xffff);
        //W2BYTE(0x111e18, 0xffff);
        //W2BYTE(0x111e1a, 0xffff);
        //W2BYTE(0x111e1c, 0xffff);

        //==========================//
        //=    Setting MOD         =//
        //==========================//
        //W2BYTE(0x103210, 0x5410);
        //W2BYTE(0x103212, 0x7632);
        W2BYTE(0x111ee2, 0xffff);
        W2BYTE(0x1032c0, 0xd000);
        W2BYTE(0x1032c0, 0xd330);
        W2BYTE(0x1032c0, 0xd320);
        W2BYTE(0x1032c2, 0x8f3f);
//        W2BYTE(0x1032c4, 0xac40); //Addr:62; bit[11:10] = 2'b11; => reg_vby1_w_r_ini[1:0]
        //W2BYTE(0x1032cc, 0xac40); //Addr:66; bit[11:10] = 2'b11; => reg_vby1_w_r_ini_osd[1:0]
        //==========================//
        //=    GPIO                 =//
        //==========================//
        W2BYTE(0x111e80, 0xffff);
        W2BYTE(0x111e82, 0xffff);
        W2BYTE(0x111e84, 0xffff);
        W2BYTE(0x111e86, 0xffff);
        W2BYTE(0x111e90, 0xffff);
        W2BYTE(0x111e92, 0xffff); 

        if(pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16MOD_CTRLA & BIT(1))  // use Dual port to decide the Vx1 1 or 2 devision config
        {
            printf("Vx1 2 division\n");
            //==========================//
            //=    Setting MOD         =//
            //==========================//
            W2BYTE(0x103240, 0x0002);  //[2:0]reg_mft_mode
            W2BYTE(0x103242, 0x1002);  //[11:0]reg_dly_value
            W2BYTE(0x103244, 0x0f00);  //[12:0]reg_hsize
            W2BYTE(0x10324c, 0x0780);  //[12:0]reg_div_len
            W2BYTE(0x1032fe, 0x0002);  //[2:0]reg_sram_usage
            W2BYTE(0x1032a6, 0x4000);  //[14]reg_vfde_mask
            //W2BYTE(0x103210, 0x6420);  //vby1 swap
            //W2BYTE(0x103212, 0x7531);
            //W2BYTE(0x1032c6, 0x1800);  //[12]vby1_8ch[11:10]pair_mirror2
        }
    }
...
{
    // MOD sw reset
    MOD_W2BYTE(REG_MOD_BK00_42_L,0x0000);
    MOD_W2BYTE(REG_MOD_BK00_42_L,0x1000);

    // Setting TCON signal through MOD PAD
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x1811);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x9811);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x0000);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x1912);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x9912);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x0000);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x1a13);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x9a13);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x0000);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x1b17);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x9b17);
    MOD_A_W2BYTE(REG_MOD_A_BK00_51_L,0x0000);
}
#endif
