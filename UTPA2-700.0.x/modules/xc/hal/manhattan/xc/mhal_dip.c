...
void Hal_SC_DWIN_sw_db(void *pInstance, P_SC_DIP_SWDB_INFO pDBreg, SCALER_DIP_WIN eWindow)
{
    if( eWindow == DIP_WINDOW )
    {
      ...
      if( pDBreg->u32H_PreScalingRatio & BIT(31))
        SC_W2BYTEMSK(0, REG_SC_BK34_6C_L, BIT(15),BIT(15));
      else
        SC_W2BYTEMSK(0, REG_SC_BK34_6C_L, 0,BIT(15));

      SC_W4BYTE(0, REG_SC_BK34_2C_L, (pDBreg->u32H_PreScalingRatio & (~(BIT(31)))));   // H pre-scaling
    ...
    {
      if( pDBreg->u32H_PreScalingRatio & BIT(31))
        SC_W2BYTEMSK(0, REG_SC_BK3B_78_L, BIT(15),BIT(15));
      else
        SC_W2BYTEMSK(0, REG_SC_BK3B_78_L, 0,BIT(15));

      SC_W4BYTE(0, REG_SC_BK3B_37_L, (pDBreg->u32H_PreScalingRatio & (~(BIT(31)))));   // H pre-scaling
  ...
  {
      if( pDBreg->u32H_PreScalingRatio & BIT(31))
        SC_W2BYTEMSK(0, REG_SC_BK3C_78_L, BIT(15),BIT(15));
      else
        SC_W2BYTEMSK(0, REG_SC_BK3C_78_L, 0,BIT(15));

      SC_W4BYTE(0, REG_SC_BK3C_37_L, (pDBreg->u32H_PreScalingRatio & (~(BIT(31)))));   // H pre-scaling
...
void Hal_SC_DWIN_get_sw_db(void *pInstance, P_SC_DIP_SWDB_INFO pDBreg, SCALER_DIP_WIN eWindow)
{
    if( eWindow == DIP_WINDOW )
    {
      pDBreg->u32H_PreScalingRatio =  SC_R4BYTE(0, REG_SC_BK34_2C_L);
      if( SC_R2BYTE(0, REG_SC_BK34_6C_L) & BIT(15) )
    ...
    {
      pDBreg->u32H_PreScalingRatio =  SC_R4BYTE(0, REG_SC_BK3B_37_L);
      if( SC_R2BYTE(0, REG_SC_BK3B_78_L) & BIT(15) )
        pDBreg->u32H_PreScalingRatio |= BIT(31);
    ...
    {
      pDBreg->u32H_PreScalingRatio =  SC_R4BYTE(0, REG_SC_BK3C_37_L);
      if( SC_R2BYTE(0, REG_SC_BK3C_78_L) & BIT(15) )
        pDBreg->u32H_PreScalingRatio |= BIT(31);
...
void HAL_XC_DIP_SetDataFmt(void *pInstance, EN_DRV_XC_DWIN_DATA_FMT fmt,SCALER_DIP_WIN eWindow)
{
    ...
    if( eWindow == DIP_WINDOW )
    {
        ...
        if( (fmt == XC_DWIN_DATA_FMT_RGB565) || (fmt == XC_DWIN_DATA_FMT_ARGB8888) )
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK34_0B_L,0,BIT(12));
        }
        else
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK34_0B_L, BIT(12),BIT(12));
        }
    }
    else if( eWindow == DWIN0_WINDOW )
    {
...
void HAL_XC_DIP_MuxDispatch(void *pInstance, SCALER_DIP_SOURCE_TYPE eSource,SCALER_DIP_WIN eWindow)
{
    if( eWindow == DIP_WINDOW )
    {
        ...
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SC2_OP_CAPTURE)
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK20_03_L, BIT(1),BIT(1));
            SC_W2BYTEMSK(0, REG_SC_BK20_2F_L, BIT(15),BIT(15));
        }
    ...
    else if( eWindow == DWIN1_WINDOW )
    {
        if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK3C_70_L, 0,BMASK(4:1));
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
            HAL_XC_DIP_SetMux(pInstance, SC_DWIN_IPMUX_SC_VOP,SC_DWIN_IPMUX_ADC_B<<2,eWindow);
            SC_W2BYTEMSK(0, REG_SC_BK3C_7A_L, BIT(15),BIT(15));
            SC_W2BYTEMSK(0, REG_SC_BK3C_70_L, BIT(4)|BIT(1),BMASK(4:1));
        }
        else if( (eSource == SCALER_DIP_SOURCE_TYPE_OP_MAIN) || (eSource == SCALER_DIP_SOURCE_TYPE_OP_SUB))
        {
...
void HAL_XC_DIP_AdjustScaling(void *pInstance,SCALER_DIP_WIN eWindow,SCALER_DIP_SOURCE_TYPE eSource)
{
...
#endif
    MS_U16 u16HSD_X = 0;//u16MaxWith=0;
    ...
    {
        DBreg.u16H_CapStart = 0;
        DBreg.u16H_CapSize = SC_R2BYTEMSK(0, REG_SC_BK81_07_L,BMASK(12:0));
    }
...
void HAL_XC_DIP_SetWinProperty(void *pInstance, MS_U8 u8BufCnt,MS_U16 u16Width,MS_U16 u16LineOft,MS_U16 u16Height,MS_PHY u64OffSet, SCALER_DIP_SOURCE_TYPE eSource,MS_BOOL bPIP, MS_BOOL b2P_Enable, SCALER_DIP_WIN eWindow)
{
    ...
    {
        u16MaxWith = SC_R2BYTEMSK(0, REG_SC_BK81_07_L,BMASK(12:0));
    }
    ...
    if( eWindow == DIP_WINDOW )
    {
        ...
        if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
        	...
        	do_div(u64_result,2);
            SC_W2BYTEMSK(0, REG_SC_BK34_6D_L, u64_result,BMASK(11:0));
        ...
        if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK34_67_L, 0,BMASK(4:2));//0: ip_main    1: ip_sub    4: sc2_ip_main
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
            SC_W2BYTEMSK(0, REG_SC_BK34_7F_L, BIT(8),BMASK(8:7));
            SC_W2BYTEMSK(0, REG_SC_BK34_67_L, BIT(4),BMASK(4:2));//0: ip_main    1: ip_sub    4: sc2_ip_main
        }
        else
        {
    ...
    else if( eWindow == DWIN0_WINDOW )
    {
        ...
        if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
			...
			u64_result = u16MaxWith+1;
        	do_div(u64_result,2);
            SC_W2BYTEMSK(0, REG_SC_BK3B_79_L, u64_result,BMASK(11:0));
        ...
        if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK3B_67_L, 0,BMASK(4:2));//0: ip_main    1: ip_sub    4: sc2_ip_main
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
            SC_W2BYTEMSK(0, REG_SC_BK3B_73_L, BIT(8),BMASK(8:7));
            SC_W2BYTEMSK(0, REG_SC_BK3B_67_L, BIT(4),BMASK(4:2));//0: ip_main    1: ip_sub    4: sc2_ip_main
        }
        else
        {
    ...
    else if( eWindow == DWIN1_WINDOW )
    {
        ...
        if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
			...
			u64_result = u16MaxWith+1;
        	do_div(u64_result,2);
            SC_W2BYTEMSK(0, REG_SC_BK3C_79_L, u64_result ,BMASK(11:0));
        ...
        if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK3C_67_L, 0,BMASK(4:2));//0: ip_main    1: ip_sub    4: sc2_ip_main
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
            SC_W2BYTEMSK(0, REG_SC_BK3C_73_L, BIT(8),BMASK(8:7));
            SC_W2BYTEMSK(0, REG_SC_BK3C_67_L, BIT(4),BMASK(4:2));//0: ip_main    1: ip_sub    4: sc2_ip_main
        }
        else
        {
    ...
    if( (bDIPRotation == TRUE) && (DIPRotation == eWindow) ){
        ...
        SC_W2BYTEMSK(0, REG_SC_BK34_6E_L, u64_result,BMASK(7:0));
    }
}
...
void HAL_XC_DIP_SetBase0(void *pInstance,MS_PHY u64BufStart,MS_PHY u64BufEnd,SCALER_DIP_WIN eWindow)
{
    ...
    if( eWindow == DIP_WINDOW )
    {
        ...
        SC_W4BYTE(0, REG_SC_BK36_10_L, u64_result);     // input address0
        //high bound
        ...
        SC_W4BYTE(0, REG_SC_BK36_30_L, u64_result);     // input address0
    }
    else if( eWindow == DWIN0_WINDOW )
    {
        //low bound
        u64_result = u64BufStart;
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK3B_10_L, u64_result);     // input address0
        //high bound
        u64_result = u64BufEnd;
        do_div(u64_result,u16BusSize);
        u64_result|=(1<<DWIN_W_LIMITE_OFT);
        //u64BufEnd /= u16BusSize;
        //u64BufEnd|=(1<<DWIN_W_LIMITE_OFT);
        SC_W4BYTE(0, REG_SC_BK3B_30_L, u64_result);     // input address0
    }
    else if( eWindow == DWIN1_WINDOW )
    {
        //low bound
        u64_result = u64BufStart;
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK3C_10_L, u64_result);     // input address0
        //high bound
        u64_result = u64BufEnd;
        do_div(u64_result,u16BusSize);
        u64_result|=(1<<DWIN_W_LIMITE_OFT);
        //u64BufEnd /= u16BusSize;
        //u64BufEnd|=(1<<DWIN_W_LIMITE_OFT);
        SC_W4BYTE(0, REG_SC_BK3C_30_L, u64_result);     // input address0
    }
    else
    {
        return;
...
void HAL_XC_DIP_SetBase1(void *pInstance, MS_PHY u64BufStart,MS_PHY u64BufEnd,SCALER_DIP_WIN eWindow)
{
    ...
    if( eWindow == DIP_WINDOW )
    {
        ...
        u64_result = u64BufStart;
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK36_20_L, u64_result);     // input address0
        //high bound
        u64_result = u64BufEnd;
        do_div(u64_result,u16BusSize);
        u64_result|=(1<<DWIN_W_LIMITE_OFT);
        //u64BufEnd /= u16BusSize;
        //u64BufEnd|=(1<<DWIN_W_LIMITE_OFT);
        SC_W4BYTE(0, REG_SC_BK36_40_L, u64_result);     // input address0
    }
    else if( eWindow == DWIN0_WINDOW )
    {
        //low bound
        u64_result = u64BufStart;
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK3B_20_L, u64_result);     // input address0
        //high bound
        u64_result = u64BufEnd;
        do_div(u64_result,u16BusSize);
        u64_result|=(1<<DWIN_W_LIMITE_OFT);
        //u64BufEnd /= u16BusSize;
        //u64BufEnd|=(1<<DWIN_W_LIMITE_OFT);
        SC_W4BYTE(0, REG_SC_BK3B_40_L, u64_result);     // input address0
    }
    else if( eWindow == DWIN1_WINDOW )
    {
        //low bound
        u64_result = u64BufStart;
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK3C_20_L, u64_result);     // input address0
        //high bound
        u64_result = u64BufEnd;
        do_div(u64_result,u16BusSize);
        u64_result|=(1<<DWIN_W_LIMITE_OFT);
        //u64BufEnd /= u16BusSize;
        //u64BufEnd|=(1<<DWIN_W_LIMITE_OFT);
        SC_W4BYTE(0, REG_SC_BK3C_40_L, u64_result);     // input address0
    }
    else
    {
        return;
...
void HAL_XC_DIP_SetDIPRProperty(void *pInstance, ST_XC_DIPR_PROPERTY *pstDIPRProperty, SCALER_DIP_WIN eWindow)
{
    ...
    SC_W4BYTE(0, REG_SC_BK36_78_L, u64_result);
    //C Buffer
    u64_result = pstDIPRProperty->u32CBufAddr;
    do_div(u64_result,DWIN_BYTE_PER_WORD);
    SC_W4BYTE(0, REG_SC_BK36_7A_L, u64_result);
    //width
    ...
    if(pstDIPRProperty->enDataFmt == DIP_DATA_FMT_YUV420_H265_10BITS)
    {
        ...
        do_div(u64_result,DWIN_BYTE_PER_WORD);
        SC_W4BYTE(0, REG_SC_BK36_12_L, u64_result);
        //C Buffer for EVD 10 bits
        u64_result = pstDIPRProperty->u32CBufAddr10Bits;
        do_div(u64_result,DWIN_BYTE_PER_WORD);
        SC_W4BYTE(0, REG_SC_BK36_14_L, u64_result);
...
void HAL_XC_DIP_Rotation(void *pInstance, MS_BOOL bRotation,EN_XC_DIP_ROTATION eRoDirection,MS_PHY u64StartAddr,SCALER_DIP_WIN eTmpWindow)
{
    ...
    if(bRotation)
    {
        SC_W2BYTEMSK(0, REG_SC_BKBF_20_L,BIT(11)|BIT(3),BIT(11)|BIT(3));
    }
    else
    {
        SC_W2BYTEMSK(0, REG_SC_BKBF_20_L,0,BIT(11)|BIT(3));
    }
...
void HAL_XC_DIP_SetPinpon(void *pInstance, MS_BOOL bPinpon,MS_PHY u64PinponAddr,MS_PHY u64OffSet,SCALER_DIP_WIN eWindow)
{
    ...
    if( eWindow == DIP_WINDOW )
    {
        ...
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK36_20_L, u64_result);
        SC_W4BYTE(0, REG_SC_BK36_50_L,u64OffSet);
    }
    else if( eWindow == DWIN0_WINDOW )
    {
        u64_result = u64PinponAddr;
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK3B_20_L, u64_result);
        SC_W4BYTE(0, REG_SC_BK3B_50_L,u64OffSet);
    }
    else if( eWindow == DWIN1_WINDOW )
    {
        u64_result = u64PinponAddr;
        do_div(u64_result,u16BusSize);
        SC_W4BYTE(0, REG_SC_BK3C_20_L, u64_result);
        SC_W4BYTE(0, REG_SC_BK3C_50_L,u64OffSet);
    }
}
...
void HAL_XC_DIP_2P_Width_Check(void *pInstance, XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_DIP_WIN eWindow)
{
    ...
    {
        u16MaxWith = SC_R2BYTEMSK(0, REG_SC_BK01_07_L,BMASK(13:0));
        if( SC_R2BYTEMSK(0, REG_SC_BK02_0A_L,BIT(15)) && (u16MaxWith > FHD_WIDTH) )
        {
