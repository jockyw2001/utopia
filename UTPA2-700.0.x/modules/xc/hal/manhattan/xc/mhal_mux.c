    ...
    else
    {
        ...
        switch (PortId)
        {
            case HAL_DVI_IP_A: // DVI0 (port A)
                ...
                W2BYTEMSK(REG_COMBO_PHY0_P1_5B_L, 0x2FFF, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P2_5B_L, 0x2FFF, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P3_5B_L, 0x2FFF, 0x3FFF);
            ...
            case HAL_DVI_IP_B: // DVI1 (port B)
                ...
                W2BYTEMSK(REG_COMBO_PHY0_P1_5B_L, 0x0000, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P2_5B_L, 0x2FFF, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P3_5B_L, 0x2FFF, 0x3FFF);
            ...
            case HAL_DVI_IP_C: // DVI2 (port C)
                ...
                W2BYTEMSK(REG_COMBO_PHY0_P0_5B_L, 0x2FFF, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P1_5B_L, 0x2FFF, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P2_5B_L, 0x0000, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P3_5B_L, 0x2FFF, 0x3FFF);
            ...
            case HAL_DVI_IP_D: // DVI3 (port D)
                ...
                W2BYTEMSK(REG_COMBO_PHY0_P1_5B_L, 0x2FFF, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P2_5B_L, 0x2FFF, 0x3FFF);
                W2BYTEMSK(REG_COMBO_PHY0_P3_5B_L, 0x0000, 0x3FFF);
...
void Hal_SC_mux_dispatch(void *pInstance, E_MUX_INPUTPORT src_port , E_MUX_OUTPUTPORT dest_port)
{
    ...
    else if ( src >= HAL_INPUTPORT_ANALOG0  && src < (HAL_INPUTPORT_ANALOG0 + NUMBER_OF_ANALOG_PORT ))
    {
        ...
        // For Monaco VGA source unstable/nosignal issue
        if (MDrv_ReadByte(REG_CHIP_REVISION) < 1)// Monaco U02 will fix this.
        {
            SCALER_WIN eWindow = MAIN_WINDOW;
            if (dest == HAL_OUTPUTPORT_SCALER_SUB_WINDOW1 )
            {
                eWindow = SUB_WINDOW;
            }

            Hal_SC_ip_set_input_sync_sample_mode(pInstance, GLITCH_REMOVAL, eWindow );
        }
#endif
        // Src -> ( IPmux )
        Hal_ADC_set_mux(pInstance, src_port);
...
/******************************************************************************/
///This function set Scaler Index Information to ADC.let ADC send correct data to scaler.
///@param  u8ScalerIndex \b Scaler Index.
/******************************************************************************/
void Hal_SC_mux_SetScalerIndexInformationToADC(void *pInstance, MS_U8 u8ScalerIndex, MS_U16 u16InputSourceType)
{
    ...
    W2BYTEMSK(REG_IPMUX_35_L, u8ScalerIndex , BIT(0));
#else
