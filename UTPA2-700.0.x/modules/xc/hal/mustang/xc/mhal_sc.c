...
void Hal_XC_H3D_LR_Toggle_Enable(void *pInstance,MS_BOOL bEn, MS_BOOL b2DTo3D, MS_BOOL bSkipDefaultLRFlag)
{
    ...
    //mustang FA out LR flag, CC.yang's advice
    ///BK20_24[1:0]
    ///0: real 3D
    ///1: rbank[0] only old chip use it,
    ///2: dual view
    ///3: t3d
    //T3d left flag enable: (for t3d active out)
    //BK12_33[15] = 1
    //BK12_30[11] = 1
    if(bEn)
    {
        if(b2DTo3D)
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_24_L, BIT(0)|BIT(1), BIT(0)|BIT(1));
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(11), (BIT(11)));
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(15), BIT(15));///2Dto3D LR flag enable
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_24_L, 0x00, BIT(0)|BIT(1));
   ...
   {
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0, (BIT(8))); //enable Enable free run 3D LR flag: if disable, when image freezed,the output is always L or always R.
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0, (BIT(1)));//L first,
        SC_W2BYTEMSK(0, REG_SC_BK20_24_L, 0x0000, BIT(0)|BIT(1));        //Output 3D L/R selection
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0, (BIT(11)));
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0, BIT(15));
    }
}
...
MS_BOOL Hal_SC_Init(void *pInstance)
{
    ...
    //For mantis issue 1170354,Repeat last point instead of black point at IPM
    //to avoid wrong scaling result at the border
    SC_W2BYTEMSK(0, REG_SC_BK02_68_L, BIT(10) , BIT(10));
    return TRUE;
}
...
void MHAL_SC_set_VOP(void *pInstance, MS_XC_VOP_Data *pstVOPData)
{
    ...
    if(E_XC_PNL_LPLL_MINILVDS_2CH_3P_8BIT == (MS_U8)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type)
    {
        u16VsyncStart = 1;
    }
