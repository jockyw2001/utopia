...
MS_BOOL MHAL_SC_Set_DynamicScaling(void *pInstance,MS_PHY u32MemBaseAddr, MS_U8 u8MIU_Select, MS_U8 u8IdxDepth, MS_BOOL bOP_On, MS_BOOL bIPS_On, MS_BOOL bIPM_On,SCALER_WIN eWindow)
{
    ...
    if (Mhal_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        ...
        if (u16DSOnOff != 0)
        {
            ...
            Mhal_XC_MLoad_WriteCmd(pInstance, REG_SC_BK1F_10_L, 0, 0x0E00);
...
//  |---------------------------|---------------------------|
    ...
    //Index detpth is decided by utopia
    //pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth = 16;
    ...
    //pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = 0x1800; //For pureSN case, DS DRAM size is 0x1800

#if (!defined(CONFIG_PURE_SN))
    pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = 0x1800;
    pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth = 16;
#endif

  //For pure SN, DS buffer size is decided by MApi_XC_SetDSInfo()
#if (defined(CONFIG_PURE_SN))
    if (pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize < DS_BUFFER_SIZE)
    {
        pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = 0x1800;
        pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth = 16;
    }
    ...
    {
        pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth = 100;
    }
#endif
    ...
    else
    {
        ...
        if (pXCResourcePrivate->sthal_SC.bTrig_pStored)
        {
            ...
            Mhal_XC_MLoad_set_IP_trig_p(pInstance, 0x03, 0x05); // reset to HW default setting
    ...
    if (u16DSCurrentOnOff != u16OnOff)
    {
        ...
        {
            _MLOAD_ENTRY(pInstance);
            Mhal_XC_MLoad_WriteCmd(pInstance, REG_SC_BK1F_10_L, u16OnOff & 0x1E00, 0x1E00);
            Mhal_XC_MLoad_Fire(pInstance, TRUE);
            _MLOAD_RETURN(pInstance);
        }
        else
