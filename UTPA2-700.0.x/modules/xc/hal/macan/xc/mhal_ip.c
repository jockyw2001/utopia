...
void Hal_SC_ip_software_reset(void *pInstance, MS_U8 u8Reset, SCALER_WIN eWindow)
{
    ...
    if(psXCInstPri->u32DeviceID == E_XC_DEVICE0)
    {
        ...
        if (SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_19_L, BIT(1)) == BIT(1))
        {
            SC_W2BYTEMSK( psXCInstPri->u32DeviceID, REG_SC_BK00_02_L, BIT(10), BIT(10));
            bReset = TRUE;
        }
        else
        {
            printf("reset  %s  %d   mute=%x   bwd=%x\n",__FUNCTION__,__LINE__,SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK10_19_L, BIT(1)), SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK46_01_L, 0xFF));
        }
        MsOS_DelayTask(1);
        ...
        if (bReset == TRUE)
        {
            SC_W2BYTEMSK( psXCInstPri->u32DeviceID, REG_SC_BK00_02_L, 0, BIT(10) );
        }
...
void Hal_SC_ip_set_capture_v_start_with_Menuload(void *pInstance, MS_U16 u16Vstart , SCALER_WIN eWindow)
{
    ...
    if(Mhal_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        ...
        if(eWindow == MAIN_WINDOW)
        {
            Mhal_XC_MLoad_set_trigger_sync(pInstance, MLOAD_TRIGGER_BY_IP_MAIN_SYNC);
            Mhal_XC_MLoad_WriteCmd(pInstance, REG_SC_BK01_04_L, u16Vstart, 0x1FFF);
        }
        else
        {
            Mhal_XC_MLoad_set_trigger_sync(pInstance, MLOAD_TRIGGER_BY_IP_SUB_SYNC);
            Mhal_XC_MLoad_WriteCmd(pInstance, REG_SC_BK03_04_L, u16Vstart, 0x1FFF);
        }
        if (!Mhal_XC_MLoad_Fire(pInstance, TRUE))
        {
                printf("Error: Set MLoad Fire Error!!!!\n ");
        }
        Mhal_XC_MLoad_set_trigger_sync(pInstance, MLOAD_TRIGGER_BY_OP_SYNC);
...
void Hal_SC_ip_set_capture_h_start_with_Menuload(void *pInstance, MS_U16 u16Hstart , SCALER_WIN eWindow)
{
    ...
    if(Mhal_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        ...
        {
            Mhal_XC_MLoad_set_trigger_sync(pInstance, MLOAD_TRIGGER_BY_IP_MAIN_SYNC);
            Mhal_XC_MLoad_WriteCmd(pInstance, REG_SC_BK01_05_L, u16Hstart, 0x1FFF);
        }
        else
        {
            Mhal_XC_MLoad_set_trigger_sync(pInstance, MLOAD_TRIGGER_BY_IP_SUB_SYNC);
            Mhal_XC_MLoad_WriteCmd(pInstance, REG_SC_BK03_05_L, u16Hstart, 0x1FFF);
        }
        if (!Mhal_XC_MLoad_Fire(pInstance, TRUE))
        {
                printf("Error: Set MLoad Fire Error!!!!\n ");
...
MS_BOOL HAL_XC_IsInputSourceDisabledByHwAutoNoSignal(void *pInstance, SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    if (eWindow == MAIN_WINDOW)
...
MS_BOOL HAL_XC_IsInputSourceDisabled(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if(gSrcInfo[eWindow].bAutoNoSignalEnabled)
        bInputSourceDisabled = HAL_XC_IsInputSourceDisabledByHwAutoNoSignal(pInstance, eWindow);
    else
