...
static MS_BOOL b2P_Ena = FALSE;
static InterruptCb pDipIntCb;
static InterruptCb pDwin0IntCb;
static InterruptCb pDwin1IntCb;
static MS_BOOL bDipIsr = FALSE,bDwin0Isr = FALSE,bDwin1Isr = FALSE,bAttached = FALSE;
...
#ifdef DIP_PATCH_CLIP_FAILED_1073531
static MS_U16 u16DIPPatchVar_MaxCaptureInputSize = 0x0;
#endif
//==============================================================================
//==============================================================================
...
#ifdef PATCH_HW_VE_2Pto1P
static MS_U16 u16DIPPatchVar_CaptureInputSize_multi_2 = 0x0;
static MS_BOOL _IsXCIP2PModePatch4VE(void)
{
    MS_BOOL bRet = FALSE;
    MS_BOOL bIsVEon = FALSE;
    bIsVEon = MDrv_Read2Byte(REG_VE_DAMMY_B);
    if(bIsVEon&BIT(0))
        bRet = TRUE;
    else
        bRet = FALSE;
...
static MS_U16 HAL_XC_DIP_GetWidth(void *pInstance, SCALER_DIP_SOURCE_TYPE eSource, SCALER_DIP_WIN eWindow)
{
    ...
    if(UtopiaInstanceGetPrivate(pInstance, (void*)&pDipPri) != 0)
   {
        XC_LOG_TRACE(XC_DBGLEVEL_PARAMETER_TRACE,"DIP", "DIP - UtopiaInstanceGetPrivate fail\n");
        return 0;
   }else
   {
	...
	if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
    {
        if(b2P_Ena)
        {
            MS_BOOL bVideoPmode = TRUE;

            u16MaxWith = SC_R2BYTEMSK(0, REG_SC_BK01_07_L,BMASK(13:0));

            // bk0x1301_reg0x1e[11] , 1: i mode, 0: p mode
            bVideoPmode = (SC_R2BYTEMSK(0, REG_SC_BK01_1E_L, BIT(11)) > 0) ? FALSE : TRUE;
            if ((( bVideoPmode == TRUE) && (u16MaxWith == UHD_WIDTH)) ||
                (( bVideoPmode == FALSE) && (u16MaxWith == FHD_WIDTH))
                )
            {
        ...
        {
#ifdef PATCH_HW_VE_2Pto1P
            if (_IsXCIP2PModePatch4VE() == TRUE)
            {
                u16MaxWith = SC_R2BYTEMSK(0, REG_SC_BK01_07_L,BMASK(13:0)) << 1;
                u16DIPPatchVar_CaptureInputSize_multi_2 = u16MaxWith;
            }
            else
            {
                u16MaxWith = SC_R2BYTEMSK(0, REG_SC_BK01_07_L,BMASK(13:0));
    ...
    {
        u16MaxWith = DIP_R2BYTEMSK(0, REG_SC_BK81_07_L,BMASK(12:0));
    }
...
#ifdef PATCH_HW_VE_2Pto1P
    else if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
    {
        if(_IsXCIP2PModePatch4VE() == TRUE)
        {
            u16MaxWith = u16MaxWith/2;

            if(u16MaxWith > FHD_WIDTH)
            {
                bNeedAverageMode = TRUE;
            }
            else
        ...
        {
            if((DIP_R2BYTEMSK(0,REG_SC_BK02_0A_L,BIT(15)) != 0 )||(u16MaxWith > FHD_WIDTH))
            {
                bNeedAverageMode = TRUE;
            }
            else
...
void HAL_XC_DIP_Init(void *pInstance, SCALER_DIP_WIN eWindow)
{
    ...
    SC_W2BYTEMSK(0, REG_SC_BK10_50_L, E_XC_DIP_OP2<<8, BMASK(10:8));
...
void HAL_XC_DIP_Averge2PTo1P(void *pInstance,MS_BOOL bAVG,SCALER_DIP_WIN eWindow)
{
    ...
    if ((DBreg.u16H_CapStart > 1) || (DBreg.u16H_CapSize + DBreg.u16H_CapStart < (u16DIPPatchVar_MaxCaptureInputSize>>1)))
    {
        MS_U32 u32OutputPatched = 0x0;
        MS_U32 u32InputPatched = 0x0;
        MS_U32 u32ClipWidthPatched = 0x0;
        MS_U32 u32ScalingRatio = 0x0;

         // patch for avg mode, partial clip failed problem.
        if(bAVG == TRUE)
        {
            DBreg.u16H_CapStart /= 2;
            u32ClipWidthPatched = (u16DIPPatchVar_MaxCaptureInputSize>>1) - DBreg.u16H_CapStart;
        }
        else
        {
            u32ClipWidthPatched = u16DIPPatchVar_MaxCaptureInputSize - DBreg.u16H_CapStart;
        }

        u32ScalingRatio = DBreg.u32H_PreScalingRatio & (~(BIT(31)));
        u32InputPatched = u32ClipWidthPatched;

        u32OutputPatched = ((((MS_U64)u32InputPatched) * (u32ScalingRatio-1)) >> 19);

        if( u32OutputPatched < u32InputPatched )
        {
            DBreg.u32H_PreScalingRatio = H_PreScalingDownRatioDIP(u32InputPatched,u32OutputPatched);
        ...
        DBreg.u16H_CapSize = u32ClipWidthPatched;

        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"[%s, %d]: g_DIPSrcInfo[eWindow].u16PreHCusScalingSrc/2 = %d, g_DIPSrcInfo[eWindow].u16PreHCusScalingDst = %d\n", __FUNCTION__, __LINE__, g_DIPSrcInfo[eWindow].u16PreHCusScalingSrc/2, g_DIPSrcInfo[eWindow].u16PreHCusScalingDst);
        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"u32ClipWidthPatched = %d\n", u32ClipWidthPatched);
        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"DBreg.u32H_PreScalingRatio = 0x%x\n", DBreg.u32H_PreScalingRatio);
        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"(g_DIPSrcInfo[eWindow].u16PreHCusScalingDst>>1) = %d\n", (g_DIPSrcInfo[eWindow].u16PreHCusScalingDst>>1));
        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"(u32ClipWidthPatched - DBreg.u16H_CapSize) = %d\n", (u32ClipWidthPatched - DBreg.u16H_CapSize));
        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"DBreg.u16H_CapSize = %d\n", DBreg.u16H_CapSize);
        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"u32InputPatched = %d u32OutputPatched = %d\n", u32InputPatched, u32OutputPatched);
        XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"u32H_PreScalingRatio = 0x%x\n", DBreg.u32H_PreScalingRatio);
    ...
    if(bAVG == TRUE)
    {
        ...
        DBreg.u16H_CapStart/=2;
        DBreg.u16H_CapSize = (DBreg.u16H_CapSize + 1) >> 1;
...
#if PATCH_HW_VE_2Pto1P
       if((_IsXCIP2PModePatch4VE() == TRUE) && (g_DIPSrcInfo[eWindow].enDIPSourceType == SCALER_DIP_SOURCE_TYPE_MAIN))
        {
            Hal_SC_DWIN_get_sw_db(pInstance,&DBreg,eWindow);
            if( u16DIPPatchVar_CaptureInputSize_multi_2 > g_DIPSrcInfo[eWindow].u16PreHCusScalingDst)
            {
                DBreg.u32H_PreScalingRatio = H_PreScalingDownRatioDIP(u16DIPPatchVar_CaptureInputSize_multi_2,g_DIPSrcInfo[eWindow].u16PreHCusScalingDst);
                DBreg.u32H_PreScalingRatio &= 0x7FFFFFL;
                DBreg.u32H_PreScalingRatio |= (BIT(31));
            }else
            {
            ...
            DBreg.u16H_CapSize = u16DIPPatchVar_CaptureInputSize_multi_2;
            Hal_SC_DWIN_sw_db(pInstance,&DBreg,eWindow);
        }
#endif
...
#ifdef DIP_PATCH_CLIP_FAILED_1073531
    u16DIPPatchVar_MaxCaptureInputSize = u16MaxWith;
#endif
...
void HAL_XC_DIP_SetMiuSel(void *pInstance, MS_U8 u8MIUSel,SCALER_DIP_WIN eWindow)
{
    if( eWindow == DIP_WINDOW )
    {
        ...
        MDrv_Write2ByteMask((REG_MIU0_BASE +(0x7c*2)), 0x00, BIT(11)); // MIU select (Group3 BIT1)
        MDrv_Write2ByteMask((REG_MIU1_BASE +(0x7c*2)), BIT(11), BIT(11)); // IP select
    ...
    else if( eWindow == DWIN0_WINDOW )
    {
        // Scaler control MIU by itself
        // So MIU1 IP-select is set to 1. At this moment, MIU0 Miu select is not working.
        MDrv_Write2ByteMask((REG_MIU0_BASE +(0x7c*2)), 0x00, BIT(14)); // MIU select (Group3 BIT1)
        MDrv_Write2ByteMask((REG_MIU1_BASE +(0x7c*2)), BIT(14), BIT(14)); // IP select
        if( ((CHIP_MIU_ID)u8MIUSel) == E_CHIP_MIU_0 )
        {
    ...
    else if( eWindow == DWIN1_WINDOW )
    {
        // Scaler control MIU by itself
        // So MIU1 IP-select is set to 1. At this moment, MIU0 Miu select is not working.
        SC_W2BYTEMSK(0, REG_SC_BK7F_11_L, BIT(11), BIT(11)); // IP select (Group6 BIT11)
        if( ((CHIP_MIU_ID)u8MIUSel) == E_CHIP_MIU_0 )
        {
...
void HAL_XC_DIP_SetDIPRProperty(void *pInstance, ST_XC_DIPR_PROPERTY *pstDIPRProperty, SCALER_DIP_WIN eWindow)
{
    ...
    //Y Buffer
    SC_W4BYTE(0, REG_SC_BK36_78_L, ((pstDIPRProperty->u32YBufAddr)/DWIN_BYTE_PER_WORD));
    //C Buffer
    SC_W4BYTE(0, REG_SC_BK36_7A_L, ((pstDIPRProperty->u32CBufAddr)/DWIN_BYTE_PER_WORD));
    //width
    ...
    if(pstDIPRProperty->enDataFmt == DIP_DATA_FMT_YUV420_H265_10BITS)
    {
        ...
        SC_W4BYTE(0, REG_SC_BK36_12_L, ((pstDIPRProperty->u32YBufAddr10Bits)/DWIN_BYTE_PER_WORD));
        //C Buffer for EVD 10 bits
        SC_W4BYTE(0, REG_SC_BK36_14_L, ((pstDIPRProperty->u32CBufAddr10Bits)/DWIN_BYTE_PER_WORD));
...
{
    // Macan not support DIP rotation.
    return ;
}
