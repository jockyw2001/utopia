...
static void _HAL_SC_BWR_set_3d_mode(void * pInstance, SCALER_WIN eWindow)
{
    ...
    if(u8ChipVersion == 0)
    {
        ...
        {
            //restore value
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_29_L, pXCResourcePrivate->sthal_SC.u16OldValue[E_STORE_VALUE_INIT_CMD_NUM], 0x1F1F);
        }
    }
...
static void _HAL_SC_BWR_set_3d_mode_burst(void * pInstance, SCALER_WIN eWindow)
{
    ...
    if(u8ChipVersion == 0)
    {
        ...
        if((MDrv_XC_Get_3D_Output_Mode(pInstance) == E_XC_3D_OUTPUT_LINE_ALTERNATIVE)&&(bBwrMode))
        {
            if(b2pMode)
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_29_L, 0x0101, 0x1F1F);
            }
            else
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_29_L, 0, 0x1F1F);
            }
        }
        else
        {
            //restore value
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_29_L, pXCResourcePrivate->sthal_SC.u16OldValue[E_STORE_VALUE_INIT_CMD_NUM], 0x1F1F);
        }
    }
    else if(u8ChipVersion >= 1)
    {
...
E_APIXC_ReturnValue MHal_XC_HDR_GetCaps(void *pInstance, XC_HDR_SUPPORTED_CAPS *pstHDRCaps)
{
    ...
    switch (enHDRType)
    {
        ...
        {
            if (MDrv_AUTH_IPCheck(IPAUTH_DOLBY_HDR_PIN))
            {
                pstHDRCaps->bSupported = TRUE;
            }
            else
