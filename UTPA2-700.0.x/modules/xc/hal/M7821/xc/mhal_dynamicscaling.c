...
// |  CMD4   |   CMD5    |
// |  CMD6   |   CMD7    |
void Hal_SC_WriteDSCommand(void *pInstance, ds_reg_ip_op_sel IPOP_Sel, MS_U32 u32Index, MS_U64 u64DSCmd, SCALER_WIN eWindow)
{
    ...
    MS_PHY u32IndexBase = 0;
    MS_U32 u32Offset = 0;
    MS_U64 *pu64Data = NULL;
    MS_U32 u32CmdCntPerMIUBus = (MS_MLOAD_BUS_WIDTH/DS_CMD_LEN_64BITS);

    if (u32CmdCntPerMIUBus == 0)
    {
        printf("assert! u32CmdCntPerMIUBus=0!! \n");
        assert(0);
    }

    u32Offset = (u32Index/u32CmdCntPerMIUBus)*MS_MLOAD_MEM_BASE_UNIT + (u32Index%u32CmdCntPerMIUBus)*DS_CMD_LEN_64BITS;

    if(eWindow == MAIN_WINDOW)
    {
        if(IPOP_Sel == DS_IP)
        {
            u32IndexBase= pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr;
        }
        else
        {
    ...
    else
    {
        if(IPOP_Sel == DS_IP)
        {
            u32IndexBase= pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_IPS;
        }
        else
        {
            u32IndexBase= pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr_OPS;
        }
    }

    u32IndexBase += (MS_U32)pXCResourcePrivate->sthal_SC.u8DynamicScalingCurrentIndex_Ring[eWindow] * pXCResourcePrivate->sthal_SC.u8DynamicScalingIndexDepth * MS_MLOAD_MEM_BASE_UNIT + u32Offset;
...
extern void KApi_XC_WriteSWDSCommand_Mask(EN_KDRV_SC_DEVICE u32DeviceID,EN_KDRV_WINDOW eWindow,E_DS_CLIENT client, MS_U32 u32CmdRegAddr, MS_U16 u16CmdRegValue,k_ds_reg_ip_op_sel IPOP_Sel,k_ds_reg_source_sel Source_Select,K_XC_DS_CMDCNT *pstXC_DS_CmdCnt,MS_U16 u16Mask);
...
void Hal_SC_ResetSWDSCommand(void *pInstance,SCALER_WIN eWindow, MS_U8 u8DSIndex)
{
    ...
    MS_U16 u16TotalCmdCnt = pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize/DS_CMD_LEN_64BITS/2;
    MS_U16 i = 0;
    ...
    else
    {
        ...
        MS_PHY u32OPBaseAddr = u32IndexBase + pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize/2;
        for(i = 0; i < u16TotalCmdCnt; i++)
        {
    ...
    if(Source_Select == DS_XC)
    {
        ...
        if(IPOP_Sel == DS_IP)
        {
            if(eWindow == MAIN_WINDOW)
            {
                ...
                {
                    Hal_SC_WriteDSCommand(pInstance, DS_IP, pstXC_DS_CmdCnt->u16CMDCNT_IPM, u64CmdTemp, eWindow);
            ...
            else//subwindow
            {
                ...
                {
                    Hal_SC_WriteDSCommand(pInstance, DS_IP, pstXC_DS_CmdCnt->u16CMDCNT_IPS, u64CmdTemp, eWindow);
        ...
        else//DS_OP
        {
            if(eWindow == MAIN_WINDOW)
            {
                ...
                {
                    Hal_SC_WriteDSCommand(pInstance, DS_OP, pstXC_DS_CmdCnt->u16CMDCNT_OPM, u64CmdTemp, eWindow);
            ...
            else
            {
                ...
                {
                    Hal_SC_WriteDSCommand(pInstance, DS_OP, pstXC_DS_CmdCnt->u16CMDCNT_OPS, u64CmdTemp, eWindow);
...
void Hal_SC_WriteSWDSCommand_NonXC(void *pInstance,SCALER_WIN eWindow,MS_U32 u32Bank,MS_U32 u32Addr, MS_U16 u16Data, MS_U16 u16Mask,ds_reg_ip_op_sel IPOP_Sel,XC_DS_CMDCNT *pstXC_DS_CmdCnt)
{
    ...
    KApi_XC_WriteSWDSCommandNonXC(u32DeviceID,eWindow,E_DS_CLIENT_XC,u32Bank,u32Addr,u16Data,u16Mask,IPOP_Sel,(K_XC_DS_CMDCNT *)pstXC_DS_CmdCnt);
    return;
#else
#if (defined MSOS_TYPE_LINUX) || (defined ANDROID)
    if(0 > _s32FdScaler)
    ...
    {
        ST_KDRV_DS_WriteSWDSCommandNonXC k_ds_WriteSWDSCommandNonXC;
        if(psXCInstPri->u32DeviceID == 0)
            k_ds_WriteSWDSCommandNonXC.u32DeviceID = E_KDRV_XC_SC0;
        else
            k_ds_WriteSWDSCommandNonXC.u32DeviceID = E_KDRV_XC_SC1;
        k_ds_WriteSWDSCommandNonXC.client = E_DS_CLIENT_XC;
        k_ds_WriteSWDSCommandNonXC.eWindow = eWindow;
        k_ds_WriteSWDSCommandNonXC.u32Bank = u32Bank;
        k_ds_WriteSWDSCommandNonXC.u32Addr = u32Addr;
        k_ds_WriteSWDSCommandNonXC.u16Data = u16Data;
        k_ds_WriteSWDSCommandNonXC.u16Mask = u16Mask;
        k_ds_WriteSWDSCommandNonXC.IPOP_Sel = IPOP_Sel;
        k_ds_WriteSWDSCommandNonXC.pstXC_DS_CmdCnt = (K_XC_DS_CMDCNT *)pstXC_DS_CmdCnt;
        if (-1 == ioctl(_s32FdScaler, MDRV_XC_IOC_DS_WRITE_SWDS_NONXC_CMD, &k_ds_WriteSWDSCommandNonXC))
        {
            perror("ioctl");
            return ;
        }
...
void Hal_SC_WriteSWDSCommand_Mask(void *pInstance,SCALER_WIN eWindow, MS_U32 u32CmdRegAddr, MS_U16 u16CmdRegValue,ds_reg_ip_op_sel IPOP_Sel,ds_reg_source_sel Source_Select,XC_DS_CMDCNT *pstXC_DS_CmdCnt,MS_U16 u16Mask)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    KApi_XC_WriteSWDSCommand_Mask(u32DeviceID,eWindow,E_DS_CLIENT_XC,u32CmdRegAddr,u16CmdRegValue,IPOP_Sel,Source_Select,(K_XC_DS_CMDCNT *)pstXC_DS_CmdCnt,u16Mask);
    return;
#else
#if (defined MSOS_TYPE_LINUX) || (defined ANDROID)
    if(0 > _s32FdScaler)
    ...
    {
        ST_KDRV_DS_WriteSWDSCommand_Mask k_ds_WriteSWDSCommand_Mask;
        if(psXCInstPri->u32DeviceID == 0)
            k_ds_WriteSWDSCommand_Mask.u32DeviceID = E_KDRV_XC_SC0;
        else
            k_ds_WriteSWDSCommand_Mask.u32DeviceID = E_KDRV_XC_SC1;
        k_ds_WriteSWDSCommand_Mask.eWindow = eWindow;
        k_ds_WriteSWDSCommand_Mask.client = E_DS_CLIENT_XC;
        k_ds_WriteSWDSCommand_Mask.u32CmdRegAddr = u32CmdRegAddr;
        k_ds_WriteSWDSCommand_Mask.u16CmdRegValue = u16CmdRegValue;
        k_ds_WriteSWDSCommand_Mask.u16Mask = u16Mask;
        k_ds_WriteSWDSCommand_Mask.IPOP_Sel = IPOP_Sel;
        k_ds_WriteSWDSCommand_Mask.Source_Select = Source_Select;
        k_ds_WriteSWDSCommand_Mask.pstXC_DS_CmdCnt = (K_XC_DS_CMDCNT *)pstXC_DS_CmdCnt;
        if (-1 == ioctl(_s32FdScaler, MDRV_XC_IOC_DS_WRITE_SWDS_CMD_MASK,&k_ds_WriteSWDSCommand_Mask))
        {
            perror("ioctl");
            return ;
        }
...
//===========================================================//
    ...
    if(Source_Select == DS_XC)
    {
        ...
        //u16MaskTemp = 0xFFFF;
        //u16DataTemp = (SC_R2BYTE(psXCInstPri->u32DeviceID,u32CmdRegAddr) & ~u16MaskTemp) | (u16CmdRegValue & u16MaskTemp);
        //u16DataTemp = (SC_R2BYTE(psXCInstPri->u32DeviceID,u32CmdRegAddr) & ~u16Mask) | (u16CmdRegValue & u16Mask);

        //printf("\033[1;31m[%s:%d] u32CmdRegAddr:%x u16CmdRegValue:%x   u16Mask:%x  \033[m\n",__FUNCTION__,__LINE__,u32CmdRegAddr,u16CmdRegValue,u16Mask);
...
void Hal_SC_Add_NullCommand(void *pInstance,SCALER_WIN eWindow,ds_reg_ip_op_sel IPOP_Sel,XC_DS_CMDCNT *pstXC_DS_CmdCnt)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        if(IPOP_Sel == DS_IP)
        {
            ...
            {
                Hal_SC_WriteDSCommand(pInstance, DS_IP, i, 0x00000009FFFF0000, eWindow);
            }
    }
    else
    {
            ...
            {
                Hal_SC_WriteDSCommand(pInstance, DS_OP, i, 0x00000009FFFF0000, eWindow);
    }
        }
    }
    else
...
//===============================================================//
    ...
    pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize = 0x1F00;
    ...
    if(u16OnOff & 0x0E00)
    {
        ...
        if(pXCResourcePrivate->sthal_SC.sbOldValueReaded == FALSE)
        {
        	...
        	pXCResourcePrivate->sthal_SC.su16OldValue_BWD_Status = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK46_01_L,0x000C);

        	pXCResourcePrivate->sthal_SC.sbOldValueReaded = TRUE;
        }
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK01_21_L, 0x4000, 0xC000);
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK46_01_L, 0x0, 0x000C); //Disable BWD [2]:BWDcrtl_memconfig  [3]:BWDcrtl_baseaddr
    ...
    else
    {
        ...
        if(pXCResourcePrivate->sthal_SC.sbOldValueReaded)
        {
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK46_01_L, pXCResourcePrivate->sthal_SC.su16OldValue_BWD_Status, 0x000C);
    ...
    else
    {
        ...
        if (pXCResourcePrivate->sthal_SC.bTrig_pStored)
        {
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK68_23_L,0, BIT(0));
        }
    }
