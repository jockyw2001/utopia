...
//                  
//**************************************************************************
MS_BOOL _Hal_tmds_CheckPortSelectMux1(MS_U8 ucPortSelect)
{
    MS_BOOL bOnLinePort = FALSE;

    if((R2BYTE(REG_HDMI2_DUAL_1_50_L) &BMASK(1:0)) == ucPortSelect)
    {
        bOnLinePort = TRUE;
    }
...
//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_CheckYUV420PortSelect1()
//  [Description]
//                  
//  [Arguments]:
//
...
//                  
//**************************************************************************
MS_BOOL _Hal_tmds_CheckYUV420PortSelect1(void)
{
    MS_BOOL bYUV420Flag = FALSE;

    if(R2BYTEMSK(REG_HDMI_DUAL_1_40_L, BMASK(6:5)) == 0x60)
    {
        bYUV420Flag = TRUE;
    }
...
//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_GetPacketReceiveStatus1()
//  [Description]
//                  
//  [Arguments]:
//
...
//                  
//**************************************************************************
MS_U16 _Hal_tmds_GetPacketReceiveStatus1(void)
{
    MS_U16 usPacketInfoPortSelect1 = R2BYTE(REG_HDMI_DUAL_1_01_L);

    if(usPacketInfoPortSelect1 &BIT(3))
    {
        bReceiveAVIPortSelect1 = TRUE;
    }
    
    return usPacketInfoPortSelect1;
}
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_ReceiveAVIInfoFrame(E_MUX_INPUTPORT enInputPortType, MS_BOOL bPortSelect0, MS_BOOL bPortSelect1)
{
    ...
    if(bPortSelect1)
    {
        ...
        if(bReceiveAVIPortSelect1)
        {
            bReceiveAVIInfo = TRUE;

            bReceiveAVIPortSelect1 = FALSE;
        }
    }
...
//
//**************************************************************************
void _Hal_tmds_HDMI20MuxSetting1(MS_BOOL bHDMI20Flag)
{
    W2BYTEMSK(REG_HDMI2_DUAL_1_54_L, bHDMI20Flag? 0: BIT(1), BMASK(1:0)); // [1:0]: reg_avg_ctrl_case
}
...
void _Hal_tmds_CheckHDMI20Status(E_MUX_INPUTPORT enInputPortType, stHDMI_POLLING_INFO *stHDMIPollingInfo)
{
    ...
    MS_BOOL bPortSelect1 = FALSE;
    MS_BOOL bYUV420PortSelect0 = FALSE;
    MS_BOOL bYUV420PortSelect1 = FALSE;
    MS_BOOL bAutoEQTrigger = FALSE;
    ...
    if(_Hal_tmds_CheckPortSelectMux1(HDMI_GET_PORT_SELECT(enInputPortType)))
    {
        bPortSelect1 = TRUE;
        bYUV420PortSelect1 = _Hal_tmds_CheckYUV420PortSelect1();
    }
    ...
    if(bDEStableFlag)
    {
        if(_Hal_tmds_ReceiveAVIInfoFrame(enInputPortType, bPortSelect0, bPortSelect1))
        {
            if(bYUV420PortSelect0 | bYUV420PortSelect1)
            {
...
void _Hal_tmds_ClearEDRVaildFlag(MS_U8 ucHDMIInfoSource)
{
    ...
    switch(ucHDMIInfoSource)
    {
        case HDMI_INFO_SOURCE1:
            usHDMIBankOffset = 0x200;
            break;
    ...
    // Clear status
    W2BYTEMSK(REG_HDMI2_DUAL_0_27_L +usHDMIBankOffset, BIT(11), BIT(11)); // clear interrupt Dolby EDR Valid
    W2BYTEMSK(REG_HDMI2_DUAL_0_27_L +usHDMIBankOffset, 0, BIT(11));

    W2BYTEMSK(REG_HDMI3_DUAL_0_2F_L +usHDMIBankOffset, BIT(13)|BIT(15), BIT(13)|BIT(15)); // clr Dolby EDR enable
    W2BYTEMSK(REG_HDMI3_DUAL_0_2F_L +usHDMIBankOffset, 0, BIT(13)|BIT(15));
}
...
void _Hal_tmds_PowerControlHandler(E_MUX_INPUTPORT enInputPortType, stHDMI_POLLING_INFO *stHDMIPollingInfo)
{
    if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bNoInputFlag) // Wait clock stable
    {
        if(_Hal_tmds_GetClockValidFlag(enInputPortType, FALSE) || _Hal_tmds_GetClockStableFlag(enInputPortType)) // Clock stable, then set wait clock unstable
        {
            ...
            if(_Hal_tmds_CheckPortSelectMux0(HDMI_GET_PORT_SELECT(enInputPortType)))
            {
                _Hal_tmds_ClearEDRVaildFlag(HDMI_INFO_SOURCE0);
            }

            if(_Hal_tmds_CheckPortSelectMux1(HDMI_GET_PORT_SELECT(enInputPortType)))
            {
                _Hal_tmds_ClearEDRVaildFlag(HDMI_INFO_SOURCE1);
            }
...
void Hal_HDMI_init(MS_BOOL bImmeswitchSupport, MS_U8 ucMHLSupportPath)// void )
{
    ...
    W2BYTEMSK(REG_PM_MHL_CBUS_20, BIT(7)| BIT(5), BIT(7)| BIT(5));
    ...
    for(ucHDMIInfoSource = HDMI_INFO_SOURCE0; ucHDMIInfoSource < HDMI_INFO_SOURCE_MAX; ucHDMIInfoSource++)
    {
        ...
        W2BYTEMSK(REG_HDMI_DUAL_0_06_L +usHDMIBankOffset, (0x6E<<8), HBMASK);
        // [1:0]: enable HDMI deep color mode
        W2BYTEMSK(REG_HDMI_DUAL_0_07_L +usHDMIBankOffset, 0x03, LBMASK);
        // [2]: Deep color FIFO type 1
        W2BYTEMSK(REG_HDMI_DUAL_0_08_L +usHDMIBankOffset, BIT(2), BIT(2));
        // [7]: Frame repetition manual mode, [1]: auto non-PCM detection
        W2BYTEMSK(REG_HDMI_DUAL_0_64_L +usHDMIBankOffset, BIT(1)|BIT(7), LBMASK);

        // turn off all the interrupt
        W2BYTE(REG_HDMI_DUAL_0_63_L +usHDMIBankOffset, 0xFFFF); // HDMI interrupt clear
        W2BYTE(REG_HDMI_DUAL_0_63_L +usHDMIBankOffset, 0);
        W2BYTE(REG_HDMI_DUAL_0_60_L +usHDMIBankOffset, 0xFFFF);  //REG_HDMI_INT_MASK

        W2BYTEMSK(REG_HDMI2_DUAL_0_27_L +usHDMIBankOffset, BMASK(15:8), BMASK(15:8)); // HDMI2 interrupt clear
        W2BYTEMSK(REG_HDMI2_DUAL_0_27_L +usHDMIBankOffset, 0, BMASK(15:8));
        W2BYTEMSK(REG_HDMI2_DUAL_0_26_L +usHDMIBankOffset, BMASK(15:8), BMASK(15:8)); // HDMI2 interrupt mask

        //W2BYTEMSK(REG_CHIP_GPIO1_10_L, BIT(3), BIT(3)); // PAD_ARC MODE
        W2BYTEMSK(REG_HDMI_DUAL_0_67_L +usHDMIBankOffset, BIT(8), BIT(8)); // reg_cts_div2_en
    }
    ...
    _Hal_tmds_HDMI20MuxSetting1(TRUE); // Always set 2P mode
...
MS_U16 Hal_HDMI_GetDataInfo(E_HDMI_GET_DATA_INFO enInfo, MS_U8 ucHDMIInfoSource)
{
    ...
    {
        case E_HDMI_GET_HDE:
            usHDMIDataInfo = R2BYTEMSK(REG_HDMI_DUAL_0_65_L +usHDMIBankOffset, 0x3FFF);
            break;
        case E_HDMI_GET_VDE:
            usHDMIDataInfo = R2BYTEMSK(REG_HDMI2_DUAL_0_31_L +usHDMIBankOffset, 0x3FFF);
            break;
        case E_HDMI_GET_HTT:
            usHDMIDataInfo = R2BYTEMSK(REG_HDMI2_DUAL_0_32_L +usHDMIBankOffset, 0x3FFF);
            break;
        case E_HDMI_GET_VTT:
            usHDMIDataInfo = R2BYTEMSK(REG_HDMI2_DUAL_0_30_L +usHDMIBankOffset, 0x3FFF);
            break;
...
MS_U16 Hal_HDMI_gcontrol_info(HDMI_GControl_INFO_t gcontrol, MS_U8 ucHDMIInfoSource)
{
    ...
    u16regvalue = R2BYTE(REG_HDMI_DUAL_0_15_L +usHDMIBankOffset);
...
MS_U32 Hal_HDMI_packet_info(MS_U8 ucHDMIInfoSource)
{
    ...
    switch(ucHDMIInfoSource)
    {
        ...
        case HDMI_INFO_SOURCE1:
            usHDMIBankOffset = 0x200;
            usPacketStatus = _Hal_tmds_GetPacketReceiveStatus1();
    ...
    usPacketStatus = R2BYTE(REG_HDMI_DUAL_0_02_L +usHDMIBankOffset) &BMASK(15:14);
    ...
    if(usPacketStatus > 0)
    {
        ...
        // Clear status
        W2BYTE(REG_HDMI_DUAL_0_02_L +usHDMIBankOffset, usPacketStatus);
    }

    usPacketStatus = R2BYTE(REG_HDMI2_DUAL_0_26_L +usHDMIBankOffset) &BIT(3);
...
void Hal_HDMI_packet_ctrl(MS_U8 ucHDMIInfoSource, MS_U16 u16pkt)
{
    ...
    W2BYTE(REG_HDMI_DUAL_0_01_L +usHDMIBankOffset, u16pkt);
}
...
EN_AVI_INFOFRAME_VERSION Hal_HDMI_avi_infoframe_info_ver(MS_U8 ucHDMIInfoSource)
{
    ...
    if (R2BYTEMSK(REG_HDMI_DUAL_0_46_L +usHDMIBankOffset, BMASK(9:8)) >> 8 == 0x00)
    {
        enPktVersion = E_AVI_INFOFRAME_VERSION_NON;
    }
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_46_L +usHDMIBankOffset, BMASK(9:8)) >> 8 == 0x01)
    {
        enPktVersion = E_AVI_INFOFRAME_VERSION1;
    }
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_46_L +usHDMIBankOffset, BMASK(9:8)) >> 8 == 0x02)
    {
        enPktVersion = E_AVI_INFOFRAME_VERSION2;
    }
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_46_L +usHDMIBankOffset, BMASK(9:8)) >> 8 == 0x03)
    {
...
MS_U8 Hal_HDMI_avi_infoframe_info(MS_U8 ucHDMIInfoSource, MS_U8 u8byte)
{
    ...
    u16Reg = R2BYTE(REG_HDMI_DUAL_0_40_L +usHDMIBankOffset + ((u8byte - 1)/2)*2);
...
MS_BOOL Hal_HDMI_get_packet_value(MS_U8 ucHDMIInfoSource, MS_HDMI_PACKET_STATE_t u8state, MS_U8 u8byte_idx, MS_U8 *pu8Value)
{
    ...
    *pu8Value = MDrv_ReadByte(u32Base_add +usHDMIBankOffset +u8byte_idx);
...
MS_U8 Hal_HDMI_audio_content_protection_info(MS_U8 ucHDMIInfoSource)
{
    ...
    return R2BYTEMSK(REG_HDMI_DUAL_0_16_L +usHDMIBankOffset, LBMASK);
}
...
MS_U8 Hal_HDMI_audio_payload_info(MS_U8 ucHDMIInfoSource, MS_U8 u8byte)
{
    ...
    if(u8byte%2)
    {
        return ((R2BYTEMSK(REG_HDMI_DUAL_0_17_L +usHDMIBankOffset +(u8byte/2)*2, HBMASK)) >> 8);
    }
    else
    {
        return R2BYTEMSK(REG_HDMI_DUAL_0_17_L +usHDMIBankOffset +(u8byte/2)*2, LBMASK);
    }
}
...
MS_U8 Hal_HDMI_err_status_update(MS_U8 ucHDMIInfoSource, MS_U8 u8value, MS_BOOL bread)
{
    ...
    if(bread)
    {
        u8regvalue = R2BYTEMSK(REG_HDMI_DUAL_0_04_L +usHDMIBankOffset, LBMASK);
    }
    else
    {
        W2BYTEMSK(REG_HDMI_DUAL_0_04_L +usHDMIBankOffset, u8value, LBMASK);
    }
    return u8regvalue;
}
/******************************************************************************/
...
MS_U8 Hal_HDMI_audio_channel_status(MS_U8 ucHDMIInfoSource, MS_U8 u8byte)
{
    ...
    if(u8byte%2)
    {
        return ((R2BYTEMSK(REG_HDMI_DUAL_0_5A_L +usHDMIBankOffset +(u8byte/2)*2, HBMASK)) >> 8);
    }
    else
    {
        return R2BYTEMSK(REG_HDMI_DUAL_0_5A_L +usHDMIBankOffset +(u8byte/2)*2, LBMASK);
    }
}
...
void Hal_DVI_SwitchSrc(E_MUX_INPUTPORT enInputPortType, MS_U8 ucHDMIInfoSource)
{
    ...
    {
        case INPUT_PORT_DVI0: // A
            W2BYTEMSK(REG_HDMI2_DUAL_0_50_L +usHDMIBankOffset, 0, BMASK(1:0));
            break;

        case INPUT_PORT_DVI1: // B
            W2BYTEMSK(REG_HDMI2_DUAL_0_50_L +usHDMIBankOffset, 1, BMASK(1:0));
            break;

        case INPUT_PORT_DVI2: // C
            W2BYTEMSK(REG_HDMI2_DUAL_0_50_L +usHDMIBankOffset, 2, BMASK(1:0));
            break;

        case INPUT_PORT_DVI3: // D
            W2BYTEMSK(REG_HDMI2_DUAL_0_50_L +usHDMIBankOffset, 3, BMASK(1:0));
            break;
        default:
            //ASSERT("Not support!\n");
            break;
...
E_HDMI_ADDITIONAL_VIDEO_FORMAT Hal_HDMI_Check_Additional_Format(MS_U8 ucHDMIInfoSource)
{
    ...
    if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(7:5)) == 0)
        val = E_HDMI_NO_ADDITIONAL_FORMAT;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(7:5)) == BIT(5))
        val = E_HDMI_4Kx2K_FORMAT;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(7:5)) == BIT(6))
...
E_XC_3D_INPUT_MODE Hal_HDMI_Get_3D_Structure(MS_U8 ucHDMIInfoSource)
{
    ...
    if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == 0)
        val = E_XC_3D_INPUT_FRAME_PACKING;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == BIT(12))
        val = E_XC_3D_INPUT_FIELD_ALTERNATIVE;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == BIT(13))
        val = E_XC_3D_INPUT_LINE_ALTERNATIVE;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == (BIT(13)|BIT(12)))
        val = E_XC_3D_INPUT_SIDE_BY_SIDE_FULL;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == BIT(14))
        val = E_XC_3D_INPUT_L_DEPTH;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == (BIT(14)|BIT(12)))
        val = E_XC_3D_INPUT_L_DEPTH_GRAPHICS_GRAPHICS_DEPTH;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == (BIT(14)|BIT(13)))
        val = E_XC_3D_INPUT_TOP_BOTTOM;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:12)) == BIT(15))
...
E_HDMI_3D_EXT_DATA_T Hal_HDMI_Get_3D_Ext_Data(MS_U8 ucHDMIInfoSource)
{
    ...
    u16temp = R2BYTEMSK(REG_HDMI_DUAL_0_35_L +usHDMIBankOffset, BMASK(7:4));
...
void Hal_HDMI_Get_3D_Meta_Field(MS_U8 ucHDMIInfoSource, sHDMI_3D_META_FIELD *pdata)
{
    ...
    pdata->b3D_Meta_Present = R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BIT(11)) > 0 ? TRUE : FALSE;
    u16temp = R2BYTEMSK(REG_HDMI_DUAL_0_35_L +usHDMIBankOffset, BMASK(15:13));
    ...
    pdata->u83D_Metadata_Length = (MS_U8)(R2BYTEMSK(REG_HDMI_DUAL_0_35_L +usHDMIBankOffset, BMASK(12:8)) >> 8);
    
    for(i=0;i<pdata->u83D_Metadata_Length;i++)
    {
        pdata->u83D_Metadata[i] = R2BYTEMSK(REG_HDMI_DUAL_0_36_L +usHDMIBankOffset +i, BMASK(7:0));
    }
}
...
MS_U8 Hal_HDMI_Get_VIC_Code(MS_U8 ucHDMIInfoSource)
{
    ...
    val = R2BYTEMSK(REG_HDMI_DUAL_0_41_L +usHDMIBankOffset, BMASK(14:8)) >> 8;
...
E_HDMI_VIC_4Kx2K_CODE Hal_HDMI_Get_4Kx2K_VIC_Code(MS_U8 ucHDMIInfoSource)
{
    ...
    if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:8)) >> 8 == 0x01)
        val = E_VIC_4Kx2K_30Hz;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:8)) >> 8 == 0x02)
        val = E_VIC_4Kx2K_25Hz;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:8)) >> 8 == 0x03)
        val = E_VIC_4Kx2K_24Hz;
    else if (R2BYTEMSK(REG_HDMI_DUAL_0_34_L +usHDMIBankOffset, BMASK(15:8)) >> 8 == 0x04)
...
MS_BOOL Hal_HDMI_Is_Input_Large_166MHz(MS_U8 ucHDMIInfoSource)
{
    ...
    reg_val = R2BYTEMSK(REG_HDMI2_DUAL_0_36_L +usHDMIBankOffset, BMASK(12:0));
...
void Hal_HDMI_AVG_ScaleringDown(MS_U8 ucHDMIInfoSource, MS_BOOL btrue)
{
    ...
    else
    {
        ...
        W2BYTEMSK(REG_HDMI2_DUAL_0_20_L +usHDMIBankOffset, 0, BIT(5)); // [5]: 0: WR_Clock_Freq
    }
}
        ...
        case PKT_HDR:
            {
                ...
                {
                    usPacketContent = R2BYTE(REG_HDMI2_DUAL_0_61_L +usHDMIBankOffset +uctemp *2);
