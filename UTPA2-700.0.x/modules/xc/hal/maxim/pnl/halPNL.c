...
#include "Maxim_pnl_lpll_tbl.h"
#include "Maxim_pnl_lpll_ext_tbl.h"
...
#define FRC_PIPE_DELAY_VCNT_FSC_FHD     0x10
        ...
        case E_PNL_LPLL_VIDEO:
        {
            switch (eLPLL_Type)
            {
                ...
                case E_PNL_TYPE_DAC_I:
                    // For Maxim HDMI Tx case, use this first,
                    // need to refine after confirm with RD
...
///Not Support
                ...
                case E_PNL_LPLL_EPI28_8P:
                    if(ldHz < 1500000000UL)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_28_8BIT_8PAIR_EPI3G_V17_150to150MHz;
                    }
                    else if((ldHz >= 1500000000UL) && (ldHz < 2400000000UL))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_28_8BIT_8PAIR_EPI3G_V17_150to240MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_28_8BIT_8PAIR_EPI3G_V17_240to300MHz;
                    }
                break;
                case E_PNL_LPLL_EPI24_12P:
                    if(ldHz < 1500000000UL)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_24_10BIT_12PAIR_V17_150to150MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_24_10BIT_12PAIR_V17_150to300MHz;
                    }
                break;
                case E_PNL_LPLL_EPI28_12P:
                    if(ldHz < 1500000000UL)
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_28_8BIT_12PAIR_V17_150to150MHz;
                    }
                    else
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI_28_8BIT_12PAIR_V17_150to300MHz;
                    }
                break;
/*
...
MS_U8 MHal_PNL_Get_Loop_DIV(void *pInstance, MS_U8 u8LPLL_Mode, MS_U8 eLPLL_Type, MS_U64 ldHz)
{
    ...
    // For Maxim HDMI Tx case, loop div follow HDMI SPEC
    if ((E_PNL_TYPE_DAC_P == eLPLL_Type) || (E_PNL_TYPE_DAC_I == eLPLL_Type))
    {
        if (ldHz < 500000000UL)
        {
            u16loop_div = 16;
        }
        else if ((ldHz >= 500000000UL) && (ldHz < 1200000000UL))
        {
            u16loop_div = 8;
        }
        else if ((ldHz >= 1200000000UL) && (ldHz < 2500000000UL))
        {
            u16loop_div = 4;
        }
        else if ((ldHz >= 2500000000UL) && (ldHz < 3000000000UL))
        {
            if ((pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16HTotal < 3000) &&
                (pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16VTotal < 2000))
            {
                u16loop_div = 2; // 1080px12bits, 297MHz
            }
            else
            {
                u16loop_div = 4; // 4K@30, 297MHz
            }
        }
        else if((ldHz >= 3000000000UL) && (ldHz < 6500000000UL))
        {
            u16loop_div = 2;
        }
        else
...
MS_U16 MHal_PNL_Get_LPLL_LoopGain(void *pInstance, MS_U8 eLPLL_Mode, MS_U8 eLPLL_Type, MS_U64 ldHz)
{
    ...
    // For Maxim HDMI Tx case, loop gain follow HDMI SPEC
    if ((E_PNL_TYPE_DAC_P == eLPLL_Type) || (E_PNL_TYPE_DAC_I == eLPLL_Type))
    {
        u16loop_gain = 4;
    }
        ...
        switch(eOutputMode)
        {
            ...
            default:
                ...
                else if( eLPLL_Type == E_PNL_LPLL_VBY1_10BIT_4LANE)
                {
                    ...
                    {
                        MOD_A_W2BYTE(REG_MOD_A_BK00_00_L, 0x0055); //[15:0]reg_output_conf[15:0]
                    }
                }
                else if( eLPLL_Type == E_PNL_LPLL_VBY1_10BIT_2LANE)
                {
                    ...
                    {
                        MOD_A_W2BYTE(REG_MOD_A_BK00_00_L, 0x0005);
                    }
                }
                break;
        }
...
void MHal_PNL_Init_XC_Clk(void *pInstance, PNL_InitData *pstPanelInitData)
{
    ...
    if ((pstPanelInitData->eLPLL_Type == E_PNL_TYPE_DAC_P) || (pstPanelInitData->eLPLL_Type == E_PNL_TYPE_DAC_I))
    {
        W2BYTEMSK(L_CLKGEN0(0x53), DISABLE, CKG_ODCLK_GATED);                          // [0] disable clock
        W2BYTEMSK(L_CLKGEN0(0x53), DISABLE, CKG_ODCLK_INVERT);                         // [1] invert clock
        W2BYTEMSK(L_CLKGEN0(0x53), CKG_ODCLK_SEL_LPLL, CKG_ODCLK_SEL_SOURCE);          // [2] select source tobe LPLL clock, 0:synthetic clock out, 1:LPLL clock out
        W2BYTEMSK(L_CLKGEN0(0x53), CKG_ODCLK_CLK_LPLL, CKG_ODCLK_MASK);               // [4..3] LPLL clock div, TTL:DIV2

        // For mantis 1208252: HDMI Tx frame lock issue, HW RD said 0x100A_44 should be 0x1C for HDMI Tx case
        W2BYTEMSK(REG_RVD_44_L, 0x001C, 0x001F); //[4:0]reg_ckg_clk_misc
    }
    else
    {
        W2BYTEMSK(L_CLKGEN0(0x53), DISABLE, CKG_ODCLK_GATED);                          // [0] disable clock
    ...
    else
    {
        ...
        if (pstPanelInitData->eLPLL_Type == E_PNL_TYPE_TTL)
        {
            ...
            W2BYTE(L_CLKGEN0(0x63),0x1100); //[15:12]reg_ckg_nossc_odclk[11:8]ckg_tx_mod [5:0]ckg_osd2mod
        }
        ...
        else
        {
            ...
            W2BYTE(L_CLKGEN0(0x63),0x1000); //[15:12]reg_ckg_nossc_odclk[11:8]ckg_tx_mod [5:0]ckg_osd2mod
        }
    ...
    else if ((E_PNL_TYPE_DAC_P == pstPanelInitData->eLPLL_Type) ||
            (E_PNL_TYPE_DAC_I == pstPanelInitData->eLPLL_Type))
    {
        //set LPLL mux
        W2BYTEMSK(L_CLKGEN0(0x7E), 0x00A0, 0x00FC);           //LPLL_ODCLK setting  reg_ckg_odclk = reg_clkgen0_reserved0[7:2]
                                                              //[0] : diable clock
                                                              //[1] : invert clock
                                                              //[4:2] : xx0 : floclk_odclk_synth_out
                                                              // 011 / 111: LPLL output clock
...
void MHal_PNL_Init(void *pInstance)
{
    ...
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK68_52_L, FRC_PIPE_DELAY_HCNT_FSC_FHD, 0x3FFF);
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK68_51_L, FRC_PIPE_DELAY_VCNT_FSC_FHD, 0x1FFF);

    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK68_20_L, BIT(5), BIT(5)); // reg_stgen_en: use simple tgen to trigger op/vip
}
...
//step 3: disable param.
    ...
    W2BYTEMSK(REG_SC2_00_L, 0x0008, 0x000F);
    W2BYTEMSK(REG_SC2_03_L, 0x0002, 0x0002);
    MsOS_DelayTask(20);
    W2BYTEMSK(REG_SC2_03_L, 0x0000, 0x0002);
#if 0
    // MOD sw reset
