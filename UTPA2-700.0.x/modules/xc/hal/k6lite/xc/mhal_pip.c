...
{
    MS_U16 u16extra_req;
    u16extra_req = ((Hal_SC_get_fbl(pInstance))? 0x0 : 0x8000) | 0x01;

    _MLOAD_ENTRY(pInstance);

    //this is moved outside this function to mdrv_sc_pip.c
    //MDrv_XC_MLoad_WriteCmd(REG_SC_BK20_10_L, (TRUE<<1), BIT(1));
...
MS_BOOL Hal_SC_Is_subwindow_enable(void *pInstance)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    return SC_R2BYTEMSK( psXCInstPri->u32DeviceID, REG_SC_BK20_10_L, BIT(1));
}
...
void Hal_SC_SetPIP_FRC(void *pInstance, MS_BOOL enable, MS_U8 u8IgonreLinesNum)
{
    ...
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_45_L, ( enable ? BIT(3):0 ), BIT(3));

    // Set Threadhold
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK12_45_L, u8IgonreLinesNum, BIT(2)|BIT(1)|BIT(0));
}
