...
MS_BOOL MHal_FRC_Check_Condition(void *pInstance)
{
    ...
    if(((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultHTotal < 3000) //Vby1 but non 4K2K out
        ...
        && IsVBY1((MS_U8)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type))
    {
        bSupport = FALSE;
    }
    ...
    // For Maxim HDMI Tx case, if output size is under FHD, or input frame rate > output frame rate, turn off FRC 
    if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE) && 
      ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq < gSrcInfo[MAIN_WINDOW].u16InputVFreq) || 
       ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultHTotal < 3000) //Vby1 but non 4K2K out
        && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVTotal < 2000))))
    {
        bSupport = FALSE;
    }
