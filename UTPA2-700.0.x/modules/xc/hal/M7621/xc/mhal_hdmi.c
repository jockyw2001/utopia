...
#define HDMI_CHIP_VERIFY_REPEATER           0
#define HDMI_CHIP_VERIFY_POWER_SAVING       0
    ...
    switch(enInputPortType)
    {
        case INPUT_PORT_DVI0:
            if(bAutoEQEnable) // Disable overwrite EQ, then enable auto EQ and trigger auto EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P0_05_L, 0, BIT(15));
            ...
            else // Disable auto EQ, then overwrite EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P0_05_L, BIT(15), BIT(15));
            }
        ...
        case INPUT_PORT_DVI1:
            if(bAutoEQEnable) // Disable overwrite EQ, then enable auto EQ and trigger auto EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P1_05_L, 0, BIT(15));
            ...
            else // Disable auto EQ, then overwrite EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P1_05_L, BIT(15), BIT(15));
            }
        ...
        case INPUT_PORT_DVI2:
            if(bAutoEQEnable) // Disable overwrite EQ, then enable auto EQ and trigger auto EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P2_05_L, 0, BIT(15));
            ...
            else // Disable auto EQ, then overwrite EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P2_05_L, BIT(15), BIT(15));
            }
        ...
        case INPUT_PORT_DVI3:
            if(bAutoEQEnable) // Disable overwrite EQ, then enable auto EQ and trigger auto EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P3_05_L, 0, BIT(15));
            ...
            else // Disable auto EQ, then overwrite EQ
            {
                ...
                W2BYTEMSK(REG_DVI_RSV_DUAL_P3_05_L, BIT(15), BIT(15));
            }
...
void _Hal_tmds_CheckHDMI20Status(E_MUX_INPUTPORT enInputPortType, stHDMI_POLLING_INFO *stHDMIPollingInfo)
{
    ...
    else // HDMI 1.4 setting
    {
        ...
        {
            if(ABS_MINUS(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usClockCount, usClockCount) > 2)
            {
                stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bTimingStableFlag = FALSE;
            }
            ...
            switch(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucAutoEQState)
            {
                ...
                case HDMI_AUTO_EQ_14_CHECK_STATUS:
                    ...
                    else if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCheckErrorTimes < TMDS_14_CHECK_ERROR_TIMES)
                    {
                        ...
                        if(_Hal_tmds_GetErrorCountStatus14(enInputPortType)) // Get error
                        {
                ...
                case HDMI_AUTO_EQ_20_CHECK_STATUS:
                    ...
                    else if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCheckErrorTimes < TMDS_20_CHECK_ERROR_TIMES)
                    {
                        ...
                        if(_Hal_tmds_GetErrorCountStatus20(enInputPortType)) // Get error
                        {
                            bAutoEQTrigger = TRUE;
...
//
//**************************************************************************
void _Hal_tmds_ClearEDRVaildFlag(void)
{
    // Clear status
...
//
//**************************************************************************
void _Hal_tmds_PostEQSetting(E_MUX_INPUTPORT enInputPortType)
{
    MS_U32 ulPHYBankOffset = 0;

    switch(enInputPortType)
    {
        case INPUT_PORT_DVI0:
            ulPHYBankOffset = 0x0000; // BK 0x170200
            break;

        case INPUT_PORT_DVI1:
            ulPHYBankOffset = 0x0200; // BK 0x170400
            break;

        case INPUT_PORT_DVI2:
            ulPHYBankOffset = 0x0400; // BK 0x170600
            break;

        case INPUT_PORT_DVI3:
            ulPHYBankOffset = 0x0600; // BK 0x170800
            break;
...
#if(TMDS_POST_EQ_FUNCTION_ENABLE)
    W2BYTEMSK(REG_COMBO_PHY0_P0_2A_L +ulPHYBankOffset, 0x4F, BMASK(15:0));
    //W2BYTEMSK(REG_COMBO_PHY1_P0_50_L +ulPHYBankOffset, 0xC8, BMASK(15:0));
    W2BYTEMSK(REG_COMBO_PHY1_P0_51_L +ulPHYBankOffset, 0xC8, BMASK(15:0));
    W2BYTEMSK(REG_COMBO_PHY1_P0_53_L +ulPHYBankOffset, BIT(15)| BIT(14)| (1 << 7)| (1 << 4)| 3, BIT(15)| BIT(14)| BIT(11)| BMASK(9:7)| BMASK(6:4)| BMASK(3:0));
    W2BYTEMSK(REG_COMBO_PHY1_P0_54_L +ulPHYBankOffset, (10 << 10), BMASK(14:10));
#else
    W2BYTEMSK(REG_COMBO_PHY1_P0_53_L +ulPHYBankOffset, 0, BIT(15));
...
//
//**************************************************************************
void _Hal_tmds_PowerSavingSetting(E_MUX_INPUTPORT enInputPortType)
{
    MS_U32 ulPHYBankOffset = 0;
...
#if(TMDS_POWER_SAVING_ENABLE)
    W2BYTEMSK(REG_COMBO_PHY1_P0_14_L +ulPHYBankOffset, BIT(0), BIT(0));
    //W2BYTEMSK(REG_DVI_RSV_DUAL_P0_00_L +ulMACBankOffset, BIT(0), BIT(0));
    //W2BYTEMSK(REG_COMBO_PHY0_P0_1A_L +ulPHYBankOffset, 10, BMASK(6:0));
    //W2BYTEMSK(REG_COMBO_PHY0_P0_1B_L +ulPHYBankOffset, 10, BMASK(6:0));
    //W2BYTEMSK(REG_COMBO_PHY1_P0_16_L +ulPHYBankOffset, (75 << 8), BMASK(15:8));
    W2BYTEMSK(REG_COMBO_PHY1_P0_17_L +ulPHYBankOffset, BMASK(14:13)| BMASK(9:8)| 0x33, BMASK(15:0));
    W2BYTEMSK(REG_DVI_RSV_DUAL_P0_02_L +ulMACBankOffset, 0x640, BMASK(15:4));
    //W2BYTEMSK(REG_DVI_RSV_DUAL_P0_0C_L +ulMACBankOffset, 0x1C20, BMASK(15:4));
    //W2BYTEMSK(REG_DVI_RSV_DUAL_P0_0D_L +ulMACBankOffset, 0x1F40, BMASK(15:4));

#else
    W2BYTEMSK(REG_DVI_RSV_DUAL_P0_00_L +ulMACBankOffset, 0, BIT(0));
...
//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_GetPowerSavingDoneFlag()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
MS_BOOL _Hal_tmds_GetPowerSavingDoneFlag(E_MUX_INPUTPORT enInputPortType)
{
    MS_BOOL bPowerSavingDoneFlag = FALSE;
    MS_U32 ulMACBankOffset = 0;
    ...
    if(R2BYTE(REG_DVI_RSV_DUAL_P0_12_L +ulMACBankOffset) &BIT(4))
    {
        bPowerSavingDoneFlag = TRUE;
    }

    return bPowerSavingDoneFlag;
}

//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_PowerSavingProc()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
void _Hal_tmds_PowerSavingEnable(E_MUX_INPUTPORT enInputPortType, MS_BOOL bPowerSavingEnable)
{
    MS_U32 ulPHYBankOffset = 0;
    MS_U32 ulMACBankOffset = 0;
    ...
    //W2BYTEMSK(REG_COMBO_PHY1_P0_14_L +ulPHYBankOffset, bPowerSavingEnable? BIT(0): 0, BIT(0));
    W2BYTEMSK(REG_DVI_RSV_DUAL_P0_00_L +ulMACBankOffset, bPowerSavingEnable? BIT(0): 0, BIT(0)); // Power saving MAC enable
}
...
//
//**************************************************************************
void _Hal_tmds_PowerSavingProc(E_MUX_INPUTPORT enInputPortType, stHDMI_POLLING_INFO *stHDMIPollingInfo)
{
    MS_BOOL bPowerSavingEnable = TRUE;
    MS_BOOL bPowerChange = FALSE;

    if(_Hal_tmds_CheckPortSelectMux0(HDMI_GET_PORT_SELECT(enInputPortType)))
    {
        bPowerSavingEnable = FALSE;
    }

    if(bPowerSavingEnable) // Enable power saving
    {
        if(!stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bPowerSavingFlag)
        {
            bPowerChange = TRUE;

            stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bPowerSavingFlag = TRUE;
        }
    }
    else // Disable power saving
    {
        if(stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bPowerSavingFlag)
        {
            bPowerChange = TRUE;

            stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bPowerSavingFlag = FALSE;
        }
    }

    if(bPowerChange)
    {
        _Hal_tmds_PowerSavingEnable(enInputPortType, bPowerSavingEnable);
    }
}

#if(HDMI_CHIP_VERIFY_POWER_SAVING)
//**************************************************************************
//  [Function Name]:
//                  _Hal_tmds_PowerSavingStateProc()
//  [Description]
//
//  [Arguments]:
//
...
//
//**************************************************************************
void _Hal_tmds_PowerSavingStateProc(E_MUX_INPUTPORT enInputPortType, stHDMI_POLLING_INFO *stHDMIPollingInfo)
{
    MS_U16 usValue = 0;
    MS_U32 ulPHYBankOffset = 0;
    MS_U32 ulMACBankOffset = 0;
    static stHDMI_POWER_SAVING_INFO stPowerSavingInfo[4];
    ...
    switch(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState)
    {
        case HDMI_POWER_SAVING_START:
            if(_Hal_tmds_GetDEStableFlag(enInputPortType, 1))
            {
                if(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount < HDMI_POWER_SAVING_CHECK_INTERVAL)
                {
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount++;
                }
                else
                {
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_ENABLE;

                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = 0;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase1FailCount = 0;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase2FailCount = 0;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase3FailCount = 0;

                    W2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset, 0xFFFF);
                }
            }
            break;

        case HDMI_POWER_SAVING_ENABLE:
#if(TMDS_POWER_SAVING_ENABLE)
            _Hal_tmds_PowerSavingEnable(enInputPortType, TRUE);
#endif

            MHAL_HDMIRX_MSG_INFO("** TMPS power saving enable port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));

            stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_WAIT_FAIL1;

            stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = 0;

            break;

        case HDMI_POWER_SAVING_WAIT_FAIL1:
            if(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount < HDMI_POWER_SAVING_CHECK_INTERVAL)
            {
                usValue = R2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset);

                if(usValue &BIT(12))
                {
                    //stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_FINISH;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase1FailCount++;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = HDMI_POWER_SAVING_CHECK_INTERVAL;

                    W2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset, 0xFFFF);

                    MHAL_HDMIRX_MSG_INFO("** TMPS HDCP fail @@ 1 port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
                }
            ...
            else
            {
                ...
                {
                    Hal_HDMI_pullhpd(FALSE, enInputPortType, FALSE, FALSE);
                }
                else
                {
                    Hal_HDMI_pullhpd(FALSE, enInputPortType, TRUE, FALSE);
                }

                Hal_DVI_ClkPullLow(TRUE, enInputPortType);

                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_HPD_LOW;

                W2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset, 0xFFFF);

                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = 0;

                MHAL_HDMIRX_MSG_INFO("** TMPS HDCP pass, then HPD low port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
            }

            break;

        case HDMI_POWER_SAVING_HPD_LOW:
            if(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount < 100)
            {
                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount++;
            }
            else
            {
                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_HPD_HIGH;

                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = 0;

                if(enInputPortType == INPUT_PORT_DVI0)
                {
                    Hal_HDMI_pullhpd(TRUE, enInputPortType, FALSE, FALSE);
                }
                else
                {
                    Hal_HDMI_pullhpd(TRUE, enInputPortType, TRUE, FALSE);
                }

                Hal_DVI_ClkPullLow(FALSE, enInputPortType);

                MHAL_HDMIRX_MSG_INFO("** TMPS HPD high, then wait HDCP auth port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
            }

            break;

        case HDMI_POWER_SAVING_HPD_HIGH:
            if(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount < HDMI_HDCP_AUTH_CHECK_INTERVAL)
            {
                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount++;
                ...
                if(usValue &BIT(12))
                {
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_WAIT_STATUS;

                    MHAL_HDMIRX_MSG_INFO("** TMPS HDCP auth = %x port %d\r\n", usValue, HDMI_GET_PORT_SELECT(enInputPortType));

                    W2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset, 0xFFFF);

                    _Hal_tmds_SetErrorCountStatus(enInputPortType, stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].bHDMI20Flag);
                }
            }
            else
            {
                ...
                MHAL_HDMIRX_MSG_INFO("** TMPS HDCP not auth then finish port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
            }
            break;

        case HDMI_POWER_SAVING_WAIT_STATUS:
            //if((R2BYTE(REG_DVI_DTOP_DUAL_P0_08_L +ulMACBankOffset) &BMASK(7:0)) > 0)
            //{
                //if((R2BYTE(REG_DVI_DTOP_DUAL_P0_09_L +ulMACBankOffset) &BMASK(7:0)) > 0)
                //{
                    //stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_WAIT_FAIL2;

                    //MHAL_HDMIRX_MSG_INFO("** TMPS get steer error into power saving port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
                //}
            //}
#if(TMDS_POWER_SAVING_ENABLE)
            if(_Hal_tmds_GetPowerSavingDoneFlag(enInputPortType))
#endif
            {
                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_WAIT_FAIL2;

                MHAL_HDMIRX_MSG_INFO("** TMPS get status into power saving port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
            }
            break;

        case HDMI_POWER_SAVING_WAIT_FAIL2:
            if(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount < HDMI_POWER_SAVING_CHECK_INTERVAL)
            {
                usValue = R2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset);
                ...
                {
                    //stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_FINISH;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase2FailCount++;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = HDMI_POWER_SAVING_CHECK_INTERVAL;

                    W2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset, 0xFFFF);

                    MHAL_HDMIRX_MSG_INFO("** TMPS HDCP fail @@ 2 port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
                }
            ...
            {
                stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_DISABLE;

                MHAL_HDMIRX_MSG_INFO("** TMPS HDCP pass, then disable power saving port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
            }

            break;

        case HDMI_POWER_SAVING_DISABLE:
#if(TMDS_POWER_SAVING_ENABLE)
            _Hal_tmds_PowerSavingEnable(enInputPortType, FALSE);
#endif

            MHAL_HDMIRX_MSG_INFO("** TMPS power saving disable port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));

            stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_WAIT_FAIL3;

            stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = 0;

            break;

        case HDMI_POWER_SAVING_WAIT_FAIL3:
            if(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount < HDMI_POWER_SAVING_CHECK_INTERVAL)
            {
                usValue = R2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset);
                ...
                {
                    //stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_FINISH;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase3FailCount++;
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingCount = HDMI_POWER_SAVING_CHECK_INTERVAL;

                    W2BYTE(REG_HDCP_DUAL_P0_01_L +ulMACBankOffset, 0xFFFF);

                    MHAL_HDMIRX_MSG_INFO("** TMPS HDCP fail @@ 3 port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
                }
            ...
            {
                if(stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingTime < HDMI_POWER_SAVING_CHECK_COUNT)
                {
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingTime++;

                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_ENABLE;

                    MHAL_HDMIRX_MSG_INFO("** TMPS HDCP pass, then enable power saving port %d\r\n", HDMI_GET_PORT_SELECT(enInputPortType));
                    MHAL_HDMIRX_MSG_INFO("** TMPS check %d times port %d\r\n", stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usPowerSavingTime, HDMI_GET_PORT_SELECT(enInputPortType));
                    MHAL_HDMIRX_MSG_INFO("** TMPS case1 fail %d port %d\r\n", stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase1FailCount, HDMI_GET_PORT_SELECT(enInputPortType));
                    MHAL_HDMIRX_MSG_INFO("** TMPS case2 fail %d port %d\r\n", stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase2FailCount, HDMI_GET_PORT_SELECT(enInputPortType));
                    MHAL_HDMIRX_MSG_INFO("** TMPS case3 fail %d port %d\r\n", stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].usCase3FailCount, HDMI_GET_PORT_SELECT(enInputPortType));
                }
                else
                {
                    stPowerSavingInfo[HDMI_GET_PORT_SELECT(enInputPortType)].ucPowerSavingState = HDMI_POWER_SAVING_FINISH;
        ...
        case HDMI_POWER_SAVING_FINISH:
            if(!_Hal_tmds_GetDEStableFlag(enInputPortType, 1))
            {
                //ucPowerSavingState[HDMI_GET_PORT_SELECT(enInputPortType)] = HDMI_POWER_SAVING_START;
            }
            break;
...
void _Hal_tmds_HDCPVerifyProc(void)
{
    ...
    for(enPortIndex = INPUT_PORT_DVI0; enPortIndex <= INPUT_PORT_DVI_END; enPortIndex++)
    {
        _Hal_tmds_HDCPWriteX74(enPortIndex, 0x20, 19, ucBuffer);
    }
    ...
    for(enPortIndex = INPUT_PORT_DVI0; enPortIndex <= INPUT_PORT_DVI_END; enPortIndex++)
    {
        _Hal_tmds_HDCPWriteX74(enPortIndex, 0x40, 3, ucBuffer);
    }
    ...
    for(enPortIndex = INPUT_PORT_DVI0; enPortIndex <= INPUT_PORT_DVI_END; enPortIndex++)
    {
        Hal_HDCP_WriteKSVList(enPortIndex, ucBuffer, 19);
    }
    ...
    if(!bInverse)
    {
        switch(enInputPortType)
        {
            case INPUT_PORT_DVI0://A
                PM_W2BYTE(REG_PM_SLEEP_27_L, BIT(0), BIT(4)|BIT(0));
                PM_W2BYTE(REG_PM_SLEEP_34_L, bHighLow ? BIT(8): 0, BIT(8));
                break;
    ...
    else
    {
        switch(enInputPortType)
        {
            case INPUT_PORT_DVI0://A
                ...
                PM_W2BYTE(REG_PM_SLEEP_34_L, 0, BIT(8));
                break;
...
void Hal_HDMI_init(MS_BOOL bImmeswitchSupport, MS_U8 ucMHLSupportPath)// void )
{
    ...
    for(port_index = INPUT_PORT_DVI0; port_index <= INPUT_PORT_DVI_END; port_index++)
    {
        ...
        // If HDMI2 ACDR mode
        W2BYTE(REG_COMBO_PHY0_P0_35_L + u16bank_offset, 0x0080); // [10]: reg_hdmi2_acdr_mode = 1
        //W2BYTEMSK(REG_COMBO_PHY0_P0_59_L +u16bank_offset, 0, BIT(1)); // Phase dac power down overwrite enable
        ...
        // Post EQ setting
        _Hal_tmds_PostEQSetting(port_index);

        // Power saving setting
        _Hal_tmds_PowerSavingSetting(port_index);
    ...
    for(port_index = INPUT_PORT_DVI0; port_index <= INPUT_PORT_DVI_END; port_index++)
    {
        ...
        // Enable auto detect scramble
        //W2BYTEMSK(REG_DVI_DTOP_DUAL_P0_3A_L + u16bank_offset, BIT(8), BIT(8));
    ...
    W2BYTEMSK(REG_DVI_DTOP_DUAL_P0_15_L, BIT(10), BMASK(15:14)| BIT(10));
    W2BYTEMSK(REG_DVI_DTOP_DUAL_P1_15_L, BIT(10), BMASK(15:14)| BIT(10));
    W2BYTEMSK(REG_DVI_DTOP_DUAL_P2_15_L, BIT(10), BMASK(15:14)| BIT(10));
    W2BYTEMSK(REG_DVI_DTOP_DUAL_P3_15_L, BIT(10), BMASK(15:14)| BIT(10));
    //-----------------------------------------------------------------------------------------------//
...
void Hal_HDMI_Set_EQ(E_MUX_INPUTPORT enInputPortType, MS_HDMI_EQ enEq, MS_U8 u8EQValue)
{
    ...
    W2BYTEMSK(REG_COMBO_PHY0_P0_6E_L + ulPHYBankOffset, 0x00F0, 0x00F0);

    switch (enEq)
    {
        case MS_HDMI_EQ_2M:
            W2BYTEMSK(REG_COMBO_PHY0_P0_73_L +ulPHYBankOffset, 0x0505, 0x1F1F);
            W2BYTEMSK(REG_COMBO_PHY0_P0_74_L +ulPHYBankOffset, 0x0505, 0x1F1F);
            break;
        case MS_HDMI_EQ_10M:
            W2BYTEMSK(REG_COMBO_PHY0_P0_73_L +ulPHYBankOffset, 0x0A0A, 0x1F1F);
            W2BYTEMSK(REG_COMBO_PHY0_P0_74_L +ulPHYBankOffset, 0x0A0A, 0x1F1F);
            break;
        case MS_HDMI_EQ_15M:
            W2BYTEMSK(REG_COMBO_PHY0_P0_73_L +ulPHYBankOffset, 0x0C0C, 0x1F1F);
            W2BYTEMSK(REG_COMBO_PHY0_P0_74_L +ulPHYBankOffset, 0x0C0C, 0x1F1F);
            break;
        case MS_HDMI_EQ_MAX:
            W2BYTEMSK(REG_COMBO_PHY0_P0_73_L +ulPHYBankOffset, 0x0F0F, 0x1F1F);
            W2BYTEMSK(REG_COMBO_PHY0_P0_74_L +ulPHYBankOffset, 0x0F0F, 0x1F1F);
            break;
        case MS_HDMI_EQ_USER_DEFINE:
            W2BYTEMSK(REG_COMBO_PHY0_P0_73_L +ulPHYBankOffset, (temp)|(temp<<8), 0x1F1F);
            W2BYTEMSK(REG_COMBO_PHY0_P0_74_L +ulPHYBankOffset, (temp)|(temp<<8), 0x1F1F);
            break;
        default:
            W2BYTEMSK(REG_COMBO_PHY0_P0_73_L +ulPHYBankOffset, 0x0A0A, 0x1F1F);
            W2BYTEMSK(REG_COMBO_PHY0_P0_74_L +ulPHYBankOffset, 0x0A0A, 0x1F1F);
    ...
    W2BYTEMSK(REG_DVI_RSV_DUAL_P0_05_L +ulMACBankOffset, BIT(15), BIT(15));
}
...
#if(TMDS_POWER_SAVING_ENABLE)
            _Hal_tmds_PowerSavingProc(enPortIndex, stHDMIPollingInfo);
#endif
...
#if(HDMI_CHIP_VERIFY_POWER_SAVING)
        _Hal_tmds_PowerSavingStateProc(enPortIndex, stHDMIPollingInfo);
#endif
    }
}
