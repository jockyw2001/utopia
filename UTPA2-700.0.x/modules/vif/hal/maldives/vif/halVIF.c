...
MS_VIF_REG_TYPE VIF_CR_IIR_LPF1_OLD[]=
{
    {CR_IIR_COEF_G,0x02},
...
MS_VIF_REG_TYPE VIF_CR_IIR_LPF1_NEW[]=
{
    {CR_IIR_COEF_G,0x10},
    {CR_IIR_COEF_G+1,0x00},
    {CR_IIR_COEF_A1,0xc1},
...
void msVifInitial(void)
{
    ...
    {
        if(g_VifChipVersion == 0)   // Munich
           msWriteRegsTbl((MS_VIF_REG_TYPE *)VIF_CR_IIR_LPF1_OLD);    // IIR LPF1 coefficients
        else                                    // Maldives
           msWriteRegsTbl((MS_VIF_REG_TYPE *)VIF_CR_IIR_LPF1_NEW);    // IIR LPF1 coefficients 
    }
    else
    ...
    // tuner step size
    if(g_VifChipVersion == 1)  // Maldives
    ...
    if (VIFInitialIn_inst.VifTunerType == 1)
    {
        ...
        {
            msWriteBit(BYPASS_N_A1, 1, _BIT2);                              // Notch_A1 bypass
            msWriteBit(BYPASS_N_A2, 1, _BIT3);                              // Notch_A2 bypass
            msWriteBit(BYPASS_SOS11, 1, _BIT0);                            // SOS11 bypass
            msWriteBit(BYPASS_SOS12, 1, _BIT1);                            // SOS12 bypass
        }
        else
        {
            msWriteBit(BYPASS_N_A1, 0, _BIT2);                              // Notch_A1 not bypass
            msWriteBit(BYPASS_N_A2, 0, _BIT3);                              // Notch_A2 not bypass
        }
    }
    else
    {
    ...
    if(g_VifChipVersion == 0)   // Munich
    {
        msWriteByte(FIRMWARE_VERSION_L, 0x04);                          // 04(dd)
        msWriteByte(FIRMWARE_VERSION_H, 0x6F);                          // 06/15 (mm/yy)  firmware version control
    }
    else                                    // Maldives
    {
        msWriteByte(FIRMWARE_VERSION_L, 0x08);                          // 08(dd)
        msWriteByte(FIRMWARE_VERSION_H, 0x7F);                          // 07/15 (mm/yy)  firmware version control
    }
