...
void msVifAdcInitial(void)
{
    ...
    // [1]  : invert clock
    RIU_WriteByte(0x128E4L, 0x01);
    RIU_WriteByte(0x128E5L, 0x01);
    ...
    // [Maxim] ADCI & ADCQ clock selection (Fs or 0.5Fs)
    RIU_WriteByte(0x128E0L, 0x00);

    // [Maxim] enable PAD_SIFP/M to ADCQ
    RIU_WriteByte(0x128E8L, 0x00);

    // [Maxim] SIF + DTV => mask ADC Q part
    RIU_WriteByte(0x12006L, 0x00);
    ...
    RIU_WriteByte(0x3320L, 0x04);
    RIU_WriteByte(0x3321L, 0x00);

    // DMD_ANA_ADC_SYNC CLK_W
    RIU_WriteByte(0x128E4L, 0x00);
...
void msVifInitial(void)
{
    ...
    // version control
    msWriteByte(FIRMWARE_VERSION_L, 0x11);                          // 17(dd)
    msWriteByte(FIRMWARE_VERSION_H, 0x11);                          // 01/17 (mm/yy)  firmware version control
