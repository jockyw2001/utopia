...
void msVifAdcInitial(void)
{
    ...
    // SRAM share clock
    RIU_WriteByte(0x52992L, 0x0C);
    RIU_WriteByte(0x11F77L, 0xC1);
    RIU_WriteByte(0x52976L, 0x1C);
    RIU_WriteByte(0x5296CL, 0xC0);
    RIU_WriteByte(0x5296DL, 0x01);
    RIU_WriteByte(0x5298DL, 0xC1);
    RIU_WriteByte(0x11F7DL, 0x18);
    RIU_WriteByte(0x52981L, 0xC1);
    ...
    // DAGC1/2 SRAM MUX
    RIU_WriteByte(0x11F2AL, 0x00);
    RIU_WriteByte(0x11F2BL, 0x00);

    // mulan_dvbt2_sram_share_en
    RIU_WriteByte(0x127C0L, 0x00);
    ...
    // SRAM Power Control
    RIU_WriteByte(0x12090L, 0x38);
    RIU_WriteByte(0x12091L, 0xBB);
    RIU_WriteByte(0x127E0L, 0x01);   // SRAM_PWR_CTRL_SEL
    RIU_WriteByte(0x127E1L, 0x00);
    RIU_WriteByte(0x127E2L, 0x40);
    ...
    RIU_WriteByte(0x12818L, 0x00);                  // ADC_Q power down 
    RIU_WriteByte(0x12819L, 0x00);

    RIU_WriteByte(0x12879L, 0x00);

    RIU_WriteByte(0x12840L, 0x60);                 // Ref Enable
    ...
    RIU_WriteByte(0x12876L, 0x02);

    RIU_WriteByte(0x120BCL, 0x07);
    RIU_WriteByte(0x120BCL, 0x00);

    // ana adcdsync fifo reset
    RIU_WriteByte(0x12880L, 0x01);
    RIU_WriteByte(0x12880L, 0x00);

    RIU_WriteByte(0x12004L, 0x23);
    RIU_WriteByte(0x12005L, 0x01);
...
void msVifSetSoundSystem(VIFSoundSystem ucSoundSystem)
{
    ...
    switch(ucSoundSystem)
    {
        case VIF_SOUND_B:
            ...
            {
                // silicon tuner
                if((g_VifShiftClk == 1)&&(VIF_IS_ADC_48MHz == 0))
                {
                ...
                else if((g_VifShiftClk == 2)&&(VIF_IS_ADC_48MHz == 0))
                {
                    VifCrRateTemp = VIFInitialIn_inst.VifCrRate_B;
...
void msVifInitial(void)
{
    ...
    msWriteBit(VNCO_INV_SEL, 0, _BIT0);

    msWriteBit(CR_PD_MODE2_SCALE, 0, _BIT4);
   ...
   // version control
    msWriteByte(FIRMWARE_VERSION_L, 0x1E);                          // 30(dd)
    msWriteByte(FIRMWARE_VERSION_H, 0x80);                          // 08/16 (mm/yy)  firmware version control
...
void msVifExit(void)
{
    ...
    // disable vif adc clock
    RIU_WriteByte(0x3314L, 0x11);
    RIU_WriteByte(0x3315L, 0x11);

    // disable vif DAC clock
    RIU_WriteByte(0x331AL, 0x11);
    RIU_WriteByte(0x331BL, 0x11);

    // disable atsc_adcd_sync clock
    ...
    // SRAM share clock
    // vif_vdagc1_sram_share
    RIU_WriteByte(0x52992L, 0x01);
    RIU_WriteByte(0x11F77L, 0x11);
    // vif_vdagc2_sram_share
    RIU_WriteByte(0x52976L, 0x11);
    RIU_WriteByte(0x5296CL, 0x11);
    RIU_WriteByte(0x5296DL, 0x00);
    RIU_WriteByte(0x5298DL, 0x11);
    // vif_dacdly_sram_share
    RIU_WriteByte(0x11F7DL, 0x11);
    RIU_WriteByte(0x52981L, 0x11);
    ...
    // MPLL Power Down
    RIU_WriteByte(0x12860L, 0xFE);
    RIU_WriteByte(0x12861L, 0x24);
    RIU_WriteByte(0x1286AL, 0x83);
    RIU_WriteByte(0x1286BL, 0x1E);    
}
        ...
        case VIF_AFC_STATUS2:
            ...
            if ((afc_foe <= 0x04) || (afc_foe >= 0xFC))             // |AFC_FOE|<=4
            {
                ...
                if (bVifDbbAcq == 0)        // 0: not channel scan; 1: channel scan
                {
                    ...
                    // for 200% over-modulation
                    msWriteBit(LOCK_LEAKY_SEL, 0, _BIT0);
                    msWriteBit(VNCO_INV_SEL, 1, _BIT0);
