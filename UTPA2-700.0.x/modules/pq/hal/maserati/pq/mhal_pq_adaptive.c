...
#define TIME_SHARING_MAX_ID 4 //Eva add one 0804
...
MS_U32 MDrv_SC_hpfTolerantReport( const MS_U8 _timeSharingID, const XC_HpfDefaultRegSetting defaultReg, MS_U32 *_hpfHdTolerantReport, const MS_U8 timingSegment )
{
    ...
    _return_hd =  ( ( MDrv_Read2Byte(REG_FSC_BK30_0A_L)&0x00FF ) << 16) + MDrv_Read2Byte(REG_FSC_BK30_07_L);
    _return_sd =  ( ( MDrv_Read2Byte(REG_FSC_BK30_0A_L)&0xFF00 ) << 8)  + MDrv_Read2Byte(REG_FSC_BK30_08_L);
...
MS_U32 MDrv_SC_dynamicHDfilterBaseReport( const MS_U8 _timeSharingID, const MS_U32 u32MotionValue, const MS_BOOL isUkosabihsMv0, const XC_HpfDefaultRegSetting defaultReg, const MS_U8 timingSegment  )
{
    ...
    _return = ( ( MDrv_Read2Byte(REG_FSC_BK30_06_L)&0xFF00 ) << 8) + MDrv_Read2Byte(REG_FSC_BK30_05_L);
...
MS_U8 MDrv_SC_SDHD_FilterBaseReport( const MS_U8 _timeSharingID, MS_U32 *_hpfHdCntReport, MS_U32 *_hpfSdCntReport, const XC_HpfDefaultRegSetting defaultReg,  const MS_U8 timingSegment , MS_U32 *hdNactCntRet )
{
    ...
    HDcnt_Act_Cur =  ( ( MDrv_Read2Byte(REG_FSC_BK30_0D_L)&0x00FF ) << 16) + MDrv_Read2Byte(REG_FSC_BK30_0B_L);
    SDcnt_Act_Cur =  ( ( MDrv_Read2Byte(REG_FSC_BK30_0D_L)&0xFF00 ) << 8 ) + MDrv_Read2Byte(REG_FSC_BK30_0C_L);
...
void MDrv_SC_SDHD_U11BaseReport( const MS_U8 _timeSharingID, MS_U32 *_hpfHdCntReport4, MS_U32 *_hpfSdCntReport4, const MS_U8 timingSegment , MS_U32 *hdNactCntRet4 )
{
    ...
    HDcnt_Act_Cur =  ( ( MDrv_Read2Byte(REG_FSC_BK30_03_L)&0x00FF ) << 16) + MDrv_Read2Byte(REG_FSC_BK30_01_L);
    SDcnt_Act_Cur =  ( ( MDrv_Read2Byte(REG_FSC_BK30_03_L)&0xFF00 ) << 8 ) + MDrv_Read2Byte(REG_FSC_BK30_02_L);
    hdNactCntCur =   ( ( MDrv_Read2Byte(REG_FSC_BK30_06_L)&0x00FF ) << 16) + MDrv_Read2Byte(REG_FSC_BK30_04_L);

    /*if( (( MDrv_Read2Byte(REG_FSC_BK30_1D_L) >>12)&0x1) != 1 ){
        HDcnt_Act = (HDcnt_Act_Cur+ HDcnt_Act) >> 1;
        SDcnt_Act = (SDcnt_Act_Cur+ SDcnt_Act) >> 1;
        hdNactCnt = (hdNactCntCur+ hdNactCnt) >> 1;
    }
    else{
        HDcnt_Act = HDcnt_Act_Cur;
        SDcnt_Act = SDcnt_Act_Cur;
        hdNactCnt = hdNactCntCur;
    }*/
    ...
    *hdNactCntRet4 = hdNactCntCur;
    ...
    if( _timeSharingID == TIMEID_sdhdFilterBaseReport )
    {
        ...
        else if( timingSegment == 1 || timingSegment == 2 ||timingSegment == 3)
        {
...
#define TIMEID_FilterOneReport 4
...
void MDrv_UFSC_PQ_PreEnh( int fHdw2, int sdw2, MS_U16 estNoise,MS_U8 estConfirm, int srEnhStr )
{
    // BLTI bias gain
    ...
    finGain = finGain*(256-fHdw2); //for maserati 2dpk issue
    finGain = finGain* srEnhStr  >> 5;
...
MS_U8 SetAdaptiveParameter(MS_U8 u8Widdth)
{
    MS_U32 u32PureBlockSumM = 0;
    MS_U32 u32PureBlockSumL = 0;
    MS_U32 u32DetailSumM = 0;
...
void Mdrv_PQ_DBK(MS_BOOL isSceneChangeY,MS_U16 u16MotionValue,MS_U16 u16Width,MS_U16 u16height)
    {
     //DBK Variables
    ...
    else if  (u8CountForDBK == 1)
    {
        if ( bDBK_initial == 1)
        {
            u8HDBKStrength = SetAdaptiveParameter(u8Widdth);
...
{
    MApi_XC_Write2ByteMask(REG_FSC_BK30_3F_L, 0x8000, 0x8000);
#if 0
...
//  mv0Comfirm_noIIR = MDrv_SC_Agate_ME1_mv0Comfirm_noIIR();
    ...
    if( (( MDrv_Read2Byte(REG_FSC_BK30_1D_L) >>13)&0x1) == 1 )
        _timeSharingID = 3;
    ...
    if (u8Ctrl2 & ENABLE_SCALER_AUTO_DBK_DRIVER)  //Edison_DBK_driver 0x01
    {
        ...
        if(MApi_XC_GetStatus(&stXCStatus, MAIN_WINDOW) == FALSE)
        {
            ...
            Mdrv_PQ_DBK(isSceneChangeY,u16MotionValue4,stXCStatus.stCropWin.width,stXCStatus.stCropWin.height);
        //}
    }
...
//
    ...
    #if !defined(MSOS_TYPE_LINUX_KERNEL)
    MS_U8 u8RandomDither = rand()%16;
    #else
    MS_U8 u8RandomDither = get_random_int()%16;
    #endif
    sdW2 = ( sdW2*(16-u8iirPre) + u32SdW*u8iirPre + u8RandomDither) /16; //iir
    fHdW2 = ( fHdW2*(16-u8iirPre) + u32FhdW*u8iirPre +u8RandomDither) /16;
    //MS_U8 u8RandomDither = rand()%16;

    if( (( MDrv_Read2Byte(REG_FSC_BK30_1D_L) >>15)&0x1) == 1 ){
        if(isSceneChange == 1){
    ...
    if( (( MDrv_Read2Byte(REG_FSC_BK30_1D_L) >>14)&0x1) == 1 ){
        if(isSceneChange == 1){
            fHdW2 =  MDrv_Read2Byte(REG_FSC_BK30_1E_L);
            sdW2 =  MDrv_Read2Byte(REG_FSC_BK30_1F_L);
        }
  ...
  if (u8UFSCCtrl3 & ENABLE_SR_DRIVER)
    {
        ...
        MDrv_UFSC_PQ_PreEnh( fHdW2, sdW2, FSC_estNoise, FSC_estConfirm, srEnhStr );
