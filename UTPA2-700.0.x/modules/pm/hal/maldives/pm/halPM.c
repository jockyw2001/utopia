...
/// @param <GLOBAL>     \b None :
////////////////////////////////////////////////////////////////////////////////
MS_U32 HAL_PM_GetIOMapBase(void)
{
...
void HAL_PM_SetSPIOffsetForMCU(MS_U32 BANK)
{
     ...
     HAL_PM_WriteRegBit(0x000e40, DISABLE, __BIT7);  //mcu51 clk=xtal

     HAL_PM_WriteRegBit(0x001018, DISABLE, __BIT3);  // i_cache rstz

     HAL_PM_WriteRegBit(0x002ba0, ENABLE,  __BIT0);  //disable i cache

     HAL_PM_Write2Byte(0x0e24,0x0000);
     HAL_PM_Write2Byte(0x0e26,0x0000);

     HAL_PM_Write2Byte(0x2e52,0x403F);   //PM 51 rst

     HAL_PM_Write2Byte(0x2e3e,0x44);    // Chip_overwrite to mips boot


     HAL_PM_Write2Byte(MCU_REG_BASE+0x14,(MS_U16)start_addr & 0x0000FFFF);
     HAL_PM_Write2Byte(MCU_REG_BASE+0x10,(MS_U16)(start_addr>>16) & 0x000000FF);
     HAL_PM_Write2Byte(MCU_REG_BASE+0x16,(MS_U16)end_addr & 0x0000FFFF);
     HAL_PM_Write2Byte(MCU_REG_BASE+0x12,(MS_U16)(end_addr>>16) & 0x000000FF);
    ...
    HAL_PM_WriteByte(MCU_REG_BASE+0xf9,(MS_U8) (0xFF&BANK) );
    HAL_PM_WriteByte(MCU_REG_BASE+0xf8,0x01);
    //set reset password 0x0e54=0x829f
   // HAL_PM_Write2Byte(PMSLEEP_REG_BASE+0x54,0x829f);
    HAL_PM_Write2Byte(0x2e54,0x829f);
    ...
    HAL_PM_WriteByte(0x2e53,0x41);
    for(u32temCnt=0; u32temCnt<0xffff; u32temCnt++);
    HAL_PM_WriteByte(0x2e53,0x40);

    for(u32temCnt=0; u32temCnt<0xffff; u32temCnt++);
    HAL_PM_WriteByte(0x2e53,0x50);

             printf("PM Wait for PM51 standby...........\n");
	while(HAL_PM_Read2Byte(0x0e24)!=0x02);
...
{
     MS_U32 start_addr = 0x0000000;
     MS_U32 end_addr = 0x00005FFF;
     HAL_PM_WriteByte(0x2e53,HAL_PM_ReadByte(0x2e53)&(~__BIT0));
     HAL_PM_WriteByte(0x2e53,HAL_PM_ReadByte(0x2e53)&(~__BIT4));   //8051 reset
     HAL_PM_WriteByte(0x0e24, 0x00);//Clear bit
     ...
     HAL_PM_WriteRegBit(0x002ba0, ENABLE , __BIT0);  //disable i cache (enable icache bypass)

     HAL_PM_WriteRegBit(0x001018, DISABLE, __BIT1); //SPI  disable

     HAL_PM_WriteRegBit(0x001018, DISABLE, __BIT2); //DRAM disable

     HAL_PM_WriteRegBit(0x001018, ENABLE , __BIT0); //SRAM enable

     HAL_PM_Write2Byte(MCU_REG_BASE+0x04,(MS_U16)start_addr & 0x0000FFFF);
     HAL_PM_Write2Byte(MCU_REG_BASE+0x00,(MS_U16)(start_addr>>16) & 0x000000FF);
     HAL_PM_Write2Byte(MCU_REG_BASE+0x06,(MS_U16)end_addr & 0x0000FFFF);
     HAL_PM_Write2Byte(MCU_REG_BASE+0x02,(MS_U16)(end_addr>>16) & 0x000000FF);
    ...
    HAL_PM_WriteByte(0x2e53,HAL_PM_ReadByte(0x2e53)|__BIT4);   //release 8051  reset
...
MS_BOOL HAL_PM_SetDRAMOffsetForMCU(MS_U32 u32Offset)
{
     ...
     HAL_PM_WriteRegBit(0x001018, ENABLE, __BIT2); //DRAM enable

     HAL_PM_WriteRegBit(0x001018, DISABLE , __BIT0); //SRAM disable

     HAL_PM_Write2Byte(MCU_REG_BASE+0x0c,(MS_U16)start_addr & 0x0000FFFF);       //reg_dram_start_addr_0
     HAL_PM_Write2Byte(MCU_REG_BASE+0x08,(MS_U16)(start_addr>>16) & 0x0000FFFF);
     HAL_PM_Write2Byte(MCU_REG_BASE+0x0e,(MS_U16)end_addr & 0x0000FFFF);         //reg_dram_end_addr_0
     HAL_PM_Write2Byte(MCU_REG_BASE+0x0A,(MS_U16)(end_addr>>16) & 0x00000FFFF);
...
MS_BOOL HAL_PM_isRunning(void)
{
    ...
    HAL_PM_Write2Byte(0x0ea2,0x1234);				//dummy register
    u16Data_old = HAL_PM_Read2Byte(0x10fc);			//pc counter
    MsOS_DelayTaskUs(10);
    u16Data_new = HAL_PM_Read2Byte(0x10fc);
	
    if((u16Data_old != u16Data_new) && (HAL_PM_Read2Byte(0x0ea2)==0x4321))
        return TRUE;
    else
        return FALSE;	
}
...
//    HAL_PM_WriteRegBit(REG_PM_CHIP_CFG_OVERWRITE, 1, CHIP_CFG_MIPS_VAL);
    ...
    HAL_PM_WriteRegBit(0x001018, ENABLE, __BIT1); //SPI  disable
    HAL_PM_WriteRegBit(0x001018, DISABLE, __BIT2); //DRAM disable
    HAL_PM_WriteRegBit(0x001018, DISABLE , __BIT0); //SRAM enable
    HAL_PM_WriteRegBit(0x001018, DISABLE, __BIT3);  // i_cache rstz
}
...
void HAL_PM_GPIO4_SetPower(MS_BOOL bOn)
{
    if(bOn)
    {
        HAL_PM_Write2Byte(REG_PM_GPIO4_OUT, (HAL_PM_Read2Byte(REG_PM_GPIO4_OUT))|(__BIT4));
        HAL_PM_Write2Byte(REG_PM_GPIO4_OEN, (HAL_PM_Read2Byte(REG_PM_GPIO4_OEN))&(~__BIT4));
    }
    else
    {
        HAL_PM_Write2Byte(REG_PM_GPIO4_OUT, (HAL_PM_Read2Byte(REG_PM_GPIO4_OUT))&(~__BIT4));
        HAL_PM_Write2Byte(REG_PM_GPIO4_OEN, (HAL_PM_Read2Byte(REG_PM_GPIO4_OEN))&(~__BIT4));
        HAL_PM_Write2Byte(REG_PM_GPIO_PM_LOCK, 0xBABE);
    }
}
...
MS_BOOL HAL_PM_GetVersion(MS_U8 *u8Val)
{
    ...
    *u8Val = HAL_PM_ReadByte(MCU_REG_BASE+0xA6);
    return TRUE; 
}
...
{
    MS_U8 u8IRKey = 0;
    u8IRKey = HAL_PM_ReadByte(MCU_REG_BASE+0xA7);
    return u8IRKey; 
}
