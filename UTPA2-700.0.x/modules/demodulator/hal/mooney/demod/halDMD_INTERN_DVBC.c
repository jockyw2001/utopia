...
#define TDE_REG_BASE  0x2800UL
#define INNC_REG_BASE 0x3800UL
#define EQE_REG_BASE  0x3900UL
...
MS_U16 INTERN_DVBC_DSPReg_Init(const MS_U8 *u8DVBC_DSPReg,  MS_U8 u8Size)
{
    ...
    ULOGD("DEMOD","INTERN_DVBC_DSPReg_Init\n");
    ...
    {
       ULOGD("DEMOD","INTERN_DVBC_DSPReg_Init NG\n"); return FALSE;
    }    
...
MS_BOOL INTERN_DVBC_Cmd_Packet_Send(S_CMDPKTREG* pCmdPacket, MS_U8 param_cnt)
{
    ...
    return TRUE;
    // ==== Command Phase ===================
    ULOGD("DEMOD","--->INTERN_DVBC (cmd=0x%x)(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,) \n",pCmdPacket->cmd_code,
...
//    HAL_DMD_RIU_WriteByte(DMDMcuBase + 0x00,  0x01);        // reset VD_MCU
    HAL_DMD_RIU_WriteByte(DMDMcuBase + 0x00,  0x00);        // reset VD_MCU  
...
//                101: 1'b0
//                110: 1'b0
//                111: clk_xtal_12M_buf
// `RIU_W((`RIUBASE_CLKGEN1>>1)+7'h0f, 2'b01, 16'h0010);
// `RIU_W((`RIUBASE_CLKGEN1>>1)+7'h0f, 2'b01, 16'h0010);
...
// [4:0]: reg_ckg_dvbtm_ts_divnum = 19
//        => TS clock = (864/3)/(2*(19+1)) = 7.2MHz
// `RIU_W((`RIUBASE_CLKGEN1>>1)+7'h00, 2'b11, 16'h0713);
// `RIU_W((`RIUBASE_CLKGEN1>>1)+7'h00, 2'b11, 16'h0713);
...
//        [0]  : disable clock 
//        [1]  : invert clock 
//        [2]  : Select clock source => for demod clkgen clk_dvbtc_adc
//               0:clk_dmdadc
//               1:clk_vif_ssc_mux
//               ^^^^^^^^^^^^^^^
//               if(reg_vif_ssc_en) => clk_vif_ssc_43p2_p(43.2 MHz)
//               else               => clk_dmplldiv10_div2(43.2 MHz)
...
//               10:  dvb_clk86_buf
//               11:  dvb_clk96_buf
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h11, 2'b11, 16'h0844);
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h11, 2'b11, 16'h0844);
    HAL_DMD_RIU_WriteByte(0x111f23,0x08);
    HAL_DMD_RIU_WriteByte(0x111f22,0x44);
// @0x3516
// [2:0] : reg_ckg_dvbc_inner
//         [0]  : disable clock 
//         [1]  : invert clock 
//         [2]: Select clock source
//                0:  dvb_clk48_buf
//                1:  1'b0
// [6:4] : reg_ckg_dvbc_inner
//         [4]  : disable clock 
//         [5]  : invert clock 
//         [6]: Select clock source
//                0:  mpll_clk18_buf
//                1:  1'b0
// [10:8] : reg_ckg_dvbc_inner
//         [8]  : disable clock 
//         [9]  : invert clock 
//         [10]: Select clock source
//                0:  mpll_clk144_buf
//                1:  1'b0
// `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h15, 2'b11, 16'h0000);
// `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h15, 2'b11, 16'h0000);
    HAL_DMD_RIU_WriteByte(0x111f2b,0x00);
    HAL_DMD_RIU_WriteByte(0x111f2a,0x00);
// @0x351d
...
// [13:8] : reg_ckg_dvbtm_sram_t14x_t24x_srd1x
//         [0]   : disable clock 
//         [1]   : invert clock 
//         [4:2] : Select clock source
//            000:  dvb_clk96_buf
...
// `RIU_W((`RIUBASE_DEMOD_8>>1)+7'h38, 2'b11, 16'h1001);
// `RIU_W((`RIUBASE_DEMOD_8>>1)+7'h38, 2'b11, 16'h1001);
    HAL_DMD_RIU_WriteByte(0x152971,0x10);
    HAL_DMD_RIU_WriteByte(0x152970,0x01);
// @0x353b //Maserati
// [15:12] : reg_ckg_dvbtm_sram_t12x_t24x_srd1x_isdbt_inner4x
//           [12] : disable clock 
//           [13] : invert clock 
//       [15:14] : Select clock source
//             00:  clk_dvbtm_sram_t12x_t24x_srd1x_p
//             01:  clk_isdbt_inner4x_p
//             10:  clk_share_dtmb_eq0p5x_isdbt_sram0_mux
//             11:  clk_adc1x_eq1x_p
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h3b, 2'b10, 16'h0111);
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h3b, 2'b10, 16'h0111);
    HAL_DMD_RIU_WriteByte(0x111f77,0x01);
// @0x353c //Maserati
// [ 3: 0] reg_ckg_dvbtm_sram_t14x_t24x_srd1x_vifssc_isdbt_inner4x[3:0]
// [ 6: 4] reg_ckg_dvbtm_sram_t12x_t22x_isdbt_inner2x[2:0]
// [10: 8] reg_ckg_dvbtm_sram_t11x_t22x_isdbt_inner2x[2:0]
// [14:12] reg_ckg_dvbtm_sram_t12x_t24x_isdbt_outer6x
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h3c, 2'b11, 16'h1110);
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h3c, 2'b11, 16'h1110);
...
//            100:  clk_dmplldiv10
//            100:  clk_adc1x_eq1x_p
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h71, 2'b11, 16'h0811);
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h71, 2'b11, 16'h0811);
    HAL_DMD_RIU_WriteByte(0x111fe3,0x08);
...
//            000:  dvb_clk48_buf
//            001:  dvb_clk43_buf
//            010:  clk_adc_div2_buf
//            011:  mpll_clk9_buf
//            100:  clk_adc0p5x_eq0p5x_p
...
// [12: 8] reg_ckg_dvbtm_ts_in
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h18, 2'b10, 16'h1c01);
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h18, 2'b10, 16'h1c01);
    HAL_DMD_RIU_WriteByte(0x111f31,0x1c);
...
{
    MS_U8            status = true;
    ULOGD("DEMOD","INTERN_DVBC_Power_On_Initialization\n");
...
void INTERN_DVBC_Driving_Control(MS_BOOL bEnable)
{
    ...
    ULOGD("DEMOD","---> INTERN_DVBC_Driving_Control(Bit0) = 0x%x \n",u8Temp);
...
MS_BOOL INTERN_DVBC_Serial_Control(MS_BOOL bEnable, MS_U8 u8TSClk)
{
    MS_U8   status = true;
    MS_U8   temp_val;
    ULOGD("DEMOD"," @INTERN_DVBC_ts... u8TSClk=%d\n", u8TSClk);
...
{
    ULOGD("DEMOD"," @INTERN_DVBC_TS1_Enable... \n");
...
MS_BOOL INTERN_DVBC_Config(MS_U16 u16SymbolRate, DMD_DVBC_MODULATION_TYPE eQamMode, MS_U32 u32IFFreq, MS_BOOL bSpecInv, MS_BOOL bSerialTS, MS_U8 u8TSClk, MS_U16 *pu16_symbol_rate_list,MS_U8 u8_symbol_rate_list_num)
{
    ...
    //ULOGD("DEMOD","INTERN_DVBC_Config, t = %ld\n",MsOS_GetSystemTime());
...
*/
  ...
  MS_U8 indx = 0;
        ...
        {
            ULOGD("DEMOD","[a1_dvbc]Error!!! %s, %s, %d, max_len < u8_symbol_rate_list_num\n",__FILE__,__FUNCTION__,__LINE__);
...
MS_BOOL INTERN_DVBC_Active(MS_BOOL bEnable)
{
    ...
    ULOGD("DEMOD"," @INTERN_DVBC_active\n");
...
MS_BOOL INTERN_DVBC_GetLock(DMD_DVBC_GETLOCK_TYPE eType, float fCurrRFPowerDbm, float fNoChannelRFPowerDbm, MS_U32 u32TimeInterval)
{
    ...
    if (fCurrRFPowerDbm < 100.0f)
    {
        ...
        {
            MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE + 0xE6, &cData);
            if (cData > 5)
            {
                bDMD_DVBC_NoChannelDetectedWithRFPower = FALSE;
                u32DMD_DVBC_NoChannelTimeAccWithRFPower = 0;
...

    {
        switch( eType )
        {
            ...
            case DMD_DVBC_GETLOCK_AGC_LOCK:
                u16Address =  TDP_REG_BASE + 0x29; //AGC Lock,
                cBitMask = BIT(0);
                break;
            ...
            case DMD_DVBC_GETLOCK_NO_CHANNEL:
                ...
                #ifdef MS_DEBUG
                {
                    ...
                    MS_U16 u16Timer=0;
                    MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE + 0xE6, &FSM);
...
MS_BOOL INTERN_DVBC_GetPostViterbiBer(float *ber)
{
    ...
    // bank 3f 0x03 [1:0] reg_bit_err_num_freeze
    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x32, &reg_frz);
    status &= MDrv_SYS_DMD_VD_MBX_WriteReg(FEC_REG_BASE+0x32, reg_frz|0x80);

    // bank 3f 0x46 [7:0] reg_bit_err_sblprd_7_0
    //             0x47 [15:8] reg_bit_err_sblprd_15_8
    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x31, &reg);
    BitErrPeriod = reg;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x30, &reg);
    BitErrPeriod = (BitErrPeriod << 8)|reg;

    // bank 3f 0x6a [7:0] reg_bit_err_num_7_0
    //             0x6b [15:8] reg_bit_err_num_15_8
    // bank 3f 0x6c [7:0] reg_bit_err_num_23_16
    //             0x6d [15:8] reg_bit_err_num_31_24
    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x3D, &reg);
    BitErr = reg;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x3C, &reg);
    BitErr = (BitErr << 8)|reg;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x3B, &reg);
    BitErr = (BitErr << 8)|reg;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x3A, &reg);
    BitErr = (BitErr << 8)|reg;
    ...
    status &= MDrv_SYS_DMD_VD_MBX_WriteReg(FEC_REG_BASE+0x32, reg_frz);
    ...
    ULOGD("DEMOD","INTERN_DVBC PostVitBER = %8.3e \n ", *ber);
...
MS_BOOL INTERN_DVBC_GetPacketErr(MS_U16 *pktErr)
{
  MS_BOOL         status = true;
    ...
    // bank 3f 0x66 [7:0] reg_uncrt_pkt_num_7_0
    //             0x67 [15:8] reg_uncrt_pkt_num_15_8
    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x3f, &reg);
    PktErr = reg;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE+0x3e, &reg);
    PktErr = (PktErr << 8)|reg;
    ...
    ULOGD("DEMOD","INTERN_DVBC PktErr = %d \n ", (int)PktErr);
    *pktErr = PktErr;
...
MS_BOOL INTERN_DVBC_Get_FreqOffset(float *pFreqOff, MS_U8 u8BW)
{
    ...
    ULOGD("DEMOD","INTERN_DVBC_Get_FreqOffset\n");
#if 1
    // bank 2c 0x3d [0] reg_bit_err_num_freeze
    status &= MDrv_SYS_DMD_VD_MBX_WriteReg(EQE_REG_BASE + 0x3A, 0x20);
    status &= MDrv_SYS_DMD_VD_MBX_WriteReg(INNC_REG_BASE + 0x05, 0x80);
    ...
    //ULOGD("DEMOD","[dvbc]Freq_Offset = %f KHz, Reg_offset = 0x%lx, Reg_Fc_over_Fs=0x%lx, SR = %d KS/s, Fc = %f %d\n",
...
MS_BOOL INTERN_DVBC_GetCurrentModulationType(DMD_DVBC_MODULATION_TYPE *pQAMMode)
{
    ...
    ULOGD("DEMOD","INTERN_DVBC_GetCurrentModulationType\n");
    ...
    switch(u8Data&0x07)
    {
        case 0:
            *pQAMMode = DMD_DVBC_QAM16;
            ULOGD("DEMOD","[dvbc]QAM=16\n");
            return TRUE;
             break;
        case 1:
            *pQAMMode = DMD_DVBC_QAM32;
            ULOGD("DEMOD","[dvbc]QAM=32\n");
            return TRUE;
            break;
        case 2:
            *pQAMMode = DMD_DVBC_QAM64;
            ULOGD("DEMOD","[dvbc]QAM=64\n");
            return TRUE;
            break;
        case 3:
            *pQAMMode = DMD_DVBC_QAM128;
            ULOGD("DEMOD","[dvbc]QAM=128\n");
            return TRUE;
            break;
        case 4:
            *pQAMMode = DMD_DVBC_QAM256;
            ULOGD("DEMOD","[dvbc]QAM=256\n");
            return TRUE;
            break;
        default:
            *pQAMMode = DMD_DVBC_QAMAUTO;
            ULOGD("DEMOD","[dvbc]QAM=invalid\n");
            return FALSE;
    }
}
