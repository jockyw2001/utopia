...
#if (AUTO_TS_DATA_RATE) //check if TS DATA RATE change
...
MS_BOOL INTERN_DVBT2_GetTsDivNum(MS_U8* u8TSDivNum)
{
    ...
    if((u8_tmp&0x01) == 0x00)// 172 MHz    172/2(N+1)
    {
        //*u8TSDivNum = (MS_U8)(floor (172000000.0/(2*(((TS_DATA_RATE*TS_DATA_RATE_RATIO)/8.0)))-1));
        if (MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_T2_TS_DIV_172, &u8_tmp) == FALSE)
        {
            DBG_INTERN_DVBT2(printf(">INTERN_DVBT2_GetLock MBX_ReadDspReg fail \n"));
            return FALSE;
        }
        *u8TSDivNum = u8_tmp;
        ...
        DBG_AUTO_TS_DATA_RATE(printf(" CLK Source: 172 MHz \n"));
    }
    else// 288 MHz   288/2(N+1)
    {
        //*u8TSDivNum = (MS_U8)(floor (288000000.0/(2*(((TS_DATA_RATE*TS_DATA_RATE_RATIO)/8.0)))-1));
