...
#define FTN_REG_BASE  	0x2800UL
#define FTNEXT_REG_BASE 0x2900UL
...
#define     D_DMD_DVBT_CFG_BW                             0x03  // BW: 0..3  for 5M, 6M, 7M, 8M Channel Allocation
...
MS_BOOL INTERN_DVBT_Reset ( void )
{
    ...
    //ULOGD("DEMOD","INTERN_DVBT_Reset, t = %ld\n",MsOS_GetSystemTime());
...
MS_BOOL INTERN_DVBT_Config ( DMD_RF_CHANNEL_BANDWIDTH BW, MS_BOOL bSerialTS, MS_BOOL bPalBG, MS_BOOL bLPSel, MS_U8 u8TSClk, MS_U32 u32IFFreq, MS_U32 u32FSFreq, MS_U8 u8IQSwap)
{
    ...
    //ULOGD("DEMOD","INTERN_DVBT_Config, t = %ld\n",MsOS_GetSystemTime());
...
MS_BOOL  INTERN_DVBT_Adaptive_TS_CLK(void)
{
	...
	TS_Clock_Temp=HAL_DMD_RIU_ReadByte(DMD_CLK_GEN+1);
	TS_Clock_Temp=(TS_Clock_Temp&(~0x01));
	HAL_DMD_RIU_WriteByte(DMD_CLK_GEN+1,TS_Clock_Temp);			
...
DMD_LOCK_STATUS INTERN_DVBT_Lock(MS_U16 u16DMD_DVBT_TPS_Timeout, MS_U16 u16DMD_DVBT_FEC_Timeout)
{
    ...
    if (INTERN_DVBT_GetLock(E_DMD_COFDM_FEC_LOCK))
    {
        ...
        if(INTERN_DVBT_GetPostViterbiBer(&fBER) == TRUE)
        {
            ...
            ULOGD("DEMOD","[dvbt]f_ber=%8.3e, g_viter_ber=%8.3e\n",fBER,fViterbiBerFiltered);
        }
...
MS_BOOL INTERN_DVBT_GetLock(DMD_DVBT_GETLOCK_TYPE eStatus)
{
	...
	{
		//ULOGD("DEMOD","Support adaptive TS CLK in polling mode! \n");
...
MS_BOOL INTERN_DVBT_GetPostViterbiBer(float *ber)
{
    ...
    ULOGD("DEMOD","INTERN_DVBT PostVitBER = %8.3e \n ", *ber);
...
#if 1 // copy from utopia2\mxlib\hal\miami\demod\halDMD_INTERN_DVBT.c
    	noise_power = noise_power/2;    
    	noise_power /=1280;
//  	  noisepower = (rand()%256)*256;
    	if (noise_power==0)//protect value 0
    	  noise_power=1;

#ifdef MSOS_TYPE_LINUX
            snr = 10*log10f((float)noise_power);
#else
            snr = 10*Log10Approx((float)noise_power);
#endif
