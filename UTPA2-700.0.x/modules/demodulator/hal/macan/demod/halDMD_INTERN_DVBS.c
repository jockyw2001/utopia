...
//wriu 0x101e39 0x03
    ...
    u8Temp = HAL_DMD_RIU_ReadByte(0x101E8A);
    u8Temp|=0x02;
    HAL_DMD_RIU_WriteByte(0x101E8A, u8Temp);
...
MS_BOOL INTERN_DVBS_BlindScan_Config(MS_U32 u32SymbolRate, DMD_DVBS_MODULATION_TYPE eQamMode, MS_U32 u32IFFreq, MS_BOOL bSpecInv, MS_BOOL bSerialTS, MS_U8 u8TSClk, MS_U16 *pu16_symbol_rate_list,MS_U8 u8_symbol_rate_list_num)
{
    ...
    // make state machine running from while in state 1 substate0
    MDrv_SYS_DMD_VD_MBX_ReadReg((TOP_REG_BASE + 0x60*2), &u8Data);
    u8Data&=~(0x08);
    MDrv_SYS_DMD_VD_MBX_WriteReg((TOP_REG_BASE + 0x60*2), u8Data);
...
#ifdef MS_DEBUG
                    INTERN_DVBS_GetPacketErr(&pkterr);
                    MDrv_SYS_DMD_VD_MBX_ReadReg( FRONTEND_REG_BASE+0x7c*2+1,&u8Data);
                    lockingtime=u8Data;
                    MDrv_SYS_DMD_VD_MBX_ReadReg( FRONTEND_REG_BASE+0x7c*2,&u8Data);
                    lockingtime=((lockingtime<<8)|u8Data);
                    DBG_INTERN_DVBS(printf("@INTERN_DVBS_Demod Locking time [%d]\n",lockingtime));
#endif
                    bRet = TRUE;
                }
                else
...
MS_BOOL INTERN_DVBS_GetSNR(float *f_snr)//V
{
    ...
    DBG_INTERN_DVBS(printf("[DVBS]: NDA_SNR ================================: %.1f\n", NDA_SNR));
...
//end claire test
    ...
    DBG_INTERN_DVBS(printf("MDrv_Demod_BlindScan_GetTunerFreq- _u16TunerCenterFreq:%d\n", _u16TunerCenterFreq));
