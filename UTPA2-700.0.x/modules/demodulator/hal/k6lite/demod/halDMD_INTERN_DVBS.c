...
#define DVBS2_REG_BASE              0x1500
#define DVBS2_INNER_REG_BASE        0x1600
#define DVBS2_INNER_EXT_REG_BASE    0x1700
#define DVBS2_INNER_EXT2_REG_BASE    0x1800
//#define DVBSTFEC_REG_BASE           0x2300    //DVBTFEC
#define FRONTENDEXT_REG_BASE        0x2200
#define FRONTENDEXT2_REG_BASE       0x2300
#define DMDANA_REG_BASE                      0x2E00    //DMDDTOP//reg_dmdana.xls
#define DVBTM_REG_BASE                       0x1E00
...
#define FRONTEND_FREEZE_DUMP                        0x21028000
#define INNER_FREEZE_DUMP                           0x16080010

#define DCR_OFFSET                                      0x2740

#define DMDTOP_REG_BASE              0x2000
#define _REG_DMDTOP(idx)             		(DMDTOP_REG_BASE + (idx)*2)
#define TOP_WR_DBG_90          (_REG_DMDTOP(0x60)+0)

#define FRONTEND_REG_BASE           0x2100
...
#define INNER_REG_BASE              0x1600
#define _REG_INNER(idx)             (INNER_REG_BASE + (idx)*2)
#define INNER_LATCH                                                      (_REG_INNER(0x04)+0)//[4]
#define INNER_DEBUG_SEL                                          (_REG_INNER(0x04)+1)
#define INNER_PLSCDEC_DEBUG_OUT0                         (_REG_INNER(0x6B)+0)
#define INNER_PLSCDEC_DEBUG_OUT1                         (_REG_INNER(0x6B)+1)
#define INNER_TR_ROLLOFF		                                     (_REG_INNER(0x0F)+0)//[1:0]
...
//  **********  DVBSFEC.XLS  **********  //
#define DVBSFEC_REG_BASE            0x2800    
#define _REG_DVBSFEC(idx)             (DVBSFEC_REG_BASE + (idx)*2)
#define DVBSFEC_VITERBI_IQ_SWAP               	            (_REG_DVBSFEC(0x41)+0)  //[2]


//  **********  DVBS2FEC.XLS  **********  //
#define DVBS2FEC_REG_BASE              0x3D00//0x2600
...
#define DVBS2FEC_LDPC_BER_COUNT0                                     	(_REG_DVBS2FEC(0x2C)+0)//(_REG_DVBS2FEC(0x32)+0)
#define DVBS2FEC_LDPC_BER_COUNT1                                     	(_REG_DVBS2FEC(0x2C)+1)//(_REG_DVBS2FEC(0x32)+1)
#define DVBS2FEC_LDPC_BER_COUNT2                                     	(_REG_DVBS2FEC(0x2D)+0)//(_REG_DVBS2FEC(0x33)+0)
#define DVBS2FEC_LDPC_BER_COUNT3                                     	(_REG_DVBS2FEC(0x2D)+1)//(_REG_DVBS2FEC(0x33)+1)
...
// For VCM
static MS_U32 u32DVBS2_DJB_START_ADDR = 0;
static DMD_DVBS_VCM_OPT u8VCM_Enabled_Opt = VCM_Disabled;
static MS_U8 u8Default_VCM_IS_ID = 0;

const MS_U8 modulation_order_array[12] = {2, 3, 4, 5, 3, 4, 5, 6, 6, 6, 7, 8};
...
// enable DISEQC PAD
// [15] reg_allpad_in
// `RIU_W((`RIUBASE_CHIP>>1)+7'h50, 2'b10, 16'h0000);
// [0] reg_if_agc_en
// [1] reg_rf_agc_en
// [2] reg_diseq_in_en = 1'b1
// [3] reg_diseq_out_en = 1'b1
// `RIU_W((`RIUBASE_CHIP>>1)+7'h1d, 2'b01, 16'h000c);
...
// `RIU_W((`RIUBASE_CLKGEN1>>1)+7'h00, 2'b11, 16'h0505);
// wriu 0x103301 0x05
// wriu 0x103300 0x05
...
//               10:  1'b0
//               11:  DFT_CLK  
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h11, 2'b11, 16'h0000);
HAL_DMD_RIU_WriteByte(0x102023, 0x11);
...
//               011:  mpll_clk18_buf
//               100:  clk_dvbtc_outer2x_c_p  
// [12:8] : reg_ckg_dvbs2_demap
//         DVBS2 demap clock control register;
//         [0]=1:gate clock, 
//         [1]=1:invert clock. 
//         [4:2]: clock rate sel.
//         0: mpll_clk216_buf
...
//         4: mpll_clk72_buf
//         5: adc_clk_buf
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h19, 2'b11, 16'h0008);
HAL_DMD_RIU_WriteByte(0x102033, 0x11);
HAL_DMD_RIU_WriteByte(0x102032, 0x11);

// [4:0]: reg_ckg_dvbs2_oppro
//         DVBS2 OPPRO clock control register;
//         [0]=1:gate clock, 
//         [1]=1:invert clock. 
//         [4:2]: clock rate sel.
//         0: mpll_clk216_buf
...
//         DVBTM ts_in sram share clock control register;
//         [0]=1:gate clock, 
//         [1]=1:invert clock. 
//         [4:2]: clock rate sel.
//         0: clk_dvbs_rs_p
//         1: mpll_clk48_buf
//         2: mpll_clk43_buf
//         3: clk_dvbs_outer1x_pre_mux4
//         4: clk_dvbs2_oppro_pre_mux4
//         5: clk_dvbtc_outer2x_c_p
//         6: adc_clk_buf
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h1a, 2'b11, 16'h1800);
HAL_DMD_RIU_WriteByte(0x102035, 0x11);
HAL_DMD_RIU_WriteByte(0x102034, 0x11);

// @0x3515
// [4:0] : reg_ckg_dvbs2_bch
//         DVBS2 BCH clock control register;
//         [0]=1:gate clock, 
//         [1]=1:invert clock. 
//         [4:2]: clock rate sel.
//         0: mpll_clk216_buf
...
//         4: mpll_clk72_buf
//         5: adc_clk_buf
// [12:8] : reg_ckg_dvbs2_bch_rs_adc
//         [8] : disable clock 
//         [9] : invert clock 
//         [11:10] : Select clock source
//               00:  clk_dvbs2_bch_pre_mux4
//               01:  clk_dvbs_rs_p
//               10:  adc_clk_buf
//               11:  
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h15, 2'b11, 16'h0800);
HAL_DMD_RIU_WriteByte(0x10202b, 0x11);
HAL_DMD_RIU_WriteByte(0x10202a, 0x11);
...
//               10:  mpll_clk18_buf
//               11:  
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h16, 2'b10, 16'h0001);
...
*/
    ...
    //0x30~0x33
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)u32DVBS2_DJB_START_ADDR);
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)(u32DVBS2_DJB_START_ADDR >> 8));
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)(u32DVBS2_DJB_START_ADDR >> 16));
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)(u32DVBS2_DJB_START_ADDR >> 24));

    DBG_INTERN_DVBS( ULOGD("DEMOD", "@@@@@ share dram address = 0x %x \n ",u32DVBS2_DJB_START_ADDR) );
   //=====================================================================
#endif
/*
    //0x30~0x33
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)(0x11) );
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)(0x22) );
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)(0x33) );
    HAL_DMD_RIU_WriteByte(DMDMcuBase+0x0C, (MS_U8)(0x44) );
*/
...
//               11:  DFT_CLK  
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h11, 2'b11, 16'h0000);
  HAL_DMD_RIU_WriteByte(0x102023, 0x00);
  HAL_DMD_RIU_WriteByte(0x102022, 0x00);
...
//               011:  1'b0
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h12, 2'b11, 16'h0000);
  HAL_DMD_RIU_WriteByte(0x102025, 0x00);
...
//         5: adc_clk_buf
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h19, 2'b11, 16'h0008);
  HAL_DMD_RIU_WriteByte(0x102033, 0x00);
...
//         6: adc_clk_buf
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h1a, 2'b11, 16'h1800);
  HAL_DMD_RIU_WriteByte(0x102035, 0x18);
  HAL_DMD_RIU_WriteByte(0x102034, 0x00);
...
//               11:  
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h15, 2'b11, 16'h0800);
  HAL_DMD_RIU_WriteByte(0x10202b, 0x08);
  HAL_DMD_RIU_WriteByte(0x10202a, 0x00);
...
//               11:  
  // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h16, 2'b10, 16'h0001);
  HAL_DMD_RIU_WriteByte(0x10202d, 0x00);
...
//               10:  xtali_clk6_buf
//               11:  xtali_clk3
// `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h13, 2'b11, 16'h0010);
HAL_DMD_RIU_WriteByte(0x102027, 0x00);
...
// ===============================================================
// Select reg_DMDTOP and reg_DMDANA are controlled by DMD MCU 
// ===============================================================
// [8] : reg_chiptop_dummy_0 (reg_dmdtop_dmd_sel)
//       1'b0->reg_DMDTOP control by HK_MCU.
//       1'b1->reg_DMDTOP control by DMD_MCU.
...
//       1'b1->reg_DMDANA control by DMD_MCU.
// ("select DMD MCU ......");
// `RIU_W((`RIUBASE_CHIP>>1)+7'h1c, 2'b10, 16'h0300);  //  select DMD MCU
...
MS_BOOL INTERN_DVBS_Power_On_Initialization(MS_BOOL bRFAGCTristateEnable, MS_U8 u8ADCIQMode, MS_U8 u8PadSel, MS_BOOL bPGAEnable, MS_U8 u8PGAGain, const MS_U8 *u8DMD_DVBS_DSPRegInitExt, MS_U8 u8DMD_DVBS_DSPRegInitSize)
{
    ...
    // Set VCM option
    status &= MDrv_SYS_DMD_VD_MBX_WriteDSPReg(E_DMD_S2_VCM_OPT, u8VCM_Enabled_Opt);  

    if(u8VCM_Enabled_Opt == VCM_Forced_Mode)
    {
        // assign IS-ID
        status &= MDrv_SYS_DMD_VD_MBX_WriteDSPReg(E_DMD_S2_IS_ID, u8Default_VCM_IS_ID);  
    }
...
/************************************************************************************************
  Subject:    Interrupt mode
  Function:   INTERN_DVBS_Demod_Interrupt_Monitor
  Parmeter:   
  Return:      MS_BOOL
  Remark:
*************************************************************************************************/

MS_BOOL INTERN_DVBS_Demod_Interrupt_Monitor(MS_U8* pu8IntType)
{
        MS_U8 u8_interrupt_type = 0;
        MS_BOOL bRet= TRUE;
        MS_U8 u8Data = 0;

        // For VCM
        MS_U8 IS_ID = 0;
        MS_U8 IS_ID_Table[32];
    
	//MS_U8 u8TsDiv = 1;
	//MS_FLOAT temp = 0;

	DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_Demod_Interrupt_Monitor\n"));

	bRet &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_LOCK_COUNT, &u8Data);
	//bRet &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_SYSTEM_TYPE, &u8Data);
        ...
        switch(u8Data)
        {
            case 0: // DVBS2
            {
               ...
               _bDemodType = TRUE;    //S2
                ...
                if( u8VCM_Enabled_Opt == VCM_MODE && INTERN_DVBS2_VCM_CHECK() )
                {
                    INTERN_DVBS2_Get_IS_ID_INFO(&IS_ID, IS_ID_Table);
                    INTERN_DVBS2_Set_Default_IS_ID(&IS_ID, IS_ID_Table);

                    ULOGD("DEMOD",">>>INTERN_DVBS_Demod VCM Default IS ID = %d<<<\n", IS_ID);                
                }
               
                DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_Demod DVBS2 Interrupt Lock<<<\n"));
            ...
            case 1: // DVBS
            {
                u8_interrupt_type = 1;                
                _bDemodType = FALSE;   //S
                DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_Demod DVBS Interrupt Lock<<<\n"));
                //For Auto Test
                DBG_INTERN_DVBS(ULOGD("DEMOD","@INTERN_DVBS_Demod Lock+++\n"));
                u8DemodLockFlag = 1;
            }
            ...
            {
                u8_interrupt_type = 2;
                DBG_INTERN_DVBS(ULOGD("DEMOD","@INTERN_DVBS_Demod Interrupt UnLock---\n"));
                u8DemodLockFlag = 0;
                _bTSDataSwap = FALSE;                
            }
            break;
            ...
            {
                u8_interrupt_type = 0;
                DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_Demod_Interrupt_Monitor Error<<<\n"));                
            }
            break;
        }

#if 0
	if((u8Data==0x00) || (u8Data==0x01))//lock
	{
        u8_interrupt_type = 1;
        if(u8Data==0x01)
        {
        ...
        INTERN_DVBS_GetTsDivNum(&temp);  //ts_div_num
        u8TsDiv = temp;
        DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_GetLock TsClkDivNum = 0x%x<<<\n", u8TsDiv));
        if (u8TsDiv > 0x1F)
            u8TsDiv=0x1F;
        HAL_DMD_RIU_WriteByte(0x103300, u8TsDiv);       
        //Ts Output Enable
    ...
    else if(u8Data==0x03)//update TS
    {
        u8_interrupt_type = 3;
        INTERN_DVBS_GetTsDivNum(&temp);  //ts_div_num
        u8TsDiv = temp;
        DBG_INTERN_DVBS(ULOGD("DEMOD",">>>INTERN_DVBS_Demod Interrupt TsClkDivNum = 0x%x<<<\n", u8TsDiv));
        if (u8TsDiv > 0x1F)
            u8TsDiv=0x1F;
        HAL_DMD_RIU_WriteByte(0x103300, u8TsDiv);
    }
...
MS_BOOL INTERN_DVBS_GetTsDivNum(MS_U32 *u32SymbolRate, MS_U8* system_type_reg, MS_U8 *code_rate_idx, MS_U8 *fec_type_idx, MS_U8 *pilot_flag, MS_U32 *u32temp, MS_U8 *code_rate_reg)
{
    ...
    //float       fSymbolRate;
    //MS_U8 ISSY_EN = 0;
    //MS_U8 code_rate_idx_temp = 0;
    //MS_U8 pilot_flag_temp = 0;
    //MS_U8 fec_type_idx_temp = 0;
    ...
    MS_U32 current_time = 0;
    MS_U8 VCM_OPT = VCM_Disabled;
...
//     DMD_DVBS_MODULATION_TYPE pQAMMode;
    ...
    status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_SYSTEM_TYPE, &u8Data);//V
    ...
    //ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum GetCurrentDemodType E_DMD_S2_SYSTEM_TYPE=%d\n", u8Data);
    ...
    *system_type_reg = u8Data;
    if(!u8Data)//DVBS2
    {
        ...
        if( INTERN_DVBS2_VCM_CHECK() ) // VCM signal
        {
            // wait current IS ID which we want to get
            current_time = MsOS_GetSystemTime();
            do
            {
                    status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_VCM_OPT, &VCM_OPT);
            }
            while( VCM_OPT != VCM_Forced_Mode && (MsOS_GetSystemTime() - current_time) < 500);
        }
        
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_CODE_RATE, code_rate_idx);
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_FEC_TYPE, fec_type_idx);
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_MOD_TYPE, &mod_type_idx);
        modulation_order = modulation_order_array[mod_type_idx];
        //modulation_order = mod_type_idx;
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_PILOT_FLAG, pilot_flag);
    ...
    else                                            //S
    {
        ...
        //DVBS Code Rate
        //switch (u8_gCodeRate)
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_CODE_RATE, &u8Data);
	 *code_rate_reg=u8Data;
        switch (u8Data)
        {
            ...
            case 0x01: //CR 2/3
               ...
               DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_GetTsDivNum INTERN_DVBS_GetCurrentCodeRate=2_3\n"));
                  /*
		    if(sDMD_DVBS_Info.bSerialTS)
...
MS_BOOL INTERN_DVBS_GetLock(DMD_DVBS_GETLOCK_TYPE eType, MS_U16 fCurrRFPowerDbm, MS_U16 fNoChannelRFPowerDbm, MS_U32 u32TimeInterval)
{
    ...
    switch( eType )
    {
        case DMD_DVBS_GETLOCK:
            ...
            INTERN_DVBS_info();
#endif
            bRet &= MDrv_SYS_DMD_VD_MBX_ReadReg((TOP_WR_DBG_90), &u8Data);
            ...
            if ((u8Data&0x02)==0x00)//manual mode
            {
                ...
                //ULOGD("DEMOD",">>>INTERN_DVBS_GetLock MailBox state=%d<<<\n", u8Data);
...
MS_BOOL INTERN_DVBS2_Set_IS_ID(MS_U8 u8IS_ID)
{
    MS_U8 VCM_OPT = 0;
    MS_U32 current_time = 0;
    MS_BOOL   status = TRUE;
    ...
    // assign IS-ID
    status &= MDrv_SYS_DMD_VD_MBX_WriteDSPReg(E_DMD_S2_IS_ID, u8IS_ID);
    ...
    while(VCM_OPT != 1 && (MsOS_GetSystemTime() - current_time) < 100);
    
    return status;
}

MS_BOOL INTERN_DVBS2_Set_Default_IS_ID(MS_U8 *u8IS_ID, MS_U8 *u8IS_ID_table)
{
    MS_U8 VCM_OPT = 0;
    MS_U32 current_time = 0;
    MS_BOOL status = TRUE;

    MS_U8 Default_IS_ID = 0;
    MS_U8 iter;
    MS_U8 temp, convert_counter;
    ...
    // assign IS-ID
    status &= MDrv_SYS_DMD_VD_MBX_WriteDSPReg(E_DMD_S2_IS_ID, Default_IS_ID);
...
MS_BOOL INTERN_DVBS2_Get_IS_ID_INFO(MS_U8 *u8IS_ID, MS_U8 *u8IS_ID_table)
{
    MS_BOOL   status = TRUE;
    MS_U8 iter;

    // get IS-ID
    status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_IS_ID, u8IS_ID);
    // get IS-ID table
    for(iter = 0; iter <= 0x0F;++iter)
    {
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_IS_ID_TABLE + 2*iter, &u8IS_ID_table[2*iter]);   
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_IS_ID_TABLE + 2*iter + 1, &u8IS_ID_table[2*iter + 1]);   
    }
    
    return status;    
}

MS_BOOL INTERN_DVBS2_VCM_INIT(DMD_DVBS_VCM_OPT u8VCM_OPT, MS_U8 u8IS_ID, MS_U32 u32DMD_DVBS2_DJB_START_ADDR)
{
    MS_BOOL   status = TRUE;
    ...
    {
        // assign IS-ID
        u8Default_VCM_IS_ID = u8IS_ID;
        // assign DJB address
        u32DVBS2_DJB_START_ADDR = u32DMD_DVBS2_DJB_START_ADDR;
    }
  
    DBG_INTERN_DVBS(ULOGD("DEMOD","-->VCM_OPT<--%d\n", u8VCM_OPT));
    DBG_INTERN_DVBS(ULOGD("DEMOD","-->VCM_ISID<--%d\n", u8IS_ID));
    DBG_INTERN_DVBS(ULOGD("DEMOD","-->VCM_DJB<--%d\n", u32DMD_DVBS2_DJB_START_ADDR));
    
    return status;
}

MS_BOOL INTERN_DVBS2_VCM_CHECK(void)
{
    MS_U8 VCM_Check = 0;
    MS_BOOL   status = TRUE;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2OPPRO_SIS_EN, &VCM_Check);       
...
MS_BOOL INTERN_DVBS2_VCM_ENABLED(MS_U8 u8VCM_ENABLED)
{
    MS_BOOL   status = TRUE;
    // Enabled VCM mode
    status &= MDrv_SYS_DMD_VD_MBX_WriteDSPReg(E_DMD_S2_VCM_OPT, u8VCM_ENABLED);  
    
    return status;
}

MS_BOOL INTERN_DVBS2_VCM_MODE(DMD_DVBS_VCM_OPT u8VCM_OPT)
{
    MS_BOOL   status = TRUE;
    u8VCM_Enabled_Opt = u8VCM_OPT;    
    // Change VCM mode
...
MS_BOOL INTERN_DVBS_GetPostViterbiBer(MS_U32 *BitErr, MS_U16 *BitErrPeriod)//POST BER //V
{
    ...
    status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_SYSTEM_TYPE, &reg);
    ...
    {   
        ...
        do
        {
            status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_LDPC_BER_COUNT3, &reg);        
        }while(reg == 0xFF && (MsOS_GetSystemTime() - current_time) < 1000);

        // freeze outer
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_OUTER_FREEZE, &reg);
        reg |= 0x01;
        status &= MDrv_SYS_DMD_VD_MBX_WriteReg(DVBS2FEC_OUTER_FREEZE, reg);

        // Get LDPC error window

        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_LDPC_ERROR_WINDOW1, &reg);
        *BitErrPeriod = reg;
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_LDPC_ERROR_WINDOW0, &reg);
        *BitErrPeriod = (*BitErrPeriod << 8) | reg;        

        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_LDPC_BER_COUNT3, &reg);
        *BitErr = reg;
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_LDPC_BER_COUNT2, &reg);
        *BitErr = (*BitErr << 8) | reg;
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_LDPC_BER_COUNT1, &reg);
        *BitErr = (*BitErr << 8) | reg;
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_LDPC_BER_COUNT0, &reg);
        *BitErr = (*BitErr << 8) | reg;
    ...
    else //DVBS    
    {
        ...
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBSFEC_REG_BASE+0x08*2, &reg_frz);//h0001    h0001    8    8    reg_ber_en
        status &= MDrv_SYS_DMD_VD_MBX_WriteReg(DVBSFEC_REG_BASE+0x08*2, reg_frz|0x08);
        ...
        *BitErr = (*BitErr << 8)|reg;
        ...
        status &= MDrv_SYS_DMD_VD_MBX_WriteReg(DVBSFEC_REG_BASE+0x08*2, reg_frz);
    }
...
MS_BOOL INTERN_DVBS_GetPacketErr(MS_U16 *pktErr)//V
{
    ...
    if(!u8Data) //DVB-S2
    {
        ...
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_REG_BASE+0x2B*2+1, &u8Data);//DVBS2FEC_BCH_EFLAG2_SUM1 (_REG_DVBS2FEC(0x2B)+1)
        u16PktErr = u8Data;
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2FEC_REG_BASE+0x2B*2, &u8Data);
    ...
    { 
        ...
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBSFEC_REG_BASE+0x1F*2+1, &u8Data);// DVBSFEC_UNCRT_PKT_NUM_15_8    (_REG_DVBSFEC(0x1F)+1)
        u16PktErr = u8Data;
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBSFEC_REG_BASE+0x1F*2, &u8Data);
        u16PktErr = (u16PktErr << 8)|u8Data;
...
MS_BOOL INTERN_DVBS_GetSNR(MS_U32 *u32NDA_SNR_A, MS_U32 *u32NDA_SNR_AB)//V
{
    ...
    */
    ...
    // freeze FW write SNR
    MDrv_SYS_DMD_VD_MBX_ReadReg(FRONTEND_INFO_07, &u8Data);
    u8Data |= 0x01;
    MDrv_SYS_DMD_VD_MBX_WriteReg(FRONTEND_INFO_07, u8Data);

    MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_MB_DMDTOP_INFO_07, &u8Data);
    *u32NDA_SNR_A = u8Data;
    
    MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_MB_DMDTOP_INFO_08, &u8Data);
    *u32NDA_SNR_AB = u8Data;

    // unfreeze FW write SNR
    MDrv_SYS_DMD_VD_MBX_ReadReg(FRONTEND_INFO_07, &u8Data);
    u8Data &= (~0x01);
    MDrv_SYS_DMD_VD_MBX_WriteReg(FRONTEND_INFO_07, u8Data);
...
MS_BOOL INTERN_DVBS_GetSignalStrength(MS_U16 fRFPowerDbm, DMD_DVBS_DEMOD_TYPE *pDemodType, MS_U8  *u8_DVBS2_CurrentCodeRateLocal,  MS_U8   *u8_DVBS2_CurrentConstellationLocal)
{
    ...
    status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_CODE_RATE, &u8Data);
    *u8_DVBS2_CurrentCodeRateLocal = u8Data;
    

    if((MS_U8)*pDemodType == (MS_U8)DMD_SAT_DVBS) // DVBS
    {
        /*
    ...
    else // DVBS2
    {
        ...
        */
        status &= MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_S2_MOD_TYPE, &u8Data);
        *u8_DVBS2_CurrentConstellationLocal = u8Data;
    }
...
MS_BOOL INTERN_DVBS_Get_IQ_MODE(HAL_DEMOD_EN_SAT_IQ_MODE *SAT_IQ_MODE)
{
    MS_BOOL status = true;
    MS_U8 u8Data = 0;

    DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_Get_IQ_MODE\n"));

    if(_bDemodType) // DVBS2
    {    
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FRONTEND_MIXER_IQ_SWAP_OUT, &u8Data);

        if( (u8Data&0x02) == 0x00 )
            *SAT_IQ_MODE = HAL_DEMOD_SAT_IQ_NORMAL;
        else
            *SAT_IQ_MODE = HAL_DEMOD_SAT_IQ_INVERSE;               
    }
    else // DVBS
    {
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBSFEC_VITERBI_IQ_SWAP, &u8Data);
...
MS_BOOL INTERN_DVBS_GetCurrentModulationType(DMD_DVBS_MODULATION_TYPE *pQAMMode)
{
    ...
    else                                        //S2
    {
      ...
      printf("@@@@@  E_DMD_S2_IS_ID=%d  \n",E_DMD_S2_IS_ID);*/
        ...
        // INNER_DEBUG_SEL
        MDrv_SYS_DMD_VD_MBX_ReadReg(INNER_DEBUG_SEL, &u8Data);
        u8Data &= 0xc0;
        MDrv_SYS_DMD_VD_MBX_WriteReg(INNER_DEBUG_SEL, u8Data);
        // reg_plscdec_debug_out
        // PLSCDEC info
        //[0:7] PLSC MODCOD
        //[8:12] modulation_type
        //[13] dummy frame
        //[14] reserved_frame
        //[15] is DVBS2X
        //[16:22] code rate
        //[23] FEC type
        //[24] pilot type
        MDrv_SYS_DMD_VD_MBX_ReadReg(INNER_PLSCDEC_DEBUG_OUT1, &u8Data);
        u16tmp = (MS_U16)(u8Data & 0x1F);
        MOD_type = u16tmp;
        switch(MOD_type)
        {
            case 2:
            ...
            case 5:
            {
                *pQAMMode = DMD_DVBS_32APSK;
                modulation_order=5;
                ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=DVBS2_32APSK\n");//DBG_INTERN_DVBS_LOCK(printf("[dvbs]ModulationType=DVBS2_QPSK\n"));
            }
            break;

            case 6:
            {
                *pQAMMode = DMD_DVBS_8APSK;
                modulation_order=3;
                ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=DVBS2_8APSK\n");//DBG_INTERN_DVBS_LOCK(printf("[dvbs]ModulationType=DVBS2_QPSK\n"));
            }
            break;
            
            case 7:
            {
                *pQAMMode = DMD_DVBS_8_8APSK;
                modulation_order=4;
                ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=DVBS2_8+8APSK\n");//DBG_INTERN_DVBS_LOCK(printf("[dvbs]ModulationType=DVBS2_QPSK\n"));
            }
            break;

            case 8:
            {
                *pQAMMode = DMD_DVBS_4_8_4_16APSK;
                modulation_order=5;
                ULOGD("DEMOD","INTERN_DVBS_GetCurrentModulationType [dvbs2]ModulationType=DVBS2_4+8+4+16APSK\n");//DBG_INTERN_DVBS_LOCK(printf("[dvbs]ModulationType=DVBS2_QPSK\n"));
            }
            break;
...
MS_BOOL INTERN_DVBS_GetCurrentDemodCodeRate(DMD_DVBS_CODE_RATE_TYPE *pCodeRate)
{
    ...
    //if((MS_U8)pDemodType == (MS_U8)DMD_SAT_DVBS2 )  //S2
    {
        ...
        case 0x0B:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_2_9;
            //_u8_DVBS2_CurrentCodeRate = 16;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=2_9\n"));            
        }
        break;   
        case 0x0C:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_13_45;
            //_u8_DVBS2_CurrentCodeRate = 17;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=13_45\n"));            
        }
        break;   
        case 0x0D:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_9_20;
            //_u8_DVBS2_CurrentCodeRate = 18;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=9_20\n"));            
        }
        break;   
        case 0x0E:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_90_180;
            //_u8_DVBS2_CurrentCodeRate = 19;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=90_180\n"));            
        }
        break;   
        case 0x0F:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_96_180;
            //_u8_DVBS2_CurrentCodeRate = 20;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=96_180\n"));            
        }
        break;   
        case 0x10:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_11_20;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=11_20\n"));            
        }
        break;   
        case 0x11:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_100_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=100_180\n"));            
        }
        break;   
        case 0x12:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_104_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=104_180\n"));            
        }
        break;   
        case 0x13:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_26_45_L;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=26_45\n"));            
        }
        break;   
        case 0x14:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_18_30;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=18_30\n"));            
        }
        break;   
        case 0x15:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_28_45;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=28_45\n"));            
        }
        break;   
        case 0x16:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_23_36;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=23_36\n"));            
        }
        break;   
        case 0x17:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_116_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=116_180\n"));            
        }
        break;   
        case 0x18:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_20_30;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=20_30\n"));            
        }
        break;   
        
        case 0x19:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_124_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=124_180\n"));
        }
        break;
        case 0x1A:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_25_36;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=25_36\n"));            
        }
        break;   
        
        case 0x1B:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_128_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=128_180\n"));
        }
        break;
        case 0x1C:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_13_18;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=13_18\n"));            
        }
        break;   
        
        case 0x1D:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_132_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=132_180\n"));
        }
        break;
        case 0x1E:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_22_30;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=22_30\n"));            
        }
        break;   
        
        case 0x1F:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_135_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=135_180\n"));
        }
        break;
        case 0x20:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_140_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=140_180\n"));            
        }
        break;   
        
        case 0x21:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_7_9;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=7_9\n"));
        }
        break;
        case 0x22:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_154_180;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=154_180\n"));            
        }
        break;   
        
        case 0x23:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_11_45;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=11_45\n"));
        }
        break;
        case 0x24:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_4_15;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=4_15\n"));            
        }
        break;   
        
        case 0x25:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_14_45;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=14_45\n"));
        }
        break;
        case 0x26:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_7_15;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=7_15\n"));            
        }
        break;   
        
        case 0x27:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_8_15;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=8_15\n"));
        }
        break;

        case 0x28:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_26_45_S;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=26_45_S\n"));
        }
        break;
            
        case 0x29:
        {
            *pCodeRate = DMD_DVBS2_CODE_RATE_32_45;
            //_u8_DVBS2_CurrentCodeRate = 5;//0;
            DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS2_GetCurrentCodeRate=32_45\n"));
        }
        break;       
...
MS_BOOL INTERN_DVBS_GetRollOff(MS_U8 *pRollOff)
{
    ...
    if(u8Data == 1) // DVBS
    {
        *pRollOff = HAL_DEMOD_SAT_RO_35;  //Rolloff 0.35
    }
    else // DVBS2
    {
        status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2OPPRO_ROLLOFF_DET_DONE, &u8Data);

        if(u8Data)
        {
            status &= MDrv_SYS_DMD_VD_MBX_ReadReg(DVBS2OPPRO_ROLLOFF_DET_VALUE, &u8Data);  

            u8Data = (u8Data & 0x30) >> 4;

            if (u8Data==0x02)
                *pRollOff = HAL_DEMOD_SAT_RO_35;  //Rolloff 0.35
            else if (u8Data==0x01)
                *pRollOff = HAL_DEMOD_SAT_RO_25;  //Rolloff 0.25
            else
                *pRollOff = HAL_DEMOD_SAT_RO_20;  //Rolloff 0.20
...
#if 0
    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(INNER_TR_ROLLOFF, &u8Data);//#define INNER_TR_ROLLOFF (_REG_INNER(0x0F)+0)
    if ((u8Data&0x03)==0x00)
...
**      GAIN & DCR /Fine CFO & PR & IIS & IQB & SNR /PacketErr & BER
****************************************************************************/
MS_BOOL INTERN_DVBS_AGC_Info(MS_U8 u8dbg_mode, MS_U16* pu16Data)
{
    MS_BOOL status = true;
    MS_U8 u8Data;
    MS_U16 u16Data;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FRONTEND_AGC_DEBUG_SEL,&u8Data);
    u8Data = (u8Data & 0xf0) | u8dbg_mode;
    status &= MDrv_SYS_DMD_VD_MBX_WriteReg(FRONTEND_AGC_DEBUG_SEL,u8Data);

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FRONTEND_LATCH,&u8Data);
    u8Data = u8Data | 0x80;
    status &= MDrv_SYS_DMD_VD_MBX_WriteReg(FRONTEND_LATCH,u8Data);

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FRONTEND_AGC_DEBUG_OUT_R1,&u8Data);
    u16Data = u8Data;
    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(FRONTEND_AGC_DEBUG_OUT_R0,&u8Data);
    u16Data = (u16Data<<8) | u8Data;
    ...
    *pu16Data=u16Data;
    ...
    if (status==FALSE)
    {
        DBG_INTERN_DVBS(ULOGD("DEMOD","INTERN_DVBS_AGC_Info  Error!!! \n"));
    }
...
MS_BOOL INTERN_DVBS_BlindScan_Start(MS_U16 u16StartFreq,MS_U16 u16EndFreq)
{
    ...
    MDrv_SYS_DMD_VD_MBX_ReadReg(TOP_WR_DBG_90, &u8Data);
    u8Data &= 0xd0;
    MDrv_SYS_DMD_VD_MBX_WriteReg(TOP_WR_DBG_90, u8Data);
    ...
    *u8Progress=0;
    ...
    else
    {
        ...
        *substate_reg=u8Data;
        ...
        {
            DBG_INTERN_DVBS(ULOGD("DEMOD", "Debug blind scan: TP not found\n"));

            //ULOGD("DEMOD", "Debug blind scan: TP not found\n");
...
*/
   ...
   //add this only for check mailbox R/W
    ...
    MDrv_SYS_DMD_VD_MBX_ReadReg(TOP_WR_DBG_90, &u8Data);//#define TOP_WR_DBG_90                           (_REG_DMDTOP(0x3A)+0)
    //u8Data=u8Data|0x10;
    MDrv_SYS_DMD_VD_MBX_WriteReg(TOP_WR_DBG_90, u8Data|0x10);//enable DiSEqC_Data_Tx
