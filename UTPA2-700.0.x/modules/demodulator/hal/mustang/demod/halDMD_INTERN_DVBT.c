...
static MS_BOOL INTERN_DVBT_LoadDSPCode(void)
{
  ...
  //  MDrv_Sys_DisableWatchDog();
    ...
    HAL_DMD_RIU_WriteByte(DMDMcuBase + 0x00,  0x02);        // reset RIU remapping reset
    HAL_DMD_RIU_WriteByte(DMDMcuBase + 0x00,  0x03);        // reset VD_MCU
...
HAL_DMD_RIU_WriteByte(0x112003, (HAL_DMD_RIU_ReadByte(0x112003)&(~(BIT(5)))));      // Release Ana misc resest
    ...
    HAL_DMD_RIU_WriteByte(0x111f43,0x04);
...
MS_BOOL  INTERN_DVBT_Adaptive_TS_CLK(void)
{
	MS_U8  u8_ts_clk=0x00;
          MS_U8  TS_Clock_Temp;
	u8_ts_clk = HAL_DMD_RIU_ReadByte(0x112615);
	//printf("*************************************************************\n");
...
DMD_LOCK_STATUS INTERN_DVBT_Lock(MS_U16 u16DMD_DVBT_TPS_Timeout, MS_U16 u16DMD_DVBT_FEC_Timeout)
{
	...
	{
		printf("==> INTERN_DVBT_Lock -- E_DMD_COFDM_NO_CHANNEL \n");
		return E_DMD_UNLOCK;
	}
