...
#define     D_DMD_DVBT_CFG_IFAGC_REF_8K                   0x10
...
#define     D_DMD_DVBT_CFG_IFAGC_REF_8K_H                 0x06
...
#define DVBT_FS     48000
#define FC_H        0xA7    // 43000, Fc = Fs - IF = 48000 - 5000 = 43000(0xA7F8)
#define FC_L        0xF8   
    ...
    if (u8DVBT_DSPReg != NULL)
    {
        ...
        if (u8DVBT_DSPReg[0] >= 1)
        {
            ...
            for (idx = 0; idx<u8Size; idx++)
            {
                ...
                u8DVBT_DSPReg++;
                printf("DSP addr:%x mask:%x read:%x write:%x\n", u16DspAddr, u8Mask, u8RegRead, u8RegWrite);
...
void INTERN_DVBT_InitClkgen(MS_BOOL bRFAGCTristateEnable)
{
    ...
    HAL_DMD_RIU_WriteByte(0x103300,0x14);  // arthur 2016.10.17
...
// `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h24, 2'b11, 16'h0000);
// wriu 0x111f49 0x00 // nike
// wriu 0x111f48 0x00 // nike
    ...
    HAL_DMD_RIU_WriteByte(0x111f50,0x88);
    // ----------------------------------------------
    //  start demod CLKGEN setting
    // ----------------------------------------------
    //  select DMD MCU
