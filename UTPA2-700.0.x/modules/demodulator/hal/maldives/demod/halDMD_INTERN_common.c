...
#define RIU_READ_BYTE(addr)         ( READ_BYTE( _hal_DMD.u32DMDBaseAddr + (addr) ) )
#define RIU_READ_2BYTE(addr)        ( READ_WORD( _hal_DMD.u32DMDBaseAddr + (addr) ) )
#define RIU_WRITE_BYTE(addr, val)   { WRITE_BYTE( _hal_DMD.u32DMDBaseAddr + (addr), val) }
#define RIU_WRITE_2BYTE(addr, val)  { WRITE_WORD( _hal_DMD.u32DMDBaseAddr + (addr), val) }
...
MS_BOOL HAL_DMD_RegInit (void)
{
    ...
    if (!MDrv_MMIO_GetBASE( &u32NonPMBank, &u32NonPMBankSize, MS_MODULE_PM))
    {
        #ifdef MS_DEBUG
        printf("HAL_DMD_RegInit failure to get MS_MODULE_PM\n");
        #endif
        _hal_DMD.u32DMDBaseAddr = BASEADDR_RIU; // TODO what to do if failed??
        _hal_DMD.bBaseAddrInitialized = 0;
        return FALSE;
    }

    //HAL_ParFlash_Config(u32NonPMBank);
    _hal_DMD.u32DMDBaseAddr=u32NonPMBank;
