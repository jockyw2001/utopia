...
MS_BOOL INTERN_DVBT_DSPReg_Init(const MS_U8 *u8DVBT_DSPReg,  MS_U8 u8Size)
{
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD","INTERN_DVBT_DSPReg_Init\n"));
    ...
    if (u8DVBT_DSPReg != NULL)
    {
        ...
        if (u8DVBT_DSPReg[0] >= 1)
        {
            ...
            for (idx = 0; idx<u8Size; idx++)
            {
                ...
                u8DVBT_DSPReg++;
                DBG_INTERN_DVBT(ULOGD("DEMOD","DSP addr:%x mask:%x read:%x write:%x\n", u16DspAddr, u8Mask, u8RegRead, u8RegWrite));
...
MS_BOOL INTERN_DVBT_Cmd_Packet_Send(S_CMDPKTREG* pCmdPacket, MS_U8 param_cnt)
{
    ...
    // ==== Command Phase ===================
    DBG_INTERN_DVBT(ULOGD("DEMOD","--->INTERN_DVBT (cmd=0x%x)(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,) \n",pCmdPacket->cmd_code,
    ...
    do
    {
        ...
        if (timeout++ > 200)
        {
            ULOGD("DEMOD","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_END clear' \n");
    ...
    do
    {
        ...
        if (timeout++ > 200)
        {
            ULOGD("DEMOD","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_START clear' \n");
            //MsOS_ReleaseMutex(_s32_Demod_DVBT_Mutex);
            return false;
        }
    } while (1);
    ...
    for (indx = 0; indx < param_cnt; indx++)
    {
        ...
        do
        {
            ...
            if (timeout++ > 200)
            {
                ULOGD("DEMOD","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_DRQ clear' \n");
                //MsOS_ReleaseMutex(_s32_Demod_DVBT_Mutex);
                return false;
            }
        } while (1);
...
{
    DBG_INTERN_DVBT(ULOGD("DEMOD"," @INTERN_DVBT_reset\n"));

    DBG_INTERN_DVBT_TIME(ULOGD("DEMOD","INTERN_DVBT_Reset, t = %ld\n",MsOS_GetSystemTime()));
...
static MS_BOOL INTERN_DVBT_LoadDSPCode(void)
{
  ...
  //  MDrv_Sys_DisableWatchDog();
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD",">Load Code...\n"));
#ifdef INTERN_DVBT_LOAD_FW_FROM_CODE_MEMORY
   ...
   SizeBy4K=BinInfo.B_Len>>12;  //deivde to 4096
...
#if (DBG_DUMP_LOAD_DSP_TIME==1)
    ULOGD("DEMOD","------> INTERN_DVBT Load DSP Time:  (%lu)\n", msAPI_Timer_DiffTimeFromNow(u32Time)) ;
#endif

#endif

    ////  Content verification ////
    DBG_INTERN_DVBT(ULOGD("DEMOD",">Verify Code...\n"));
    ...
    for ( i = 0; i < sizeof(INTERN_DVBT_table); i++)
    {
        ...
        if (udata != INTERN_DVBT_table[i])
        {
            ...
            if (fail_cnt++ > 10)
            {
                ULOGD("DEMOD",">DVB-T DSP Loadcode fail!");
                return false;
            }
        }
    }
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD",">DSP Loadcode done."));
...
{
	HAL_DMD_RIU_WriteByte(0x103c0e,0x00);
	...
	// [Maxim] enable ADCI clock & ADCQ clock
	// h0010	h0010	3	0	reg_ckg_dvbtc_adc_i	3	0	4	h1
	// h0010	h0010	11	8	reg_ckg_dvbtc_adc_q	3	0	4	h1
	// `RIU_W((`RIUBASE_CLKGEN1>>1)+7'h10, 2'b11, 16'h0000);		// enable dvbc adc clock
	// `RIU_W((`RIUBASE_CLKGEN1>>1)+7'h10, 2'b11, 16'h0000);		// enable dvbc adc clock
	//wriu 0x103321 0x00
	//wriu 0x103320 0x00
	HAL_DMD_RIU_WriteByte(0x103321,0x00);
	HAL_DMD_RIU_WriteByte(0x103320,0x00);
    ...
    // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h08, 2'b11, 16'h0000);
    HAL_DMD_RIU_WriteByte(0x111f21,0x44);
    HAL_DMD_RIU_WriteByte(0x111f20,0x40);
    ...
    HAL_DMD_RIU_WriteByte(0x111f2c,0x41);


    HAL_DMD_RIU_WriteByte(0x111f2f,0x0c);
    HAL_DMD_RIU_WriteByte(0x111f2e,0x04);


    HAL_DMD_RIU_WriteByte(0x111f31,0x00);
    HAL_DMD_RIU_WriteByte(0x111f30,0x04);

	HAL_DMD_RIU_WriteByte(0x111f33,0x3c);
	HAL_DMD_RIU_WriteByte(0x111f32,0x00);


    HAL_DMD_RIU_WriteByte(0x111f35,0x10);
    HAL_DMD_RIU_WriteByte(0x111f34,0x10);
    ...
    HAL_DMD_RIU_WriteByte(0x111f3d,0x0c);
    HAL_DMD_RIU_WriteByte(0x111f3c,0x04);
    
    HAL_DMD_RIU_WriteByte(0x111f45,0x04);
    HAL_DMD_RIU_WriteByte(0x111f44,0x44);    
    
    HAL_DMD_RIU_WriteByte(0x111f69,0x00);
    HAL_DMD_RIU_WriteByte(0x111f68,0x00);    
    
    HAL_DMD_RIU_WriteByte(0x111f6b,0x00);
    HAL_DMD_RIU_WriteByte(0x111f6a,0x00);
    
    HAL_DMD_RIU_WriteByte(0x111f6d,0x00);    
    HAL_DMD_RIU_WriteByte(0x111f6c,0x10);    

    HAL_DMD_RIU_WriteByte(0x111f6f,0x0c);    
    HAL_DMD_RIU_WriteByte(0x111f6e,0x40);  	
    ...
    HAL_DMD_RIU_WriteByte(0x111f79,0x40);    
    HAL_DMD_RIU_WriteByte(0x111f78,0x00);  	

    HAL_DMD_RIU_WriteByte(0x111f7b,0x00);    
    HAL_DMD_RIU_WriteByte(0x111f7a,0x04);  	

    HAL_DMD_RIU_WriteByte(0x111f7d,0x00);    
    HAL_DMD_RIU_WriteByte(0x111f7c,0x00);  	

    HAL_DMD_RIU_WriteByte(0x111f7f,0x40);    
    HAL_DMD_RIU_WriteByte(0x111f7e,0x40);  	

    HAL_DMD_RIU_WriteByte(0x111fe1,0x04);    
    HAL_DMD_RIU_WriteByte(0x111fe0,0x04);  	

    HAL_DMD_RIU_WriteByte(0x111ff0,0x04);    

    HAL_DMD_RIU_WriteByte(0x111fe3,0x04);    
    HAL_DMD_RIU_WriteByte(0x111fe2,0x0c);  	
    ...
    HAL_DMD_RIU_WriteByte(0x111fe9,0x04);    
    HAL_DMD_RIU_WriteByte(0x111fe8,0x0c);  	

    HAL_DMD_RIU_WriteByte(0x111feb,0x88);    
    HAL_DMD_RIU_WriteByte(0x111fea,0x00);  	
	
    HAL_DMD_RIU_WriteByte(0x111fed,0x00);    
    HAL_DMD_RIU_WriteByte(0x111fec,0x08);  

    HAL_DMD_RIU_WriteByte(0x111fef,0x00);    
    HAL_DMD_RIU_WriteByte(0x111fee,0x88);  	

    HAL_DMD_RIU_WriteByte(0x15298f,0x00);    
    HAL_DMD_RIU_WriteByte(0x15298e,0x00); 

    HAL_DMD_RIU_WriteByte(0x152991,0x00);    
    HAL_DMD_RIU_WriteByte(0x152990,0x00);  
    HAL_DMD_RIU_WriteByte(0x152992,0x00);    

    HAL_DMD_RIU_WriteByte(0x1529e5,0x00);   
    HAL_DMD_RIU_WriteByte(0x1529e4,0x00);   	


    HAL_DMD_RIU_WriteByte(0x152971,0x10);   
    HAL_DMD_RIU_WriteByte(0x152970,0x01);   	

    HAL_DMD_RIU_WriteByte(0x111f43,0x40);   	
    HAL_DMD_RIU_WriteByte(0x111f42,0x04);   
...
MS_BOOL INTERN_DVBT_Power_On_Initialization(MS_BOOL bRFAGCTristateEnable, MS_U8 u8ADCIQMode, MS_U8 u8PadSel, MS_BOOL bPGAEnable, MS_U8 u8PGAGain, const MS_U8 *u8DMD_DVBT_DSPRegInitExt, MS_U8 u8DMD_DVBT_DSPRegInitSize)
{
    ...
    MS_U8   cData = 0;
    //U8            cal_done;
    DBG_INTERN_DVBT(ULOGD("DEMOD","INTERN_DVBT_Power_On_Initialization\n"));
    ...
    //// Firmware download //////////
    DBG_INTERN_DVBT(ULOGD("DEMOD","INTERN_DVBT Load DSP...\n"));
    //MsOS_DelayTask(100);
    ...
    //if (HAL_DMD_RIU_ReadByte(0x101E3E) != 0x02) // DVBT = BIT1 -> 0x02
    {
        ...
        {
            ULOGD("DEMOD","DVB-T Load DSP Code Fail\n");
            return FALSE;
        }
        else
        {
            DBG_INTERN_DVBT(ULOGD("DEMOD","DVB-T Load DSP Code OK\n"));
        }
    }


    //// MCU Reset //////////
    DBG_INTERN_DVBT(ULOGD("DEMOD","INTERN_DVBT Reset...\n"));
    if (INTERN_DVBT_Reset() == FALSE)
    {
        DBG_INTERN_DVBT(ULOGD("DEMOD","Fail\n"));
        return FALSE;
    }
    else
    {
        DBG_INTERN_DVBT(ULOGD("DEMOD","OK\n"));
    }
...
void INTERN_DVBT_Driving_Control(MS_BOOL bEnable)
{
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD","---> INTERN_DVBT_Driving_Control(Bit0) = 0x%x \n",u8Temp));
...
void INTERN_DVBT_Clk_Inversion_Control(MS_BOOL bInversionEnable)
{
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD","---> Inversion(Bit9) = 0x%x \n",u8Temp));
...
MS_BOOL INTERN_DVBT_Serial_Control(MS_BOOL bEnable, MS_U8 u8TSClk)
{
    MS_U8   status = true;
    MS_U8 temp_val;
    DBG_INTERN_DVBT(ULOGD("DEMOD"," @INTERN_DVBT_ts... u8TSClk=%d\n",u8TSClk));
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD","---> Inversion(Bit5) = 0x%x \n",gsCmdPacket.param[1] ));
...
{
    DBG_INTERN_DVBT(ULOGD("DEMOD"," @INTERN_DVBT_TS1_Enable... \n"));
...
MS_BOOL INTERN_DVBT_Config ( DMD_RF_CHANNEL_BANDWIDTH BW, MS_BOOL bSerialTS, MS_BOOL bPalBG, MS_BOOL bLPSel, MS_U8 u8TSClk, MS_U32 u32IFFreq, MS_U32 u32FSFreq, MS_U8 u8IQSwap)
{
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD"," @INTERN_DVBT_config %d %d %d %d %d %ld %ld %d\n", BW, bSerialTS, bPalBG, bLPSel, u8TSClk, u32IFFreq, u32FSFreq, u8IQSwap));
    DBG_INTERN_DVBT_TIME(ULOGD("DEMOD","INTERN_DVBT_Config, t = %ld\n",MsOS_GetSystemTime()));
...
MS_BOOL INTERN_DVBT_Active(MS_BOOL bEnable)
{
    ...
    DBG_INTERN_DVBT(ULOGD("DEMOD"," @INTERN_DVBT_active\n"));
...
MS_BOOL  INTERN_DVBT_Adaptive_TS_CLK(void)
{
	MS_U8  u8_ts_clk=0x00;
  MS_U8  TS_Clock_Temp;
	u8_ts_clk = HAL_DMD_RIU_ReadByte(MBRegBase+0x15);
	//printf("*************************************************************\n");
	//printf("      The TS clock: %x\n",u8_ts_clk);

	//reg_atsc_dvb_div_reset =1
	TS_Clock_Temp=HAL_DMD_RIU_ReadByte(DMD_CLK_GEN+0x02);
	TS_Clock_Temp=TS_Clock_Temp|0x01;
	HAL_DMD_RIU_WriteByte(DMD_CLK_GEN+0x02,TS_Clock_Temp);
				
	//set TS clock source div 5
	TS_Clock_Temp=HAL_DMD_RIU_ReadByte(DMD_CLK_GEN+0x01);
	TS_Clock_Temp=(TS_Clock_Temp&(~0x01));
	HAL_DMD_RIU_WriteByte(DMD_CLK_GEN+0x01,TS_Clock_Temp);			

	//set ts clk, REG_BASE[TOP_CKG_DVBTM_TS + 1] = TS_Clock_Set;	
	TS_Clock_Temp=HAL_DMD_RIU_ReadByte(DMD_CLK_GEN);
	TS_Clock_Temp=(TS_Clock_Temp&0xE0) |u8_ts_clk;
	HAL_DMD_RIU_WriteByte(DMD_CLK_GEN,TS_Clock_Temp);
  ...
  // set ts FIFO
	...
	MDrv_SYS_DMD_VD_MBX_WriteReg(BACKEND_REG_BASE + (0x16*2+1), 0x15) ;						   

  // enable ts
	MDrv_SYS_DMD_VD_MBX_ReadReg(BACKEND_REG_BASE + (0x10*2), &TS_Clock_Temp) ;
	TS_Clock_Temp=TS_Clock_Temp|0x01;
	MDrv_SYS_DMD_VD_MBX_WriteReg(BACKEND_REG_BASE + (0x10*2), TS_Clock_Temp) ;

  //debug: re-check ts clock
	//TS_Clock_Temp=HAL_DMD_RIU_ReadByte(DMD_CLK_GEN);
	//TS_Clock_Temp=(TS_Clock_Temp&0x1F) ;
	//printf("-------------------------------------------------------\n");
	//printf("      System report:  %x\n",TS_Clock_Temp);   			
	//printf("*************************************************************\n");

    return TRUE;
}

MS_BOOL  INTERN_DVBT_Locked_Task(void)
{
	INTERN_DVBT_Adaptive_TS_CLK();
...
DMD_LOCK_STATUS INTERN_DVBT_Lock(MS_U16 u16DMD_DVBT_TPS_Timeout, MS_U16 u16DMD_DVBT_FEC_Timeout)
{
    ...
    if (INTERN_DVBT_GetLock(E_DMD_COFDM_FEC_LOCK))
    {
        if (bFECLock ==  FALSE)
        {
            ...
            DBG_INTERN_DVBT(ULOGD("DEMOD","++++++++[utopia]dvbt lock\n"));
        }
/*bryan temp mark*/
/*
        if(INTERN_DVBT_GetPostViterbiBer(&fBER) == TRUE)
        {
            ...
            DBG_INTERN_DVBT(ULOGD("DEMOD","[dvbt]f_ber=%8.3e, g_viter_ber=%8.3e\n",fBER,fViterbiBerFiltered));
        }
*/
	...
	if (INTERN_DVBT_GetLock(E_DMD_COFDM_NO_CHANNEL))
	{
		ULOGD("DEMOD","==> INTERN_DVBT_Lock -- E_DMD_COFDM_NO_CHANNEL \n");
		return E_DMD_UNLOCK;
	}
    ...
    if(!bTPSLock)
    {
        ...
        {
            DBG_INTERN_DVBT(ULOGD("DEMOD","==> INTERN_DVBT_Lock -- TPSLock \n"););
            bTPSLock = TRUE;
        }
    }
    if(bTPSLock)
    {
        DBG_INTERN_DVBT(ULOGD("DEMOD","TPSLock %ld\n",MsOS_GetSystemTime()));
...
MS_BOOL INTERN_DVBT_GetLock(DMD_DVBT_GETLOCK_TYPE eStatus)
{
    ...
    switch( eStatus )
    {
        ...
        #ifdef SUPPORT_ADAPTIVE_TS_CLK
        MDrv_SYS_DMD_VD_MBX_ReadReg(MBX_REG_BASE+0x16, &lock_to_unlock_flag);
        if (((cData == 0x0B) &&  (bFECLock ==  FALSE)) ||((cData == 0x0B) && ((lock_to_unlock_flag & 0x01)==0x01))   )
	      {
	      //	printf("Support adaptive TS CLK in polling mode! \n");
		    INTERN_DVBT_Locked_Task();
		    if((lock_to_unlock_flag & 0x01)==0x01)
		    {
        MDrv_SYS_DMD_VD_MBX_WriteReg(MBX_REG_BASE+0x16,0x00);
		    }
        }
        ...
        case E_DMD_COFDM_DCR_LOCK:
            u16Address =  0x2737; //DCR Lock,
            cBitMask = BIT(0);
            break;

        case E_DMD_COFDM_AGC_LOCK:
            u16Address =  0x271D; //AGC Lock,
            cBitMask = BIT(0);
            break;
...
MS_BOOL INTERN_DVBT_GetPostViterbiBer(MS_U16 *BitErrPeriod_reg, MS_U32 *BitErr_reg, MS_U16 *PktErr_reg)
{
    MS_BOOL            status = true;
...
void INTERN_DVBT_GetSNR (MS_U32 *noise_power_reg)
{
    ...
    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(ISDBT_FDPE_REG_BASE + 0x5B, &reg);
    noise_power = reg & 0x07;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(ISDBT_FDPE_REG_BASE + 0x5A, &reg);
    noise_power = (noise_power << 8)|reg;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(ISDBT_FDPE_REG_BASE + 0x59, &reg);
    noise_power = (noise_power << 8)|reg;

    status &= MDrv_SYS_DMD_VD_MBX_ReadReg(ISDBT_FDPE_REG_BASE + 0x58, &reg);
    noise_power = (noise_power << 8)|reg;
...
MS_BOOL INTERN_DVBT_GetSignalStrength(MS_U16 *strength,const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    DBG_INTERN_DVBT_TIME(ULOGD("DEMOD","INTERN_DVBT_GetSignalStrength, t=%ld\n",MsOS_GetSystemTime()));
    ...
    if(INTERN_DVBT_Get_TPS_Parameter_Const(&tps_info_qam, TS_MODUL_MODE) == FALSE)
        ULOGD("DEMOD","[dvbt]TPS qam parameter retrieve failure\n");

    if(INTERN_DVBT_Get_TPS_Parameter_Const(&tps_info_cr, TS_CODE_RATE) == FALSE)
        ULOGD("DEMOD","[dvbt]TPS cr parameter retrieve failure\n");
    ...
    DBG_GET_SIGNAL(ULOGD("DEMOD",">>> SSI_CH_PWR(dB) = %f , Score = %d<<<\n", ch_power_db, *strength));
    DBG_GET_SIGNAL(ULOGD("DEMOD",">>> SSI = %d <<<\n", (int)*strength));
...
MS_BOOL INTERN_DVBT_GetSignalQuality(MS_U16 *quality, const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    DBG_INTERN_DVBT_TIME(ULOGD("DEMOD","INTERN_DVBT_GetSignalQuality, t=%ld\n",MsOS_GetSystemTime()));
    ...
    if (TRUE == INTERN_DVBT_GetLock(E_DMD_COFDM_PSYNC_LOCK) )
    {
        ...
        if(fViterbiBerFiltered<= 0.0)
        {
            ...
            {
                DBG_INTERN_DVBT(ULOGD("DEMOD","GetPostViterbiBer Fail!\n"));
                return FALSE;
            }
            fViterbiBerFiltered = fber;
        }
    ...
    DBG_GET_SIGNAL(ULOGD("DEMOD","SNR = %f, QAM = %d, code Rate = %d\n", cn_rec, tps_cnstl, tps_cr));
    DBG_GET_SIGNAL(ULOGD("DEMOD","BER = %8.3e\n", fber));
    DBG_GET_SIGNAL(ULOGD("DEMOD","Signal Quility = %d\n", *quality));
    return status;
}
#endif
...
MS_BOOL INTERN_DVBT_Get_FreqOffset(float *pFreqOff, MS_U8 u8BW)
{
    ...
    *pFreqOff = (-1.0f)*(FreqIcfo + (FreqCfoFd + FreqCfoTd)/1000);
    // DBG_GET_SIGNAL(ULOGD("DEMOD","FCFO = %f\n", FreqCfoFd));
    // DBG_GET_SIGNAL(ULOGD("DEMOD","TCFO = %f\n", FreqCfoTd));
    // DBG_GET_SIGNAL(ULOGD("DEMOD","ICFO = %f\n", FreqIcfo));
    DBG_GET_SIGNAL(ULOGD("DEMOD","CFOE = %f\n", *pFreqOff));
...
MS_BOOL INTERN_DVBT_Show_SW_ChannelLength(void)
{
    ...
    ULOGD("DEMOD","[dvbt]SW_len=%d, peak_num=%d, insideGI=%d, stoptrack=%d, short_echo=%d, fsa_mode=%d\n",
...
MS_BOOL INTERN_DVBT_Show_ACI_CI(void)
{
    ...
    ULOGD("DEMOD","[dvbt]DigACI=%d, Flag_CI=%d, td_coef=%d\n",digACI,flag_CI,td_coef);
...
MS_BOOL INTERN_DVBT_Show_FD_CH_LEN_S_SEL(void)
{
    ...
    ULOGD("DEMOD","[dvbt]fd=0x%x, ch_len=0x%x, snr_sel=0x%x, pertone_num=0x%x\n",fd,ch_len,snr_sel,pertone_num);
...
MS_BOOL INTERN_DVBT_Get_CFO(void)
{
    ...
    ULOGD("DEMOD","[CFO]t_cfo=%f Hz, f_cfo=%f Hz, icfo=%f KHz, cfo=%f KHz\n", FreqCfoTd,FreqCfoFd,FreqIcfo,total_cfo);
...
MS_BOOL INTERN_DVBT_Get_SFO(void)
{
    ...
    // ULOGD("DEMOD","\nReg_FSA_SFO = 0x%x\n",Reg_FSA_SFO);
    ULOGD("DEMOD","[SFO]tdp_sfo=%f, fdp_sfo=%f, fsa_sfo=%f, Tot_sfo=%f, fsa_sfo_in=%f\n",Float_TDP_SFO,Float_FDP_SFO,Float_FSA_SFO,sfo_value,Float_FSA_IN);
...
void INTERN_DVBT_Get_SYA_status(void)
{
    ...
    ULOGD("DEMOD","[SYA][1]sya_k = 0x%x, sya_th = 0x%x, sya_offset=0x%x\n",sya_k,sya_th,sya_offset);
    ULOGD("DEMOD","[SYA][2]track_reg=0x%x, len_m = %d, len_e = %d [%d,%d]\n",tracking_reg,len_m,len_b-len_a,len_a,len_b);
...
void INTERN_DVBT_Get_cci_status(void)
{
    ...
    ULOGD("DEMOD","[CCI]fsweep=0x%x, k=0x%x\n",cci_fsweep,cci_kp);
...
MS_BOOL INTERN_DVBT_Get_Lock_Time_Info(MS_U16 *locktime)
{
    ...
    *locktime = 0xffff;
    ULOGD("DEMOD","[dvbt]INTERN_DVBT_Get_Lock_Time_Info not implement\n");
...
MS_BOOL INTERN_DVBT_Show_Lock_Time_Info(void)
{
    ...
    ULOGD("DEMOD","[DVBT]lock_time = %d ms\n",locktime);
    return status;
}
...
MS_BOOL INTERN_DVBT_Show_AGC_Info(void)
{
    ...
    ULOGD("DEMOD","[dvbt]agc_k=0x%x, agc_ref=0x%x, d1_k=0x%x, d1_ref=0x%x, d2_k=0x%x, d2_ref=0x%x\n",
        agc_k,agc_ref,d1_k,d1_ref,d2_k,d2_ref);

    ULOGD("DEMOD","[dvbt]agc_g=0x%x, d1_g=0x%x, d2_g=0x%x, agc_err=0x%x\n",if_agc_gain,d1_gain,d2_gain,if_agc_err);
    ULOGD("DEMOD","[dvbt]agc_lock=0x%x, d1_lock=0x%x, d2_lock=0x%x\n",agc_lock,d1_lock,d2_lock);
...
MS_BOOL INTERN_DVBT_Show_WIN_Info(void)
{
    ...
    ULOGD("DEMOD","[dvbt]win_len = %d, trigger=0x%x\n",win_len,trigger);
...
void INTERN_DVBT_Show_td_coeff(void)
{
    ...
    ULOGD("DEMOD","[td]w1=0x%x, w2=0x%x\n",w1,w2);
...
MS_BOOL INTERN_DVBT_Show_Modulation_info(void)
{
    ...
    // ULOGD("DEMOD","[DVBT]TPS info, freq=%ld  ",CurRFParam.RfFreqInKHz);
    ...
    if(INTERN_DVBT_Get_TPS_Info( &tps_info))
    {
        ...
        {
            case 0:
                ULOGD("DEMOD","HP_CR=1/2, ");
                break;
            case 1:
                ULOGD("DEMOD","HP_CR=2/3, ");
                break;
            case 2:
                ULOGD("DEMOD","HP_CR=3/4, ");
                break;
            case 3:
                ULOGD("DEMOD","HP_CR=5/6, ");
                break;
            case 4:
                ULOGD("DEMOD","HP_CR=7/8, ");
                break;
            default:
                ULOGD("DEMOD","unknow hp_cr, ");
                break;
        }
        ...
        {
            case 0:
                ULOGD("DEMOD","LP_CR=1/2, ");
                break;
            case 1:
                ULOGD("DEMOD","LP_CR=2/3, ");
                break;
            case 2:
                ULOGD("DEMOD","LP_CR=3/4, ");
                break;
            case 3:
                ULOGD("DEMOD","LP_CR=5/6, ");
                break;
            case 4:
                ULOGD("DEMOD","LP_CR=7/8, ");
                break;
            default:
                ULOGD("DEMOD","unknow lp_cr, ");
                break;
        }

        ULOGD("DEMOD"," Hiearchy=0x%x\n",hiearchy);
...
MS_BOOL INTERN_DVBT_Show_Lock_Info(void)
{
  ...
  ULOGD("DEMOD","[dvbt]INTERN_DVBT_Show_Lock_Info not implement!!!\n");
  return false;
}
