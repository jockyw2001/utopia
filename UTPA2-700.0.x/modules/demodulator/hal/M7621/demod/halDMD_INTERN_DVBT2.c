...
#define AUTO_TS_DATA_RATE 1

#if(AUTO_TS_DATA_RATE)
#define DBG_AUTO_TS_DATA_RATE(x)
#endif
...
#if (AUTO_TS_DATA_RATE)
MS_BOOL INTERN_DVBT2_GetTsDivNum(MS_U8* u8TSDivNum);
MS_BOOL INTERN_DVBT2_ConfigAdaptiveTsDivNum(void);
#endif
...
//#ifdef INTERN_DVBT2_LOAD_FW_FROM_CODE_MEMORY
    ...
    else
    {
        ...
        if((u32DMD_DVBT2_FW_START_ADDR & 0x5000) != 0x5000)
        {
            for(i=0; i<50; i++)
        {
            ...
            printf(">>> [INTERN_DVBT2_LoadDSPCode]DVBT2_EQ_START_ADDR=0x%x \n", u32DMD_DVBT2_EQ_START_ADDR);
            printf(">>> [INTERN_DVBT2_LoadDSPCode]DVBT2_TDI_START_ADDR=0x%x \n", u32DMD_DVBT2_TDI_START_ADDR);
            printf(">>> [INTERN_DVBT2_LoadDSPCode]DVBT2_DJB_START_ADDR=0x%x \n", u32DMD_DVBT2_DJB_START_ADDR);
            printf(">>> [INTERN_DVBT2_LoadDSPCode]DVBT2_FW_START_ADDR=0x%x \n", u32DMD_DVBT2_FW_START_ADDR);
        }
        else
        {
...
//    HAL_DMD_RIU_WriteByte(0x111f24,0x00);
    ...
    // 32+4K xdata sram
    HAL_DMD_RIU_WriteByte(0x1117e0,(0x21+INTERNAL_DVBT2_PS2XDATA_LEN-1));
    HAL_DMD_RIU_WriteByte(0x1117e1,0x21);
    HAL_DMD_RIU_WriteByte(0x1117e4,(INTERNAL_DVBT2_PS2XDATA_LEN-2));
    ...
    HAL_DMD_RIU_WriteByte(0x111707,(INTERNAL_DVBT2_DRAM_OFFSET-1)>>8);
    HAL_DMD_RIU_WriteByte(0x111706,(INTERNAL_DVBT2_DRAM_OFFSET-1)&0xff);
    ...
    HAL_DMD_RIU_WriteByte(0x11170d,INTERNAL_DVBT2_DRAM_OFFSET>>8);
    HAL_DMD_RIU_WriteByte(0x11170c,INTERNAL_DVBT2_DRAM_OFFSET&0xff);
...
#if (AUTO_TS_DATA_RATE)
            INTERN_DVBT2_ConfigAdaptiveTsDivNum();
#endif
...
MS_BOOL INTERN_DVBT2_GetLock(DMD_DVBT2_GETLOCK_TYPE eStatus)
{
    ...
    {
#if (AUTO_TS_DATA_RATE)   
        if(eStatus == E_DMD_DVBT2_FEC_LOCK)
            INTERN_DVBT2_ConfigAdaptiveTsDivNum();
#endif
...
MS_BOOL INTERN_DVBT2_GetTsDivNum(MS_U8* u8TSDivNum)
{
    ...
    if (MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_T2_TS_DATA_RATE_3, &u8_tmp) == FALSE)
    {
        DBG_INTERN_DVBT2(printf(">INTERN_DVBT2_GetLock MBX_ReadDspReg fail \n"));
        return FALSE;
    }
    //printf("[dvbt2] TS_DATA_RATE_3 = 0x%x \n\n", u8_tmp);
    TS_DATA_RATE = u8_tmp;

    if (MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_T2_TS_DATA_RATE_2, &u8_tmp) == FALSE)
    {
        DBG_INTERN_DVBT2(printf(">INTERN_DVBT2_GetLock MBX_ReadDspReg fail \n"));
        return FALSE;
    }
    //printf("[dvbt2] TS_DATA_RATE_2 = 0x%x \n\n", u8_tmp);
    TS_DATA_RATE = (TS_DATA_RATE<<8) |u8_tmp;

    if (MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_T2_TS_DATA_RATE_1, &u8_tmp) == FALSE)
    {
        DBG_INTERN_DVBT2(printf(">INTERN_DVBT2_GetLock MBX_ReadDspReg fail \n"));
        return FALSE;
    }
    //printf("[dvbt2] TS_DATA_RATE_1 = 0x%x \n\n", u8_tmp);
    TS_DATA_RATE = (TS_DATA_RATE<<8) |u8_tmp;

    if (MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_T2_TS_DATA_RATE_0, &u8_tmp) == FALSE)
    {
        DBG_INTERN_DVBT2(printf(">INTERN_DVBT2_GetLock MBX_ReadDspReg fail \n"));
        return FALSE;
    }
    //printf("[dvbt2] TS_DATA_RATE_0 = 0x%x \n\n", u8_tmp);
    TS_DATA_RATE = (TS_DATA_RATE<<8) |u8_tmp;

    DBG_AUTO_TS_DATA_RATE(printf("[dvbt2] TS_DATA_RATE_total = 0x%x   %d \n\n", TS_DATA_RATE, TS_DATA_RATE));


    u8_tmp=HAL_DMD_RIU_ReadByte(0x103301);
    u8_tmp &= 0x01;

    if(u8_tmp == 0x01)
    {
        if (MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_T2_TS_DIV_288, &u8_tmp) == FALSE)
        {
            DBG_INTERN_DVBT2(printf(">INTERN_DVBT2_GetLock MBX_ReadDspReg fail \n"));
            return FALSE;
        }
        *u8TSDivNum = u8_tmp;
        ...
        DBG_AUTO_TS_DATA_RATE(printf(" CLK Source: 288 MHz \n"));
    }
    else
    {
        printf("@@@@@@@@@@@@@ DVB-T2 TS clock source error!!!\n");
    }


    if (*u8TSDivNum > 0x1f)// 36 MHz/8 = 4.5 MHz
        *u8TSDivNum = 0x1f;

    if (*u8TSDivNum < 0x0f)// 72 MHz/8 = 9 MHz
        *u8TSDivNum = 0x0f;

    DBG_AUTO_TS_DATA_RATE(printf(">>>INTERN_DVBT2_GetTsDivNum = 0x%x<<<\n", *u8TSDivNum));
...
MS_BOOL INTERN_DVBT2_ConfigAdaptiveTsDivNum(void)
{
    ...
    if (MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_T2_TS_DATA_RATE_CHANGE_IND, &u8_tmp) == FALSE)
    {
        DBG_INTERN_DVBT2(printf(">INTERN_DVBT2_GetLock MBX_ReadDspReg fail \n"));
        return FALSE;
    }
    ...
    if (u8_tmp ==1)
    {
        ...
        DBG_INTERN_DVBT2(printf(">>>INTERN_DVBT2_GetLock TsClkDivNum = 0x%x<<<\n", u8TSDivNum));
        DBG_AUTO_TS_DATA_RATE(printf(">>>TS_DATA_RATE_CHANGE Detected: TsClkDivNum = 0x%x<<<\n", u8TSDivNum));
        // ** Caution: for TS parallel mode
        HAL_DMD_RIU_WriteByte(0x103300, u8TSDivNum);
        MDrv_SYS_DMD_VD_MBX_WriteDSPReg((MS_U32)E_DMD_T2_TS_DATA_RATE_CHANGE_IND,0x00);
    }
