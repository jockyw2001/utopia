...
MS_BOOL INTERN_DVBT_DSPReg_Init(const MS_U8 *u8DVBT_DSPReg,  MS_U8 u8Size)
{
    ...
    if (u8DVBT_DSPReg != NULL)
    {
        ...
        {
            ULOGE("DEMOD","FATAL: parameter version incorrect\n");
        }
    }

    if(MDrv_SYS_DMD_VD_MBX_WriteDSPReg(E_DMD_DVBT_N_PARAM_VERSION, UTOPIA_DRIVER_VERSION) != TRUE) 
    {
        ULOGD("DEMOD","INTERN_DVBT_DSPReg_Init NG\n");
        return FALSE;
    }    
...
MS_BOOL INTERN_DVBT_Cmd_Packet_Send(S_CMDPKTREG* pCmdPacket, MS_U8 param_cnt)
{
    ...
    do
    {
        ...
        if (timeout++ > 200)
        {
            ULOGE("DEMOD","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_END clear' \n");
    ...
    do
    {
        ...
        if (timeout++ > 200)
        {
            ULOGE("DEMOD","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_START clear' \n");
            //MsOS_ReleaseMutex(_s32_Demod_DVBT_Mutex);
            return false;
        }
    } while (1);
    ...
    for (indx = 0; indx < param_cnt; indx++)
    {
        ...
        do
        {
            ...
            if (timeout++ > 200)
            {
                ULOGE("DEMOD","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_DRQ clear' \n");
                //MsOS_ReleaseMutex(_s32_Demod_DVBT_Mutex);
                return false;
            }
        } while (1);
...
MS_BOOL INTERN_DVBT_SoftStop ( void )
{
    ...
    while(HAL_DMD_RIU_ReadByte(MBRegBase + 0x00)!= 0x5A)           // wait MB_CNTL set done
    {
        ...
        if (u8WaitCnt++ >= 0xFF)
        {
            ULOGE("DEMOD",">> DVBT SoftStop Fail!\n");
            return FALSE;
        }
    }
...
static MS_BOOL INTERN_DVBT_LoadDSPCode(void)
{
  ...
  //  MDrv_Sys_DisableWatchDog();
    ...
    for ( i = 0; i < sizeof(INTERN_DVBT_table); i++)
    {
        ...
        {
            ULOGE("DEMOD",">fail add = 0x%x\n", i);
            ULOGE("DEMOD",">code = 0x%x\n", INTERN_DVBT_table[i]);
            ULOGE("DEMOD",">data = 0x%x\n", udata);

            if (fail_cnt++ > 10)
            {
                ULOGE("DEMOD",">DVB-T DSP Loadcode fail!");
                return false;
            }
        }
    }
...
MS_BOOL INTERN_DVBT_Power_On_Initialization(MS_BOOL bRFAGCTristateEnable, MS_U8 u8ADCIQMode, MS_U8 u8PadSel, MS_BOOL bPGAEnable, MS_U8 u8PGAGain, const MS_U8 *u8DMD_DVBT_DSPRegInitExt, MS_U8 u8DMD_DVBT_DSPRegInitSize)
{
    ...
    
    {
        ...
        {
            ULOGE("DEMOD","DVB-T Load DSP Code Fail\n");
            return FALSE;
        }
        else
        {
...
MS_BOOL INTERN_DVBT_Active(MS_BOOL bEnable)
{
    ...
    MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_DVBT_N_PARAM_VERSION, &reg_frz);
    ULOGD("DEMOD","##########DVBT------>(Driver) = 0x%x #########\n",reg_frz); 
    MDrv_SYS_DMD_VD_MBX_ReadDSPReg(E_DMD_DVBT_N_OP_RFAGC_EN, &reg_frza);
    ULOGD("DEMOD","##########DVBT------>(FW) = 0x%x #########\n",reg_frza);
    if (reg_frz < reg_frza)
    {
        while(1)
        ULOGD("DEMOD","##########--------->Abnormal case, please update demod utopia driver version!!! #########\n"); 
    }
    else{
        ULOGD("DEMOD","##########--------->Normal case! #########\n"); 
    }
#endif
...
#if 1 // copy from DEMOD2\mxlib\hal\miami\demod\halDMD_INTERN_DVBT.c
...
MS_BOOL INTERN_DVBT_GetSignalStrength(MS_U16 *strength,const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    if(INTERN_DVBT_Get_TPS_Parameter_Const(&tps_info_qam, TS_MODUL_MODE) == FALSE)
        ULOGE("DEMOD","[dvbt]TPS qam parameter retrieve failure\n");

    if(INTERN_DVBT_Get_TPS_Parameter_Const(&tps_info_cr, TS_CODE_RATE) == FALSE)
        ULOGE("DEMOD","[dvbt]TPS cr parameter retrieve failure\n");
...
MS_BOOL INTERN_DVBT_GetSignalQuality(MS_U16 *quality, const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    if (TRUE == INTERN_DVBT_GetLock(E_DMD_COFDM_PSYNC_LOCK) )
    {
        ...
        if(fViterbiBerFiltered<= 0.0)
        {
            ...
            {
                ULOGE("DEMOD","GetPostViterbiBer Fail!\n");
                return FALSE;
            }
            fViterbiBerFiltered = fber;
        }
...
MS_BOOL INTERN_DVBT_Get_Lock_Time_Info(MS_U16 *locktime)
{
    ...
    *locktime = 0xffff;
    ULOGE("DEMOD","[dvbt]INTERN_DVBT_Get_Lock_Time_Info not implement\n");
...
MS_BOOL INTERN_DVBT_Show_Lock_Info(void)
{
  ...
  ULOGE("DEMOD","[dvbt]INTERN_DVBT_Show_Lock_Info not implement!!!\n");
  return false;
}
