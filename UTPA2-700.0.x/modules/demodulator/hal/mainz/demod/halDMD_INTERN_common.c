...
MS_BOOL HAL_DMD_RegInit (void)
{
    ...
    {
        #ifdef MS_DEBUG
        ULOGE("DEMOD","HAL_DMD_RegInit failure to get MS_MODULE_PM\n");
        #endif
...
MS_U8 HAL_DMD_RIU_ReadByte(MS_U32 u32Addr)
{
    ...
    {
        #ifdef MS_DEBUG
        ULOGE("DEMOD","%s base address un-initialized\n", __FUNCTION__);
        #endif
    }
    return 0;
}
                                                     ...
                                                     DMD_IFAGC_ERR *pIfagcErr_HiRef, MS_U16 u16IfagcErr_HiRef_Size)
{
    ...
    else
    {
        ...
        if (fRFPowerDbm>=100.0) // unreasonable input value, get RF level from RFAGG
        {
            ULOGE("DEMOD","Error!! please add AGC table\n");
        }
        #endif
...
MS_BOOL HAL_DMD_TSO_Clk_Control(MS_U8 *u8cmd_array)
{
    ...
    if ( (u8Temp&0x01) == 0x00)
    {
        ULOGE("DEMOD","[DEMOD][halDMD]Error!!!, we shall select clk_dmplldiv3\n");
        return false;
    }
    switch (u8cmd_array[0])
    {
        ...
        case 0x03:
            ...
            break;
        default:
            ULOGE("DEMOD","[DEMOD][halDMD]Error!!!, cmd invalid\n");
            break;
