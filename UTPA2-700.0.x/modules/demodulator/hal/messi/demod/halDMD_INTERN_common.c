...
MS_BOOL HAL_DMD_RegInit (void)
{
    ...
    {
        #ifdef MS_DEBUG
        ULOGE("Utopia","HAL_DMD_RegInit failure to get MS_MODULE_PM\n");
        #endif
...
MS_U8 HAL_DMD_RIU_ReadByte(MS_U32 u32Addr)
{
    ...
    {
        #ifdef MS_DEBUG
        ULOGE("Utopia","%s base address un-initialized\n", __FUNCTION__);
        #endif
    }
    return 0;
}
                                                     ...
                                                     DMD_IFAGC_ERR *pIfagcErr_HiRef, MS_U16 u16IfagcErr_HiRef_Size)
{
    ...
    {      
        ...
        if (fRFPowerDbm>=100.0) // unreasonable input value, get RF level from RFAGG
        {
            ...
            if (rf_agc_val >=pRfagcSsi[0].sar3_val)
            {
                ...
                #ifdef MS_DEBUG
                ULOGD("Utopia","RF Level from SAR:%f\n", ch_power_rf);
                ULOGD("Utopia","SSI_RFAGC (SAR-4) = 0x%x\n", rf_agc_val);
                ULOGD("Utopia","rf prev %f %x\n", ch_power_rfa, rf_agc_vala);
                ULOGD("Utopia","rf next %f %x\n", ch_power_rfb, rf_agc_valb);                    
                #endif                    
            }
        }
        else
        {
            #ifdef MS_DEBUG
            ULOGD("Utopia","RF Level from tuner: %f\n",fRFPowerDbm);
            #endif
        ...
        // get IFAGC status
        {
            ...
            #ifdef MS_DEBUG
            ULOGD("Utopia","SSI_IFAGC_H = 0x%x 0x%x\n", if_agc_val,if_agc_val_lsb);
            #endif
            ...
            #ifdef MS_DEBUG
            ULOGD("Utopia","if prev %f %x\n", ch_power_ifa, if_agc_vala);
            ULOGD("Utopia","if next %f %x\n", ch_power_ifb, if_agc_valb);  
            #endif
            ...
            #ifdef MS_DEBUG
            ULOGD("Utopia","ch_power_rf = %f\n", ch_power_rf);
            ULOGD("Utopia","ch_power_if = %f\n", ch_power_if);
            ULOGD("Utopia","ch_power_takeover = %f\n", ch_power_takeover);
            #endif
    ...
    else
    {
        ...
        if (fRFPowerDbm>=100.0) // unreasonable input value, get RF level from RFAGG
        {
            ULOGE("Utopia","Error!! please add AGC table\n");
        }
        #endif
...
{
    #ifdef MS_DEBUG
    ULOGD("Utopia","HAL_DMD_TS1_Tristate %d\n",bEnable);
    #endif
    if (bEnable)
    {
...
void HAL_DMD_RFAGC_Tristate(MS_BOOL bEnable)
{
    ...
    #ifdef MS_DEBUG
    ULOGD("Utopia","HAL_DMD_RFAGC_Tristate %d\n",bEnable);
    #endif
...
void HAL_DMD_IFAGC_Tristate(MS_BOOL bEnable)
{
    ...
    #ifdef MS_DEBUG
    ULOGD("Utopia","HAL_DMD_IFAGC_Tristate %d\n",bEnable);
    #endif
    u8RegMuxBackup = HAL_DMD_RIU_ReadByte(0x101E39);
    HAL_DMD_RIU_WriteByte(0x101E39, 0x00); //mux from DMD MCU to HK
    if (bEnable)
...
{
    #ifdef MS_DEBUG
    ULOGD("Utopia","HAL_DMD_IFAGC_TS_Tristate %d\n",bEnable);
    #endif
...
MS_BOOL HAL_DMD_ADC_IQ_Switch(MS_U8 u8ADCIQMode, MS_U8 u8PadSel, MS_BOOL bPGAEnable, MS_U8 u8PGAGain)
{
    ...
    u8PGAGain=u8PGAGain;
    #ifdef MS_DEBUG
    ULOGD("Utopia","HAL_DMD_ADC_IQ_Switch %d %d %d %d\n",u8ADCIQMode, u8PadSel, bPGAEnable, u8PGAGain);
    #endif

    u8RegMuxBackup = HAL_DMD_RIU_ReadByte(0x101E39);
    HAL_DMD_RIU_WriteByte(0x101E39, 0x00); //mux from DMD MCU to HK
    #ifdef MS_DEBUG
    ULOGD("Utopia","before 0x112803 %x\n",HAL_DMD_RIU_ReadByte(0x112803));
    #endif
    switch(u8ADCIQMode)
    {
...
MS_BOOL HAL_DMD_TSO_Clk_Control(MS_U8 *u8cmd_array)
{
    ...
    if ( (u8Temp&0x01) == 0x00)
    {
        ULOGE("Utopia","[utopia][halDMD]Error!!!, we shall select clk_dmplldiv3\n");
        return false;
    }
    switch (u8cmd_array[0])
    {
        ...
        case 0x03:
            ...
            break;
        default:
            ULOGE("Utopia","[utopia][halDMD]Error!!!, cmd invalid\n");
            break;

    }
#ifdef MS_DEBUG
    ULOGD("Utopia","0x103300: 0x%x\n",HAL_DMD_RIU_ReadByte(0x103300));
    ULOGD("Utopia","0x103301: 0x%x\n",HAL_DMD_RIU_ReadByte(0x103301));
    ULOGD("Utopia","0x10330B: 0x%x\n",HAL_DMD_RIU_ReadByte(0x10330B));    
#endif
    return true;
}
