...
MS_BOOL INTERN_DVBT_DSPReg_Init(const MS_U8 *u8DVBT_DSPReg,  MS_U8 u8Size)
{
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia","INTERN_DVBT_DSPReg_Init\n"));
    ...
    if (u8DVBT_DSPReg != NULL)
    {
        ...
        if (u8DVBT_DSPReg[0] >= 1)
        {
            ...
            for (idx = 0; idx<u8Size; idx++)
            {
                ...
                u8DVBT_DSPReg++;
                DBG_INTERN_DVBT(ULOGD("Utopia","DSP addr:%x mask:%x read:%x write:%x\n", u16DspAddr, u8Mask, u8RegRead, u8RegWrite));
...
MS_BOOL INTERN_DVBT_Cmd_Packet_Send(S_CMDPKTREG* pCmdPacket, MS_U8 param_cnt)
{
    ...
    // ==== Command Phase ===================
    DBG_INTERN_DVBT(ULOGD("Utopia","--->INTERN_DVBT (cmd=0x%x)(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,) \n",pCmdPacket->cmd_code,
    ...
    do
    {
        ...
        if (timeout++ > 200)
        {
            ULOGE("Utopia","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_END clear' \n");
    ...
    do
    {
        ...
        if (timeout++ > 200)
        {
            ULOGE("Utopia","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_START clear' \n");
            //MsOS_ReleaseMutex(_s32_Demod_DVBT_Mutex);
            return false;
        }
    } while (1);
    ...
    for (indx = 0; indx < param_cnt; indx++)
    {
        ...
        do
        {
            ...
            if (timeout++ > 200)
            {
                ULOGE("Utopia","---> INTERN_DVBT_Cmd_Packet_Send fail on 'wait _BIT_DRQ clear' \n");
                //MsOS_ReleaseMutex(_s32_Demod_DVBT_Mutex);
                return false;
            }
        } while (1);
...
{
    DBG_INTERN_DVBT(ULOGD("Utopia"," @INTERN_DVBT_reset\n"));

    DBG_INTERN_DVBT_TIME(ULOGD("Utopia","INTERN_DVBT_Reset, t = %ld\n",MsOS_GetSystemTime()));
...
static MS_BOOL INTERN_DVBT_LoadDSPCode(void)
{
  ...
  //  MDrv_Sys_DisableWatchDog();
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia",">Load Code...\n"));
#ifdef INTERN_DVBT_LOAD_FW_FROM_CODE_MEMORY
...
#if (DBG_DUMP_LOAD_DSP_TIME==1)
    ULOGD("Utopia","------> INTERN_DVBT Load DSP Time:  (%lu)\n", msAPI_Timer_DiffTimeFromNow(u32Time)) ;
#endif

#endif

    ////  Content verification ////
    DBG_INTERN_DVBT(ULOGD("Utopia",">Verify Code...\n"));
    ...
    for ( i = 0; i < sizeof(INTERN_DVBT_table); i++)
    {
        ...
        if (udata != INTERN_DVBT_table[i])
        {
            ...
            if (fail_cnt++ > 10)
            {
                ULOGE("Utopia",">DVB-T DSP Loadcode fail!");
                return false;
            }
        }
    }
    ...
    for (i=0;i<=SizeBy4K;i++)
    {
        ...
        srcaddr = u32GEAddr+(0x1000*i);
        //ULOGD("Utopia","\t i = %08LX\n", i);
        //ULOGD("Utopia","\t len = %08LX\n", len);
        op = 1;
        u16Counter = 0 ;
        //ULOGD("Utopia","\t (B=0x%bx)(Src=0x%x)Data =",i,srcaddr);
        while(len--)
        {
            u16Counter ++ ;
            //ULOGD("Utopia","file: %s, line: %d\n", __FILE__, __LINE__);
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia",">DSP Loadcode done."));
...
//    HAL_DMD_RIU_WriteByte(0x11208E, (HAL_DMD_RIU_ReadByte(0x11208E)&(~(BIT(0)))));
    ...
    //         110:mem_clock div 2
    //         111:select XTAL
    HAL_DMD_RIU_WriteByte(0x10331f,0x00);//5566
    ...
    udatatemp = HAL_DMD_RIU_ReadByte(0x111f2a);//5566
    HAL_DMD_RIU_WriteByte(0x111f2a,udatatemp&0xF8);//5566
...
// wriu 0x111f8a 0x44
    ...
    HAL_DMD_RIU_WriteByte(0x111f89,0x44);
    HAL_DMD_RIU_WriteByte(0x111f88,0x44);
    
    HAL_DMD_RIU_WriteByte(0x111f8b,0x00);    
    HAL_DMD_RIU_WriteByte(0x111f8a,0x44);  

    HAL_DMD_RIU_WriteByte(0x111f8d,0x18);    
    HAL_DMD_RIU_WriteByte(0x111f8c,0x00); 
    
    HAL_DMD_RIU_WriteByte(0x111f8f,0x00);    
    HAL_DMD_RIU_WriteByte(0x111f8e,0x40);           
    // ----------------------------------------------
    //  Turn TSP
    // ----------------------------------------------
...
MS_BOOL INTERN_DVBT_Power_On_Initialization(MS_BOOL bRFAGCTristateEnable, MS_U8 u8ADCIQMode, MS_U8 u8PadSel, MS_BOOL bPGAEnable, MS_U8 u8PGAGain, const MS_U8 *u8DMD_DVBT_DSPRegInitExt, MS_U8 u8DMD_DVBT_DSPRegInitSize)
{
    ...
    MS_U8   cData = 0;
    //U8            cal_done;
    DBG_INTERN_DVBT(ULOGD("Utopia","INTERN_DVBT_Power_On_Initialization\n"));
    ...
    //// Firmware download //////////
    DBG_INTERN_DVBT(ULOGD("Utopia","INTERN_DVBT Load DSP...\n"));
    //MsOS_DelayTask(100);
    ...
    
    {
        ...
        {
            ULOGE("Utopia","DVB-T Load DSP Code Fail\n");
            return FALSE;
        }
        else
        {
            DBG_INTERN_DVBT(ULOGD("Utopia","DVB-T Load DSP Code OK\n"));
        }
    }


    //// MCU Reset //////////
    DBG_INTERN_DVBT(ULOGD("Utopia","INTERN_DVBT Reset...\n"));
    if (INTERN_DVBT_Reset() == FALSE)
    {
        DBG_INTERN_DVBT(ULOGE("Utopia","Fail\n"));
        return FALSE;
    }
    else
    {
        DBG_INTERN_DVBT(ULOGD("Utopia","OK\n"));
    }
...
void INTERN_DVBT_Driving_Control(MS_BOOL bEnable)
{
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia","---> INTERN_DVBT_Driving_Control(Bit0) = 0x%x \n",u8Temp));
...
void INTERN_DVBT_Clk_Inversion_Control(MS_BOOL bInversionEnable)
{
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia","---> Inversion(Bit9) = 0x%x \n",u8Temp));
...
MS_BOOL INTERN_DVBT_Serial_Control(MS_BOOL bEnable, MS_U8 u8TSClk)
{
    MS_U8   status = true;
    MS_U8 temp_val;
    DBG_INTERN_DVBT(ULOGD("Utopia"," @INTERN_DVBT_ts... u8TSClk=%d\n",u8TSClk));
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia","---> Inversion(Bit5) = 0x%x \n",gsCmdPacket.param[1] ));
...
{
    DBG_INTERN_DVBT(ULOGD("Utopia"," @INTERN_DVBT_TS1_Enable... \n"));

    if(flag) // PAD_TS1 Enable TS CLK PAD
    {
        //ULOGD("Utopia","=== TS1_Enable ===\n");
...
MS_BOOL INTERN_DVBT_Config ( DMD_RF_CHANNEL_BANDWIDTH BW, MS_BOOL bSerialTS, MS_BOOL bPalBG, MS_BOOL bLPSel, MS_U8 u8TSClk, MS_U32 u32IFFreq, MS_U32 u32FSFreq, MS_U8 u8IQSwap)
{
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia"," @INTERN_DVBT_config %d %d %d %d %d %ld %ld %d\n", BW, bSerialTS, bPalBG, bLPSel, u8TSClk, u32IFFreq, u32FSFreq, u8IQSwap));
    DBG_INTERN_DVBT_TIME(ULOGD("Utopia","INTERN_DVBT_Config, t = %ld\n",MsOS_GetSystemTime()));
...
MS_BOOL INTERN_DVBT_Active(MS_BOOL bEnable)
{
    ...
    DBG_INTERN_DVBT(ULOGD("Utopia"," @INTERN_DVBT_active\n"));
...
DMD_LOCK_STATUS INTERN_DVBT_Lock(MS_U16 u16DMD_DVBT_TPS_Timeout, MS_U16 u16DMD_DVBT_FEC_Timeout)
{
    ...
    if (INTERN_DVBT_GetLock(E_DMD_COFDM_FEC_LOCK))
    {
        if (bFECLock ==  FALSE)
        {
            ...
            DBG_INTERN_DVBT(ULOGD("Utopia","++++++++[utopia]dvbt lock\n"));
        }
        ...
        if(INTERN_DVBT_GetPostViterbiBer(&fBER) == TRUE)
        {
            ...
            DBG_INTERN_DVBT(ULOGD("Utopia","[dvbt]f_ber=%8.3e, g_viter_ber=%8.3e\n",fBER,fViterbiBerFiltered));
        }
	...
	{
		ULOGD("Utopia","==> INTERN_DVBT_Lock -- E_DMD_COFDM_NO_CHANNEL \n");
		return E_DMD_UNLOCK;
	}
    ...
    if(!bTPSLock)
    {
        ...
        {
            DBG_INTERN_DVBT(ULOGD("Utopia","==> INTERN_DVBT_Lock -- TPSLock \n"););
            bTPSLock = TRUE;
        }
    }
    if(bTPSLock)
    {
        DBG_INTERN_DVBT(ULOGD("Utopia","TPSLock %ld\n",MsOS_GetSystemTime()));
...
MS_BOOL INTERN_DVBT_GetPostViterbiBer(float *ber)
{
    ...
    DBG_GET_SIGNAL(ULOGD("Utopia","INTERN_DVBT PostVitBER = %8.3e \n ", *ber));
    DBG_GET_SIGNAL(ULOGD("Utopia","INTERN_DVBT PktErr = %d \n ", (int)PktErr));
...
MS_BOOL INTERN_DVBT_GetSignalStrength(MS_U16 *strength,const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    DBG_INTERN_DVBT_TIME(ULOGD("Utopia","INTERN_DVBT_GetSignalStrength, t=%ld\n",MsOS_GetSystemTime()));
    ...
    if(INTERN_DVBT_Get_TPS_Parameter_Const(&tps_info_qam, TS_MODUL_MODE) == FALSE)
        ULOGE("Utopia","[dvbt]TPS qam parameter retrieve failure\n");

    if(INTERN_DVBT_Get_TPS_Parameter_Const(&tps_info_cr, TS_CODE_RATE) == FALSE)
        ULOGE("Utopia","[dvbt]TPS cr parameter retrieve failure\n");
    ...
    DBG_GET_SIGNAL(ULOGD("Utopia",">>> SSI_CH_PWR(dB) = %f , Score = %d<<<\n", ch_power_db, *strength));
    DBG_GET_SIGNAL(ULOGD("Utopia",">>> SSI = %d <<<\n", (int)*strength));
...
MS_BOOL INTERN_DVBT_GetSignalQuality(MS_U16 *quality, const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    DBG_INTERN_DVBT_TIME(ULOGD("Utopia","INTERN_DVBT_GetSignalQuality, t=%ld\n",MsOS_GetSystemTime()));
    ...
    if (TRUE == INTERN_DVBT_GetLock(E_DMD_COFDM_PSYNC_LOCK) )
    {
        ...
        if(fViterbiBerFiltered<= 0.0)
        {
            ...
            {
                DBG_INTERN_DVBT(ULOGE("Utopia","GetPostViterbiBer Fail!\n"));
                return FALSE;
            }
            fViterbiBerFiltered = fber;
        }
    ...
    DBG_GET_SIGNAL(ULOGD("Utopia","SNR = %f, QAM = %d, code Rate = %d\n", cn_rec, tps_cnstl, tps_cr));
    DBG_GET_SIGNAL(ULOGD("Utopia","BER = %8.3e\n", fber));
    DBG_GET_SIGNAL(ULOGD("Utopia","Signal Quility = %d\n", *quality));
    return status;
}
...
MS_BOOL INTERN_DVBT_Get_FreqOffset(float *pFreqOff, MS_U8 u8BW)
{
    ...
    *pFreqOff = (-1.0f)*(FreqIcfo + (FreqCfoFd + FreqCfoTd)/1000);
    // DBG_GET_SIGNAL(ULOGD("Utopia","FCFO = %f\n", FreqCfoFd));
    // DBG_GET_SIGNAL(ULOGD("Utopia","TCFO = %f\n", FreqCfoTd));
    // DBG_GET_SIGNAL(ULOGD("Utopia","ICFO = %f\n", FreqIcfo));
    DBG_GET_SIGNAL(ULOGD("Utopia","CFOE = %f\n", *pFreqOff));
...
MS_BOOL INTERN_DVBT_Show_ChannelLength(void)
{
    ...
    len = (len<<8)|tmp;
    ULOGD("Utopia","[dvbt]Hw_channel=%d\n",len);
    return status;
}
...
MS_BOOL INTERN_DVBT_Show_SW_ChannelLength(void)
{
    ...
    ULOGD("Utopia","[dvbt]SW_len=%d, peak_num=%d, insideGI=%d, stoptrack=%d, short_echo=%d, fsa_mode=%d\n",
...
MS_BOOL INTERN_DVBT_Show_ACI_CI(void)
{
    ...
    ULOGD("Utopia","[dvbt]DigACI=%d, Flag_CI=%d, td_coef=%d\n",digACI,flag_CI,td_coef);
...
MS_BOOL INTERN_DVBT_Show_FD_CH_LEN_S_SEL(void)
{
    ...
    ULOGD("Utopia","[dvbt]fd=0x%x, ch_len=0x%x, snr_sel=0x%x, pertone_num=0x%x\n",fd,ch_len,snr_sel,pertone_num);
...
MS_BOOL INTERN_DVBT_Get_CFO(void)
{
    ...
    ULOGD("Utopia","[CFO]t_cfo=%f Hz, f_cfo=%f Hz, icfo=%f KHz, cfo=%f KHz\n", FreqCfoTd,FreqCfoFd,FreqIcfo,total_cfo);
...
MS_BOOL INTERN_DVBT_Get_SFO(void)
{
    ...
    // ULOGD("Utopia","\nReg_FSA_SFO = 0x%x\n",Reg_FSA_SFO);
    ULOGD("Utopia","[SFO]tdp_sfo=%f, fdp_sfo=%f, fsa_sfo=%f, Tot_sfo=%f, fsa_sfo_in=%f\n",Float_TDP_SFO,Float_FDP_SFO,Float_FSA_SFO,sfo_value,Float_FSA_IN);
...
void INTERN_DVBT_Get_SYA_status(void)
{
    ...
    ULOGD("Utopia","[SYA][1]sya_k = 0x%x, sya_th = 0x%x, sya_offset=0x%x\n",sya_k,sya_th,sya_offset);
    ULOGD("Utopia","[SYA][2]track_reg=0x%x, len_m = %d, len_e = %d [%d,%d]\n",tracking_reg,len_m,len_b-len_a,len_a,len_b);
...
void INTERN_DVBT_Get_cci_status(void)
{
    ...
    ULOGD("Utopia","[CCI]fsweep=0x%x, k=0x%x\n",cci_fsweep,cci_kp);
...
MS_BOOL INTERN_DVBT_Get_Lock_Time_Info(MS_U16 *locktime)
{
    ...
    *locktime = 0xffff;
    ULOGE("Utopia","[dvbt]INTERN_DVBT_Get_Lock_Time_Info not implement\n");
...
MS_BOOL INTERN_DVBT_Show_Lock_Time_Info(void)
{
    ...
    ULOGD("Utopia","[DVBT]lock_time = %d ms\n",locktime);
    return status;
}
...
MS_BOOL INTERN_DVBT_Show_AGC_Info(void)
{
    ...
    ULOGD("Utopia","[dvbt]agc_k=0x%x, agc_ref=0x%x, d1_k=0x%x, d1_ref=0x%x, d2_k=0x%x, d2_ref=0x%x\n",
        agc_k,agc_ref,d1_k,d1_ref,d2_k,d2_ref);

    ULOGD("Utopia","[dvbt]agc_g=0x%x, d1_g=0x%x, d2_g=0x%x, agc_err=0x%x\n",if_agc_gain,d1_gain,d2_gain,if_agc_err);
    ULOGD("Utopia","[dvbt]agc_lock=0x%x, d1_lock=0x%x, d2_lock=0x%x\n",agc_lock,d1_lock,d2_lock);
...
MS_BOOL INTERN_DVBT_Show_WIN_Info(void)
{
    ...
    ULOGD("Utopia","[dvbt]win_len = %d, trigger=0x%x\n",win_len,trigger);
...
void INTERN_DVBT_Show_td_coeff(void)
{
    ...
    ULOGD("Utopia","[td]w1=0x%x, w2=0x%x\n",w1,w2);
...
MS_BOOL INTERN_DVBT_Show_Modulation_info(void)
{
    ...
    // ULOGD("Utopia","[DVBT]TPS info, freq=%ld  ",CurRFParam.RfFreqInKHz);
    ...
    if(INTERN_DVBT_Get_TPS_Info( &tps_info))
    {
        ...
        ULOGD("Utopia","tps=0x%x  ",tps_info);

        switch(fft)
        {
            case 0:
                ULOGD("Utopia","mode = 2K,");
                break;
            case 1:
                ULOGD("Utopia","mode = 8K,");
                break;
            default:
                ULOGE("Utopia","mode = unknow,");
                break;
        }
        switch(constel)
        {
            ...
            case 2:
                ...
                break;
            default:
                ULOGE("Utopia","unknow QAM, ");
                break;
        }
        switch(gi)
        {
            case 0:
                ULOGD("Utopia","GI=1/32, ");
                break;
            case 1:
                ULOGD("Utopia","GI=1/16, ");
                break;
            case 2:
                ULOGD("Utopia","GI= 1/8, ");
                break;
        ...
        {
            case 0:
                ULOGD("Utopia","HP_CR=1/2, ");
                break;
            case 1:
                ULOGD("Utopia","HP_CR=2/3, ");
                break;
            case 2:
                ULOGD("Utopia","HP_CR=3/4, ");
                break;
            case 3:
                ULOGD("Utopia","HP_CR=5/6, ");
                break;
            case 4:
                ULOGD("Utopia","HP_CR=7/8, ");
                break;
            default:
                ULOGE("Utopia","unknow hp_cr, ");
                break;
        }
        ...
        {
            case 0:
                ULOGD("Utopia","LP_CR=1/2, ");
                break;
            case 1:
                ULOGD("Utopia","LP_CR=2/3, ");
                break;
            case 2:
                ULOGD("Utopia","LP_CR=3/4, ");
                break;
            case 3:
                ULOGD("Utopia","LP_CR=5/6, ");
                break;
            case 4:
                ULOGD("Utopia","LP_CR=7/8, ");
                break;
            default:
                ULOGE("Utopia","unknow lp_cr, ");
                break;
        }

        ULOGD("Utopia"," Hiearchy=0x%x\n",hiearchy);
...
MS_BOOL INTERN_DVBT_Show_Lock_Info(void)
{
  ...
  ULOGE("Utopia","[dvbt]INTERN_DVBT_Show_Lock_Info not implement!!!\n");
  return false;
}
