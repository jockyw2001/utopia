...
MS_BOOL INTERN_DVBT_DSPReg_Init(const MS_U8 *u8DVBT_DSPReg,  MS_U8 u8Size)
{
    ...
    ULOGD("DEMOD","INTERN_DVBT_DSPReg_Init\n");
    ...
    if (u8DVBT_DSPReg != NULL)
    {
        ...
        if (u8DVBT_DSPReg[0] >= 1)
        {
            ...
            for (idx = 0; idx<u8Size; idx++)
            {
                ...
                u8DVBT_DSPReg++;
                ULOGD("DEMOD","DSP addr:%x mask:%x read:%x write:%x\n", u16DspAddr, u8Mask, u8RegRead, u8RegWrite);
...
MS_BOOL INTERN_DVBT_Cmd_Packet_Send(S_CMDPKTREG* pCmdPacket, MS_U8 param_cnt)
{
    ...
    // ==== Command Phase ===================
    ULOGD("DEMOD","--->INTERN_DVBT (cmd=0x%x)(0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,) \n",pCmdPacket->cmd_code,
...
MS_BOOL INTERN_DVBT_Power_On_Initialization(MS_BOOL bRFAGCTristateEnable, MS_U8 u8ADCIQMode, MS_U8 u8PadSel, MS_BOOL bPGAEnable, MS_U8 u8PGAGain, const MS_U8 *u8DMD_DVBT_DSPRegInitExt, MS_U8 u8DMD_DVBT_DSPRegInitSize)
{
    ...
    MS_U8   cData = 0;
    //U8            cal_done;
    ULOGD("DEMOD","INTERN_DVBT_Power_On_Initialization\n");
    ...
    //// Firmware download //////////
    ULOGD("DEMOD","INTERN_DVBT Load DSP...\n");
    //MsOS_DelayTask(100);
    ...
    //if (HAL_DMD_RIU_ReadByte(0x101E3E) != 0x02) // DVBT = BIT1 -> 0x02
    {
        ...
        {
            ULOGD("DEMOD","DVB-T Load DSP Code OK\n");
        }
    }


    //// MCU Reset //////////
    ULOGD("DEMOD","INTERN_DVBT Reset...\n");
...
void INTERN_DVBT_Driving_Control(MS_BOOL bEnable)
{
    ...
    ULOGD("DEMOD","---> INTERN_DVBT_Driving_Control(Bit0) = 0x%x \n",u8Temp);
...
void INTERN_DVBT_Clk_Inversion_Control(MS_BOOL bInversionEnable)
{
    ...
    ULOGD("DEMOD","---> Inversion(Bit9) = 0x%x \n",u8Temp);
...
MS_BOOL INTERN_DVBT_Serial_Control(MS_BOOL bEnable, MS_U8 u8TSClk)
{
    MS_U8   status = true;
    MS_U8 temp_val;
    ULOGD("DEMOD"," @INTERN_DVBT_ts... u8TSClk=%d\n",u8TSClk);
    ...
    ULOGD("DEMOD","---> Inversion(Bit5) = 0x%x \n",gsCmdPacket.param[1] );
...
{
    ULOGD("DEMOD"," @INTERN_DVBT_TS1_Enable... \n");
...
MS_BOOL INTERN_DVBT_Active(MS_BOOL bEnable)
{
    ...
    ULOGD("DEMOD"," @INTERN_DVBT_active\n");
...
*/
    ...
    if(!bTPSLock)
    {
        ...
        {
            ULOGD("DEMOD","==> INTERN_DVBT_Lock -- TPSLock \n");
            bTPSLock = TRUE;
        }
    }
    if(bTPSLock)
    {
        ULOGD("DEMOD","TPSLock %d\n",MsOS_GetSystemTime());
...
MS_BOOL INTERN_DVBT_GetPostViterbiBer(MS_U16 *BitErrPeriod_reg, MS_U32 *BitErr_reg, MS_U16 *PktErr_reg)
{
	...
	*PktErr_reg = PktErr;
    ...
    ULOGD("DEMOD","INTERN_DVBT PktErr = %d \n ", (int)PktErr);
...
MS_BOOL INTERN_DVBT_GetSignalStrength(MS_U16 *strength,const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    ULOGD("DEMOD","INTERN_DVBT_GetSignalStrength, t=%ld\n",MsOS_GetSystemTime());
    ...
    ULOGD("DEMOD",">>> SSI_CH_PWR(dB) = %f , Score = %d<<<\n", ch_power_db, *strength);
    ULOGD("DEMOD",">>> SSI = %d <<<\n", (int)*strength);
...
MS_BOOL INTERN_DVBT_GetSignalQuality(MS_U16 *quality, const DMD_DVBT_InitData *sDMD_DVBT_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    ULOGD("DEMOD","INTERN_DVBT_GetSignalQuality, t=%ld\n",MsOS_GetSystemTime());
    ...
    ULOGD("DEMOD","SNR = %f, QAM = %d, code Rate = %d\n", cn_rec, tps_cnstl, tps_cr);
    ULOGD("DEMOD","BER = %8.3e\n", fber);
    ULOGD("DEMOD","Signal Quility = %d\n", *quality);
    return status;
}
#endif
