...
//		MS_U8 temp_val;
    ...
    HAL_DMD_RIU_WriteByte(0x111f4b,0x01);
    HAL_DMD_RIU_WriteByte(0x111f4a,0x11);
...
MS_BOOL  INTERN_DVBC_Adaptive_TS_CLK(void)
{
  ...
  MS_U8  TS_Clock_Temp;
	...
	TS_Clock_Temp=HAL_DMD_RIU_ReadByte(DMD_CLK_GEN+0x00);
	TS_Clock_Temp=(TS_Clock_Temp&0xE0) |u8_ts_clk ;
	HAL_DMD_RIU_WriteByte(DMD_CLK_GEN+0x00,TS_Clock_Temp);
...
MS_BOOL  INTERN_DVBC_Locked_Task(void)
{
	INTERN_DVBC_Adaptive_TS_CLK();
...

    {
        switch( eType )
        {
            case DMD_DVBC_GETLOCK_FEC_LOCK:
                ...
                #ifdef       SUPPORT_ADAPTIVE_TS_CLK
	              MDrv_SYS_DMD_VD_MBX_ReadReg(MBX_REG_BASE + 0x16, &unlock_indicator);
                #endif
                if (cData == 0x0C)
                {
                  #ifdef       SUPPORT_ADAPTIVE_TS_CLK
...
MS_BOOL INTERN_DVBC_GetCurrentSymbolRate(MS_U16 *u16SymbolRate)
{
    ...
    ULOGD("DEMOD","INTERN_DVBC_GetCurrentSymbolRate\n");
