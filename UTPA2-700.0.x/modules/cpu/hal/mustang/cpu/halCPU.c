...
//  Global Variables
//-------------------------------------------------------------------------------------------------
static MS_U32   u32NonPMBaseAdd;
static MS_U32   u32CoProBaseAdd;
static MS_U32   u32PMRiuBaseAdd;
...
void HAL_COPRO_RegSetBase(MS_U32 u32Base)
{
    MS_U32  u32RiuBaseAddr;
    MS_U32  u32RiuBaseSize;
...
void HAL_COPRO_Disable(void)
{
    ...
    {
#if defined(__mips__) || defined(__arm__)// HK = MIPS or ARM
        COPRO_HAL_INFO("Disable R2\r\n");
        HAL_COPRO_RegWriteByte(R2_REG_STOP, 0x00); // reg_r2_enable = 0x00
//        MsOS_DelayTask(50);
...
void HAL_COPRO_Enable(MS_U32 u32_ADR)
{
    ...
    // Set to 216 MHz
    HAL_CPU_NPM_RegWriteByte(NPM_REG_CLKGEN0_SECR2, 0x00);

#if 0
    //(3) Set CPU SDR base
    base_addr = u32_ADR;
    HAL_COPRO_RegWrite2Byte(R2_REG_SDR_LO_INST_BASE, (base_addr&0x0000FFFF)); //I Fetch Offset - Low
    HAL_COPRO_RegWrite2Byte(R2_REG_SDR_HI_INST_BASE, (base_addr>>16)); //I Fetch Offset - High
    HAL_COPRO_RegWrite2Byte(R2_REG_SDR_LO_DATA_BASE, (base_addr&0x0000FFFF)); //D Fetch Offset - Low
...
#endif
    //(4)Set MAU Mapping
    HAL_COPRO_RegWrite2Byte(R2_MAULV1_REG, 0x8500);
    //(4) Set RIU   base
    ...
    //(7) With QMEM
#if 0//R2_MMAP_MIU1
    HAL_COPRO_RegWrite2Byte(R2_REG_QMEM_BASE_HIGH, 0x8000); // QMEM base address = 0x80000000
    HAL_COPRO_RegWrite2Byte(R2_REG_QMEM_MASK_HIGH, 0xF000); // QMEM less, set DCU Decoding..
#endif

    //(1) Enable R2 -> Start
    HAL_COPRO_RegWriteByte(R2_REG_STOP, 0x24); // miu_sw_rst and sdram_boot = 1
    HAL_COPRO_RegWriteByte(R2_REG_STOP, 0x27); // sdram_boot and (miu/r2/r2_rst) =1
...
MS_U32 HAL_CPU_QueryClock(void)
{
    ...
    dwCpuSpeed = (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(NPM_REG_LPF_HIGH) & 0x0000FFFF);
    dwCpuSpeed <<= 16;
    dwCpuSpeed |= (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(NPM_REG_LPF_LOW) & 0x0000FFFF);

    if(dwCpuSpeed == 0)
    {
        dwCpuSpeed = (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(NPM_REG_CPU_CLOCK) & 0x0000FFFF);
        if(dwCpuSpeed == 0)
        {
            dwCpuSpeed = 804;
        }
    ...
    {
        dwCpuSpeed = (0xA2000000 / dwCpuSpeed);
    }
