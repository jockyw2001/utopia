...
{
    MS_U32  u32CpuControl = 0;
    MS_U32  u32DwCpuSpeed = 0;
    MS_U32  u32Timer = 0;
    MS_BOOL bTimeout = FALSE;

    // Set calc_en
    u32CpuControl = (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(REG_CPU_CALC));
    u32CpuControl |= 0x0001;
    HAL_COPRO_RegWrite2Byte(REG_CPU_CALC, u32CpuControl);

    u32Timer = MsOS_GetSystemTime();

    // Check calc_done
    do{
         if (MsOS_GetSystemTime()-u32Timer > CPU_CALC_TIMEOUT)
         {
            COPRO_HAL_ERR("CPU calc wait done timeout!\n");
            bTimeout = TRUE;
            break;
         }
         u32CpuControl = (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(REG_CPU_CALC));
         u32CpuControl &= 0x3;
    }while(u32CpuControl != 0x3);
    ...
    {
        u32DwCpuSpeed = (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(REG_CPU_DETECT));
        u32DwCpuSpeed = (u32DwCpuSpeed * 1000 * 16)/83333;
    }
  
    // Reset calc_en
    u32CpuControl = (MS_U32) ((MS_U32) HAL_COPRO_RegRead2Byte(REG_CPU_CALC));
    u32CpuControl &= 0xFFFE;
    HAL_COPRO_RegWrite2Byte(REG_CPU_CALC, u32CpuControl);
    
    COPRO_HAL_ERR("(CPU Speed %d MHz)\n", (MS_U32) u32DwCpuSpeed);
    return (u32DwCpuSpeed * 1000000);
}
