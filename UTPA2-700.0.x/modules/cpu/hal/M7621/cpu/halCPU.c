...
#define COPRO_HAL_INFO(x, args...)        //{ULOGI(TAG_CPU, x, ##args);}
#define COPRO_HAL_ERR(x, args...)         //{ULOGE(TAG_CPU, x, ##args);}
//-------------------------------------------------------------------------------------------------
//  Local Structures
//-------------------------------------------------------------------------------------------------
...
/// @param  <IN>     \b None :L_CPU_NPM_RegWriteByte
/// @param  <OUT>    \b None :
/// @param  <RET>    \b None :
////////////////////////////////////////////////////////////////////////////////
//[CPU][HAL][001] Co-processor Init Flow [START]
void HAL_COPRO_Enable(MS_U32 u32_ADR)
{
    ...
    //(2) Set R2 clock
    // [1:0] = 2'b00
    // [3:2] =  2'b00: 288MHz,  2'b01: 240HMz, 2'b10: 216MHz, 2'b11: Xtal Clock
    // Set to 216 MHz
    HAL_CPU_NPM_RegWriteByte(NPM_REG_CLKGEN1_SECR2, 00UL);
...
//[CPU][HAL][001] Co-processor Init Flow [END]
////////////////////////////////////////////////////////////////////////////////
...
void HAL_FRCR2_Disable(void)
{
    ...
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_STOP, 0x00UL);
    #else
...
void HAL_FRCR2_Enable(MS_U32 u32_ADR)
{
    ...
    //(3) Set CPU SDR base
    base_addr = u32_ADR;
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_SDR_LO_INST_BASE, (base_addr&0x0000FFFFUL)); //I Fetch Offset - Low
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_SDR_HI_INST_BASE, (base_addr>>16)); //I Fetch Offset - High
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_SDR_LO_DATA_BASE, (base_addr&0x0000FFFFUL)); //D Fetch Offset - Low
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_SDR_HI_DATA_BASE, (base_addr>>16)); //D Fetch Offset - High
    #else
    //(3) Set CPU reset base(vector)
    base_addr = u32_ADR >> 16;
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_RST_BASE, base_addr); // reset vector address 0x0(64K alignment)
    #endif

    //(4)Set MAU Mapping
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_MAULV_REG, 0x8900UL);
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_MAULV_2_0_REG, 0x8b00UL);

    //(5) Set RIU   base
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_RIU_BASE, 0xFA00UL); // RIU  base address
    //(6) Set SPI/UART base
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_IO1_BASE, 0xF800UL); // UART base address
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_SPI_BASE, 0xF900UL);
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_SPI_BASE1, 0xF900UL);
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_DQMEM_BASE, 0xFB00UL);
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_QMEM_MASK_HIGH, 0xFF00UL);
    //(7) Set IO space enable (UART, RIU) with QMEM space disabled
    HAL_CPU_PM_RegWrite2Byte(FRC_R2_REG_SPACE_EN, 0x0003UL); // io_space_en[3:0]: UART,RIU, qmem_space_en[4] : disable

    //(8) Set QMEM
    //HAL_CPU_PM_RegWrite2Byte(R2_REG_QMEM_BASE_HIGH, 0x8000UL); // QMEM base address = 0x80000000
    //HAL_CPU_PM_RegWrite2Byte(R2_REG_QMEM_MASK_HIGH, 0xF000UL); // QMEM less, set DCU Decoding..

    //(1) Enable R2 -> Start
    HAL_CPU_PM_RegWriteByte(FRC_R2_REG_STOP, 0x24UL); // miu_sw_rst and sdram_boot = 1
    HAL_CPU_PM_RegWriteByte(FRC_R2_REG_STOP, 0x27UL); // sdram_boot and (miu/r2/r2_rst) =1
#else
    //(2) Set FRC clocks
    //(2-1) R2 MCU(RIU) clock -216 MHz(2'b100000)
    // [1:0] : 2'b00
    ...
    //(4)Set MAU Mapping
    HAL_FRCR2_RegWrite2Byte(FRC_R2_MAULV_REG, 0x8900UL);
    HAL_FRCR2_RegWrite2Byte(FRC_R2_MAULV_2_0_REG, 0x8b00UL);
...
//[CPU][HAL][002] CPU Clock Query Flow [START]
MS_U32 HAL_CPU_QueryClock(void)
{
    MS_U32  dwCpuSpeed = 0;

    // get CA53 processor clock
...
//[CPU][HAL][002] CPU Clock Query Flow [END]
