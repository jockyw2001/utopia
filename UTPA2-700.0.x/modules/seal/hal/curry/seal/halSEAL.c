...
#define SEAL_NON_PM_GROUP23 \
    ...
    E_SEAL_CRYPTODMA_SEC_R2_PROT_NONPM, \
...
#define SEAL_LOCK_GROUP9 \
    ...
    E_SEAL_VE_DCS_MV_WP_N, \
    E_SEAL_VE_DCS_MV_RP_N, \
    E_SEAL_LOCK_ECO, \
    E_SEAL_VTRACK_PROTN
...
static SRAM_TZPC_NSGroup _gSEAL_NSSramBuffer;
...
//TODO check complier flag
    HAL_Seal_SecureMasterSet(E_SEAL_ARM2MIU_NON_SECURE_PROT_NONPM, 0);

#ifdef CONFIG_OPTEE
    // SW patch for SRAM can't be read issue (debug method also cannot be used)
    // init buffer for OPTEE
    {
        MS_U16 u16Cluster;
        MS_U16 u16Client;
        MS_S32 hwIdx;
        MS_U64 mask = 0x1ffULL;

        for(u16Cluster = 0; u16Cluster < SEAL_CLUSTER_NUM; u16Cluster++)
            for(u16Client = 0; u16Client < SEAL_CLUSTER_SIZE; u16Client++)
                _gSEAL_NSSramBuffer.cluster[u16Cluster].flags[u16Client] = ~0ULL; // set all bit to 1 (non-secure)

        // MIU hosts configured here MUST be the same as configured in sboot halSEAL.c - HAL_SEAL_Init
        hwIdx = HAL_SEAL_GetNonPmIpIdx(E_SEAL_ARM2MIU_SECURE_PROT_NONPM);
        _gSEAL_NSSramBuffer.cluster[hwIdx/4].flags[0] &= ~(mask << ((hwIdx%4)*9));
        hwIdx = HAL_SEAL_GetNonPmIpIdx(E_SEAL_CRYPTODMA_GEN_SECURE_PROT_NONPM);
        _gSEAL_NSSramBuffer.cluster[hwIdx/4].flags[0] &= ~(mask << ((hwIdx%4)*9));
        hwIdx = HAL_SEAL_GetNonPmIpIdx(E_SEAL_CRYPTODMA_GEN_NON_SEcure_PROT_NONPM); // HW limitation, only for C2/K7
        _gSEAL_NSSramBuffer.cluster[hwIdx/4].flags[0] &= ~(mask << ((hwIdx%4)*9));
    }
#endif
    ...
    HAL_Seal_SecureMasterSet(E_SEAL_CRYPTODMA_SEC_R2_PROT_NONPM, 1);
...
void HAL_SEAL_ResetSram(void)
{
    ...
    for(u16Cluster = 0; u16Cluster < SEAL_CLUSTER_NUM; u16Cluster++)
    {
        for(u16Client = 0; u16Client < SEAL_CLUSTER_SIZE; u16Client++)
        {
            _gSEAL_NS0GroupBase->cluster[u16Cluster].flags[u16Client] = _gSEAL_NSSramBuffer.cluster[u16Cluster].flags[u16Client] = ~0ULL; // set all bit to 1 (non-secure)
...
//    if(u64StartAddr < HAL_MIU1_BASE)
    ...
    if ( (u32Attribute == 0) || (u64StartAddr == 0 && u64EndAddr == 0))
    {
        ...
        HAL_SEAL_NonPmWriteByte(u32RegAttribute, (E_SEAL_NONSECURE_READ | E_SEAL_NONSECURE_WRITE | E_SEAL_SECURE_READ | E_SEAL_SECURE_WRITE));
    }
    else
    {
        ...
        if (u64StartAddr == HAL_SEAL_NonPmRead6Byte(u32RegStartAddr) && u64EndAddr == HAL_SEAL_NonPmRead6Byte(u32RegEndAddr))
        {
            // Set secure range attribute
...
MS_BOOL HAL_Seal_SecureMasterSet(MS_U32 u32MasterId, MS_BOOL bSecure)
{
    ...
    if(bSecure == 1)
    {
        *((MS_U64*)(_gSEAL_NSSramBuffer.cluster[u16ClusterId].flags)) &= ~(0x1ffULL<<(u16ClientId*SEAL_CLIENT_BITS_COUNT));
        *((MS_U64*)(_gSEAL_NS0GroupBase->cluster[u16ClusterId].flags)) = *((MS_U64*)(_gSEAL_NSSramBuffer.cluster[u16ClusterId].flags));
        __delay();
    }
    else
    {
...
MS_BOOL HAL_Seal_SecureMasterQuery(MS_U32 u32MasterId, MS_BOOL *pSecure)
{
    ...
    if(s32HwIdx == -1)
    {
        HAL_SEAL_ERR("PM Master ID:%u doesn't exist !\n", (unsigned int)u32MasterId);
        return FALSE;
    }
    ...
    if( (*((MS_U64*)(_gSEAL_NSSramBuffer.cluster[u16ClusterId].flags)) & (0x1ffULL<<(u16ClientId*SEAL_CLIENT_BITS_COUNT))) == 0x00 )
    {
        *pSecure = 1;
    }
    else
