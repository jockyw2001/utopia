...
#define SEAL_BANK_RIU_NUM               (0x80UL)
#define SEAL_SECURE_RANGE_RIU_NUM       (0x40UL)
...
//  Local Structures
//-------------------------------------------------------------------------------------------------
//[SEAL][HAL][013] Non-PM IP Table [START]
...
//[SEAL][HAL][013] Non-PM IP Table [END]

//[SEAL][HAL][014] PM IP Table [START]
const eSeal_IP PmIpTbl[SEAL_PM_TBL_IP_NUM] =
{
    SEAL_PM_GROUP0, SEAL_PM_GROUP1, SEAL_PM_GROUP2, SEAL_PM_GROUP3
};
//[SEAL][HAL][014] PM IP Table [END]

//[SEAL][HAL][015] Processor Table [START]
const eSeal_ProcessorId ProcessorTbl[SEAL_TBL_PROCESSOR_NUM] =
{
    SEAL_PROCESSOR_GROUP0
};
//[SEAL][HAL][015] Processor Table [END]

//[SEAL][HAL][016] Lock Table [START]
...
static MS_U16 SEAL_SECURE0_Buffer[SEAL_BANK_RIU_NUM];
static MS_U16 SEAL_SECURE1_Buffer[SEAL_BANK_RIU_NUM];
static MS_U16 SEAL_SECURE2_Buffer[SEAL_BANK_RIU_NUM];
static MS_U16 SEAL_TZPC_NONPM_Buffer[SEAL_BANK_RIU_NUM];
//static MS_U16 SEAL_TZPC_NONPM_2ND_Buffer[SEAL_BANK_RIU_NUM];
...
)
{
    ...
    {
        u32RegDetEn = REG_SECURE2_DETECT_ENABLE;

        switch(u8SecureRangeId)
        {
            case E_HAL_SEAL_SECURE_RANGE_ID0:
                u32RegStartAddr = REG_SECURE2_RANGE0_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE0_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE0_ATTRIBUTE;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID1:
                u32RegStartAddr = REG_SECURE2_RANGE1_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE1_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE1_ATTRIBUTE;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID2:
                u32RegStartAddr = REG_SECURE2_RANGE2_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE2_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE2_ATTRIBUTE;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID3:
                u32RegStartAddr = REG_SECURE2_RANGE3_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE3_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE3_ATTRIBUTE;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID4:
                u32RegStartAddr = REG_SECURE2_RANGE4_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE4_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE4_ATTRIBUTE;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID5:
                u32RegStartAddr = REG_SECURE2_RANGE5_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE5_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE5_ATTRIBUTE;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID6:
                u32RegStartAddr = REG_SECURE2_RANGE6_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE6_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE6_ATTRIBUTE;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID7:
                u32RegStartAddr = REG_SECURE2_RANGE7_START_ADDR;
                u32RegEndAddr = REG_SECURE2_RANGE7_END_ADDR;
                u32RegAttribute = REG_SECURE2_RANGE7_ATTRIBUTE;
                break;
            default:
                return false;
        }
    ...
    if ( (u32Attribute == 0x0)|| (u64StartOffset == 0 && u64EndOffset == 0) )
    {
        ...
        // Set start address
        HAL_SEAL_NonPmWrite6Byte(u32RegStartAddr, 0x0);

        // Set end address
        HAL_SEAL_NonPmWrite6Byte(u32RegEndAddr, 0x0);

        // Set secure range attribute
        HAL_SEAL_NonPmWriteByte(u32RegAttribute, E_SEAL_SECURE_READ | E_SEAL_SECURE_WRITE | E_SEAL_NONSECURE_READ | E_SEAL_NONSECURE_WRITE);
    }
    ...
    else if (u64StartOffset == HAL_SEAL_NonPmRead6Byte(u32RegStartAddr) && u64EndOffset == HAL_SEAL_NonPmRead6Byte(u32RegEndAddr))
    {
        // Set secure range attribute
    ...
    MS_U64 *pEndAddr)
{
    ...
    else if( u8MiuDev == E_HAL_MIU_DEV2 )
    {
        switch(u8SecureRangeId)
        {
            case E_HAL_SEAL_SECURE_RANGE_ID0:
...
MS_BOOL HAL_Seal_GetMiuHitFlag(MS_U8 u8MiuDev)
{
    ...
    switch(u8MiuDev)
    {
        ...
        case E_HAL_MIU_DEV1:
            ...
            break;
        case E_HAL_MIU_DEV2:
            u32RegLogAddr = REG_SECURE2_HITTED_STATUS;
            break;
        default:
            return FALSE;
    }
...
MS_U64 HAL_Seal_GetMiuHitAddr(MS_U8 u8MiuDev)
{
    ...
    switch(u8MiuDev)
    {
        ...
        case E_HAL_MIU_DEV1:
            ...
            break;
        case E_HAL_MIU_DEV2:
            u32RegAddr = REG_SECURE2_HITTED_ADDR;
            break;
        default:
            return FALSE;
    }
...
MS_BOOL HAL_Seal_SetSecureRange(MS_PHY u64phy, MS_U32 u32Length, MS_U32 u32IsSecure)
{
    ...
    if (u32IsSecure) // setup a new securerange
    {
        for (i=E_HAL_SEAL_SECURE_RANGE_ID0; i<E_HAL_SEAL_SECURE_RANGE_NUM; i++, u32RegStartAddr+=RANGE_ADDR_OFFSET)
        {
            ...
            if (u64StartAddr == u64Offset && (u8Val & (1<<i)))
            {
...
{
    MS_U32 u32RegAddr = 0;
    MS_U32 u32RegOffset = 0;
    MS_U16 index;

    //Secure range
    for ( index = 0; index < SEAL_SECURE_RANGE_RIU_NUM; index++ )
    {
        u32RegAddr = REG_SECURE0_RANGE0_START_ADDR + u32RegOffset;
        SEAL_SECURE0_Buffer[index] = HAL_SEAL_NonPmRead2Byte(u32RegAddr);

        u32RegAddr = REG_SECURE1_RANGE0_START_ADDR + u32RegOffset;
        SEAL_SECURE1_Buffer[index] = HAL_SEAL_NonPmRead2Byte(u32RegAddr);

        u32RegAddr = REG_SECURE2_RANGE0_START_ADDR + u32RegOffset;
        SEAL_SECURE2_Buffer[index] = HAL_SEAL_NonPmRead2Byte(u32RegAddr);

        u32RegOffset += 0x02;
    }

    //Detect enable
    SEAL_SECURE0_Buffer[0x76] = HAL_SEAL_NonPmRead2Byte(REG_SECURE0_DETECT_ENABLE);
    SEAL_SECURE1_Buffer[0x76] = HAL_SEAL_NonPmRead2Byte(REG_SECURE1_DETECT_ENABLE);
    SEAL_SECURE2_Buffer[0x76] = HAL_SEAL_NonPmRead2Byte(REG_SECURE2_DETECT_ENABLE);

    //Secure range lock
    SEAL_SECURE0_Buffer[0x7f] = HAL_SEAL_NonPmRead2Byte(REG_SECURE0_LOCK);
    SEAL_SECURE1_Buffer[0x7f] = HAL_SEAL_NonPmRead2Byte(REG_SECURE1_LOCK);
    SEAL_SECURE2_Buffer[0x7f] = HAL_SEAL_NonPmRead2Byte(REG_SECURE2_LOCK);

    u32RegOffset = 0x00;
    for ( index = 0; index < SEAL_BANK_RIU_NUM; index++ )
    {
        //tzpc_nonpm
        u32RegAddr = SEAL_TZPC_NONPM + u32RegOffset;
        SEAL_TZPC_NONPM_Buffer[index] = HAL_SEAL_NonPmRead2Byte(u32RegAddr);

        //tzpc_nonpm_2nd
        //u32RegAddr = SEAL_TZPC_NONPM_2ND + u32RegOffset;
        //SEAL_TZPC_NONPM_2ND_Buffer[index] = HAL_SEAL_NonPmRead2Byte(u32RegAddr);
...
MS_BOOL HAL_SEAL_POWER_RESUME(void)
{
    ...
    {
        u32RegAddr = REG_SECURE0_RANGE0_START_ADDR + u32RegOffset;
        HAL_SEAL_NonPmWrite2Byte(u32RegAddr, SEAL_SECURE0_Buffer[index]);

        u32RegAddr = REG_SECURE1_RANGE0_START_ADDR + u32RegOffset;
        HAL_SEAL_NonPmWrite2Byte(u32RegAddr, SEAL_SECURE1_Buffer[index]);

        u32RegAddr = REG_SECURE2_RANGE0_START_ADDR + u32RegOffset;
        HAL_SEAL_NonPmWrite2Byte(u32RegAddr, SEAL_SECURE2_Buffer[index]);

        u32RegOffset += 0x02;
    }

    //Detect enable
    HAL_SEAL_NonPmWrite2Byte(REG_SECURE0_DETECT_ENABLE,SEAL_SECURE0_Buffer[0x76]);
    HAL_SEAL_NonPmWrite2Byte(REG_SECURE1_DETECT_ENABLE,SEAL_SECURE1_Buffer[0x76]);
    HAL_SEAL_NonPmWrite2Byte(REG_SECURE2_DETECT_ENABLE,SEAL_SECURE2_Buffer[0x76]);

    //Secure range lock
    HAL_SEAL_NonPmWrite2Byte(REG_SECURE0_LOCK,SEAL_SECURE0_Buffer[0x7f]);
    HAL_SEAL_NonPmWrite2Byte(REG_SECURE1_LOCK,SEAL_SECURE1_Buffer[0x7f]);
    HAL_SEAL_NonPmWrite2Byte(REG_SECURE2_LOCK,SEAL_SECURE2_Buffer[0x7f]);
    ...
    for ( index = 0; index < SEAL_BANK_RIU_NUM; index++ )
    {
        //tzpc_nonpm
        u32RegAddr = SEAL_TZPC_NONPM + u32RegOffset;
        HAL_SEAL_NonPmWrite2Byte(u32RegAddr,SEAL_TZPC_NONPM_Buffer[index]);

        //tzpc_nonpm_2nd
        //u32RegAddr = SEAL_TZPC_NONPM_2ND + u32RegOffset;
        //HAL_SEAL_NonPmWrite2Byte(u32RegAddr,SEAL_TZPC_NONPM_2ND_Buffer[index]);
