...
#define __delay() {int i=0; for (i=0; i<100000; i+=2) {i=i-1; _gSealLocalDelayVar+=i;}}
...
#define SEAL_NON_PM_GROUP6 \
    ...
    E_SEAL_ANA_MISC_MPOP_PROT_NONPM, \
    E_SEAL_DUMMY, \
...
#define SEAL_NON_PM_GROUP8 \
    ...
    E_SEAL_HI_VDR2_PROT_NONPM, \
    E_SEAL_HI_VDR2_HIREG_PROT_NONPM, \
...
#define SEAL_NON_PM_GROUP19 \
    ...
    E_SEAL_HI_CODEC_LITE_BLOCK_PROT_NONPM, \
    E_SEAL_PATGEN_HI_CODEC_BLOCK_LITE_NONPM, \
...
void HAL_SEAL_SetIOMapBase(MS_U32 u32NonPmBaseAddr, MS_U32 u32PmBaseAddr)
{
    ...
    HAL_SEAL_NonPmWrite2Byte(u32RegAddr0, 0x0008);
...
void HAL_SEAL_ResetSram(void)
{
    ...
    for(u16Cluster = 0; u16Cluster < SEAL_CLUSTER_NUM; u16Cluster++)
    {
        for(u16Client = 0; u16Client < SEAL_CLIENT_NUM; u16Client++)
        {
            _gSEAL_NS0GroupBase->cluster[u16Cluster].client[u16Client] = 0xff;
            _gSEAL_NS1GroupBase->cluster[u16Cluster].client[u16Client] = 0xff;
...
)
{
    ...
    /* This patch is for K7 only, remove this patch in other chips */
    MS_BOOL bMiu1Patch = FALSE;
    MS_U32 u32RegStartAddrPatch = 0;
    MS_U32 u32RegEndAddrPatch = 0;
    MS_U32 u32RegAttributePatch = 0;
    MS_U8 u8SecureRangeIdPatch = 0;
    ...
    else
    {
        ...
        switch(u8SecureRangeId)
        {
            case E_HAL_SEAL_SECURE_RANGE_ID0:
                ...
                u32RegStartAddrPatch = REG_SECURE1_RANGE4_START_ADDR;
                u32RegEndAddrPatch = REG_SECURE1_RANGE4_END_ADDR;
                u32RegAttributePatch = REG_SECURE1_RANGE4_ATTRIBUTE;
                u8SecureRangeIdPatch = u8SecureRangeId + 4;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID1:
                ...
                u32RegStartAddrPatch = REG_SECURE1_RANGE5_START_ADDR;
                u32RegEndAddrPatch = REG_SECURE1_RANGE5_END_ADDR;
                u32RegAttributePatch = REG_SECURE1_RANGE5_ATTRIBUTE;
                u8SecureRangeIdPatch = u8SecureRangeId + 4;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID2:
                ...
                u32RegStartAddrPatch = REG_SECURE1_RANGE6_START_ADDR;
                u32RegEndAddrPatch = REG_SECURE1_RANGE6_END_ADDR;
                u32RegAttributePatch = REG_SECURE1_RANGE6_ATTRIBUTE;
                u8SecureRangeIdPatch = u8SecureRangeId + 4;
                break;
            case E_HAL_SEAL_SECURE_RANGE_ID3:
                ...
                u32RegStartAddrPatch = REG_SECURE1_RANGE7_START_ADDR;
                u32RegEndAddrPatch = REG_SECURE1_RANGE7_END_ADDR;
                u32RegAttributePatch = REG_SECURE1_RANGE7_ATTRIBUTE;
                u8SecureRangeIdPatch = u8SecureRangeId + 4;
                break;
            default:
		        return false;
    ...
    if ( (u32Attribute == 0x0)|| (u64StartAddr == 0 && u64EndAddr == 0))
    {
        // Disable secure range detection
        u8Val = HAL_SEAL_NonPmReadByte(u32RegDetEn);
        u8Val &= ~(1 << u8SecureRangeId);
        if (bMiu1Patch == TRUE)
            u8Val &= ~(1 << u8SecureRangeIdPatch);
        HAL_SEAL_NonPmWriteByte(u32RegDetEn, u8Val);

        // Reset secure range attribute
        HAL_SEAL_NonPmWriteByte(u32RegAttribute, 0xFF);
        if (bMiu1Patch == TRUE)
            HAL_SEAL_NonPmWriteByte(u32RegAttributePatch, 0xFF);

        // Set start address
        HAL_SEAL_NonPmWrite6Byte(u32RegStartAddr, 0x00);
        if (bMiu1Patch == TRUE)
            HAL_SEAL_NonPmWrite6Byte(u32RegStartAddrPatch, 0x00);

        // Set end address
        HAL_SEAL_NonPmWrite6Byte(u32RegEndAddr, 0x00);
        if (bMiu1Patch == TRUE)
            HAL_SEAL_NonPmWrite6Byte(u32RegEndAddrPatch, 0x00);
    }
    ...
    else if (u64StartAddr == HAL_SEAL_NonPmRead6Byte(u32RegStartAddr) && u64EndAddr == HAL_SEAL_NonPmRead6Byte(u32RegEndAddr))
    {
        // Set secure range attribute
        HAL_SEAL_NonPmWriteByte(u32RegAttribute, u32Attribute);
        if (bMiu1Patch == TRUE)
            HAL_SEAL_NonPmWriteByte(u32RegAttributePatch, u32Attribute);
    }
    else
    {
        // Disable secure range detection
        ...
        // Set start address
        HAL_SEAL_NonPmWrite6Byte(u32RegStartAddr, u64StartAddr);
        if (bMiu1Patch == TRUE)
            HAL_SEAL_NonPmWrite6Byte(u32RegStartAddrPatch, u64StartAddr | (1<<31)); // enable bit 31 for CPU

        // Set end address
        HAL_SEAL_NonPmWrite6Byte(u32RegEndAddr, u64EndAddr);
        if (bMiu1Patch == TRUE)
            HAL_SEAL_NonPmWrite6Byte(u32RegEndAddrPatch, u64EndAddr | (1<<31)); // enable bit 31 for CPU
...
MS_BOOL HAL_Seal_SecureMasterSet(MS_U32 u32MasterId, MS_BOOL bSecure)
{
    ...
    u16ClusterId = (MS_U16)(s32HwIdx / SEAL_CLIENT_NUM);
    u16ClientId = (MS_U16)(s32HwIdx % SEAL_CLIENT_NUM);

    HAL_SEAL_DBG(printf("%s:u32MasterId=%d, u16ClusterId=%d, u16ClientId=%d\n",__FUNCTION__ ,(int)u32MasterId, (int)u16ClusterId, (int)u16ClientId));
    __delay();
    ...
    if(bSecure == 1)
    {
        _gSEAL_NS0GroupBase->cluster[u16ClusterId].client[u16ClientId] = 0x00;
        __delay();
        _gSEAL_NS1GroupBase->cluster[u16ClusterId].client[u16ClientId] = 0x00;
        __delay();
    }
    else
    {
        _gSEAL_NS0GroupBase->cluster[u16ClusterId].client[u16ClientId] = 0xFF;
        __delay();
        _gSEAL_NS1GroupBase->cluster[u16ClusterId].client[u16ClientId] = 0xFF;
        __delay();
    }
...
MS_BOOL HAL_Seal_SecureMasterQuery(MS_U32 u32MasterId, MS_BOOL *pSecure)
{
    ...
    if(s32HwIdx == -1)
    {
        HAL_SEAL_ERR("PM Master ID:%d doesn't exist !\n", (int)u32MasterId);
        return FALSE;
    }
    ...
    u16ClusterId = (MS_U16)(s32HwIdx / SEAL_CLIENT_NUM);
    u16ClientId = (MS_U16)(s32HwIdx % SEAL_CLIENT_NUM);
    u16DumpSel = (u16ClientId / 2);

    //Set dump selection
    u32RegAddr = REG_TZPC_MIU0_CTL;
    u16Val = HAL_SEAL_NonPmRead2Byte(u32RegAddr);
    u16Val &= ~0x0300;
    u16Val |= (u16DumpSel << 8);
    HAL_SEAL_NonPmWrite2Byte(u32RegAddr, u16Val);

    if(_gSEAL_NS0GroupBase->cluster[u16ClusterId].client[u16ClientId] == 0x00)
    {
        *pSecure = 1;
    }
    else
