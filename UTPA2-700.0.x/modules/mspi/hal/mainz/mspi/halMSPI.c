...
static MS_BOOL _HAL_MSPI_CheckDone(void)
{
    ...
    DEBUG_MSPI(E_MSPI_DBGLV_ERR,ULOGE(TAG_MSPI, "ERROR:MSPI Operation Timeout!!!!!\n"));
    return FALSE;
}
...
static MS_U32 _HAL_MSPI_GetCLK(MS_U16 u16CLK)
{
    ...
    case 7: 
        ...
        return u32MspiClk;
    default:
        ULOGE(TAG_MSPI, "ERROR CLOCK SETTING NOT SUPPORT \r\n");
        return u32MspiClk;
    }
}
...
void HAL_MSPI_Init(void)
{
    MS_U16 TempData;
    //init  MSP
    DEBUG_MSPI(E_MSPI_DBGLV_INFO,ULOGI(TAG_MSPI, "HAL_MSPI_Init\n"));
...
{
    DEBUG_MSPI(E_MSPI_DBGLV_INFO,ULOGI(TAG_MSPI, "%s(0x%08X)\n", __FUNCTION__, (int)u32NONPMRegBaseAddr));
    if(u8DeviceIndex)
...
MS_BOOL HAL_MSPI_Read(MS_U8 *pData, MS_U16 u16Size)
{
    ...
    for(i = 0; i < u16Size; i+= MAX_READ_BUF_SIZE)
    {
        ...
        HAL_MSPI_Trigger();
        DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,ULOGD(TAG_MSPI, "Read Size %x\n",j));
        for(u8Index = 0; u8Index < j; u8Index++)
        {
            ...
            if(u8Index & 1)
            {
                ...
                DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,ULOGD(TAG_MSPI, "read Buf data %x index %d\n",u16TempBuf, u8Index));
...
MS_BOOL HAL_MSPI_Wirte(MS_U8 *pData, MS_U16 u16Size)
{
    ...
    for(i = 0; i < u16Size; i+= MAX_WRITE_BUF_SIZE)
    {
        ...
        DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,ULOGD(TAG_MSPI, "Write Size %x\n",j));
        for(u8Index = 0; u8Index < j; u8Index++)
        {
            if(u8Index & 1)
            {
                u16TempBuf = (pData[u8Index] << 8) | pData[u8Index-1];
                DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,ULOGD(TAG_MSPI, "write Buf data %x index %d\n",u16TempBuf, u8Index));
