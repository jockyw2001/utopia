...
#define XC_DLC_FRAME_WIDTH_4K2K_HALF 1920 //need DLC information
#define XC_DLC_FRAME_WIDTH_FHD_HALF 960 //need DLC information
...
void msDlcInitWithCurve( WORD wHStart, WORD wHEnd, WORD wVStart, WORD wVEnd )
{
    ...
    {
        u16HGuardBangSize= MDrv_Read2Byte( REG_ADDR_H_GUARD_BAND_OUTPUT_SIZE_MAIN) & 0xFFF;

        // Main Window
        MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_HST, wHStart/8);

        if((MDrv_Read2Byte(REG_ADDR_VOP_OUTPUT_H_TOTAL) & 0x3FFF)>3000)    // 4K output
        {
            MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_HEN, XC_DLC_FRAME_WIDTH_4K2K_HALF/8);
        }
        else
        {
            MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_HEN, XC_DLC_FRAME_WIDTH_FHD_HALF/8);
        }

        MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_HST_SEC, u16HGuardBangSize/8);

        if((MDrv_Read2Byte(REG_ADDR_VOP_OUTPUT_H_TOTAL) & 0x3FFF)>3000)   // 4K output
        {
            MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_HEN_SEC, (wHEnd-XC_DLC_FRAME_WIDTH_4K2K_HALF+u16HGuardBangSize)/8);
        }
        else
        {
            MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_HEN_SEC, (wHEnd-XC_DLC_FRAME_WIDTH_FHD_HALF+u16HGuardBangSize)/8);
        }

    }
    // Set histogram range - V
    // Main Window
    MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_VST, wVStart/8);
    MDrv_Write2Byte(REG_ADDR_HISTOGRAM_RANGE_M_VEN, wVEnd/8);
    ...
    g_wLumiTotalCount = (MDrv_Read2Byte(REG_ADDR_HISTOGRAM_TOTAL_COUNT_L)
                    | (MDrv_Read2Byte(REG_ADDR_HISTOGRAM_TOTAL_COUNT_L+2)<<16))/4;
...
BOOL msGetHistogramHandler(BOOL bWindow)
{
    ...
    if (g_DlcParameters.ucDlcTimeOut)
    {
        ...
        for (ucTmp = 0, wTmpHistogramSum = 0, ucHistogramOverflowIndex = 0xFF; ucTmp < 32; ucTmp++)
        {
            ...
            g_wLumaHistogram32H[ucTmp]=MDrv_Read2Byte(REG_ADDR_HISTOGRAM_DATA_32+ (4*ucTmp)+2)&0x000F;
            g_wLumaHistogram32H[ucTmp] <<= 16;
            g_wLumaHistogram32H[ucTmp] |= MDrv_Read2Byte(REG_ADDR_HISTOGRAM_DATA_32+ (4*ucTmp));
            g_wLumaHistogram32H[ucTmp] = g_wLumaHistogram32H[ucTmp] /4;
        ...
        g_wLumiAverageTemp = ((MDrv_Read2Byte( REG_ADDR_HISTOGRAM_TOTAL_SUM_L+2 )<<16) | MDrv_Read2Byte( REG_ADDR_HISTOGRAM_TOTAL_SUM_L ))/4;
