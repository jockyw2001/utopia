...
void Hal_DLC_init_riu_base(MS_VIRT u32riu_base)
{
...
void Hal_DLC_set_HDR(MS_BOOL bEnable)
{
    halDlc_FuncEnter();

    MS_U16 u16SC0Mux = ((R2BYTEMSK(0x102E02,0x00F0)) >> 4);
    MS_U16 u16HDRMux = ((R2BYTEMSK(0x102E06,0x0F00)) >> 8);
    printf("[%s] DLC_HDR_enable %d, HDR_Mux 0x%x, SC0_Mux 0x%x\033[0m\n",__FUNCTION__,bEnable,u16HDRMux,u16SC0Mux);
    //Below flow because of Kano's HDR module is in front of XC_IPMUX, but can not bypass FD_MASK control signal to XC
    //To compatible with SDK flow, we must set HDR mux and clock here.
    //SDK flow: MApi_XC_Mux_CreatePath => MApi_XC_SetInputSource => MApi_XC_DLC_SetHDRInit => MDrv_MVOP_SetInputCfg

    //because of apiXC header not ready, we marked E_XC_INIT_MISC_A_IPMUX_HDR_MODE temp
    XC_INITMISC stXC_MISC = {0};
    MApi_XC_GetMISCStatus(&stXC_MISC);
    if(bEnable == TRUE)
    {
        //stXC_MISC.u32MISC_A |= E_XC_INIT_MISC_A_IPMUX_HDR_MODE;
        //MApi_XC_Init_MISC(&stXC_MISC,sizeof(stXC_MISC));

        if(u16SC0Mux == 3)
        {
            //set HDR mux to MVOP
            W2BYTEMSK(0x102E06,0x0300,0x0F00);
            //set HDR CLK
            W2BYTEMSK(0x100B16,0x000C,0x003C);
            //disable HSK
            SC_W2BYTEMSK(REG_SC_BK02_10_L, 0x0, 0x25);
        }
        else if(u16SC0Mux == 1)
        {
            //set HDR mux to DVI
            W2BYTEMSK(0x102E06,0x0100,0x0F00);
            //set HDR CLK
            W2BYTEMSK(0x100B16,0x0004,0x003C);
        }
        else if(u16SC0Mux == 8)
        {
            //set HDR mux to MVOP2
            W2BYTEMSK(0x102E06,0x0300,0x0F00);
            //set HDR CLK
            W2BYTEMSK(0x100B16,0x0020,0x003C);
            //disable HSK
            SC_W2BYTEMSK(REG_SC_BK02_10_L, 0x0, 0x2D);
        }
        //set SC0 mux to HDR
        W2BYTEMSK(0x102E02,0x0060,0x00F0);
    }
    else
    {
        //stXC_MISC.u32MISC_A &= ~E_XC_INIT_MISC_A_IPMUX_HDR_MODE;
        ...
        if(u16SC0Mux == 6)
        {
            ...
            {
                //set SC0 mux to MVOP
                W2BYTEMSK(0x102E02,0x0030,0x00F0);
                u16SC0Mux = 3;
            }
            else if(u16HDRMux == 1)
            {
                 //set SC0 mux to HDMI
                W2BYTEMSK(0x102E02,0x0010,0x00F0);
                u16SC0Mux = 1;
            }
            else if(u16HDRMux == 8)
            {
                 //set SC0 mux to MVOP2
                W2BYTEMSK(0x102E02,0x0080,0x00F0);
                u16SC0Mux = 8;
            }
        }
        ...
        if(u16SC0Mux == 3)
        {
            SC_W2BYTEMSK(REG_SC_BK02_10_L, 0x25, 0x25);
        }
        else if(u16SC0Mux == 8)
        {
            SC_W2BYTEMSK(REG_SC_BK02_10_L, 0x2D, 0x2D);
        }
