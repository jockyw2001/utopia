...
void msDlc_SetCaptureRange(MS_U16 wHStart,MS_U16 wHEnd, MS_U16 wVStart, MS_U16 wVEnd)
{
    ...
    if (REG_ADDR_HISTOGRAM_RANGE_M_HST != REG_NULL)
    {
        if(msReadByte(REG_ADDR_VIP_HISTOGRAM_4K2K_MODE) & BIT(2)||((wHEnd-wHStart)>=3000))
        {
            // Set histogram range - H
            // Main Window
            msWriteByte(REG_ADDR_HISTOGRAM_RANGE_M_HST, wHStart/8/2);
            msWriteByte(REG_ADDR_HISTOGRAM_RANGE_M_HEN, wHEnd/8/2);

            // Virtical size need to change by display
            // Set histogram range - V
            // Main Window
            msWriteByte(REG_ADDR_HISTOGRAM_RANGE_M_VST, wVStart/8/2);
            msWriteByte(REG_ADDR_HISTOGRAM_RANGE_M_VEN, wVEnd/8/2);
        }
        else
        {
            // Set histogram range - H
