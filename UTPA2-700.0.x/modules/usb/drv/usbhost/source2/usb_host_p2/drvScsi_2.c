...
struct us_data Mass_stor_us1_port2;
#if 1
struct LUN_Device  Mass_stor_device_port2[MAX_LUN_SUPPORT];
#else
struct LUN_Device Mass_stor_device_1_port2; //support 4 LUN
struct LUN_Device Mass_stor_device_2_port2;
struct LUN_Device Mass_stor_device_3_port2;
struct LUN_Device Mass_stor_device_4_port2;
struct LUN_Device Mass_stor_device_5_port2;
#endif
...
struct stForceModeSense1A gForceModeSense1A_Port2[] =
{
        {0x0DD8, 0x1448},
        {0x1AA6, 0x0201},
        {0x0DD8, 0x1414},
...
extern void MDrv_USBGetVIDPID_Port2(U16 *pVID, U16 *pPID);

//BOOLEAN gDeviceFatalError=FALSE;
void Init_Mass_Storage_Structure_Port2(void)
{
  U16   uVID, uPID;

  //Mass_stor_us=&Mass_stor_us1;
  Mass_stor_us1_port2.Mass_stor_device=&Mass_stor_device_port2[0];
#ifdef   ENABLE_CBI_HOST
  if ( (psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceProtocol==0x00)||
       (psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceProtocol==0x01) )
    Mass_stor_us1_port2.max_lun=0;
  else
#endif
  {
    ...
    if ( ((uVID == 0x8564) && (uPID == 0x1000)) ||
         ((uVID == 0xFCA) && (uPID == 0x8004)) ) //20130301, for blackberry phone
          ...
          Mass_stor_us1_port2.max_lun=   Host20_MSCD_GetMaxLUN_Port2();
  }
  //printf("Max Lun:%02bx\n",Mass_stor_us1.max_lun);
    if (Mass_stor_us1_port2.max_lun > (MAX_LUN_SUPPORT-1) )
        Mass_stor_us1_port2.max_lun = MAX_LUN_SUPPORT-1;
}
//***************************************************************************************
...
//***************************************************************************************
// Function Name:vSCSICmd_MODE_SENSE_OperCode5A_Port2
// Description:
// Input:
// Output:
// Status:
//***************************************************************************************
void vSCSICmd_MODE_SENSE_OperCode5A_Port2( Scsi_Cmnd *srb,INT8U u8PageCode,INT8U *u8Buffer)
{
  ...
  //srb->cmnd[4] = DATA_LENGTH_MODE_SENSE;
  memset(&pOTGH_PT_BLK_Port2->sCBW.u8CB[0], 0, 16);
  pOTGH_PT_BLK_Port2->sCBW.u8CB[0]= MODE_SENSE_5A;
  pOTGH_PT_BLK_Port2->sCBW.u8CB[2]= u8PageCode;

#if 0
  pOTGH_PT_BLK_Port2->sCBW.u8CB[4]= DATA_LENGTH_MODE_SENSE_OperCode5A;		//RCC?
#else
  pOTGH_PT_BLK_Port2->sCBW.u8CB[7] = (INT8U)(DATA_LENGTH_MODE_SENSE_OperCode5A >> 8);
  pOTGH_PT_BLK_Port2->sCBW.u8CB[8] = (INT8U)(DATA_LENGTH_MODE_SENSE_OperCode5A );
#endif
  ...
  for(u8j =0; u8j< ScsiCmd_Fail_Retry ;u8j++)
  {
	...
	if(Mass_stor_us1_port2.srb->result == SAM_STAT_GOOD)
	{
	  ...
	  Mass_stor_us1_port2.device.u8DeviceType = (u8InquiryData[0] & 0x1F);
        ...
        Mass_stor_us1_port2.Mass_stor_device[lun].bRMB = (u8InquiryData[1] & 0x80) >> 7;
        //printf("Removable: %d\n", Mass_stor_us1_port2.Mass_stor_device[lun].bRMB);
...
//***************************************************************************************
// Function Name:bSCSI_MODE_SENSE_OperCode5A_Port2
// Description:To know the mode of device <1>.Write Protect => Y/N
// Input:
// Output:
// Status:P-OK
//***************************************************************************************
///////////////////////////////////////////////////////////////////////////////
//		bSCSI_MODE_SENSE_OperCode5A_Port2()
//		Description:
//			1. scsi mode sense command.
//		input: none
//		output: TRUE or FALSE (BOOLEAN)
///////////////////////////////////////////////////////////////////////////////
BOOLEAN bSCSI_MODE_SENSE_OperCode5A_Port2(U8 lun)
{
  INT8U  u8j;
  INT8U *u8ModeSenseData;
  BOOLEAN result;
  ...
  for(u8j =0; u8j< ScsiCmd_Fail_Retry ;u8j++)
  {
    memset(Mass_stor_us1_port2.srb, 0, sizeof(struct scsi_cmnd ));
    //Build SCSI command.
		vSCSICmd_MODE_SENSE_OperCode5A_Port2( Mass_stor_us1_port2.srb,0x3F, u8ModeSenseData);
    Mass_stor_us1_port2.srb->lun=lun;
	...
	if(Mass_stor_us1_port2.srb->result == SAM_STAT_GOOD)
	{
	  ...
	  //Now we only use u8ModeSenseData[2], this byte save device
	  //write protection information
	  Mass_stor_us1_port2.Mass_stor_device[lun].bWriteProtect = (u8ModeSenseData[3]==0x80)?
												TRUE:FALSE;
	  return TRUE;
	}
  }
...
BOOLEAN bSCSI_TEST_UNIT_READY2_Port2(U8 lun)
{
  ...
  for(u8j =0; u8j< TestUnitReady_Fail_Retry ;u8j++)
  {
	...
	else
	{
	  ...
	  result=vSCSI_REQUEST_SENSE_Port2(lun);
      if ((pUsbData_Port2[12]==0x3a)&&(pUsbData_Port2[13]==0)) return FALSE;  //no media
      if ((pUsbData_Port2[12]==0x28)&&(pUsbData_Port2[13]==0)) return FALSE;  //media change
	  if (result==FALSE) return result;
	}
    #ifndef ATV_SERISE_USE
	MsOS_DelayTask(10);
...
void GetValidLun_Port2(void)
{
  ...
  if ( (Mass_stor_us1_port2.max_lun == 0) && (!LunDevice[0].bRMB) && LunDevice[0].bDeviceReady)
  {
  ...
  for (LunIdx=0; LunIdx<=Mass_stor_us1_port2.max_lun; LunIdx++)
  {
    if (LunDevice[LunIdx].bDeviceValid)
    {
      ...
      else
      {
        ...
        {
            if(!bSCSI_READ_CAPACITY_Port2(LunIdx))
            {
              if (!bSCSI_READ_FORMAT_CAPACITY_Port2(LunIdx))
              {
...
BOOLEAN bInit_USB_LUN_Port2(U8 Lun)
{
  ...
  if(!bSCSI_READ_CAPACITY_Port2(Lun))
  {
    ...
    ii = 0;
    while ( (gForceModeSense1A_Port2[ii].VID != 0) && (gForceModeSense1A_Port2[ii].PID != 0) )
    {
        if ( (uVID == gForceModeSense1A_Port2[ii].VID) && (uPID == gForceModeSense1A_Port2[ii].PID) )
        {
        ...
        ii++;
        if (ii >= (sizeof(gForceModeSense1A_Port2) / sizeof(struct stForceModeSense1A)))
          break;
    }
    ...
    if (bIsForceMS1A)
        ...
        bSCSI_MODE_SENSE_OperCode5A_Port2(Lun);
