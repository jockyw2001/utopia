...
U8  MDrv_Usb_Device_Enum_EX_Port2(void);
#endif
...
U8 gUsbRetryCount_Port2=3;
U16 gUsbChipID_Port2 = 0xFFFF;
U8 gUsbChipVersion_Port2 = 0xFF;
...
#define USB_HOST_P2_DRV_VERSION                  /* Character String for DRV/API version             */  \
    MSIF_TAG,                           /* 'MSIF'                                           */  \
    MSIF_CLASS,                         /* '00'                                             */  \
    MSIF_CUS,                           /* 0x0000                                           */  \
    MSIF_MOD,                           /* 0x0000                                           */  \
...
static MSIF_Version _drv_usb_host_p2_version = {
    .DDI = { USB_HOST_P2_DRV_VERSION },
};
...
struct stDontReEnumList gDontReEnumList_Port2[] =
{
        {0x0951, 0x1603},
        {0x05E3, 0x0718},
        {0x0BC2, 0xA013},
...
extern void drvUSBHost_TurnOffPowerDownMode_Port2(void);
extern void drvUSBHost_TurnOnPowerDownMode_Port2(void);
...
U8 MDrv_USB_Host_GetLibVer_Port2(const MSIF_Version **ppVersion)
{
    if(!ppVersion)
        return 1;

    *ppVersion = &_drv_usb_host_p2_version;
    return 0;
}

U16 MDrv_USBGetChipID_Port2(void)
{
    if (gUsbChipID_Port2== 0xFFFF)
        gUsbChipID_Port2 = HAL_USB_GetChipID();

    return gUsbChipID_Port2;
}

extern MS_U8 MDrv_SYS_GetChipRev(void);
U8 MDrv_USBGetChipVersion_Port2(void)
{
    if (gUsbChipVersion_Port2== 0xFF)
        gUsbChipVersion_Port2 = MDrv_SYS_GetChipRev();

    return gUsbChipVersion_Port2;
}
...
{
    XBYTE_OR(gUTMI2_BASE, offset, val);
}

void UTMI2_ANDXBYTE(U8 offset,U8 val)
{
    XBYTE_AND(gUTMI2_BASE, offset, val);
}

void UTMI2_SETXBYTE(U8 offset,U8 val)
{
    XBYTE_SET(gUTMI2_BASE, offset, val);
}

U8 UTMI2_READXBYTE(U8 offset)
{
    return XBYTE_READ(gUTMI2_BASE, offset);
}

void UHC2_ORXBYTE(U8 offset,U8 val)
{
    XBYTE_OR(gUHC2_BASE, offset, val);
}

void UHC2_ANDXBYTE(U8 offset,U8 val)
{
    XBYTE_AND(gUHC2_BASE, offset, val);
}

void UHC2_SETXBYTE(U8 offset,U8 val)
{
    XBYTE_SET(gUHC2_BASE, offset, val);
}

U8 UHC2_READXBYTE(U8 offset)
{
    return XBYTE_READ(gUHC2_BASE, offset);
}

void USBC2_ORXBYTE(U8 offset,U8 val)
{
    XBYTE_OR(gUSBC2_BASE, offset, val);
}

void USBC2_ANDXBYTE(U8 offset,U8 val)
{
    XBYTE_AND(gUSBC2_BASE, offset, val);
}

void USBC2_SETXBYTE(U8 offset,U8 val)
{
    XBYTE_SET(gUSBC2_BASE, offset, val);
}

U8 USBC2_READXBYTE(U8 offset)
{
    return XBYTE_READ(gUSBC2_BASE, offset);
}

#ifdef Enable_Burning_Test
U8 buf2[512];
void UsbTestPort2(void)
{
  U16 i,j;
  U32 idx=0,xxx;
  ...
  //buf2 = (U8*) msAPI_Memory_Allocate(0x200, BUF_ID_USB_HOST );
  printf("buf2: %X\n", (UINT)buf2);
  ...
  while (1)
  {
    ...
    //MDrv_MIU_Copy
	xxx=idx % 10000;
    if (MDrv_UsbBlockWriteFromMIU_Port2(LunIndex,50+xxx,1,(U32)VirtoPhyAddr(buf2))==FALSE)
    {
      printf("\r\n write failed\n");
      break;
    }

    if (MDrv_UsbBlockReadToMIU_Port2(LunIndex,50+xxx,1,(U32)VirtoPhyAddr(buf2))==FALSE)
    {
      printf("\r\n read failed\n");
      break;
    }
  ...
  //msAPI_Memory_Free((void*)buf2,BUF_ID_USB_HOST);
}
#endif
#if 0
...
BOOLEAN MDrv_UsbHost_Init_Port2(void)
{
  ...
  UTMI2_ORXBYTE(0x06,0x40);                //Force HS TX current enable and CDR stage select
  UTMI2_ANDXBYTE(0x06,~0x20);                      //clear bit 5
  if (MDrv_USBGetChipID_Port2() == CHIPID_EMERALD)
      UTMI2_ORXBYTE(0x06, 0x04);  //Force HS TX current source enable, for MHL noise issue, for Emerald only

  UTMI2_ORXBYTE(0x06,0x03);                //reset UTMI
  MsOS_DelayTask(2);
  UTMI2_ANDXBYTE(0x06,0xfc);

    if ( (MDrv_USBGetChipID_Port2() == CHIPID_NEPTUNE) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_ERIS) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_PLUTO) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TRITON) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA2) )
    {
        UTMI2_SETXBYTE(0x29,0x08);
    }
    else if (MDrv_USBGetChipID_Port2()==CHIPID_EUCLID) // Euclid
    {
        UTMI2_ANDXBYTE(0x29,0xF7);               //disable full speed retime
    }

#if defined(Enable_Issue_TestPacket) && !defined(ENABLE_HOST_TEST)
  UTMI2_SETXBYTE(0x03,0);              //for device disconnect status bit
#else
  UTMI2_SETXBYTE(0x03,0xa8);               //for device disconnect status bit
#endif

  //XBYTE[gUTMI2_BASE+0x07]|=0x02;      //
  #ifdef DTV_STANDARD_LIB
   if  (MDrv_USBGetChipID_Port2()==CHIPID_NEPTUNE)            //Neptune , after U06
   {
      if (MDrv_USBGetChipVersion_Port2()>= 0x06)              //U07
        {
            UTMI2_ORXBYTE(0x07, 0x02);
            UTMI2_SETXBYTE(0x2c,0xc1);
            UTMI2_SETXBYTE(0x2d,0x3b);//enable TX common mode,
            UTMI2_ORXBYTE(0x2f, 0x0e);            //preemsis
        }
    }
     else if ((MDrv_USBGetChipID_Port2()>=CHIPID_ERIS)&&(MDrv_USBGetChipID_Port2()<=CHIPID_TITANIA))                    //Eris: 3 ,Titania: 4, Pluto: 5
    {
            UTMI2_SETXBYTE(0x2c,0xc5);
            UTMI2_SETXBYTE(0x2d,0x3b);//enable TX common mode,
            UTMI2_ORXBYTE(0x2f, 0x0e);            //preemsis
    }
       else if (MDrv_USBGetChipID_Port2()==CHIPID_PLUTO)          // Pluto: 5
    {
            UTMI2_ORXBYTE(0x2c, 0xc1);
            UTMI2_ORXBYTE(0x2d, 0x3);//enable TX common mode,
            UTMI2_ORXBYTE(0x2f, 0x4a);            //preemsis
            UTMI2_ORXBYTE(0x13, 0x70);
            XBYTE_AND(gUTMI_BASE,0,0xfe);       //clear port 0 IREF
     }
     else if (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA2)          // Titania 2
    {
            UTMI2_ORXBYTE(0x2c, 0xc1);
            UTMI2_ORXBYTE(0x2d, 0x3);//enable TX common mode,
            UTMI2_ORXBYTE(0x2f, 0x4a);            //preemsis
     }
    else if (MDrv_USBGetChipID_Port2()==CHIPID_TRITON)        //Triton=6
    {
            UTMI2_SETXBYTE(0x2c,0xc1);
            UTMI2_SETXBYTE(0x2d,0x3b);//enable TX common mode,
            UTMI2_ORXBYTE(0x2f,0x0e);            //preemsis
    ...
    else if ( (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA3) ||         // Titania 3, Titania 4
                (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA4) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA7) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA8) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA9) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA12) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA13) ||
                (MDrv_USBGetChipID_Port2() == CHIPID_JANUS) ||
                (MDrv_USBGetChipID_Port2() == CHIPID_JANUS2) ||
                (MDrv_USBGetChipID_Port2() == CHIPID_AMBER6) )
    {
            UTMI2_ORXBYTE(0x2c, 0x10);  //TX-current adjust to 105%
            UTMI2_ORXBYTE(0x2d, 0x02);  //Pre-emphasis enable
            UTMI2_ORXBYTE(0x2f, 0x81);  //HS_TX common mode current enable (100mV);Pre-emphasis enable (10%)
            UTMI2_ORXBYTE(0x09,0x81);     //UTMI RX anti-dead-lock, ISI improvement
    }
    else if ( (MDrv_USBGetChipID_Port2()==CHIPID_AMBER1) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_AMBER5) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_AMBER7) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_AMBER3) ||
                (MDrv_USBGetChipID_Port2()==CHIPID_AMETHYST)  ||
                (MDrv_USBGetChipID_Port2()==CHIPID_EAGLE))
    {
            //for Amber1 later 40nm before Agate
            UTMI2_ORXBYTE(0x2c, 0x98);
            UTMI2_ORXBYTE(0x2d, 0x02);
            UTMI2_ORXBYTE(0x2e, 0x10);
            UTMI2_ORXBYTE(0x2f, 0x01);
            UTMI2_ORXBYTE(0x09,0x81);     //UTMI RX anti-dead-lock, ISI improvement
    }
    else if ( (MDrv_USBGetChipID_Port2() == CHIPID_EDISON) ||
              (MDrv_USBGetChipID_Port2() == CHIPID_EIFFEL) )
    {
            //for Agate later 40nm, same as 55nm setting2
            UTMI2_ORXBYTE(0x2c, 0x90);  //TX-current adjust to 105%
            UTMI2_ORXBYTE(0x2d, 0x02);  //Pre-emphasis enable
            UTMI2_ORXBYTE(0x2f, 0x81);  //HS_TX common mode current enable (100mV);Pre-emphasis enable (10%)
            UTMI2_ORXBYTE(0x09,0x81);     //UTMI RX anti-dead-lock, ISI improvement
    }
    ...
    if ( (MDrv_USBGetChipID_Port2() == CHIPID_EIFFEL) )
        UTMI2_ANDXBYTE(0x08, ~0x08); //Special setting for Eiffel analog LIB issue
    else
    {
  //for Edison later 240M clock enhance, early chip will ignore this 2 bits
  UTMI2_ORXBYTE(0x08,0x08);    //bit<3> for 240's phase as 120's clock set 1, bit<4> for 240Mhz in mac 0 for faraday 1 for etron
    }

  /* Enable Cross Point ECO 2012/03/20 */
  if ((MDrv_USBGetChipID_Port2() == CHIPID_ELK) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_AMBER5) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_EAGLE) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_EMERALD) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_EDISON) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_AGATE))
  {
      UTMI2_ORXBYTE(0x04,0x40);    //deglitch SE0 (low-speed cross point)
  }
  else if((MDrv_USBGetChipID_Port2() == CHIPID_JANUS2))
  {
      UTMI2_ORXBYTE(0x0a,0x80);    //deglitch SE0 (low-speed cross point)
  }

  /* Enable Power Noice ECO 2012/03/20 */
  if ((MDrv_USBGetChipID_Port2() == CHIPID_JANUS2) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_ELK) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_AMBER5) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_EAGLE) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_EMERALD) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_EDISON) ||
       (MDrv_USBGetChipID_Port2() == CHIPID_AGATE))
  {
      USBC2_ORXBYTE(0x02,0x40);    //use eof2 to reset state machine (power noise)
  }
  ...
  if((MDrv_USBGetChipID_Port2() == CHIPID_ELK) ||
      ...
      (MDrv_USBGetChipID_Port2() == CHIPID_AGATE))
  {
      UTMI2_ORXBYTE(0x04,0x20);    //hw auto deassert sw reset (tx/rx reset)
  }

  /* enable patch for the assertion of interrupt(Lose short packet interrupt) 2012/03/28 */
  if((MDrv_USBGetChipID_Port2() == CHIPID_AMBER5) ||
      (MDrv_USBGetChipID_Port2() == CHIPID_AGATE))
  {
      USBC2_ORXBYTE(0x04,0x80);    //patch for the assertion of interrupt
  }
  ...
  if((MDrv_USBGetChipID_Port2() == CHIPID_AMBER5) ||
      (MDrv_USBGetChipID_Port2() == CHIPID_AGATE))
  {
      USBC2_ORXBYTE(0x04,0x40);    //add patch to Period_EOF1
  }

  /* enable eco for short packet MDATA 2012/07/05 */
  if((MDrv_USBGetChipID_Port2() == CHIPID_EDISON) ||
      (MDrv_USBGetChipID_Port2() == CHIPID_EMERALD))
  {
      USBC2_ORXBYTE(0x00,0x10);    //short packet MDATA in Split transaction clears ACT bit (LS dev under a HS hub)
  }

  /* enable eco for pv2mi bridge mis-behavior 2012/12/05 */
  if((MDrv_USBGetChipID_Port2() == CHIPID_EINSTEIN) ||
     (MDrv_USBGetChipID_Port2() == CHIPID_NAPOLI) ||
     (MDrv_USBGetChipID_Port2() == CHIPID_MIAMI))
  {
      USBC2_ORXBYTE(0x0A,0x40);    //fix pv2mi bridge mis-behavior
  }

  if (MDrv_USBGetChipID_Port2()==CHIPID_PLUTO)                   // If is Pluto
      UTMI2_ORXBYTE(0x09,0x01);     //ISI improvement
  else
      UTMI2_ORXBYTE(0x13,0x02);     //ISI improvement
  ...
  {
    UTMI2_ORXBYTE(0x0b, 0x80); //TX timing select latch path
  }

  if (MDrv_USBGetChipID_Port2()==CHIPID_EUCLID) // Euclid
  {
      UTMI2_SETXBYTE(0x09,0x81);//0x20;       //patch low tempture,FL meta issue and enable new FL RX engin
  }
  else
  {
      UTMI2_ORXBYTE(0x09,0x60);//0x20;       //patch low tempture,FL meta issue and enable new FL RX engin
  }

#if 1
  DRV_USB_DBG(printf("Host: %X\n", mwOTG20_Control_HOST_SPD_TYP_Rd()));
  if (mwOTG20_Control_HOST_SPD_TYP_Rd()==1)            //low speed,for HID
      UTMI2_ANDXBYTE(0x09,~0x40);                      //old setting
#endif
  ...
  if ( (MDrv_USBGetChipID_Port2() == CHIPID_NEPTUNE) ||
      ...
      (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA2) )
  {
      UTMI2_ANDXBYTE(0x27,0xf3);
      UTMI2_ORXBYTE(0x27,0x08);  //(1) Offset 27 (．h3AA7) bit <3:2> set 2・b10   // RX bias current => 60uA (default 40uA)

      //(2) Offset 2A (．h3AAA) bit <3:2> set 2・b11               // Squelch voltage => 100mV (default 150mV)
  #ifdef DTV_STANDARD_LIB
      UTMI2_SETXBYTE(0x2a,0x07);
  #endif
  }
  ...
  if (  (MDrv_USBGetChipID_Port2() == CHIPID_NEPTUNE) ||
        ...
        (MDrv_USBGetChipID_Port2() == CHIPID_TRITON) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA2) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA3) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA4) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA7) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA8) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA9) ||
        ...
        (MDrv_USBGetChipID_Port2() == CHIPID_TITANIA13) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_JANUS) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_MARIA10) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_MACAW12) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_JANUS2) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_AMBER1) ||
        (MDrv_USBGetChipID_Port2() == CHIPID_AMBER3)  ||
        (MDrv_USBGetChipID_Port2() == CHIPID_AMBER5)  ||
        (MDrv_USBGetChipID_Port2() == CHIPID_AMBER6)  ||
        (MDrv_USBGetChipID_Port2() == CHIPID_AMBER7)   ||
        (MDrv_USBGetChipID_Port2() == CHIPID_AMETHYST) )
    {
        UTMI2_ORXBYTE(0x15,0x20); //HOST CHIRP Detect
    }
    else
    {
        UTMI2_ORXBYTE(0x15,0x60); // change to 55 interface (bit6)
    }
...
BOOLEAN _MDrv_UsbHost_Init0_Port2(void)
{
  //BOOLEAN result=FALSE;
...
BOOLEAN _MDrv_UsbHost_Init1_Port2(void)
{
  BOOLEAN result=FALSE;
...
BOOLEAN MDrv_UsbHost_Init_Port2(void)
{
    ...
    result = _MDrv_UsbHost_Init0_Port2();
    if (result == FALSE)
        return FALSE;

    MsOS_DelayTask(USB_HOST_INIT_DELAY);
    return(_MDrv_UsbHost_Init1_Port2());
}
#endif
...
U8 MDrv_GET_VALID_LUN_NUM_PORT2(void)
{
...
U8 MDrv_GET_MASS_VALID_LUN_PORT2()
{
  ...
  if (psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceClass != USB_INTERFACE_CLASS_MSD)
  {
...
void MDrv_USB_Init_Port2(U32 USBAdr)
{
  ...
  gUsbRetryCount_Port2=3;

  if (USB_BUFFER_START_ADR_4K_ALIGN_Var_Port2 % 4096 != 0)
  {
    DRV_USB_DBG( printf("Error USB Port2 Starting address is not 4K alignmented\n"));
  }
  ...
  if (!drvUSBHost_isPowerSaveModeEnable())
  {
    ...
    if ( (MDrv_USBGetChipID_Port2()==CHIPID_NEPTUNE) ||  //Neptune
         (MDrv_USBGetChipID_Port2()==CHIPID_ERIS) ||  //Eris
         (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA) ||  //Titania
         (MDrv_USBGetChipID_Port2()==CHIPID_PLUTO) ||  //Pluto
         (MDrv_USBGetChipID_Port2()==CHIPID_TRITON) ||  //Triton
         (MDrv_USBGetChipID_Port2()==CHIPID_TITANIA2) )   // Titania 2
    {
        UHC2_ANDXBYTE(0x34,0xBF); //set suspend
        UHC2_ORXBYTE(0x34,0x40); //clr suspend
        MsOS_DelayTask(2);
        UTMI2_ORXBYTE(0x00,0x01);		// override mode enable for power down control
        UTMI2_ORXBYTE(0x01,0x40);	// enable IREF power down
        UTMI2_ORXBYTE(0x01,0x02);   // enable PLL power down
        UTMI2_ANDXBYTE(0x01,0xFD);   // disable PLL power down
    }
...
void  MDrv_ClearUsbDeviceStatusPort2(void)
{
  //if (gUsbDeviceState_Port2==BAD_DEVICE)
  //{
...
void ResetUsbHardwarePort2(void)
{
  UTMI2_ORXBYTE(0x06,0x03);        //reset UTMI
  UTMI2_ANDXBYTE(0x06,0xfc);
  mbHost20_USBCMD_HCReset_Set();
...
void UsbPort2UTMIInitial(void)
{
   ...
   UTMI2_ORXBYTE(0x01,0x02);   // enable PLL power down
   UTMI2_ANDXBYTE(0x01,0xFD);   // disable PLL power down
   UTMI2_ANDXBYTE(0x00,~0x01);		// override mode enable for power down control
...
BOOLEAN isInitForTestPkt_Port2 = TRUE;
...
BOOLEAN MDrv_UsbDeviceConnect_Port2(void)
{
  ...
  */
#ifdef Enable_Issue_TestPacketByHW
    if (isInitForTestPkt_Port2)
    {
        isInitForTestPkt_Port2= FALSE;
        MDrv_UsbSendTestPacket_Port2();
    }
    return FALSE;
#endif
  ...
  if (drvUSBHost_isPowerSaveModeEnable())
  {
    ...
    {
        if (MsOS_Timer_DiffTimeFromNow(UsbPowerSavingTimerPort2) < 300 )
            return FALSE;
    }
    if (gUsbDeviceState_Port2==POWER_SAVING)
    {
#ifdef ENABLE_HOST_TEST
       if (USBC2_READXBYTE(8)&0xC0)
#else
       if (USBC2_READXBYTE(8)&0x40)
#endif
       {
           drvUSBHost_TurnOffPowerDownMode_Port2();

           UsbPowerSavingTimerPort2 = MsOS_GetSystemTime();
  ...
  if (mwHost20_PORTSC_ConnectStatus_Rd())
  {
    if ( (gUsbDeviceState_Port2==USB11_DEVICE) ||(gUsbDeviceState_Port2==USB20_DEVICE) )
    {
        ...
        if (mwHost20_PORTSC_ConnectChange_Rd())
        {
            ...
            UHC2_ANDXBYTE(0x40, (U8)~0x80);//clear force enter FSmode
  ...
  else
  {
    ...
    #ifndef ATV_SERISE_USE
    if (MsOS_Timer_DiffTimeFromNow(UsbStartTimePort2) > 1000 )
    {

      UsbStartTimePort2=MsOS_GetSystemTime();
...
BOOLEAN MDrv_UsbBlockReadToMIU_Port2(U8 lun,U32 u32BlockAddr, U32 u32BlockNum,U32 u32MIUAddr)
{
  ...
  {
    retrycnt++;
    result= bSCSI_Read_Write10_Port2(FALSE,lun,  u32BlockAddr, u32BlockNum, PhytoCacheAddr(u32MIUAddr));
    if (result==TRUE) break;
    if (result==FALSE)
    {
      if (retrycnt > gUsbRetryCount_Port2)
      {
       if (gUsbStatus_Port2==USB_TIMEOUT)
             gUsbDeviceState_Port2=BAD_DEVICE;    //mark as bad device
       else if(gUsbStatus_Port2 == USB_OK)
       {//give another chance : to restart the host & device(for example : read failed with device STALL)
            if(retrycnt > gUsbRetryCount_Port2+1)
                break;
      ...
      MINI_DEBUG( printf("USBDisk Read failed\n"));
       if (gUsbStatus_Port2==USB_TIMEOUT)
       {
        if ((retrycnt==2)&&(mwOTG20_Control_HOST_SPD_TYP_Rd()==2) )              //make sure it is hi speed
        {
          ...
          UHC2_ORXBYTE(0x40,0x80);//force enter FSmode
        }
	    #ifndef ATV_SERISE_USE
        msAPI_Timer_ResetWDT();
		#else
...
BOOLEAN MDrv_UsbBlockWriteFromMIU_Port2(U8 lun, U32 u32BlockAddr, U32 u32BlockNum,U32 u32MIUAddr)
{
  ...
  result= bSCSI_Read_Write10_Port2(TRUE,lun,  u32BlockAddr, u32BlockNum, PhytoCacheAddr(u32MIUAddr));
...
U8 MDrv_USB_GetXDataExtStackCountPort2(void)
{
...
BOOLEAN  MDrv_Usb_Device_Enum_Port2(void)
{
  ...
  if ( (gUsbDeviceState_Port2==USB11_DEVICE) ||(gUsbDeviceState_Port2==USB20_DEVICE) )
  {
      ...
      MDrv_USBGetVIDPID_Port2(&uVID, &uPID);
      ii = 0;
      while ( (gDontReEnumList_Port2[ii].VID != 0) && (gDontReEnumList_Port2[ii].PID != 0) )
      {
          if ( (uVID == gDontReEnumList_Port2[ii].VID) && (uPID == gDontReEnumList_Port2[ii].PID) )
          {
          ...
          ii++;
          if (ii >= (sizeof(gDontReEnumList_Port2) / sizeof(struct stDontReEnumList)))
            break;
      }
  }
...
port2_redo_init:
  ...
  if  (flib_Host20_Enumerate_Port2(1,3)==0)
  {
  ...
  {
    UTMI2_SETXBYTE(0x2a,0);

    rootUSBDeviceInterfaceClass= psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceClass;
    ...
    //else if(rootUSBDeviceInterfaceClass==USB_INTERFACE_CLASS_MSD)//Mass storage class
		   ...
		   (psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceProtocol == 0x50) )
    {
	  ...
	  if((gSpeed_Port2==0)&&(gUsbStatus_Port2==USB_TIMEOUT))
	  {
        ...
        UTMI2_ANDXBYTE(0x06,0xfc);
		   ...
		   ((psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceProtocol == 0x00)||
		   (psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceProtocol == 0x01)) )
    {
    ...
    {
        if (drvUSBHost_HID_Init(USB_PORT_2) == 0)
        {
            result = TRUE;
...
#else // support device enumeration dividable
U8  _MDrv_Usb_Device_Enum0_Port2(void)
{
    if (gUsbDeviceState_Port2==BAD_DEVICE)
...
BOOLEAN  _MDrv_Usb_Device_Enum_OK_Port2(void)
{
    BOOLEAN result=FALSE;
    U8 rootUSBDeviceInterfaceClass = USB_INTERFACE_CLASS_NONE;
...
BOOLEAN  MDrv_Usb_Device_Enum_Port2(void)
{
  ...
  retval = _MDrv_Usb_Device_Enum0_Port2();
  if (retval != 2)
...
port2_redo_init:
  ...
  {
    result = _MDrv_Usb_Device_Enum_OK_Port2();
  }
...
2: fail
*/
U8 enum_state_port2 = 0;
U32 usbDevEnumTimer_port2;
U8  MDrv_Usb_Device_Enum_EX_Port2(void)
{
  BOOLEAN result=FALSE;
  U8 retval;
...
port2_redo_init_ex:
  //USB ROOT
    ...
    switch (enum_state_port2)
    {
        ...
        case 1: // first entry
            RetryCount++;
            if (_MDrv_UsbHost_Init0_Port2()==FALSE)
                goto port2_fail_exit_ex; // MDrv_UsbHost_Init() part 1
            enum_state_port2 = 2;
            usbDevEnumTimer_port2 = MsOS_GetSystemTime();
            return DEVENUM_INPROGRESS;

        case 2: // in-progress stage 1
            if (MsOS_Timer_DiffTimeFromNow(usbDevEnumTimer_port2) < USB_HOST_INIT_DELAY)
                return DEVENUM_INPROGRESS;

            if (_MDrv_UsbHost_Init1_Port2()==FALSE) // MDrv_UsbHost_Init() part 2
                goto port2_fail_exit_ex;

            fr = _flib_Host20_Enumerate_Port2(1, 3); // flib_Host20_Enumerate() part 1
            if (fr ==0)
                break;
        ...
        case 3: // in-progress stage 2
            if (MsOS_Timer_DiffTimeFromNow(usbDevEnumTimer_port2) < USB_ENUM_DELAY)
                return DEVENUM_INPROGRESS;
    }
  ...
  if  (fr==0)
  {
    if ((gUsbStatus_Port2==USB_TIMEOUT)||(gUsbStatus_Port2==USB_INIT_FAIL))
    {
      ...
      msAPI_Timer_ResetWDT();
      #endif
      enum_state_port2 = 1; // retry use
      goto port2_redo_init_ex;
    }
    gUsbDeviceState_Port2=BAD_DEVICE;
    goto port2_fail_exit_ex;
  }
...
port2_fail_exit_ex:
  ...
  if (mwHost20_PORTSC_ConnectChange_Rd())
    mwHost20_PORTSC_ConnectChange_Set();
  enum_state_port2 = 0; // reset to initial
...
//
/////////////////////////////////////////////////////////////////////////////////////////
U8 UTMI_Reg_14_Port2 = 0xff, UTMI_Reg_15_Port2 = 0xff;
void Port1_IssueTestPacket_Initial(void)
{
    UTMI2_SETXBYTE(0x02, 0x84);
    UTMI2_SETXBYTE(0x03, 0x20);

    USBC2_SETXBYTE(0x00, 0x0A);
    USBC2_SETXBYTE(0x00, 0x28);

    UTMI2_SETXBYTE(0x06, 0x00);
    UTMI2_SETXBYTE(0x07, 0x00);
    UTMI2_SETXBYTE(0x10, 0x00);
    UTMI2_SETXBYTE(0x11, 0x00);
    UTMI2_SETXBYTE(0x2c, 0x00);
    UTMI2_SETXBYTE(0x2d, 0x00);
    UTMI2_SETXBYTE(0x2e, 0x00);
    UTMI2_SETXBYTE(0x2f, 0x00);
    if (UTMI_Reg_14_Port2== 0xff)
        UTMI_Reg_14_Port2= UTMI2_READXBYTE(0x14);
    else
        UTMI2_SETXBYTE(0x14, UTMI_Reg_14_Port2);

    if (UTMI_Reg_15_Port2== 0xff)
        UTMI_Reg_15_Port2= UTMI2_READXBYTE(0x15);
    else
        UTMI2_SETXBYTE(0x15, UTMI_Reg_15_Port2);
    ...
    if  (MDrv_USBGetChipID_Port2()==CHIPID_NEPTUNE)            //Neptune , after U06
   {
      if (MDrv_USBGetChipVersion_Port2()>= 0x6)              //U07
        {
            UTMI2_ORXBYTE(0x07, 0x02);
            UTMI2_SETXBYTE(0x2c,0xc1);
            UTMI2_SETXBYTE(0x2d,0x3b);//enable TX common mode,
    ...
    else if ((MDrv_USBGetChipID_Port2()>=CHIPID_ERIS)&&(MDrv_USBGetChipID_Port2()<=CHIPID_TITANIA))                    //Eris: 3 ,Titania: 4,
    {
            UTMI2_SETXBYTE(0x2c,0xc5);
            UTMI2_SETXBYTE(0x2d,0x3b);//enable TX common mode,
            UTMI2_ORXBYTE(0x2f, 0x0e);            //preemsis
...
extern void USB_DACHE_FLUSH_Port2(U32 addr, U32 length);

void IssueTestPacket_Port2(U8 *TestDataAddr)
{
  ...
  printf("Start Test Packet on Port1\n");
  ...
  USB_DACHE_FLUSH_Port2((U32)TestDataAddr, 128);
  //printf("[9]=0x%bx\n", TestData[9]);

  //DbgPortEanble();

  UHC2_ORXBYTE(0x50,0x14); //enable test packet and lookback

  UTMI2_ORXBYTE(0x06,0x03); //TR/RX reset
  UTMI2_ANDXBYTE(0x06,0xFC);
  ...
  while(1)
  {
     ...
     UHC2_SETXBYTE(0x74,(U8)DMAAddress);
     UHC2_SETXBYTE(0x75,(U8)(DMAAddress>>8));
     UHC2_SETXBYTE(0x76,(U8)(DMAAddress>>16));
     UHC2_SETXBYTE(0x77,(U8)(DMAAddress>>24));
    ...
    UHC2_SETXBYTE(0x70,(U8)datreg32);
    UHC2_SETXBYTE(0x71,(U8)(datreg32>>8));
    UHC2_SETXBYTE(0x72,(U8)(datreg32>>16));
    UHC2_SETXBYTE(0x73,(U8)(datreg32>>24));

    UHC2_ORXBYTE(0x70,0x01);//DMA start
    ...
    while(!(UHC2_READXBYTE(0x44) &0x08))
    {
...
//#ifdef Enable_Issue_TestPacketByHW
#if 1
void MDrv_SendTestPacketByHW_Port2(void)
{
    printf("Start test packet on port 1\n");
    UTMI2_SETXBYTE(0x14, 0x00);
    UTMI2_SETXBYTE(0x15, 0x06);

    UTMI2_SETXBYTE(0x10, 0x38);
    UTMI2_SETXBYTE(0x11, 0x00);

    UTMI2_SETXBYTE(0x32, 0xFE);
    UTMI2_SETXBYTE(0x33, 0x0B);
}
#endif

void IssueTestJ_Port2(void)
{
    printf("TEST_J on Port1\n");

    UTMI2_SETXBYTE(0x2c, 0x04);
    UTMI2_SETXBYTE(0x2d, 0x20);
    UTMI2_SETXBYTE(0x2e, 0x00);
    UTMI2_SETXBYTE(0x2f, 0x00);

    USBC2_ORXBYTE(0, 0x02); //Enable UHC_RST
    MsOS_DelayTask(10);
    USBC2_ANDXBYTE(0, 0xFD);

    //UHCREG(0x32)|=0x01;
    UHC2_ORXBYTE(0x32,0x01);
    MsOS_DelayTask(10);
    //UHCREG(0x32)&=0xfe;
    UHC2_ANDXBYTE(0x32,0xfe);

    MsOS_DelayTask(10);
    UHC2_ORXBYTE(0x50,0x01); //enable test J
}

void IssueTestK_Port2(void)
{
    printf("TEST_K on port1\n");
    ...
    MsOS_DelayTask(10);
    UHC2_ORXBYTE(0x50,0x02); //enable test K
}
...
void IssueSE0_Port2(void)
{
    ...
    UTMI2_SETXBYTE(0x06, 0xA0);
    UTMI2_SETXBYTE(0x07, 0x04);
...
{
    return gUsbTimeout_Port2;
}

void MDrv_USBSetqTDTimeoutValue_Port2(U8 u8Value)
{
    gUsbTimeout_Port2= u8Value;
}

U8 MDrv_USBGetIORetryCount_Port2(void)
{
    return gUsbRetryCount_Port2;
}

void MDrv_USBSetIORetryCount_Port2(U8 u8Value)
{
    gUsbRetryCount_Port2= u8Value;
}

void MDrv_USBGetVIDPID_Port2(U16 *pVID, U16 *pPID)
{
    *pVID = ((U16) psAttachDevice_Port2->sDD.bVIDHighByte << 8 ) |
                  (U16) psAttachDevice_Port2->sDD.bVIDLowByte;

    *pPID = ((U16) psAttachDevice_Port2->sDD.bPIDHighByte<< 8 ) |
                  (U16) psAttachDevice_Port2->sDD.bPIDLowByte;
}
