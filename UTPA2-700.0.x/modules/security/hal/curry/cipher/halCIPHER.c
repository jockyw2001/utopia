...
#define REG_READ 0
#define REG_WRITE 1
#define REG32_R(Addr)    _REG32_R((MS_VIRT)Addr)
#define REG32_W(Addr, Value)    _REG32_W((MS_VIRT)Addr, Value)
#define REG16_R(Addr)    _REG16_R((MS_VIRT)Addr)
#define REG16_W(Addr, Value)    _REG16_W((MS_VIRT)Addr, Value)
...
//  Inline Function
//--------------------------------------------------------------------------------------------------
#ifdef CIPHER_SMC_MODE
static MS_U64 REG_REE2ATF(MS_U64 u64Addr, MS_U64 u64Value, MS_U64 u64Status)
{
    MS_U64 u64Tmp = 0;
    ...
    asm volatile(
        ...
        "mov %0, x0\n\t"
        : "=r" (u64Tmp)
        : "m" (u64Addr), "m" (u64Value), "m" (u64Status):"x0", "x1", "x2", "x3"
    );
...
static MS_U32 _REG32_R(MS_VIRT u32Reg)
{
#ifdef CIPHER_SMC_MODE
    return REG_REE2ATF(u32Reg, 0, REG_READ) ;
#else
    return *((volatile MS_U32*)u32Reg);
#endif
}

static void _REG32_W(MS_VIRT u32Reg, MS_U32 u32Value)
{
#ifdef CIPHER_SMC_MODE
    REG_REE2ATF(u32Reg, u32Value, REG_WRITE);
#else
    *((volatile MS_U32*)u32Reg) = u32Value;
#endif
}

static MS_U16 _REG16_R(MS_VIRT u32Reg)
{
    return REG32_R(u32Reg) & 0x0000FFFFUL;
}

static void _REG16_W(MS_VIRT u32Reg, MS_U16 u16Value)
{
...
void WRITE_CMDQ(MS_U32 u32Value)
{
    ...
    REG32_W(&_AESDMACtrl->Cmd_Queue, u32Value) ;
}
...
{
    MS_U32 u32DMACtrl = REG32_R(&_AESDMACtrl->Dma_Ctrl[REG_DMA_CTRL]);
    REG32_W(&_AESDMACtrl->Dma_Ctrl[REG_DMA_CTRL], u32DMACtrl | REG_EXCEPT_FLAG_CLEAR);  //clear all except flag
    REG32_W(&_AESDMACtrl->Dma_Ctrl[REG_DMA_CTRL], u32DMACtrl & ~REG_EXCEPT_FLAG_CLEAR);  //enable acpu except flag
}
...
{
    MS_U32 DMA_CTRL = REG32_R(&_AESDMACtrl->Dma_Ctrl[REG_DMA_CTRL]);
    REG32_W(&_AESDMACtrl->Dma_Ctrl[REG_DMA_CTRL], DMA_CTRL | REG_DMA_SW_RESET);  //reset CryptoDMA
    REG32_W(&_AESDMACtrl->Dma_Ctrl[REG_DMA_CTRL], DMA_CTRL & ~REG_DMA_SW_RESET); //enable CryptoDMA
...
void HAL_CIPHER_DMA_GetData(MS_U8 *u8Data)
{
    ...
    tmp = REG32_R(&_AESDMACtrl->Dma_Out[0]);
    u8Data[0] = (tmp >> 0)  & 0xFF ;
    u8Data[1] = (tmp >> 8)  & 0xFF ;
    u8Data[2] = (tmp >> 16) & 0xFF ;
    u8Data[3] = (tmp >> 24) & 0xFF ;

    tmp = REG32_R(&_AESDMACtrl->Dma_Out[1]);
    u8Data[4] = (tmp >> 0)  & 0xFF ;
    u8Data[5] = (tmp >> 8)  & 0xFF ;
    u8Data[6] = (tmp >> 16) & 0xFF ;
    u8Data[7] = (tmp >> 24) & 0xFF ;

    tmp = REG32_R(&_AESDMACtrl->Dma_Out[2]);
    u8Data[8] = (tmp >> 0)  & 0xFF ;
    u8Data[9] = (tmp >> 8)  & 0xFF ;
    u8Data[10] = (tmp >> 16) & 0xFF ;
    u8Data[11] = (tmp >> 24) & 0xFF ;

    tmp = REG32_R(&_AESDMACtrl->Dma_Out[3]);
...
void HAL_CIPHER_DMA_GetRpt(MS_U32 *DmaRpt)
{
    ...
    {
        DmaRpt[i] = REG32_R(&_AESDMACtrl->Dma_Reportp[i]);
    }
}
...
MS_BOOL HAL_CIPHER_DMA_Start(MS_BOOL Decrypt, HAL_CIPHER_INTMODE IntMode, MS_BOOL bWaitCmdQ, MS_U16 u16CmdID)
{
    ...
    if(CMD_DMA_KSEL_TRNG == _uDmaCtrl0.DmaCtrl0.KeySel)
    {
        ...
        {
            HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]STR Key invalid!\n",__func__, __LINE__));
            return FALSE;
        }
    }

    MS_U32 MIUCrossBar = REG32_R(_u32RegBase + REG_MIU_CROSSBAR);  //MIU CrossBar Bank 0x1613
    REG32_W(_u32RegBase + REG_MIU_CROSSBAR, MIUCrossBar | 0xf);  //enable MIU crossbar
...
DMA_RETRY:
    ...
    u32SpareCnt = REG32_R(&_AESDMACtrl->SpareCnt) ;
    ...
    if (u32SpareCnt > CmdLen)
    {
        ...
        REG32_W(&_AESDMACtrl->SpareCnt, u32SpareCnt) ;
        return TRUE ;
...
{
	MS_U32 u32ExcTmp = 0;
    MS_U32 u32Rpt   = REG32_R(&_AESDMACtrl->Dma_Reportp[0]) ;
    MS_U32 u32RptId = u32Rpt & REG_CIPHER_RPT_THREAD_MSK;

    if(u32RptId == u32CmdID)
    {
        u32ExcTmp = (MS_U32) REG32_R(&_AESDMACtrl->Dma_Ctrl[REG_EXCEPT_FLAG]);
        *u32Ret = u32ExcTmp;
    }
    else
    {
...
{
    MS_U32 u32ExcTmp = (MS_U32) REG32_R(&_AESDMACtrl->Dma_Ctrl[REG_EXCEPT_FLAG]);
...
{
    MS_U16 u16MaskTmp = REG16_R(_u32RegBase + REG_HST0_FIQ_MASK_63_48);
    u16MaskTmp &= REG_HTS0_FIQ_MASK_CRYPTODMA;  //FIQ 56 in reg_hst0_fiq_mask_63_48_
    REG16_W(_u32RegBase + REG_HST0_FIQ_MASK_63_48, u16MaskTmp);
}

void HAL_CIPHER_IntClear(void)
{
    REG16_W(_u32RegBase + REG_HST0_FIQ_STATUS_63_48, REG_HTS0_FIQ_CRYPTODMA); //set 1 to clear interrupt
}
...
void HAL_CIPHER_Hash_GetRpt(MS_U32 *HashRpt, MS_U32 u32Size)
{
    ...
    for (i = 0 ; i < tmp ; i++)
    {
        HashRpt[i] = REG32_R(&_AESDMACtrl->Hash_Reportp[i]);
    }
}
...
SHA_RETRY:
        ...
        if (u32SpareCnt > CmdLen)
        {
            ...
            {
                REG32_W(&_AESDMACtrl->Hash_Reportp[1], _u32InitWordCnt);
            }
...
MS_BOOL HAL_CIPHER_Hash_CmdDone(MS_U32 u32CmdID, MS_U32 *u32Ret)
{
    ...
    MS_U32 u32Tmp = REG32_R(&_AESDMACtrl->Hash_Reportp[0]) ;
...
MS_BOOL HAL_CIPHER_Misc_Random(MS_U8 *pu8Buf, MS_U32 u32Size)
{
    ...
    while(u32Count < u32Size)
    {
        do{
            do{
                ...
                MS_U32 u32Tmp=REG32_R(_u32RegBase+REG_RNG_TRNG_SCPU);
                u32Tmp^= REG_RNG_TRNG_ACK_SCPU;
                REG32_W(_u32RegBase+REG_RNG_TRNG_SCPU, u32Tmp);
            }//Read random number valid
            while( !(REG32_R(_u32RegBase+REG_RNG_TRNG_SCPU) & REG_RNG_TRNG_VALID_SCPU_MASK) );
        }
        while( (u16TRN = (REG32_R(_u32RegBase+REG_RNG_TRNG_SCPU) & REG_RNG_TRNG_OUT_SCPU_MASK))==u16PreTRN );
        u16PreTRN = u16TRN;
    ...
    while(u32Count < u32Size)
    {
        do{
            do{
                //Toggle random number bit
                MS_U32 u32Tmp=REG32_R(_u32RegBase+REG_RNG_TRNG_ACPU);
                u32Tmp^= REG_RNG_TRNG_ACK_ACPU;
                REG32_W(_u32RegBase+REG_RNG_TRNG_ACPU, u32Tmp);
            }//Read random number valid
            while( !(REG32_R(_u32RegBase+REG_RNG_TRNG_ACPU) & REG_RNG_TRNG_VALID_ACPU_MASK) );
        }
        while( (u16TRN = (REG32_R(_u32RegBase+REG_RNG_TRNG_ACPU) & REG_RNG_TRNG_OUT_ACPU_MASK))==u16PreTRN );
        u16PreTRN = u16TRN;
