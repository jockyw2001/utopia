...
#define __delay() {int i=0; for (i=0; i<1000; i+=2) {i=i-1; _gSealLocalDelayVar+=i;}}
...
MS_U8 AESDMA_Rand(void)
{
   ...
   __delay(); // delay a short time to prevent CPU accessing too quick and get same register value
   while(retryCount-- && !(_AESDMA_REG32_R(&_SECUREBASECtrl[0].Reg0203)&REG_RNG_READY));
