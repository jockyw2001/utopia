...
void AESDMA_Reset(void)
{
    ...
    REG32_W((&_PARSERCtrl[0].Parser_Pid1),0x01000000);
...
{
    // in Mustang biforst bring-up, we fond interrupt handler is keeping calling so that R2 hang
    // so we temporary disable interrupt for Mustang
...
void SHA_SelMode(AESDMA_SHAMode eMode)
{
    ...
    //// comment for mustung SHA Fail: avoid toggle speed mode n
...
void SHA_SetByPassTable(MS_BOOL bEnable)
{
    if (bEnable == TRUE)
    {
        REG32_W((&_SHARNGCtrl[0].Sha_Ctrl) , _AESDMA_REG32_R(&_SHARNGCtrl[0].Sha_Ctrl)|SHARNG_CTRL_SHA_BYPASS_TABLE_EN);
    }
    else
    {
        REG32_W((&_SHARNGCtrl[0].Sha_Ctrl) , _AESDMA_REG32_R(&_SHARNGCtrl[0].Sha_Ctrl)&~(SHARNG_CTRL_SHA_BYPASS_TABLE_EN));
...
void SHA_SetInitialHashEn(MS_BOOL bEnable)
{
    if (bEnable == TRUE)
    {
        REG32_W((&_SHARNGCtrl[0].Sha_Ctrl) , _AESDMA_REG32_R(&_SHARNGCtrl[0].Sha_Ctrl)|SHARNG_CTRL_SHA_INITIAL_HASH_EN);
    }
    else
    {
        REG32_W((&_SHARNGCtrl[0].Sha_Ctrl) , _AESDMA_REG32_R(&_SHARNGCtrl[0].Sha_Ctrl)&~(SHARNG_CTRL_SHA_INITIAL_HASH_EN));
...
void SHA_SetShaManualMode(MS_BOOL bEnable)
{
    if (bEnable == TRUE)
    {
        REG32_W((&_SHARNGCtrl[0].Sha_Ctrl) , _AESDMA_REG32_R(&_SHARNGCtrl[0].Sha_Ctrl)|SHARNG_CTRL_SHA_WORKMODE_MANUAL_EN);
    }
    else
    {
        REG32_W((&_SHARNGCtrl[0].Sha_Ctrl) , _AESDMA_REG32_R(&_SHARNGCtrl[0].Sha_Ctrl)&~(SHARNG_CTRL_SHA_WORKMODE_MANUAL_EN));
...
void SHA_SetInitialSha(MS_U32 u32Buf)
{
    ...
    for( Index = 0; Index < u32Size; Index++ )
    {
        REG32_W((&_SHARNGCtrl[0].Sha_Out[Index + u32offset]), *((MS_U32 *)u32Buf + Index));
    }
}
...
void HDCP_ProcessCipher (MS_U8 u8Idx, MS_U8 *pu8Data, MS_U8* pu8privData)
{
    //
    //Ks^LC128:
    //P0: Bank 0x162A_h・00~07
    //P1: Bank 0x162B_h・00~07
    //P2: Bank 0x162C_h・00~07
    //P3: Bank 0x162D_h・00~07
    //Riv:
    //P0: Bank 0x162A_h・08~0B
    //P1: Bank 0x162B_h・08~0B
    //P2: Bank 0x162C_h・08~0B
    //P3: Bank 0x162D_h・08~0B
    //
    ...
    if (u8Idx >= MAX_HDMI_PORT_MUN) 
    {
        printf ("[Error] %s (%d), Wrong HDMI Port\n", __FUNCTION__, __LINE__);
        return;
    }
    ...
    offset = u8Idx * 0x200;
    BKOffset = u8Idx * 0x200;
    
    pData = (REG_HDCP22Data*)((MS_U32)_HDCP22Data + offset);
    pCtrl = (REG_HDCP22SEKCtrl*)((MS_U32)_HDCP22SekCtrl + BKOffset);
...
void HDCP_GetHdcpCipherState(MS_U8 u8Idx, MS_U8 *pu8State)
{
    //
    // Set SKE successful:
    // P0: Bank 0x162A_h・0E[0]
    // P1: Bank 0x162B_h・0E[0]
    // P2: Bank 0x162C_h・0E[0]
    // P3: Bank 0x162D_h・0E[0]
    //
