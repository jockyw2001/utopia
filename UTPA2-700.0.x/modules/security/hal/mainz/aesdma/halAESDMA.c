...
static REG_MIPSOCCCLKCtrl    *_MispOccCtrl = (REG_MIPSOCCCLKCtrl*)REG_MIPS_OCC_CLK_BASE;
...
void HAL_AESDMA_SetBank(MS_VIRT u32NonPmBankAddr, MS_VIRT u32PmBankAddr)
{
    ...
    _MispOccCtrl = (REG_MIPSOCCCLKCtrl*)(_u32RegBase + REG_MIPS_OCC_CLK_BASE);
}
...
void AESDMA_Enable_Clk(void)
{
    ...
    //
    // clk_rc_p  0x1239_0x03 = 0x004F
    // Due to the security concern
    // Set this CLK at SBOOT
    //

    //
    // clk_mips_occ12  0x110c_0x11[10] = 1
    //
    u32Val = _AESDMA_REG32_R(&_MispOccCtrl[0].Reg0001);    
    u32Val |= 0x04000000;
    REG32_W((&_MispOccCtrl[0].Reg0001) , u32Val);
...
void AESDMA_Parser_Set_Pid(MS_U8 u8Index, MS_U16 u16Pid)
{
    ...
    {
        ULOGE(TAG_AESDMA, "Edison do not support other PID\n");
    }
}
...
void HDCP_ProcessCipher (MS_U8 u8Idx, MS_U8 *pu8Data, MS_U8* pu8privData)
{
    ...
    {
        int off = i*4;
        cipherkey[i] = ((MS_U32)(pu8Data[off]) & 0xff) | (((MS_U32)(pu8Data[off+1]) & 0xff)<<8) | (((MS_U32)(pu8Data[off+2]) & 0xff)<<16) | (((MS_U32)(pu8Data[off+3]) & 0xff)<<24);
    }
    for (i=0; i<2; ++i)
    {
        int off = i*4;
        riv[i] = ((MS_U32)(pu8privData[off]) & 0xff) | (((MS_U32)(pu8privData[off+1]) & 0xff)<<8) | (((MS_U32)(pu8privData[off+2]) & 0xff)<<16) | (((MS_U32)(pu8privData[off+3]) & 0xff)<<24);
    }
