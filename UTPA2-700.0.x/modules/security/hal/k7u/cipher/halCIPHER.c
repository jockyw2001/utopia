...
MS_U32 _HAL_CIPHER_GetCmdLen(HAL_CIPHER_CMDTYPE CmdType)
{
    ...
    if ( E_CIPHER_TYPE_DMA == CmdType )
    {
        ...
        {
            if((_uDmaCtrl0.DmaCtrl0.Aes_mode == CMD_AES_MODE_192) || (_uDmaCtrl0.DmaCtrl0.Aes_mode == CMD_AES_MODE_256))
            {
                u32CmdLen += 8 ;
            }
            else
...
HAL_CIPHER_ALGO _HAL_CIPHER_TransMainAlgo(CIPHER_MAIN_ALGO algo)
{
    ...
    case E_CIPHER_MAIN_RC4_128:
        ...
        break;
    case E_CIPHER_MAIN_SM4:
        hal_algo = E_DMA_ALGO_SM4;
        break;
    default:
...
MS_BOOL HAL_CIPHER_ResetKey(MS_U32 u32KeyIdx)
{
    ...
    _uDmaCtrl0.DmaCtrl0.Key_Index = u32KeyIdx;
...
MS_BOOL HAL_CIPHER_DMA_Set_OutputDstTo(CIPHER_MEM_TYPE OutputDstTo, MS_U8* pu8Data, MS_U32 u32Size)
{
    ...
    _uDmaCtrl1.DmaCtrl1.Dest0= (MS_U32)dst;
...
void HAL_CIPHER_DMA_Set_OutputDstKL(MS_BOOL bDstKL)
{
    if(bDstKL)
    {
        _uDmaCtrl1.DmaCtrl1.Dest1 = CMD_DMA_OUTPUT_SRAM_KL;
        _uDmaCtrl0.DmaCtrl0.DK = TRUE;
    }
    else
    {
        _uDmaCtrl1.DmaCtrl1.Dest1 = CMD_DMA_OUTPUT_DEST;
    }
}
...
MS_BOOL HAL_CIPHER_DMA_Set_Key(DRV_CIPHER_KEY stKey)
{
    ...
    if(E_DMA_KSEL_REGKEY == KeySrc)
    {
        ...
        {
            if(E_DMA_ALGO_AES == algo)
            {
                if((stKey.u8KeyLen != E_DMA_AES_KEY128) && (stKey.u8KeyLen != E_DMA_AES_KEY192) && (stKey.u8KeyLen != E_DMA_AES_KEY256))
                {
                    HAL_DEBUG_CIPHER(CIPHER_DBG_ERR, printf("[%s][%d]Invalid KeyLen[%d]\n",
                        __func__, __LINE__, stKey.u8KeyLen));
                    return FALSE ;
                }

                if(stKey.u8KeyLen == E_DMA_AES_KEY192)
                {
                    _uDmaCtrl0.DmaCtrl0.Aes_mode = CMD_AES_MODE_192;
                }
                else if(stKey.u8KeyLen == E_DMA_AES_KEY256)
                {
                    _uDmaCtrl0.DmaCtrl0.Aes_mode = CMD_AES_MODE_256;
                }
            }
...
SET_KEY_DONE:
    ...
    _uDmaCtrl0.DmaCtrl0.Key_Index = stKey.u8KeyIdx;
...
{
    _uDmaCtrl1.DmaCtrl1.Dest0 = OutputReg ;
...
MS_BOOL HAL_CIPHER_DMA_Start(MS_BOOL Decrypt, HAL_CIPHER_INTMODE IntMode, MS_BOOL bWaitCmdQ, MS_U16 u16CmdID)
{
    ...
    if(_uDmaCtrl1.DmaCtrl1.Dest0 == CMD_DMA_OUTPUT_DRAM)
    {
...
DMA_RETRY:
    ...
    if (u32SpareCnt > CmdLen)
    {
        ...
        if (_uDmaCtrl0.DmaCtrl0.DK == TRUE )
        {
            ...
            if(_uDmaCtrl0.DmaCtrl0.Aes_mode == CMD_AES_MODE_192)
            {
                WRITE_CMDQ(_pu8CipherKey[24]<<24 |_pu8CipherKey[25]<<16 | _pu8CipherKey[26]<<8 | _pu8CipherKey[27]);
                WRITE_CMDQ(_pu8CipherKey[28]<<24 |_pu8CipherKey[29]<<16 | _pu8CipherKey[30]<<8 | _pu8CipherKey[31]);
                ...
                WRITE_CMDQ(_pu8CipherKey[16]<<24 |_pu8CipherKey[17]<<16 | _pu8CipherKey[18]<<8 | _pu8CipherKey[19]);
                WRITE_CMDQ(_pu8CipherKey[20]<<24 |_pu8CipherKey[21]<<16 | _pu8CipherKey[22]<<8 | _pu8CipherKey[23]);
            }
            else if(_uDmaCtrl0.DmaCtrl0.Aes_mode == CMD_AES_MODE_256)
            {
                WRITE_CMDQ(_pu8CipherKey[ 0]<<24 |_pu8CipherKey[ 1]<<16 | _pu8CipherKey[ 2]<<8 | _pu8CipherKey[ 3]);
                WRITE_CMDQ(_pu8CipherKey[ 4]<<24 |_pu8CipherKey[ 5]<<16 | _pu8CipherKey[ 6]<<8 | _pu8CipherKey[ 7]);
                WRITE_CMDQ(_pu8CipherKey[ 8]<<24 |_pu8CipherKey[ 9]<<16 | _pu8CipherKey[10]<<8 | _pu8CipherKey[11]);
        ...
        if (_uDmaCtrl3.DmaCtrl3.DK2 == TRUE )
        {
            if(_uDmaCtrl3.DmaCtrl3.Aes_mode2 == CMD_AES_MODE_192)
            {
                WRITE_CMDQ(_pu8CipherKey2[24]<<24 |_pu8CipherKey2[25]<<16 | _pu8CipherKey2[26]<<8 | _pu8CipherKey2[27]);
                WRITE_CMDQ(_pu8CipherKey2[28]<<24 |_pu8CipherKey2[29]<<16 | _pu8CipherKey2[30]<<8 | _pu8CipherKey2[31]);
                ...
                WRITE_CMDQ(_pu8CipherKey2[16]<<24 |_pu8CipherKey2[17]<<16 | _pu8CipherKey2[18]<<8 | _pu8CipherKey2[19]);
                WRITE_CMDQ(_pu8CipherKey2[20]<<24 |_pu8CipherKey2[21]<<16 | _pu8CipherKey2[22]<<8 | _pu8CipherKey2[23]);
            }
            else if(_uDmaCtrl3.DmaCtrl3.Aes_mode2 == CMD_AES_MODE_256)
            {
                WRITE_CMDQ(_pu8CipherKey2[ 0]<<24 |_pu8CipherKey2[ 1]<<16 | _pu8CipherKey2[ 2]<<8 | _pu8CipherKey2[ 3]);
                WRITE_CMDQ(_pu8CipherKey2[ 4]<<24 |_pu8CipherKey2[ 5]<<16 | _pu8CipherKey2[ 6]<<8 | _pu8CipherKey2[ 7]);
                WRITE_CMDQ(_pu8CipherKey2[ 8]<<24 |_pu8CipherKey2[ 9]<<16 | _pu8CipherKey2[10]<<8 | _pu8CipherKey2[11]);
...
MS_BOOL HAL_CIPHER_Hash_SetHMACKey(DRV_CIPHER_HMAC_KEY stHMACKey, CIPHER_HMAC_KPAD eKpad, MS_BOOL bClear)
{
    ...
    switch(KeySrc)
    {
        ...
        case E_DMA_KSEL_MK1:
            ...
            break;
        case E_DMA_KSEL_SK0:
            u32HMACKeySel = CMD_HMAC_KSEL_SK;
            _uHashCtrl0.HashCtrl0.HASH_Key_Index  = stHMACKey.u8KeyIdx;
            break;
        default:
            u32HMACKeySel = 0;
            break;
    ...
    if(E_DMA_KSEL_REGKEY == KeySrc)
    {
        if ((0 != stKey.pu8KeyData) && (stKey.u8KeyLen != 0) && (stKey.u8KeyLen <= HAL_CRYPTODMA_KEYLEN_MAX) )
        {
            if(E_DMA_ALGO_AES == algo)
            {
                ...
                if(stKey.u8KeyLen == E_DMA_AES_KEY192)
                {
                    _uDmaCtrl3.DmaCtrl3.Aes_mode2 = CMD_AES_MODE_192;
                }
                else if(stKey.u8KeyLen == E_DMA_AES_KEY256)
                {
                    _uDmaCtrl3.DmaCtrl3.Aes_mode2 = CMD_AES_MODE_256;
                }
            }
