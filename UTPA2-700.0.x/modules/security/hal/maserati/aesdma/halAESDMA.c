...
{
    MS_U32 Dma_Ctrl[19];
    MS_U32 Secure_dma3_ctrl;
    MS_U32 Parser_Pid1;
    MS_U32 Parser_Ctrl;
    MS_U32 Kslot_Index;
} AESDMA_REGISTER_BACKUP ;

static AESDMA_REGISTER_BACKUP _gAESDMARegisterBackup;
...
void AESDMA_Enable_Clk(void)
{
    ...
    u32Val |= AESDMA_CLK_288M;   //Set Clk to 288M
...
void HDCP_ProcessCipher (MS_U8 u8Idx, MS_U8 *pu8Data, MS_U8* pu8privData)
{
    ...
    MS_U32 *cipherkey = (MS_U32 *)((void *)pu8Data);
    MS_U32 *riv = (MS_U32 *)((void *)pu8privData);
...
static void AESDMA_BackupRegisters(void)
{
    MS_VIRT Reg_AESDMA;
    MS_U32 i = 0;
    Reg_AESDMA = (MS_VIRT)(&_AESDMACtrl[0].Dma_Ctrl);
    ...
    for (i = 0 ; i < 19 ; i++)
    {
       _gAESDMARegisterBackup.Dma_Ctrl[i] =  _AESDMA_REG32_R((REG32 *)(Reg_AESDMA+(i*8)));
    }
    // reset secure_dma3_ctrl register
    _gAESDMARegisterBackup.Secure_dma3_ctrl = _AESDMA_REG32_R((&_DMASECURECtrl[0].Secure_dma3_ctrl));

    // reset hw_parser_ctrl register
    _gAESDMARegisterBackup.Parser_Pid1 = _AESDMA_REG32_R((&_PARSERCtrl[0].Parser_Pid1));
    _gAESDMARegisterBackup.Parser_Ctrl = _AESDMA_REG32_R((&_PARSERCtrl[0].Parser_Ctrl));
    _gAESDMARegisterBackup.Kslot_Index = _AESDMA_REG32_R((&_AESDSCMB6Ctrl[0].Reg0001));
}

static void AESDMA_RestoreRegisters(void)
{
    MS_VIRT Reg_AESDMA;
    MS_U32 i = 0;
    Reg_AESDMA = (MS_VIRT)(&_AESDMACtrl[0].Dma_Ctrl);
    ...
    for (i = 0 ; i < 19 ; i++)
    {
        REG32_W((REG32 *)(Reg_AESDMA+(i*8)), _gAESDMARegisterBackup.Dma_Ctrl[i]);
    }
    // reset secure_dma3_ctrl register
    REG32_W((&_DMASECURECtrl[0].Secure_dma3_ctrl), _gAESDMARegisterBackup.Secure_dma3_ctrl);

    // reset hw_parser_ctrl register
    REG32_W((&_PARSERCtrl[0].Parser_Pid1), _gAESDMARegisterBackup.Parser_Pid1);
    REG32_W((&_PARSERCtrl[0].Parser_Ctrl), _gAESDMARegisterBackup.Parser_Ctrl);
    REG32_W((&_AESDSCMB6Ctrl[0].Reg0001), _gAESDMARegisterBackup.Kslot_Index);
}
...
void AESDMA_NormalReset(void)
{
    ...
    REG32_W((&_PARSERCtrl[0].Parser_Pid1),0x00000000);
    REG32_W((&_PARSERCtrl[0].Parser_Ctrl),0x00000000);
    REG32_W((&_AESDSCMB6Ctrl[0].Reg0001), AESDMA_DSCRMB6_KSLOT_INDEX_DEFAULT);
}
...
{
    if ( (u32Index < 0x10 || u32Index > 0x1F) && (u32Index != 0)) {
        return;
    }
    MS_U32 u32Value = 0;
#if AESDMS_SECURED_BANK_EN
    ...
    //
    if (bIsOddKey == FALSE) {
        //Even Key (default key), [7:0]
        u32Value = _AESDMA_REG32_R(&_AESDSCMB6Ctrl[0].Reg0001) & 0xFFFFFF00;
        //Set Key Index, 0 = Default key
        u32Value = u32Index == 0 ? (u32Value | 0x1) : (u32Value | u32Index);
    }
    else {
        //Odd Key (HW Parser key)
        u32Value = _AESDMA_REG32_R(&_AESDSCMB6Ctrl[0].Reg0001) & 0xFFFF00FF;
        //Set Key Index, 0 = Default key
        u32Value = u32Index == 0 ? (u32Value | 0x2) : (u32Value | ((u32Index<<8) & 0xFF00));
    }
    REG32_W((&_AESDSCMB6Ctrl[0].Reg0001) , u32Value);
#else
    //
    // For Normal Bank
    //
    if (bIsOddKey == FALSE) {
        //Even Key (default key), [7:0]
        u32Value = _AESDMA_REG32_R(&_AESDSCMB7Ctrl[0].Reg0001) & 0xFFFFFF00;
        //Set Key Index, 0 = Default key
        u32Value = u32Index == 0 ? (u32Value | 0x0) : (u32Value | u32Index);
    }
    else {
        //Odd Key (HW Parser key)
        u32Value = _AESDMA_REG32_R(&_AESDSCMB7Ctrl[0].Reg0001) & 0xFFFF00FF;
        //Set Key Index, 0 = Default key
        u32Value = u32Index == 0 ? (u32Value | 0x2) : (u32Value | ((u32Index<<8) & 0xFF00));
    }
    REG32_W((&_AESDSCMB7Ctrl[0].Reg0001) , u32Value);
#endif  //#if AESDMS_SECURED_BANK_EN
}
...
void AESDMA_SET_CTR64(MS_BOOL bEnable)
{
    if(bEnable)
    {
        REG32_W((&_AESDMACtrl[0].Dma_PS_Pattern_Mask), _AESDMA_REG32_R(&_AESDMACtrl[0].Dma_PS_Pattern_Mask)|AESDMA_CTR_IV_LSB64);
    }
    else
    {
        REG32_W((&_AESDMACtrl[0].Dma_PS_Pattern_Mask), _AESDMA_REG32_R(&_AESDMACtrl[0].Dma_PS_Pattern_Mask) & ~AESDMA_CTR_IV_LSB64);
...
#if AESDMS_SECURED_BANK_EN
    u32Ctrl = _AESDMA_REG32_R(&_AESDMACtrl[0].Dma_Ctrl3_Secure);
#else 
    u32Ctrl = _AESDMA_REG32_R(&_AESDMACtrl[0].Dma_Ctrl2);
#endif

    switch (u8KeyIdx)
    {
        case E_DRVAESDMA_SECRETKEY_1:
            u32Ctrl &= ~AESDMA_USE_SECRET_KEY2;
            u32Ctrl |= AESDMA_USE_SECRET_KEY;
#if AESDMS_SECURED_BANK_EN
            REG32_W((&_AESDMACtrl[0].Dma_Ctrl3_Secure), u32Ctrl);
#else
            REG32_W((&_AESDMACtrl[0].Dma_Ctrl2), u32Ctrl);
#endif
            break;
        case E_DRVAESDMA_SECRETKEY_2:
#if AESDMS_SECURED_BANK_EN
            u32Ctrl &= ~AESDMA_USE_SECRET_KEY1;
            u32Ctrl |= AESDMA_USE_SECRET_KEY2;
            REG32_W((&_AESDMACtrl[0].Dma_Ctrl3_Secure), u32Ctrl);
#else
            u32Ctrl |= AESDMA_USE_SECRET_KEY1;
...
#endif
            break;
        case E_DRVAESDMA_SECRETKEY_EAEK:
        {
            AESDMA_BackupRegisters();

            AESDMA_Reset();
            REG32_W((&_AESDMACtrl[0].Dma_Filein_Num), 0x10); // size
            AESDMA_Set_CIPHER_ENG(E_AESDMA_CIPHER_ECB, TRUE); // AES ECB decrypt
            REG32_W((&_AESDMACtrl[0].Dma_Matched_Pat), _AESDMA_REG32_R(&_AESDMACtrl[0].Dma_Matched_Pat) & 0xFFFF0000 );
            REG32_W((&_AESDMACtrl[0].Dma_Matched_Pat), _AESDMA_REG32_R(&_AESDMACtrl[0].Dma_Matched_Pat) | AESDMA_GEN_INTERNALKEY2 | AESDMA_HWKEY_EN); // h72 = 0x0022
            REG32_W((&_AESDMACtrl[0].Dma_Ctrl), (_AESDMA_REG32_R(&_AESDMACtrl[0].Dma_Ctrl) | AESDMA_CTRL_FILEOUT_START)); // h50 = 0x0001
            while (!AESDMA_Get_AESDMA_IsFinished());
            REG32_W((&_AESDMACtrl[0].Dma_Ctrl), (_AESDMA_REG32_R(&_AESDMACtrl[0].Dma_Ctrl) & 0xFFFF0000 )); // h50 = 0x0000

            AESDMA_RestoreRegisters();

            REG32_W((&_AESDSCMB6Ctrl[0].Reg0001), (_AESDMA_REG32_R(&_AESDSCMB6Ctrl[0].Reg0001) & 0xFFFF0000));
            REG32_W((&_AESDSCMB6Ctrl[0].Reg0001), (_AESDMA_REG32_R(&_AESDSCMB6Ctrl[0].Reg0001) | AESDMA_DSCRMB6_KSLOT_INDEX_USE_INTERNAL_KEY2));  // h00 = 0x0206 , // [7:0] 0x6 use_int_key_2, [15:7] 0x2 default value
            break;
        }
        default:
            break;
