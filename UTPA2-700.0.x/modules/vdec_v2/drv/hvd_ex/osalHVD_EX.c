...
#endif
// Interrupt
inline MS_BOOL OSAL_HVD_ISR_Disable(MS_BOOL isHVDIsr)
{  
#if SUPPORT_EVD
    MS_U32 u32IsrVer  = (isHVDIsr) ? HVD_ISR_VECTOR : EVD_ISR_VECTOR;
#else
    UNUSED(isHVDIsr);
    MS_U32 u32IsrVer= HVD_ISR_VECTOR;
#endif
    return MsOS_DisableInterrupt(u32IsrVer);
}

inline MS_BOOL OSAL_HVD_ISR_Enable(MS_BOOL isHVDIsr)
{
#if SUPPORT_EVD
    MS_U32 u32IsrVer  = (isHVDIsr) ? HVD_ISR_VECTOR : EVD_ISR_VECTOR;
#else
...
inline MS_BOOL OSAL_HVD_ISR_Attach(MS_BOOL isHVDIsr, void* pfnISRCB)
{
#if SUPPORT_EVD
    MS_U32 u32IsrVer  = (isHVDIsr) ? HVD_ISR_VECTOR : EVD_ISR_VECTOR;
#else
...
inline MS_BOOL OSAL_HVD_ISR_Detach(MS_BOOL isHVDIsr)
{
#if SUPPORT_EVD
    MS_U32 u32IsrVer  = (isHVDIsr) ? HVD_ISR_VECTOR : EVD_ISR_VECTOR;
#else
