...
#if SUPPORT_EVD
#define _MaskMiuReq_EVD_RW( m )         _HVD_WriteRegBit(MIU0_REG_RQ3_MASK+1, m, BIT(5))
#define _MaskMiuReq_EVD_BBU_R( m )      _HVD_WriteRegBit(MIU0_REG_RQ3_MASK+1, m, BIT(4))
#define _MaskMiu1Req_EVD_RW( m )         _HVD_WriteRegBit(MIU1_REG_RQ3_MASK+1, m, BIT(5))
#define _MaskMiu1Req_EVD_BBU_R( m )      _HVD_WriteRegBit(MIU1_REG_RQ3_MASK+1, m, BIT(4))
#define HVD_EVD_RW_ON_MIU1              ((_HVD_Read2Byte(MIU0_REG_SEL3) & BIT(13)) == BIT(13))
#define HVD_EVD_BBU_R_ON_MIU1           ((_HVD_Read2Byte(MIU0_REG_SEL3) & BIT(12)) == BIT(12))
#endif
...
//static MS_BOOL g_RstFlag = FALSE;
...
static void _HVD_EX_Context_Init_HAL(void)
{
    ...
    pHVDHalContext->u32VPUClockType = 320; //it should same as:_VPU_EX_InitAll() eClkSpeed
    pHVDHalContext->u32HVDClockType = 345;//160;
#if SUPPORT_EVD
    pHVDHalContext->u32EVDClockType = 345;
#endif
...
MS_BOOL HAL_HVD_EX_InitHW(MS_U32 u32Id,VPU_EX_DecoderType DecoderType)
{
    ...
    if (E_HVD_INIT_HW_HEVC == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK))
    {
        _HVD_WriteWordMask(EVD_REG_RESET, 1, EVD_REG_RESET_HK_HEVC_MODE);

        if (E_HVD_INIT_MAIN_LIVE_STREAM == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK))
        {
            _HVD_WriteWordMask(EVD_REG_RESET, 1, EVD_REG_RESET_HK_TSP2EVD_EN);
        }
        goto RESET;
    }
#endif
...
void HAL_HVD_EX_PowerCtrl(MS_BOOL bEnable)
{
    if (bEnable)
    {
        ...
        _HVD_WriteWordMask(REG_TOP_HVD, ~TOP_CKG_VP8_DIS, TOP_CKG_VP8_DIS);
    ...
    switch (pHVDHalContext->u32HVDClockType)
    {
        case 345:
        {
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_HVD_345MHZ, TOP_CKG_HVD_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD, TOP_CKG_VP8_240MHZ, TOP_CKG_VP8_MASK);
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_VP6_192MHZ, TOP_CKG_VP6_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD_IDB, ~TOP_CKG_HVD_IDB_CLK_HVD_MIU, TOP_CKG_HVD_IDB_CLK_HVD_MIU);  // TODO://FIXME:  need to check miu clk,
        ...
        case 320:
        {
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_HVD_320MHZ, TOP_CKG_HVD_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD, TOP_CKG_VP8_240MHZ, TOP_CKG_VP8_MASK);
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_VP6_192MHZ, TOP_CKG_VP6_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD_IDB, ~TOP_CKG_HVD_IDB_CLK_HVD_MIU, TOP_CKG_HVD_IDB_CLK_HVD_MIU);  // TODO://FIXME:  need to check miu clk,
                                                                                                               // TODO://if(hvd_clk > miu_clk) set to TOP_CKG_HVD_IDB_CLK_HVD_MIU = 0
        ...
        case 216:
        {
            ...
            _HVD_WriteWordMask(REG_TOP_HVD, TOP_CKG_VP8_172MHZ, TOP_CKG_VP8_MASK);
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_VP6_172MHZ, TOP_CKG_VP6_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD_IDB, TOP_CKG_HVD_IDB_CLK_HVD_MIU, TOP_CKG_HVD_IDB_CLK_HVD_MIU);  // TODO://FIXME:  need to check miu clk,
                                                                                                               // TODO://if(hvd_clk > miu_clk) set to TOP_CKG_HVD_IDB_CLK_HVD_MIU = 0
                                                                                                               // TODO://else set to TOP_CKG_HVD_IDB_CLK_HVD_MIU = 1
            break;
        ...
        case 144:
        {
            ...
            _HVD_WriteWordMask(REG_TOP_HVD, TOP_CKG_VP8_144MHZ, TOP_CKG_VP8_MASK);
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_VP6_144MHZ, TOP_CKG_VP6_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD_IDB, TOP_CKG_HVD_IDB_CLK_HVD_MIU, TOP_CKG_HVD_IDB_CLK_HVD_MIU);  // TODO://FIXME:  need to check miu clk,
                                                                                                               // TODO://if(hvd_clk > miu_clk) set to TOP_CKG_HVD_IDB_CLK_HVD_MIU = 0
                                                                                                               // TODO://else set to TOP_CKG_HVD_IDB_CLK_HVD_MIU = 1
            break;
...
HVD_Return HAL_HVD_EX_InitVariables(MS_U32 u32Id)
{
    ...
    if(((pCtrl->InitParams.u32ModeFlag) & E_HVD_INIT_HW_MASK)  == E_HVD_INIT_HW_AVS)
    {
        if(pHVDHalContext->u32HVDClockType > 288)
            pHVDHalContext->u32HVDClockType = 288;
    }
...
HVD_Return HAL_HVD_EX_InitShareMem(MS_U32 u32Id)
{
    ...
    //Chip info
    pShm->u16ChipID = E_MSTAR_CHIP_MUJI;
...
MS_U32 HAL_HVD_EX_GetData(MS_U32 u32Id, HVD_GetData eType)
{
    ...
    // share memory
        ...
        {
            /// TODO:
            u32Ret=0; //pShm->u32RealFrameRate;
            break;
        }
...
void HAL_EVD_EX_PowerCtrl(MS_BOOL bEnable)
{
    ...
    switch (pHVDHalContext->u32EVDClockType)
    {
        case 345:
        {
            _HVD_WriteWordMask(REG_TOP_CKG_EVD_PPU, TOP_CKG_EVD_PPU_345MHZ, TOP_CKG_EVD_PPU_MASK);
            _HVD_WriteWordMask(REG_TOP_CKG_EVD, TOP_CKG_EVD_345MHZ, TOP_CKG_EVD_MASK);
            break;
        }
        case 320:
        {
        ...
        default:
        {
            _HVD_WriteByteMask(REG_TOP_CKG_EVD_PPU, TOP_CKG_EVD_PPU_345MHZ, TOP_CKG_EVD_PPU_MASK);
            _HVD_WriteByteMask(REG_TOP_CKG_EVD, TOP_CKG_EVD_345MHZ, TOP_CKG_EVD_MASK);
            break;
        }
    }
