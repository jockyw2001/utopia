...
#define SET_CMD_RET_FALSE(_cmdVal, _pStcmdArg)                                  \
    do {                                                                        \
        ...
        {                                                                       \
            MVD_DEBUGERROR(MVD_ERR("Command: 0x%x fail!!\r\n", _cmdVal));        \
            return FALSE;                                                       \
        }                                                                       \
    } while(0)
...
#define HAL_MVD_InvalidBuffRetFalse(x)                                                              \
    do {                                                                                            \
        if ((x)==0)                                                                                 \
        {                                                                                           \
            MVD_DEBUGERROR(MVD_ERR("%s(%d) error: NULL buffer address.\n", __FUNCTION__, __LINE__)); \
            return FALSE;                                                                           \
        }                                                                                           \
    } while(0)
...
static MS_BOOL bSHMMiuSel=MIU_SEL_1;
#if 0
...
static MVD_ALIVEInfo aliveInfo[MAX_DEC_NUM];
...
typedef struct
{
    ...
    HAL_MVD_EX_ClockSpeed _eMVDClockSpeed;
    MS_U32 _u32MVDClockType;
    MS_U32 _u32MVD2ClockType;
    //pre_set
...
#if 0
static HAL_MVD_EX_ClockSpeed _eMVDClockSpeed = E_HAL_MVD_EX_CLOCK_SPEED_DEFAULT;
static MS_U32 _u32MVDClockType = CKG_MVD_172MHZ;
static MS_U32 _u32MVD2ClockType = CKG_MVD2_172MHZ;
#endif
//-------------------------------------------------------------------------------------------------
//  Local Variables
//-------------------------------------------------------------------------------------------------
...
static void HAL_MVD_Context_Init(void)
{
    ...
    pMVDHalContext->_eMVDClockSpeed = E_HAL_MVD_EX_CLOCK_SPEED_DEFAULT;
    pMVDHalContext->_u32MVDClockType = CKG_MVD_160MHZ;
    pMVDHalContext->_u32MVD2ClockType = CKG_MVD_160MHZ;
}
...
static MS_U32 MVD_GetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U8 u8Offset, MS_U8 u8Size)
{
    ...
    MVD_DEBUGVERBAL(MVD_PRINT("%s: u32Val=%ld for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
...
void HAL_MVD_MemGetMap(MS_U8 u8Idx, MS_U8 u8type, MS_U32* pu32addr, MS_U32* pu32len)
{
    ...
    switch (u8type)
    {
        case E_MVD_MMAP_ALL:
        case E_MVD_MMAP_FW:
            *pu32addr = _PA2Offset(pstMemCfg->u32FWCodeAddr);
            *pu32len = pstMemCfg->u32FWCodeSize;
            break;
        case E_MVD_MMAP_BS:
            *pu32addr = _PA2Offset(pstMemCfg->u32BSAddr);
            *pu32len = pstMemCfg->u32BSSize;
            break;
        case E_MVD_MMAP_FB:
            *pu32addr = _PA2Offset(pstMemCfg->u32FBAddr);
            *pu32len = pstMemCfg->u32FBSize;
            break;
        case E_MVD_MMAP_DRV:
            *pu32addr = _PA2Offset(pstMemCfg->u32DrvBufAddr);
...
#endif
    {
        if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_1)
        {
            u32Address += MIU1_BASEADDR;
        }
    }
...
MS_BOOL HAL_MVD_SetMEMCfg(MS_U8 u8Idx, MVD_MEMCfg* pMEMCfg)
{
    ...
    pMVDHalContext->stMiuCfg.bFWMiuSel = pMEMCfg->bFWMiuSel;
    pMVDHalContext->stMiuCfg.bHWMiuSel = pMEMCfg->bHWMiuSel;
    return TRUE;
}
...
MVD_ALIVEInfo* HAL_MVD_GetAliveInfo(MS_U8 u8Idx)
{
    MVD_ALIVEInfo* pInfo = NULL;
    pInfo = &(pMVDHalContext->aliveInfo[u8Idx]);
    return pInfo;
}

void HAL_MVD_PowerCtrl(MS_BOOL bOn)
{
    HAL_MVD_RegWriteByteMask(REG_CKG_MVD, pMVDHalContext->_u32MVDClockType, CKG_MVD_MASK);
    HAL_MVD_RegWriteBit(REG_CKG_MVD, !bOn, CKG_MVD_GATED);
    HAL_MVD_RegWriteBit(REG_CKG_MVD, !bOn, CKG_MVD_INVERT);

    HAL_MVD_RegWriteByteMask(REG_CKG_MVD2, pMVDHalContext->_u32MVD2ClockType, CKG_MVD2_MASK);
...
{
    MS_U32 u32BufStart2 = GET_FRMINFO_BUFFADD(u8Idx);
    _miu_offset_to_phy(pMVDHalContext->stMiuCfg.bFWMiuSel,u32BufStart2,u32BufStart2);      
    MS_U32* temp2 = (MS_U32*)MsOS_PA2KSEG1(u32BufStart2+OFFSET_CMD_HANDSHAKE_INDEX);

    _MVD_MemResetBit(temp2,cmd);
    MsOS_FlushMemory();
}
...
MS_BOOL HAL_MVD_TimeOut(MS_U8 u8Idx)
{
    ...
    for ( i = 0; i < MVD_PollingTimes; i++ )
    {
        ...
        {
            MVD_DEBUGINFO(MVD_PRINT("%s: bStopped(%x) or timeout(%ld)\n", __FUNCTION__, pMVDHalContext->bStopped[u8Idx], MsOS_GetSystemTime()-u32StartTime));
            return TRUE;
        }

        //HAL_MVD_Sleep(5);
    }
    MVD_DEBUGERROR( MVD_ERR("MVD_TimeOut=%lx\n", i) );
    return TRUE;
}
...
_CMD_DONE:
    ...
    if (!bRet)
    {
        ...
        MS_U32 x=0;
        for (x=0; x<30; x++)
            MVD_DEBUG_FWCMD(MVD_PRINT("0x%lx\n", HAL_VPU_EX_GetProgCnt()));
...
MS_BOOL HAL_MVD_InitHW(void)
{
    ...
    if(!HAL_MVD_RstHW())
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_MVDInit:MVD4ResetHW failed\n"));
        return FALSE;
    }
    else
    {
...
static MS_BOOL _MVD_Check_FW_Rdy(MS_U32 u32VpuSid)
{
    ...
    if (u32TimeOut == 0)
    {
        MVD_ERROR(MVD_ERR("MVD_FW_IF_Version=%lx inconsistent with header file(%x)!\n",
...
MS_BOOL _DumpCtrl(MS_U8 u8Idx)
{
    ...
    //MVD_PRINT("u32VA=0x%lx, PA=0x%lx\n", u32VA, pstMemCfg->u32FWCodeAddr);
    ...
    while (CTL_TASK_CMDRDY != ctl_ptr->statue)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("CTRL timeout!!! %d\n", __LINE__));
            return FALSE;
        }
    }
...
MS_BOOL HAL_MVD_Init(MS_U8 u8HalIdx,MVD_CodecType eCodecType, MS_U32 u32VpuSid)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s:HAL_MVD_InitFW(%x) failed\n", __FUNCTION__, u8HalIdx));
        if (!HAL_MVD_DeleteTask(u8HalIdx, (HAL_VPU_StreamId)u32VpuSid))
        {
            MVD_DEBUGERROR(MVD_ERR("%s:HAL_MVD_DeleteTask failed\n", __FUNCTION__));
        }
...
void HAL_MVD_SetFrameBuffAddr(MS_U8 u8Idx, MS_U32 u32addr, MS_U8 u8fbMode)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_SetFrameBuffAddr: only support 28bit add!\n"));
        return;
    }
...
void HAL_MVD_SetHeaderBufferAddr (MS_U8 u8Idx, MS_U32 u32addr )
{
    ...
    if(u32SharememoryBase[u8Idx] != MVD_U32_MAX && bSHMMiuSel!=pstMemCfg->bFWMiuSel)
        u32addr += HAL_VPU_EX_MIU1BASE();
#endif
    MS_ASSERT((u32addr%8)==0);
    u32addr >>= 3;
...
MS_U32 HAL_MVD_GetTsFileESReadPtr(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR( MVD_ERR( "Ctrl: 0x%x fail!!\n", CMD_PARSER_READ_POSITION) );
    }
...
MS_U32 HAL_MVD_GetTsFileESWritePtr(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGINFO(MVD_PRINT("ES wrapping Wr=0x%lx ==> ", u32WrPtr));
...
MS_BOOL HAL_MVD_PauseDisp(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR( MVD_ERR( "Command: 0x%x(ON) fail!!\r\n", CMD_DISPLAY_PAUSE) );
        bRst = FALSE;
    }
    return bRst;
}
...
static MS_U8 MVD_GetSyncStat(MS_U8 u8Idx, MS_U8 u8ArgIdx)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("Sync On/Off %x, Done %x, Stat %x, diff=%ld\n",
...
static MS_BOOL MVD_WriteFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U8 u8Offset, MS_U8 u8Size, MS_U32 u32Val)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s: u32Val=%ld for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
                          __FUNCTION__, u32Val, u8Idx, eBuffType, u8Offset, u8Size));
    return bRet;
}
...
MVD_PicType HAL_MVD_GetPicType(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s error: pu8MVDGetFrameInfoBufStart=NULL\n", __FUNCTION__));
...
MS_U32 HAL_MVD_GetPTS(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR( "Ctrl: CMD_RD_PTS fail!!\n"  ) );
        u32PTS = MVD_U32_MAX;
    }
    else
    {
        u32PTS = ((MS_U32)mvdcmd.Arg3 << 24 | (MS_U32)mvdcmd.Arg2 << 16 |
                  (MS_U32)mvdcmd.Arg1 << 8  | (MS_U32)mvdcmd.Arg0);
        MVD_DEBUGVERBAL(MVD_PRINT("MDrv_MVD_GetPTS:0x%lx\n", u32PTS));
    }
#endif
...
MS_U64 HAL_MVD_GetU64PTS(MS_U8 u8Idx)
{
    MS_U64 u64PTS = 0;
...
MS_BOOL HAL_MVD_IsCmdFinished(MS_U8 u8Idx, MVD_HANDSHAKE_CMD eCmd)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("u32CmdState.value = 0x%x\n", u32CmdState.value));
    switch (eCmd)
    {
...
MS_BOOL HAL_MVD_ClearCmdFinished(MS_U8 u8Idx, MVD_HANDSHAKE_CMD eCmd)
{
    ...
    u32CmdState.value = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_CMD_HANDSHAKE_INDEX, sizeof(MS_U32));
    MVD_DEBUGINFO(MVD_PRINT("before CLR u32CmdState.value = 0x%x\n", u32CmdState.value));
    switch (eCmd)
    {
        case MVD_HANDSHAKE_SCALER_INFO:
    ...
    MVD_DEBUGINFO(MVD_PRINT("after CLR u32CmdState.value = 0x%lx\n",
...
void HAL_MVD_GetErrInfo(MS_U8 u8Idx, MVD_ErrCode *errCode, MVD_ErrStatus *errStatus)
{
    ...
    if(pu8MVDGetFrameInfoBufStart==0)
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_GetErrInfo error!\n"));
        return;
    }
...
{
    if (u32PhyAdd >= pMVDHalContext->stMiuCfg.u32Miu1BaseAddr)
    {
...
MS_BOOL HAL_MVD_SLQTblSendPacket(MS_U8 u8Idx, MVD_PacketInfo *pstVideoPKT)
{
    ...
    //SEC IP clip mode dummy pkt. no need to update pts.
    MS_U8 *pu8Data = (MS_U8 *)HAL_MVD_PA2NonCacheSeg(pstVideoPKT->u32StAddr + pstMemCfg->u32BSAddr);
    MS_BOOL bIsNeedUpdatePTS = TRUE;
#if _SLQTBL_DUMP_PKT
   // static MS_U32 u32SendTimes[MAX_DEC_NUM]= {0, 0};

    MVD_PRINT("Pkt[%lx] st=0x%lx len=0x%lx pts=0x%lx id_l=0x%lx id_h=0x%lx\n", u32SendTimes[u8Idx]++,
    ...
    //Fixed me. SEC IP clip mode dummy pkt. size = 256. pattern : 000001C6FFFF...FF.
    //we only check first 8 bytes for speed.
    if (HAL_MVD_IsMStreamerMode(u8Idx) && (pstVideoPKT->u32Length == 256))
    {
        //dummy pkt, no need to udpate PTS entry~~
        if((pu8Data[0] == 0x00)
        && (pu8Data[1] == 0x00)
        && (pu8Data[2] == 0x01)
        && (pu8Data[3] == 0xC6)
        ...
        && (pu8Data[7] == 0xFF))
        {
            bIsNeedUpdatePTS = FALSE;
        }
    }

    if (u32MVDFWPtsTblAddr && (bIsNeedUpdatePTS == TRUE))
    {
        ...
        //MVD_PRINT("PTS=0x%lx(%lx), idx=0x%lx add=0x%lx\n", pstVideoPKT->u32TimeStamp,
    ...
    *u32LastEntry += 8;
    ...
    if (pstSlqTblInfo->pDrvSlqTbl->u32Empty> SLQ_ENTRY_LEN)
    {
...
void HAL_MVD_GetFrameInfo(MS_U8 u8Idx, MVD_FrameInfo *pinfo)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_GetFrameInfo error: pu8MVDGetVolBufStart=NULL\n"));
        return;
    }
    ...
    if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_1)
    {
        gvolInfo = (*(volatile FW_VOL_INFO*)(HAL_MVD_PA2NonCacheSeg(GET_VOL_BUFFADD(u8Idx)+pMVDHalContext->stMiuCfg.u32Miu1BaseAddr)));
    }
    else
    {
        gvolInfo = (*(volatile FW_VOL_INFO*)(HAL_MVD_PA2NonCacheSeg(GET_VOL_BUFFADD(u8Idx))));
    }

#if 0
    MVD_DEBUGINFO(MVD_PRINT("vol info,vol_info=%d,sprite_usage=%d,pts_incr=%d,\n",
                  gvolInfo.vol_info,gvolInfo.sprite_usage,gvolInfo.pts_incr));
    MVD_DEBUGINFO(MVD_PRINT("vol info,width=%x,height=%x,frame_rate=%d,aspect_ratio=%x,\n",
                  gvolInfo.width,gvolInfo.height,gvolInfo.frame_rate,gvolInfo.aspect_ratio));
    MVD_DEBUGINFO(MVD_PRINT("vol info,progressive_sequence=%x,mpeg1=%x,play_mode=%x,bit_rate=%x,\n",
    ...
    if (pinfo->u16par_width == 0 || pinfo->u16par_height == 0)
    {
        if ((pinfo->u8AspectRate > 0) && (pinfo->u8AspectRate < 5 ))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    MVD_DEBUGVERBAL(MVD_PRINT("u32PAR_Width:%ld,u32PAR_Height:%ld,GCD:%ld\n",u32PAR_Width,u32PAR_Height,u32GCD));
                }
            }
        }
        else
        ...
        MVD_DEBUGVERBAL(MVD_PRINT("u32DAR_Width:%ld,u32DAR_Height%ld\n",u32DAR_Width,u32DAR_Height));
    }
    ...
    else if ((curCodecType == E_MVD_CODEC_VC1_ADV) || (curCodecType == E_MVD_CODEC_VC1_MAIN))
    {
        ...
        MVD_DEBUGVERBAL(MVD_PRINT("MVD: vc1_frameRate=%ld\n", pinfo->u32FrameRate));
    }
    else if (curCodecType == E_MVD_CODEC_MPEG4)
    {
        ...
        {
            pinfo->u32FrameRate = (pMVDHalContext->u32DmxFrameRate[u8Idx]/pMVDHalContext->u32DmxFrameRateBase[u8Idx])*1000;
        }
        else
        {
    ...
    MVD_DEBUGVERBAL(MVD_PRINT("===> MVD: frameRate=%ld  curCodecType=0x%x\n", pinfo->u32FrameRate, curCodecType));
    ...
    //for dynamic scaling
    pinfo->bEnableMIUSel = pMVDHalContext->stMiuCfg.bFWMiuSel;
    if (pstMemCfg->bEnableDynScale)
    {
        if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_1)
        {
            pinfo->u32DynScalingAddr= GET_DS_BUFFADD(u8Idx) + pMVDHalContext->stMiuCfg.u32Miu1BaseAddr;
        }
        else
        {
            pinfo->u32DynScalingAddr= GET_DS_BUFFADD(u8Idx);
        }
        MVD_CtrlCfg* pCtrlCfg = HAL_MVD_GetCtrlCfg(u8Idx);
...
E_MVD_Result MVD_RstFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType)
{
    ...
    //MVD_PRINT("%s u32DecFrmInfoAdd = 0x%lx\n", __FUNCTION__, u32DecFrmInfoAdd);
    if (E_MVD_FRMINFO_DECODE == eType)
    {
...
static MS_BOOL MVD_GetFWBuffAdd(MS_U32 u32Start, MS_U32 u32AvailLen, MVD_FWBuff* pBuff)
{
    ...
    if(pBuff == NULL)
    {
        MVD_DEBUGERROR(MVD_ERR("%s err: NULL pBuff\n", __FUNCTION__));
        return FALSE;
    }
    ...
    if ((pBuff->u32DecFrmInfoAdd+MVD_FW_DECFRM_INFO_BUF_LEN-u32Start) > u32ShMemBoundary)
    {
        ...
        MVD_DEBUGERROR(MVD_ERR("%s err: 0x%lx out of memory boundary!\n", __FUNCTION__,
...
static MS_BOOL MVD_SetFWBuffAdd(MS_U8 u8Idx, MVD_FWBuff* pBuff)
{
...
#if 0 //test for miu0
    if(u32SharememoryBase[u8Idx] == MVD_U32_MAX)
    {
        if (stMiuCfg.bFWMiuSel == MIU_SEL_1)
        {
            pBuff->u32VolAdd += stMiuCfg.u32Miu1BaseAddr;
        }
    }
    else
    {
    ...
    if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_1)
    {
        pBuff->u32VolAdd += pMVDHalContext->stMiuCfg.u32Miu1BaseAddr;
    }
    ...
    if (pstMemCfg->bEnableDynScale)
    {
        ...
        MVD_DEBUGINFO(MVD_PRINT("bEnableMIUSel    = 0x%x\n", pMVDHalContext->stMiuCfg.bFWMiuSel));
...
E_MVD_Result HAL_MVD_GetFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType, MVD_FrmInfo* pInfo)
{
    ...
    if (NULL == u32DecFrmInfoAdd)
    {
        MVD_DEBUGERROR(MVD_ERR("GetFrmInfo NULL u32DecFrmInfoAdd!\n"));
        return E_MVD_RET_FAIL;
    }
    //MVD_PRINT("%s u32DecFrmInfoAdd = 0x%lx\n", __FUNCTION__, u32DecFrmInfoAdd);
    ...
    MVD_PRINT("Luma=0x%lx, ", pInfo->u32LumaAddr  );
    MVD_PRINT("Chroma=0x%lx, ", pInfo->u32ChromaAddr);
    MVD_PRINT("Pts=%lu, ", pInfo->u32TimeStamp );
    MVD_PRINT("ID_H=%lu, ", pInfo->u32ID_H      );
    MVD_PRINT("ID_L=0x%lx\n", pInfo->u32ID_L      );
#endif
    ...
    if (pMVDHalContext->stMiuCfg.bHWMiuSel == MIU_SEL_0)
    {
    ...
    {
        pInfo->u32LumaAddr = pInfo->u32LumaAddr * 8 + pMVDHalContext->stMiuCfg.u32Miu1BaseAddr;
        pInfo->u32ChromaAddr = pInfo->u32ChromaAddr * 8 + pMVDHalContext->stMiuCfg.u32Miu1BaseAddr;
    }

    //MVD_PRINT("===> Luma=0x%lx, Chroma=0x%lx\n", pInfo->u32LumaAddr, pInfo->u32ChromaAddr);
    return eRet;
}
...
E_MVD_Result HAL_MVD_SetDynScalingParam(MS_U8 u8Idx, MS_PHYADDR u32StAddr, MS_U32 u32Size)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s !bEnableDynScale\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s fail to set ScalerInfoAdd\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s timeout!!!\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
    ...
    MVD_DEBUGINFO(MVD_PRINT("=====> %s u32TimeOut = 0x%lx\n", __FUNCTION__, u32TimeOut));
    return E_MVD_RET_OK;
}
...
static MS_BOOL MVD_CheckFrmBuffSizeMin(MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("Framebuffer size(0x%lx) < (0x%x+0x%lx)!\n",
...
MS_BOOL HAL_MVD_CheckFrmBuffSize(MS_U8 u8Idx, MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    *pu8FBMode = MVD3_SD_MODE;
    ...
    if (u32AvailFrmBuffSize >= (MVD_HD_FBSIZE*MVD_FBNUM_MAX))
    {
...
MS_BOOL HAL_MVD_SetCodecInfo(MS_U8 u8Idx, MVD_CodecType eCodecType, MVD_SrcMode eSrcMode, MS_U8 bDisablePESParsing)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_Init:_MVD_MVDSetInternalBuffAddr failed\n"));
        return FALSE;
    }
    else
    {
...
BDMA_CpyType MVD_GetBDMAType(void)
{
    ...
    if (pMVDHalContext->stMiuCfg.bHWMiuSel == MIU_SEL_1)
    {
        if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_0)
        {
            bdmaCpyType = E_BDMA_SDRAM2SDRAM1;
        }
        else if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_1)
        {
    ...
    else if (pMVDHalContext->stMiuCfg.bHWMiuSel == MIU_SEL_0)
    {
        if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_0)
        {
...
MS_BOOL HAL_MVD_Stop(MS_U8 u8Idx)
{
    ...
    if (HAL_MVD_MVDCommand(CMD_STOP, &mvdcmd) == FALSE)
    {
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("*** MVD ERR: STOP TIMEOUT!!! ***\n"));
        }
    }
...
MS_BOOL HAL_MVD_DeinitHW(void)
{
    ...
    if (!HAL_MVD_RstHW())
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_Exit:MVD4ResetHW failed\n"));
    }
    else
    {
...
void HAL_MVD_SetFrameInfo(MS_U8 u8Idx, MVD_FrameInfo *pinfo )
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_SetFrameInfo error: pu8MVDSetHeaderBufStart=NULL\n"));
        return;
    }
...
///////////////////////////////////////////////////////////////////////////////
MS_U32 HAL_CC_CM_GetMVDRB_HWAddr(MS_U8 u8CC608)
{
#if 1
    MVD_CmdArg mvdcmd;
    MS_U32 u32CCWrPtr = 0;
...
MS_BOOL HAL_MVD_GetUsrDataInfo(MS_U8 u8Idx, MVD_UsrDataInfo* pUsrInfo)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s: no data? Rd=0x%lx Wr=0x%lx\n", __FUNCTION__, pCtrlCfg->u32UsrDataRd, u32UsrDataWr));
        return FALSE;
    }

    //miuOffset --> physical add --> noncached add
    u32UsrData = (pMVDHalContext->stMiuCfg.bFWMiuSel==MIU_SEL_1)?(pCtrlCfg->u32UsrDataRd+pMVDHalContext->stMiuCfg.u32Miu1BaseAddr):(pCtrlCfg->u32UsrDataRd);
    ...
    MVD_PRINT("0x%lx, ", pUsrInfo->u32DataBuf);
...
MS_U32 HAL_MVD_Map2DrvSlqTbl(MS_U8 u8Idx, MS_U32 u32HWPtr)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("Invalid u32HWPtr=0x%lx\n", u32HWPtr));
        return 0;
    }
...
static void MVD_SLQTblGetFileEndPkt(MS_U8 u8Idx, MVD_PacketInfo* pFileEnd)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("u32EndPattern(0x%lx)=0x%lx 0x%lx 0x%lx 0x%lx\n", pFileEnd->u32StAddr,
...
#if 0
    MVD_PRINT("u32DummyES(0x%lx-->0x%lx, size=0x%lx)=0x%08lx 0x%08lx 0x%08lx 0x%08lx\n", u32DummyES,
...
MS_BOOL HAL_MVD_SLQTblInsertPattern(MS_U8 u8Idx, MVD_PatternType ePattern)
{
    ...
    {   //insert dummy pattern
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("Invalid MVD_PatternType! Won't insert pattern!\n"));
            return FALSE;
        }
        #if SLQ_NEW_PUSH
        pstSlqTblInfo->pSlqStatus->bSlqFireRdy = TRUE;
        #endif
        //MVD_PRINT("WrPtr 0x%lx ", pstSlqTblInfo->pDrvSlqTbl->u32WrPtr);
...
E_MVD_Result HAL_MVD_IsDispFinish(MS_U8 u8Idx)
{
    ...
    if (GET_FRMINFO_BUFFADD(u8Idx)==0)
    {
        MVD_DEBUGERROR(MVD_ERR("%s err: pu8MVDGetFrameInfoBufStart=NULL\n", __FUNCTION__));
        return E_MVD_RET_FAIL;
    }
            ...
            (HAL_MVD_IsMStreamerMode(u8Idx) == FALSE))
        {
            ...
            if ((HAL_MVD_GetTime() - u32TimeCnt) >= CMD_TIMEOUT_MS)
            {
                ...
                if (E_MVD_CODEC_VC1_MAIN == HAL_MVD_GetCodecType(u8Idx))
                {
                    ...
                    {
                        MVD_DEBUGERROR(MVD_ERR("RCV payloadLen(0x%lx) invalid!\n",
                            HAL_MVD_GetPayloadLen(u8Idx)));
                    }
                }
                MVD_DEBUGERROR(MVD_ERR("***** fe=%lx, rd=%lx(%lx,%lx), wr=%lx, empty=%lx, u32FeByteCnt=%lx\n",
...
static void HAL_MVD_SetSLQStartEnd(MS_U8 u8Idx, MS_U32 u32start, MS_U32 u32end)
{
    ...
    if(u32SharememoryBase[u8Idx] != MVD_U32_MAX && bSHMMiuSel!=pstMemCfg->bFWMiuSel)
    {
        u32start += MIU1_BASEADDR;
        u32end += MIU1_BASEADDR;
    }
#endif
    if ((u32start > SLQ_ADDR_LEN) || ((u32end+1) > SLQ_ADDR_LEN))
    {
        MVD_DEBUGERROR(MVD_ERR("MDrv_MVD_SetSLQStartEnd: only support 27bit add!\n"));
    }
...
MS_BOOL HAL_MVD_DecodeIFrame(MS_U8 u8Idx, MS_PHYADDR u32FrameBufAddr, MS_PHYADDR u32StreamBufAddr, MS_PHYADDR u32StreamBufEndAddr )
{
    ...
    {
        MVD_DEBUGERROR( MVD_ERR( "HAL_MVD_StepDecode fail!!\r\n") );
        return FALSE;
    }
...
#define WAIT_INIT_SUCCESS_TIME 100 //100ms
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s: wait init_success timeout!!!\n", __FUNCTION__));
    }
...
#define WAIT_DECODE_DONE_TIME 33 //To decode 1 frame should take less than 33ms
    ...
    {
        MVD_DEBUGERROR(MVD_ERR ("MDrv_MVD_DecodeIFrame time out(du=%ld, st=%ld, now=%ld)\n", u32time, u32deley, HAL_MVD_GetTime()));
        MVD_DEBUGERROR(MVD_ERR("frmCnt=%ld state=0x%x lastCmd=0x%x\n", HAL_MVD_GetPicCounter(u8Idx), HAL_MVD_GetDecodeStatus(u8Idx), HAL_MVD_GetLastCmd(u8Idx)));
        return FALSE;
    }
    MVD_DEBUGINFO(MVD_PRINT ("MDrv_MVD_DecodeIFrame time (%ld, %ld)\n", u32time, u32deley));
...
MS_BOOL HAL_MVD_SetInternalBuffAddr(MS_U8 u8Idx, MS_U32 u32start, MS_U32 u32len)
{
    ...
    MS_U32 u32BuffStart = u32start+(MVD_FW_TASK_OFFSET*u8Idx);
    MS_U32 VPUSHMAddr = HAL_VPU_EX_GetSHMAddr();

    if(VPUSHMAddr != 0)
    {
        if(VPUSHMAddr >= HAL_MIU1_BASE) // miu1
        {
            VPUSHMAddr -= (HAL_MIU1_BASE+MVD_FW_CODE_LEN);
        }
        else  // miu0
        {
            VPUSHMAddr -= (MVD_FW_CODE_LEN);
        }

        if(u8Idx == 0)
        {
            u32SharememoryBase[u8Idx] = VPUSHMAddr; // for main decoder
        }
        else if(u8Idx == 1)
        {
            u32SharememoryBase[u8Idx] = VPUSHMAddr + 0x20000; // VPUSHMAddr+128K bytes
        }
    }

    if(u32SharememoryBase[u8Idx] != MVD_U32_MAX)
    {
        u32BuffStart = u32SharememoryBase[u8Idx];
    }
...
// The reason is that these 3 buffers are used by MVD HW engine.
    {
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("MVD HW buffers larger than FB size!!!\n"));
        }
    ...
    || (pFwCfg->stFBReduction.ChromaFBReductionMode  != E_MVD_FB_REDUCTION_NONE))
    {
        ...
        MVD_DEBUGERROR(MVD_ERR("MVD Err: Not support FB reduction mode!!\n"));
    }
...
static void _SLQTbl_DumpPtsTbl(MS_U8 u8Idx, MS_U32 u32EntryStart, MS_U32 u32EntryEnd)
{
    ...
    for (i=u32EntryStart; i<u32EntryEnd; i++)
    {
        ...
        MVD_PRINT("ES[%lx] Start=0x%lx End=0x%lx u32EsRp=%lx\n",
    ...
    for (i=u32EntryStart; i<u32EntryEnd; i++)
    {
        MVD_PRINT("0x%lx\t 0x%08lx\t 0x%08lx\t 0x%08lx\t 0x%08lx\t 0x%08lx\n", u32MVDFWPtsTblAddr+i*MVD_FW_SLQTBL_PTS_LEN,
...
#endif
{
    ...
    if(pstSlqTblInfo->pSlqStatus->bSlqCtrlBit)
    {
        ...
        {
             //MVD_PRINT("Show KC the SlqPushLength = 0x%lx\n",pstSlqTblInfo->pSlqStatus->u32SlqPushLength);
    ...
    //MVD_PRINT("u32DivXPattern(0x%lx==>0x%lx)=0x%lx 0x%lx\n", u32DivXPattern, pDivxHdr->u32StAddr,
...
static void MVD_SLQTblInitFileEndPkt(MS_U8 u8Idx, MVD_CodecType eType)
{
    ...
    if ((E_MVD_CODEC_FLV == eType)||(E_MVD_CODEC_MPEG4_SHORT_VIDEO_HEADER == eType))
    {
        ...
        //MVD_PRINT("##########FileEnd for FLV/SVH!, u32EndPattern=%lx\n",u32EndPattern);
    }
    else if (E_MVD_CODEC_DIVX311 == eType)
    {
        ...
        HAL_MVD_MemWrite4Byte(u32EndPattern+12,END_PATTERN_2); //scw
        HAL_MVD_MemWrite4Byte(u32EndPattern+16,END_PATTERN_3); //scw
        //MVD_PRINT("##########FileEnd for DIVX311!, u32EndPattern=%lx\n",u32EndPattern);
    }
    else if ((E_MVD_CODEC_MPEG2 == eType)||(E_MVD_CODEC_MPEG4 == eType))
    {
        ...
        //MVD_PRINT("##########FileEnd for MPEG2/4!, u32EndPattern=%lx\n",u32EndPattern);
    }
    else
    {
        ...
        //MVD_PRINT("##########FileEnd for VC1!, u32EndPattern=%lx\n",u32EndPattern);
    }

    MVD_DEBUGINFO(MVD_PRINT("u32EndPattern(0x%lx)=0x%lx 0x%lx\n", u32EndPattern,
            HAL_MVD_MemRead4Byte(u32EndPattern), HAL_MVD_MemRead4Byte(u32EndPattern+4)));
}
...
#if 0
    MVD_PRINT("u32DummyES(0x%lx)=0x%08lx 0x%08lx 0x%08lx 0x%08lx\n", u32DummyES, HAL_MVD_MemRead4Byte(u32DummyES),
            HAL_MVD_MemRead4Byte(u32DummyES+4),HAL_MVD_MemRead4Byte(u32DummyES+8),HAL_MVD_MemRead4Byte(u32DummyES+12));
#endif
...
MS_BOOL HAL_MVD_SLQTblInit(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s: unknow codec type!\n", __FUNCTION__));
        return FALSE;
    }
...
//static MS_U32 writePtrLast[MAX_DEC_NUM];   //SlqTbl debug
MS_U32 HAL_MVD_GetQueueVacancy(MS_U8 u8Idx, MS_BOOL bCached)
{
    ...
    //MVD_PRINT("QV=0x%lx rd=0x%lx==>", u32Empty, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr);
    if (pstSlqTblInfo->pDrvSlqTbl->u32RdPtr >= (pstSlqTblInfo->pDrvSlqTbl->u32StAdd+SLQ_ENTRY_LEN))
    {
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("%s: readPtr 0x%lx out of range: too large!\n",
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s: readPtr 0x%lx out of range: too small!\n",
                       __FUNCTION__, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr));
    ...
    //MVD_PRINT("%s r=0x%lx w=0x%lx u32Empty=0x%lx\n", __FUNCTION__,
...
E_MVD_Result HAL_MVD_PushQueue(MS_U8 u8Idx, MVD_PacketInfo* pInfo)
{
...
#if _SLQTBL_DUMP_PUSHQ
    {
        ...
        MVD_PRINT("Push[%lx] st=0x%lx len=0x%lx pts=0x%lx\n", pMVDHalContext->u32pqTimes[u8Idx]++,
    ...
    if (pInfo->u32StAddr >= pstMemCfg->u32BSSize)
    {
        ...
        MVD_DEBUGERROR(MVD_ERR("PushQueue invalid u32StAddr=0x%lx, bsSize=0x%lx\n", pInfo->u32StAddr, pstMemCfg->u32BSSize));
        return E_MVD_RET_INVALID_PARAM;
    }
    ...
    {   //put packets
#if SLQ_NEW_PUSH
        if(FALSE == pMVDHalContext->stCtrlCfg[u8Idx].stDbgModeCfg.bBypassInsertStartCode)
        {
            if (E_MVD_CODEC_DIVX311 == curCodecType
    ...
    else
    {
        ...
        MVD_DEBUGERROR(MVD_ERR("PushQueue FULL!!! empty=0x%lx\n", pstSlqTblInfo->pDrvSlqTbl->u32Empty));
    ...
    {
        MVD_DEBUGERROR(MVD_ERR("%s ret = %d\n", __FUNCTION__, eRet));
    }
    return eRet;
}
...
#define MAX_VALIDLEN    0x200000    //2M
    ...
    else
    {
        ...
        MVD_DEBUGINFO(MVD_PRINT("(%x) ValidLen=0x%lx CurLen=0x%lx\n",
...
static E_MVD_Result MVD_FlushSlqTblQueue(MS_U8 u8Idx)
{
           ...
           (TRUE != MVD_PatternLenIsValid(u8Idx, u32PatternByteCnt)))
    {
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("RCV timeout!!! pl=%ld, pbc=%ld, emp=%ld\n",
    ...
    if (TRUE != HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_PAUSE))
    {
        ...
        MVD_DEBUGERROR(MVD_ERR("ValidLen=0x%lx CurPatternLen=0x%lx\n",
                      HAL_MVD_GetPayloadLen(u8Idx), u32PatternByteCnt));
...
void HAL_MVD_DbgDumpBits(MS_U8 u8Idx, MS_PHYADDR u32base, MS_U32 u32size)
{
    ...
    MVD_PRINT("%s base=0x%lx size=0x%lx\n", __FUNCTION__, u32base, u32size);
...
void HAL_MVD_SetDivXCfg(MS_U8 u8Idx, MS_U8 u8MvAdjust, MS_U8 u8IdctSel)
{
    ...
    if (FALSE == pMVDHalContext->stCtrlCfg[u8Idx].stDbgModeCfg.bBypassDivxMCPatch)
    {
        SETUP_CMDARG(stCmdArg);
...
MS_U32 MVD_GetSlqTblESReadPtr(MS_U8 u8Idx)
{
    ...
    MS_U32 u32EsEnd;
    u32EsEnd = HAL_MVD_MemRead4Byte(u32EsRp+4);
    MVD_PRINT("GetESReadPtr ES[%lx] Start=0x%lx End=0x%lx u32EsRp=%lx u32SlqRp=%lx\n",
...
#if 0
    MVD_PRINT("GetESWritePtr[%lx] ES Start=0x%lx End=0x%lx u32EsWp=%lx\n",
...
MS_U32 HAL_MVD_GetSLQReadPtr(MS_U8 u8Idx)
{
    ...
    //MVD_PRINT("##### HAL_MVD_GetSLQReadPtr 0x%lx\n", readPtr);
...
MS_BOOL HAL_MVD_SetSLQWritePtr(MS_U8 u8Idx, MS_BOOL bCheckData)
{
    ...
    if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
    {
        ...
        while ((HAL_MVD_GetTime() - u32TimeCnt) < CMD_TIMEOUT_MS)
        {
            if (HAL_MVD_GetDecodeStatus(u8Idx) != E_MVD_STAT_IDLE)
            {
                ...
                MVD_DEBUGVERBAL(MVD_PRINT("time=%ld ms, ", (HAL_MVD_GetTime() - u32TimeCnt)));
                break;
            }
        }
        if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
        {
            MVD_DEBUGERROR(MVD_ERR("%s: err timeout(%ld)! stat=0x%x\n", __FUNCTION__,
    ...
    if (pstCtrlCfg->bSlqTblSync)
    {
        ...
        MS_U32 u32TblWr;
        //MVD_PRINT("===>offset(Src=0x%lx, Dst=0x%lx)", u32SrcAdd, u32DstAdd);
        if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_1)
        {
            u32SrcAdd = u32SrcOffset + pMVDHalContext->stMiuCfg.u32Miu1BaseAddr;
        }
        //MVD_PRINT(" PA(Src=0x%lx, Dst=0x%lx)", u32SrcAdd, u32DstAdd);
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("%s MDrv_BDMA_MemCopy fail ret=%x!\n", __FUNCTION__, bdmaRlt));
        }
        ...
        //MVD_PRINT(" wr=0x%lx, tblWr=0x%lx\n", writePtr, u32TblWr);
        u32WrPtr = u32TblWr;
    }
    //MVD_PRINT("wPtr= 0x%lx(0x%lx) rPtr=0x%lx(0x%lx)\n", writePtr, pstSlqTblInfo->pDrvSlqTbl->u32WrPtr,
    ...
    else
    {
        ...
        {
            MVD_DEBUGERROR(MVD_ERR("Ooops! SlqWrPtr update fail!!! 0x%lx != 0x%lx\n", HAL_MVD_SlqTblProbeWrPtr(u8Idx), u32WrPtr));
            return FALSE;
        }
    }
...
void HAL_MVD_DbgDump(MS_U8 u8Idx)
{
    ...
    MVD_PRINT("input: vfifo=%d(full=%d,empty=%d), vdCnt=%ld(%ld), vldErr=%ld(%ld); ",
    ...
    if (curSrcMode == E_MVD_SLQ_TBL_MODE)
    {
        MVD_PRINT("fe=%lx, rd=%lx(%lx), wr=%lx, empty=%lx; ", pstSlqTblInfo->u32FileEndPtr, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr,
                HAL_MVD_Map2DrvSlqTbl(u8Idx, HAL_MVD_GetSLQReadPtr(u8Idx)), pstSlqTblInfo->pDrvSlqTbl->u32WrPtr, pstSlqTblInfo->pDrvSlqTbl->u32Empty);
...
MS_BOOL HAL_MVD_ParserRstDone(MS_U8 u8Idx, MS_BOOL bEnable)
{
    ...
    {
        MVD_DEBUGERROR( MVD_ERR( "Ctrl: 0x%x fail timeout!!\r\n", CMD_PTS_TBL_RESET ) );
        return FALSE;
    }
    return TRUE;
}
...
MS_BOOL _HAL_MVD_CrcTimeOut(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGERROR( MVD_ERR("_HAL_MVD_CrcTimeOut=%lx\n", i) );
    return TRUE;
}
...
E_MVD_Result HAL_MVD_GetCrcValue(MS_U8 u8Idx, MVD_CrcIn *pCrcIn, MVD_CrcOut *pCrcOut)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s width=0x%lx, height=0x%lx, pitch=0x%lx, luma=0x%lx, chroma=0x%lx\n",
...
{
    MS_U32 u32MVDClockType = 0, u32MVD2ClockType = 0;

    MVD_DEBUGVERBAL(MVD_PRINT("mvd clock setting %d\n", eClockSpeed));

    switch (eClockSpeed)
    {
        case E_HAL_MVD_EX_CLOCK_SPEED_HIGHEST:
            u32MVDClockType = CKG_MVD_172MHZ;
            u32MVD2ClockType = CKG_MVD2_172MHZ;
            break;
        case E_HAL_MVD_EX_CLOCK_SPEED_HIGH:
            u32MVDClockType = CKG_MVD_160MHZ;
            u32MVD2ClockType = CKG_MVD2_160MHZ;
            break;
        case E_HAL_MVD_EX_CLOCK_SPEED_MEDIUM:
            u32MVDClockType = CKG_MVD_144MHZ;
            u32MVD2ClockType = CKG_MVD2_144MHZ;
            break;
        case E_HAL_MVD_EX_CLOCK_SPEED_LOW:
            u32MVDClockType = CKG_MVD_123MHZ;
            u32MVD2ClockType = CKG_MVD2_123MHZ;
            break;
        case E_HAL_MVD_EX_CLOCK_SPEED_LOWEST:
            u32MVDClockType = CKG_MVD_123MHZ;
            u32MVD2ClockType = CKG_MVD2_123MHZ;
            break;
        case E_HAL_MVD_EX_CLOCK_SPEED_DEFAULT:
            u32MVDClockType = CKG_MVD_172MHZ;
            u32MVD2ClockType = CKG_MVD2_172MHZ;
            break;
        default:
    ...
    if((pMVDHalContext->_eMVDClockSpeed != eClockSpeed)
    && (TRUE == HAL_VPU_EX_MVDInUsed()))
    {
        MVD_DEBUGERROR(MVD_ERR("mvd or hvd is running, cannot preset mvd clock\n"));
        return E_MVD_RET_FAIL;
    }

    pMVDHalContext->_eMVDClockSpeed = eClockSpeed;
    pMVDHalContext->_u32MVDClockType = u32MVDClockType;
    pMVDHalContext->_u32MVD2ClockType = u32MVD2ClockType;
...
MS_BOOL HAL_MVD_SetShareMemoryBase(MS_U8 u8Idx, MS_U32 u32base, MS_BOOL bsel)
{
    ...
    bSHMMiuSel= bsel;
    if(u32base == MVD_U32_MAX || u8Idx >= MAX_DEC_NUM)
    {
        return FALSE;
    }
    else
...
#define CMD_CC_ENABLE_EXTERNAL_BUFFER 0xB3 //wait for fw release to remove
MS_BOOL HAL_MVD_SetExternal_CC_Buffer(MS_U8 u8Idx, MS_U32 u32base, MS_U8 u8size, MS_U8 cc_type)
{
    ...
    if (pMVDHalContext->stMiuCfg.bFWMiuSel == MIU_SEL_1)
        _MVD_Memset(u32base-MIU1_BASEADDR,0,u32cc_size);
    else
        _MVD_Memset(u32base,0,u32cc_size);
    ...
    _MVD_SetUserDataBufStart(u8Idx,u32base,cc_type);
...
MS_U32 HAL_MVD_GetVsyncBridgeAddr(MS_U8 u8Idx)
{
    MVD_MEMCfg* pMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    MS_U32 CodeAddr = pMemCfg->u32FWCodeAddr;
    MS_U32 VPUSHMAddr = HAL_VPU_EX_GetSHMAddr();
    MS_U32 VsyncBridgeAddr = 0;

    if(VPUSHMAddr != 0)  // TEE project
    {
        if(VPUSHMAddr >= HAL_MIU1_BASE && CodeAddr >= HAL_MIU1_BASE) // miu1
        {
            VsyncBridgeAddr = VPUSHMAddr + VSYNC_BRIGE_SHM_OFFSET;
        }
        else if(VPUSHMAddr < HAL_MIU1_BASE && CodeAddr < HAL_MIU1_BASE) // miu0
        {
            VsyncBridgeAddr = VPUSHMAddr + VSYNC_BRIGE_SHM_OFFSET;
        }
        else
        {
            VsyncBridgeAddr = 0;
            MVD_DEBUGERROR(MVD_ERR("share Memeory and VDEC f/w is in different MIU,(%x,%x)\n",(unsigned int)VPUSHMAddr,(unsigned int)pMemCfg->u32FWCodeAddr));
        }
    }
    else  // normal project
    {
        VsyncBridgeAddr = CodeAddr + VSYNC_BRIGE_SHM_START;
    }

    return VsyncBridgeAddr;
}
...
E_MVD_Result HAL_MVD_PVR_Seamless_mode(MS_U8 u8Idx, MS_U8 u8Arg)
{
    ...
    if(u8Arg != 2)
    {
        ...
        _miu_offset_to_phy(pMVDHalContext->stMiuCfg.bFWMiuSel,u32BufStart,u32BufStart);
