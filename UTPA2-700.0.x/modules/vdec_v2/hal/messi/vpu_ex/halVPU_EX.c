...
static MS_BOOL _VPU_EX_LoadVLCTable(VPU_EX_VLCTblCfg *pVlcCfg, MS_U8 u8FwSrcType)
{
    ...
    else
    {
        ...
        else
        {
            ...
            pu8HVD_VLC_Binary = (MS_U8 *) ((MS_VIRT) u8HVD_VLC_Binary | 0xA0000000UL);

            VPU_MSG_DBG("Load VLC inD2D: dest:0x%lx source:%lx size:%lx\n",
                        pVlcCfg->u32FrameBufAddr + pVlcCfg->u32VLCTableOffset, ((MS_VIRT) pu8HVD_VLC_Binary),
            ...
            VPU_MSG_INFO("Load VLC inD2D: dest:0x%lx source:%lx size:%x\n",
                        MsOS_VA2PA(pVlcCfg->u32DstAddr), (MS_VIRT) u8HVD_VLC_Binary,
                        (MS_U32) sizeof(u8HVD_VLC_Binary));
...
static void _VPU_EX_ClockSpeed(MS_U32 u32type)
{
    switch (u32type)
    {
        ...
        case VPU_CLOCK_144MHZ:
            ...
            break;
        default:
            _VPU_WriteWordMask(REG_TOP_VPU, VPU_CLOCK_320MHZ, TOP_CKG_VPU_CLK_MASK);
            break;
    }
}
...
#if (ENABLE_DECOMPRESS_FUNCTION==TRUE)
static MS_BOOL _VPU_EX_DecompressBin(MS_VIRT u32SrcAddr, MS_VIRT u32SrcSize, MS_VIRT u32DestAddr, MS_VIRT u32SlidingAddr)
{
...
MS_BOOL HAL_VPU_EX_LoadCode(VPU_EX_FWCodeCfg *pFWCodeCfg)
{
    ...
    MS_VIRT u32Size      = pFWCodeCfg->u32BinSize;
#if (ENABLE_DECOMPRESS_FUNCTION==TRUE)
    ...
    else
    {
        ...
        VPU_MSG_INFO("Load FW inD2D: dest=0x%lx, source=0x%lx, size=%d\n",
                    u32DestAddr, ((MS_VIRT) u8HVD_FW_Binary),
                    (MS_U32) sizeof(u8HVD_FW_Binary));

#if (ENABLE_DECOMPRESS_FUNCTION==TRUE)
        if(_VPU_EX_DecompressBin((MS_VIRT)u8HVD_FW_Binary, (MS_VIRT)sizeof(u8HVD_FW_Binary), u32DestAddr, u32DestAddr+u32DestSize-WINDOW_SIZE)==TRUE)
        {
            if(_VPU_EX_IsNeedDecompress(u32DestAddr)==FALSE)
            {
                VPU_MSG_INFO("Decompress ok!!!\n");
