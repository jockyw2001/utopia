...
// MVD5
#define SLQ_TBL_ENTRY_LEN        0x7FFFFFFFUL //31-bit
...
static MS_VIRT u32SharememoryBase[MAX_DEC_NUM] ={MVD_U32_MAX,MVD_U32_MAX};
...
static MS_U32 MVD_GetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U8 u8Offset, MS_U8 u8Size)
{
    ...
    MVD_DEBUGVERBAL(printf("%s: u32Val=%d for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
...
void HAL_MVD_MemGetMap(MS_U8 u8Idx, MS_U8 u8type, MS_VIRT* pu32addr, MS_VIRT* pu32len)
{
...
MS_PHY HAL_MVD_PA2NonCacheSeg(MS_PHY u32PhyAddr)
{
...
void HAL_MVD_ResetHandShake(MS_U8 u8Idx, MVD_HANDSHAKE_CMD cmd)
{
    ...
    HAL_MVD_CPU_Sync();
    HAL_MVD_ReadMemory();
    MS_U32 val = HAL_MVD_MemRead4Byte(u32BufAddr + OFFSET_CMD_HANDSHAKE_INDEX);
    val = val & (~(0x1<<cmd));
    HAL_MVD_MemWrite4Byte(u32BufAddr + OFFSET_CMD_HANDSHAKE_INDEX,0);
    MsOS_FlushMemory();
}
...
MS_BOOL HAL_MVD_TimeOut(MS_U8 u8Idx)
{
    ...
    for ( i = 0; i < MVD_PollingTimes; i++ )
    {
        ...
        {
            MVD_DEBUGINFO(printf("%s: bStopped(%x) or timeout(%d)\n", __FUNCTION__, pMVDHalContext->bStopped[u8Idx], MsOS_GetSystemTime()-u32StartTime));
            return TRUE;
        }

        //HAL_MVD_Sleep(5);
    }
    MVD_DEBUGERROR( printf("MVD_TimeOut=%x\n", i) );
    return TRUE;
}
...
static MS_BOOL _MVD_Check_FW_Rdy(MS_U32 u32VpuSid)
{
    ...
    if (u32TimeOut == 0)
    {
        MVD_ERROR(printf("MVD_FW_IF_Version=%x inconsistent with header file(%x)!\n",
    ...
    {
        MVD_DEBUGINFO(printf("Warning! FWBinVer(%x) != FWHdrVer(%x)\n", HAL_MVD_GetFWVer(u32VpuSid), FW_VERSION));
    }
...
static void _MVD_SetUserDataBufSize(MS_U8 u8Idx, MS_U32 u32size, MS_U8 u8arg3)
{
    ...
    u32size >>= 3;
    MVD_DEBUGINFO(printf("%s add=0x%x arg3=0x%x\n", __FUNCTION__, u32size, u8arg3));
...
MS_VIRT HAL_MVD_GetTsFileESReadPtr(MS_U8 u8Idx)
{
    ...
    if (HAL_MVD_MVDCommand( CMD_PARSER_READ_POSITION, &mvdcmd ) == TRUE)
    {
        u32Add = (((MS_VIRT)mvdcmd.Arg3) <<24) |
                 (((MS_VIRT)mvdcmd.Arg2) <<16) |
                 (((MS_VIRT)mvdcmd.Arg1) << 8) |
                 (((MS_VIRT)mvdcmd.Arg0));
    }
    else
    {
...
MS_BOOL HAL_MVD_SkipData(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(printf("====> %s (t1=%u t2=%u diff=%u)\n", __FUNCTION__,
...
{// TODO: be careful! CMDARG only receive the least 32 bits !!! by jeffrey
    MVD_CmdArg mvdcmd;
#if 0 //test for miu0
    MVD_MEMCfg* pstMemCfg = NULL;
    pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
...
static MS_U8 MVD_GetSyncStat(MS_U8 u8Idx, MS_U8 u8ArgIdx)
{
    ...
    MVD_DEBUGINFO(printf("Sync On/Off %x, Done %x, Stat %x, diff=%d\n",
...
static MS_BOOL MVD_WriteFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U8 u8Offset, MS_U8 u8Size, MS_U32 u32Val)
{
    ...
    MVD_DEBUGINFO(printf("%s: u32Val=%d for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
                          __FUNCTION__, u32Val, u8Idx, eBuffType, u8Offset, u8Size));
    return bRet;
}
...
MS_BOOL HAL_MVD_ClearCmdFinished(MS_U8 u8Idx, MVD_HANDSHAKE_CMD eCmd)
{
    ...
    MVD_DEBUGINFO(printf("after CLR u32CmdState.value = 0x%x\n",
...
MS_U32 HAL_MVD_GetXcLowDelayIntState(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(printf("MDrv_MVD_GetXcLowDelayIntState()=%08x,%08x,%08x,%08x,%08x\n",
...
MS_VIRT HAL_MVD_GetMemOffset(MS_PHY u32PhyAdd)
{
...
MS_BOOL HAL_MVD_SLQTblSendPacket(MS_U8 u8Idx, MVD_PacketInfo *pstVideoPKT)
{
    ...
    MS_VIRT u32EntryWord = 0;
    MS_U32 u32Index = 0;
    MS_U32 u32Pts;
    MS_VIRT u32ESStart=0;
    MS_VIRT u32ESEnd=0;
...
void HAL_MVD_GetFrameInfo(MS_U8 u8Idx, MVD_FrameInfo *pinfo)
{
    ...
    if (pinfo->u16par_width == 0 || pinfo->u16par_height == 0)
    {
        if ((pinfo->u8AspectRate > 0) && (pinfo->u8AspectRate < 5 ))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    MVD_DEBUGVERBAL(printf("u32PAR_Width:%d,u32PAR_Height:%d,GCD:%d\n",u32PAR_Width,u32PAR_Height,u32GCD));
                }
            }
        }
        else
        ...
        MVD_DEBUGVERBAL(printf("u32DAR_Width:%d,u32DAR_Height%d\n",u32DAR_Width,u32DAR_Height));
    }
    ...
    else if ((curCodecType == E_MVD_CODEC_VC1_ADV) || (curCodecType == E_MVD_CODEC_VC1_MAIN))
    {
        ...
        MVD_DEBUGVERBAL(printf("MVD: vc1_frameRate=%d\n", pinfo->u32FrameRate));
    }
    ...
    MVD_DEBUGVERBAL(printf("===> MVD: frameRate=%d  curCodecType=0x%x\n", pinfo->u32FrameRate, curCodecType));
...
E_MVD_Result HAL_MVD_SetDynScalingParam(MS_U8 u8Idx, MS_PHY u32StAddr, MS_VIRT u32Size)
{
    ...
    MVD_DEBUGINFO(printf("=====> %s u32TimeOut = 0x%x\n", __FUNCTION__, u32TimeOut));
    return E_MVD_RET_OK;
}
...
MS_BOOL HAL_MVD_CheckFrmBuffSize(MS_U8 u8Idx, MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    MS_VIRT u32AvailFrmBuffSize = memCfg->u32FBSize - MVD_HW_BUF_TOTAL_LEN;
    ...
    *pu8FBMode = MVD3_SD_MODE;
    ...
    if (MVD3_SD_MODE == *pu8FBMode)
    {
        ...
        *pu8FBNum = MVD_FBNUM_MIN;
        MVD_DEBUGINFO(printf("[MVD_SD_MODE] u8FBNum=%u, FBSize=0x%lx\n", *pu8FBNum, memCfg->u32FBSize));
        goto _GET_USED_SIZE;
    }
...
MS_BOOL HAL_MVD_Stop(MS_U8 u8Idx)
{
    ...
    if(pMVDHalContext->stFwCfg[u8Idx].eSrcMode == E_MVD_TS_MODE)
    {
...
static void MVD_SLQTblGetFileEndPkt(MS_U8 u8Idx, MVD_PacketInfo* pFileEnd)
{
    ...
    MVD_DEBUGINFO(printf("u32EndPattern(0x%lx)=0x%08x 0x%08x 0x%x 0x%08x\n", pFileEnd->u32StAddr,
            ...
            (HAL_MVD_IsMStreamerMode(u8Idx) == FALSE))
        {
            ...
            if ((HAL_MVD_GetTime() - u32TimeCnt) >= CMD_TIMEOUT_MS)
            {
                ...
                if (E_MVD_CODEC_VC1_MAIN == HAL_MVD_GetCodecType(u8Idx))
                {
                    ...
                    {
                        MVD_DEBUGERROR(printf("RCV payloadLen(0x%x) invalid!\n",
                            HAL_MVD_GetPayloadLen(u8Idx)));
                    }
                }
                MVD_DEBUGERROR(printf("***** fe=%lx, rd=%lx(%lx,%lx), wr=%lx, empty=%lx, u32FeByteCnt=%x\n",
...
//------------------------------------------------------------------------------
static void HAL_MVD_SetSLQStartEnd(MS_U8 u8Idx, MS_VIRT u32start, MS_VIRT u32end)
{
    MVD_CmdArg mvdcmd;
#if 0 //test for miu0
    MVD_MEMCfg* pstMemCfg = NULL;
...
#define WAIT_DECODE_DONE_TIME 33 //To decode 1 frame should take less than 33ms
    ...
    {
        MVD_DEBUGERROR(printf ("MDrv_MVD_DecodeIFrame time out(du=%u, st=%u, now=%u)\n", u32time, u32deley, HAL_MVD_GetTime()));
        MVD_DEBUGERROR(printf("frmCnt=%u state=0x%x lastCmd=0x%x\n", HAL_MVD_GetPicCounter(u8Idx), HAL_MVD_GetDecodeStatus(u8Idx), HAL_MVD_GetLastCmd(u8Idx)));
        return FALSE;
    }
...
MS_BOOL HAL_MVD_SetInternalBuffAddr(MS_U8 u8Idx, MS_VIRT u32start, MS_U32 u32len)
{
    ...
    MS_VIRT u32BuffStart = u32start+(MVD_FW_TASK_OFFSET*u8Idx);
...
static void MVD_SLQTblInitFileEndPkt(MS_U8 u8Idx, MVD_CodecType eType)
{
    ...
    MVD_DEBUGINFO(printf("u32EndPattern(0x%lx)=0x%08x 0x%08x\n", u32EndPattern,
            HAL_MVD_MemRead4Byte(u32EndPattern), HAL_MVD_MemRead4Byte(u32EndPattern+4)));
}
...
E_MVD_Result HAL_MVD_PushQueue(MS_U8 u8Idx, MVD_PacketInfo* pInfo)
{
    ...
    {
        MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
        MS_U8* temp = (MS_U8*)(MS_PA2KSEG1((MS_VIRT) pInfo->u32StAddr+pstMemCfg->u32BSAddr));
        MS_U32 i;
        MS_U32 u32DummyPattern[3];
        MS_U32 u32PatternSize;
...
#define MAX_VALIDLEN    0x200000    //2M
    ...
    else
    {
        ...
        MVD_DEBUGINFO(printf("(%x) ValidLen=0x%08x CurLen=0x%08x\n",
...
static E_MVD_Result MVD_FlushSlqTblQueue(MS_U8 u8Idx)
{
           ...
           (TRUE != MVD_PatternLenIsValid(u8Idx, u32PatternByteCnt)))
    {
        ...
        {
            MVD_DEBUGERROR(printf("RCV timeout!!! pl=%d, pbc=%d, emp=%ld\n",
    ...
    if (TRUE != HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_PAUSE))
    {
        ...
        MVD_DEBUGERROR(printf("ValidLen=0x%x CurPatternLen=0x%x\n",
                      HAL_MVD_GetPayloadLen(u8Idx), u32PatternByteCnt));
...
MS_BOOL HAL_MVD_EnableInt(MS_U8 u8Idx, MS_U32 bEn)
{
    ...
    MVD_DEBUGINFO(printf("u32IntFlag = 0x%x\n", u32IntFlag));
    SETUP_CMDARG(mvdcmd);
    ...
    if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
    {
        ...
        while ((HAL_MVD_GetTime() - u32TimeCnt) < CMD_TIMEOUT_MS)
        {
            if (HAL_MVD_GetDecodeStatus(u8Idx) != E_MVD_STAT_IDLE)
            {
                ...
                MVD_DEBUGVERBAL(printf("time=%d ms, ", (HAL_MVD_GetTime() - u32TimeCnt)));
                break;
            }
        }
        if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
        {
            MVD_DEBUGERROR(printf("%s: err timeout(%d)! stat=0x%x\n", __FUNCTION__,
...
void HAL_MVD_DbgDump(MS_U8 u8Idx)
{
    ...
    printf("input: vfifo=%d(full=%d,empty=%d), vdCnt=%d(%d), vldErr=%d(%d); ",
...
MS_BOOL _HAL_MVD_CrcTimeOut(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGERROR( printf("_HAL_MVD_CrcTimeOut=%x\n", i) );
    return TRUE;
}
...
E_MVD_Result HAL_MVD_GetCrcValue(MS_U8 u8Idx, MVD_CrcIn *pCrcIn, MVD_CrcOut *pCrcOut)
{
    ...
    MVD_DEBUGINFO(printf("%s width=0x%x, height=0x%x, pitch=0x%x, luma=0x%lx, chroma=0x%lx\n",
...
MS_BOOL HAL_MVD_SetExternal_CC_Buffer(MS_U8 u8Idx, MS_U32 u32base, MS_U8 u8size, MS_U8 cc_type)
{
    ...
    MS_VIRT *u32WritePtr = (MS_VIRT *)(HAL_MVD_PA2NonCacheSeg(u32base+4));
    MS_U32 *u32LatchOverflow = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base+8));
    MS_VIRT *u32CpuBaseAddr = (MS_VIRT *)(HAL_MVD_PA2NonCacheSeg(u32base+12));
    ...
    *u32CpuBaseAddr =  HAL_MVD_GetMEMCfg(u8Idx)->u32FWBinAddr;
    ...
    _MVD_SetUserDataBufStart(u8Idx,_PA2Offset(u32base),cc_type);
...
{
    MVD_MEMCfg* pMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    MS_VIRT CodeAddr = pMemCfg->u32FWCodeAddr;
    MS_VIRT VPUSHMAddr = HAL_VPU_EX_GetSHMAddr();
    MS_VIRT VsyncBridgeAddr = 0;
    ...
    if(VPUSHMAddr != 0)  // TEE project
    {
        ...
        {
            VsyncBridgeAddr = 0;
            MVD_DEBUGERROR(printf("share Memeory and VDEC f/w is in different MIU,(%lx,%lx)\n",VPUSHMAddr,pMemCfg->u32FWCodeAddr));
        }
    }
    else  // normal project
    {
...
E_MVD_Result HAL_MVD_SetTimeIncPredictParam(MS_U8 u8Idx, MS_U32 u32time)
{
    ...
    MVD_DEBUGINFO(printf("CMD_TIME_INCR_PREDICT=%x\n",u32time));
