...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
	...
	if((ptiming->u16Height >= 2160 || ptiming->u16Width >= 3840) && (ptiming->bInterlace == FALSE))
    {
        //Monet/Mooney 4k2k pmode default 30 lines forwarding.
        g_pHalMVOPCtx->bNewVSyncMode = TRUE;
        g_pHalMVOPCtx->u16VsyncLines = 0;
    }
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECOVersion)
{
    ...
    case E_MVOP_DEV_0:
    {
        ...
        //set for miu 16+16 in hsk mode
        HAL_WriteByte(VOP_LSB_DMA0, 0x10);
        HAL_WriteByte(VOP_LSB_DMA1, 0x07);
        HAL_MVOP_LoadReg();
        /*****************************************************/
        break;
    }
    ...
    case E_MVOP_DEV_1:
    {
        ...
        //set for miu 16+16 in hsk mode
        HAL_WriteByte(SUB_REG(VOP_LSB_DMA0), 0x10);
        HAL_WriteByte(SUB_REG(VOP_LSB_DMA1), 0x07);
        HAL_MVOP_SubLoadReg();
        /*****************************************************/
        break;
    }
