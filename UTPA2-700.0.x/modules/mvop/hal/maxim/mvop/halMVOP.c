...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#define MPLL_CLOCK_216           (216000000ULL)
#define MPLL_CLOCK_432           (432000000ULL)
...
#define VSYNC_LINE_4KDV_30        0x60
#define VSYNC_LINE_4KDV_60        0x80
#define VSYNC_LINE_4K_NON_265_60  0x3E
#define VSYNC_LINE_4K_30          0
#define VSYNC_LINE_4K_60          0x62
#define VSYNC_LINE_4K_30_MIR      0
#define VSYNC_LINE_fHD_30_I       0xAD
#define VSYNC_LINE_fHD_60_P       0x2E
#define VSYNC_LINE_fHD_30_MIR     0
...
void _HAL_MVOP_SubInitVarCtx(void)
{
    ...
    g_pHalMVOPCtx->u16SubVsyncLines = 0;
...
void HAL_MVOP_Enable(MS_BOOL bEnable, MS_U8 u8Framerate)
{
    ...
    {
        regval &= ~0x1;
        HAL_Write2Byte(VOP_BF_VS_MVD, 0x200);
        HAL_Write2Byte(VOP_TF_VS_MVD, 0x200);
    }
#if 0
    // sw patch: for mvop hsk mode, switching mvop enable/disable may cause video was stuck.
...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
	...
	if(((((ptiming->u16V_TotalCount >= 2160) && (ptiming->u16H_TotalCount >= 3840)) || ((ptiming->u16V_TotalCount >= 2160) && (ptiming->u16H_TotalCount >= 1920) && g_pHalMVOPCtx->bIs2p))
        && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        g_pHalMVOPCtx->bNewVSyncMode = TRUE;
        if(g_pHalMVOPCtx->bIs265DV)
        {
            if(ptiming->u8Framerate > 30) //for 4k60
            {
                g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_4KDV_60;
            }
            else
            {
                g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_4KDV_30;//mantis:1186138 (from 0x40->0x60)
            }
        ...
        {
            if(ptiming->u8Framerate > 30 && (!g_pHalMVOPCtx->bMirrorModeVer)) //for 4k60
            {
                g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_4K_60;
            }
            else
            {
                g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_4K_30;
    ...
    // patch for manhattan + FRC
    else if(((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
    {
        if(g_pHalMVOPCtx->bMirrorModeVer)
        {
            g_pHalMVOPCtx->bNewVSyncMode = FALSE;
            HAL_WriteRegBit(VOP_CTRL1, 1, VOP_MVD_VS_SEL); // 4 lins forwarding; Mantis ID:1074519
        }
        else
        {
            g_pHalMVOPCtx->bNewVSyncMode = TRUE;
            HAL_MVOP_SetFieldInverse(DISABLE, ENABLE);
            g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_fHD_30_I; //mantis1251174: 0x40 -> 0xAD
            HAL_WriteByte(VOP_DMA0, 0x18);
            HAL_WriteByteMask(VOP_DMA1, 0x01, VOP_BURST_EXT);
        }
    }
    else if(((ptiming->u8Framerate >= 59) && (ptiming->bInterlace == 0)) ||
        (((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 23) && (ptiming->bInterlace == 0)))
    {
        g_pHalMVOPCtx->bNewVSyncMode = TRUE;
        g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_fHD_60_P;
    }
    else
    {
        g_pHalMVOPCtx->u16VsyncLines = 0;
    ...
    if(g_pHalMVOPCtx->bNewVSyncMode)
    {
        ...
        if(g_pHalMVOPCtx->u16VsyncLines == 0)
            ...
            u16TopField_VS2MVD = ptiming->u16V_TotalCount - g_pHalMVOPCtx->u16VsyncLines;
...
#if SUPPORT_FILED_DB // t/b signal 30 lines forwarding
        HAL_WriteRegBit(VOP_MIRROR_CFG_HI, 0, VOP_DC2MVD_FLD_SEL);
        HAL_WriteRegBit(VOP_REG_BW_THD_H, 1, REG_MVD_FLD_SEL);

        HAL_WriteByte((VOP_TF_STR_MVD    ), LOWBYTE( u16TopField_VS2MVD ));
        HAL_WriteByte((VOP_TF_STR_MVD + 1), HIGHBYTE( u16TopField_VS2MVD ));
        HAL_WriteByte((VOP_BF_STR_MVD    ), LOWBYTE( u16BottomField_VS2MVD ));
        HAL_WriteByte((VOP_BF_STR_MVD + 1), HIGHBYTE( u16BottomField_VS2MVD ));
#endif
...
void HAL_MVOP_ResetMFdec()
{
    HAL_WriteRegBit(REG_MFDEC_CFG_H, 1, REG_MFDEC_CFG);
    MsOS_DelayTask(2);
    HAL_WriteRegBit(REG_MFDEC_CFG_H, 0, REG_MFDEC_CFG);
}

void HAL_MVOP_SetFBDecClk(MVOP_DevID eID, MS_BOOL bEnable)
{
    if(bEnable == FALSE)
    {
//Assign Huffman table need to enable FBDec Clk, can not disable in mvop.
#if 0
        if(((eID == E_MVOP_DEV_0) && (g_pHalMVOPCtx->bSubIsInit == FALSE))
            || ((eID == E_MVOP_DEV_1) && (g_pHalMVOPCtx->bIsInit == FALSE))) //main and sub both disable
        {
            HAL_WriteRegBit(REG_CKG_FBDEC, 1, CKG_FBDEC_GATED);
        }
#endif
#if MVOP_SUPPORT_SUB
        if(((eID == E_MVOP_DEV_0) && (!g_pHalMVOPCtx->bSubIsInit)) || ((eID == E_MVOP_DEV_1) && (!g_pHalMVOPCtx->bIsInit)))
#endif
        {
            HAL_MVOP_ResetMFdec();
        }
    ...
    {
        HAL_WriteRegBit(REG_CKG_FBDEC, 0, CKG_FBDEC_GATED);
    }
    //tmp for maxim
    HAL_WriteByteMask(REG_CKG_FBDEC, 0, CKG_FBDEC_MASK);
}
...
void HAL_MVOP_SetSynClk(MVOP_Timing *ptiming)
{
    ...
    {
        MS_U64 u64mpll_clock = MPLL_CLOCK_216 << 27 ; //mvop hw bug, tsp default use 216MHz mpll clock @ maserati
        if(HAL_ReadRegBit(REG_CLK_SYN_STC, BIT0) == BIT0) //check stc1 clock use 432 or 216
        {
            u64mpll_clock = MPLL_CLOCK_432 << 27 ;
        }
        MS_U64 u64exp_clock = (((MS_U64)ptiming->u16H_TotalCount * (MS_U64)ptiming->u16V_TotalCount * (MS_U64)ptiming->u16ExpFrameRate)/1000);
    ...
    else if (_bEnable60P)
    {
        ...
        HAL_WriteRegBit(VOP_REG_YC422_EN_H, 0, VOP_FRAME_RST); // reg_frame_rst = 0
...
void HAL_MVOP_SetMonoMode(MS_BOOL bEnable)
{
    if(bEnable)
    {
#if 0 //0x28[9] reg_h264_purey not work in monet, use vc1 range map replace
        HAL_WriteByte(VOP_U_PAT  , 0x80);
        HAL_WriteByte((VOP_U_PAT+1), 0x80);

        HAL_WriteRegBit(VOP_INPUT_SWITCH1, 1, BIT1);    // Mono mode enable
#endif
        HAL_WriteByteMask(VOP_RAMAP_CHROMA, 0x0, VOP_RAMAP_CHROMA_VAL);
        HAL_MVOP_LoadReg(); // VOP_RAMAP_CHROMA_VAL double buffer
        HAL_WriteRegBit(VOP_RAMAP_CHROMA, 1, VOP_RAMAP_CHROMA_EN);
        HAL_WriteRegBit(VOP_INFO_FROM_CODEC_L, 0, VOP_INFO_FROM_CODEC_RANGE_MAP);
    }
    else
    {
#if 0
...
#endif
        HAL_WriteByteMask(VOP_RAMAP_CHROMA, 0x8, VOP_RAMAP_CHROMA_VAL);
        HAL_MVOP_LoadReg(); // VOP_RAMAP_CHROMA_VAL double buffer
        HAL_WriteRegBit(VOP_RAMAP_CHROMA, 0, VOP_RAMAP_CHROMA_EN);
        HAL_WriteRegBit(VOP_INFO_FROM_CODEC_L, 1, VOP_INFO_FROM_CODEC_RANGE_MAP);
...
/******************************************************************************/
/// Set MVOP for EVD Dolby Vision Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SetEVDHardwireMode_DV(void)
{
    // Hardwire mode
    HAL_WriteByte(VOP_MPG_JPG_SWITCH, 0x00);
    ...
    HAL_WriteRegBit(VOP_REG_4K2K_2P, 1, VOP_TRIG_REFER_VB_END);

    g_pHalMVOPCtx->bIs265DV = 1;
...
void HAL_MVOP_SetVP9HardwireMode(MS_U16 u16ECOVersion)
{
    ...
    // VP9 use WISHBONE(R2) interface
    HAL_WriteRegBit(VOP_INPUT_SWITCH0, 1, VOP_R2_WISHBONE);

    // Enable VP9 dram continue mode
    //HAL_WriteRegBit(VOP_MPG_JPG_SWITCH, 1, VOP_DRAM_RD_MODE);
...
void HAL_MVOP_SetEVDFeature(MVOP_DevID eID, MVOP_EVDFeature* stEVDInput)
{
    switch(eID)
    {
        case E_MVOP_DEV_0:
            ...
            {
                //HAL_WriteByteMask(VOP_REG_MASK, BIT1, VOP_LSB_REQ_MASK);
                MVOP_DBG(MVOP_PRINTF("[Debug] 10 bits bw riu control default enable\n"););
            }
...
void HAL_MVOP_GetMaxFramerate(MVOP_DevID eDevID, MVOP_GetMaxFps* stStreamInfo)
{
    ...
    switch(eDevID)
    {
...
#endif
        {
            ...
            do_div(u64MaxClk, u16HsizeTiming);
            do_div(u64MaxClk, u16VsizeTiming);
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECOVersion)
{
    ...
    case E_MVOP_DEV_0:
    {
        ...
        // Reset MVOP setting
        HAL_WriteByteMask(VOP_TST_IMG, 0x00, BIT2 | BIT1 | BIT0);
        HAL_WriteRegBit(VOP_TST_IMG, BIT6, BIT6);
        HAL_MVOP_Set3DLRAltOutput_VHalfScaling(DISABLE); //reset to default: disable 3D L/R alternative output.
        HAL_MVOP_Set3DLR2ndCfg(DISABLE);    //reset to default: disable 3D L/R 2nd pitch.
        HAL_MVOP_SetRgbFormat(E_MVOP_RGB_NONE); //reset rgb format
        HAL_WriteRegBit(VOP_CTRL1, 0, VOP_MVD_VS_MD); //default use original vsync
        ...
        HAL_WriteByteMask(VOP_REG_MFDEC_0_L, 0x70, VOP_MF1_BURST|VOP_MF0_BURST|VOP_MFDEC_EN);
        ...
        // All codec use WISHBONE(R2) interface in manhathan
        HAL_WriteRegBit(VOP_INPUT_SWITCH0, 1, VOP_R2_WISHBONE);
        ...
        HAL_MVOP_Enable2DCTimingSync(DISABLE);
        HAL_MVOP_EnableHDRSetting(DISABLE);
#endif
#if 0
		if(u16ECOVersion >= 1) //monet u02 support mvop lsb bw control by wb
		{
			HAL_WriteByteMask(VOP_REG_MASK, 0x0, VOP_LSB_REQ_MASK);
			HAL_WriteRegBit(VOP_UV_SHIFT, 0, VOP_GCLK_MIU_ON);
		}
#endif
        g_pHalMVOPCtx->eInterlaceType = E_MVOP_PRO;
        //Disable DV vision
        g_pHalMVOPCtx->bIs265DV = 0;
        HAL_WriteRegBit(VOP_REG_4K2K_2P, 0, VOP_TRIG_REFER_VB_END);
#if SUPPORT_FILED_DB // t/b signal 30 lines forwarding
        HAL_WriteRegBit(VOP_MIRROR_CFG_HI, 1, VOP_DC2MVD_FLD_SEL);
        HAL_WriteRegBit(VOP_REG_BW_THD_H, 0, REG_MVD_FLD_SEL);

        HAL_WriteByte((VOP_TF_STR_MVD    ), 0);
        HAL_WriteByte((VOP_TF_STR_MVD + 1), 0);
        HAL_WriteByte((VOP_BF_STR_MVD    ), 0);
        HAL_WriteByte((VOP_BF_STR_MVD + 1), 0);
#endif
        HAL_WriteRegBit(VOP_REG_DUMMY_5D_L, 0, XC_RESET_HCOUNT); //disable hw patch for kano gop, only enable at xc gen  timing+de mode
        //set for ddr4: 1014_16 = 0x000a
        HAL_WriteByte(VOP_LSB_DMA0, 0x12);
        HAL_WriteByte(VOP_LSB_DMA1, 0x00);
        //reset mono mode
        HAL_WriteByte(VOP_RAMAP_CHROMA, 0x08);
        HAL_WriteRegBit(VOP_INFO_FROM_CODEC_L, 1, VOP_INFO_FROM_CODEC_RANGE_MAP);
    ...
    case E_MVOP_DEV_1:
    {
        ...
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 0, VOP_EVD_10B_UV_EN);
        // Enable 420 BW Saving mode
        HAL_MVOP_SubSet420BWSaveMode(TRUE, u16ECOVersion);
        // VP9 MODE disable
        ...
        // Setting MF burst len
        HAL_WriteByteMask(SUB_REG(VOP_REG_MFDEC_0_L), 0x70, VOP_MF1_BURST|VOP_MF0_BURST|VOP_MFDEC_EN);
        // Only for monaco: Disable mfdec setting from wb
...
#ifdef UFO_MVOP_DOLBY_HDR
        HAL_MVOP_Enable2DCTimingSync(DISABLE);
        HAL_MVOP_SubEnableHDRSetting(DISABLE);
#endif
        ...
        g_pHalMVOPCtx->eSubInterlaceType = E_MVOP_PRO;
#if SUPPORT_FILED_DB // t/b signal 30 lines forwarding
        HAL_WriteRegBit(SUB_REG(VOP_MIRROR_CFG_HI), 1, VOP_DC2MVD_FLD_SEL);
        HAL_WriteRegBit(SUB_REG(VOP_REG_BW_THD_H), 0, REG_MVD_FLD_SEL);

        HAL_WriteByte(SUB_REG(VOP_TF_STR_MVD    ), 0);
        HAL_WriteByte(SUB_REG(VOP_TF_STR_MVD + 1), 0);
        HAL_WriteByte(SUB_REG(VOP_BF_STR_MVD    ), 0);
        HAL_WriteByte(SUB_REG(VOP_BF_STR_MVD + 1), 0);
#endif

        HAL_WriteRegBit(SUB_REG(VOP_REG_DUMMY_5D_L), 0, XC_RESET_HCOUNT); //disable hw patch for kano gop, only enable at xc gen  timing+de mode
        //set for ddr4: 1014_16 = 0x000a
        HAL_WriteByte(SUB_REG(VOP_LSB_DMA0), 0x12);
        HAL_WriteByte(SUB_REG(VOP_LSB_DMA1), 0x00);
        //reset mono mode
        HAL_WriteByte(SUB_REG(VOP_RAMAP_CHROMA), 0x08);
        HAL_WriteRegBit(SUB_REG(VOP_INFO_FROM_CODEC_L), 1, VOP_INFO_FROM_CODEC_RANGE_MAP);
...
void HAL_MVOP_SetHandShakeMode(MVOP_DevID eID, MS_BOOL bEnable, MS_U8 u8Framerate)
{
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            if(!((bEnable == FALSE) && (g_pHalMVOPCtx->bIsHS == FALSE)) && !(g_pHalMVOPCtx->bIs265DV)) //prevent switch VOP_CTRL0 (mvop enable), for MHEG5 verify
            {

                HAL_WriteRegBit(VOP_CTRL0, 0, BIT0);
                g_pHalMVOPCtx->bIsHS = bEnable;
                HAL_WriteRegBit(VOP_REG_HS_OUTPUT, bEnable, VOP_HS_MODE);
#if 0  // maxim default mvop timing gen
                ...
                HAL_WriteRegBit(VOP_REG_4K2K_2P, bEnable, VOP_TRIG_REFER_VB_END);//data need to wait for xc setting
        ...
        {
#if MVOP_SUPPORT_SUB
            if(!((bEnable == FALSE) && (g_pHalMVOPCtx->bSubIsHS == FALSE)) && !(g_pHalMVOPCtx->bIs265DV)) //prevent switch VOP_CTRL0 (mvop enable), for MHEG5 verify
            {
                ...
                HAL_WriteRegBit(SUB_REG(VOP_REG_4K2K_2P), bEnable, VOP_TRIG_REFER_VB_END); //data need to wait for xc setting
...
void HAL_MVOP_SetCropforXC(MVOP_DevID eID, MVOP_XCGetCrop* stXCCrop, MS_U16 u16Width, MS_U16 u16Height)
{
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
...
#if 0 //sw patch, for monet/manhattan: maserati fix
            if((stXCCrop->u16YSize <= 512) && (stXCCrop->u16YStart == 0) && (g_pHalMVOPCtx->bMirrorModeVer == TRUE))
            {
                MVOP_PRINTF("[MVOP][Dbg] crop + mirror patch\n");
                stXCCrop->u16YStart = 1;
                stXCCrop->u16YStart = stXCCrop->u16YStart | 0x2000;

                g_pHalMVOPCtx->u16CropYSize = stXCCrop->u16YSize;
                if(g_pHalMVOPCtx->bIsH265 == FALSE)
                {
                    stXCCrop->u16YSize -= 1;
                }
            }
            ...
            //from maserati
            if(g_pHalMVOPCtx->bMirrorModeVer == TRUE)
            {
                stXCCrop->u16YStart |= 0x2000;
            }
            if(stXCCrop->u16YStart == 0)
            {
                stXCCrop->u16YStart |= 0x4000;
            }
            g_pHalMVOPCtx->u16CropYSize = stXCCrop->u16YSize;
        ...
        case E_MVOP_DEV_1:
        {
...
#if 0 //sw patch, for monet/manhattan: maserati fix
            if((stXCCrop->u16YSize <= 512) && (stXCCrop->u16YStart == 0) && (g_pHalMVOPCtx->bSubMirrorModeVer == TRUE))
            {
                MVOP_PRINTF("[MVOP][Dbg] crop + mirror patch\n");
                stXCCrop->u16YStart = 1;
                stXCCrop->u16YStart = stXCCrop->u16YStart | 0x2000;
                ...
                if(g_pHalMVOPCtx->bSubIsH265 == FALSE)
                {
                    stXCCrop->u16YSize -= 1;
                }
            }
            ...
            //from maserati
            if(g_pHalMVOPCtx->bSubMirrorModeVer == TRUE)
            {
                stXCCrop->u16YStart |= 0x2000;
            }
            if(stXCCrop->u16YStart == 0)
...
#endif
#ifdef UFO_MVOP_DOLBY_HDR
void HAL_MVOP_Enable2DCTimingSync(MS_BOOL bEnable)
{
    MS_U8 regval;

    regval = HAL_ReadRegBit(SUB_REG(VOP_LSB_DMA1), VOP_SYNC_2_DC_TIMING);

    if(VOP_SYNC_2_DC_TIMING == regval) //previous enable sync
    {
        HAL_WriteRegBit(SUB_REG(VOP_CTRL0), 0, BIT0); // sub should not work here, turn off
        HAL_WriteRegBit(VOP_CTRL0, 0, BIT0);
        HAL_WriteRegBit(SUB_REG(VOP_LSB_DMA1), bEnable, VOP_SYNC_2_DC_TIMING);
        if(bEnable)
        {
            HAL_WriteRegBit(VOP_CTRL0, 1, BIT0);
        }
        ...
        {
            HAL_WriteRegBit(VOP_CTRL0, g_pHalMVOPCtx->bIsEnable, BIT0);
            HAL_WriteRegBit(SUB_REG(VOP_CTRL0), g_pHalMVOPCtx->bSubIsEnable, BIT0);
        }
    }
    else //previous disable sync
    {
        if(bEnable)
        {
            HAL_WriteRegBit(SUB_REG(VOP_CTRL0), 0, BIT0); // sub should not work here, turn off
...
void HAL_MVOP_EnableHDRSetting(MS_BOOL bEnable)
{
    HAL_WriteRegBit(VOP_REG_YC422_EN_H, bEnable, BIT0);
    HAL_WriteRegBit(VOP_REG_HS_OUTPUT, bEnable, VOP_HS_MODE);
    HAL_WriteRegBit(VOP_CTRL0, bEnable, BIT4);
    HAL_WriteRegBit(VOP_MIRROR_CFG_HI, bEnable, VOP_HK_MASK); //bk background
    if(bEnable)
...
void HAL_MVOP_SubEnable(MS_BOOL bEnable, MS_U8 u8Framerate)
{
    ...
    {
        regval &= ~0x1;
        HAL_Write2Byte(SUB_REG(VOP_BF_VS_MVD), 0x200);
        HAL_Write2Byte(SUB_REG(VOP_TF_VS_MVD), 0x200);
    }
#if 0
    // sw patch: for mvop hsk mode, switching mvop enable/disable may cause video was stuck.
...
void HAL_MVOP_SubSetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion )
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        //Maxim sw patch: hw bug sub can not use bw saving
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    if(((((ptiming->u16V_TotalCount >= 2160) && (ptiming->u16H_TotalCount >= 3840)) || ((ptiming->u16V_TotalCount >= 2160) && (ptiming->u16H_TotalCount >= 1920) && g_pHalMVOPCtx->bSubIs2p))
            && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        ...
        if(g_pHalMVOPCtx->bIs265DV)
        {
            if(ptiming->u8Framerate > 30) //for 4k60
            {
                g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_4KDV_60;
            }
            else
            {
                g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_4KDV_30;//mantis:1186138 (from 0x40->0x60)
            }
        ...
        {
            if(ptiming->u8Framerate > 30 && (!g_pHalMVOPCtx->bSubMirrorModeVer)) //for 4k60
            {
                g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_4K_60;
            }
            else
            {
                g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_4K_30;
    ...
    else if(((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
    {
        ...
        {
            g_pHalMVOPCtx->bSubNewVSyncMode = FALSE;
            HAL_WriteRegBit(SUB_REG(VOP_CTRL1), 1, VOP_MVD_VS_SEL); // 4 lins forwarding; Mantis ID:1074519
        }
        else
        {
            g_pHalMVOPCtx->bSubNewVSyncMode = TRUE;
            HAL_MVOP_SubSetFieldInverse(DISABLE, ENABLE);
            g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_fHD_30_I; //mantis1251174: 0x40 -> 0xAD
            HAL_WriteByte(SUB_REG(VOP_DMA0), 0x18);
            HAL_WriteByteMask(SUB_REG(VOP_DMA1), 0x01, VOP_BURST_EXT);
        }
    }
    ...
    {
        g_pHalMVOPCtx->bSubNewVSyncMode = TRUE;
        g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_fHD_60_P;
    }
    else
    {
        g_pHalMVOPCtx->u16SubVsyncLines = 0;
        HAL_WriteByte(SUB_REG(VOP_DMA0), 0x08);
        HAL_WriteByteMask(SUB_REG(VOP_DMA1), 0x02, VOP_BURST_EXT);
        HAL_WriteRegBit(SUB_REG(VOP_CTRL1), 1, VOP_MVD_VS_SEL); //Default 4 lines forwarding
    }
    ...
    if (g_pHalMVOPCtx->bSubNewVSyncMode)
    {
        ...
        if(g_pHalMVOPCtx->u16SubVsyncLines == 0)
            u16BottomField_VS2MVD = ptiming->u16BottomField_VS - SUB_NEW_VSYNC_MODE_ADVANCE_LINECNT;
        else
            u16BottomField_VS2MVD = ptiming->u16BottomField_VS - g_pHalMVOPCtx->u16SubVsyncLines;
        MVOP_DBG(MVOP_PRINTF("BottomField VS ori=0x%x, new=0x%x\n", ptiming->u16BottomField_VS, u16BottomField_VS2MVD);)
        ...
        if(g_pHalMVOPCtx->u16SubVsyncLines == 0)
            u16TopField_VS2MVD = ptiming->u16V_TotalCount - SUB_NEW_VSYNC_MODE_ADVANCE_LINECNT;
        else
            u16TopField_VS2MVD = ptiming->u16V_TotalCount - g_pHalMVOPCtx->u16SubVsyncLines;
        MVOP_DBG(MVOP_PRINTF("TopField VS Vtt=0x%x, new=0x%x\n", ptiming->u16V_TotalCount, u16TopField_VS2MVD);)
...
#if SUPPORT_FILED_DB // t/b signal 30 lines forwarding
        HAL_WriteRegBit(SUB_REG(VOP_MIRROR_CFG_HI), 0, VOP_DC2MVD_FLD_SEL);
        HAL_WriteRegBit(SUB_REG(VOP_REG_BW_THD_H), 1, REG_MVD_FLD_SEL);

        HAL_WriteByte(SUB_REG(VOP_TF_STR_MVD    ), LOWBYTE( u16TopField_VS2MVD ));
        HAL_WriteByte(SUB_REG(VOP_TF_STR_MVD + 1), HIGHBYTE( u16TopField_VS2MVD ));
        HAL_WriteByte(SUB_REG(VOP_BF_STR_MVD    ), LOWBYTE( u16BottomField_VS2MVD ));
        HAL_WriteByte(SUB_REG(VOP_BF_STR_MVD + 1), HIGHBYTE( u16BottomField_VS2MVD ));
#endif
...
void HAL_MVOP_SubSetSynClk(MVOP_Timing *ptiming)
{
    ...
    if ( g_pHalMVOPCtx->bIs265DV == 1)
    {
        g_pHalMVOPCtx->bSubEnableFreerunMode = g_pHalMVOPCtx->bEnableFreerunMode;
        _bSubEnable60P = _bEnable60P;
        _bSubEnable4k2kClk = _bEnable4k2kClk;
    }

    if(g_pHalMVOPCtx->bSubEnableFreerunMode)
    {
        MS_U64 u64mpll_clock = MPLL_CLOCK_216 << 27 ; //mvop hw bug, tsp default use 216MHz mpll clock @ maserati
        if(HAL_ReadRegBit(REG_CLK_SYN_STC, BIT0) == BIT4) //check stc1 clock use 432 or 216
        {
            u64mpll_clock = MPLL_CLOCK_432 << 27 ;
        }
        MS_U64 u64exp_clock = (((MS_U64)ptiming->u16H_TotalCount * (MS_U64)ptiming->u16V_TotalCount * (MS_U64)ptiming->u16ExpFrameRate)/1000);
    ...
    else if (_bSubEnable60P)
    {
        ...
        HAL_WriteRegBit(SUB_REG(VOP_REG_YC422_EN_H), 0, VOP_FRAME_RST); // reg_frame_rst = 0
...
#endif
        HAL_WriteByteMask(SUB_REG(VOP_RAMAP_CHROMA), 0x0, VOP_RAMAP_CHROMA_VAL);
        HAL_MVOP_SubLoadReg(); // VOP_RAMAP_CHROMA_VAL double buffer
        HAL_WriteRegBit(SUB_REG(VOP_RAMAP_CHROMA), 1, VOP_RAMAP_CHROMA_EN);
        HAL_WriteRegBit(SUB_REG(VOP_INFO_FROM_CODEC_L), 0, VOP_INFO_FROM_CODEC_RANGE_MAP);
    }
    else
    {
#if 0
...
#endif
        HAL_WriteByteMask(SUB_REG(VOP_RAMAP_CHROMA), 0x8, VOP_RAMAP_CHROMA_VAL);
        HAL_MVOP_SubLoadReg(); // VOP_RAMAP_CHROMA_VAL double buffer
        HAL_WriteRegBit(SUB_REG(VOP_RAMAP_CHROMA), 0, VOP_RAMAP_CHROMA_EN);
        HAL_WriteRegBit(SUB_REG(VOP_INFO_FROM_CODEC_L), 1, VOP_INFO_FROM_CODEC_RANGE_MAP);
    }
}
...
void HAL_MVOP_SubSetVP9HardwireMode(MS_U16 u16ECOVersion)
{
    ...
    // Enable VP9 dram continue mode
    //HAL_WriteRegBit(SUB_REG(VOP_MPG_JPG_SWITCH), 1, VOP_DRAM_RD_MODE);
...
MS_BOOL bValue = FALSE;
    ...
    else
    {
        ...
        HAL_WriteRegBit(SUB_REG(VOP_REG_DUMMY), 0, VOP_420_BW_SAVE);
        HAL_WriteRegBit(SUB_REG(VOP_REG_BW_SAVE), 0, VOP_420_BW_SAVE_EX);
        MVOP_DBG(MVOP_PRINTF("%s Maxim sw patch: hw bug sub can not use bw saving\n", __FUNCTION__);)
        HAL_MVOP_SubLoadReg();
        return TRUE;
    }
}
...
#ifdef UFO_MVOP_DOLBY_HDR
void HAL_MVOP_SubEnableHDRSetting(MS_BOOL bEnable)
{
    HAL_WriteRegBit(SUB_REG(VOP_REG_YC422_EN_H), bEnable, BIT0);
    HAL_WriteRegBit(SUB_REG(VOP_REG_HS_OUTPUT), bEnable, VOP_HS_MODE);
    HAL_WriteRegBit(SUB_REG(VOP_CTRL0), bEnable, BIT4);
    HAL_WriteRegBit(SUB_REG(VOP_MIRROR_CFG_HI), bEnable, VOP_HK_MASK); //bk background
    if(bEnable)
