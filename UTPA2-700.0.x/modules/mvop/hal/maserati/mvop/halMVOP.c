...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
        ...
        && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        ...
        if(g_pHalMVOPCtx->bIs265DV)
        {
            ...
            {
                g_pHalMVOPCtx->u16VsyncLines = 0x10D; //0x9d->0x10d for 120Hz trick mode.
            }
            else
            {
                g_pHalMVOPCtx->u16VsyncLines = 0x60;//mantis 1205202
            }
            HAL_WriteByte(VOP_DMA0, 0x18);
            HAL_WriteByteMask(VOP_DMA1, 0x01, VOP_BURST_EXT);
        }
        else if(ptiming->bInterlace == 1)
        {
            g_pHalMVOPCtx->u16VsyncLines = 0x20;
            HAL_WriteByte(VOP_DMA0, 0x18);
            HAL_WriteByteMask(VOP_DMA1, 0x01, VOP_BURST_EXT);
        }
        else
        {
            ...
            else
            {
                ...
                HAL_WriteByte(VOP_DMA0, 0x08);
                HAL_WriteByteMask(VOP_DMA1, 0x02, VOP_BURST_EXT);
                g_pHalMVOPCtx->bNewVSyncMode = 0;
                HAL_WriteRegBit(VOP_CTRL1, 1, VOP_MVD_VS_SEL); //4k 30 only 4 lines forwarding: mantis 1185981
            }
        }
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECOVersion)
{
    ...
    case E_MVOP_DEV_0:
    {
        ...
        HAL_WriteRegBit(VOP_REG_MFDEC_2_L, 1, VOP_MF_FROM_WB); //str huffman table issue
        // MIU select from WB
        ...
        HAL_WriteByte(VOP_LSB_DMA0, 0x0A);
    ...
    case E_MVOP_DEV_1:
    {
        ...
        g_pHalMVOPCtx->eSubInterlaceType = E_MVOP_PRO;
        //set for ddr4: 1014_16 = 0x000a
        HAL_WriteByte(SUB_REG(VOP_LSB_DMA0), 0x0A);
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            if(!((bEnable == FALSE) && (g_pHalMVOPCtx->bIsHS == FALSE)) && !(g_pHalMVOPCtx->bIs265DV)) //prevent switch VOP_CTRL0 (mvop enable), for MHEG5 verify
            {
...
#if 0 // maserati default mvop timing gen
                ...
                HAL_WriteRegBit(VOP_REG_4K2K_2P, bEnable, VOP_TRIG_REFER_VB_END);
        ...
        case E_MVOP_DEV_1:
        {
            ...
            if(!((bEnable == FALSE) && (g_pHalMVOPCtx->bSubIsHS == FALSE)) && !(g_pHalMVOPCtx->bIs265DV)) //prevent switch VOP_CTRL0 (mvop enable), for MHEG5 verify
            {
                ...
                HAL_WriteRegBit(SUB_REG(VOP_REG_4K2K_2P), bEnable, VOP_TRIG_REFER_VB_END);
...
void HAL_MVOP_EnableHDRSetting(MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        HAL_WriteRegBit(VOP_REG_YC422_EN_H, 0, VOP_FRAME_RST); // for Dolby crop
    }
    g_pHalMVOPCtx->bIsHS = bEnable;
}
#endif
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
            ...
            && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        ...
        if(g_pHalMVOPCtx->bIs265DV)
        {
            if(ptiming->u8Framerate > 30) //for 4k60
            {
                g_pHalMVOPCtx->u16SubVsyncLines = 0x10D;//0x9d->0x10d for 120Hz trick mode.
            }
            else
            {
                g_pHalMVOPCtx->u16SubVsyncLines = 0x60;//mantis 1205202
            }
        ...
        {
            g_pHalMVOPCtx->u16SubVsyncLines = 0x20;
            HAL_WriteByte(SUB_REG(VOP_DMA0), 0x18);
            HAL_WriteByteMask(SUB_REG(VOP_DMA1), 0x01, VOP_BURST_EXT);
        }
        else
        {
            ...
            else
            {
                ...
                HAL_WriteByte(SUB_REG(VOP_DMA0), 0x08);
                HAL_WriteByteMask(SUB_REG(VOP_DMA1), 0x02, VOP_BURST_EXT);
                g_pHalMVOPCtx->bSubNewVSyncMode = 0;
                HAL_WriteRegBit(SUB_REG(VOP_CTRL1), 1, VOP_MVD_VS_SEL); //4k 30 only 4 lines forwarding: mantis 1185981
            }
        }
    }
    else if(((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
...
void HAL_MVOP_SubEnableHDRSetting(MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        HAL_WriteRegBit(SUB_REG(VOP_REG_YC422_EN_H), 0, VOP_FRAME_RST); // for Dolby crop
    }
    g_pHalMVOPCtx->bSubIsHS = bEnable;
    if(g_pHalMVOPCtx->bMirrorModeVer)
    {
