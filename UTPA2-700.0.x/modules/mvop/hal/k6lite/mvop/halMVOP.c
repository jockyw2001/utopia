...
void HAL_SetTSPCW(MVOP_DevID eID)
{
    HAL_WriteByte((REG_TSP_CLK + 1), 0x00);
    HAL_WriteRegBit(REG_TSP_RST, 0, BIT1);
    HAL_WriteRegBit(REG_TSP_RST, 1, BIT1);
}
...
void HAL_MVOP_SetFrequency(HALMVOPFREQUENCY enFrequency)
{
    ...
    switch(enFrequency)
    {
        ...
        case HALMVOP_FREERUNMODE:
            ...
            break;
        case HALMVOP_320MHZ:
            //K6LITE not support 4k2k
            //HAL_WriteByteMask(REG_CKG_DC0, CKG_DC0_320MHZ, CKG_DC0_MASK);
            MVOP_PRINTF("Attention! not support 320MHz!\n");
            break;
        case HALMVOP_108MHZ:
...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    g_pHalMVOPCtx->u16Hsize = ptiming->u16Width;
    //for supporting out of spec, line buffer only 2048
    if(ptiming->u16Width > 2048)
    {
        HAL_WriteRegBit(VOP_REG_DUMMY, 0, VOP_420_BW_SAVE);
        HAL_WriteRegBit(VOP_REG_BW_SAVE, 0, VOP_420_BW_SAVE_EX);
        HAL_MVOP_LoadReg();
...
MS_BOOL HAL_MVOP_Set420BWSaveMode(MS_BOOL bEnable)
{
    ...
    bValue = (g_pHalMVOPCtx->b3DLRAltSBSOutput || g_pHalMVOPCtx->b3DLRAltOutput /*|| g_pHalMVOPCtx->b3DLRMode*/ || g_pHalMVOPCtx->bIs422 || (g_pHalMVOPCtx->u16Hsize > 2048));
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECONumber)
{
    ...
    HAL_WriteRegBit(VOP_REG_MASK, 0, VOP_LSB_REQ_MASK);//10 bits bw control by vdec fw
    HAL_WriteRegBit(VOP_EVD_10B_EN, 0, VOP_EVD_10B_Y_EN);
    HAL_WriteRegBit(VOP_EVD_10B_EN, 0, VOP_EVD_10B_UV_EN);
    HAL_WriteRegBit(VOP_UV_SHIFT, 1, VOP_GCLK_MIU_ON);
    ...
    case E_MVOP_DEV_1:
    {
        ...
        HAL_WriteRegBit(SUB_REG(VOP_REG_MASK), 0, VOP_LSB_REQ_MASK);//10 bits bw control by vdec fw
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 0, VOP_EVD_10B_Y_EN);
        HAL_WriteRegBit(SUB_REG(VOP_EVD_10B_EN), 0, VOP_EVD_10B_UV_EN);
        // Disable 420 BW Saving mode
...
HALMVOPFREQUENCY HAL_MVOP_SetFixClk(MVOP_DevID eID, MS_U32 u32MVOPClk)
{
    ...
    if(eID == E_MVOP_DEV_0)
    {
        ...
        {
            g_pHalMVOPCtx->u32MVOPFixClk = HAL_MVOP_GetMaximumClk();
        }
        else
        {
    ...
    else if(eID == E_MVOP_DEV_1)
    {
        if(eRet > HALMVOP_192MHZ)
        {
            g_pHalMVOPCtx->u32SubMVOPFixClk = HAL_MVOP_GetMaximumClk();
        }
        else
        {
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    g_pHalMVOPCtx->u16SubHsize = ptiming->u16Width;
    //for supporting out of spec, line buffer only 2048
    if(ptiming->u16Width > 2048)
    {
...
void HAL_MVOP_SubSetDCSRAMClk(MS_U8 clkNum, MS_BOOL bEnable)
{
    ...
    {
        if(bEnable)
            HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x40, CKG_DC1_SRAM); //Dc1 enable & dc1_mfdec_en =0  57[7:4]= 0x4, others  57[7:4]=0x0
        else
...
MS_BOOL HAL_MVOP_SubSet420BWSaveMode(MS_BOOL bEnable, MS_U16 u16ECOVersion)
{
    ...
    //hw limtation: 3DLA/3DSBS/422/p mode in, i mode out/i mode in, p mode out(only need to check in MCU mode)
    bValue = (g_pHalMVOPCtx->bSub3DLRAltSBSOutput || g_pHalMVOPCtx->bSub3DLRAltOutput/* || g_pHalMVOPCtx->bSub3DLRMode */|| g_pHalMVOPCtx->bSubIs422 || (g_pHalMVOPCtx->u16SubHsize > 2048));
