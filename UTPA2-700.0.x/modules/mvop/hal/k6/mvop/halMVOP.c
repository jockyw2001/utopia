...
typedef struct
{
    ...
    MS_BOOL bSubIsEnLSB;
    MS_BOOL bSubIsMCUMfdec;
    MS_U8   u8SubLumaMIU;
    MS_U8   u8SubChromaMIU;
#endif
}MVOP_CTX_HAL;
...
void _HAL_MVOP_InitVarCtx(void)
{
    ...
    g_pHalMVOPCtx->u8LumaMIU = 0;
    g_pHalMVOPCtx->u8SubChromaMIU = 0;
}
...
void HAL_MVOP_SelMIU(MVOP_DevID eDevID, HALMVOPMIUSEL eMiuMSB0, HALMVOPMIUSEL eMiuMSB1, HALMVOPMIUSEL eMiuLSB0, HALMVOPMIUSEL eMiuLSB1)
{
    ...
    switch(eDevID)
    {
        case E_MVOP_DEV_0:
        {
            ...
            g_pHalMVOPCtx->u8LumaMIU = eMiuMSB0;
            g_pHalMVOPCtx->u8ChromaMIU = eMiuMSB1;
            break;
        }
#if MVOP_SUPPORT_SUB
        case E_MVOP_DEV_1:
        {
            ...
            g_pHalMVOPCtx->u8SubLumaMIU = eMiuMSB0;
            g_pHalMVOPCtx->u8SubChromaMIU = eMiuMSB1;
            break;
        }
#endif
        default:
...
void HAL_MVOP_SetMFDECInfo(MVOP_DevID eID, HALMVOPMFDECINFO *pMFDECInfo)
{
    ...
    switch(eID)
    {
        ...
        {
            #if 1
            g_pHalMVOPCtx->bIsMCUMfdec = pMFDECInfo->bMFDEC_EN;
            if(pMFDECInfo->bMFDEC_EN)
            {
                HAL_WriteRegBit(VOP_REG_MFDEC_0_L, TRUE, VOP_MFDEC_EN); // enable mfdec
                HAL_WriteRegBit(REG_MF_BASIC_L, TRUE, MF_MFDEC_EN); // enable mfdec
                HAL_WriteByteMask(REG_MF_BASIC_H, 0x30, (MF_RIU_MODE|MF_RIU1_MODE));

                u16Hszie = HAL_Read2Byte(VOP_JPG_HSIZE);
                u16Vsize = HAL_Read2Byte(VOP_JPG_VSIZE);
                ...
                HAL_MVOP_LOAD_MFDEC_TABLE(pMFDECInfo->u8MFDEC_ID);//k6 mcu mode need to use 0x1122 bank
...
#endif
                HAL_WriteByteMask(REG_MF_BASIC_L, (pMFDECInfo->u32UNCOMPRESS_MODE << 5), MF_UNCOMP_MODE);
                HAL_Write2Byte(REG_MF_BT_ADDR_L, u64tmp & 0xffff);
                HAL_Write2Byte(REG_MF_BT_ADDR_H, (u64tmp >> 16) & 0x3ff);
                HAL_Write2Byte(REG_MF_FB_WIDTH, u16Hszie>>4);
                HAL_Write2Byte(REG_MF_FB_HEIGHT, u16Vsize);
                HAL_Write2Byte(REG_MF_FB_PITCH, u16Hszie>>3);

                HAL_WriteByteMask(REG_MF_MIU, (pMFDECInfo->u8BITLEN_FB_MIU << 4), MF_BITLEN_MIU);
                HAL_WriteByteMask(REG_MF_MIU, g_pHalMVOPCtx->u8LumaMIU, MF_LUMA_MIU);
                HAL_WriteByteMask(REG_MF_MIU, g_pHalMVOPCtx->u8ChromaMIU<<2, MF_CHROMA_MIU);
                HAL_WriteByte(REG_MF_BT_PITCH, pMFDECInfo->u32BITLEN_FB_PITCH);
                HAL_WriteRegBit(VOP_REG_MFDEC_5E, pMFDECInfo->bBITLEN_SHT_8, VOP_BITLEN_SHT);
            }
            else
            {
        ...
        case E_MVOP_DEV_1:
        {
            g_pHalMVOPCtx->bSubIsMCUMfdec = pMFDECInfo->bMFDEC_EN;
            if(pMFDECInfo->bMFDEC_EN)
            {
                ...
                HAL_WriteRegBit(SUB_REG(VOP_REG_MFDEC_5E), pMFDECInfo->bBITLEN_SHT_8, VOP_BITLEN_SHT);
            }
            else
            {
...
MS_U16 MFDec_HUFF_Table[MFEDC_TABLE_INDEX_NUM][4] =
{
...
void HAL_MVOP_LOAD_MFDEC_TABLE(MS_U8 mfdec_id)
{
    MS_U8 u8index=0;

    //MVOP_PRINTF("[apple] HAL_MVOP_LOAD_MFDEC_TABLE\n");

    //HAL_Write2ByteMask(REG_MF_BASIC_H, 0x3000 , BMASK(15:0));
    HAL_Write2ByteMask(REG_MF_CW_LEN2(mfdec_id), 0x0007 , BMASK(11:0));
    HAL_Write2ByteMask(REG_MF_CW_LEN3(mfdec_id), 0x007F , BMASK(11:0));
    HAL_Write2ByteMask(REG_MF_CW_LEN4(mfdec_id), 0x0DFE , BMASK(11:0));
    HAL_Write2ByteMask(REG_MF_CW_LEN5(mfdec_id), 0x09FE , BMASK(11:0));

    for (u8index=0;u8index< MFEDC_TABLE_INDEX_NUM; u8index++)
    {
        HAL_Write2ByteMask(REG_MF_CW_BASE(mfdec_id), MFDec_HUFF_Table[u8index][0] , BMASK(15:0));
        HAL_Write2ByteMask(REG_MF_CW_MAX(mfdec_id), MFDec_HUFF_Table[u8index][1] , BMASK(15:0));
        HAL_Write2ByteMask(REG_MF_SYMB_BASE(mfdec_id), MFDec_HUFF_Table[u8index][2] , BMASK(15:0));
        HAL_Write2ByteMask(REG_MF_HUF_TAB(mfdec_id), MFDec_HUFF_Table[u8index][3] , BMASK(15:0));
    }
    //HAL_Write2ByteMask(REG_MF_BASIC_H, 0x0000 , BMASK(15:0));
}
