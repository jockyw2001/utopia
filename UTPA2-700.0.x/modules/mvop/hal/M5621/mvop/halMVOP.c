...
void HAL_MVOP_SetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion )
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
		...
		if(pparam->bYUV422 || (g_pHalMVOPCtx->eMainRgbFmt != E_MVOP_RGB_NONE))
        {
...
{
    UNUSED(bEnable);
    _bEnable4k2kClk = false;
    MVOP_PRINTF("[Warning][%s] mustang not support 4k timing\n",__FUNCTION__);
}
...
#endif
    MVOP_PRINTF("[Warning][%s] mustang not support 2p mode\n",__FUNCTION__);
}
...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    if(((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
    {
        if(g_pHalMVOPCtx->bMirrorModeVer)
        {
            g_pHalMVOPCtx->bNewVSyncMode = FALSE;
            HAL_WriteRegBit(VOP_CTRL1, 1, VOP_MVD_VS_SEL); // 4 lins forwarding; from maserati
        }
        else
        {
            g_pHalMVOPCtx->bNewVSyncMode = TRUE;
...
MS_BOOL HAL_MVOP_Set420BWSaveMode(MS_BOOL bEnable)
{
    ...
    bValue = (g_pHalMVOPCtx->b3DLRAltSBSOutput || g_pHalMVOPCtx->b3DLRAltOutput || g_pHalMVOPCtx->bIs422 || (!(g_pHalMVOPCtx->eMainRgbFmt == E_MVOP_RGB_NONE)));
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECOVersion)
{
    ...
    case E_MVOP_DEV_1:
    {
        ...
        HAL_WriteRegBit(SUB_REG(VOP_REG_DUMMY), 1, VOP_32x32_WB); //32x32 from wb, need to set after HAL_MVOP_SubSet420BWSaveMode for firing same double buffer register.
...
void HAL_MVOP_SetByTiming(MVOP_DevID eID, MVOP_VidStat *pstVideoStatus)
{
    MVOP_DBG(MVOP_PRINTF("[MVOP][DBG] HAL_MVOP_SetByTiming\n");)
    switch(eID)
    {
        case E_MVOP_DEV_0:
            if(pstVideoStatus->u16HorSize > 2048)
            {
                //bw saving line buffer only 2048
                HAL_WriteRegBit(VOP_REG_DUMMY, 0, VOP_420_BW_SAVE);
                HAL_WriteRegBit(VOP_REG_BW_SAVE, 0, VOP_420_BW_SAVE_EX);
                HAL_MVOP_LoadReg();
            }
        ...
        case E_MVOP_DEV_1:
            ...
            break;
        default:
            MVOP_PRINTF("[MVOP][ERR] error input DevID\n");
            break;
    }
}
...
void HAL_MVOP_SetCropforXC(MVOP_DevID eID, MVOP_XCGetCrop* stXCCrop, MS_U16 u16Width, MS_U16 u16Height)
{
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            ...
            {
                MVOP_PRINTF("[Warning] crop x size or start may out of boundary.");
            }

            if((stXCCrop->u16YStart + stXCCrop->u16YSize) > u16Height)
            {
                MVOP_PRINTF("[Warning] crop y size or start may out of boundary.");
            }
...
void HAL_MVOP_SetMiuSaveArea(MVOP_DevID eID ,MVOP_MIUReadPrct stMiuSave)
{
    ...
    {
    case E_MVOP_DEV_0:
        HAL_WriteRegBit(VOP_REG_MIU_LIMIT, stMiuSave.bEnable, VOP_LIMIT_EN);
        if(stMiuSave.u8MIU == 0)
        {
            HAL_Write2Byte(VOP_REG_MIU0_MIN_ADDR_L, stMiuSave.u32ProtectStart);
            HAL_Write2Byte(VOP_REG_MIU0_MIN_ADDR_H, stMiuSave.u32ProtectStart >> 8);
            HAL_Write2Byte(VOP_REG_MIU0_MAX_ADDR_L, stMiuSave.u32ProtectEnd);
            HAL_Write2Byte(VOP_REG_MIU0_MAX_ADDR_H, stMiuSave.u32ProtectEnd >> 8);
        }
        else
        {
            HAL_Write2Byte(VOP_REG_MIU1_MIN_ADDR_L, stMiuSave.u32ProtectStart);
            HAL_Write2Byte(VOP_REG_MIU1_MIN_ADDR_H, stMiuSave.u32ProtectStart >> 8);
            HAL_Write2Byte(VOP_REG_MIU1_MAX_ADDR_L, stMiuSave.u32ProtectEnd);
            HAL_Write2Byte(VOP_REG_MIU1_MAX_ADDR_H, stMiuSave.u32ProtectEnd >> 8);
        }
        break;
#if MVOP_SUPPORT_SUB
    case E_MVOP_DEV_1:
        HAL_WriteRegBit(SUB_REG(VOP_REG_MIU_LIMIT), stMiuSave.bEnable, VOP_LIMIT_EN);
        if(stMiuSave.u8MIU == 0)
        {
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU0_MIN_ADDR_L), stMiuSave.u32ProtectStart);
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU0_MIN_ADDR_H), stMiuSave.u32ProtectStart >> 8);
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU0_MAX_ADDR_L), stMiuSave.u32ProtectEnd);
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU0_MAX_ADDR_H), stMiuSave.u32ProtectEnd >> 8);
        }
        else
        {
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU1_MIN_ADDR_L), stMiuSave.u32ProtectStart);
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU1_MIN_ADDR_H), stMiuSave.u32ProtectStart >> 8);
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU1_MAX_ADDR_L), stMiuSave.u32ProtectEnd);
            HAL_Write2Byte(SUB_REG(VOP_REG_MIU1_MAX_ADDR_H), stMiuSave.u32ProtectEnd >> 8);
        }
        break;
#endif
    default:
...
void HAL_MVOP_SubSetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion )
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        if(pparam->bYUV422 || (g_pHalMVOPCtx->eSubRgbFmt != E_MVOP_RGB_NONE))
        {
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    if(((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
    {
        ...
        {
            g_pHalMVOPCtx->bSubNewVSyncMode = FALSE;
            HAL_WriteRegBit(SUB_REG(VOP_CTRL1), 1, VOP_MVD_VS_SEL); // 4 lins forwarding; from maserati
        }
        else
        {
...
MS_BOOL HAL_MVOP_SubSet420BWSaveMode(MS_BOOL bEnable, MS_U16 u16ECOVersion)
{
    ...
    //hw limtation: 3DLA/3DSBS/422/p mode in, i mode out/i mode in, p mode out(only need to check in MCU mode)
    bValue = (g_pHalMVOPCtx->bSub3DLRAltSBSOutput || g_pHalMVOPCtx->bSub3DLRAltOutput || g_pHalMVOPCtx->bSubIs422 || (!(g_pHalMVOPCtx->eSubRgbFmt == E_MVOP_RGB_NONE)) );
