...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
	...
	if(((ptiming->u16V_TotalCount >= 2160) || (ptiming->u16H_TotalCount >= 3840)) && (ptiming->u8Framerate >= 25))
    {
        g_pHalMVOPCtx->bNewVSyncMode = TRUE;
    }
    // patch for manhattan + FRC
    if(((ptiming->u16V_TotalCount >= 1080) || (ptiming->u16H_TotalCount >= 1920)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
    {
        g_pHalMVOPCtx->bNewVSyncMode = TRUE;
...
void HAL_MVOP_SetJpegHardwireMode(void)
{
    ...
    // MJPEG use WISHBONE(R2) interface
    //HAL_WriteRegBit(VOP_INPUT_SWITCH0, 1, VOP_R2_WISHBONE);
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECOVersion)
{
    ...
    case E_MVOP_DEV_0:
    {
        ...
        HAL_WriteByteMask(VOP_REG_MFDEC_0_L, 0xF0, VOP_MF1_BURST|VOP_MF0_BURST|VOP_MFDEC_EN);
    ...
    case E_MVOP_DEV_1:
    {
        ...
        // Setting MF burst len
        HAL_WriteByteMask(SUB_REG(VOP_REG_MFDEC_0_L), 0xF0, VOP_MF1_BURST|VOP_MF0_BURST|VOP_MFDEC_EN);
        // Only for monaco: Disable mfdec setting from wb
...
#endif
#if 1 //sw patch, for monet/manhattan
...
MS_U16 HAL_MVOP_ReadBank(MVOP_DevID eID ,MS_U16 u16Length)
{
	...
	if(eID == E_MVOP_DEV_0)
	{
		u16Reg = HAL_Read2Byte(MVOP_REG_BASE + u16Length);
	}
#ifdef MVOP_SUPPORT_SUB
	else if(eID == E_MVOP_DEV_1)
	{
		u16Reg = HAL_Read2Byte(SUB_REG(MVOP_REG_BASE + u16Length));
	}
#endif
	return u16Reg;
}
...
void HAL_MVOP_WriteBank(MVOP_DevID eID ,MS_U16 u16Length,MS_U16 u16Data)
{
	if(eID == E_MVOP_DEV_0)
	{
		HAL_Write2Byte(MVOP_REG_BASE + u16Length,u16Data);
	}
#ifdef MVOP_SUPPORT_SUB
	else if(eID == E_MVOP_DEV_1)
	{
		HAL_Write2Byte(SUB_REG(MVOP_REG_BASE + u16Length),u16Data);
	}
#endif
}
...
MS_U16 HAL_MVOP_ReadClkBank(MS_U16 u16Length)
{
	...
	if(u16Length < 2)
    {
		u16Reg = HAL_Read2Byte(CHIP_REG_BASE + REG_STC0_CW_L + u16Length);
    }
    // dc clk
    else if(u16Length < 6)
    {
        u16Length -= 2;
		u16Reg = HAL_Read2Byte(CHIP_REG_BASE + REG_CKG_DC0 + u16Length);
    }
    // freerun/sync clk
    else
    {
        u16Length -= 6;
		u16Reg = HAL_Read2Byte(CHIP_REG_BASE + REG_UPDATE_DC0_CW + u16Length);
    }
...
void HAL_MVOP_WriteClkBank(MS_U16 u16Length,MS_U16 u16Data)
{
	...
	HAL_Write2Byte(CHIP_REG_BASE + u16Length,u16Data);
    // stc cw
	if(u16Length < 2)
    {
		HAL_Write2Byte((CHIP_REG_BASE + REG_STC0_CW_L + u16Length), u16Data);
    }
    // dc clk
    else if(u16Length < 6)
    {
        u16Length -= 2;
		HAL_Write2Byte((CHIP_REG_BASE + REG_CKG_DC0 + u16Length), u16Data);
    }
    // freerun/sync clk
    else
    {
        u16Length -= 6;
		HAL_Write2Byte((CHIP_REG_BASE + REG_UPDATE_DC0_CW + u16Length), u16Data);
    }
