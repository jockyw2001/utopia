...
typedef struct
{
    ...
    MS_U16 eInterlaceType;//tmp remove enum
    MS_BOOL bIsXcTrig;
    MS_BOOL bIs265DV;
#if MVOP_SUPPORT_SUB
    ...
    MS_U16 eSubInterlaceType;//tmp remove enum
...
void _HAL_MVOP_InitVarCtx(void)
{
    ...
    g_pHalMVOPCtx->eInterlaceType = 0x0; //tmp remove enum
...
void _HAL_MVOP_SubInitVarCtx(void)
{
    ...
    g_pHalMVOPCtx->eSubInterlaceType = 0x0;//tmp remove enum
...
void HAL_MVOP_Enable(MS_BOOL bEnable, MS_U8 u8Framerate)
{
    ...
    else
    {
        ...
        {
            HAL_Write2Byte(VOP_BF_VS_MVD, 0x200); //CL: 1359300 from manhattan
...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
        ...
        && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        ...
        else
        {
            ...
            {
                g_pHalMVOPCtx->u16VsyncLines = 0x24; //mazda(branch already): 0x64, but roku need to set 0x24 for fb overwrite
...
void HAL_MVOP_SetInterlaceType(MVOP_DevID eDevID, MS_U16 u16ECONumber, MS_U8 u8Interlace)
{
    ...
    case 0x1:
		...
		{
            g_pHalMVOPCtx->eInterlaceType = 0x02;//tmp remove enum
		}
		else
		{
		    g_pHalMVOPCtx->eInterlaceType = 0x01;//tmp remove enum
		}
        break;
    case 0x2:
		if(g_pHalMVOPCtx->bIsH265)
    ...
    {
    case E_MVOP_DEV_0:
        if((g_pHalMVOPCtx->eInterlaceType == 0x02) && (g_pHalMVOPCtx->bIsH265))//tmp remove enum
        {
            HAL_WriteRegBit(VOP_INT_TYPE, 1, VOP_EVD_INT_SEP);
        }
		else if((g_pHalMVOPCtx->eInterlaceType == 0x01) && (g_pHalMVOPCtx->bIsH265))//tmp remove enum
		{
            HAL_WriteRegBit(VOP_INT_TYPE, 0, VOP_EVD_INT_SEP);
			HAL_WriteRegBit(VOP_MIRROR_CFG, 0, VOP_FIELD_FROM_ADDR);
        }
...
#if MVOP_SUPPORT_SUB
    case E_MVOP_DEV_1:
        if((g_pHalMVOPCtx->eSubInterlaceType == 0x02) && (g_pHalMVOPCtx->bIsH265)) //tmp remove enum
        {
            HAL_WriteRegBit(SUB_REG(VOP_INT_TYPE), 1, VOP_EVD_INT_SEP);
        }
		else if((g_pHalMVOPCtx->eSubInterlaceType == 0x01) && (g_pHalMVOPCtx->bIsH265))//tmp remove enum
		{
            HAL_WriteRegBit(SUB_REG(VOP_INT_TYPE), 0, VOP_EVD_INT_SEP);
...
#ifdef UFO_MVOP_SET_2P
void HAL_MVOP_SetMVOP2P(MVOP_DevID eID, MS_BOOL bEnable)
{
    switch(eID)
    {
        case E_MVOP_DEV_0:
...
void HAL_MVOP_SubEnable(MS_BOOL bEnable, MS_U8 u8Framerate)
{
    ...
    else
    {
        ...
        if(!g_pHalMVOPCtx->bIs265DV)
        {
            HAL_Write2Byte(SUB_REG(VOP_BF_VS_MVD), 0x200); //CL: 1359300 from manhattan
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
            ...
            && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        ...
        else
        {
            ...
            {
                g_pHalMVOPCtx->u16SubVsyncLines = 0x24; //mazda(branch already): 0x64, but roku need to set 0x24 for fb overwrite
            }
