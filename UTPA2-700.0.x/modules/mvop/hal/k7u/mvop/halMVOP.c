...
#define HAL_Write2ByteMask( u32Reg, u16Val, u16Msk )                                      \
    MST_MACRO_START                                                                     \
    RIU16[u32Reg] = (RIU16[u32Reg] & ~(u16Msk)) | ((u16Val) & (u16Msk));              \
    MST_MACRO_END
...
void HAL_MVOP_SetMIUReqMask(MS_BOOL bEnable)
{
    ...
    eMIUClientID eClientID = MVOP_MIU_CLIENT_MAIN;
...
void HAL_MVOP_SetFrequency(HALMVOPFREQUENCY enFrequency)
{
    ...
    switch(enFrequency)
    {
        ...
        case HALMVOP_320MHZ:
            ...
            break;
        case HALMVOP_345MHZ:
            HAL_WriteByteMask(REG_CKG_DC0, CKG_DC0_345MHZ, CKG_DC0_MASK);
            break;
        case HALMVOP_123MHZ:
...
void HAL_MVOP_SetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion)
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        if(g_pHalMVOPCtx->bIs422 || (g_pHalMVOPCtx->eMainRgbFmt != E_MVOP_RGB_NONE))
        {
            HAL_WriteRegBit(VOP_REG_DUMMY, FALSE, VOP_420_BW_SAVE);
            HAL_WriteRegBit(VOP_REG_BW_SAVE, FALSE, VOP_420_BW_SAVE_EX);
        }
        ...
        // HSize
        MS_U16 u16HSize = ALIGN_UPTO_2(pparam->u16HSize);
...
#if SUPPORT_BACK_DRI_FWD
        if(g_pHalMVOPCtx->bIsXcTrig)
        {
            HAL_WriteRegBit(VOP_REG_SINGLE_3D_H, 1, VOP_MVD_ADV_VS_MD);
        }
#endif
...
void HAL_MVOP_SetDCSRAMClk(MS_U8 clkNum, MS_BOOL bEnable)
{
    ...
    {
        //set hw select
        HAL_WriteByteMask(REG_CKG_DC_SRAM2, 0x0, VOP_SRAM0_CLK_SEL);//mfdec0
        HAL_WriteByteMask(REG_CKG_DC_SRAM2, 0x0, VOP_SRAM1_CLK_SEL);//mfdec1

        //default hw select
        HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x0, CKG_DC0_SRAM_MASK);//mfdec0
        HAL_WriteByteMask(REG_CKG_DC_SRAM, 0x0, CKG_DC1_SRAM_MASK);//mfdec1
    }
}
...
void HAL_MVOP_SetFBDecClk(MVOP_DevID eID, MS_BOOL bEnable)
{
    if(bEnable == FALSE)
    {
        //Not Disable clock(DIP may use)
    }
    else
    {
        HAL_WriteRegBit(REG_CKG2_FBDEC_L, 0, CKG_FBDEC_GATED);
        HAL_WriteRegBit(REG_CKG2_FBDEC_L, 0, CKG_FBDEC1_GATED);
    }
    //tmp for maxim
    HAL_WriteByteMask(REG_CKG2_FBDEC_L, CKG_FBDEC_432MHZ, CKG_FBDEC_MASK);//default 432MHz
    HAL_WriteByteMask(REG_CKG2_FBDEC_L, CKG_FBDEC1_432MHZ, CKG_FBDEC1_MASK);//default 432MHz
    HAL_WriteByteMask(MFDEC_REG_PW_SAVE, MFDEC_REG_CKG_MASK, MFDEC_REG_CKG_MASK);//power auto saving
}
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
            ...
            if(stEVDInput->eEVDBit[Y_INFO] || stEVDInput->eEVDBit[UV_INFO])
            {
                ...
                HAL_WriteRegBit(VOP_REG_422_OUT_EN, VOP_10TO8_EN, VOP_10TO8_EN);
                HAL_WriteByteMask(VOP_REG_422_OUT_EN, BIT2, VOP_10TO8); //10 to 8 random mode
        ...
        case E_MVOP_DEV_1:
            ...
            if(stEVDInput->eEVDBit[Y_INFO] || stEVDInput->eEVDBit[UV_INFO])
            {
                ...
                HAL_WriteRegBit(SUB_REG(VOP_REG_422_OUT_EN), VOP_10TO8_EN, VOP_10TO8_EN);
                HAL_WriteByteMask(SUB_REG(VOP_REG_422_OUT_EN), BIT2, VOP_10TO8);
                MVOP_DBG(MVOP_PRINTF("[Debug] 10 bits display\n"););
            }
            else
            {
...
MVOP_HSMode HAL_MVOP_GetHandShakeMode(MVOP_DevID eID)
{
    ...
    {
        case E_MVOP_DEV_0:
            if(HAL_ReadRegBit(VOP_REG_HS_OUTPUT, BIT0))
            {
        ...
        case E_MVOP_DEV_1:
#if MVOP_SUPPORT_SUB
            if(HAL_ReadRegBit(SUB_REG(VOP_REG_HS_OUTPUT), BIT0))
            {
                eRet = E_MVOP_HS_ENABLE;
            }
            else
...
void HAL_MVOP_SetTimingFromXC(MVOP_DevID eID, MS_BOOL bEnable)
{
    switch(eID)
    {
        ...
        case E_MVOP_DEV_0:
        {
            ...
            if(bEnable)
            {
                HAL_WriteByteMask(VOP_REG_HS_OUTPUT, 0x06, VOP_TRIG_FROM_XC|VOP_VSUNC_FROM_XC);
                //HAL_WriteRegBit(VOP_CTRL1, 0, BIT4); // disable inverse the field to IP - not in kano

            }
            else
            {
                HAL_WriteByteMask(VOP_REG_HS_OUTPUT, 0x00, VOP_TRIG_FROM_XC|VOP_VSUNC_FROM_XC);
        ...
        case E_MVOP_DEV_1:
        {
            ...
            if(bEnable)
            {
                HAL_WriteByteMask(SUB_REG(VOP_REG_HS_OUTPUT), 0x06, VOP_TRIG_FROM_XC|VOP_VSUNC_FROM_XC);
            }
            else
            {
                HAL_WriteByteMask(SUB_REG(VOP_REG_HS_OUTPUT), 0x00, VOP_TRIG_FROM_XC|VOP_VSUNC_FROM_XC);
            }
            HAL_MVOP_SubLoadReg();
#endif
            break;
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID, MS_U16 u16ECONumber)
{
    ...
    //apple for debug
    HAL_WriteRegBit(VOP_REG_HS_OUTPUT, 1, VOP_HS_MODE);
    ...
    HAL_WriteRegBit(VOP_UV_SHIFT, 0x0C, VOP_GCLK_MIU_ON|VOP_GCLK_VCLK_ON);// for auto saving power, but 0x30[2] =1 can not yc seperate before. checked.
    g_pHalMVOPCtx->eInterlaceType = E_MVOP_PRO;
...
#endif
#if SUPPORT_DDR4_REMAP
    HAL_WriteByteMask(VOP_REG_DDR4_REMAP, 0x60, VOP_MSB_REMAP|VOP_LSB_REMAP);
#endif
    HAL_WriteRegBit(VOP_REG_422_OUT_EN, 0, VOP_10TO8_EN);
    HAL_WriteByteMask(VOP_REG_422_OUT_EN, 0, VOP_10TO8);
#if SUPPORT_BACK_DRI_FWD
     HAL_WriteRegBit(VOP_REG_SINGLE_3D_H, 0, VOP_MVD_ADV_VS_MD);
#endif
    ...
    case E_MVOP_DEV_1:
    {
        ...
        HAL_WriteRegBit(SUB_REG(VOP_REG_HS_OUTPUT), 1, VOP_HS_MODE);
        ...
        HAL_WriteRegBit(SUB_REG(VOP_UV_SHIFT), 0x0C, VOP_GCLK_MIU_ON|VOP_GCLK_VCLK_ON);// for auto saving power, but 0x30[2] =1 can not yc seperate before. checked.
        g_pHalMVOPCtx->eSubInterlaceType = E_MVOP_PRO;
...
#endif
        HAL_WriteRegBit(SUB_REG(VOP_REG_422_OUT_EN), 0, VOP_10TO8_EN);
        HAL_WriteByteMask(SUB_REG(VOP_REG_422_OUT_EN), 0, VOP_10TO8);
#if SUPPORT_BACK_DRI_FWD
        HAL_WriteRegBit(SUB_REG(VOP_REG_SINGLE_3D_H), 0, VOP_MVD_ADV_VS_MD);
#endif
    ...
    switch(eID)
    {
        case E_MVOP_DEV_0:
        {
            if(!((bEnable == FALSE) && (g_pHalMVOPCtx->bIsHS == FALSE)) && !(g_pHalMVOPCtx->bIs265DV)) //prevent switch VOP_CTRL0 (mvop enable), for MHEG5 verify
            {
                ...
                if(g_pHalMVOPCtx->bIs265DV)
                {
                    HAL_WriteRegBit(VOP_REG_4K2K_2P, bEnable, VOP_TRIG_REFER_VB_END); //need verify
                }
                HAL_WriteRegBit(VOP_CTRL0, 1, BIT0);
                HAL_MVOP_LoadReg();
            }
        ...
        case E_MVOP_DEV_1:
        {
            ...
            if(!((bEnable == FALSE) && (g_pHalMVOPCtx->bSubIsHS == FALSE)) && !(g_pHalMVOPCtx->bIs265DV)) //prevent switch VOP_CTRL0 (mvop enable), for MHEG5 verify
            {
                ...
                if(g_pHalMVOPCtx->bIs265DV)
                {
                    HAL_WriteRegBit(SUB_REG(VOP_REG_4K2K_2P), bEnable, VOP_TRIG_REFER_VB_END); //need verify
                }
...
{
    HALMVOPFREQUENCY eRet = HALMVOP_320MHZ;

    if(u32MVOPClk > HALMVOP_345MHZ*2)
    {
        MVOP_PRINTF("[Warning] MVOP clock out of spec(640MHz), set freerun mode.\n");
        eRet = HALMVOP_FREERUNMODE;
    }
    else if(u32MVOPClk <= HALMVOP_345MHZ*2 && u32MVOPClk > HALMVOP_345MHZ)
    {
        eRet = u32MVOPClk;
    }
    else if(u32MVOPClk <= HALMVOP_345MHZ && u32MVOPClk > HALMVOP_320MHZ)
    {
        eRet = u32MVOPClk;
    }
...
{
    HAL_WriteRegBit(VOP_REG_422_OUT_EN, bEnable, BIT0);
    HAL_WriteRegBit(VOP_REG_HS_OUTPUT, bEnable, VOP_HS_MODE);
    HAL_WriteRegBit(VOP_CTRL0, bEnable, BIT4);
    HAL_WriteRegBit(VOP_MIRROR_CFG_HI, bEnable, VOP_HK_MASK); //bk background
    if(bEnable)
    {
        HAL_WriteRegBit(VOP_REG_422_OUT_EN, 0, VOP_FRAME_RST); // for Dolby crop
    }
    g_pHalMVOPCtx->bIsHS = bEnable;
}
...
void HAL_MVOP_SubSetMIUReqMask(MS_BOOL bEnable)
{
    ...
    eMIUClientID eClientID = MVOP_MIU_CLIENT_SUB;
...
void HAL_MVOP_SubSetFrequency(HALMVOPFREQUENCY enFrequency)
{
    ...
    switch(enFrequency)
    {
        ...
        case HALMVOP_320MHZ:
            ...
            break;
        case HALMVOP_345MHZ:
            HAL_WriteByteMask(REG_CKG_SUB_DC0, CKG_SUB_DC0_345MHZ, CKG_SUB_DC0_MASK);
            break;
        case HALMVOP_123MHZ:
...
void HAL_MVOP_SubSetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion )
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        if(g_pHalMVOPCtx->bSubIs422 || (g_pHalMVOPCtx->eSubRgbFmt != E_MVOP_RGB_NONE))
        {
            HAL_WriteRegBit(SUB_REG(VOP_REG_DUMMY), FALSE, VOP_420_BW_SAVE);
            HAL_WriteRegBit(SUB_REG(VOP_REG_BW_SAVE), FALSE, VOP_420_BW_SAVE_EX);
        }
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    if (g_pHalMVOPCtx->bSubNewVSyncMode)
    {
        ...
        {
            HAL_WriteRegBit(SUB_REG(VOP_REG_SINGLE_3D_H), 1, VOP_MVD_ADV_VS_MD);
        }
#endif
...
{
    HAL_WriteRegBit(SUB_REG(VOP_REG_422_OUT_EN), bEnable, BIT0);
    HAL_WriteRegBit(SUB_REG(VOP_REG_HS_OUTPUT), bEnable, VOP_HS_MODE);
    HAL_WriteRegBit(SUB_REG(VOP_CTRL0), bEnable, BIT4);
    HAL_WriteRegBit(SUB_REG(VOP_MIRROR_CFG_HI), bEnable, VOP_HK_MASK); //bk background
    if(bEnable)
    {
        HAL_WriteRegBit(SUB_REG(VOP_REG_422_OUT_EN), 0, VOP_FRAME_RST); // for Dolby crop
    }
    g_pHalMVOPCtx->bSubIsHS = bEnable;
    if(g_pHalMVOPCtx->bMirrorModeVer)
    {
