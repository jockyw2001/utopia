...
GOP_CHIP_PROPERTY g_GopChipPro =
{
    ...
    .GOP_Zorder_Mux_Offset =    {0x3, 0x3, 0x3, 0x3, 0x3},
    ...
    .bGOPVscalePipeDelay  =     {FALSE, FALSE, FALSE, TRUE, TRUE, FALSE},
...
GOP_Result HAL_GOP_SetGOPEnable2Mode1(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 gopNum, MS_BOOL bEnable)
{
    ...
    */
    ...
    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_MUX, &muxValue);
      if (gopNum== (muxValue & GOP_MUX0_MASK))    //enable mux0 to SC
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_L, bEnable?(GOP_BIT5):(~GOP_BIT5), GOP_BIT5);
    }
    else if (gopNum== ((muxValue & GOP_MUX1_MASK)>>(GOP_MUX_SHIFT*1))) //enable mux1
    {
        //printf("");
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_L, bEnable?(GOP_BIT13):(~GOP_BIT13), GOP_BIT13);
    }
    else if (gopNum== ((muxValue & GOP_MUX2_MASK)>>(GOP_MUX_SHIFT*2))) //enable mux2
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_H, bEnable?(GOP_BIT5):(~GOP_BIT5), GOP_BIT5);
    }
    else if (gopNum== ((muxValue & GOP_MUX3_MASK)>>(GOP_MUX_SHIFT*3))) //enable mux3
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_VOP2BLENDING_H, bEnable?(GOP_BIT13):(~GOP_BIT13), GOP_BIT13);
    }
    else
    {
        return GOP_FAIL;
    ...
    switch (u16Regval&GOP_DST_MASK)
    {
        case 0:
            *pGopDst = E_DRV_GOP_DST_IP0;
            ...
            break;
        case 1:
            HAL_GOP_GetMixerDst(pGOPHalLocal,pGopDst);
            ret = GOP_SUCCESS;
            break;
        case 2:
