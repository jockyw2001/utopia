...
void GE2_WriteReg(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_U16 addr, MS_U16 value)
{
    ...
    {
        printf("[%s][%d] Reg Index [%x]is out of GE_TABLE_REGNUM [0x%lx]range!!!!\n",__FUNCTION__,__LINE__, addr, GE_TABLE_REGNUM);
    }
    GE_WaitCmdQAvail(pGEHalLocal, GE_CMD_SIZE);
    GE2_REG(addr)= value;
    return;
...
void GE_Init(GE_CTX_HAL_LOCAL *pGEHalLocal, GE_Config *cfg)
{
    ...
    {
#if GE_CMDQ_ENABLE
            GE_WriteReg(pGEHalLocal, REG_GE_CFG, 0);
            GE_WriteReg(pGEHalLocal, REG_GE_CFG, GE_CFG_CMDQ); // enable command queue
#endif

        GE_WriteReg(pGEHalLocal, REG_GE_EN, 0);
        GE_WriteReg(pGEHalLocal, REG_GE_EN, GE_EN_GE);
        ...
        if (cfg->u32VCmdQSize >= GE_VCMDQ_SIZE_MIN)
        {
            ...
            GE_WriteReg(pGEHalLocal, REG_GE_CFG, 0);
            GE_WriteReg(pGEHalLocal, REG_GE_CFG, GE_CFG_CMDQ | GE_CFG_VCMDQ);

            GE_SetVQBufMIUId(pGEHalLocal, _GFXAPI_MIU_ID(PhyVQAddr));
            PhyVQAddr = GE_ConvertAPIAddr2HAL(pGEHalLocal, _GFXAPI_MIU_ID(PhyVQAddr), _GFXAPI_PHYS_ADDR_IN_MIU(PhyVQAddr));

            GE_WriteReg(pGEHalLocal, REG_GE_VCMDQ_BASE_L, (MS_U32)(PhyVQAddr) & 0xFFFF);
            GE_WriteReg(pGEHalLocal, REG_GE_VCMDQ_BASE_H, (MS_U32)(PhyVQAddr) >> 16);
            GE_WriteReg(pGEHalLocal, REG_GE_VCMDQ_SIZE,  GE_MapVQ2Reg(cfg->u32VCmdQSize));
        }
...
GE_Result GE_GetTLBSRCADDR(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_PHY* addr)
{
    ...
    miu= ((GE2_ReadReg(pGEHalLocal, REG_GE_SRC_TLB_BASE_H)&0x8000)>>15)|((GE2_ReadReg(pGEHalLocal, REG_GE_TLB_BASE_MIU_H)&GE_SB_TLB_SRC_MIU_SEL_H)>>12);
...
GE_Result GE_GetTLBDSTADDR(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_PHY* addr)
{
    ...
    miu= ((GE2_ReadReg(pGEHalLocal, REG_GE_DST_TLB_BASE_H)&0x8000)>>15)|((GE2_ReadReg(pGEHalLocal, REG_GE_TLB_BASE_MIU_H)&GE_SB_TLB_DST_MIU_SEL_H)>>13);
...
GE_Result GE_SetTLBSrcBaseAddr(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_PHY addr)
{
    ...
    Phyoffset=_GFXAPI_PHYS_ADDR_IN_MIU(addr);

    if(u8miu==0)
    {
        GE2_WriteReg(pGEHalLocal, REG_GE_SRC_TLB_BASE_L, Phyoffset);
        GE2_WriteReg(pGEHalLocal, REG_GE_SRC_TLB_BASE_H, (Phyoffset&0x7fff0000)>>16);
    ...
    {
        GE2_WriteReg(pGEHalLocal, REG_GE_SRC_TLB_BASE_L, Phyoffset);
        GE2_WriteReg(pGEHalLocal, REG_GE_SRC_TLB_BASE_H, ((Phyoffset&0x7fff0000)>>16)|(u8miu<<15));
        regval= GE2_ReadReg(pGEHalLocal, REG_GE_TLB_BASE_MIU_H)&(~GE_SB_TLB_SRC_MIU_SEL_H);
        GE2_WriteReg(pGEHalLocal, REG_GE_TLB_BASE_MIU_H, regval);
    }
    else
...
GE_Result GE_SetTLBDstBaseAddr(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_PHY addr)
{
    ...
    if(u8miu==0)
    {
        GE2_WriteReg(pGEHalLocal, REG_GE_DST_TLB_BASE_L, Phyoffset);
        GE2_WriteReg(pGEHalLocal, REG_GE_DST_TLB_BASE_H, ((Phyoffset&0x7fff0000)>>16));
    ...
    {
        GE2_WriteReg(pGEHalLocal, REG_GE_DST_TLB_BASE_L, Phyoffset);
        GE2_WriteReg(pGEHalLocal, REG_GE_DST_TLB_BASE_H, ((Phyoffset&0x7fff0000)>>16)|(u8miu<<15));
        regval= GE2_ReadReg(pGEHalLocal, REG_GE_TLB_BASE_MIU_H)&(~GE_SB_TLB_DST_MIU_SEL_H);
        GE2_WriteReg(pGEHalLocal, REG_GE_TLB_BASE_MIU_H, regval);
    }
    else
