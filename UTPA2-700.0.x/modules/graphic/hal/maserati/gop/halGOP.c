...
GOP_CHIP_PROPERTY g_GopChipPro =
{
    ...
    .bhastilemode  =            TRUE,
    .bInitNotEnableGOPToSC =    TRUE,  //For Uranus4 mux init setting, enable GOP to SC in GOP init would cause problem
    ...
    .bAFBC_Support=             {FALSE, FALSE, FALSE, FALSE, FALSE, FALSE}, // disable GOP before AFBC patch implement
    ...
    .GOP_MUX_Delta  =           0x1,
    .GOP_Mux_Offset =           {0x0, 0x42, 0x36, 0x3C, 0x48},
...
MS_U32 _getLayerFromOpmux(Gop_MuxSel eGopMux)
{
    MS_U32 u32Layer = 0xFFFF;
    int i=0;

    for (i=0; i<MAX_GOP_MUX; i++)
    {
        if (g_GopChipPro.GOP_MapLayer2Mux[i] == eGopMux)
        {
            u32Layer = i;
            break;
        }
...
void _SetDualOpMux(GOP_CTX_HAL_LOCAL*pGOPHalLocal, Gop_MuxSel eGopMux, MS_U8 u8GOPNum)
{
    MS_U8 u8_mainGop = INVAILD_GOP_NUM, u8_combinedGop = INVAILD_GOP_NUM;
    ...
    {
        // for GOP1+GOOP2
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_MUX4_4K120, 1 << (GOP_4K120_MUX_SHIFT*(eGopMux-E_GOP_DUALRATE_OP_MUX0)) , GOP_4K120_REGMUX_MASK<<(GOP_4K120_MUX_SHIFT*(eGopMux-E_GOP_DUALRATE_OP_MUX0)));
    }
    else if (u8_mainGop == E_GOP4)
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_MUX4_4K120, 2 << (GOP_4K120_MUX_SHIFT*(eGopMux-E_GOP_DUALRATE_OP_MUX0)) , GOP_4K120_REGMUX_MASK<<(GOP_4K120_MUX_SHIFT*(eGopMux-E_GOP_DUALRATE_OP_MUX0)));
    }
}

MS_BOOL _isAfbcFunctionEnabled(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8AfbcCore)
{
    MS_U16 reg_val = 0x0;
    HAL_GOP_Read16Reg(pGOPHalLocal, REG_AFBC_CORE_EN(u8AfbcCore), &reg_val);

    return (reg_val & GOP_BIT0) ? TRUE : FALSE;
}
...
GOP_Result HAL_GOP_SetGOPACKMask(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U16 u16GopMask)
{
    ...
    if( FALSE == Hal_get4K120CombinedGOP(pGOPHalLocal, (MS_U8)u16GopIdx, &u8_mainGop, &u8_combinedGop) )
    {
        GOP_H_ERR("Skipped [%s] Line %d !!", __FUNCTION__, __LINE__);
        return GOP_FAIL;
    }
    _GetBnkOfstByGop(u8_mainGop, &u32MainBnkOfst);
    HAL_GOP_Read16Reg(pGOPHalLocal, u32MainBnkOfst + GOP_4G_CTRL1, &u16GopDst);

    if (u16Mask0 & (GOP_BIT12 | GOP_BIT15))
    {
        if ((u16GopDst & 0xf) == 0xB) //E_DRV_GOP_DST_OP_DUAL_RATE
        {
...
void HAL_GOP_Init(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOPNum)
{
    ...
    if(g_GopChipPro.bAFBC_Support[u8GOPNum] ==TRUE)
    {
        ...
        HAL_GOP_Write16Reg(pGOPHalLocal, REG_AFBC_CORE_EN(0), GOP_BIT4, GOP_BIT4);           	//quick_preload_0
        HAL_GOP_Write16Reg(pGOPHalLocal, REG_AFBC_CORE_EN(1), GOP_BIT4, GOP_BIT4);           	//quick_preload_1
    }
...
void HAL_GOP_Write16Reg(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U32 u32addr, MS_U16 u16val, MS_U16 mask)
{
    ...
    switch (u32addr & 0xFF00)
    {
        case GOP_REG_BASE:
        {
            ...
            else if (bank==0x7000)// Manual Crop
            {
                direct_addr = MANUAL_CROP_REG_BASE + (u32addr & 0xFF);  //Direct_Base + addr_offset
                GOP_WRITE2BYTE((direct_addr&0xFFFFF), u16val);
                break;
            }
...
GOP_Result HAL_GOP_GWIN_SetDstPlane(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 GopNum, DRV_GOPDstType eDstType,MS_BOOL bOnlyCheck)
{
    ...
    */
    ...
    if(bOnlyCheck == FALSE)
    {
        ...
        if (E_DRV_GOP_DST_OP_DUAL_RATE == eDstType)
        {
            MS_U32 u32SubBnkOffSet=0;
            _GetBnkOfstByGop(u8_combinedGop, &u32SubBnkOffSet);
            HAL_GOP_Write16Reg(pGOPHalLocal, u32SubBnkOffSet+GOP_4G_CTRL1, u16RegVal, BMASK(GOP_BIT3:0));
            HAL_GOP_Write16Reg(pGOPHalLocal, u32SubBnkOffSet+GOP_4G_BW, GOP_DMA_ACCESS_HIGH_PRIORITY, GOP_REG_HW_MASK);
            HAL_GOP_Write16Reg(pGOPHalLocal, u32SubBnkOffSet+GOP_4G_BW, GOP_FIFO_BURST_ALL, GOP_FIFO_BURST_MASK);
            HAL_GOP_Write16Reg(pGOPHalLocal, u32SubBnkOffSet+GOP_4G_BW, GOP_FIFO_THRESHOLD, GOP_REG_LW_MASK);
        }
...
void HAL_GOP_GWIN_SetMUX(GOP_CTX_HAL_LOCAL*pGOPHalLocal, MS_U8 u8GOPNum, Gop_MuxSel eGopMux)
{
    ...
    switch(eGopMux)
    {
        ...
        case E_GOP_MUX4:
            ...
            // get layer from op mux
            MS_U32 u32Layer = _getLayerFromOpmux(eGopMux);
            if (u32Layer == 0xFFFF)
            {
                GOP_H_ERR("can't get layer from mux = %d !!", (MS_U16)eGopMux);
                break;
            }

            // set dual op mux
            _SetDualOpMux(pGOPHalLocal, g_GopChipPro.GOP_MapLayer2DualOpMux[u32Layer], u8GOPNum);
            break;

        case E_GOP_DUALRATE_OP_MUX0:
        case E_GOP_DUALRATE_OP_MUX1:
        case E_GOP_DUALRATE_OP_MUX2:
            _SetDualOpMux(pGOPHalLocal, eGopMux, u8GOPNum);
            break;
    ...
    case E_DRV_GOP_DST_OP_DUAL_RATE:
        {
            ...
            if (u8_mainGop == E_GOP0)
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOPCLK, 0, CKG_GOPG0_DISABLE_CLK_MASK);
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP3CLK, 0, CKG_GOPG3_DISABLE_CLK_MASK);
            ...
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP2CLK, 0, CKG_GOPG2_DISABLE_CLK_MASK);
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOPCLK, 0, CKG_GOPG1_DISABLE_CLK_MASK);
...
GOP_Result HAL_GOP_Set_MIU(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOP,MS_U8 miusel)
{
    ...
    if (pGOPHalLocal->pHALShared->GOP_Dst[u8_mainGop] == E_DRV_GOP_DST_OP_DUAL_RATE)
    {
        ...
        if(pGOPHalLocal->pGopChipPro->bInternalMIUSelect[u8GOP]==TRUE)
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_MIU_IP_SEL, 1<<mask_shift, 1<<mask_shift );//Set Control MIU select by GOP itself
...
GOP_Result HAL_GOP_GetIPInterlace(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_BOOL *bInterlace)
{
    ...
    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_SC_IP_MAIN_INTERLACE, &reg_val);
    if(reg_val & BIT(11))
        *bInterlace = TRUE;
    else
        *bInterlace = FALSE;
    ...
    )
{
    ...
    MS_U16 u16Gop_stretch_vstart = 0x0;
    ...
    else
    {
        ...
        HAL_GOP_Read16Reg(pGOPHalLocal, u32MainBankOffSet + GOP_4G_STRCH_VSTR, &u16Gop_stretch_vstart);
        ...
        // crop left part
        crop_hstart = 0x0;
        crop_hend = (pGOPHalLocal->pHALShared->u16GopSplitMode_LRWIDTH[u8_mainGop] != 0) ?
                    ((pGOPHalLocal->pHALShared->u16GopSplitMode_LRWIDTH[u8_mainGop] >> 1) - 1) : 0x77F;
        crop_vstart = 0x0;
        crop_vend = (u16VScalingRatio != 0) ? ((SCALING_MULITPLIER * (MS_U32)gop_stretch_height) / u16VScalingRatio + u16Gop_stretch_vstart) : 0x870;
        ...
        // crop right part
        crop_hstart = (pGOPHalLocal->pHALShared->u16GopSplitMode_LRWIDTH[u8_mainGop] != 0) ?
                      (pGOPHalLocal->pHALShared->u16GopSplitMode_LRWIDTH[u8_mainGop] >> 1): 0x780;
        crop_hend = (pGOPHalLocal->pHALShared->u16GopSplitMode_LRWIDTH[u8_mainGop] != 0) ?
                    (pGOPHalLocal->pHALShared->u16GopSplitMode_LRWIDTH[u8_mainGop] - 1): 0xeff;
        crop_vstart = 0x0;
        crop_vend = (u16VScalingRatio != 0) ? ((SCALING_MULITPLIER * (MS_U32)gop_stretch_height) / u16VScalingRatio + u16Gop_stretch_vstart) : 0x870;
...
#if ENABLE_MANUAL_CROP==1
        MS_U16 u16ManualCropHstart = 0x0, u16ManualCropHend = 0x0;

        u16ManualCropHstart = 0x0;
        u16ManualCropHend = gop_stretch_width - 1;
        HAL_GOP_Write16Reg(pGOPHalLocal, REG_GOP_CROP_ORIHSTART(u8_mainGop), u16ManualCropHstart, GOP_REG_WORD_MASK);
        HAL_GOP_Write16Reg(pGOPHalLocal, REG_GOP_CROP_ORIHEND(u8_mainGop), u16ManualCropHend, GOP_REG_WORD_MASK);

        u16ManualCropHstart = gop_stretch_width - 2;
        u16ManualCropHend = (gop_stretch_width<<1) - 1;
        HAL_GOP_Write16Reg(pGOPHalLocal, REG_GOP_CROP_ORIHSTART(u8_combinedGop), u16ManualCropHstart, GOP_REG_WORD_MASK);
        HAL_GOP_Write16Reg(pGOPHalLocal, REG_GOP_CROP_ORIHEND(u8_combinedGop), u16ManualCropHend, GOP_REG_WORD_MASK);

        HAL_GOP_Write16Reg(pGOPHalLocal, REG_GOP_CROP_PRECALDONE(u8_mainGop), GOP_BIT0, GOP_BIT0);
        HAL_GOP_Write16Reg(pGOPHalLocal, REG_GOP_CROP_PRECALDONE(u8_combinedGop), GOP_BIT0, GOP_BIT0);

        // if afbc enable, use manual crop mode
        MS_U8 u8afbc_core_number = 0x0;
        HAL_GOP_AFBC_GetCore(pGOPHalLocal, u8_mainGop, &u8afbc_core_number);
        if (_isAfbcFunctionEnabled(pGOPHalLocal, u8afbc_core_number))
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, u32MainBankOffSet + GOP_4G_MANUAL_CROP1, GOP_BIT4 | GOP_BIT5, GOP_BIT4 | GOP_BIT5);
            HAL_GOP_Write16Reg(pGOPHalLocal, u32MainBankOffSet + GOP_4G_BANK_FWR, GOP_BIT4 | GOP_BIT5, GOP_BIT4 | GOP_BIT5);
        }
#endif
...
GOP_Result HAL_GOP_CMDQ_SetGOPACKMask(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U16 u16GopMask)
{
    ...
    if(g_GopChipPro.bAFBC_Merge_GOP_Trig == FALSE)
    {
        if(g_GopChipPro.bAFBCMIUSelDoubleBuffer == FALSE)
        {
            HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,REG_AFBC_MIU,u16AFBCMIUSelect[gop]<<4, 0xFFFF);
        }
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,REG_AFBC_TRIGGER,GOP_BIT1, 0xFFFF);
