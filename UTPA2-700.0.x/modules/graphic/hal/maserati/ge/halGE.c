...
#ifdef  GE_USE_HW_SEM
#ifdef MSOS_TYPE_OPTEE    //R2
        GE_H_INFO("%04x ", GE1_REG(i) );
#else
...
#ifdef  GE_USE_HW_SEM
#ifdef MSOS_TYPE_OPTEE    //R2
        return GE1_REG(addr-GE_TABLE_REGNUM);
#else
...
#ifdef  GE_USE_HW_SEM
#ifdef MSOS_TYPE_OPTEE    //R2
    return (GE1_REG(addr)&u16NoFIFOMask)|(pGEHalLocal->u16RegGETable[addr]&~u16NoFIFOMask);
#else
...
#ifdef  GE_USE_HW_SEM
#ifdef MSOS_TYPE_OPTEE    //R2
                    GE1_REG(i)= pGEHalLocal->u16RegGETable[i];
                    GE1_REG(REG_GE_CMD)= pGEHalLocal->u16RegGETable[REG_GE_CMD];
#else
...
#ifdef MSOS_TYPE_OPTEE    //R2
        if(GE_Map_Share_Reg(pGEHalLocal,addr)== E_GE_OK)
            pGEHalLocal->pHALShared->u16ShareRegImage[addr]= GE1_REG(addr);
        pGEHalLocal->u16RegGETable[addr] = GE1_REG(addr);
#else
        if(GE_Map_Share_Reg(pGEHalLocal,addr)== E_GE_OK)
...
void GE_Init(GE_CTX_HAL_LOCAL *pGEHalLocal, GE_Config *cfg)
{
    ...
    if ((u16temp & BIT(1)) != BIT(1)) //if VQ is Not Enabled
    {
        ...
        if (cfg->u32VCmdQSize >= GE_VCMDQ_SIZE_MIN)
        {
            ...
            GE_SetVCmdBuffer(pGEHalLocal, PhyVQAddr, u32VcmdqBufSz);
...
GE_Result GE_CalcBltScaleRatio(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_U16 u16SrcWidth, MS_U16 u16SrcHeight, MS_U16 u16DstWidth, MS_U16 u16DstHeight, GE_ScaleInfo *pScaleinfo)
{
    ...
    if((u16SrcHeight/2) > u16DstHeight)
    {
        pScaleinfo->init_y = 0;
    }
...
GE_Result GE_SetVCmdBuffer(GE_CTX_HAL_LOCAL *pGEHalLocal, MS_PHY PhyAddr, GE_VcmqBufSize enBufSize)
{
    ...
    u16temp2 = GE_ReadReg(pGEHalLocal, REG_GE_YUV_MODE);
    if(PhyAddr>= HAL_MIU2_BASE)
        GE_WriteReg(pGEHalLocal, REG_GE_YUV_MODE, u16temp2|GE_VCMQ_MIU_SEL_H);
    else
        GE_WriteReg(pGEHalLocal, REG_GE_YUV_MODE, u16temp2&(~GE_VCMQ_MIU_SEL_H));
