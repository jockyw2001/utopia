...
GOP_CHIP_PROPERTY g_GopChipPro =
{
    ...
    .bGOPWithVscale =           {TRUE, TRUE, TRUE, FALSE, TRUE}, //setting GOP with/without Vscale
...
GOP_Result HAL_GOP_MIXER_EnableOldBlendMode(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOP, MS_BOOL bEn)
{
    ...
    if(gopDstType == E_DRV_GOP_DST_MIXER2VE)
    {
        ...
        if(bEn)
        {
            ...
            {
                 HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD_EN, (~BIT(2))|BIT(1), BIT(1)|BIT(2));
            }
            else if(u8GOP == ((regval&(GOP_BIT12|GOP_BIT13))>>12))
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD1_EN, (~BIT(2))|BIT(1), BIT(1)|BIT(2));
            }
            else
            {
        ...
        else
        {
            ...
            if(u8GOP == ((regval&(GOP_BIT10|GOP_BIT11))>>10))
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD_EN, (~BIT(1))|BIT(2), BIT(1)|BIT(2));
            }
            else if(u8GOP == ((regval&(GOP_BIT12|GOP_BIT13))>>12))
            {
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD1_EN, (~BIT(1))|BIT(2), BIT(1)|BIT(2));
            }
            else
            {
                printf("[%s][%d]GOP not exisit in VE mux\n",__FUNCTION__,__LINE__);
...
GOP_Result HAL_GOP_EnableVENewAlphaMode(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 veMuxNum, MS_BOOL bEnable)
{
    MS_U16 u16val = 0x0;
    u16val = (bEnable? BIT(2): 0);
    switch (veMuxNum) //Enable OP per pixel new alpha mode
    {
        case 0:
                //printf("[%s][%d]====enable_new alpha====\n",__FUNCTION__,__LINE__);
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD_EN, u16val, BIT(2));
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD_EN_ALPHA_INV, (bEnable? ~(BIT(7)): BIT(7)), BIT(7));
                break;

        case 1:
                HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD1_EN, u16val, BIT(2));
...
GOP_Result HAL_GOP_VE_SetOSDEnable(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_BOOL bEnable, EN_VE_OSD_ENABLE eOSD, MS_U8 gopNum)
{
    ...
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, u32Reg, BIT(0), BIT(0));
    }
    else // disable OSD
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, u32Reg, 0x0, BIT(0));
    }
    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVE_SWRST, BIT(2), BIT(2));
    //set VE alpha inverse
    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_VE_TVS_OSD1_EN, BIT(5), BIT(5));
