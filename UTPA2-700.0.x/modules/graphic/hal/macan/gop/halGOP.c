...
extern Gop_MuxSel u16GopMuxChanged[MAX_GOP_SUPPORT];
extern MS_U32 u32GopPipeDelay[MAX_GOP_SUPPORT];
extern MS_BOOL bGopUseOsdAlpha[MAX_GOP_SUPPORT];
#endif
GOP_CHIP_PROPERTY g_GopChipPro =
{
    ...
    .bInternalMIUSelect=        {TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE}, //{E_GOP0, E_GOP1, E_GOP2, E_GOP3, E_GOP4, E_GOP_Dwin, E_GOP_MIXER, E_GOP5}
...
#ifdef GOP_HW_MIUSEL_PATCH
    else
    {
        if(u8GOPNum == E_GOP2)
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_MIU_IP_SEL, 0, 1<<GOP_MIU_CLIENT_GOP2);
    }
#endif
...
GOP_Result HAL_GOP_GWIN_SetDstPlane(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 GopNum, DRV_GOPDstType eDstType,MS_BOOL bOnlyCheck)
{
    ...
    */
    ...
    switch (eDstType)
    {
        ...
        case E_DRV_GOP_DST_MOD:
            //GOP destination --> SC OP
            u16RegVal = 0x2;
            bEnable = TRUE;
            u16HSyncMask=GOP_BIT14;
            break;
    ...
    //for Monaco, IP need set Vsync inverse in IP
...
#ifdef GOP_CMDQ_ENABLE
void HAL_GOP_GWIN_SetMUX_ThroughCmdq(GOP_CTX_HAL_LOCAL*pGOPHalLocal,
CAF_Struct *p_cmdq_struct, MS_U32 *p_number, MS_U8 u8GOPNum)
{
    Gop_MuxSel eGopMux = E_GOP_MUX0;
    MS_U32 pipe_delay;
    MS_BOOL osd_alpha;
    MS_U16 gop_idx = 0;
    MS_U32 u32BankOffSet = 0;
    MS_U16 op_mux_val = 0x0;
    MS_BOOL bScMode1En = FALSE;
    MS_U16 u16scMode1Reg = 0x0;

    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_MUX, &op_mux_val);
    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_SC_GOPENMODE1, &u16scMode1Reg);

    for(gop_idx = 0; gop_idx<MAX_GOP_SUPPORT; gop_idx++)
    {
        MS_U16 mask = 0xFFFF;
        MS_U16 u16val = 0x0;

        eGopMux = u16GopMuxChanged[gop_idx];

        if (eGopMux == E_GOP_MUX_INVAILD)
        {
            continue;
        }
        ...
        switch(eGopMux)
        {
            ...
            case E_GOP_MUX3:
            case E_GOP_MUX4:
                mask = GOP_REGMUX_MASK<<(GOP_MUX_SHIFT*eGopMux);
                u16val = gop_idx <<(GOP_MUX_SHIFT*eGopMux);

                op_mux_val &= ~mask;
                u16val &=  mask;
                op_mux_val |=  u16val;

                HAL_GOP_GetGOPAlphaMode1(pGOPHalLocal, gop_idx, &bScMode1En);
                if (E_GOP_MUX0 == eGopMux) {
                    mask = (0x30);
                    u16val = (TRUE == bScMode1En) ? (0x10) : (~0x10);
                } else if (E_GOP_MUX1 == eGopMux) {
                    mask = (0x3000);
                    u16val = (TRUE == bScMode1En) ? (0x1000) : (~0x1000);
                } else if (E_GOP_MUX2 == eGopMux) {
                    mask = (0xC0);
                    u16val = (TRUE == bScMode1En) ? (0x40) : (~0x40);
                } else if (E_GOP_MUX3 == eGopMux) {
                    mask = (0x300);
                    u16val = (TRUE == bScMode1En) ? (0x100) : (~0x100);
                } else if (E_GOP_MUX4 == eGopMux) {
                    mask = (0xC000);
                    u16val = (TRUE == bScMode1En) ? (0x4000) : (~0x4000);
                }

                u16scMode1Reg &= ~mask;
                u16val &=  mask;
                u16scMode1Reg |=  u16val;
                break;

            default:
                printf("[%s] Only support OP mux switch now, ERROR mux setting\n",__FUNCTION__);
                break;
        }
    }
    HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,p_cmdq_struct,p_number, GOP_MUX, op_mux_val, 0xFFFF);//current GOP force write en
    HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,p_cmdq_struct,p_number, GOP_SC_GOPENMODE1, u16scMode1Reg, 0xFFFF);//current GOP force write en


    for(gop_idx = 0; gop_idx<MAX_GOP_SUPPORT; gop_idx++)
    {
        MS_U16 u16BnkWrVal;
        MS_U16 u16GopReg0Val;
        eGopMux = u16GopMuxChanged[gop_idx];
        pipe_delay = u32GopPipeDelay[gop_idx];
        osd_alpha = bGopUseOsdAlpha[gop_idx];

        u16GopMuxChanged[gop_idx] = E_GOP_MUX_INVAILD;
        u32GopPipeDelay[gop_idx] = 0;
        ...
        _GetBnkOfstByGop(gop_idx, &u32BankOffSet);
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,p_cmdq_struct,p_number, u32BankOffSet+GOP_4G_HS_PIPE, pipe_delay, 0xFFFF);

        HAL_GOP_Read16Reg(pGOPHalLocal, u32BankOffSet+GOP_4G_CTRL0, &u16GopReg0Val);
        if (bGopUseOsdAlpha[gop_idx] == TRUE) {
            u16GopReg0Val |= GOP_BIT15;
        } else {
            u16GopReg0Val &= (~GOP_BIT15);
        }
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,p_cmdq_struct,p_number, u32BankOffSet+GOP_4G_CTRL0, u16GopReg0Val, 0xFFFF);

        HAL_GOP_Read16Reg(pGOPHalLocal, u32BankOffSet+GOP_4G_BANK_FWR, &u16BnkWrVal);
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,p_cmdq_struct,p_number,u32BankOffSet+GOP_4G_BANK_FWR,(u16BnkWrVal|(GOP_BIT0)) ,0xFFFF);//current GOP force write en
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,p_cmdq_struct,p_number,u32BankOffSet+GOP_4G_BANK_FWR,(u16BnkWrVal&(~GOP_BIT0)) ,0xFFFF);//current GOP force write en
    }
}
#endif
    ...
    ******************/
    ...
    case E_DRV_GOP_DST_VE:
     ...
     case E_DRV_GOP_DST_IP1:
        ...
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOP2CLK, CKG_GOPG1_IDCLK1, CKG_GOPG2_MASK);
        }
        else if (gopNum==E_GOP0)
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, GOP_GOPCLK, CKG_GOPG1_IDCLK1, CKG_GOPG0_MASK);
        }
        else
        {
            MS_ASSERT(0);
...
void HAL_GOP_SetIPSel2SC(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_IPSEL_GOP ipSelGop)
{
    ...
    case MS_DRV_MVOP_SEL:
        ...
        break;
    case MS_DRV_IP1_SEL:
        HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC1_CHANNELSYNC, 0, GOP_BIT11);
        u16RegMsk = 0;
        break;
    default:
...
MS_BOOL HAL_GOP_EnableScalingDownSram(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U8 u8GOP,MS_BOOL bEn)
{
    ...
    {
        printf("[%s] Error message GOP not support scaling down!!",__FUNCTION__);
        return GOP_FAIL;
    }
    if(bEn==TRUE)
    {
...
GOP_Result HAL_GOP_HScalingDown(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U8 u8GOP, MS_BOOL bEnable,MS_U16 src, MS_U16 dst)
{
    ...
    if(bEnable ==TRUE)
    {
        ...
        if( u16VScalReg != 0x1000 ) //Not support H/V scaling up and down at same time.
        {
            ...
            printf("[%s] Warning message about GOP not support H/V scaling up and down at same time!!!!",__FUNCTION__);
        }
...
GOP_Result HAL_GOP_OC_SetMODEn(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_BOOL bMODEn)
{
    MS_U16 u16RegModVal;

    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_SC_MOD, (bMODEn? GOP_BIT0:0)  , GOP_BIT0);
...
GOP_Result HAL_GOP_CMDQ_WriteCommand(GOP_CTX_HAL_LOCAL *pGOPHalLocal,CAF_Struct *cmdq_struct,MS_U32 *number,MS_U32 u32addr, MS_U16 u16val, MS_U16 mask)
{
    ...
    switch (u32addr & 0xFF00)
    {
        case GOP_REG_BASE:
        {
            ...
            cmdq_struct[(*number)].mask = 0xFFFF; // ((~mask)&0xFFFF);
...
GOP_Result HAL_GOP_CMDQ_SetGOPACK(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U8 gop)
{
    ...
    MS_U32 fireGOP;
    MS_BOOL bSettingMiuClient = FALSE;
    ...
    if(bMIUSelect[gop] == TRUE)
    {
        ...
        bSettingMiuClient = TRUE;
    }
    HAL_GOP_GWIN_SetMUX_ThroughCmdq(pGOPHalLocal, fire_struct, &number, gop);
