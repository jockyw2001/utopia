...
#define OTP_REG(addr)           (*((volatile MS_U32*)(u32hal_sys_baseaddr + (0x190000<<1) + addr )))
...
MS_BOOL HAL_SYS_SetPadMux(SYS_PAD_MUX_SET ePadMuxType,SYS_PAD_SEL ePadSel)
{
    ...
    if(ePadMuxType == E_TS0_PAD_SET)
    {
        ...
        else if(ePadSel == E_MSPI_PAD_ON)
        {
            ...
            u16data = HAL_SYS_Read2Byte(REG_TS_MODE_OFFSET) & (~(REG_MSPI1_MODE1_MASK<<REG_MSPI1_MODE1_SHIFT));
    ...
    else if(ePadMuxType == E_TS1_PAD_SET)
    {
        ...
        else if(ePadSel == E_MSPI_PAD_ON)
        {
            ...
            u16data = HAL_SYS_Read2Byte(REG_TS_MODE_OFFSET) & (~(REG_MSPI1_MODE3_MASK<<REG_MSPI1_MODE3_SHIFT));
...
MS_BOOL HAL_SYS_Query(E_SYS_QUERY id)
{
    ...
    switch (id)
    {
        ...
        case E_SYS_QUERY_4K_SUPPORTED: // 0x193D_16[4]
            u32Val = (OTP_REG(0x3D14) >> 16);
            bRet = (u32Val & (BIT4)) == 0x0 ? TRUE : FALSE;
            break;

        case E_SYS_QUERY_TSIN_SUPPORTED: // 0x193D_17[0]
            u32Val = (OTP_REG(0x3D14) >> 24);
            bRet = (u32Val & (BIT0)) == 0x0 ? TRUE : FALSE;
            break;

        case E_SYS_QUERY_VP8_SUPPORTED: // 0x193D_0F[7]
            u32Val = (OTP_REG(0x3D0C) >> 24);
            bRet = (u32Val & (BIT7)) == 0x0 ? TRUE : FALSE;
            break;

        case E_SYS_QUERY_VP9_SUPPORTED: // 0x193D_12[3]
            u32Val = (OTP_REG(0x3D10) >> 16);
            bRet = (u32Val & (BIT3)) == 0x0 ? TRUE : FALSE;
            break;

        case E_SYS_QUERY_SECURED_IC_SUPPORTED:
            bRet = (u32Val & (BIT0)) == 0x0 ? FALSE : TRUE;
            break;
        default:
...
MS_BOOL HAL_SYS_SetTSClockPhase(SYS_PAD_MUX_SET ePadMuxType,MS_U16 u16Val)
{
    ...
    if(ePadMuxType == E_TS0_PAD_SET)
    {
        ...
        HAL_SYS_Write2Byte(0x100B56, HAL_SYS_Read2Byte(0x100B56) | 0x1);
    }
    else if(ePadMuxType == E_TS1_PAD_SET)
    {
...
MS_U32 HAL_SYS_QueryDolbyHashInfo(E_SYS_DOLBY_HASH_INFO index)
{
    ...
    switch(index)
    {
        ...
        case E_SYS_DOLBY_CONTROL_BIT:
            return (MS_U32)((u32OTP_0x3D10_shadow >> 21) & (0x01)); // 0x3D10 [21]
...
void HAL_SYS_GetDolbyKeyCustomer(MS_U8 * u8pkey)
{
#ifndef CONFIG_MBOOT  //Add For GPL (content protection)
    MS_U8 u8index;
...
void SysDvfsCpuPowerAdjustment(MS_U32 dwCpuPowerVoltage)
{
    ...
    {
        DVFS_DEBUG("Error- dwCpuPowerVoltage is smaller than CONFIG_DVFS_CPU_POWER_SHIFT_PRADO!!!\n");
        return;
    }
    else if(dwCpuPowerVoltage > CONFIG_DVFS_CPU_POWER_MAX)
    {
        DVFS_DEBUG("Error- dwCpuPowerVoltage is larger than CONFIG_DVFS_CPU_POWER_MAX!!!\n");
        return;
    }
    ...
    if(hMstarDvfsInfo.dwPowerChipId == CONFIG_DVFS_CHIP_ID_PRADO)
    {
        ...
        {
            DVFS_DEBUG("I2C Read Bytes with CONFIG_DVFS_POWER_SWI2C_ADDR_CPU Fail!!!\n");
            return;
        }
...
void SysDvfsCorePowerAdjustment(MS_U32 dwCorePowerVoltage)
{
    ...
    dwRegisterValue = *(volatile MS_U16 *)(u32hal_sys_baseaddr + (0x001424 << 1));

    if(dwCorePowerVoltage < CONFIG_DVFS_CORE_POWER_DEFAULT)
    {
        //VID = 2'b0 = Core Power is 0.93V
        dwRegisterValue &= ~(0x01 << 3);
    }
    else
    {
        //VID = 2'b1 = Core Power is 0.98V
        dwRegisterValue |= (0x01 << 3);
    }

    *(volatile MS_U16 *)(u32hal_sys_baseaddr + (0x001424 << 1)) = dwRegisterValue;
...
#endif
#if CONFIG_DVFS_CORE_POWER_GPIO_ENABLE
    *(volatile MS_U16 *)(u32hal_sys_baseaddr + (0x001424 << 1)) |= (0x01 << 3);
    *(volatile MS_U16 *)(u32hal_sys_baseaddr + (0x001422 << 1)) &= ~(0x01 << 11);
    *(volatile MS_U16 *)(u32hal_sys_baseaddr + (0x100512 << 1)) = CONFIG_DVFS_DYNAMIC_POWER_ADJUST_INIT;
#endif
}

MS_S16 SysDvfsGetCurrentTemperature(void)
{
    return HAL_SYS_Read2Byte(0x100504);
}
