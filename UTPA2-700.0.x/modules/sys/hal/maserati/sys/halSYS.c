        ...
        if((g_SysIoProc.SysSwI2CReadBytes != 0) && (g_SysIoProc.SysSwI2CWriteBytes != 0))
        {
            ...
            else
            {
                while(1)
                {
                    ...
                    {
                        if ((hMstarDvfsInfo.bDvfStrResume[0]==0) && (hMstarDvfsInfo.bDvfStrResume[1]==0))
                        {
                            SysDvfsInit();
                        }
                        hMstarDvfsInfo.bDvfStrResume[hMstarDvfsReadInfo.dwCluster] = 1;

                        if ((hMstarDvfsInfo.bDvfStrResume[0]==1) && (hMstarDvfsInfo.bDvfStrResume[1]==1))
                        {
                            hMstarDvfsInfo.bDvfStrResume[0] = 0;
                            hMstarDvfsInfo.bDvfStrResume[1] = 0;
                        }
                    }
                    else if(hMstarDvfsReadInfo.dwVoltageType == CONFIG_DVFS_OTP_MONITOR)
                    {
                        SysDvfsPradoProtect(hMstarDvfsReadInfo.dwCluster);
                    }
...
/// @param <GLOBAL>     \b None:
////////////////////////////////////////////////////////////////////////////////
void SysDvfsPradoProtect(MS_U8 byCluster)
{
    MS_U8   byTargetRegAddress[5] =
            {
                0xFF, 0xFF, 0xFF, 0xFF, 0xFF
    ...
    if(hMstarDvfsInfo.dwPowerChipId == CONFIG_DVFS_CHIP_ID_PRADO)
    {
        ...
        if(g_SysIoProc.SysSwI2CReadBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(byCluster), 2, byTargetRegAddress, 2, byTargetData) == TRUE)
        {
            ...
            {
                *(volatile MS_U16 *)(u32hal_sys_baseaddr + (0x100514 << 1)) = 1;
            }
            else
            {
                *(volatile MS_U16 *)(u32hal_sys_baseaddr + (0x100514 << 1)) = 0;
            }
        }
    }
}
...
/// @param <GLOBAL>     \b None:
////////////////////////////////////////////////////////////////////////////////
void SysDvfsPradoOTPFlagInit(MS_U8 byCluster)
{
    MS_U8   byTargetRegAddress[5] =
            {
                0xFF, 0xFF, 0xFF, 0xFF, 0xFF
...
{
    MS_BOOL bDDR443 = FALSE;
    MS_BOOL bDDR43 = FALSE;

    MS_BOOL bInputByPartEn = FALSE;
    MS_U32  u32Ret = E_SYS_OK;

    //
    // check hybird platform
    //
    if (((HAL_SYS_Read2Byte(0x161566) & 0x8) >  0) && /* MIU0 0x1615_33[0], 1:DDR4*/
       ((HAL_SYS_Read2Byte(0x162266) & 0x8) ==  0)) /* MIU1 0x1622_33[0], 1:DDR3*/
    {
        bDDR43 = TRUE;
    }
    else if (((HAL_SYS_Read2Byte(0x161566) & 0x8) >  0) && /* MIU0 0x1615_33[0], 1:DDR4*/
        ((HAL_SYS_Read2Byte(0x162266) & 0x8) >  0) && /* MIU1 0x1622_33[0], 1:DDR4*/
        ((HAL_SYS_Read2Byte(0x162366) & 0x8) == 0))   /* MIU2 0x1623_33[0], 0:DDR3*/
    {
        bDDR443 = TRUE;
    }
   ...
   if(bDDR43 == TRUE)
   {
       if((eInputTiming == E_HALSYS_INPUT_4K2K50_60) || (eInputTiming == E_HALSYS_INPUT_4K2K120))
       {
            *retEn = TRUE;
       }
       else
   ...
   else
   {
    switch (eInputTiming)
    {
    case E_HALSYS_INPUT_FHD24_25_30:
    case E_HALSYS_INPUT_FHD48:
    case E_HALSYS_INPUT_FHD50_60:
        if ( eSource == E_HALSYS_SOURCE_HDMI)
        {
            bInputByPartEn = TRUE;
        }
        break;
    case E_HALSYS_INPUT_4K2K24_25_30:
    case E_HALSYS_INPUT_4K2K48:
    case E_HALSYS_INPUT_4K2K50_60:
    case E_HALSYS_INPUT_4K2K120:
    case E_HALSYS_INPUT_4K2K_UNKNOWN:
        if ( eSource == E_HALSYS_SOURCE_HDMI)
        {
            bInputByPartEn = TRUE;
        }
        break;
    default:
        u32Ret = E_SYS_NOT_SUPPORT;
        break;
    }

    //
    // Only Bypart XC under the following conditions
    // 1. DDR4 + DDR4 + DDR3
    // 2. Input Source is HDMI 4K, HDMI FHD
    // 3. Output timing is 4k@120 Hz
    //
    if (bDDR443 == TRUE &&
        bInputByPartEn == TRUE &&
        (eOutputTiming == E_HALSYS_INPUT_4K2K120 || eOutputTiming == E_HALSYS_INPUT_4K2K50_60))
    {
        *retEn = TRUE;
    }
    else
