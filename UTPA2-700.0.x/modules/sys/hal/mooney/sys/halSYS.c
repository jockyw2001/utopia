...
MS_BOOL HAL_SYS_ReadEfuseHDCPKey(MS_U16 u16ReadAddr, MS_U32 *u32HDCPKey)
{
    ...
    if( u16ReadAddr < 40)
    {
        HAL_SYS_EfuseWrite2Byte(REG_EFUSE_FSM0_CTRL,HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_CTRL)&~BIT8);//sel read mode

        HAL_SYS_EfuseWrite2Byte(REG_EFUSE_FSM0_CTRL,((HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_CTRL)&(~EFUSE_ADDRESS_MASK))|(u16ReadAddr+EFUSE_OFFSET_HDCP_FSM0)));
        HAL_SYS_EfuseWrite2Byte(REG_EFUSE_FSM0_TRIG,HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_TRIG)|BIT0);//trigger

        while((HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_TRIG)&BIT0)==BIT0);

        if( ( u16ReadAddr % 2 ) == 0)
        {
            hdcp_rdata_lo = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_RDATA_15_0);
            hdcp_rdata_hi = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_RDATA_31_16);
        }
        else
        {
            hdcp_rdata_lo = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_RDATA_47_32);
            hdcp_rdata_hi = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM0_RDATA_63_48);
        }
    }
    else
    {
        HAL_SYS_EfuseWrite2Byte(REG_EFUSE_FSM1_CTRL,HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_CTRL)&~BIT8);//sel read mode

        HAL_SYS_EfuseWrite2Byte(REG_EFUSE_FSM1_CTRL,((HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_CTRL)&(~EFUSE_ADDRESS_MASK))|(u16ReadAddr-EFUSE_OFFSET_HDCP_FSM1)));
        HAL_SYS_EfuseWrite2Byte(REG_EFUSE_FSM1_TRIG,HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_TRIG)|BIT0);//trigger

        while((HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_TRIG)&BIT0)==BIT0);

        if( ( u16ReadAddr % 2 ) == 0)
        {
            hdcp_rdata_lo = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_RDATA_15_0);
            hdcp_rdata_hi = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_RDATA_31_16);
        }
        else
        {
            hdcp_rdata_lo = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_RDATA_47_32);
            hdcp_rdata_hi = HAL_SYS_EfuseRead2Byte(REG_EFUSE_FSM1_RDATA_63_48);
        }
    }
...
void SysDvfsCpuPowerAdjustment(MS_U32 dwCpuPowerVoltage, MS_U8 byCluster)
{
    ...
    if(hMstarDvfsInfo.dwPowerChipId == CONFIG_DVFS_CHIP_ID_PRADO)
    {
        ...
        if(g_SysIoProc.SysSwI2CReadBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(byCluster, hMstarDvfsInfo.dwI2cBus), 2, byTargetRegAddress, 2, byTargetData) == TRUE)
        {
        ...
        if (byTargetData[1] == 0)
        {
            dwOriginalCpuPowerVoltage = dwCpuPowerVoltage - 3;
        }
        ...
        if(hMstarDvfsInfo.bDvfsInitOk == 1)
        {
            if(dwCpuPowerVoltage > dwOriginalCpuPowerVoltage)
            {
                for(;dwSourceRegisterSetting <= dwTargetRegisterSetting; dwSourceRegisterSetting += CONFIG_DVFS_CPU_POWER_STEP(byCluster))
                {
                    ...
                    if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(byCluster, hMstarDvfsInfo.dwI2cBus), 4, byTargetRegAddress, 0, byTargetData) == TRUE)
                    {
...
void SysDvfsCpuPowerInit(void)
{
    ...
    for (byloop=0;byloop<3;byloop++)
    {
        ...
        if(g_SysIoProc.SysSwI2CReadBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(0, byloop), 2, byTargetRegAddress, 2, byTargetData) == TRUE)
        {
            hMstarDvfsInfo.dwI2cBus = byloop;
            break;
        }
    }
    ...
    for (byloop=0;byloop<1;byloop++)
    {
        ...
        if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(byloop, hMstarDvfsInfo.dwI2cBus), 5, byTargetRegAddress, 0, byTargetData) != TRUE)
        {
            DVFS_DEBUG("\033[33m[ERROR] I2C_Enter_I2C Failed\033[m\n");
        }

        byTargetRegAddress[0] = 0x7F;
        if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(byloop, hMstarDvfsInfo.dwI2cBus), 1, byTargetRegAddress, 0, byTargetData) != TRUE)
        {
        ...
        byTargetRegAddress[0] = 0x10;
        byTargetRegAddress[1] = 0xc0;
        if(g_SysIoProc.SysSwI2CReadBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(byloop, hMstarDvfsInfo.dwI2cBus), 2, byTargetRegAddress, 2, byTargetData) == TRUE)
        {
        ...
        byTargetRegAddress[2] = 0x10;
        byTargetRegAddress[3] = dwRegisterValue;
        if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(byloop, hMstarDvfsInfo.dwI2cBus), 4, byTargetRegAddress, 0, byTargetData) == TRUE)
        {
...
#error "No Support CPU Power Adjustment by GPIO in Mooney Platform"
#endif
}
