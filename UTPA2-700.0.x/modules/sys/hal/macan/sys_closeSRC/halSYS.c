...
MS_BOOL HAL_SYS_SetPadMux(SYS_PAD_MUX_SET ePadMuxType,SYS_PAD_SEL ePadSel)
{
    ...
    {
        u16data = HAL_SYS_Read2Byte(0x101EAE) & ~(BIT14|BIT15);

        if(ePadSel == E_SERIAL_IN)
        {
            HAL_SYS_Write2Byte(0x101EAE, u16data|BIT14);
        }
        else if(ePadSel == E_PARALLEL_IN)
        {
            HAL_SYS_Write2Byte(0x101EAE, u16data|BIT15);
        }
        else
        {
            ret = FALSE;
    ...
    {
        u16data = HAL_SYS_Read2Byte(0x101E80) & ~(BIT10|BIT11);
        if(ePadSel == E_PARALLEL_IN)
        {
            HAL_SYS_Write2Byte(0x101E80, u16data|BIT11);
        }
        else if(ePadSel == E_SERIAL_IN)
        {
            HAL_SYS_Write2Byte(0x101E80, u16data|BIT10);
        }
        else
        {
            return FALSE;
...
void SysDvfsCpuPowerAdjustment(MS_U32 dwCpuPowerVoltage)
{
    ...
    if(dwCpuPowerVoltage > CONFIG_DVFS_CPU_POWER_MAX || dwCpuPowerVoltage < CONFIG_DVFS_CPU_POWER_MIN)
    {
        DVFS_DEBUG("\033[37m[ERROR] Get error CpuPowerVoltage!\033[m\n");
        return;
    }
    ...
    if(hMstarDvfsInfo.dwPowerChipId == CONFIG_DVFS_CHIP_ID_PRADO)
    {
        ...
        if(g_SysIoProc.SysSwI2CReadBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(hMstarDvfsInfo.dwSWI2CBusId), 2, byTargetRegAddress, 2, byTargetData) == TRUE)
        {
        ...
        if(hMstarDvfsInfo.bDvfsInitOk == 1)
        {
            if(dwCpuPowerVoltage > dwOriginalCpuPowerVoltage)
            {
                for(;dwSourceRegisterSetting <= dwTargetRegisterSetting; dwSourceRegisterSetting += CONFIG_DVFS_CPU_POWER_STEP)
                {
                    ...
                    if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(hMstarDvfsInfo.dwSWI2CBusId), 4, byTargetRegAddress, 0, byTargetData) == TRUE)
                    {
...
void SysDvfsCpuPowerInit(void)
{
    ...
    MS_U8   SWI2C_Bus_idx = 0;

    for(SWI2C_Bus_idx = 0; SWI2C_Bus_idx < CONFIG_DVFS_POWER_SWI2C_BUS_NUM; SWI2C_Bus_idx++)
    {
        ...
        if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(SWI2C_Bus_idx), 5, byTargetRegAddress, 0, byTargetData) != TRUE)
        {
            DVFS_DEBUG("\033[33m[ERROR] I2C_Enter_I2C Failed\033[m\n");
        }

        byTargetRegAddress[0] = 0x7F;
        if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(SWI2C_Bus_idx), 1, byTargetRegAddress, 0, byTargetData) != TRUE)
        {
        ...
        if(g_SysIoProc.SysSwI2CReadBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(SWI2C_Bus_idx), 2, byTargetRegAddress, 2, byTargetData) == TRUE)
        {
            ...
            hMstarDvfsInfo.dwSWI2CBusId = SWI2C_Bus_idx;
        }
        else
        {
            DVFS_DEBUG("\033[33m[ERROR] I2C_IDENTIFY_PMIC Failed\033[m\n");
            continue;
        }
        ...
        if(hMstarDvfsInfo.dwPowerChipId == CONFIG_DVFS_CHIP_ID_PRADO)
        {
            ...
            if(g_SysIoProc.SysSwI2CWriteBytes(CONFIG_DVFS_POWER_SWI2C_ADDR_CPU(SWI2C_Bus_idx), 4, byTargetRegAddress, 0, byTargetData) != TRUE)
            {
