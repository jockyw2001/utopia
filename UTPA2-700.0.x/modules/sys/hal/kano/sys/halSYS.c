...
void SysDvfsCpuPowerAdjustment(MS_U32 dwCpuPowerVoltage, MS_U8 byCluster)
{
    ...
    {
        DVFS_DEBUG("\033[33m[ERROR] dwCpuPowerVoltage is 0 !!!\033[m\n");
        return;
    }
    ...
    if(hMstarDvfsInfo.dwPowerChipId == CONFIG_DVFS_CHIP_ID_PRADO)
    {
        ...
        if ((byTargetData[1] == 0) || (byTargetData[1] == 0xff) || (byTargetData[1] <= 0xe))
        {
...
void SysDvfsCpuPowerInit(void)
{
    ...
    for (byloop=0;byloop<10;byloop++)
    {
        DVFS_DEBUG("\033[37mSysDvfsCpuPowerInit eason\033[m\n");
    }
...
#error "No Support CPU Power Adjustment by GPIO in kano Platform"
#endif
}
