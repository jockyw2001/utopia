...
MS_BOOL HAL_SYS_DMD_VD_MBX_Init(void)
{
    ...
    {
        ULOGE(TAG_DMD_VD_MBX,"IOMap failure to get MAP_NONPM_BANK\n");
        return FALSE;
    }
    ...
    #ifdef MS_DEBUG
    ULOGD(TAG_DMD_VD_MBX,"HAL_SYS_DMD_VD_MBX_Init %lx\n",u32NonPMBank);
    #endif
    return TRUE;
}
...
{
    #ifdef MS_DEBUG
    ULOGD(TAG_DMD_VD_MBX,"HAL_SYS_DMD_VD_MBX_GetType %d\n",MBX_SetType);
    #endif
...
MS_BOOL HAL_SYS_DMD_VD_MBX_ATV_WaitReady(void)
{
    ...
    #ifdef MS_DEBUG
    ULOGE(TAG_DMD_VD_MBX,"HAL_SYS_DMD_VD_MBX_ATV_WaitReady Fail\n");
    #endif
    return FALSE;
}
...
MS_BOOL HAL_SYS_DMD_VD_MBX_ATV_ReadByte(MS_U32 u32Reg, MS_U8 *u8Value)
{
    ...
    #ifdef MS_DEBUG
    ULOGE(TAG_DMD_VD_MBX,"HAL_SYS_DMD_VD_MBX_ATV_ReadByte Fail\n");
    #endif
    *u8Value = 0;
    return FALSE;
}
...
MS_BOOL HAL_SYS_DMD_VD_MBX_DVB_WaitReady(void)
{
    ...
    while (RIU_ReadByte(MBRegBase + 0x00)) // wait VDMCU ready
    {
        ...
        {
            ULOGE(TAG_DMD_VD_MBX,"HAL_SYS_DMD_VD_MBX_DVB_WaitReady Timeout\n");
            return FALSE;
        }
    }
    return TRUE;
...
void HAL_SYS_DMD_VD_MBX_DVB_WaitHandShake(void)
{
    ...
    while(RIU_ReadByte(MBRegBase + 0x00) != 0xFF)           // wait MB_CNTL set done
    {
        if (MsOS_Timer_DiffTimeFromNow(u32StartTime)>DMD_MBX_TIMEOUT)
        {
            ULOGE(TAG_DMD_VD_MBX,"HAL_SYS_DMD_VD_MBX_DVB_WaitHandShake Timeout\n");
            break;
        }
    }
}
