...
// Local variable
//------------------------------------------------------------------------------
static MS_VIRT _gPWS_MapBase = 0xA0000000;
...
static void *PwsPowerMonitorThread(void *pThreadInfo)
{
    ...
    while(1)
    {
        ...
        if((HAL_PWS_Read2Byte((REG_CKG_VE))&(CKG_VE_CLK_DISABLE|CKG_VE_13_CLK_DISABLE|CKG_VEDEC_CLK_DISABLE)) & (HAL_PWS_Read2Byte((REG_CKG_VE_IN)) & (CKG_VE_IN_CLK_DISABLE)))
            bVEIsClockOn = FALSE;
        else
            bVEIsClockOn = TRUE;
