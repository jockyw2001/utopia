...
#define UART_HAL_ERR(x, args...)        //{ULOGI(TAG_UART, x, ##args);}  
...
static MS_U32 _gUART2_Status =0;
...
static void _HAL_UART_PIU_Isr(UART_DEVICE_TYPE uart_dev)//ISR Body
{
    ...
    if (!(iir & UART_IIR_NO_INT))
    {
       ...
       if ( (iir & UART_IIR_TOI)||(iir & UART_IIR_NO_INT) )// UART time out & busy
       {
             ...
             else if( uart_dev == E_UART_PIU_UART2 )
             {
                   _gUART2_Status = UART_IIR_TOI;
             }
             else
             {
...
void HAL_UART2_PIU_Isr(MS_U32 *pHalReg, MS_U32 u32Data)
{
    UNUSED(pHalReg);
    UNUSED(u32Data);
    _HAL_UART_PIU_Isr(E_UART_PIU_UART2);
...
//#define IRQ_UART0   (MDRV_IRQ_BEGIN + 0)
    ...
    if (rx_cb)
    {
        ...
        else if( u->regbase == UART2_PIU_ADDR )
        {
           MsOS_AttachInterrupt(E_INT_IRQ_UART2, (InterruptCb)HAL_UART2_PIU_Isr);
           MsOS_EnableInterrupt(E_INT_IRQ_UART2);
        }
        else
        {
    ...
    else
    {
        ...
        {
		   MsOS_DisableInterrupt(E_INT_IRQ_UART2);
		   UART_REG8(UART_IER) &= ~UART_IER_RDI;
		   MsOS_DetachInterrupt(E_INT_IRQ_UART2);
    	}
    	else
    	{
...
void HAL_UART2_PIU_Isr_HalReg(MS_U32 *pHalReg, MS_U32 u32Data)
{
    #if !defined(CONFIG_FRC)
    _HAL_UART_PIU_Isr_HalReg(E_UART_PIU_UART2, pHalReg, u32Data);
    MsOS_EnableInterrupt(E_INT_IRQ_UART2);
    #endif
}
...
//#define IRQ_UART0   (MDRV_IRQ_BEGIN + 0)
    ...
    if (rx_cb)
    {
        ...
        {
           MsOS_AttachInterrupt(E_INT_IRQ_UART2, (InterruptCb)HAL_UART2_PIU_Isr_HalReg);
           MsOS_EnableInterrupt(E_INT_IRQ_UART2);
        }
        else
        {
...
void HAL_UART_SetUARTSecurityBank(UART_DEVICE_TYPE uart_dev)
{
    switch (uart_dev)
    {
        case E_UART_PIU_UART1:
        {
            //set BK0x1239, 16-bit mode, offset: 0x14[13] = 0
            MS_U8 u8Val=0;
            MS_U32 u32Reg = 0x123900+ 0x14*2 +1;

            u8Val = _HAL_REG_PM_ReadByte(u32Reg);
            u8Val &= 0xDF;

            _HAL_REG_PM_WriteByte(u32Reg,u8Val);
            break;
        }

        default:
            printf("not only support SetUARTSecurityBank for uart1\n");
            break;
    }
    return;
}
