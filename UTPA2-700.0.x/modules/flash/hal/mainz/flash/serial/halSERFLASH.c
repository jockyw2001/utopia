...
//CHIP_TOP
#define CHIP_TOP_WRITE(addr, val)               WRITE_WORD(_hal_isp.VirtChipTopBaseAddr + ((addr)<<2), (val))
#define CHIP_TOP_WRITE_MASK(addr, val, mask)    WRITE_WORD_MASK(_hal_isp.VirtChipTopBaseAddr + ((addr)<<2), (val), (mask))
...
static MS_BOOL _HAL_SERFLASH_WaitWriteCmdRdy(void)
{
    ...
    if (bRet == FALSE)
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ERR, ULOGE(TAG_SERFLASH,"Wait for SPI Write Cmd Ready fails!\n"));
    }
...
static MS_BOOL _HAL_SERFLASH_WaitWriteDataRdy(void)
{
    ...
    if (bRet == FALSE)
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ERR, ULOGE(TAG_SERFLASH,"Wait for SPI Write Data Ready fails!\n"));
    }
...
static MS_BOOL _HAL_SERFLASH_WaitReadDataRdy(void)
{
    ...
    if (bRet == FALSE)
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ERR, ULOGE(TAG_SERFLASH,"Wait for SPI Read Data Ready fails!\n"));
    }
...
static MS_BOOL _HAL_SERFLASH_WaitWriteDone(void)
{
    ...
    if (bRet == FALSE)
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ERR, ULOGE(TAG_SERFLASH,"Wait for Write to be done fails!\n"));
    }
...
static void _HAL_SERFLASH_ActiveFlash_Set_HW_WP(MS_BOOL bEnable)
{
    ...
    else
    {
        ...
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_NOTICE, ULOGD(TAG_SERFLASH,"msFlash_ActiveFlash_Set_HW_WP() is not defined in this system\n"));
...
static MS_BOOL _HAL_SetAeon_SPIMappingAddr(MS_U32 u32addr)
{
    ...
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH,"%s(0x%08X, 0x%08X)\n", __FUNCTION__, (int)u32addr, (int)u16MHEGAddr));
...
static MS_BOOL _HAL_SERFLASH_RIURead(MS_U32 u32Addr, MS_U32 u32Size, MS_U8 *pu8Data)
{
    ...
    {
        ULOGE(TAG_SERFLASH,"%s ENTRY fails!\n", __FUNCTION__);
        return FALSE;
    }
...
MS_BOOL HAL_SERFLASH_Set2XREAD(MS_BOOL b2XMode)
{
    ...
    {
        UNUSED(b2XMode);
        ULOGE(TAG_SERFLASH,"%s This flash does not support 2XREAD!!!\n", __FUNCTION__);
    }
...
{
    MS_BOOL Ret = FALSE;
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH,"%s(0x%08X)\n", __FUNCTION__, (int)u8FlashIndex));
    switch (u8FlashIndex)
    {
        case FLASH_ID0:
...
{
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH,"%s(0x%08X, 0x%08X, 0x%08X)\n", __FUNCTION__, (int)u32PMRegBaseAddr, (int)u32NonPMRegBaseAddr, (int)u32XiuBaseAddr));
    ...
    _hal_isp.VirtChipTopBaseAddr = u32NonPMRegBaseAddr + BK_CHIPTOP;
}
...
void HAL_SERFLASH_Init(void)
{
    ...
    CHIP_TOP_WRITE_MASK(REG_SPI_NONPM,SPI_MODE_ON,SPI_MODE_MASK);
}
    ...
    if (HAL_SERFLASH_ReadID(u8FlashId, sizeof(u8FlashId))== TRUE)
    {
        ...
        for (u32Index = 0; _hal_SERFLASH_table[u32Index].u8MID != 0; u32Index++)
        {
                ...
                )
            {
                ...
                DEBUG_SER_FLASH(E_SERFLASH_DBGLV_INFO,
                                ULOGI(TAG_SERFLASH,"Flash is detected (0x%04X, 0x%02X, 0x%02X, 0x%02X)\n",
       ...
       // printf("[%x]\n",_hal_SERFLASH.u16FlashType);
        ...
        if( bDetect != TRUE )
        {
            ...
            #endif
            DEBUG_SER_FLASH(E_SERFLASH_DBGLV_INFO,
                            ULOGI(TAG_SERFLASH,"Unsupport flash type (0x%02X, 0x%02X, 0x%02X), please add flash info to serial flash driver\n",
                                   u8FlashId[0],
                                   u8FlashId[1],
                                   u8FlashId[2]
                                   )
...
MS_BOOL HAL_SERFLASH_BlockErase(MS_U32 u32StartBlock, MS_U32 u32EndBlock, MS_BOOL bWait)
{
    ...
    {
        ULOGE(TAG_SERFLASH,"%s (0x%08X, 0x%08X, %d)\n", __FUNCTION__, (int)u32StartBlock, (int)u32EndBlock, bWait);
    ...
    for( u32I = u32StartBlock; u32I <= u32EndBlock; u32I++)
    {
        ...
        if(bWait == TRUE )
        {
            if(!_HAL_SERFLASH_WaitWriteDone())
            {
                ULOGE(TAG_SERFLASH,"%s : Wait Write Done Fail!!!\n", __FUNCTION__ );
                bRet = FALSE;
            }
            else
            {
...
MS_BOOL HAL_SERFLASH_SectorErase(MS_U32 u32SectorAddress)
{
    ...
    if (FALSE == MsOS_ObtainMutex(_s32SERFLASH_Mutex, SERFLASH_MUTEX_WAIT_TIME))
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH,"%s ENTRY fails!\n", __FUNCTION__));
        return bRet;
    }
        ...
        if (_hal_SERFLASH.u8MID == MID_ATMEL)
        {
            ...
            // programming sector protection
            {
                     ...
                     )
                {
                    ...
                    {
                        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH,"u8BlockProtectBits = 0x%X, u32LowerBound = 0x%X, u32UpperBound = 0x%X\n",
...
//u32StartBlock, u32EndBlock --> 0~285
  ...
  MS_U8 dybStatusReg=0;
    ...
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG,ULOGD(TAG_SERFLASH,"%s ENTRY \n", __FUNCTION__));
    MS_ASSERT( MsOS_In_Interrupt() == FALSE );
    if (FALSE == MsOS_ObtainMutex(_s32SERFLASH_Mutex, SERFLASH_MUTEX_WAIT_TIME))
    {
        ULOGE(TAG_SERFLASH,"%s ENTRY fails!\n", __FUNCTION__);
...
MS_BOOL HAL_SERFLASH_ReadID(MS_U8 *pu8Data, MS_U32 u32Size)
{
    ...
    for ( u32I = 0; u32I < u32Size; u32I++ )
    {
        ...
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH," 0x%02X", u8ptr[u32I]));
    }
    bRet = TRUE;
...
MS_U64 HAL_SERFLASH_ReadUID(void)
{
    ...
    for ( u32I = 0; u32I < u32Size; u32I++ )
    {
        ...
        u8ptr[u32I] = ISP_READ(REG_ISP_SPI_RDATA);
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH,"%s, %d 0x%02X\n",__FUNCTION__, (int)u32I, u8ptr[u32I]));
    }
...
MS_BOOL HAL_SERFLASH_ReadREMS4(MS_U8 * pu8Data, MS_U32 u32Size)
{
    ...
    for ( u32Index = 0; u32Index < u32Size; u32Index++ )
    {
        ...
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH," 0x%02X",  u8ptr[u32Index]));
    }
...
MS_BOOL HAL_SERFLASH_DMA(MS_U32 u32FlashStart, MS_U32 u32DRAMStart, MS_U32 u32Size)
{
    ...
    if (bRet == FALSE)
    {
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ERR, ULOGE(TAG_SERFLASH,"DMA timeout!\n"));
    }
...
MS_BOOL HAL_SERFLASH_ReadStatusReg(MS_U8 *pu8StatusReg)
{
    ...
    *pu8StatusReg = ISP_READ(REG_ISP_SPI_RDATA);
    ...
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, ULOGD(TAG_SERFLASH," 0x%02X", *pu8StatusReg));
...
MS_BOOL HAL_SPI_EnterIBPM(void)
{
    ...
    if((ISP_READ(REG_ISP_SPI_RDATA) & BIT(7)) == BIT(7))
    {
        ...
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG,
                        ULOGD(TAG_SERFLASH,"MXIC Security Register 0x%02X\n", ISP_READ(REG_ISP_SPI_RDATA)));
    }
...
MS_BOOL HAL_SPI_SingleBlockLock(MS_PHYADDR u32FlashAddr, MS_BOOL bLock)
{
    ...
    if ( _bIBPM != TRUE )
    {
        ULOGE(TAG_SERFLASH,"%s not in Individual Block Protect Mode\n", __FUNCTION__);
        MsOS_ReleaseMutex(_s32SERFLASH_Mutex);
        return bRet;
    }
                      ...
                      if(j==0xFFFFFFFF)
                        {
                            j = i;
                            *pu8BlockProtectBits = _hal_SERFLASH.pWriteProtectTable[j].u8BlockProtectBits;
                            DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG,ULOGD(TAG_SERFLASH,"pu8BlockProtectBits = %x\n",*pu8BlockProtectBits));
                        }
                      else
                        {
...
MS_U32 HAL_SERFLASH_WriteProtect_Area_Boundary(void)
{
    ...
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG,ULOGD(TAG_SERFLASH,"u8stats = %x\n",u8stats&_hal_SERFLASH.u8WrsrBlkProtect));
        ...
        )
    {
        ...
        {
            k = i;
            DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG,ULOGD(TAG_SERFLASH,"u32UpperBound = %x\n",(unsigned int)_hal_SERFLASH.pWriteProtectTable[k].u32UpperBound));
...
MS_BOOL HAL_WriteByte(MS_U32 u32RegAddr, MS_U8 u8Val)
{
    if (!u32RegAddr)
    {
        ULOGE(TAG_SERFLASH,"%s reg error!\n", __FUNCTION__);
        return FALSE;
    }
...
void HAL_FSP_WriteBufs(MS_U8 u8Idx, MS_U8 u8Data)
{
    ...
    case 9:
        ...
        break;
    default:
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ERR, ULOGE(TAG_SERFLASH,"HAL_FSP_WriteBufs fails\n"));
        break;
    }
}
...
MS_BOOL HAL_FSP_Write_Burst(MS_U32 u32Addr, MS_U32 u32Size, MS_U8* pu8Data)
{
    ...
    if(!bRet)
    {
        ULOGE(TAG_SERFLASH,"Write enable command Fail!!!!\r\n");
        return bRet;
    }
    ...
    for(u32K = 0; u32K < u32PageIdx; u32K++)
    {
        ...
        if(!bRet)
        {
            ULOGE(TAG_SERFLASH,"Write command Fail!!!!\r\n");
            return bRet;
        }
...
MS_BOOL HAL_FSP_Write(MS_U32 u32Addr, MS_U32 u32DataSize, MS_U8 *pu8Data)
{
    if(_bFSPMode)
    {
        ULOGE(TAG_SERFLASH,"Burst Mode Write Data!!!!\r\n");
        return HAL_FSP_Write_Burst(u32Addr, u32DataSize, pu8Data);
    }
    else
    {
        ULOGE(TAG_SERFLASH,"Normal Mode Write Data!!!!\r\n");
