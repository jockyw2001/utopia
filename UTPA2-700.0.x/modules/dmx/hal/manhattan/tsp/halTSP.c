...
#define TSP_CLKGEN2_REG(addr)       (*((volatile MS_U16*)(_virtRegBase + 0x1400UL + ((addr)<<2UL))))
    #define REG_CLKGEN2_TSN_CLKFI               0x0DUL
        ...
        //                                     6: TSOOUT
        //                                     7: Internal Demmod
    #define REG_CLKGEN2_TSN_CLKTS3              0x11UL
...
#define TSP_TOP_REG(addr)           (*((volatile MS_U16*)(_virtRegBase + 0x3c00UL + ((addr)<<2UL))))
    ...
    #define REG_TOP_TSO_MUX                     0x10UL
        ...
        #define REG_TOP_TSO_MUX_TSO_SHIFT                   12UL                //tso in mux
    ...
    #define REG_TOP_TSCONFIG                    0x57UL
        ...
        #define REG_TOP_TS_TS2_CFG_MASK                     0xC000UL
        ...
        //                     5: mspi
        // bit[15:14] ->  0: Disable
        //                     1: TS2 use I2S & GPIO pads
        //                     2: TS2 use all PAD_TS2 pads
...
MS_BOOL HAL_TSP_CSA_Set_CACtrl(MS_U8 u8EngId, MS_U8 u8SrcTSIF, MS_U32 u32Dst)
{
    ...
    MS_U32 u32CAInTS2[2]        = {TSP_CA0_INPUT_TSIF2, TSP_CA1_INPUT_TSIF2};
    ...
    MS_U32 u32CAOutPktDmx2[2]   = {TSP_CA0_OUTPUT_PKTDMX2, TSP_CA1_OUTPUT_PKTDMX2};
...
MS_BOOL HAL_TSP_CSA_Get_CACtrl(MS_U8 u8EngId, MS_U8* pu8SrcTSIF, MS_U32* pu32Dst)
{
    ...
    *pu32Dst = 0UL;
    ...
    {
        *pu32Dst = TSP_SRC_FROM_TSIF1;
    }
    else if(u32ScmbPath & u32CAOutPktDmx2[u8EngId])
    {
        *pu32Dst = TSP_SRC_FROM_TSIF2;
    }
...
MS_U32 HAL_TSP_PVR_StartingEngs_Get(MS_U32 u32PktDmxSrc)
{
    ...
    u32Src = 1UL << ((_HAL_REG32_R(&_TspCtrl[0].FIFO_Src) & TSP_PVR2_SRC_MASK) >> TSP_PVR2_SRC_SHIFT);
    if(u32PktDmxSrc & u32Src)
    {
...
MS_BOOL HAL_TSP_TsOutPadCfg(MS_U32 u32OutPad, MS_U32 u32OutPadMode, MS_U32 u32InPad, MS_BOOL bInParallel)
{
    ...
    if((u32OutPadMode == HAL_TSP_OUTPAD_S2P) || (u32OutPadMode == HAL_TSP_OUTPAD_S2P1))
    {
        ...
        // S2P mux
        _HAL_REG16_W(&(_TspCtrl5[0].TS_MUX_CFG_S2P), RESET_FLAG1(_HAL_REG16_R(&(_TspCtrl5[0].TS_MUX_CFG_S2P)), (TS_MUX_CFG_S2P0_MUX_MASK << u16S2pRegShift)));
        _HAL_REG16_W(&(_TspCtrl5[0].TS_MUX_CFG_S2P), SET_FLAG1(_HAL_REG16_R(&(_TspCtrl5[0].TS_MUX_CFG_S2P)), u16Data));
        TSP_TSO0_REG(REG_TSO0_CFG0) |= u16S2pCfg;

        // TSO out clk
        u16S2pCfg = ((u32OutPadMode == HAL_TSP_OUTPAD_S2P) ? REG_TSO_OUT_S2P0 : REG_TSO_OUT_S2P1);
        TSP_TS_SAMPLE_REG(REG_TSO_OUT_CLK_SEL) = ((TSP_TS_SAMPLE_REG(REG_TSO_OUT_CLK_SEL) & ~REG_TSO_OUT_CLK_SEL_MASK) | u16S2pCfg);  //TSO out (S2P)
...
void HAL_TSP_HwPatch(void)
{
    ...
    _HAL_REG32_W(&_TspCtrl[0].reg160C,
        _HAL_REG32_R(&_TspCtrl[0].reg160C)|(TSP_RM_DMA_GLITCH|/*TSP_DOUBLE_BUF_DESC|*/
            TSP_VQTX0_BLOCK_DIS|TSP_VQTX2_BLOCK_DIS|TSP_VQTX3_BLOCK_DIS));
    ...
    _HAL_REG32_W(&_TspCtrl[0].PVR2_Config,
        _HAL_REG32_R(&_TspCtrl[0].PVR2_Config)|(TSP_PVR2_PVR_ALIGN_EN   |
                                                TSP_PVR1_BLOCK_DIS  |
                                                TSP_PVR2_BLOCK_DIS  |
                                                TSP_V_BLOCK_DIS |
                                                TSP_V3D_BLOCK_DIS   |
...
{
    MS_U32 u32Flag[4] = {TSP_REMOVE_DUP_VIDEO_PKT, TSP_REMOVE_DUP_AUDIO_PKT, TSP_REMOVE_DUP_AUDIOB_PKT, TSP_REMOVE_DUP_VIDEO3D_PKT};
...
void HAL_TSP_Stc_ctrl(MS_U32 u32EngId, MS_U32 u32Sync)
{
    ...
    u32value = ((u32EngId == 0) ? REG_CLKGEN0_STC_CW_SEL : REG_CLKGEN0_STC1_CW_SEL);
    ...
    u32value = ((u32EngId == 0) ? REG_CLKGEN0_DC0_STC_CW_L : (REG_CLKGEN0_DC0_STC_CW_L+2));
    TSP_CLKGEN0_REG(u32value) = (MS_U16)(u32Sync & 0xFFFF);
    u32value = ((u32EngId == 0) ? REG_CLKGEN0_DC0_STC_CW_H : (REG_CLKGEN0_DC0_STC_CW_H+2));
    TSP_CLKGEN0_REG(u32value) = (MS_U16)(u32Sync >> 16UL);

    // set STC synth
    u32value = ((u32EngId == 0) ? REG_CLKGEN0_STC_CW_EN : REG_CLKGEN0_STC1_CW_EN);
    TSP_CLKGEN0_REG(REG_CLKGEN0_DC0_SYTNTH) &= ~u32value;
    TSP_CLKGEN0_REG(REG_CLKGEN0_DC0_SYTNTH) |= u32value;
    TSP_CLKGEN0_REG(REG_CLKGEN0_DC0_SYTNTH) &= ~u32value;
...
MS_U32 HAL_TSP_GetSTCSynth(MS_U32 u32EngId)
{
    ...
    u32value = ((u32EngId == 0) ? REG_CLKGEN0_DC0_STC_CW_H : (REG_CLKGEN0_DC0_STC_CW_H+2));
    u32sync |= (((MS_U32)TSP_CLKGEN0_REG(u32value)) << 16UL);
...
#define CKG_TSP_STC0       0x16A8UL  //0x2A
    ...
    #define CLK_STC1_DISABLE         0x1000UL
    #define CLK_STC1_INVERT          0x2000UL
    #define CLK_STC1_CLK_MASK        0xC000UL
...
#define CKG2_TSP_TSFI      0x1434UL  //0x0D
...
void HAL_TSP_PowerCtrl(MS_BOOL bOn)
{
    if (bOn)
    {
        ...
        _HAL_REG32L_W((REG32_L *)(_virtRegBase+CKG_TSP_STAMP),
            SET_FLAG1(_HAL_REG32L_R((REG32_L *)(_virtRegBase+CKG_TSP_STAMP)), (CLK_SYN_STC0_432M|CLK_SYN_STC1_432M)));
        ...
        // Enable STC1 clock
        _HAL_REG32L_W((REG32_L *)(_virtRegBase+CKG_TSP_STC0),
            RESET_FLAG1(_HAL_REG32L_R((REG32_L *)(_virtRegBase+CKG_TSP_STC0)), (CLK_STC1_DISABLE|CLK_STC1_INVERT|CLK_STC1_CLK_MASK)));
    ...
    else
    {
        ...
        // Disable STC1 clock
        _HAL_REG32L_W((REG32_L *)(_virtRegBase+CKG_TSP_STC0), SET_FLAG1(_HAL_REG32L_R((REG32_L *)(_virtRegBase+CKG_TSP_STC0)), CLK_STC1_DISABLE));
        ...
        {
            TSP_TOP_REG(REG_TOP_TS3_PE) &= ~REG_TOP_TS3_PE_MASK;
            _bTsPadUsed[3] = FALSE;
        }
    }
}
...
{
    MS_U32 u32Value = (TSP_MCU_CMD_SCMSTS_GET | ((MS_U32)u8FltId & 0xFFUL));
    _HAL_REG32_W(&_TspCtrl[0].MCU_Cmd, u32Value);
    while (0 != _HAL_REG32_R(&_TspCtrl[0].MCU_Cmd));
