...
static MS_PHY         _phyPVRBufMiuOffset = 0UL;
...
#define TSO_CLKGEN2_REG(addr)       (*((volatile MS_U16*)(_virtTSORegBase + 0x1400UL + ((addr)<<2UL))))
    #define REG_CLKGEN2_TSO1_IN                         0x10UL
        ...
        //                                     100: Sel TS4 Clk
        //                                     111: Sel Dmd Clk
    #define REG_CLKGEN2_MMT_IN                          0x19UL
        #define REG_CLKGEN2_MMT_IN_MASK                 0x1F00UL
        #define REG_CLKGEN2_MMT_IN_SHIFT                8UL
        #define REG_CLKGEN2_MMT_IN_DISABLE              0x0001UL
        #define REG_CLKGEN2_MMT_IN_INVERT               0x0002UL
        // bit[4:0]  -> 0: disable clock
        //                   1: invert clock
        //                   bit [4:2] -> 000: Sel TS0 Clk
        //                                     001: Sel TS1 Clk
...
#define TSP_TOP_REG(addr)           (*((volatile MS_U16*)(_virtTSORegBase + 0x3c00UL + ((addr)<<2UL))))
    ...
    #define REG_TOP_TS2_CONFIG                          0x5AUL
        #define REG_TOP_TS2_CONFIG_MASK                 0x7000UL
            #define REG_TOP_TS2_CONFIG_PARALLEL_IN      0x2000UL
            #define REG_TOP_TS2_CONFIG_SERIAL_IN        0x1000UL
...
#define TSP_TSP5_REG(addr)                (*((volatile MS_U16*)(_virtTSORegBase + 0xc7600 + ((addr)<<2))))
    ...
    #define REG_TSP5_MMT_MUX                            0x16
        #define REG_TSP5_MMT_MUX_SHIFT                  0UL
        #define REG_TSP5_MMT_MUX_MASK                   0x000FUL
...
void HAL_TSO_SetBank(MS_VIRT virtBankAddr)
{
    ...
    _TSOCtrl2 = (REG_Ctrl_TSO2*)(_virtTSORegBase+ REG_CTRL_BASE_TSO2);
...
void HAL_TSO_Init(void)
{
    ...
    //reset MMT
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_21), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_21)), TSO_MMT_SW_RST));
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_21), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_21)), TSO_MMT_SW_RST));
...
void HAL_TSO_PowerCtrl(MS_BOOL bOn)
{
    if (bOn)
    {
        TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_IN) &= ~REG_CLKGEN0_TSO_TRACE_MASK;
        ...
        TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN) &= ~REG_CLKGEN2_TSO1_IN_MASK;
        TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN) &= ~REG_CLKGEN2_TSO2_IN_MASK;
    }
    else
    {
        TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_OUT_CLK) |= REG_CLKGEN0_TSO_OUT_CLK_DISABLE;
        TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_IN) |= REG_CLKGEN0_TSO_TRACE_DISABLE;
        TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_IN) |= REG_CLKGEN0_TSO_IN_DISABLE;
        TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN) |= REG_CLKGEN2_TSO1_IN_DISABLE; 
        TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN) |= REG_CLKGEN2_TSO2_IN_DISABLE; 
    }
}
#endif
...
void HAL_TSO_Recover_TSOutMode(MS_U8 u8Eng)
{
    ...
    TSP_TOP_REG(REG_TOP_TS4_CFG) = (TSP_TOP_REG(REG_TOP_TS4_CFG) & ~REG_TOP_TS_OUT_MODE_MASK) | _stOutPadCtrl.u16TSOutModeOld[u8Eng];
}
...
MS_BOOL HAL_TSO_OutPad(MS_U8 u8Eng, MS_U16* pu16OutPad, MS_BOOL bSet)
{
    if(bSet)
    {
        ...
        _stOutPadCtrl.u16TSOutModeOld[u8Eng] = TSP_TOP_REG(REG_TOP_TS4_CFG) & REG_TOP_TS_OUT_MODE_MASK;
        TSP_TOP_REG(REG_TOP_TS_CONFIG)   = TSP_TOP_REG(REG_TOP_TS_CONFIG) & ~REG_TOP_TS1_CONFIG_MASK;
        TSP_TOP_REG(REG_TOP_TS4_CFG)  = (TSP_TOP_REG(REG_TOP_TS4_CFG) & ~REG_TOP_TS_OUT_MODE_MASK) | REG_TOP_TS1_OUT_MODE_TSO;
    }
    else
    {
        if(((TSP_TOP_REG(REG_TOP_TS4_CFG) & REG_TOP_TS_OUT_MODE_MASK) == REG_TOP_TS1_OUT_MODE_TSO)
            && ((TSP_TOP_REG(REG_TOP_TS_CONFIG) & REG_TOP_TS1_CONFIG_MASK) == 0))
        {
...
MS_BOOL HAL_TSO_SelPad(MS_U8 u8Eng, MS_U8 u8TsIf, MS_U16 u16InPadSel, MS_BOOL bParallel)
{
    ...
    switch(u8TsIf)
    {
        ...
        case HAL_TSO_TSIF_LIVE3:
            ...
            break;
        case HAL_TSO_TSIF_LIVE_MMT:
            u16MuxReg = REG_TSP5_MMT_MUX;
            u16MuxRegMask = REG_TSP5_MMT_MUX_MASK;
            u16RegShift = REG_TSP5_MMT_MUX_SHIFT;
            break;
        default:
            return FALSE;
    }
...
MS_BOOL HAL_TSO_Set_InClk(MS_U8 u8Eng, MS_U8 u8TsIf, MS_U16 u16ClkSel, MS_BOOL bClkInvert, MS_BOOL bEnable)
{
    ...
    switch(u8TsIf)
    {
        ...
        case HAL_TSO_TSIF_LIVE3:
            ...
            break;
        case HAL_TSO_TSIF_LIVE_MMT:
            u16Reg = REG_CLKGEN2_MMT_IN;
            u16RegMask = REG_CLKGEN2_MMT_IN_MASK;
            u16RegShift = REG_CLKGEN2_MMT_IN_SHIFT;
            u16value = TSO_CLKGEN2_REG(u16Reg) & ~u16RegMask;
            break;
...
MS_BOOL HAL_TSO_GetInputTSIF_Status(MS_U8 u8Eng, MS_U8 u8TsIf, MS_U16* pu16Pad, MS_BOOL* pbClkInvert, MS_BOOL* pbExtSync, MS_BOOL* pbParl)
{
    ...
    switch(u8TsIf)
    {
        ...
        case HAL_TSO_TSIF_LIVE3:
            ...
            break;
        case HAL_TSO_TSIF_LIVE_MMT:
            u16Reg = REG_TSP5_MMT_MUX;
            u16RegMask = REG_TSP5_MMT_MUX_MASK;
            u16RegShift = REG_TSP5_MMT_MUX_SHIFT;
            break;
    ...
    *pu16Pad = (TSP_TSP5_REG(u16Reg) & (u16RegMask << u16RegShift)) >> u16RegShift;
    ...
    switch(u8TsIf)
    {
        case HAL_TSO_TSIF_LIVE1:
            ...
            break;
        case HAL_TSO_TSIF_LIVE2:
            u16data = (TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN) & REG_CLKGEN2_TSO1_IN_MASK) >> REG_CLKGEN2_TSO1_IN_SHIFT;
            reg16 = &(_TSOCtrl->TSO_CH0_IF5_CFG2);
            break;
        case HAL_TSO_TSIF_LIVE3:
            u16data = (TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN) & REG_CLKGEN2_TSO2_IN_MASK) >> REG_CLKGEN2_TSO2_IN_SHIFT;
            reg16 = &(_TSOCtrl->TSO_CH0_IF6_CFG2);
            break;
        case HAL_TSO_TSIF_LIVE_MMT:
            u16data = (TSO_CLKGEN2_REG(REG_CLKGEN2_MMT_IN) & REG_CLKGEN2_MMT_IN_MASK) >> REG_CLKGEN2_MMT_IN_SHIFT;
            *pbExtSync = _HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_0D)) & TSO_MMT_EXTSYNC;
            *pbParl = _HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_0D)) & TSO_MMT_PARL;
...
MS_BOOL HAL_TSO_PreTsoOutClk(MS_U8 u8Eng, MS_U16* pu16PreTsoOutSel, MS_BOOL bSet)
{
    if(bSet == TRUE)
    {
        if(*pu16PreTsoOutSel > HAL_PRE_TSO_OUT_SEL_TS4IN)
        {
            return FALSE;
        }
...
MS_BOOL HAL_TSO_ChIf_Cfg(MS_U8 u8Eng, MS_U8 u8ChIf, MS_U16 u16Cfg, MS_BOOL bEnable)
{
    ...
    switch(u8ChIf)
    {
        ...
        case HAL_TSO_TSIF_LIVE3:
            ...
            break;
        case HAL_TSO_TSIF_LIVE_MMT:
            pReg = &(_TSOCtrl2->TSO_CFG_20);
            u16Cfg = HAL_TSO_MMT_Cfg_Map(u16Cfg);
            break;
        default:
            return FALSE;
    }
...
MS_U16 HAL_TSO_MMT_Cfg_Map(MS_U16 u16Cfg)
{
    switch(u16Cfg)
    {
        case TSO_CHCFG_TS_SIN_C0:
            return TSO_MMT_TS_SIN_C0;
        case TSO_CHCFG_TS_SIN_C1:
            return TSO_MMT_TS_SIN_C1;
        case TSO_CHCFG_P_SEL:
            return TSO_MMT_PARL;
        case TSO_CHCFG_EXT_SYNC_SEL:
            return TSO_MMT_EXTSYNC;
        default:
            return 0;
    }
}
...
{
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_0D), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_0D)), TSP_FLUSH_EN));
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_0D), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_0D)), TSP_FLUSH_EN));
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_RST_WADR));
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_RST_WADR));
}
...
void HAL_TSO_PVR_SetBuffer(MS_U8 u8PVRId, MS_PHY phyBufStart0, MS_PHY phyBufStart1, MS_U32 u32BufSize0, MS_U32 u32BufSize1)
{
    ...
    _phyPVRBufMiuOffset = _HAL_TSO_MIU_OFFSET(phyBufStart0);
    _HAL_REG32_W(&(_TSOCtrl2->TSO_PVR_Head1), ((MS_U32)((phyBufStart0-_phyPVRBufMiuOffset)>> MIU_BUS)));
    _HAL_REG32_W(&(_TSOCtrl2->TSO_PVR_Tail), ((MS_U32)((phyBufEnd-_phyPVRBufMiuOffset)>> MIU_BUS)));

    phyBufEnd = phyBufStart1+ u32BufSize1;

    _HAL_REG32_W(&(_TSOCtrl2->TSO_PVR_Head2), ((MS_U32)((phyBufStart1-_phyPVRBufMiuOffset)>> MIU_BUS)));
    _HAL_REG32_W(&(_TSOCtrl2->TSO_PVR_Tail2), ((MS_U32)((phyBufEnd-_phyPVRBufMiuOffset)>> MIU_BUS)));


    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00),
        SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_PINGPONG));

    // flush PVR buffer
    HAL_TSO_PVR_WaitFlush(u8PVRId);
}
...
MS_PHY HAL_TSO_PVR_GetBufWrite(MS_U8 u8PVRId)
{
    ...
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_0D),
        RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_0D)), TSO_CH_BW_WP_LD));

    u32value = _HAL_REG32_R(&(_TSOCtrl2->TSO_PVR_WPTR));

    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_0D),
        SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_0D)), TSO_CH_BW_WP_LD));

    return ((((MS_PHY)u32value) << MIU_BUS) + _phyPVRBufMiuOffset);
...
void HAL_TSO_PVR_Enable(MS_U8 u8PVRId,MS_BOOL bEnable)
{
    if (bEnable)
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), (_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)) & ~TSO_BURST_LEN_MASK) | TSO_BURST_LEN_4);
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_0D), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_0D)), TSO_PVR_DMA_FLUSH_EN));
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_ENABLE));
    }
    else
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_ENABLE));
    }
}
...
void HAL_TSO_PVR_PacketMode(MS_U8 u8PVRId, MS_BOOL bSet)
{
    if (bSet)
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_RECORD192_EN));
    }
    else
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_RECORD192_EN));
    }
}
...
{
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_LPCR1_WLD));

    _HAL_REG32_W(&(_TSOCtrl2->PVR1_LPcr1),u32Stamp);

    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_LPCR1_WLD));
}
...
MS_U32  HAL_TSO_GetPVRTimeStamp(MS_U8 u8PVRId)
{
    ...
    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_LPCR1_RLD));

    u32lpcr = _HAL_REG32_R(&(_TSOCtrl2->PVR1_LPcr1));

    _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_PVR_LPCR1_RLD));
...
void HAL_TSO_SetPVRTimeStampClk(MS_U8 u8PVRId, MS_U32 u32ClkSrc)
{
    ...
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_14),
            RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_14)), TSO_PVR_CLK_STAMP_27_EN));
    }
    else // 27M
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_14),
            SET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_14)), TSO_PVR_CLK_STAMP_27_EN));
    }
...
{
    return ((_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)) & TSO_PVR_ENABLE) > 0);
}

void HAL_TSO_PVR_Src(MS_U32 u32Src)
{
    if(u32Src == HAL_TSO_PVR_SVQ)//from SVQ
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_14), (_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_14)) & ~TSO_PVR_SRC_MASK) | TSO_PVR_SRC_SVQ);
    }
    else if(u32Src == HAL_TSO_PVR_MMT)
    {   
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_14), (_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_14)) & ~TSO_PVR_SRC_MASK) | TSO_PVR_SRC_MMT);
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_00), RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_00)), TSO_RECORD192_EN)); //MMT mode will add timestamp automatically
    }
    else
    {
        _HAL_REG16_W(&(_TSOCtrl2->TSO_CFG_14),
            RESET_FLAG1(_HAL_REG16_R(&(_TSOCtrl2->TSO_CFG_14)), TSO_PVR_SRC_MASK));
    }
}
...
MS_BOOL HAL_TSO_ChIf_Ng_Protocol_ID(MS_U8 u8Eng, MS_U8 u8ChIf, MS_U16 u16ID)
{
    ...
    switch(u8ChIf)
    {
        ...
        case HAL_TSO_TSIF_LIVE2:
            ...
            break;
        case HAL_TSO_TSIF_LIVE3:
            p32Reg = &(_TSOCtrl1->TSO_DONGLE_TSIF6);
            break;
        default:
            return FALSE;
    }
...
MS_BOOL HAL_TSO_SaveRegs(void)
{
    ...
    _u16TSOTopReg[0][0] =  TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_IN);
    _u16TSOTopReg[0][1] =  TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_OUT_PHASE);
    _u16TSOTopReg[0][2] =  TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_OUT_CLK);
    _u16TSOTopReg[0][3] =  TSO_CLKGEN0_REG(REG_CLKGEN0_RESERVED0);
    _u16TSOTopReg[0][4] =  TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN);
    _u16TSOTopReg[0][5] =  TSO_CLKGEN2_REG(REG_CLKGEN2_MMT_IN);
    _u16TSOTopReg[0][6] =  TSP_TS_SAMPLE_REG(REG_TSO_OUT_CLK_SEL);

    _u16TSOTopReg[1][0] =  TSP_TOP_REG(REG_TOP_TSO_EVD);
    _u16TSOTopReg[1][1] =  TSP_TOP_REG(REG_TOP_TS4_CFG);
    _u16TSOTopReg[1][2] =  TSP_TOP_REG(REG_TOP_TS_CONFIG);
    _u16TSOTopReg[1][3] =  TSP_TOP_REG(REG_TOP_TS2_CONFIG);
    _u16TSOTopReg[1][4] =  TSP_TOP_REG(REG_TOP_TS3_CONFIG);

    _u16TSOTopReg[2][0] =  TSP_TSP5_REG(REG_TSP5_TSOIN_MUX);
    _u16TSOTopReg[2][1] =  TSP_TSP5_REG(REG_TSP5_TSOOUT_MUX);
    _u16TSOTopReg[2][2] =  TSP_TSP5_REG(REG_TSP5_MMT_MUX);
...
MS_BOOL HAL_TSO_RestoreRegs(void)
{
    ...
    TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_IN) = _u16TSOTopReg[0][0];
    TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_OUT_PHASE) = _u16TSOTopReg[0][1];
    TSO_CLKGEN0_REG(REG_CLKGEN0_TSO_OUT_CLK) = _u16TSOTopReg[0][2];
    TSO_CLKGEN0_REG(REG_CLKGEN0_RESERVED0) = _u16TSOTopReg[0][3];
    TSO_CLKGEN2_REG(REG_CLKGEN2_TSO1_IN) = _u16TSOTopReg[0][4];
    TSO_CLKGEN2_REG(REG_CLKGEN2_MMT_IN) = _u16TSOTopReg[0][5];
    TSP_TS_SAMPLE_REG(REG_TSO_OUT_CLK_SEL) = _u16TSOTopReg[0][6];

    TSP_TOP_REG(REG_TOP_TSO_EVD) = _u16TSOTopReg[1][0];
    TSP_TOP_REG(REG_TOP_TS4_CFG) = _u16TSOTopReg[1][1];
    TSP_TOP_REG(REG_TOP_TS_CONFIG) = _u16TSOTopReg[1][2] ;
    TSP_TOP_REG(REG_TOP_TS2_CONFIG) = _u16TSOTopReg[1][3];
    TSP_TOP_REG(REG_TOP_TS3_CONFIG) = _u16TSOTopReg[1][4];

    TSP_TSP5_REG(REG_TSP5_TSOIN_MUX) = _u16TSOTopReg[2][0];
    TSP_TSP5_REG(REG_TSP5_TSOOUT_MUX) = _u16TSOTopReg[2][1];
    TSP_TSP5_REG(REG_TSP5_MMT_MUX) = _u16TSOTopReg[2][2];
