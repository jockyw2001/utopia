...
static MS_BOOL _bTsPadUsed[TSP_TS_PAD_NUM] = {[0 ... (TSP_TS_PAD_NUM-1)] = FALSE };
...
#define TSP_TOP_REG(addr)           (*((volatile MS_U16*)(_virtRegBase + 0x3c00UL + ((addr)<<2UL))))
    ...
    #define REG_TOP_TSCONFIG                    0x57UL
        ...
        #define REG_TOP_TS_TS0_CFG_PARALLEL                0x0100UL
        #define REG_TOP_TS_TS0_CFG_SERIAL                  0x0200UL

        #define REG_TOP_TS_TS1_CFG_MASK                    0x3800UL
        #define REG_TOP_TS_TS1_CFG_PARALLEL                0x0800UL
        #define REG_TOP_TS_TS1_CFG_SERIAL                  0x1800UL
        // bit[10:8] ->    0: Disable
...
void HAL_TSP_SelPad(MS_U32 u32EngId, MS_U32 u32Flow, MS_U32 u32Pad)
{
    ...
    MS_ASSERT(TSP_TS_PAD_NUM >= u32Pad);
...
void HAL_TSP_SelPad_Parl(MS_U32 u32EngId, MS_BOOL bParl, MS_U32 u32Flow)
{
    ...
    u32Pad = _u32FlowPadMap[u32Flow];
    if (bParl)  // parallel
    {
        switch (u32Pad)
        {
            case TSP_MUX_TS0:
                TSP_TOP_REG(REG_TOP_TSCONFIG) = (TSP_TOP_REG(REG_TOP_TSCONFIG) & ~REG_TOP_TS_TS0_CFG_MASK) | REG_TOP_TS_TS0_CFG_PARALLEL;
                break;
            case TSP_MUX_TS1:
                TSP_TOP_REG(REG_TOP_TSCONFIG) = (TSP_TOP_REG(REG_TOP_TSCONFIG) & ~REG_TOP_TS_TS1_CFG_MASK) | REG_TOP_TS_TS1_CFG_PARALLEL;
                break;
            default:
                break;
        }
    ...
    else        // serial
    {
        ...
        {
            case TSP_MUX_TS0:
                TSP_TOP_REG(REG_TOP_TSCONFIG) = (TSP_TOP_REG(REG_TOP_TSCONFIG) & ~REG_TOP_TS_TS0_CFG_MASK) | REG_TOP_TS_TS0_CFG_SERIAL;
                break;
            case TSP_MUX_TS1:
                TSP_TOP_REG(REG_TOP_TSCONFIG) = (TSP_TOP_REG(REG_TOP_TSCONFIG) & ~REG_TOP_TS_TS1_CFG_MASK) | REG_TOP_TS_TS1_CFG_SERIAL;
                break;
            default:
                break;
        }
...
void HAL_TSP_RemoveDupAVPkt(MS_BOOL bEnable)
{
    if(bEnable)
    {
        _HAL_REG32_W(&_TspCtrl[0].PktChkSizeFilein,
            _HAL_REG32_R(&_TspCtrl[0].PktChkSizeFilein) | (TSP_DUP_PKT_SKIP_VD|TSP_DUP_PKT_SKIP_V3D|TSP_DUP_PKT_SKIP_AV));
    }
    else
    {
        _HAL_REG32_W(&_TspCtrl[0].PktChkSizeFilein,
...
{
    MS_U32 u32Flag[4] = {TSP_DUP_PKT_SKIP_VD, TSP_DUP_PKT_SKIP_AV, 0, TSP_DUP_PKT_SKIP_V3D};
...
void HAL_TSP_PS_Path_Enable(MS_U32 u32TsDmaCtrl)
{
    switch (u32TsDmaCtrl)
    {
        case TSP_TSDMA_CTRL_VPES0:
            ...
            {
                _HAL_REG32_W(&_TspCtrl[0].Hw_Config4,
                    RESET_FLAG1(_HAL_REG32_R(&_TspCtrl[0].Hw_Config4), TSP_HW_CFG4_PS_AUD_EN));
                _HAL_REG32_W(&_TspCtrl[0].Dnld_Ctrl2,
                    RESET_FLAG1(_HAL_REG32_R(&_TspCtrl[0].Dnld_Ctrl2), TSP_PS_VID3D_EN));
        ...
        case TSP_TSDMA_CTRL_APES0:
            ...
            {
                _HAL_REG32_W(&_TspCtrl[0].Hw_Config4,
                    RESET_FLAG1(_HAL_REG32_R(&_TspCtrl[0].Hw_Config4), TSP_HW_CFG4_PS_VID_EN));
                _HAL_REG32_W(&_TspCtrl[0].Dnld_Ctrl2,
                    RESET_FLAG1(_HAL_REG32_R(&_TspCtrl[0].Dnld_Ctrl2), TSP_PS_VID3D_EN));
...
void HAL_TSP_HwPatch(void)
{
    ...
    _HAL_REG32_W(&_TspCtrl3[0].Hw_Config,
                SET_FLAG1(_HAL_REG32_R(&_TspCtrl3[0].Hw_Config), TSP_PREVENT_SRAM_COLLISION));
    ...
    _HAL_REG16_W(&_TspCtrl2[0].HwCfg0, _HAL_REG16_R(&_TspCtrl2[0].HwCfg0) | (PREVENT_SRAM_COLLISION|MASK_SCR_VID_EN|MASK_SCR_VID_3D_EN|MASK_SCR_AUD_EN));

    _HAL_REG16_W(&_TspCtrl2[0].HwCfg2, _HAL_REG16_R(&_TspCtrl2[0].HwCfg2) | REG_RST_CC_MODE);

    //_HAL_REG32_W(&_TspCtrl3[0].Hw_ECO, _HAL_REG32_R(&_TspCtrl3[0].Hw_ECO) | TSP_TIMESTAMP_RING | TSP_LPCR_RING);
    //_HAL_REG32_W(&_TspCtrl3[0].Hw_Semaphore1, _HAL_REG32_R(&_TspCtrl3[0].Hw_Semaphore1) | TSP_TIMESTAMP_ECO);
...
void HAL_ResetAll(void)
{
 ...
 #define CKG_TSP_STC0       0x16A8UL  //0x2a
    ...
    #define CLK_PAR_CLK_192          0x0040UL
...
void HAL_TSP_PowerCtrl(MS_BOOL bOn)
{
    if (bOn)
    {
        ...
        //Set CLK_PARSER_192
        _HAL_REG32L_W((REG32_L *)(_virtRegBase+CKG_TSP_STC0),
            SET_FLAG1(_HAL_REG32L_R((REG32_L *)(_virtRegBase+CKG_TSP_STC0)), (CLK_PAR_CLK_192)));
...
{
    MS_U32 u32Value = TSP_MCU_CMD_SCMSTS_GET | (MS_U32)(u8FltId & 0xFF);
...
MS_U32 HAL_TSP_Get_DisContiCnt(TSP_DisconPktCnt_Info* TspDisconPktCntInfo)
{
    if(TspDisconPktCntInfo->TspCmd == TSP_DEBUG_CMD_CLEAR)
    {
        ...
        _HAL_REG32_W(&(_TspCtrl[0].DbgInfo_Ctrl1),
            _HAL_REG32_R(&(_TspCtrl[0].DbgInfo_Ctrl1)) | (TSP_DISCONTI_VD_CLR | TSP_DISCONTI_V3D_CLR | TSP_DISCONTI_AUD_CLR));
        _HAL_REG32_W(&(_TspCtrl[0].DbgInfo_Ctrl1),
            _HAL_REG32_R(&(_TspCtrl[0].DbgInfo_Ctrl1)) & ~(TSP_DISCONTI_VD_CLR | TSP_DISCONTI_V3D_CLR | TSP_DISCONTI_AUD_CLR));
    }
