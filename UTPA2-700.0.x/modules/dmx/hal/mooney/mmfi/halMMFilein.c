...
MS_BOOL HAL_MMFI_Chk_CmdQResetDone(MS_U32 u32Path)
{
    ...
    if(u32Path == MMFI_RST_CMDQ_AU)
    {
         _HAL_REG32_W(&_MFCtrl_AU->SWRst_HWInt, (_HAL_REG32_R(&_MFCtrl_AU->SWRst_HWInt) | MMFI_RST_TSIF_AU));
        _HAL_REG32_W(&_MFCtrl_AU->SWRst_HWInt, (_HAL_REG32_R(&_MFCtrl_AU->SWRst_HWInt) & ~MMFI_RST_TSIF_AU));
    }
    else
    {
        _HAL_REG32_W(&_MFCtrl_AU->SWRst_HWInt, (_HAL_REG32_R(&_MFCtrl_AU->SWRst_HWInt) | MMFI_RST_TSIF_VD));
        _HAL_REG32_W(&_MFCtrl_AU->SWRst_HWInt, (_HAL_REG32_R(&_MFCtrl_AU->SWRst_HWInt) & ~MMFI_RST_TSIF_VD));
    }
