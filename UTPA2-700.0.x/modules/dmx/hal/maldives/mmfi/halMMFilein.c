...
static MS_U32   _u32MMFIRegBase = 0;
...
void HAL_MMFI_SetBank(MS_U32 u32BankAddr)
{
    _u32MMFIRegBase = u32BankAddr;
    _MFCtrl = (REG_Ctrl_MMFI*)(_u32MMFIRegBase+ REG_CTRL_BASE_MMFI0);
    _MFCtrl2 = (REG_Ctrl_MMFI2*)(_u32MMFIRegBase + REG_CTRL2);
}
...
void HAL_MMFI_PidFlt_Set(MS_U8 u8Eng, MS_U8 u8Idx, MS_U16 u16PID, MS_U32 u32entype)
{
    ...
    if(u8Idx < MMFI_PIDFLT_GROUP0)
    {
        Reg = &(_MFCtrl[u8Eng].PidFlt[u8Idx]);
    }
    else
    {
        Reg = &(_MFCtrl2[0].PidFlt[u8Eng][u8Idx - MMFI_PIDFLT_GROUP0]);
    }

    _HAL_REG32_W(Reg, (MS_U32)u16PID | u32entype);
}
...
void HAL_MMFI_PidFlt_SetPid(MS_U8 u8Eng, MS_U8 u8Idx, MS_U16 u16PID)
{
    ...
    u32data = (_HAL_REG32_R(Reg) & ~MMFI_PIDFLT_PID_MASK) | (MS_U32)u16PID;
