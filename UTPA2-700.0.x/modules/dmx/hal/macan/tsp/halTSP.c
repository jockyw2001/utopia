...
{
    // Fixed filein_192 timestamp ring back first issue
    //_HAL_REG16_W(&_TspCtrl5[0].HwCfg1, SET_FLAG1(_HAL_REG16_R(&_TspCtrl5[0].HwCfg1), TSP_FIXED_TIMESTAMP_RING_BACK_EN|TSP_FIXED_LPCR_RING_BANK_EN));
    _HAL_REG16_W(&_TspCtrl3[0].HWeco0,
                SET_FLAG1(_HAL_REG16_R(&_TspCtrl3[0].HWeco0), HW_ECO_INIT_STAMP_RSTART_EN));

    // Enable update match_pid_scr when the pkt is with PUSI
    _HAL_REG16_W(&_TspCtrl2[0].HW_Cfg,
                SET_FLAG1(_HAL_REG16_R(&_TspCtrl2[0].HW_Cfg), TSP_UPDATE_MATCH_PID_PUSI));
