...
{
    _HAL_REG16_W(&(_MFCtrl[u8Eng].CFG2), _HAL_REG16_R(&(_MFCtrl[u8Eng].CFG2)) | MMFI_CFG2_FILEIN_PAUSE);
...
{
    MS_U16 u16data = (_HAL_REG16_R(&(_MFCtrl[u8Eng].FileIn_Timer)) & ~MMFI_FILEIN_TIMER_MASK) | ((MS_U16)(u8timer&0xFF) << MMFI_FILEIN_TIMER_SHIFT);
    _HAL_REG16_W(&(_MFCtrl[u8Eng].FileIn_Timer), u16data);
}
...
void HAL_MMFI_Cfg2_Enable(MS_U8 u8Eng, MS_U16 u16CfgItem, MS_BOOL benable)
{
    if(benable)
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].CFG2), (_HAL_REG16_R(&(_MFCtrl[u8Eng].CFG2)) | u16CfgItem));
    }
    else
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].CFG2), (_HAL_REG16_R(&(_MFCtrl[u8Eng].CFG2)) & ~u16CfgItem));
...
{
    _HAL_REG16_W(&(_MFCtrl[u8Eng].CFG2), u16CfglItem);
}

MS_U16 HAL_MMFI_Cfg2_Get(MS_U8 u8Eng)
{
    return (_HAL_REG16_R(&(_MFCtrl[u8Eng].CFG2)));
}
...
void HAL_MMFI_Cfg3_Enable(MS_U8 u8Eng, MS_U16 u16CfgItem, MS_BOOL benable)
{
    if(benable)
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].CFG3), (_HAL_REG16_R(&(_MFCtrl[u8Eng].CFG3)) | u16CfgItem));
    }
    else
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].CFG3), (_HAL_REG16_R(&(_MFCtrl[u8Eng].CFG3)) & ~u16CfgItem));
...
{
    _HAL_REG16_W(&(_MFCtrl[u8Eng].CFG3), u16CfglItem);
}

MS_U16 HAL_MMFI_Cfg3_Get(MS_U8 u8Eng)
{
    return (_HAL_REG16_R(&(_MFCtrl[u8Eng].CFG3)));
}
...
{
    _HAL_REG16_W(&(_MFCtrl1[0].SWRst), (_HAL_REG16_R(&(_MFCtrl1[0].SWRst)) & ~MMFI_SW_RSTZ_MMFILEIN_DISABLE));
    _HAL_REG16_W(&(_MFCtrl1[0].SWRst), (_HAL_REG16_R(&(_MFCtrl1[0].SWRst)) | MMFI_SW_RSTZ_MMFILEIN_DISABLE));
}

void HAL_MMFI_Reset_SubItem(MS_U16 u16RstItem)
{
    _HAL_REG16_W(&(_MFCtrl1[0].SWRst), (_HAL_REG16_R(&(_MFCtrl1[0].SWRst)) | u16RstItem));
    _HAL_REG16_W(&(_MFCtrl1[0].SWRst), (_HAL_REG16_R(&(_MFCtrl1[0].SWRst)) & ~u16RstItem));
}

void HAL_MMFI_Reset_All(void)
{
    MS_U16 u16data = _HAL_REG16_R(&(_MFCtrl1[0].SWRst)) & ~MMFI_SWRST_MASK;

    _HAL_REG16_W(&(_MFCtrl1[0].SWRst), u16data | MMFI_RST_ALL);
    _HAL_REG16_W(&(_MFCtrl1[0].SWRst), (u16data & ~MMFI_SWRST_MASK) | MMFI_SW_RSTZ_MMFILEIN_DISABLE);
}

void HAL_MMFI_HWInt_Enable(MS_BOOL benable, MS_U16 u16init)
{
    MS_U16 u16data = _HAL_REG16_R(&(_MFCtrl1[0].HWInt)) & ~u16init;

    if(benable)
    {
        _HAL_REG16_W(&(_MFCtrl1[0].HWInt), (u16data | u16init));
    }
    else
    {
        _HAL_REG16_W(&(_MFCtrl1[0].HWInt), u16data);
    }
}

void HAL_MMFI_HWInt_Clear(MS_U16 u16Int)
{
    _HAL_REG16_W(&(_MFCtrl1[0].HWInt), (_HAL_REG16_R(&(_MFCtrl1[0].HWInt)) | u16Int));
    _HAL_REG16_W(&(_MFCtrl1[0].HWInt), (_HAL_REG16_R(&(_MFCtrl1[0].HWInt)) & ~u16Int));
}

MS_U32 HAL_MMFI_HWInt_Status(void)
{
    return (MS_U32)(_HAL_REG16_R(&(_MFCtrl1[0].HWInt)) & MMFI_HWINT_STS_MASK);
}
...
void HAL_MMFI_TimeStampClk_Set(MS_U8 u8Eng, MS_U32 u32ClkSrc)
{
    ...
    {
        _HAL_REG16_W(&(_MFCtrl1[0].SWRst), (_HAL_REG16_R(&(_MFCtrl1[0].SWRst)) & ~u16Mask));
    }
    else                   // 27M
    {
        _HAL_REG16_W(&(_MFCtrl1[0].SWRst), (_HAL_REG16_R(&(_MFCtrl1[0].SWRst)) | u16Mask));
...
MS_BOOL HAL_MMFI_RVU_Enable(MS_U8 u8Eng, MS_BOOL bEnable)
{
    if(bEnable)
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].RVU), _HAL_REG16_R(&(_MFCtrl[u8Eng].RVU)) | MMFI_RVU_EN);
    }
    else
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].RVU), _HAL_REG16_R(&(_MFCtrl[u8Eng].RVU)) & ~MMFI_RVU_EN);
...
MS_BOOL HAL_MMFI_RVU_Timestamp_Enable(MS_U8 u8Eng, MS_BOOL bEnable)
{
    if(bEnable)
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].RVU), _HAL_REG16_R(&(_MFCtrl[u8Eng].RVU)) | MMFI_RVU_TIMESTAMP_EN);
    }
    else
    {
        _HAL_REG16_W(&(_MFCtrl[u8Eng].RVU), _HAL_REG16_R(&(_MFCtrl[u8Eng].RVU)) & ~MMFI_RVU_TIMESTAMP_EN);
