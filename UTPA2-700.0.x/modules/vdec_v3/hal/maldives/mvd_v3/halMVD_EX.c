...
typedef struct
{
    ...
    MS_U8 u8IsAliveCount[MAX_DEC_NUM];
...
MS_BOOL HAL_MVD_SetSpeed(MS_U8 u8Idx, MVD_SpeedType eSpeedType, MS_U8 u8Multiple)
{
    ...
    if(pMVDHalContext->bTrickPlay2xAVSync[u8Idx] == TRUE && ((eSpeedType == E_MVD_SPEED_FAST && u8Multiple == 2)||(eSpeedType == E_MVD_SPEED_SLOW)))
    {
        mvdcmd.Arg2 = 1;
    }
    else
...
void HAL_MVD_InitVar(MS_U8 u8Idx)
{
    ...
    pMVDHalContext->u8IsAliveCount[u8Idx] = 0;
...
E_MVD_Result HAL_MVD_IsAlive(MS_U8 u8Idx)
{
    ...
    else
    {
        ...
        if(pMVDHalContext->u8IsAliveCount[u8Idx] < ISALIVE_TRY_COUNTER)
        {
            pMVDHalContext->u8IsAliveCount[u8Idx]++;
            return E_MVD_RET_OK;
        }
        else
        {
