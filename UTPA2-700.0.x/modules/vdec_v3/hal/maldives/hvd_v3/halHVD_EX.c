...
#define HVD_HW_MAX_PIXEL            (1920*1088*61000ULL) // 4kx2k@30p
...
static void _HVD_EX_Context_Init_HAL(void)
{
    ...
    if(sysInfo->Chip.Version == VER_MALDIVES)
        pHVDHalContext->u32HVDClockType = 288;//160;
    else
...
#endif
{
    ...
    if((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP8)
    {
        if(sysInfo->Chip.Version == VER_MUNICH)
        {
            HVD_EX_MSG_ERR("Munich does not support VP8 codec!!\n");
            return FALSE;
        }
    }
    ...
    if ((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP9)
    {
        if(sysInfo->Chip.Version == VER_MUNICH)
        {
            HVD_EX_MSG_ERR("Munich does not support VP9 codec!!\n");
            return FALSE;
        }
    }
#endif
...
RESET:
    ...
    if (isEVD)
    {
        printf("EVD BBU 128 bits\n");
        _HVD_Write2Byte(EVD_REG_BBU_MIU_WIDTH, EVD_REG_BBU_MIU_128);
    }
#endif
#if 0 //defined(SUPPORT_NEW_MEM_LAYOUT) || defined(SUPPORT_NEW_VDEC_FLOW)
...
void HAL_HVD_EX_PowerCtrl(MS_BOOL bEnable)
{
    ...
    switch (pHVDHalContext->u32HVDClockType)
    {
        ...
        case 288: // for maldives only
        {
            if(sysInfo->Chip.Version == VER_MALDIVES)
            {
                _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_HVD_288MHZ_MALDIVES, TOP_CKG_HVD_CLK_MASK);
...
#endif
{
    ...
    //Chip info
    pShm->u16ChipID = E_MSTAR_CHIP_MUNICH;
