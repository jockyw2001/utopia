...
#endif
{
    ...
    MsOS_ReadMemory();
    VPRINTF("before: version=0x%x, statue=0x%x, last_ctl_cmd=0x%x, last_ctl_arg=0x%x, t0=%d, t1=%d\n",
         ctl_ptr->verion, ctl_ptr->statue, ctl_ptr->last_ctl_cmd, ctl_ptr->last_ctl_arg, ctl_ptr->task_statue[0], ctl_ptr->task_statue[1]);
...
#define MAX_CODEC_TYPE_NUM 18
    ...
    unsigned int capability[MAX_CODEC_TYPE_NUM][MAX_CAPABILITY_INFO_NUM] =
    {
            ...
            { 1920, 1080,     60,     E_VDEC_EX_CODEC_PROFILE_VP9_2,            E_VDEC_EX_CODEC_LEVEL_NONE,         E_VDEC_EX_CODEC_VERSION_NONE,        50,        0},//E_HVD_EX_CODEC_TYPE_VP9
    };
...
*/
    ...
    else
    {
        ...
        if (bTSP)
        {
            ...
            {
                VPRINTF("[NDec][0x%x][%d] bTSP, use bbu %d \033[0m\n", u32Id, u8TaskId, u8TaskId);
