...
#define VPU_MSG_ERR(format, args...)                \
    ...
    {                                               \
        ...
        {                                           \
            ULOGE("VPU", "[ERR]%s:", __FUNCTION__); \
            ULOGE("VPU", format, ##args);           \
        }                                           \
    } while (0)
...
#define VPU_MSG_DBG(format, args...)                \
    ...
    {                                               \
        ...
        {                                           \
            ULOGD("VPU", "[DBG]%s:", __FUNCTION__); \
            ULOGD("VPU", format, ##args);           \
        }                                           \
    } while (0)
...
#define VPU_MSG_INFO(format, args...)               \
    ...
    {                                               \
        ...
        {                                           \
            ULOGI("VPU", "[INF]%s:", __FUNCTION__); \
            ULOGI("VPU", format, ##args);           \
        }                                           \
    } while (0)
...
#define _MaskMiu2Req_VPU_D_RW(m)    _VPU_WriteRegBit(MIU2_REG_RQ0_MASK, m, BIT(6))
#define _MaskMiu2Req_VPU_Q_RW(m)    _VPU_WriteRegBit(MIU2_REG_RQ0_MASK, m, BIT(6))
#define _MaskMiu2Req_VPU_I_R(m)     _VPU_WriteRegBit(MIU2_REG_RQ0_MASK+1, m, BIT(0))


#define VPU_D_RW_ON_MIU0            (((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == 0) && ((_VPU_ReadByte(MIU2_REG_SEL0) & BIT(6)) == 0))
#define VPU_Q_RW_ON_MIU0            (((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == 0) && ((_VPU_ReadByte(MIU2_REG_SEL0) & BIT(6)) == 0))
#define VPU_I_R_ON_MIU0             (((_VPU_ReadByte(MIU0_REG_SEL0+1) & BIT(0)) == 0) && ((_VPU_ReadByte(MIU2_REG_SEL0+1) & BIT(0)) == 0)) //g08
#define VPU_D_RW_ON_MIU1            (((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == BIT(6)) && ((_VPU_ReadByte(MIU2_REG_SEL0) & BIT(6)) == 0))
#define VPU_Q_RW_ON_MIU1            (((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == BIT(6)) && ((_VPU_ReadByte(MIU2_REG_SEL0) & BIT(6)) == 0))
#define VPU_I_R_ON_MIU1             (((_VPU_ReadByte(MIU0_REG_SEL0+1) & BIT(0)) == BIT(0)) && ((_VPU_ReadByte(MIU2_REG_SEL0+1) & BIT(0)) == 0)) //g08
#define VPU_D_RW_ON_MIU2            (((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == 0) && ((_VPU_ReadByte(MIU2_REG_SEL0) & BIT(6)) == BIT(6)))
#define VPU_Q_RW_ON_MIU2            (((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == 0) && ((_VPU_ReadByte(MIU2_REG_SEL0) & BIT(6)) == BIT(6)))
#define VPU_I_R_ON_MIU2             (((_VPU_ReadByte(MIU0_REG_SEL0+1) & BIT(0)) == 0) && ((_VPU_ReadByte(MIU2_REG_SEL0+1) & BIT(0)) == BIT(0))) //g08
#endif
...
#define _VPU_MIU_SetReqMask(miu_clients, mask)  \
    ...
    {                                           \
       ...
       {                                       \
           ...
           else if (miu_clients##_ON_MIU2 == 1)   \
           {                                           \
...
#define VPU_FIELD_DETECTOR          TRUE
#else
#define VPU_FIELD_DETECTOR          FALSE
#endif
...
typedef struct
{
    ...
    MS_S32 s32VpuFdTaskId[MAX_SUPPORT_DECODER_NUM];
    MS_BOOL bFDInit[MAX_SUPPORT_DECODER_NUM];
...
static void _VPU_EX_Context_Init(void)
{
    ...
    for (i = 0; i < MAX_SUPPORT_DECODER_NUM; i++)
    {
        pVPUHalContext->s32VpuFdTaskId[i] = -1;
        pVPUHalContext->bFDInit[i] = FALSE;
    }
...
static MS_BOOL _VPU_EX_InitAll(VPU_EX_NDecInitPara *pInitPara)
{
    ...
    VPU_EX_ClockSpeed eClkSpeed = E_VPU_EX_CLOCK_432MHZ;
...
static void _VPU_EX_ClockSpeed(MS_U32 u32type)
{
    switch (u32type)
    {
        ...
        case VPU_CLOCK_432MHZ:
            ...
            break;
        default:
            _VPU_WriteWordMask(REG_TOP_VPU, VPU_CLOCK_384MHZ, TOP_CKG_VPU_CLK_MASK);
            break;
    }
}
...
#if VPU_FIELD_DETECTOR

#include "FieldDetection.h"

static MS_BOOL _HAL_VPU_EX_Field_Detecor_Task_HVD(MS_U32 u32Id)
{
    MS_U8 u8Offset              = _VPU_EX_GetOffsetIdx(u32Id);
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    //VPU_FD_DispFrameInfo DispFrmInfo;
    MsOS_ReadMemory();
    //memset(&DispFrmInfo,0,sizeof(VPU_FD_DispFrameInfo));

    if(pShm->u32CodecType == E_HVD_Codec_AVS)
    {
    ...
    {
        MsOS_DelayTask(1);
        HVD_Frm_Information *pFrmInfo = (HVD_Frm_Information *)&pShm->DispQueue[pShm->u8PpQueueWPtr];
        MS_U32 u32SrcMiuSel = (pShm->u32VDEC_MIU_SEL >> VDEC_PPIN_MIUSEL) & VDEC_MIUSEL_MASK;

        FrameInfo stFrameInfo;
        stFrameInfo.bFieldOrderSet = (pFrmInfo->u8ModeFlag & E_HVD_PP_FD_TOP_FIELD_FIRST)? 1:0;
        _miu_offset_to_phy(u32SrcMiuSel, pFrmInfo->u32LumaAddr, stFrameInfo.phyBuffAddr);
        stFrameInfo.u16Height = pFrmInfo->u16Height;
        stFrameInfo.u16Width= pFrmInfo->u16Width;
        stFrameInfo.u32BuffSize = (pFrmInfo->u16Height)*(pFrmInfo->u16Width);

        if(pShm->u32CodecType == E_HVD_Codec_AVS
            && pShm->u8FrmPostProcSupport & E_HVD_POST_PROC_FIELD_DETECT_ONGOING)
        {
            if(pVPUHalContext->bFDInit[u8Offset]==FALSE)
            {
                FODInfo stFod;
                stFod.height =pFrmInfo->u16Height;
                stFod.width=pFrmInfo->u16Width;
                stFod.pitch=pFrmInfo->u16Pitch;
                stFod.tile_width = 16;
                stFod.tile_height= 32;
                VPU_MSG_INFO("Init Field Detector!!\n");
                InitFieldDetection(&stFod);

                InitMemoryFromVPU(stFrameInfo.phyBuffAddr+stFrameInfo.u32BuffSize*20,
                        stFrameInfo.phyBuffAddr+stFrameInfo.u32BuffSize*21,
                        stFrameInfo.phyBuffAddr+stFrameInfo.u32BuffSize*22);
                    pVPUHalContext->bFDInit[u8Offset] = TRUE;
            }

            VPU_MSG_INFO("[%dx%d], [0x%x, 0x%x], %d ", stFrameInfo.u16Width, stFrameInfo.u16Height,
            (MS_U32)stFrameInfo.phyBuffAddr, (MS_U32)stFrameInfo.u32BuffSize, stFrameInfo.bFieldOrderSet);

            MS_U32 u32Timer= MsOS_GetSystemTime();
            eIsFieldOrderInverse eResult = IsFieldOrderInverse(&stFrameInfo);
            VPU_MSG_INFO("[%dms] ", MsOS_GetSystemTime()-u32Timer);
            if(eResult == E_VDEC_IFOI_TRUE)
            {
                pShm->u8FrmPostProcSupport |= E_HVD_POST_PROC_FIELD_DETECT_NEED_REVERSE;
                VPU_MSG_INFO("reverse=1\n");
            }
            else if(eResult == E_VDEC_IFOI_FALSE)
            {
                pShm->u8FrmPostProcSupport &= ~E_HVD_POST_PROC_FIELD_DETECT_NEED_REVERSE;
                VPU_MSG_INFO("reverse=0\n");
            }
...
static MS_BOOL _HAL_VPU_EX_Field_Detecor_Task_MVD(MS_U32 u32Id)
{
    u32Id = u32Id;
    while(1)
    {
        MsOS_DelayTask(1);
        // TODO: handle MVD case here
    }
    return TRUE;
}

static void _HAL_VPU_EX_FdTask_Delete(MS_U32 u32Id, VPU_Hal_CTX* pVPUHalContext)
{
    MS_U8 u8Offset              = _VPU_EX_GetOffsetIdx(u32Id);
    FInitFieldDetection();
    pVPUHalContext->bFDInit[u8Offset] = FALSE;
    MsOS_DeleteTask(pVPUHalContext->s32VpuFdTaskId[u8Offset]);
    pVPUHalContext->s32VpuFdTaskId[u8Offset] = -1;
}
...
#if VPU_FIELD_DETECTOR
    if(pVPUHalContext->s32VpuFdTaskId[0] == -1 && pVPUHalContext->s32VpuFdTaskId[1] == -1
        && pVPUHalContext->s32VpuFdTaskId[2] == -1 && pVPUHalContext->s32VpuFdTaskId[3] == -1
        && pVPUHalContext->s32VpuFdTaskId[4] == -1 && pVPUHalContext->s32VpuFdTaskId[5] == -1
        && pVPUHalContext->s32VpuFdTaskId[6] == -1 && pVPUHalContext->s32VpuFdTaskId[7] == -1
        && pVPUHalContext->s32VpuFdTaskId[8] == -1 && pVPUHalContext->s32VpuFdTaskId[9] == -1
        && pVPUHalContext->s32VpuFdTaskId[10] == -1 && pVPUHalContext->s32VpuFdTaskId[11] == -1
        && pVPUHalContext->s32VpuFdTaskId[12] == -1 && pVPUHalContext->s32VpuFdTaskId[13] == -1
        && pVPUHalContext->s32VpuFdTaskId[14] == -1 && pVPUHalContext->s32VpuFdTaskId[15] == -1
        )
    {
        MS_S32 s32VPUFDTaskId = -1;
        if (E_VPU_EX_DECODER_MVD == pTaskInfo->eDecType)
        {
            s32VPUFDTaskId = MsOS_CreateTask((TaskEntry)_HAL_VPU_EX_Field_Detecor_Task_MVD,
                                                u32Id,
                                                E_TASK_PRI_MEDIUM,
                                                TRUE,
                                                NULL,
                                                32, // stack size..
                                                "VPU_FieldDetector_MVD_task");
        }
        else if(E_VPU_EX_DECODER_HVD == pTaskInfo->eDecType ||E_VPU_EX_DECODER_EVD == pTaskInfo->eDecType)
        {
            VPU_MSG_INFO(" Create _HAL_VPU_EX_Field_Detecor_Task_HVD()\n");
            s32VPUFDTaskId = MsOS_CreateTask((TaskEntry)_HAL_VPU_EX_Field_Detecor_Task_HVD,
                                                u32Id,
                                                E_TASK_PRI_MEDIUM,
                                                TRUE,
                                                NULL,
                                                32, // stack size..
                                                "VPU_FieldDetector_HVD_task");
        }
        if (s32VPUFDTaskId < 0)
        {
            VPU_MSG_ERR("Field Detector Task create failed\n");
        }
        else
        {
            pVPUHalContext->s32VpuFdTaskId[u8Offset]= s32VPUFDTaskId;
        }
...
#if VPU_FIELD_DETECTOR
    _HAL_VPU_EX_FdTask_Delete(u32Id, pVPUHalContext);
#endif
...
MS_BOOL HAL_VPU_EX_SwRst(MS_BOOL bCheckMauIdle)
{
    ...
    if (tempreg & 0xf || pVPUHalContext->u8ForceRst == 1) {
        ...
        do
        {
            if (--idle_cnt == 0)
            {
                printf("VPU_REG_CPU_STALL_EN is not set\n");
                break;
            }
            MsOS_DelayTask(1);
        ...
        while (_VPU_Read2Byte(VPU_REG_ICU_DBG_DAT0) & VPU_REG_ICPU_REQ)
        {
            if (--idle_cnt == 0)
            {
                printf("CPU keeps requesting ICU\n");
                break;
            }
            MsOS_DelayTask(1);
        }
...
{
    return TRUE;// maserati disable SN DS
}
...
#define MAX_CODEC_TYPE_NUM 18
    ...
    unsigned int capability[MAX_CODEC_TYPE_NUM][MAX_CAPABILITY_INFO_NUM] =
    {
            ...
            { 1920, 1080,     60,     E_VDEC_EX_CODEC_PROFILE_MP2_MAIN,         E_VDEC_EX_CODEC_LEVEL_MP2_HIGH,     E_VDEC_EX_CODEC_VERSION_NONE,        40,        0},//E_HVD_EX_CODEC_TYPE_MPEG2
            ...
            { 1920, 1080,     30,     E_VDEC_EX_CODEC_PROFILE_NONE,             E_VDEC_EX_CODEC_LEVEL_NONE,         E_VDEC_EX_CODEC_VERSION_NONE,        10,        0},//E_HVD_EX_CODEC_TYPE_MJPEG
            ...
            { 1920, 1080,     30,     E_VDEC_EX_CODEC_PROFILE_NONE,             E_VDEC_EX_CODEC_LEVEL_NONE,         E_VDEC_EX_CODEC_VERSION_NONE,        20,        0},//E_HVD_EX_CODEC_TYPE_VP8
            { 4096, 2160,     60,     E_VDEC_EX_CODEC_PROFILE_H265_MAIN_10,     E_VDEC_EX_CODEC_LEVEL_H265_5_1_HT,  E_VDEC_EX_CODEC_VERSION_NONE,       160,        0},//E_HVD_EX_CODEC_TYPE_HEVC
            { 4096, 2160,     60,     E_VDEC_EX_CODEC_PROFILE_VP9_2,            E_VDEC_EX_CODEC_LEVEL_NONE,         E_VDEC_EX_CODEC_VERSION_NONE,       160,        0},//E_HVD_EX_CODEC_TYPE_VP9
    };
...
MS_BOOL HAL_VPU_EX_Push2VBBU(MS_U32 u32Id, HAL_VPU_EX_PacketInfo *stVpuPkt, MS_VIRT u32VBBUAddr)
{
    ...
    stEntry->u8Version = stVpuPkt->u8Version;
                ...
                case E_CAP_NDEC_FB_MIU_SELECT:
                    {
                        ...
                        if((eCodecType == E_VPU_EX_CODEC_TYPE_HEVC) ||
                           (eCodecType == E_VPU_EX_CODEC_TYPE_VP9)  ||
                           (eCodecType == E_VPU_EX_CODEC_TYPE_HEVC_DV))
                        {
                            *pSupport = TRUE;
                        }
                        else
...
#include "halVPU_EX.h"
#include "drvMMIO.h"
#include "../../../../vdec_v2/hal/maserati/hvd_ex/regHVD_EX.h"
#include "halCHIP.h"
