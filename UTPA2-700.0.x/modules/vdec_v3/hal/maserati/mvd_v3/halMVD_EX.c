...
#define MVDHW_ON_MIU0      ((HAL_MVD_RegReadByte(MIU0_SEL2_L) & BIT4) == 0 && (HAL_MVD_RegReadByte(MIU2_SEL2_L) & BIT4) == 0)
#define MVDHW_ON_MIU1      ((HAL_MVD_RegReadByte(MIU0_SEL2_L) & BIT4) == BIT4 && (HAL_MVD_RegReadByte(MIU2_SEL2_L) & BIT4) == 0)
#define MVDHW_ON_MIU2      ((HAL_MVD_RegReadByte(MIU0_SEL2_L) & BIT4) == 0 && (HAL_MVD_RegReadByte(MIU2_SEL2_L) & BIT4) == BIT4)
...
#ifdef VDEC3
static void MVD_SetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U16 u16Offset, MS_U8 u8Size, MS_U32 u32Value)
{
    MS_VIRT u32BufStart = NULL;
...
void HAL_MVD_SetReqMask(MS_BOOL bEnMask)
{
    ...
    {
        HAL_MVD_RegWriteBit(MIU2_RQ2_MASK_L, bEnMask, BIT4);  //MVD R/W
        HAL_MVD_RegWriteBit(MIU2_RQ0_MASK_H, bEnMask, BIT4);  //MVD bbu R/W
    }
    HAL_MVD_Delayms(1);
...
#if defined(MVD_SUPPORT_X4_CC)
    u32UsrDataWr = HAL_CC_CM_GetMVDRB_HWAddr(4);
#else
    u32UsrDataWr = HAL_CC_CM_GetMVDRB_HWAddr(2);
#endif
    ...
    if ((E_MVD_SLQ_TBL_MODE == curSrcMode) && (TRUE != FLAG_LAST_FRM_SHOW))
    {
        ...
        if(pstCtrlCfg->u8McuMode == TRUE)
        {
            ...
            while ((HAL_MVD_GetTime() - u32TimeCnt) < 20)  // timeout 20 ms
            {
                ...
                {
                    HAL_VPU_EX_PacketInfo stVpuPkt = {0};
                    MVD_PacketInfo stDummyPkt;
                    MVD_SLQTblGetFileEndPkt(u8Idx,&stDummyPkt);
...
MS_BOOL HAL_MVD_GetColorInfo(MS_U8 u8Idx, MVD_Color_Info* pstColorInfo)
{
    ...
    {
        MVD_DEBUGERROR(printf("HAL_MVD_GetColorInfo error: pu8MVDGetFrameInfoBufStart=NULL\n"));
        return FALSE;
    }
...
E_MVD_Result HAL_MVD_SetDVXCShmAddr(MS_U8 u8Idx, MS_PHY u32FWBaseAddr, MS_PHY u32DVXCShmAddr)
{
    ...
    {
        MVD_DEBUGERROR(printf("[ERROR] XC share memory and VDEC Code base MIU selections are different\n"));
        return E_MVD_RET_FAIL;
    }
...
MS_BOOL HAL_MVD_PreConnectInputTsp(MS_U8 u8Idx, MS_BOOL bEnable, MVD_INPUT_TSP eInputTsp, MVD_Original_Stream eStream)
{
    ...
    if (curSrcMode == E_MVD_TS_MODE)
    {
        ...
        MVD_DEBUGINFO(printf("%s CMD_PRESET_CONNET_INPUT_TSP arg0=%x\n", __FUNCTION__, u8Input));
