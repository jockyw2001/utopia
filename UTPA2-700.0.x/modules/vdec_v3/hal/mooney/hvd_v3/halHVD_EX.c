...
#if SUPPORT_EVD
#define _MaskMiuReq_EVD_RW( m )         _HVD_WriteRegBit(MIU0_REG_RQ4_MASK, m, BIT(0))
#define _MaskMiuReq_EVD_BBU_R( m )      _HVD_WriteRegBit(MIU0_REG_RQ4_MASK, m, BIT(0))
#define _MaskMiu1Req_EVD_RW( m )        _HVD_WriteRegBit(MIU1_REG_RQ4_MASK, m, BIT(0))
#define _MaskMiu1Req_EVD_BBU_R( m )     _HVD_WriteRegBit(MIU1_REG_RQ4_MASK, m, BIT(0))

#define HVD_EVD_RW_ON_MIU0              ((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(0)) == 0)
#define HVD_EVD_BBU_R_ON_MIU0           ((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(0)) == 0)
#define HVD_EVD_RW_ON_MIU1              ((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(0)) == BIT(0))
#define HVD_EVD_BBU_R_ON_MIU1           ((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(0)) == BIT(0))
#endif
...
static void _HVD_EX_Context_Init_HAL(void)
{
    ...
    pHVDHalContext->u32VPUClockType = 384;
...
#endif
{
    ...
    //Chip info
    pShm->u16ChipID = E_MSTAR_CHIP_MOONEY;
    ...
    if (pHVDHalContext->pHVDPreCtrl_Hal[_HVD_EX_GetStreamIdx(u32Id)]->bVP94k2kCheck)
    {
        pShm->u32PreSetControl |= PRESET_VP9_4K2K_CHECK;
    }
...
{
    //chip dependent, mooney doesn't support any DV profile
...
{
    //mooney doesn't support any DV profile
