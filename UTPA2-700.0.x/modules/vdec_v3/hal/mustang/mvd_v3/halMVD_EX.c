...
typedef struct
{
    ...
    MS_BOOL bCMAUsed;
#if defined(SUPPORT_CMA) //20170110
...
MS_BOOL HAL_MVD_MVDCommand ( MS_U8 u8cmd, MVD_CmdArg *pstCmdArg )
{
    ...
    MS_U32 u32CmdArg;
    MS_U32 u32Ret = FALSE;
    //HVD_DRAM_COMMAND_QUEUE_SEND_STATUS u32Ret = E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL; //20170110
    MS_U32 u32Timeout;
    ...
    if(HAL_MVD_IsDisplayCommand(u8cmd) == TRUE)
    {
        if(HAL_MVD_IsNeedResponseCommand(u8cmd) == TRUE)
        {
            ...
            u32BufStart2 = GET_VOL_BUFFADD(u8HalIdx);//20170110
...
MS_BOOL HAL_MVD_StepDisp(MS_U8 u8Idx)
{
    ...
    MVD_SetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_STEP_DISP_DONE, sizeof(MS_U8), 0);//20170110
...
void HAL_MVD_GetFrameInfo(MS_U8 u8Idx, MVD_FrameInfo *pinfo)
{
    ...
    else if (curCodecType == E_MVD_CODEC_MPEG4)
    {
        ...
        else
        {
            ...
            MS_U32 u32GuessSourceFrameRate = 30000; // guess source frame rate for no pts frame
            HAL_MVD_MemWrite4Byte(pu8MVDSetHeaderBufStart+OFFSET_DIVX_FRAME_RATE,u32GuessSourceFrameRate);
            #endif
        }
    ...
    if (pstMemCfg->bEnableDynScale)
        {
        #ifdef ENABLE_EXTERNAL_DS_BUFFER //20170110
...
MS_VIRT HAL_CC_CM_GetMVDRB_HWAddr(MS_U8 u8CC608)
{
    ...
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg3 = u8CC608;
    //SET_DECNUM(mvdcmd, u8Idx);//20170110
...
MS_BOOL HAL_MVD_EnableInt(MS_U8 u8Idx, MS_U32 bEn)
{
    ...
    u32IntFlag = (((bEn & E_MVD_EVENT_USER_DATA) == E_MVD_EVENT_USER_DATA) ? INT_USER_DATA : 0) |
                 ...
                 //(((bEn & E_MVD_EVENT_PTS_DISCONTINUE) == E_MVD_EVENT_PTS_DISCONTINUE) ? INT_PTS_DISCONTINUE : 0);//20170110
