...
#ifdef VDEC3
    if (!pCtrl->bShareBBU || ((_HAL_EX_IS_EVD(pCtrl->InitParams.u32ModeFlag) && !_HAL_EX_BBU_EVD_InUsed()) ||
...
static void _HAL_HVD_EX_Inv_Cache(void *pVA, MS_U32 u32Size)
{
    ...
    MsOS_MPool_Dcache_Flush((MS_VIRT)pVA, u32Size);//20170110
}
...
#endif
{
    ...
    pShm->u8ExternalHeapIdx = 0xFF; //20170110
...
HVD_Return HAL_HVD_EX_PushPacket(MS_U32 u32Id, HVD_BBU_Info *pInfo)
{
    ...
    HVD_EX_MSG_ERR(">>> halHVD pts,idH = %u, %u\n", pInfo->u32TimeStamp, pInfo->u32ID_H);    //STS input
    printf(">>> halHVD pts,idH = %u, %u\n", pInfo->u32TimeStamp, pInfo->u32ID_H);    //STS input
