...
static MS_U32 MVD_GetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U16 u8Offset, MS_U8 u8Size)
{
    MS_U32 u32Val = 0;
...
void HAL_MVD_SetReqMask(MS_BOOL bEnMask)
{
    ...
    if (MVDHW_ON_MIU1)
    {
        ...
        HAL_MVD_RegWriteBit(MIU1_RQ4_MASK_L, bEnMask, BIT6);  //MVD TLB Mheg Codec
    }
    else
    {
        HAL_MVD_RegWriteBit(MIU0_RQ2_MASK_L, bEnMask, BIT4);  //MVD R/W
        HAL_MVD_RegWriteBit(MIU0_RQ0_MASK_H, bEnMask, BIT4);  //MVD bbu R/W
        HAL_MVD_RegWriteBit(MIU0_RQ4_MASK_L, bEnMask, BIT6);  //MVD TLB Mheg Codec
    }
    HAL_MVD_Delayms(1);
...
#ifdef SUPPORT_MVD2// macan mvd2 is null
...
MS_VIRT HAL_MVD_GetTsFileESWritePtr(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGINFO(printf("ES wrapping Wr=0x%lx ==> ", (unsigned long)u32WrPtr));
...
MS_BOOL MVD_IntDispFinish(MS_U32 u32IntStat)
{
    return (((u32IntStat&INT_DISP_FINISH)==INT_DISP_FINISH) ? TRUE : FALSE);
}
...
MS_U32 HAL_MVD_GetXcLowDelayIntState(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(printf("MDrv_MVD_GetXcLowDelayIntState()=%x,%x,%x,%x,%x\n",
...
MS_BOOL HAL_MVD_SLQTblSendPacket(MS_U8 u8Idx, MVD_PacketInfo *pstVideoPKT)
{
    ...
    MS_U32 u32ESEnd=0;
    MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    MS_U64 u64Pts = (((MS_U64)pstVideoPKT->u32ID_H)<<32)|((MS_U64)pstVideoPKT->u32TimeStamp);
    MS_U32 u32Pts_high = pstVideoPKT->u32ID_H;
    MS_U32 u32Pts_low  = pstVideoPKT->u32TimeStamp;


#if _SLQTBL_DUMP_PKT
   // static MS_U32 u32SendTimes[MAX_DEC_NUM]= {0, 0};

    printf("Pkt[%x] st=0x%x len=0x%x pts=0x%x id_l=0x%x id_h=0x%x\n", u32SendTimes[u8Idx]++,
    ...
    if(pstCtrlCfg->u8McuMode == FALSE)
    {
        ...
        if (u32MVDFWPtsTblAddr)
        {
            ...
            {
                u64Pts = _MS_TO_90K(u64Pts);
                u32Pts_high = u64Pts>>32;
                u32Pts_low = u64Pts&0xffffffff;
            }
            ...
            HAL_MVD_MemWrite4Byte(u32PtsTblEntryAddr+12, u32Pts_high); //idHigh
            HAL_MVD_MemWrite4Byte(u32PtsTblEntryAddr+16, u32Pts_low);  //PTS
            HAL_MVD_MemWrite4Byte(u32PtsTblEntryAddr, (pstSlqTblInfo->u32SlqByteCnt)&0xffffff); //byteCnt
            //printf("PTS=0x%x(%x), idx=0x%x add=0x%x\n", pstVideoPKT->u32TimeStamp,
    ...
    *pu32EsNew += 8;
    ...
    //printf("===>[%x] u32ESStart=0x%x u32ESEnd=0x%x u32EsLast=0x%x\n",
    ...
    //printf("===> u32EntryWord1 addr=0x%x\n", (*u32LastEntry));
    //printf("===> u32EntryWord0=0x%x\n", u32EntryWord);

    u32EntryWord = u32ESStart;
    HAL_MVD_MemWrite4Byte((*u32LastEntry)+4, u32EntryWord);
    //printf("===> u32EntryWord1 addr=0x%x\n", (*u32LastEntry)+4);
    //printf("===> u32EntryWord1=0x%x\n", u32EntryWord);
...
void HAL_MVD_GetFrameInfo(MS_U8 u8Idx, MVD_FrameInfo *pinfo)
{
    ...
    if (pstMemCfg->bEnableDynScale)
    {
        ...
        pinfo->u8DynScalingDepth= gvolInfo.DS_Depth;
...
E_MVD_Result MVD_RstFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType)
{
    ...
    //printf("%s u32DecFrmInfoAdd = 0x%x\n", __FUNCTION__, u32DecFrmInfoAdd);
    if (E_MVD_FRMINFO_DECODE == eType)
    {
...
static MS_BOOL MVD_GetFWBuffAdd(MS_VIRT u32Start, MS_U32 u32AvailLen, MVD_FWBuff* pBuff)
{
    ...
    {
        //shared memory should not overlap with FW memory pool.
        MVD_DEBUGERROR(printf("%s err: 0x%x out of memory boundary!\n", __FUNCTION__,
...
E_MVD_Result HAL_MVD_GetFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType, MVD_FrmInfo* pInfo)
{
    ...
    printf("Luma=0x%x, ", pInfo->u32LumaAddr  );
    printf("Chroma=0x%x, ", pInfo->u32ChromaAddr);
...
E_MVD_Result HAL_MVD_SetDynScalingParam(MS_U8 u8Idx, MS_PHY u32StAddr, MS_SIZE u32Size)
{
    ...
    if ((u32StAddr==0) || (u32Size==0) || (u32Size>MVD_FW_SCALER_INFO_BUF_LEN))
    {
...
#else
    memCfg->bSupportSDModeOnly = FALSE;
#endif
    *pu8FBMode = 0xff;
...
void HAL_MVD_InitVar(MS_U8 u8Idx)
{
    ...
    memset((void*)(&(pMVDHalContext->aliveInfo[u8Idx])), 0, sizeof(MVD_ALIVEInfo));
...
MS_BOOL HAL_MVD_GetUsrDataInfo(MS_U8 u8Idx, MVD_UsrDataInfo* pUsrInfo)
{
    ...
    printf("0x%x, ", pUsrInfo->u32DataBuf);
...
static void MVD_SLQTblGetFileEndPkt(MS_U8 u8Idx, MVD_PacketInfo* pFileEnd)
{
    ...
    MVD_DEBUGINFO(printf("u32EndPattern(0x%lx)=0x%x 0x%x 0x%x 0x%x\n", (unsigned long)pFileEnd->u32StAddr,
...
#if 0
    printf("u32DummyES(0x%x-->0x%x, size=0x%x)=0x%08lx 0x%08lx 0x%08lx 0x%08lx\n", u32DummyES,
...
MS_BOOL HAL_MVD_SLQTblInsertPattern(MS_U8 u8Idx, MVD_PatternType ePattern)
{
    ...
    {   //insert dummy pattern
        ...
        #endif
        //printf("WrPtr 0x%x ", pstSlqTblInfo->pDrvSlqTbl->u32WrPtr);
...
static void _SLQTbl_DumpPtsTbl(MS_U8 u8Idx, MS_U32 u32EntryStart, MS_U32 u32EntryEnd)
{
    ...
    for (i=u32EntryStart; i<u32EntryEnd; i++)
    {
        ...
        printf("ES[%x] Start=0x%x End=0x%x u32EsRp=%x\n",
    ...
    for (i=u32EntryStart; i<u32EntryEnd; i++)
    {
        printf("0x%x\t 0x%08lx\t 0x%08lx\t 0x%08lx\t 0x%08lx\t 0x%08lx\n", u32MVDFWPtsTblAddr+i*MVD_FW_SLQTBL_PTS_LEN,
...
#endif
{
    ...
    //printf("u32DivXPattern(0x%x==>0x%x)=0x%x 0x%x\n", u32DivXPattern, pDivxHdr->u32StAddr,
...
static void MVD_SLQTblInitFileEndPkt(MS_U8 u8Idx, MVD_CodecType eType)
{
    ...
    if ((E_MVD_CODEC_FLV == eType)||(E_MVD_CODEC_MPEG4_SHORT_VIDEO_HEADER == eType))
    {
        ...
        //printf("##########FileEnd for FLV/SVH!, u32EndPattern=%x\n",u32EndPattern);
    ...
    else if (E_MVD_CODEC_DIVX311 == eType)
    {
        ...
        HAL_MVD_MemWrite4Byte(u32EndPattern+12,END_PATTERN_2); //scw
        HAL_MVD_MemWrite4Byte(u32EndPattern+16,END_PATTERN_3); //scw
        //printf("##########FileEnd for DIVX311!, u32EndPattern=%x\n",u32EndPattern);
    ...
    else if ((E_MVD_CODEC_MPEG2 == eType)||(E_MVD_CODEC_MPEG4 == eType))
    {
        ...
        //printf("##########FileEnd for MPEG2/4!, u32EndPattern=%x\n",u32EndPattern);
    ...
    else
    {
        ...
        //printf("##########FileEnd for VC1!, u32EndPattern=%x\n",u32EndPattern);
    ...
    MVD_DEBUGINFO(printf("u32EndPattern(0x%lx)=0x%x 0x%x\n", (unsigned long)u32EndPattern,
            HAL_MVD_MemRead4Byte(u32EndPattern), HAL_MVD_MemRead4Byte(u32EndPattern+4)));
}
...
#if 0
    printf("u32DummyES(0x%x)=0x%08lx 0x%08lx 0x%08lx 0x%08lx\n", u32DummyES, HAL_MVD_MemRead4Byte(u32DummyES),
            HAL_MVD_MemRead4Byte(u32DummyES+4),HAL_MVD_MemRead4Byte(u32DummyES+8),HAL_MVD_MemRead4Byte(u32DummyES+12));
#endif
...
MS_U32 HAL_MVD_GetQueueVacancy(MS_U8 u8Idx, MS_BOOL bCached)
{
    ...
    //printf("QV=0x%x rd=0x%x==>", u32Empty, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr);
    ...
    //printf("%s r=0x%x w=0x%x u32Empty=0x%x\n", __FUNCTION__,
...
E_MVD_Result HAL_MVD_PushQueue(MS_U8 u8Idx, MVD_PacketInfo* pInfo)
{
...
#if _SLQTBL_DUMP_PUSHQ
    {
        ...
        printf("Push[%x] st=0x%x len=0x%x pts=0x%x\n", pMVDHalContext->u32pqTimes[u8Idx]++,
...
#define MAX_VALIDLEN    0x200000    //2M
    ...
    else
    {
        ...
        MVD_DEBUGINFO(printf("(%x) ValidLen=0x%x CurLen=0x%x\n",
...
MS_BOOL HAL_MVD_EnableInt(MS_U8 u8Idx, MS_U32 bEn)
{
    ...
    u32IntFlag = (((bEn & E_MVD_EVENT_USER_DATA) == E_MVD_EVENT_USER_DATA) ? INT_USER_DATA : 0) |
                 ...
                 (((bEn & E_MVD_EVENT_DISP_FINISH) == E_MVD_EVENT_DISP_FINISH) ? INT_DISP_FINISH: 0)|
...
MS_VIRT MVD_GetSlqTblESReadPtr(MS_U8 u8Idx)
{
    ...
    MS_U32 u32EsEnd;
    u32EsEnd = HAL_MVD_MemRead4Byte(u32EsRp+4);
    printf("GetESReadPtr ES[%x] Start=0x%x End=0x%x u32EsRp=%x u32SlqRp=%x\n",
...
#if 0
    printf("GetESWritePtr[%x] ES Start=0x%x End=0x%x u32EsWp=%x\n",
...
MS_VIRT HAL_MVD_GetSLQReadPtr(MS_U8 u8Idx)
{
    ...
    //printf("##### HAL_MVD_GetSLQReadPtr 0x%x\n", readPtr);
    ...
    if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
    {
        ...
        while ((HAL_MVD_GetTime() - u32TimeCnt) < CMD_TIMEOUT_MS)
        {
            if (HAL_MVD_GetDecodeStatus(u8Idx) != E_MVD_STAT_IDLE)
            {
                ...
                MVD_DEBUGVERBAL(printf("time=0x%x ms, ", (HAL_MVD_GetTime() - u32TimeCnt)));
                break;
            }
        }
        if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
...
void HAL_MVD_DbgDump(MS_U8 u8Idx)
{
    ...
    printf("frm Dec Y=%lx UV=%lx Pitch=%x; ", (unsigned long)stFrm.u32LumaAddr, (unsigned long)stFrm.u32ChromaAddr, stFrm.u16Pitch);
    ...
    {
        #ifndef VDEC3
        printf("fe=%x, rd=%x(%x), wr=%x, empty=%x; ", pstSlqTblInfo->u32FileEndPtr, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr,
...
{
#define SIZE_DSBUF 0x100000
#define SIZE_DSINFOBUF 0x100

    MS_BOOL bRet = TRUE;
    MVD_FWBuff* pBuff = &(pMVDHalContext->stFWBuff[u8Idx]);

    pBuff->u32DynScalingAdd = u32VPUAddr;
    pBuff->u32ScalerInfoAdd = u32DrvAddr + SIZE_DSBUF - SIZE_DSINFOBUF;  //FIX ME
...
E_MVD_Result HAL_MVD_IsAlive(MS_U8 u8Idx)
{
    ...
    printf("[%s]%d,%d,%d,%d,%d,%d,%d,%d,%d\n",__FUNCTION__,
                                        ...
                                        (MS_U32)(MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_IDLE_COUNT, sizeof(MS_U32))));
    #endif
...
E_MVD_DISP_QUEUE HAL_MVD_IsDispQueueEmpty(MS_U8 u8Idx)
{
    ...
    //printf("dispQ_rd: %u,dispQ_wr: %u\n",pTemp->dispQ_rd,pTemp->dispQ_wr);
    if(pTemp)
    {
        if (pTemp->dispQ_rd == pTemp->dispQ_wr)
            return MVD_DISP_QUEUE_EMPTY;
        else
            return MVD_DISP_QUEUE_NOT_EMPTY;
    }
    else
        return MVD_DISP_QUEUE_INVALID_ADDR;
}
