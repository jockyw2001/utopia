...
MS_BOOL HAL_VPU_EX_ReleaseFreeStream(MS_U8 u8Idx)
{
    ...
    {
        pVPUHalContext->_stVPUStream[u8Idx].eStreamId = E_HAL_VPU_N_STREAM0;
        pVPUHalContext->_stVPUStream[0].eDecodertype = E_VPU_EX_DECODER_NONE;
        pVPUHalContext->_stVPUStream[1].eDecodertype = E_VPU_EX_DECODER_NONE;
    }
...
MS_U16 HAL_VPU_EfuseRead2Byte(MS_VIRT u32Base,MS_U32 u32RegAddr)
{
    return ((volatile MS_U16*)(u32Base))[u32RegAddr];
}

MS_U16 HAL_VPU_EfuseWrite2Byte(MS_VIRT u32Base,MS_U32 u32RegAddr, MS_U16 u16Val)
{
    return ((volatile MS_U16*)(u32Base))[u32RegAddr] = u16Val;
}
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: HAL_VPU_EX_Efuse_Support_VPX()
/// @brief \b Function \b Description: For Mazda to turn off VP8/VP9
/// @return - success/fail
//-----------------------------------------------------------------------------
MS_BOOL HAL_VPU_EX_Efuse_Support_VPX(void)
{
    ...
    if (!MDrv_MMIO_GetBASE( &u32RiuBaseAdd, &u32NonPMBankSize, MS_MODULE_PM))
    {
        VPRINTF("MMIO_GetBASE failure\n");
        return FALSE;
    }
    else
    {
        VPRINTF("u32RiuBaseAdd=0x%lx\n", (unsigned long)u32RiuBaseAdd);
    }

    VPU_REG_ADDRESS u16RiuRegEfuseToggle;
    u16RiuRegEfuseToggle.reg_bank = 0x20;
    u16RiuRegEfuseToggle.reg_offset =0x2c*2;

    VPU_REG_ADDRESS u16RiuRegDisableVPX;
    u16RiuRegDisableVPX.reg_bank = 0x20;
    u16RiuRegDisableVPX.reg_offset = 0x28*2;

    HAL_VPU_EfuseWrite2Byte(u32RiuBaseAdd,u16RiuRegDisableVPX.reg__bank_offset,0x21BC);//0x28

    while ((HAL_VPU_EfuseRead2Byte(u32RiuBaseAdd,u16RiuRegDisableVPX.reg__bank_offset) & 0x2000UL) != 0)
    {
        if (u32timeout_count++ > MAX_TIMEOUT_COUNT)
        {
            VPRINTF ("[Error] %s(%d) Read time out!!\n", __FUNCTION__, __LINE__);
            return FALSE;
        }
        MsOS_DelayTaskUs(10);
    }

    reg_val = HAL_VPU_EfuseRead2Byte(u32RiuBaseAdd,u16RiuRegEfuseToggle.reg__bank_offset);//0x2c
    VPRINTF("\033[1;32m[%s] %d  0x2058UL reg_val = 0x%x \033[m\n",__FUNCTION__,__LINE__,reg_val);

    if(reg_val&0x80)//bit 7, regret bit
        return TRUE;
    else if(reg_val&0x20)//bit 5, disable bit
        return FALSE;
    else
        return TRUE;;
}
...
#define MAX_CODEC_TYPE_NUM 18
    ...
    unsigned int capability[MAX_CODEC_TYPE_NUM][MAX_CAPABILITY_INFO_NUM] =
    {
            ...
            { 1920, 1080,     30,     E_VDEC_EX_CODEC_PROFILE_NONE,             E_VDEC_EX_CODEC_LEVEL_NONE,         E_VDEC_EX_CODEC_VERSION_NONE,         40,        0},//E_HVD_EX_CODEC_TYPE_MJPEG
            { 1920, 1080,     30,     E_VDEC_EX_CODEC_PROFILE_H264_HIP,         E_VDEC_EX_CODEC_LEVEL_H264_5_1,     E_VDEC_EX_CODEC_VERSION_NONE,        135,        0},//E_HVD_EX_CODEC_TYPE_MVC
    ...
    if(!HAL_VPU_EX_Efuse_Support_VPX() && (E_HVD_EX_CODEC_TYPE_VP9 == eCodecType || E_HVD_EX_CODEC_TYPE_VP8 == eCodecType))
    {
        return FALSE;
    }
...
*/
    ...
    if(pTaskInfo->eDecType == E_VPU_EX_DECODER_MVD) // MVD case
    {
        ...
        else
        {
            ...
            else if (slq_state[u8TaskId].u32Used == FALSE) { // 2nd priority is task id
    ...
    else
    {
        ...
        else
        {
            ...
            else if (bbu_state[u8TaskId].u32Used == FALSE) { // 2nd priority is task id
                bbu_state[u8TaskId].u32Used |= (1 << u8TaskId);
                return u8TaskId;
            }
            else if (avaliable_bbu_idx != HAL_VPU_INVALID_BBU_ID) { // 3rd priority is avaliable bbu id
...
#include "halVPU_EX.h"
#include "drvMMIO.h"
#include "../../../../vdec_v2/hal/macan/hvd_ex/regHVD_EX.h"
#include "halCHIP.h"
