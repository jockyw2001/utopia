...
#define  _HAL_HVD_Entry()                                                       \
    ...
    {                                                                           \
        ...
        {                                                                       \
            ...
            {                                                                   \
                VPRINTF("[HAL HVD][%06d] Mutex taking timeout\n", __LINE__);     \
            }                                                                   \
        }                                                                       \
    } while (0)
...
#define HVD_MVD_RW_ON_MIU0              ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(4)) == 0)
#define HVD_MVD_BBU_R_ON_MIU0           ((_HVD_Read2Byte(MIU0_REG_SEL0) & BIT(12)) == 0)
#define HVD_HVD_RW_ON_MIU0              ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(2)) == 0)
#define HVD_HVD_BBU_R_ON_MIU0           ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(1)) == 0)
...
#if SUPPORT_EVD
#define _MaskMiuReq_EVD_RW( m )         _HVD_WriteRegBit(MIU0_REG_RQ2_MASK+1, m, BIT(2))
#define _MaskMiuReq_EVD_BBU_R( m )      _HVD_WriteRegBit(MIU0_REG_RQ2_MASK+1, m, BIT(2))
#define _MaskMiu1Req_EVD_RW( m )        _HVD_WriteRegBit(MIU1_REG_RQ2_MASK+1, m, BIT(2))
#define _MaskMiu1Req_EVD_BBU_R( m )     _HVD_WriteRegBit(MIU1_REG_RQ2_MASK+1, m, BIT(2))

#define HVD_EVD_RW_ON_MIU0              ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(10)) == 0)
#define HVD_EVD_BBU_R_ON_MIU0           ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(10)) == 0)
#define HVD_EVD_RW_ON_MIU1              ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(10)) == BIT(10))
#define HVD_EVD_BBU_R_ON_MIU1           ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(10)) == BIT(10))
#endif
...
#ifdef VDEC3
#define HAL_HVD_EX_MAX_SUPPORT_STREAM   4
#else
...
static void _HVD_EX_Context_Init_HAL(void)
{
    ...
    pHVDHalContext->u32HVDClockType = 240;
#if SUPPORT_EVD
...
//    MS_U32 u32StartOffset;
    ...
    if (E_HVD_INIT_HW_VP9 == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK))
    {
        ...
        {
            HVD_EX_MSG_INF("G2 VP9 power on\n");
...
RESET:
    ...
    if (isEVD)
    {
        ...
        // This is for Mainz using 256 bits bbu.
        // If BBU has its own port, need to set bbu 128/256; otherwise, keep default value reg_bbu_miu_256 = 1
                                          ...
                                          MSOS_SHM_QUERY))
    {
                                             ...
                                             MSOS_SHM_CREATE))
        {
            ...
            if(pHVDHalContext == NULL)
            {
                ...
                HVD_EX_MSG_INF("[%s]Global structure init Success!!!\n",__FUNCTION__);
            }
            else
            {
                HVD_EX_MSG_INF("[%s]Global structure exists!!!\n",__FUNCTION__);
            }
            //return FALSE;
        }
        else
...
void HAL_HVD_EX_PowerCtrl(MS_BOOL bEnable)
{
    ...
    switch (pHVDHalContext->u32HVDClockType)
    {
        ...
        case 240:  // Mainz HVD5P, default use this
        {
...
#endif
{
    ...
    //Chip info
    pShm->u16ChipID = E_MSTAR_CHIP_MAINZ;
	...
	/* Mainz AVC does not support 4k2k; do not use IMI
...
MS_VIRT HAL_HVD_EX_GetData(MS_U32 u32Id, HVD_GetData eType)
{
    ...
    if(pShm == NULL)
    {
        HVD_EX_MSG_ERR("########## VDEC patch for Debug ###########\n");
        return 0x0;
    }
...
void HAL_HVD_EX_Dump_FW_Status(MS_U32 u32Id)
{
    ...
    if (u32UartCtrl & E_HVD_UART_CTRL_DBG)
    {
        ...
        HVD_EX_MSG_INF("\tSystime=%u, FWVersionID=0x%x, FwState=0x%x, ErrCode=0x%x, ProgCnt=0x%x\n",
            HVD_GetSysTime_ms(), pShm->u32FWVersionID, pShm->u32FwState, (MS_U32) pShm->u16ErrCode, HAL_VPU_EX_GetProgCnt());

        HVD_EX_MSG_INF("\tTime: DispSTC=%u, DispT=%u, DecT=%u, CurrentPts=%u, Last Cmd=0x%x, Arg=0x%x, Rdy1=0x%x, Rdy2=0x%x\n",
        ...
        HVD_EX_MSG_INF("\tFlag: InitDone=%d, SpsChange=%d, IsIFrmFound=%d, 1stFrmRdy=%d, SyncStart=%d, SyncReach=%d\n",
                    pShm->bInitDone, pShm->bSpsChange, pShm->bIsIFrmFound,
                pShm->bIs1stFrameRdy, pShm->bIsSyncStart, pShm->bIsSyncReach);

        HVD_EX_MSG_INF("\tQueue: BBUQNumb=%u, DecQNumb=%d, DispQNumb=%d, ESR=%u, ESRfromFW=%u, ESW=%u, ESLevel=%u\n",
        ...
        HVD_EX_MSG_INF("\tCounter: DecodeCnt=%u, DispCnt=%u, DataErrCnt=%u, DecErrCnt=%u, SkipCnt=%u, DropCnt=%u, idle=%u, MainLoopCnt=%u, VsyncCnt=%u\n",
