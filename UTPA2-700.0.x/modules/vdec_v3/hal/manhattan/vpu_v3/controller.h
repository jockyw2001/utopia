...
#define VSYNC_BRIDGE_OFFSET      0xC000
#define VSYNC_BRIDGE_EXT_OFFSET  0xC800
...
#define DS_IPOP_PAGE_OFFSET      0x7000
#define DS_SCALER_INFO_OFFSET    0x8F00
#endif
...
struct _ctl_info {
    ...
    unsigned int u32HVDBBUTblStAddr;      // CTL_INFO_ADDR + 0x90
    unsigned int u32EVDBBUTblStAddr;      // CTL_INFO_ADDR + 0x94
    unsigned int u32VP8BBUTblStAddr;      // CTL_INFO_ADDR + 0x98
    unsigned int u32MVDSLQTblStAddr;      // CTL_INFO_ADDR + 0x9C
    unsigned int u32VsyncBridgeAddr;      // CTL_INFO_ADDR + 0xA0
    unsigned int u32VsyncBridgeExtAddr;   // CTL_INFO_ADDR + 0xA4
    unsigned int u32DSIPOPAddr;           // CTL_INFO_ADDR + 0xA8
    unsigned int u32DSScalerInfoAddr;     // CTL_INFO_ADDR + 0xAC

    unsigned int FB_ADDRESS;              // CTL_INFO_ADDR + 0xB0 , this value is offset of miu, unit is byte
    unsigned int FB_Total_SIZE;           // CTL_INFO_ADDR + 0xB4 , unit is byte
    unsigned int FB_Used_SIZE;            // CTL_INFO_ADDR + 0xB8 , unit is byte
#ifdef VDEC3_FB
    unsigned int u32FrameBufAddr;         // CTL_INFO_ADDR + 0xBC frame buffer base address
    unsigned int u32FrameBufSize;         // CTL_INFO_ADDR + 0xC0 frame buffer size for all tasks
    unsigned char u8FrameBufSegment;      // CTL_INFO_ADDR + 0xC4 select one enumeration from Split_FB
    unsigned char bFrameBufUsed[4];       // CTL_INFO_ADDR + 0xC5 record if each segment is used.
#endif
    unsigned char u8UseIMITaskId;         // CTL_INFO_ADDR + 0xC9 indicate which task is using IMI
    unsigned char u8HicodecType;          // CTL_INFO_ADDR + 0xCA Kano, 0:Hicodec 1:Hicodec_Lite
    unsigned char u8MultiHVDTaskSameBBU;  // CTL_INFO_ADDR + 0xCB
    unsigned int  u32DolbyVisionXCShmAddr;// CTL_INFO_ADDR + 0xCC record the dolby vision XC share memory address for transfer DM/composer
    unsigned int  u32DVDumpMetaAddrWithXC;// CTL_INFO_ADDR + 0xD0 record dolby meta XC test pattern
    unsigned char STCindex[2];            // CTL_INFO_ADDR + 0xD4
    unsigned char STCMode[2];             // CTL_INFO_ADDR + 0xD6
#if defined(SUPPORT_VDEC_STR)
    unsigned int  u32StrAddrOffset;       // CTL_INFO_ADDR + 0xD8
#endif
} ;
