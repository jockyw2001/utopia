...
#define MAX_SUPPORT_DECODER_NUM 4
#else
...
#define VPU_MSG_ERR(format, args...)                \
    ...
    {                                               \
        ...
        {                                           \
            ULOGE("VDEC","[VPU][ERR]%s:", __FUNCTION__);  \
            ULOGE("VDEC",format, ##args);                 \
        }                                           \
    } while (0)
...
#define VPU_MSG_DBG(format, args...)                \
    ...
    {                                               \
        ...
        {                                           \
            ULOGD("VDEC","[VPU][DBG]%s:", __FUNCTION__);  \
            ULOGD("VDEC",format, ##args);                 \
        }                                           \
    } while (0)
...
#define VPU_MSG_INFO(format, args...)               \
    ...
    {                                               \
        ...
        {                                           \
            ULOGI("VDEC","[VPU][INF]%s:", __FUNCTION__);  \
            ULOGI("VDEC",format, ##args);                 \
        }                                           \
    } while (0)
...
#define _MaskMiuReq_VPU_I_R(m)      _VPU_WriteRegBit(MIU0_REG_RQ0_MASK, m, BIT(6))
...
#define _MaskMiu1Req_VPU_I_R(m)     _VPU_WriteRegBit(MIU1_REG_RQ0_MASK, m, BIT(6))

#define VPU_D_RW_ON_MIU0            ((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(5)) == 0)
#define VPU_Q_RW_ON_MIU0            ((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(5)) == 0)
#define VPU_I_R_ON_MIU0             ((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == 0)
...
#define VPU_I_R_ON_MIU1             ((_VPU_ReadByte(MIU0_REG_SEL0) & BIT(6)) == BIT(6))
#else
#define _MaskMiuReq_VPU_D_RW(m)     _VPU_WriteRegBit(MIU0_REG_RQ0_MASK, m, BIT(5))
#define _MaskMiuReq_VPU_Q_RW(m)     _VPU_WriteRegBit(MIU0_REG_RQ0_MASK, m, BIT(5))
#define _MaskMiuReq_VPU_I_R(m)      _VPU_WriteRegBit(MIU0_REG_RQ0_MASK, m, BIT(6))
...
#endif
{
    ...
    if (E_VPU_EX_DECODER_MVD == pTaskInfo->eDecType)
    {
            VDEC_VBBU *pTemp4 = (VDEC_VBBU *)MsOS_PA2KSEG1(MsOS_VA2PA(pInitPara->pFWCodeCfg->u32DstAddr) + VBBU_TABLE_START + u8Offset*0x100000);

            memset(pTemp4,0,sizeof(VDEC_VBBU));

            *((unsigned int*)(pTemp4->u8Reserved)) = MsOS_VA2PA(pInitPara->pFWCodeCfg->u32DstAddr)-HAL_MIU1_BASE;

            DISPQ_IN_DRAM *pTemp = (DISPQ_IN_DRAM *)MsOS_PA2KSEG1(MsOS_VA2PA(pInitPara->pFWCodeCfg->u32DstAddr) + DISP_QUEUE + u8Offset*0x100000);

            memset(pTemp,0,sizeof(DISPQ_IN_DRAM));

            CMD_QUEUE *pTemp2 = (CMD_QUEUE *)MsOS_PA2KSEG1(MsOS_VA2PA(pInitPara->pFWCodeCfg->u32DstAddr) + VCOMMANDQ_INFO + u8Offset*0x100000);

            memset(pTemp2,0,sizeof(CMD_QUEUE));

            pTemp2->u32HVD_DISPCMDQ_DRAM_ST_ADDR = VDISP_COMMANDQ + u8Offset*0x100000;

            pTemp2->u32HVD_CMDQ_DRAM_ST_ADDR = VNORMAL_COMMANDQ + u8Offset*0x100000;

            unsigned char* pTemp3 = (unsigned char*)MsOS_PA2KSEG1(MsOS_VA2PA(pInitPara->pFWCodeCfg->u32DstAddr) + VDISP_COMMANDQ + u8Offset*0x100000);

            memset(pTemp3,0,0x2000);

            unsigned int* pVersion = (unsigned int*)MsOS_PA2KSEG1(MsOS_VA2PA(pInitPara->pFWCodeCfg->u32DstAddr) + OFFSET_BASE + u8Offset*0x100000);
                                          ...
                                          MSOS_SHM_QUERY))
    {
                                             ...
                                             MSOS_SHM_CREATE))
        {
            ...
            if(pVPUHalContext == NULL)
            {
                ...
                VPU_MSG_INFO("[%s]Global structure init Success!!!\n",__FUNCTION__);
            }
            else
            {
                VPU_MSG_INFO("[%s]Global structure exists!!!\n",__FUNCTION__);
            }
            //return FALSE;
        }
        else
...
MS_BOOL HAL_VPU_EX_SwRst(MS_BOOL bCheckMauIdle)
{
    ...
    if (tempreg & 0xf) {
        ...
        if (idle_cnt == 0)
        {
             VPU_MSG_INFO("BIU DCU idle time out~~~~~\n");
        }
        ...
        {
            if (--idle_cnt == 0) {
                VPU_MSG_INFO("BIU ICU idle time out~~~~~\n");
                break;
            }
            MsOS_DelayTask(1);
        }
...
//MAU has been removed since manhattan, so it is not necessary to check MAU status
    ...
    if (bCheckMauIdle)
    {
        ...
        if (mau_idle_cnt == 0)
        {
            VPU_MSG_INFO("MAU idle time out~~~~~\n");
        }
    }
#endif
...
#define MAX_CODEC_TYPE_NUM 18
    ...
    unsigned int capability[MAX_CODEC_TYPE_NUM][MAX_CAPABILITY_INFO_NUM] =
    {
            ...
            { 3840, 2160,     30,     E_VDEC_EX_CODEC_PROFILE_H264_HIP,         E_VDEC_EX_CODEC_LEVEL_H264_5,       E_VDEC_EX_CODEC_VERSION_NONE,         0,        0},//E_HVD_EX_CODEC_TYPE_H264
            ...
            { 1920, 1080,     30,     E_VDEC_EX_CODEC_PROFILE_NONE,             E_VDEC_EX_CODEC_LEVEL_NONE,         E_VDEC_EX_CODEC_VERSION_NONE,         0,        0},//E_HVD_EX_CODEC_TYPE_MJPEG
            ...
            { 4096, 2160,     60,     E_VDEC_EX_CODEC_PROFILE_H265_MAIN_10,     E_VDEC_EX_CODEC_LEVEL_H265_5_1_HT,  E_VDEC_EX_CODEC_VERSION_NONE,         0,        0},//E_HVD_EX_CODEC_TYPE_HEVC
            { 4096, 2160,     60,     E_VDEC_EX_CODEC_PROFILE_VP9_2,            E_VDEC_EX_CODEC_LEVEL_NONE,         E_VDEC_EX_CODEC_VERSION_NONE,         0,        0},//E_HVD_EX_CODEC_TYPE_VP9
    };
    ...
    MS_U64 *offset, MS_SIZE *length, MS_U64 total_length, MS_SIZE unUseSize)
{
    ...
    VPU_MSG_INFO("[HAL][%s]:[%d] total_length:%llu, cType:%d, sType:%d\n", __FUNCTION__, __LINE__,
    ...
    VPU_MSG_INFO("[HAL][%s]:[%d] FrameSize:%llu, offset:%llu, length:%llu ", __FUNCTION__, __LINE__,
