...
//  Hardware Capability
//-------------------------------------------------------------------------------------------------
#define HVD_FW_VERSION 0x00001491
#define HVD_FW_IF_VERSION 0x00740191
...
#else
#define HVD_VBBU_DRAM_ST_SIZE           (0x0)
#define HVD_CMDQ_DRAM_ST_SIZE           (0x0) //Command Queue must align (CMD + ARG) length, ex. 8 bytes, or will encounter bug when use circular buff
#define HVD_DISPCMDQ_DRAM_ST_SIZE   (0x0) //Command Queue must align (CMD + ARG) length, ex. 8 bytes, or will encounter bug when use circular buff
...
#if defined(VDEC3)
#define MAX_PTS_TABLE_SIZE              1024
#else
...
#if defined(VDEC3)
#define HVD_BBU_DRAM_TBL_ENTRY          (0x2000/8) // bbu entry. 64bits(8 bytes) every entry. This variable should consist with "EVD_HVD_BBU_NAL_TABLE_SZIE" in controller.h
#else
...
#if defined(VDEC3)
#define RVD_BBU_DRAM_TBL_ENTRY          (0x2000/8) // bbu entry. 64bits(8 bytes) every entry. This variable should consist with "EVD_HVD_BBU_NAL_TABLE_SZIE" in controller.h
#else
...
#define VSYNC_BRIGE_EXT_SHM_ADDR        0xD0000
#else
#define HVD_SHARE_MEM_ST_OFFSET         0xA0000
#define VSYNC_BRIGE_SHM_ADDR            0xBFA00
#define VSYNC_BRIGE_EXT_SHM_ADDR        0xC0000
#endif
...
#if 1
#ifdef VDEC3
#define INPUT_QUEUE_OFFSET              0xE0000
#define HVD_PTS_TABLE_ST_OFFSET         0xE4000
#define MAX_PTS_TABLE_SIZE              1024
#else
...
// For Git Changes
#define GIT_TIMESTAMP 1472564636
...
typedef struct
{
    ...
    volatile MS_U8 u8IQmemCtrl;         //0x0CE9
    MS_U8 bIsIQMEMSupport;              //0x0CEA
    MS_U8 bIQmemEnableIfSupport;        //0x0CEB
    ...
    MS_U8  bIsTSPIn;                    //0x0DE9
    MS_U8  u8NumRefFrame;               //0x0DEA
    MS_U8  bUseCorrectVlcAddr;          //0x0DEB
    DynmcDispPath    stDynmcDispPath;   //0x0DEC
    MS_U8 reserved8_8[1];                   // 0x0DEF
    MS_U32 u32NotSupportInfo;       // 0x0DF0
    MS_U32 u32CurMinTspDataSize;        //0x0DF4 //byte
    MS_U8  reserved8_9[0xF9C-0xDF8];    //0x0DF8
...
{
    MS_U8 u8Version;
    MS_U8 u8Current_index;
    MS_U32 u32DM_length;
    MS_U32 u32DM_addr;
    MS_U8 u8Compse_enable;
    MS_U32 u32Comp_length;
    MS_U32 u32Comp_addr;
    MS_U8 u8Reserved[13];
} HVD_DV_XC_ShareMem;
...
typedef enum
{
    ...
    E_MSTAR_CHIP_M12,
    E_MSTAR_CHIP_EAGLE,     //remove
    E_MSTAR_CHIP_EMERALD,    //remove
    E_MSTAR_CHIP_EDISON,    //remove
    E_MSTAR_CHIP_EIFFEL,    //remove
    E_MSTAR_CHIP_CEDRIC,
    E_MSTAR_CHIP_NUGGET,    //remove
    E_MSTAR_CHIP_KAISER,
    E_MSTAR_CHIP_NIKE,    //remove
    E_MSTAR_CHIP_KENYA,
    E_MSTAR_CHIP_EINSTEIN,    //remove
    E_MSTAR_CHIP_NIKON,    //remove
    E_MSTAR_CHIP_NAPOLI,
...
typedef enum
{
    ...
    /*0x20220*/E_HVD_CMD_AVC_FORCE_BROKEN_BY_US,    // Arg, 0: Disable, 1:enable, force enable broken by us mode. FW does need it anymore.
    ...
    /*0x2022C*/E_HVD_CMD_G2VP9_FHD_BW_BALANCE_MODE, // Arg, 0:disable, 1:enable.G2_VP9 will do bandwidth balance mechanism when resolution greater than FHD. Otherwise, it would be 2560*1400
    ...
    /*0x2032C*/E_HVD_CMD_DYNAMIC_CONNECT_DISP_PATH, // Arg, bit[0]: connect/disconnect, bit[4:1]: display path, [31:5]: reserved.
    /*0x2032D*/E_HVD_CMD_AVSYNC_DISP_AUTO_DROP,     // Arg, 0:disable, 1:enable.
    /*0x2032E*/E_HVD_CMD_SET_SLOW_SYNC,                  // Arg, bits[31:16]: reserved, bits[15:8]: slow repeat frequency (0: disable slow repeat), bits[7:0]: slow drop frequency (0: disable slow drop).
    /*0x2032F*/E_HVD_CMD_ENABLE_QOS_INFO,           // Arg, 0:disable, 1:enable. report qos info
