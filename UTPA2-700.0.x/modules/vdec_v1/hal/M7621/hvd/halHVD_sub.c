...
static MS_U8 _u8HVD_Sub_Mutex[] = {"HVD_Sub_Mutex"};

#define _HAL_HVD_MutexCreate()                                                     \
            if( s32HVDMutexID < 0 )                                                 \
            {                                                                                                       \
                s32HVDMutexID = OSAL_HVD_MutexCreate( _u8HVD_Sub_Mutex );        \
            }
...
static MS_U32 u32VPUClockType = 144;
static MS_U32 u32HVDClockType = 144;
...
static MS_U32 UDMA_fpga_HVDShareMemAddr=0;
#endif
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
...
//  Local Functions
//-------------------------------------------------------------------------------------------------
MS_U32 _HAL_HVD_Sub_GetBBUQNumb(void);


static MS_U16 _HAL_HVD_Sub_GetBBUReadptr(void)
{
    MS_U16 u16Ret=0;
    //_HAL_HVD_Entry();
...
static void _HAL_HVD_Sub_SetBBUWriteptr(MS_U16 u16BBUNewWptr )
{
...
static MS_BOOL _HAL_HVD_Sub_MBoxSend(MS_U8 u8MBox, MS_U32 u32Msg)
{
...
static MS_BOOL _HAL_HVD_Sub_MBoxReady(MS_U8 u8MBox)
{
    MS_BOOL bResult = TRUE;
    switch(u8MBox)
    {
...
static MS_BOOL _HAL_HVD_Sub_MBoxRead(MS_U8 u8MBox, MS_U32 *u32Msg)
{
    MS_BOOL bResult = TRUE;
    switch(u8MBox)
    {
...
static void _HAL_HVD_Sub_MBoxClear(MS_U8 u8MBox)
{
    switch(u8MBox)
    {
    case E_HVD_RISC_0:
...
void HAL_HVD_Sub_Dump_HW_Status(MS_U32 numb)
{
    MS_U32 i=0;
    MS_U32 value=0;
    HVD_SUB_MSG_DEG("HVD Dump HW status:");
    for(  i=0 ; i <= numb ; i++ )
    {
...
static MS_U32 _HAL_HVD_Sub_GetPC(void)
{
    MS_U32 u32PC=0;
    u32PC = HAL_VPU_GetProgCnt();
//    HVD_SUB_MSG_DEG("<gdbg>pc0 =0x%lx\n",u32PC);
    return u32PC;
}

static MS_U32 _HAL_HVD_Sub_GetFWState(void)
{
    MS_U32 u32Ret=0;
    if( _HAL_HVD_Sub_MBoxRead(HAL_HVD_REG_FW_STATE , &u32Ret ))
    {
        return u32Ret;
    }
    else
...
static MS_U32 _HAL_HVD_Sub_GetESWritePtr(void)
{
    MS_U32 data=0;
    if( (pHVDCtrl_Hal->InitParams.u32ModeFlag&E_HVD_INIT_INPUT_MASK )==E_HVD_INIT_INPUT_DRV)
    {
        ...
        if( data > pHVDCtrl_Hal->MemMap.u32BitstreamBufSize)
        {
            ...
            HVD_SUB_MSG_ERR( "HVD HAL: _HAL_HVD_Sub_GetESWritePtr(): app should not put this kind of packet\n");
        }
    }
    else
    {
...
static MS_U32 _HAL_HVD_Sub_GetESReadPtr(MS_BOOL bDbug)
{
    MS_U32 data=0;
    ...
    || (TRUE == bDbug))
    {
        ...
        _HVD_WriteWordMask(HVD_REG_ESB_RPTR_BS2, 0, HVD_REG_ESB_RPTR_POLL);
        // set reg_poll_nal_rptr 1
        _HVD_WriteWordMask(HVD_REG_ESB_RPTR_BS2, HVD_REG_ESB_RPTR_POLL, HVD_REG_ESB_RPTR_POLL);
        // read reg_nal_rptr_hi
#if 0
        if( HVD_HW_RUBBER3 )
        {
            data=_HVD_Read2Byte(  HVD_REG_ESB_RPTR_BS2 ) & 0xFF80;
            data>>=7;
            data |= _HVD_Read2Byte(  HVD_REG_ESB_RPTR_H_BS2 ) << 9;
        }
        else    // rubber2
#endif
    //_HAL_HVD_Entry();
        data=_HVD_Read2Byte(  HVD_REG_ESB_RPTR_BS2 ) & 0xFFC0;
        data>>=6;
        data |= _HVD_Read2Byte(  HVD_REG_ESB_RPTR_H_BS2 ) << 10;
    //_HAL_HVD_Release();
    // patch for XDemux
    #if 0
    /*
    ...
    */
        ...
        if( (pHVDCtrl_Hal->InitParams.u32ModeFlag&E_HVD_INIT_INPUT_MASK )==E_HVD_INIT_INPUT_DRV)
        {
            ...
            else if(  _HAL_HVD_Sub_GetBBUQNumb() ==0 &&  (( data - u32ESWptr )< 16) && (( _HAL_HVD_Sub_GetFWState() & E_HVD_FW_STATE_MASK )==E_HVD_FW_PLAY))
            {
                //HVD_SUB_MSG_INFO("HVD Warn: ESRptr(%lx %lx) is running over ESWptr(%lx)\n" ,  data , pHVDCtrl_Hal->u32LastESRptr, u32ESWptr  );
                data = u32ESWptr;
            }
...
static MS_BOOL _HAL_HVD_Sub_SetCMDArg(MS_U32 u32Arg)
{
    MS_U16 u16TimeOut = 0xFFFF;
    MS_BOOL bResult = FALSE;

    //HVD_SUB_MSG_DEG("Send argument 0x%lx to HVD \n", u32Arg);
    while(--u16TimeOut)
    {
        if( _HAL_HVD_Sub_MBoxReady(HAL_HVD_CMD_MBOX)&&
            _HAL_HVD_Sub_MBoxReady(HAL_HVD_CMD_ARG_MBOX))
        {
            bResult = _HAL_HVD_Sub_MBoxSend(HAL_HVD_CMD_ARG_MBOX, u32Arg);
            break;
        }
    }
    return bResult;
...
static MS_BOOL _HAL_HVD_Sub_SetCMD(MS_U32 u32Cmd)
{
    MS_U16 u16TimeOut = 0xFFFF;
    MS_BOOL bResult = FALSE;

    //HVD_SUB_MSG_DEG("Send CMD 0x%lx to HVD \n", u32Cmd);
    while(--u16TimeOut)
    {
        if(_HAL_HVD_Sub_MBoxReady(HAL_HVD_CMD_MBOX))
        {
            bResult = _HAL_HVD_Sub_MBoxSend(HAL_HVD_CMD_MBOX, u32Cmd);
            break;
        }
    }
    return bResult;
...
static HVD_Return _HAL_HVD_Sub_SendCmd( MS_U32 u32Cmd , MS_U32 u32CmdArg)
{
    MS_U32 u32timeout= HVD_GetSysTime_ms() +u32HVDCmdTimeout ;
    while( !_HAL_HVD_Sub_SetCMDArg( u32CmdArg )  )
    {
        if( HVD_GetSysTime_ms()  >  u32timeout )
        {
            HVD_SUB_MSG_ERR( "HVD cmd:%lx ;cmd arg timeout:%lx\n" , u32Cmd , u32CmdArg  );
            return E_HVD_RETURN_TIMEOUT;
        }
        if( u32Cmd == E_HVD_CMD_STOP  )
        {
            _HAL_HVD_Sub_MBoxSend(HAL_HVD_CMD_MBOX, E_HVD_CMD_STOP);
            _HAL_HVD_Sub_MBoxSend(HAL_HVD_CMD_ARG_MBOX, 0);
            HVD_SUB_MSG_ERR( "HVD cmd force stop:%lx ;cmd arg:%lx\n" , u32Cmd , u32CmdArg  );
    ...
    while( !_HAL_HVD_Sub_SetCMD( u32Cmd )  )
    {
        if( HVD_GetSysTime_ms()  >  u32timeout )
        {
            HVD_SUB_MSG_ERR( " cmd timeout: %lx\n" , u32Cmd );
            return E_HVD_RETURN_TIMEOUT;
        }
        //_HAL_HVD_Sub_GetPC();
        HAL_HVD_Sub_Dump_FW_Status();
...
static void _HAL_HVD_Sub_SetMIUProtectMask(MS_BOOL bEnable)
{
...
static void _HAL_HVD_Sub_RstMVDParser(void)
{
    _HAL_HVD_Sub_SetMIUProtectMask(TRUE);
//    _HVD_WriteRegBit(MVD_REG_STAT_CTRL, 1, MVD_REG_CTRL_RST|MVD_REG_CTRL_INIT);
    ...
    _HAL_HVD_Sub_SetMIUProtectMask(FALSE);
    return;
}

static void _HAL_HVD_Sub_SVD_Release(void)
{
    // release SW reset
...
static void _HAL_HVD_Sub_SetBufferAddr(void)
{
    MS_U16 u16Reg       = 0;
    MS_U32 u32StAddr    = 0;
    ...
    HVD_SUB_MSG_DEG("_HAL_HVD_Sub_SetBufferAddr: nal StAddr:%lx \n", u32StAddr);
    ...
    _HVD_Write2Byte(HVD_REG_NAL_TAB_LEN_BS2, (MS_U16)(u32BBUEntryNum - 1));
    ...
    HVD_SUB_MSG_DEG("_HAL_HVD_Sub_SetBufferAddr: ESb StAddr:%lx \n", u32StAddr);
    ...
    _HVD_Write2Byte(HVD_REG_ESB_LENGTH_L_BS2,
                    HVD_LWORD(pHVDCtrl_Hal->MemMap.u32BitstreamBufSize >> 3));
    _HVD_Write2Byte(HVD_REG_ESB_LENGTH_H_BS2,
...
#if 0
    {
        ...
        HVD_SUB_MSG_DEG("_HAL_HVD_Sub_SetBufferAddr: MIF offset:%lx \n" , u32StAddr);
        offaddr = (MS_U16)((u32StAddr )>>20);
        offaddr &= BMASK(  HVD_REG_MIF_OFFSET_L_BITS_BS2 :0 );//0x1FF;   // 9 bits(L + H)
        u16Reg=_HVD_Read2Byte(HVD_REG_MIF_BBU_BS2) ;
        u16Reg&= ~HVD_REG_MIF_OFFSET_H_BS2 ;
        u16Reg&=~(BMASK( HVD_REG_MIF_OFFSET_L_BITS_BS2 :0)) ;
        if(offaddr & BIT( HVD_REG_MIF_OFFSET_L_BITS_BS2 ) )
        {
            u16Reg |= HVD_REG_MIF_OFFSET_H_BS2;
        }
        _HVD_Write2Byte(HVD_REG_MIF_BBU_BS2,
                       (u16Reg | (offaddr & BMASK(HVD_REG_MIF_OFFSET_L_BITS_BS2 :0))));
    }
#endif
}

static MS_U32 _HAL_HVD_Sub_GetESLevel(void)
{
    MS_U32 u32Wptr = 0;
    MS_U32 u32Rptr = 0;
    MS_U32 u32CurMBX=0;
    ...
    u32Wptr = _HAL_HVD_Sub_GetESWritePtr();
    u32Rptr = _HAL_HVD_Sub_GetESReadPtr(FALSE);
...
MS_BOOL _HAL_HVD_Sub_SwCPURst(void)
{
    ...
    if( !u16Timeout )
    {
        HVD_SUB_MSG_ERR("_HAL_HVD_Sub_SwCPURst timeout \n");
    }

    return (u16Timeout>0) ? TRUE : FALSE;
}

MS_BOOL _HAL_HVD_Sub_LoadVLCTable(HVD_FWInputSourceType eType  )
{
#if HVD_ENABLE_RV_FEATURE
    if(  eType == E_HVD_FW_INPUT_SOURCE_FLASH )
    {
    #if HVD_ENABLE_BDMA_FW_FLASH_2_SDRAM
        HVD_SUB_MSG_DEG("HVD Loading VLC outF2D: dest:0x%lx source:%lx size:%lx\n",  (MS_U32)pHVDCtrl_Hal->MemMap.u32FrameBufAddr+u32RV_VLCTableAddr ,
        ...
        if( pHVDCtrl_Hal->MemMap.u32VLCBinarySize != 0  )
        {
            ...
            {
                HVD_SUB_MSG_ERR("HVD ERR: HVD_BDMAcpy VLC table Flash 2 DRAM failed: dest:0x%lx src:0x%lx size:0x%lx flag:%lu\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr  , pHVDCtrl_Hal->MemMap.u32FWBinaryAddr  , pHVDCtrl_Hal->MemMap.u32FWBinarySize  , (MS_U32)cpyflag  );
                return FALSE;
            }
        }
        else
        {
            HVD_SUB_MSG_ERR("HVD ERR: During copy VLC from Flash to Dram, the source size of FW is zero\n");
            return FALSE;
        }
    #else
        HVD_SUB_MSG_ERR("HVD ERR: driver not enable to use BDMA copy VLC from flash 2 sdram.\n");
        return FALSE;
    #endif
    }
    else
    {
        if(  eType == E_HVD_FW_INPUT_SOURCE_DRAM)
        {
            ...
            {
                HVD_SUB_MSG_DEG("HVD Loading VLC outD2D: dest:0x%lx source:%lx size:%lx\n",  pHVDCtrl_Hal->MemMap.u32FrameBufVAddr+u32RV_VLCTableAddr ,
            ...
            {
                HVD_SUB_MSG_ERR("HVD ERR: During copy VLC from out Dram to Dram, the source size or virtual address of VLC is zero\n");
                return FALSE;
            }
        }
        else
        {
        #if HVD_ENABLE_EMBEDDED_FW_BINARY
            HVD_SUB_MSG_DEG("HVD Loading VLC inD2D: dest:0x%lx source:%lx size:%lx\n", pHVDCtrl_Hal->MemMap.u32FrameBufVAddr+u32RV_VLCTableAddr,
                ((MS_U32)u8HVD_VLC_Binary) , (MS_U32)sizeof(u8HVD_VLC_Binary) );
            HVD_memcpy( (void*)( pHVDCtrl_Hal->MemMap.u32FrameBufVAddr+u32RV_VLCTableAddr ),
                (void*)((MS_U32)u8HVD_VLC_Binary) , sizeof(u8HVD_VLC_Binary)  );
        #else
            HVD_SUB_MSG_ERR("HVD ERR: driver not enable to use embedded VLC binary.\n");
            return FALSE;
        #endif
        }
    }
...
MS_BOOL _HAL_HVD_Sub_SetRegCPU(void)
{
    MS_U32 u32FirmVer = 0;
    MS_U32 u32Timeout = 20000;
    MS_BOOL bNeedReloadFW = TRUE;

    HVD_SUB_MSG_DEG("HVD HW ver id: 0x%04lx\n", HAL_HVD_Sub_Get_HWVersionID()  );

    if(!_HAL_HVD_Sub_SwCPURst())
    {
        HVD_SUB_MSG_ERR("HVD reset failed...\n");
        return FALSE;
    }
    ...
    if(TRUE == bNeedReloadFW)
    {
        ...
        {
        #if HVD_ENABLE_BDMA_FW_FLASH_2_SDRAM
            HVD_SUB_MSG_DEG("HVD Loading FW outF2D: dest:0x%lx source:%lx size:%lx\n",  (MS_U32)pHVDCtrl_Hal->MemMap.u32CodeBufAddr ,
            ...
            if( pHVDCtrl_Hal->MemMap.u32FWBinarySize != 0  )
            {
                ...
                {
                    HVD_SUB_MSG_ERR("HVD ERR: HVD_BDMAcpy Flash 2 DRAM failed: dest:0x%lx src:0x%lx size:0x%lx flag:%lu\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr  , pHVDCtrl_Hal->MemMap.u32FWBinaryAddr  , pHVDCtrl_Hal->MemMap.u32FWBinarySize  , (MS_U32)cpyflag  );
                    return FALSE;
                }
            }
            else
            {
                HVD_SUB_MSG_ERR("HVD ERR: During copy FW from Flash to Dram, the source size of FW is zero\n");
                return FALSE;
            }
        #else
            HVD_SUB_MSG_ERR("HVD ERR: driver not enable to use BDMA copy FW Bin from flash 2 sdram.\n");
            return FALSE;
        #endif
        }
        else
        {
            if(  pHVDCtrl_Hal->MemMap.eFWSourceType == E_HVD_FW_INPUT_SOURCE_DRAM)
            {
                ...
                {
                    HVD_SUB_MSG_DEG("HVD Loading FW outD2D: dest:0x%lx source:%lx size:%lx\n",  pHVDCtrl_Hal->MemMap.u32CodeBufVAddr ,
                ...
                {
                    HVD_SUB_MSG_ERR("HVD ERR: During copy FW from out Dram to Dram, the source size or virtual address of FW is zero\n");
                    return FALSE;
                }
            }
            else
            {
            #if HVD_ENABLE_EMBEDDED_FW_BINARY
                HVD_SUB_MSG_DEG("HVD Loading FW inD2D: dest:0x%lx source:%lx size:%lx\n",  pHVDCtrl_Hal->MemMap.u32CodeBufVAddr ,
                    ((MS_U32)u8HVD_FW_Binary) , (MS_U32)sizeof(u8HVD_FW_Binary) );
                HVD_memcpy( (void*)( pHVDCtrl_Hal->MemMap.u32CodeBufVAddr ),
                    (void*)((MS_U32)u8HVD_FW_Binary), sizeof(u8HVD_FW_Binary) );
            #else
                HVD_SUB_MSG_ERR("HVD ERR: driver not enable to use embedded FW binary.\n");
                return FALSE;
            #endif
            }
        }
        ...
        if( ((pHVDCtrl_Hal->InitParams.u32ModeFlag) & E_HVD_INIT_HW_MASK )
           == E_HVD_INIT_HW_RM  )
        {
            if( _HAL_HVD_Sub_LoadVLCTable( pHVDCtrl_Hal->MemMap.eFWSourceType ) == FALSE)
            {
                return FALSE;
            }
        }
        _HVD_Sub_Chip_Flush_Memory();
        //HVD_SUB_MSG_DEG("HVD FW data compare: dest:0x%lx %lx %lx source:%lx %lx %lx\n",
        ...
        HVD_SUB_MSG_DEG("HVD Load FW done\n" );
    }
...
*/
        HVD_SUB_MSG_DEG("_HAL_HVD_Sub_SetRegCPU: VPU settings:%lx \n" , u32Addr);
    ...
    //T8: use miu128bit
    HVD_SUB_MSG_DEG("(be)Miu128 bits Status = %x <<<<<<<\n",_HVD_Read2Byte(HVD_REG_RESET));
    _HVD_Write2Byte(HVD_REG_RESET, (_HVD_Read2Byte(HVD_REG_RESET) | HVD_REG_RESET_MIU_128));
    HVD_SUB_MSG_DEG("(af)Miu128 bits Status = %x <<<<<<<\n",_HVD_Read2Byte(HVD_REG_RESET));

    _HAL_HVD_Sub_SetBufferAddr();
    _HAL_HVD_Sub_RstMVDParser();

    // release sw and cpu rst
    _HAL_HVD_Sub_SVD_Release();
    HVD_SUB_MSG_DEG("HVD CPU/HW release done\n" );

    while(u32Timeout)
    {
        u32FirmVer =HAL_HVD_Sub_GetData(E_HVD_GDATA_FW_INIT_DONE);
        if( u32FirmVer != 0)
        {
            u32FirmVer =HAL_HVD_Sub_GetData(E_HVD_GDATA_FW_VERSION_ID);
            break;
        }
        u32Timeout--;
        HVD_Delay_ms(1);
...
HVD_Return _HAL_HVD_Sub_UpdatePTSTable(HVD_BBU_Info* pInfo)
{
    ...
    {
        HVD_SUB_MSG_ERR("HVD ERR: PTS table full. Read Ptr(%lx) == new Write ptr(%lx) ,Pre Wptr(%lx) \n" ,u32PTSRptr,u32PTSWptr , u32PTSPreWptr );
        return E_HVD_RETURN_FAIL;
    }
    // add one PTS entry
...
#endif
    // update Write ptr
    if( !HAL_VPU_MemWrite(  u32PTSWptrAddr ,  u32PTSPreWptr) )
    {
        HVD_SUB_MSG_ERR("HVD ERR: PTS table SRAM write failed\n"  );
        return E_HVD_RETURN_FAIL;
    }
...
HVD_Return _HAL_HVD_Sub_UpdateESWptr(MS_U32 nal_offset , MS_U32 nal_len)
{
    //---------------------------------------------------
...
//    MS_U8 pbuf[HVD_MAX_PACKET_SIZE]; // temp buffer
    ...
    addr = gSubHVDCtrl.MemMap.u32CodeBufAddr + HVD_BBU_DRAM_ST_ADDR + ((u32BBUWptr-(u32BBUWptr%2))<<3);
    ...
    _HVD_Sub_Chip_Flush_Memory();
    //HVD_SUB_MSG_DEG( "in UpdateESWptr:%lx %lx %lx\n"  , addr , pHVDCtrl_Hal->MemMap.u32CodeBufVAddr  , u32BBUWptr );
#endif
    // add nal ptr
    //_HAL_HVD_Sub_SetBBUWriteptr( HVD_LWORD(u32BBUNewWptr) );
...
static MS_U32 _HAL_HVD_Sub_GetVUIDispInfo(void)
{
...
MS_U32 _HAL_HVD_Sub_GetBBUQNumb(void)
{
    ...
    u32ReadPtr =_HAL_HVD_Sub_GetBBUReadptr();
	//HVD_SUB_MSG_DEG("_HAL_HVD_Sub_GetBBUQNumb:%lx %lx %lx\n" , u32ReadPtr , u32BBUWptr ,(MS_U32)u32BBUEntryNum );
...
MS_U32 _HAL_HVD_Sub_GetPTSQNumb(void)
{
    MS_U32 u32ReadPtr=0;
    MS_U32 eRet=0;
    ...
    {
        HVD_SUB_MSG_ERR("HVD ERR: GetPTSQNumb: PTS table Read Ptr(%lx) > max table size(%lx) \n" ,u32ReadPtr ,(MS_U32)MAX_PTS_TABLE_SIZE);
        return 0;
    }
	//HVD_SUB_MSG_DEG("_HAL_HVD_Sub_GetBBUQNumb:%lx %lx %lx\n" , u32ReadPtr , u32BBUWptr ,(MS_U32)u32BBUEntryNum );
...
void HAL_HVD_Sub_CheckMIUSel(MS_BOOL bChange)
{
    ...
    if( pHVDCtrl_Hal->InitParams.bDynamicScaling )
    {
        if( pHVDCtrl_Hal->MemMap.u32DynSacalingBufAddr >= pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr )
        {
            ...
            {
                HVD_SUB_MSG_ERR("HVD Drv Err: dynamic scaling address(%lx) is at MIU1, but MIU sel is set(VPU qdma WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(13))  );
                if( bChange )
                {
                    // VPU qdma WR
        ...
        else
        {
            if( ((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(13)) == BIT(13))  )
            {
                HVD_SUB_MSG_ERR("HVD Drv Err: dynamic scaling address(%lx) is at MIU0, but MIU sel is set(VPU qdma WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(13))  );
                if( bChange )
                {
                    // VPU qdma WR
                    _HVD_WriteWordMask(MIU0_REG_SEL2 , BIT(13), BIT(13));
    ...
    if( pHVDCtrl_Hal->MemMap.u32CodeBufAddr >= pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr )
    {
        ...
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: code start address(%lx) is at MIU1, but MIU sel is set(VPU d-cache WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(9)));
            if( bChange )
            {
                // VPU d-cache WR
        ...
        if( !((_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(13)) == BIT(13))  )
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: code start address(%lx) is at MIU1, but MIU sel is set(VPU qdma WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(13))  );
            if( bChange )
            {
                // VPU qdma WR
                _HVD_WriteWordMask(MIU0_REG_SEL2 , BIT(13), BIT(13));
        ...
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: code start address(%lx) is at MIU1, but MIU sel is set(VPU i-cache WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL0) & BIT(13)) );
            if( bChange )
            {
                // VPU i-cache WR
    ...
    if( pHVDCtrl_Hal->MemMap.u32FrameBufAddr>= pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr )
    {
        ...
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: Frame Buf address(%lx) is at MIU1, but MIU sel is set(HVD RW) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL0) & BIT(12)));
            if( bChange )
            {
                // HVD RW
        ...
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: Frame Buf address(%lx) is at MIU1, but MIU sel is set(MVD WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL3) & BIT(4)) );
            if( bChange )
            {
                // MVD WR
    ...
    if( pHVDCtrl_Hal->MemMap.u32BitstreamBufAddr>= pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr )
    {
        ...
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: Bitstream Buf address(%lx) is at MIU1, but MIU sel is set(HVD BBU R) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(0)) );
            if( bChange )
            {
                // HVD BBU R
        ...
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: Bitstream Buf address(%lx) is at MIU1, but MIU sel is set(MVD WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr, (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL2) & BIT(6)) );
            if( bChange )
            {
                // MVD WR
        ...
        {
            HVD_SUB_MSG_ERR("HVD Drv Err: Bitstream Buf address(%lx) is at MIU1, but MIU sel is set(MVD BBU WR) to MIU0. (MIU1 base:%lx) (reg:%lx)\n" , pHVDCtrl_Hal->MemMap.u32CodeBufAddr , pHVDCtrl_Hal->MemMap.u32MIU1BaseAddr , (MS_U32)(_HVD_Read2Byte(MIU0_REG_SEL3) & BIT(5)) );
            if( bChange )
            {
                // MVD BBU WR
...
void HAL_HVD_Sub_PowerCtrl(MS_BOOL bEnable)
{
    ...
    case 144:
        ...
        break;
    case 123:
        _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_HVD_123MHZ ,  TOP_CKG_HVD_CLK_MASK);
        break;
    default:
        _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_HVD_160MHZ ,  TOP_CKG_HVD_CLK_MASK);
        break;
...
HVD_Return HAL_HVD_Sub_InitVariables(MS_U32 drvctrl)
{
    ...
    if( ((pHVDCtrl_Hal->InitParams.u32ModeFlag) & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_RM )
    {
        ...
        {
            HVD_SUB_MSG_ERR("HAL_HVD_InitVariables failed: frame buffer size too small. FB:%lx min:%lx\n",
    ...
    {
        HVD_SUB_MSG_DEG("input memory: %lx %lx %lx %lx\n",
    ...
    {
        HVD_SUB_MSG_ERR("HAL_HVD_InitVariables failed: %lx %lx %lx %lx %lx\n",
                    (MS_U32)pHVDShareMem,
...
HVD_Return HAL_HVD_Sub_InitShareMem(void)
{
    ...
    //Chip info
    pHVDShareMem->u16ChipID = E_MSTAR_CHIP_T8;
    pHVDShareMem->u16ChipECONum = 0;

    if (pHVDCtrl_Hal->InitParams.u32ModeFlag & E_HVD_INIT_MAIN_FILE_RAW)
    {
        pHVDShareMem->u8SrcMode = E_HVD_SRC_MODE_FILE;
    }
    else if (pHVDCtrl_Hal->InitParams.u32ModeFlag & E_HVD_INIT_MAIN_FILE_TS)
    {
...
HVD_Return HAL_HVD_Sub_InitRegCPU( void)
{
    ...
    // Init VPU
    {
        VPU_Init_Params VPUInitParams = { E_VPU_CLOCK_160MHZ , FALSE , -1, VPU_DEFAULT_MUTEX_TIMEOUT, FALSE};
        ...
        case 144:
            ...
            break;
        case 108:
            VPUInitParams.eClockSpeed=E_VPU_CLOCK_108MHZ;
            break;
        case 72:
            VPUInitParams.eClockSpeed=E_VPU_CLOCK_72MHZ;
            break;
        default:
            break;
        }
    ...
    HAL_HVD_Sub_MVD_PowerCtrl( TRUE );
    #if 0
    // check MVD power on
    if( _HVD_Read2Byte(REG_TOP_MVD) & (TOP_CKG_MHVD_DIS) )
    {
        HVD_SUB_MSG_INFO( "HVD warning: MVD is not power on before HVD init.\n" );
    ...
    {
        HVD_SUB_MSG_INFO( "HVD warning: VPU is not power on before HVD init.\n" );
    ...
    {
        HVD_SUB_MSG_INFO( "HVD warning: HVD is not power on before HVD init.\n" );
        HAL_HVD_PowerCtrl(TRUE);
        HVD_Delay_ms(1);
    }
    #endif
    bInitRet = _HAL_HVD_Sub_SetRegCPU();
    if( !bInitRet )
    {
        //_HAL_HVD_Return( E_HVD_RETURN_FAIL);
        return E_HVD_RETURN_FAIL;
    }
    bInitRet=HAL_HVD_Sub_RstPTSCtrlVariable();
    if( !bInitRet )
    {
        //_HAL_HVD_Return( E_HVD_RETURN_FAIL);
        return E_HVD_RETURN_FAIL;
...
// Mailbox
    case E_HVD_SDATA_TRIGGER_DISP:    // HVD HI mbox 0
        if( u32Data != 0)
        {
            _HAL_HVD_Sub_MBoxSend( HAL_HVD_REG_DISP_CTL  ,  1 );
        }
        else
        {
            _HAL_HVD_Sub_MBoxSend( HAL_HVD_REG_DISP_CTL  ,  0 );
        }
        break;

    case E_HVD_SDATA_GET_DISP_INFO_DONE:
        _HAL_HVD_Sub_MBoxClear( HAL_HVD_REG_DISP_INFO_CHANGE );
        break;

    case E_HVD_SDATA_GET_DISP_INFO_START:
        _HAL_HVD_Sub_MBoxClear( HAL_HVD_REG_DISP_INFO_COPYED );
        break;
...
// share memory
    ...
    case E_HVD_GDATA_ES_LEVEL:
        u32Ret=(MS_U32)(_HAL_HVD_Sub_GetESLevel());
        break;
    ...
    case E_HVD_GDATA_BBU_Q_NUMB:
        u32Ret=_HAL_HVD_Sub_GetBBUQNumb();
        break;
    ...
    case E_HVD_GDATA_PTS_Q_NUMB:
        u32Ret=_HAL_HVD_Sub_GetPTSQNumb();
        break;
    ...
    case E_HVD_GDATA_AVC_VUI_DISP_INFO:
        u32Ret=_HAL_HVD_Sub_GetVUIDispInfo();
        break;
...
// Mailbox
    ...
    case E_HVD_GDATA_ES_READ_PTR:    //
        u32Ret= _HAL_HVD_Sub_GetESReadPtr(FALSE);
        break;
    case E_HVD_GDATA_ES_WRITE_PTR:   //
        u32Ret= _HAL_HVD_Sub_GetESWritePtr();
        break;
    case E_HVD_GDATA_BBU_READ_PTR:
        u32Ret= _HAL_HVD_Sub_GetBBUReadptr();
        break;
...
// old SVD cmds
    if( (MS_U32)u32Cmd < E_HVD_CMD_SVD_BASE )
    {
        //if( (MS_U32)u32Cmd < E_HVD_CMD_SVD_BASE )
        {
            HVD_SUB_MSG_ERR("HVD Err: Old SVD FW cmd(%lx %lx) used in HVD.\n" , (MS_U32)u32Cmd ,u32CmdArg );
        }
        _HAL_HVD_Return(E_HVD_RETURN_INVALID_PARAMETER);
    }
    HVD_SUB_MSG_DEG("HVD DBG: Send cmd:0x%lx Arg:0x%lx\n", (MS_U32)u32Cmd , (MS_U32)u32CmdArg );
    eRet=_HAL_HVD_Sub_SendCmd( (MS_U32)u32Cmd  , u32CmdArg );
    _HAL_HVD_Return( eRet);
}
...
#endif
    eRet=HAL_HVD_Sub_SetCmd(E_HVD_CMD_PAUSE, 0);
    if (E_HVD_RETURN_SUCCESS != eRet)
        HVD_SUB_MSG_ERR("HVDERR %s(%d) HVD fail to PAUSE %d\n", __FUNCTION__, __LINE__, eRet);

    eRet=HAL_HVD_Sub_SetCmd(E_HVD_CMD_STOP, 0);
...
*/
#if HVD_ENABLE_TIME_MEASURE
    HVD_SUB_MSG_DEG( "HVD Stop Time(Wait FW):%d\n"   ,  HVD_GetSysTime_ms()-ExitTimeCnt );
#endif
    ...
    HAL_VPU_SwRst(); //CPU hold
    {
        ...
        HAL_VPU_DeInit();
        HAL_HVD_Sub_PowerCtrl(FALSE);
        HAL_HVD_Sub_MVD_PowerCtrl( FALSE );
...
HVD_Return HAL_HVD_Sub_PushPacket(HVD_BBU_Info* pInfo  )
{
    ...
    eRet=_HAL_HVD_Sub_UpdatePTSTable(pInfo);
    ...
    //T8: for 128 bit memory. BBU need to get 2 entry at a time.
    eRet=_HAL_HVD_Sub_UpdateESWptr( 0 , 0);
    if( eRet != E_HVD_RETURN_SUCCESS )
    {
        //_HAL_HVD_Return( eRet);
        return eRet;
    ...
    eRet=_HAL_HVD_Sub_UpdateESWptr( u32Addr  , pInfo->u32Length);
    if( eRet  != E_HVD_RETURN_SUCCESS )
    {
        //_HAL_HVD_Return( eRet);
        return eRet;
...
MS_BOOL HAL_HVD_Sub_RstPTSCtrlVariable(void)
{
    if(  (pHVDCtrl_Hal->InitParams.u32ModeFlag & E_HVD_INIT_INPUT_MASK) == E_HVD_INIT_INPUT_DRV  )
    {
        ...
        HVD_SUB_MSG_DEG( "HVD hal:PTS table: WptrAddr:%lx RptrAddr:%lx ByteCnt:%lx PreWptr:%lx\n"  ,  u32PTSWptrAddr  ,  u32PTSRptrAddr ,u32PTSByteCnt ,u32PTSPreWptr );
    }
    return TRUE;
}
...
{
    MS_U32 preWptr = u32BBUWptr;
    if( preWptr == 0 )
    {
        preWptr = u32BBUEntryNum-1;
    }
    else
    {
        preWptr--;
    }
    return (MS_BOOL)(preWptr%2);
}
...
void HAL_HVD_Sub_UpdateESWptr_Fire(void)
{
    ...
    if( bBitMIU1  != bCodeMIU1 )
    {
        ...
        {
            HVD_SUB_MSG_ERR("HVD Err:MDrv_BDMA_MemCopy fail in %s(), ret=%x!\n", __FUNCTION__, bdmaRlt);
        }
    #else
        MS_U32 u32DstAdd=0 , u32SrcAdd=0 , u32tabsize=0;
...
void HAL_HVD_Sub_Dump_FW_Status(void)
{
    ...
    _HAL_HVD_Sub_MBoxRead( HAL_HVD_CMD_MBOX  , &tmp1 );
    _HAL_HVD_Sub_MBoxRead( HAL_HVD_CMD_ARG_MBOX  , &tmp2 );
    HVD_SUB_MSG_DEG("=====HVD Dump Systime:%lu FW Ver:%lx status: %lx Err Code: %lx PC: %lx =====\n" , HVD_GetSysTime_ms() , pHVDShareMem->u32FWVersionID , _HAL_HVD_Sub_GetFWState() , (MS_U32)pHVDShareMem->u16ErrCode , HAL_VPU_GetProgCnt() );

    HVD_SUB_MSG_DEG("Time: STC:%lu DispT:%lu Dec:%lu PTS(skip,seek):%lu;  Last Cmd:%lx _Arg:%lx _Rdy1:%lx _Rdy2:%lx\n" ,
        ...
        tmp1  ,   tmp2   ,  (MS_U32)_HAL_HVD_Sub_MBoxReady(HAL_HVD_CMD_MBOX) , (MS_U32)_HAL_HVD_Sub_MBoxReady(HAL_HVD_CMD_ARG_MBOX) );

    HVD_SUB_MSG_DEG("Flag: InitDone:%d SPS_change:%d IFrm:%d 1stFrmRdy:%d Sync_Start:%d _Reach:%d \n" ,
        pHVDShareMem->bInitDone , _HAL_HVD_Sub_MBoxReady( HAL_HVD_REG_DISP_INFO_CHANGE )  ,  pHVDShareMem->bIsIFrmFound , pHVDShareMem->bIs1stFrameRdy ,
        pHVDShareMem->bIsSyncStart , pHVDShareMem->bIsSyncReach );

    HVD_SUB_MSG_DEG("Queue: BBU:%lu Dec:%d Disp:%d ESR:%lu ESRfromFW:%lu ESW:%lu ESLevel:%lu\n" ,
        _HAL_HVD_Sub_GetBBUQNumb() , pHVDShareMem->u16DecQNumb , pHVDShareMem->u16DispQNumb ,
        _HAL_HVD_Sub_GetESReadPtr(TRUE), pHVDShareMem->u32ESReadPtr, _HAL_HVD_Sub_GetESWritePtr() , _HAL_HVD_Sub_GetESLevel() );

    HVD_SUB_MSG_DEG("Counter: Dec:%lu Disp:%lu Err_Data:%lu _Dec:%lu Skip:%lu Drop:%lu Idle:%lu Main:%lu Vsync:%lu\n" ,
    ...
    HVD_SUB_MSG_DEG("Mode: ShowErr:%d RepLastField:%d SyncOn:%d FileEnd:%d Skip:%d Drop:%d DispSpeed:%d FRC:%d BlueScreen:%d FreezeImg:%d 1Field:%d\n" ,
...
void HAL_HVD_Sub_Dump_BBUs(  MS_U32 u32StartIdx, MS_U32 u32EndIdx, MS_BOOL bShowEmptyEntry )
{
    ...
    do
    {
        ...
        HAL_HVD_Sub_GetBBUEntry( u32CurIdx ,  &u32NalOffset , &u32NalSize   );
        if( (bShowEmptyEntry ==FALSE) ||
             ( bShowEmptyEntry &&  (u32NalOffset ==0) &&  (u32NalSize ==0) ))
        {
            HVD_SUB_MSG_DEG(  "HVD BBU Entry: Idx:%lu Offset:%lx Size:%lx\n",  u32CurIdx  , u32NalOffset   , u32NalSize );
        }
        u32CurIdx++;
