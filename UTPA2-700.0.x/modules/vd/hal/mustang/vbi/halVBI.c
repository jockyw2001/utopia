...
#define R1BYTE(u32Addr, u8mask)            \
    (READ_BYTE (_u32VBIRiuBaseAddr + ((u32Addr) << 1) - ((u32Addr) & 1)) & (u8mask))

#define W1BYTE(u32Addr, u8Val, u8mask)     \
    (WRITE_BYTE(_u32VBIRiuBaseAddr + ((u32Addr) << 1) - ((u32Addr) & 1), (R1BYTE(u32Addr, 0xFF) & ~(u8mask)) | ((u8Val) & (u8mask))))

///////////////////////////////////////////////////////////////
#define MDrv_WriteByte( u32Reg, u8Val )                                                 \
     do {                                                                     \
     (WRITE_BYTE(_u32VBIRiuBaseAddr + ((u32Reg) << 1) - ((u32Reg) & 1), u8Val));    \
          }while(0)

#define MDrv_WriteWord( u32Reg, u16Val )                                                 \
     do {                                                                     \
     (WRITE_WORD(_u32VBIRiuBaseAddr + ((u32Reg) << 1), u16Val));              \
     }while(0)
     
#define MDrv_WriteByteMask( u32Reg, u8Val, u8Mask )                           \
    do {                                                                     \
    (WRITE_BYTE(_u32VBIRiuBaseAddr + ((u32Reg) << 1) - ((u32Reg) & 1), (R1BYTE((u32Reg), 0xFF) & ~(u8Mask)) | ((u8Val) & (u8Mask))));    \
        }while(0)

#define MDrv_ReadByte( u32Reg) (READ_BYTE (_u32VBIRiuBaseAddr + ((u32Reg) << 1) - ((u32Reg) & 1)))
...
//  Local Variables
//-------------------------------------------------------------------------------------------------
static MS_U32 _u32VBIRiuBaseAddr;
...
void VBI_WSSInit(MS_U32 u32Addr)
{
    MS_U8 tmp = 0;
    _u32VBIRiuBaseAddr = u32Addr;
...
MS_BOOL VBI_GetCompleteVPS_Data(MS_U8** pData, MS_U32* dataLen)
{
    if(HAL_VBI_ReadByte(BK_VBI_AF)&BIT(4))
    {
        ...
        _u8VPSData[0x0D - 1] = HAL_VBI_ReadByte(BK_VBI_AD);
        _u8VPSData[0x0E - 1] = HAL_VBI_ReadByte(BK_VBI_AE);
        _u8VPSData[0x05 - 1] = HAL_VBI_ReadByte(BK_VBI_A6);
        _u8VPSData[0x0B - 1] = HAL_VBI_ReadByte(BK_VBI_A7);
...
void VBI_TTX_DMA_CopyPacket(MS_PHYADDR src_addr, MS_PHYADDR dest_addr, MS_U8 aPacketType)
{
    src_addr = src_addr>>3;
...
void VBI_CC_Init(MS_U32 u32RiuAddr, MS_PHYADDR u32Addr, MS_U16 u8Len)
{
    ...
    // VBI RIU base
    _u32VBIRiuBaseAddr = u32RiuAddr;

    // U8 u8Bank; <- remove the bank change code
    if((u32Addr >> 3) >= (1 << CC_BUF_BIT))
    {
        printf("[VBI][%s] ERROR, buffer address out of bound\n", __FUNCTION__);
        //MS_ASSERT(0);
    }
    ...
    HAL_VBI_WriteByte(BK_VBI_1D, u8Tmp); // CcBaseAddr_24, 25, 26, 27
    HAL_VBI_WriteByte(BK_VBI_5D, (u32Addr >> 19) & 0xFF); // CcBaseAddr_23_16 (default value is 0xFF so it needs to specify to 0x00)
    HAL_VBI_WriteByte(BK_VBI_5E, (u32Addr >> 11) & 0xFF); // CcBaseAddr_15_8 (high order)
    HAL_VBI_WriteByte(BK_VBI_5F, (u32Addr >> 3) & 0xFF);  // CCBaseAddr_7_0 (low order)
