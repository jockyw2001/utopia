...
MS_U32 HAL_MsOS_MPool_PA2BA(MS_U32 u32PhyAddr)
{
     MS_U32 u32BusAddr = 0x0;

     // ba = pa + offset
     if( (u32PhyAddr >= HAL_MIU0_BASE) && (u32PhyAddr < HAL_MIU1_BASE) )     // MIU0
                u32BusAddr = u32PhyAddr - HAL_MIU0_BASE + HAL_MIU0_BUS_BASE;
     else if( (u32PhyAddr >= HAL_MIU1_BASE) && (u32PhyAddr < HAL_MIU2_BASE) )        // MIU1
                u32BusAddr = u32PhyAddr - HAL_MIU1_BASE + HAL_MIU1_BUS_BASE;
     else
                u32BusAddr = u32PhyAddr - HAL_MIU2_BASE + HAL_MIU2_BUS_BASE;    // MIU2
...
//for mboot
    ...
    //===========================================
    // MIU0 (0 ~ 1024MB) (VA) 0x2000:0000 ~ 0x3fff:ffff -> (PA) 0x0000:0000 ~ 0x1fff:ffff (cached)
    if ((HAL_MIU0_BUS_BASE <= u32Virt) && ((HAL_MIU0_BUS_BASE + 0x1fffffff) >= u32Virt))
    {
        return (MS_U32)(u32Virt - HAL_MIU0_BUS_BASE);
    }

    //===========================================
    // MIU0 (0 ~ 1024MB) (VA) 0x4000:0000 ~ 0x5fff:ffff -> (PA) 0x0000:0000 ~ 0x1fff:ffff (uncached)
    if (((HAL_MIU0_BUS_BASE + 0x20000000UL) <= u32Virt) && ((HAL_MIU0_BUS_BASE + 0x3fffffff) >= u32Virt))
    {
        return (MS_U32)(u32Virt - (HAL_MIU0_BUS_BASE+0x20000000UL));
    }

    //===========================================
    // MIU1 (low 512MB) (VA) 0x6000:0000 ~ 0x7fff:ffff -> (PA) 0x8000:0000 ~ 0x9fff:ffff (cached)
    if ((HAL_MIU1_BUS_BASE <= u32Virt) && ((HAL_MIU1_BUS_BASE + 0x1fffffffUL) >= u32Virt))
    {
        return (MS_U32)((u32Virt - HAL_MIU1_BUS_BASE) + HAL_MIU1_BASE);
    }

    //===========================================
    // MIU1 (0 ~ 1024MB) (VA) 0x8000:0000 ~ 0x9fff:ffff -> (PA) 0x8000:0000 ~ 0x9fff:ffff (uncached)
    if (((HAL_MIU1_BUS_BASE + 0x20000000UL) <= u32Virt) && ((HAL_MIU1_BUS_BASE + 0x3fffffff) >= u32Virt))
    {
        return (MS_U32)((u32Virt - HAL_MIU1_BUS_BASE - 0x20000000UL) + HAL_MIU1_BASE);
    }
...
//for arm
    ...
    if ((0x00000000UL <= u32Phys) && (0x1fffffffUL >= u32Phys))
    {
        return (MS_U32)((u32Phys & 0x1fffffffUL) + HAL_MIU0_BUS_BASE);
    }

    //===========================================
    // MIU1 (low 512MB) - (PA) 0x8000:0000 ~ 0x9fff:ffff -> (VA) 0xA000:0000 ~ 0xbfff:ffff (cached)
    if ((HAL_MIU1_BASE <= u32Phys) && ((HAL_MIU1_BASE + 0x1fffffffUL) >= u32Phys))
    {
        return (MS_U32)((u32Phys - HAL_MIU1_BASE) + HAL_MIU1_BUS_BASE);
    }
...
//for arm
    ...
    if ((0x00000000UL <= u32Phys) && (0x1fffffffUL >= u32Phys))
    {
        return (MS_U32)((u32Phys & 0x1fffffffUL) + HAL_MIU0_BUS_BASE + 0x20000000UL);
    }
    ...
    if ((HAL_MIU1_BASE <= u32Phys) && ((HAL_MIU1_BASE + 0x1fffffffUL) >= u32Phys))
    {
        return (MS_U32)((u32Phys - HAL_MIU1_BASE) + HAL_MIU1_BUS_BASE + 0x20000000UL);
    }
